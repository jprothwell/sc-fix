...
INT32 FS_Format(PCSTR pszDevName, UINT8 iFsType, UINT8 iFlag)
{
    ...
    case FS_TYPE_FAT:
        ...
        {
            D( ( DL_ERROR, "Device[%s] is not permitted to access for FS",pszDevName) );
...
INT32 FS_FindFirstFile( PCSTR pszFileName, PFS_FIND_DATA  pFindData )
{
    ...
    iULen = (UINT16)DSM_TcStrLen((PCWSTR)pszFileName);    
...
label_exit:
    FS_Up();
    //CSW_TRACE(BASE_DSM_TS_ID, "read hd = 0x%x,size = 0x%x,buff:",fd,iLen);
...
INT32 FS_MakeDir(PCSTR pszDirName, UINT32 iMode)
{
    ...
    if(!path)
    {
        ...
        //DSM_Dump((UINT8*)pszDirName,iULen,16);
        DSM_ASSERT(0,"path is null.pszDirName = 0x%x",pszDirName);
        return ERR_FS_NO_MORE_MEMORY;
    }
    ...
    if(!iRet)
    {
        D( ( DL_FATERROR, "in FS_MakeDir,find_entry error, iRet = %d\n",iRet) );
        //DSM_Dump((UINT8*)pszDirName,iULen,16);
        iErrCode = ERR_FS_FILE_EXIST;
        goto step1_failed;
    }
...
INT32 FS_RemoveDir(PCSTR pszDirName)
{
    ...
    */
    ...
    {
        D( ( DL_FATERROR, "in FS_RemoveDir, psDe->inode->i_nlinks = %d\n",psDe->inode->i_nlinks) );
        iErrCode = ERR_FS_DIR_NOT_EMPTY;
        goto step2_failed;
    }*/
...
INT32 FS_PowerOn(VOID)
{
    ...
    if(ERR_SUCCESS == iResult)
    {
        ...
        if(ERR_SUCCESS == iResult)
        {
            ...
            {          
                ...
                {
                    if(g_pFsDevInfo[i].dev_type != FS_DEV_TYPE_FLASH || g_pFsDevInfo[i].is_root != TRUE) 
                    {
                        continue;
                    }
        ...
        {
            D( ( DL_FATERROR, "in FS_PowerOn, REG_GetValue Error, iResult = %d\n",iResult) );
            iRet = ERR_FS_ACCESS_REG_FAILED;
        }
        REG_CloseKey(hKey);
    }
    ...
    {
        D( ( DL_FATERROR, "in FS_PowerOn, REG_OpenKey Error, iResult = %d\n",iResult) );
        iRet = ERR_FS_ACCESS_REG_FAILED;
    }
...
INT32 FS_PowerOff(VOID)
{
    ...
    if(ERR_SUCCESS == iResult)
    {
        ...
        if(ERR_SUCCESS != iResult)
        {
            D( ( DL_FATERROR, "in FS_PowerOff, REG_SetValue Error, iResult = %d\n",iResult) );
            iRet = ERR_FS_ACCESS_REG_FAILED;         
        }
        REG_CloseKey(hKey);
    }
    ...
    {
        D( ( DL_FATERROR, "in FS_PowerOff, REG_OpenKey Error, iResult = %d\n",iResult) );
        iRet = ERR_FS_ACCESS_REG_FAILED;
    }
