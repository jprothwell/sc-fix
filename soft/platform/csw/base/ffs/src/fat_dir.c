...
INT32 fat_trunc_fatentry(FAT_SB_INFO *sb_info, UINT32 lFstCluLO)
{
    ...
    {
        D((DL_FATERROR, "fat_trunc_fatentry: lFstCluLO = 0x%x, _ERR_FAT_PARAM_ERROR!\n", lFstCluLO));
    ...
    do
    {
        ...
        {
            D((DL_FATERROR, "fat_trunc_fatentry: fat_get_next_cluster() return %d", lErrCode));
            return lErrCode;
        }
	   if(nextClu == 0)
        {
            D((DL_FATERROR, "fat_trunc_fatentry: nextClu is zero."));
            bSynchFAT = TRUE;           	
     	   }
        ...
        {
            D( ( DL_FATERROR,"in fat_trunc_fatentry,fat_set_fat_entry_value failed, ErrorCode = %d \n",lErrCode)); 
            return lErrCode;
        }
	 ...
	 if(DSM_GetDevType(sb_info->sb->s_dev) == DSM_MEM_DEV_FLASH)
 	 {
 	       for(i = 0;  i < sb_info->iSecPerClus; i++)
       	{
			...
			{
				D( ( DL_FATERROR,"in fat_trunc_fatentry,fat_revert_cluster failed, err_code = %d \n",lErrCode)); 
				return lErrCode;
			}
       	}		
	 }
...
delete_entry:
    ...
    if(!bRename && (de->DIR_FstClusLO != CLUSTER_FREE || de->DIR_FstClusHI != CLUSTER_FREE))
    {
        ...
        if(_ERR_FAT_SUCCESS != lErrCode)
        {
            D( ( DL_FATERROR,"In  fat_remove_entry,fat_trunc_fatentry error, ErrorCode = %d \n",lErrCode));
            goto end;
        }
        ...
        if(_ERR_FAT_SUCCESS != lErrCode)
        {
            D( ( DL_FATERROR,"In  fat_remove_entry,fat_set_fat_entry_value error, ErrorCode = %d \n",lErrCode));
            goto end;
        }*/
    }
end:    
            ...
            UINT32* bclu, UINT16 *bsec, UINT16 *eoff, WCHAR* pszFileName, BOOL bRename)
{
    ...
    DSM_ASSERT(curclu2 >= 2,"In  fat_remove_entry,custer number is invalid: 0x%x.",bclu);
    ...
    if((UINT16)(ecount*sizeof(FAT_DIR_ENTRY)) > sb->iSecPerClus*sb->iBytesPerSec+leftspace)
    {
        ...
        DSM_ASSERT(curclu2 >= 2,"curclu2(%d) < 2,preclu2 = %d.",curclu2,preclu2);
        // Set the FAT table to 0xFFFF for the new-cluster.
        iRet = fat_set_fat_entry_value(sb, curclu2, CLUSTER_NULL, TRUE, NULL);
        if(_ERR_FAT_SUCCESS != iRet)
        {
...
free_entry_found:
    if((de->DIR_Attr&ATTR_DIR) && !bRename)
    {
        ...
        if(iRet < 0 || curclu2 < 2)
        {
  	    ...
  	    DSM_ASSERT(curclu2 >= 2,"In  fat_add_entry,fat_get_free_cluster error, ErrorCode = %d.", iRet); 	
    ...
    *eoff = freebegoff;
    ...
    if((de->DIR_Attr&ATTR_DIR) && !bRename)
    {
        ...
        DSM_MemCpy(dent.DIR_Name, "..         ",11);
        dent.DIR_Attr |= ATTR_DIR;
        if ((unsigned int)FS_ROOT_INO == diri->i_ino)
        {// 用簇号0表示上级目录为根目录。
...
free_entry_found:
    if((de->DIR_Attr&ATTR_DIR) && !bRename)
    {
        ...
        DSM_ASSERT(iFreeClu >= 2," in fat_add_root_entry,fat_get_free_cluster error ErrorCode = %d.",iRet);
        ...
        if (sb_info->iFreeCnterInitFlag)
        {
            D((DL_FATWARNING, " in fat_add_root_entry,fsb_info->iFreeCluCnt = %d\n",sb_info->iFreeCluCnt));
            ...
            {
                 D((DL_FATERROR, " in fat_add_root_entry,fsb_info->iFreeCluCnt = %d!\n",sb_info->iFreeCluCnt));
             }
        }
    }
