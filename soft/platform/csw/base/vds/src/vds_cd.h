...
#define VDS_NR_PB_PER_SECT(part_pointer) \
	( (part_pointer->sPartInfo.iFlashSectorSize - VDS_SZ_SH - VDS_SZ_PB(part_pointer)) \
	/ (VDS_SZ_PBD + part_pointer->sPartInfo.iVBSize))
...
#define VDS_ADDR_PB(part_pointer, sect_no) \
	(((VDS_ADDR_PBDT(part_pointer, sect_no) + VDS_SZ_PBDT(part_pointer) + VDS_SZ_PB(part_pointer))/VDS_SZ_PB(part_pointer))*VDS_SZ_PB(part_pointer))
...
#define VDS_CACHE_ERASE_TIME_MAX                    50
