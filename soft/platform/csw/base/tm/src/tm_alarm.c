...
/*
*Description:
*  reset the sysytem,bake system time,bake RTC time,clear
*   把参数中的时间存入FLASH,然后RTC从零记时
*RTC time .
*Parameter:
*  none
*Return:
...
*/
BOOL TM_ResetRTC(UINT32 sysTime)
{



   TM_PARAMETERS TM_para,paraBak;

    CSW_PROFILE_FUNCTION_ENTER(TM_ResetRTC);

	TIM_GetTimePara(&TM_para);
    paraBak = TM_para;
	TM_para.iBase = sysTime - RTC_reset_value / TIM_CLOCKS_PER_SECOND;//yyj 070626
    TIM_SetTimePara(&TM_para);

    CSW_TRACE(DEBUG_PRINT_SWITCH,"TM_ResetRTC() sysTime = 0x%x!",sysTime);


     //need set systime to flash
#ifdef TIM_REG_FFS_USED
    if(!TIM_WriteRegData("TM_BASE", &TIM_Glo_Infor_Buf[TIM_TIME_BASE_POS],TIM_TIME_FORMAT_OR_BASE_LEN))
    {
        TIM_SetTimePara(&paraBak);

        CSW_PROFILE_FUNCTION_EXIT(TM_ResetRTC);
        return FALSE;
    }
#else
    if(!TIM_WriteFlash())
    {
        TIM_SetTimePara(&paraBak);
        CSW_TRACE(DEBUG_PRINT_SWITCH,"TM_ResetRTC()==FLASH ERR==\n");
        CSW_PROFILE_FUNCTION_EXIT(TM_ResetRTC);
        TIM_ERR_RETURN(TM_WRITE_FLASH_ERROR);
    }
#endif

    hal_timRTC_SetTime(RTC_reset_value);// add 2006.8.25 chenwf//yyj 070626 changed
    CSW_TRACE(DEBUG_PRINT_SWITCH,"TM_ResetRTC()==OK==\n");
    CSW_PROFILE_FUNCTION_EXIT(TM_ResetRTC);
...
PRIVATE BOOL TIM_SetHalAlmRTC(UINT8 idx, UINT32 time)
{
    ...
    //得到当前时间
    TIM_GetTimePara(&TM_para);
    RtcTime  = hal_timRTC_GetTime()/TIM_CLOCKS_PER_SECOND;
    nowSecod = TM_para.iBase+RtcTime;

    if(idx != 0 && time != 0
          && ((time - nowSecod > 0)
          &&  (time - nowSecod  + RtcTime < RTC_ALARM_TIMERSET_LIMIT)))
    {

        hal_timRTC_SetAlarm((time - nowSecod  + RtcTime)*TIM_CLOCKS_PER_SECOND);//set the alarm
    }
    else//190天的自定义时间
    {
        almLimit = (UINT32)(RTC_ALARM_TIMERSET_LIMIT);
        hal_timRTC_SetAlarm((UINT32)(almLimit * TIM_CLOCKS_PER_SECOND));
    }
...
#if (CHIP_ASIC_ID == CHIP_ASIC_ID_JADE)
        rtcTime = hal_timRTC_GetTime();//得到当前RTC时间 070613 YYJ

        if(rtcTime/TIM_CLOCKS_PER_SECOND >= RTC_ALARM_TIMERSET_LIMIT)//070613 YYJ
        {
            if(!TM_ResetRTC((UINT32)(savedSec+RTC_ALARM_TIMERSET_LIMIT)))//继续设自定义闹钟
	    	{
	            CSW_PROFILE_FUNCTION_EXIT(dm_AlarmDisposeInEvt);
	            return FALSE;
	    	}
...
#if (CHIP_ASIC_ID == CHIP_ASIC_ID_JADE)
        rtcTime = hal_timRTC_GetTime();//得到当前RTC时间
        if(rtcTime/TIM_CLOCKS_PER_SECOND >= RTC_ALARM_TIMERSET_LIMIT)
        {
            //如果闹钟响时正好自定义闹钟也到时了,重置RTC,虽然这种情况很难发生,但在这里做了保护
            if(!TM_ResetRTC((UINT32)(savedSec+rtcTime/TIM_CLOCKS_PER_SECOND)))
        	{
                CSW_PROFILE_FUNCTION_EXIT(dm_AlarmDisposeInEvt);
                return FALSE;
        	}
...
BOOL TM_PowerOn()
{
...
#if (CHIP_ASIC_ID == CHIP_ASIC_ID_JADE)
	if ((rtcProgram = hal_timRtcNotProgrammed())||(RTC_reset_value > hal_timRTC_GetTime()))//判断RTC是否曾掉电//yyj 070626 changed
	{
        //send message
        //* pRtcLostPower = (1<<1);
        dm_SendArlarmMessage(&ev);//告诉MMI重设时间
        CSW_TRACE(DEBUG_PRINT_SWITCH,"TM_PowerOn()==SET THE TIME PLEASE==\n");
		//hal_timPower(HAL_SHUTDOWN);
	}
	CSW_TRACE(DEBUG_PRINT_SWITCH, "RTC TIME NOW IS ========0x%x",hal_timRTC_GetTime());
#else
    ...
    if((rtcProgram = hal_TimRtcNotProgrammed()))
    {
        ...
        RtcTime.day = 1;
        RtcTime.month = 1;
        RtcTime.year = 11; //Change default year is 2011.
        RtcTime.wDay = 6; // It's a Saturday.
...
#if (CHIP_ASIC_ID == CHIP_ASIC_ID_JADE)
    hal_timRTC_SetTime(timeToset*TIM_CLOCKS_PER_SECOND);
#else
    HAL_TIM_RTC_TIME_T RtcTime;
    TM_SYSTEMTIME SystemTime;
