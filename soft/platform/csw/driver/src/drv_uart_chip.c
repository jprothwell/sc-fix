...
{
#if (CHIP_ASIC_ID == CHIP_ASIC_ID_JADE)
	hal_SetUsrVectorMsk(0+9, HAL_UART_MASK_NIL );

	CSW_TRACE(CSW_DRV_TS_ID, TSTXT("uart0_api_callback\n")  );
	
	if(NULL != uart_status[0].callback )
	{
		(*uart_status[0].callback)(status);
	}
...
#if (CHIP_ASIC_ID == CHIP_ASIC_ID_JADE)
void uart1_api_callback(UINT16 status)
{
	hal_SetUsrVectorMsk(1+9, HAL_UART_MASK_NIL );
	CSW_TRACE(CSW_DRV_TS_ID, TSTXT("uart1_api_callback \n")  );
	
	if(NULL != uart_status[1].callback )
	{
		(*uart_status[1].callback)(status);
	}
...
#if (CHIP_ASIC_ID == CHIP_ASIC_ID_JADE)
void uart2_api_callback(UINT16 status)
{
	hal_SetUsrVectorMsk(2+9, HAL_UART_MASK_NIL );
	CSW_TRACE(CSW_DRV_TS_ID, TSTXT("uart1_api_callback \n")  );
	
	if(NULL != uart_status[2].callback )
	{
		(*uart_status[2].callback)(status);
	}
	...
	switch(uart_status[UartID].mode)			
	{
		case DRV_UART_TRANSFERT_MODE_DIRECT_IRQ:
		{
			switch(UartID)
			{
				...
				{
#if (CHIP_ASIC_ID == CHIP_ASIC_ID_JADE)
					hal_SetUsrVector(UartID+9, uart0_api_callback);       //register interrupt function
					hal_SetUsrVectorMsk(UartID+9, HAL_UART_MASK_TX );
#else
                    // TODO error
#endif
				}
				...
				{
#if (CHIP_ASIC_ID == CHIP_ASIC_ID_JADE)
					hal_SetUsrVector(UartID+9, uart1_api_callback);       //register interrupt function
					hal_SetUsrVectorMsk(UartID+9, HAL_UART_MASK_TX );
#else
                    HAL_UART_IRQ_STATUS_T mask =
                    {
				...
				{
#if (CHIP_ASIC_ID == CHIP_ASIC_ID_JADE)
					hal_SetUsrVector(UartID+9, uart2_api_callback);       //register interrupt function
					hal_SetUsrVectorMsk(UartID+9, HAL_UART_MASK_TX );
#else
                    HAL_UART_IRQ_STATUS_T mask =
                    {
...
#if (CHIP_ASIC_ID == CHIP_ASIC_ID_JADE)
					hal_SetUsrVector(UartID+9, uart0_api_callback);       //register interrupt function
					hal_SetUsrVectorMsk(UartID+9, HAL_UART_MASK_DMA_TX );
#else
                    // TODO Error
#endif
				}
		...
		{
#if (CHIP_ASIC_ID == CHIP_ASIC_ID_JADE)
			hal_SetUsrVectorMsk(UartID+9, HAL_UART_MASK_NIL );
#else
            HAL_UART_IRQ_STATUS_T mask =
                    {
                        .txModemStatus      = 0,
...
#if (CHIP_ASIC_ID == CHIP_ASIC_ID_JADE)
					hal_SetUsrVector(UartID+9, uart0_api_callback);       //register interrupt function
					hal_SetUsrVectorMsk(UartID+9, HAL_UART_MASK_RX );
#else
                    // TODO Error
#endif
				}
...
#if (CHIP_ASIC_ID == CHIP_ASIC_ID_JADE)
					hal_SetUsrVector(UartID+9, uart0_api_callback);       //register interrupt function
					hal_SetUsrVectorMsk(UartID+9, HAL_UART_MASK_DMA_RX );
#else
                    // TODO Error            
#endif
				}
