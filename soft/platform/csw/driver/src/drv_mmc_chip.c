...
#else
            result = mcd_Open(&(tflash_dev->mcd_csd),mcdVer);
#endif
...
#ifdef CSW_DRV_MMC_BLOCK_ACCESS
            result = mcd_Write(tflash_dev->w_cache.uFstBlkNo, tflash_dev->w_cache.pBuff, tflash_dev->w_cache.uSize/MMC_DEF_SEC_SIZE);
#else
            result = mcd_Write(tflash_dev->w_cache.uFstBlkNo*MMC_DEF_SEC_SIZE, tflash_dev->w_cache.pBuff, tflash_dev->w_cache.uSize);
#endif  
            if (result != MCD_ERR_NO)
            {
...
{    
    ...
    if(NULL != tflash_dev->w_cache.pBuff            &&
       ...
       blk_nr <   tflash_dev->w_cache.uFstBlkNo + tflash_dev->w_cache.uSize/MMC_DEF_SEC_SIZE 
      )
    {           
        memcpy(pBuffer,tflash_dev->w_cache.pBuff + (blk_nr - tflash_dev->w_cache.uFstBlkNo)* MMC_DEF_SEC_SIZE,MMC_DEF_SEC_SIZE); 
        //CSW_TRACE(BASE_FFS_TS_ID, "cache:DRV_ReadTFlash(%d) read form w_cache, fst_blk_no = 0x%x.\n",blk_nr,g_WCacheInfo.uFstBlkNo);
        goto func_end;            
    }
    ...
    if(NULL != tflash_dev->r_cache.pBuff            &&
       ...
       blk_nr <   tflash_dev->r_cache.uFstBlkNo + tflash_dev->r_cache.uSize/MMC_DEF_SEC_SIZE
      )
    {
        memcpy(pBuffer,tflash_dev->r_cache.pBuff  + (blk_nr - tflash_dev->r_cache.uFstBlkNo)* MMC_DEF_SEC_SIZE,MMC_DEF_SEC_SIZE); 
        // CSW_TRACE(BASE_FFS_TS_ID, "cache:DRV_ReadTFlash(%d) read form r_cache, fst_blk_no = %d.\n",blk_nr,g_RCacheInfo.uFstBlkNo);
        goto func_end;             
    }
    ...
    { 
#ifdef CSW_DRV_MMC_BLOCK_ACCESS
        result = mcd_Read(blk_nr, tflash_dev->r_cache.pBuff, tflash_dev->r_cache.uSize/MMC_DEF_SEC_SIZE);
#else
        result = mcd_Read(blk_nr*MMC_DEF_SEC_SIZE, tflash_dev->r_cache.pBuff, tflash_dev->r_cache.uSize);
#endif  
        if (result != MCD_ERR_NO )
        {            
            try_times ++;
            ...
            CSW_TRACE(BASE_FFS_TS_ID, "DRV_ReadMMC:call mcd_Read(0x%x) returns error(%d)!try times = %d.",blk_nr, result,try_times);
        ...
        {
            tflash_dev->r_cache.uFstBlkNo = blk_nr;
...
INT32 DRV_WriteTFlash (HANDLE dev_handle, UINT32 blk_nr, CONST BYTE* pBuffer)
{
   ...
   {
       memcpy(tflash_dev->r_cache.pBuff + (blk_nr -tflash_dev->r_cache.uFstBlkNo) * MMC_DEF_SEC_SIZE,pBuffer,MMC_DEF_SEC_SIZE);       
   ...
   if(NULL !=  tflash_dev->w_cache.pBuff           &&
      ...
      blk_nr < tflash_dev->w_cache.uFstBlkNo + tflash_dev->w_cache.uSize/MMC_DEF_SEC_SIZE)   
   {
       memcpy(tflash_dev->w_cache.pBuff + (blk_nr -tflash_dev->w_cache.uFstBlkNo) * MMC_DEF_SEC_SIZE,pBuffer,MMC_DEF_SEC_SIZE);       
   ...
   {   
        ...
        {        
            ...
            { 
                ...
                {   
                    tflash_dev->w_cache.uSign = DRV_CACHE_CLEAN;
                    // CSW_TRACE(BASE_FFS_TS_ID, "DRV_WriteMMC:called mcd_Write() ok. blk_nr = 0x%x,write_sz = 0x%x.",g_WCacheInfo.uFstBlkNo,g_WCacheInfo.uSize);         
                    break;
                }     
            }while(try_times < TFLASH_TRYING_TIMES);     
        }
        ...
        {
#ifdef CSW_DRV_MMC_BLOCK_ACCESS
            result = mcd_Read(blk_nr, tflash_dev->w_cache.pBuff, tflash_dev->w_cache.uSize/MMC_DEF_SEC_SIZE);
#else
            result = mcd_Read(blk_nr*MMC_DEF_SEC_SIZE, tflash_dev->w_cache.pBuff, tflash_dev->w_cache.uSize);
#endif    
            if (result != MCD_ERR_NO)
            {
                try_times ++;
                CSW_TRACE(BASE_FFS_TS_ID, "DRV_WriteTFlash:call mcd_Read(0x%x) returns error(%d)!try times = %d.",blk_nr, result,try_times);
                if(try_times == TFLASH_TRYING_TIMES)
                {
                    ret = result;
                    goto _func_end;
            ...
            {               
                ...
                tflash_dev->w_cache.uFstBlkNo = blk_nr;
...
#endif   
    mcd_GetCardSize(&m_cardsize);

    *pBlockNr = m_cardsize.nbBlock*(m_cardsize.blockLen/MMC_DEF_SEC_SIZE);
    *pBlockSize = MMC_DEF_SEC_SIZE;
