...
// from bat_prv.h
#define CHARGING_ADAPTER_NO_CONNECTED           0
#define CHARGING_ADAPTER_IS_CONNECTED           1
#define CHARGING_ADAPTER_CONNECTED_AND_CHRGING  2
#define CHARGING_ADAPTER_CONNECTED_AND_FINISHED 3
#define CHARGING_ERROR_OR_INTERRUPTED           4
#define CHARGING_ERROR_FOR_FALSE_TEMPERATURE    5

extern VOID sxr_Sleep (UINT32 Period);

// @todo probably move this to some tgt config, this is battery dependant
// anyway it's unused by mmi
// time in second ?
//#define BATT_FULL_TIME  0x13c680 //NEED TO ENSURE
...
#else
#ifdef PM_INCREASE_BATTERY_DETECT
...
/// delay between earpiece detection during IDLE but active
#define PM_EARPIECE_DET_IDLE_TIME        1 * HAL_TICK1S
/// delay between earpiece detection during CALL
#define PM_EARPIECE_DET_CALL_TIME        1 * HAL_TICK1S / 10

#define PM_EARPIECE_DET_MAX_TIME        0xFFFFFFFF
/// delay after earpiece plugin when using interrupt mode
#define PM_EARPIECE_DET_INTER_TIME         1 * HAL_TICK1S / 10
#define PM_MAX_DET_IDLE_TIME    0xFFFFFFFF
// to filt the error voltage 
#define PM_EARPIECE_DET_DEBOUNCE_TIME    1 * HAL_TICK1S / 20
...
typedef struct
{
    UINT16 levels[PM_AVERAGE_EAR_NB];
    /// current index in the array
    UINT8 idx;
    UINT16 count;
    UINT32 sum;
...
{
      .levels = { [0 ... PM_AVERAGE_EAR_NB-1] = 0 },
      .idx = 0,
      .count = 0,
      .sum = 0,
      .lastvolt = 0,
...
PRIVATE UINT32 g_pmEarPieceDetTime     = PM_EARPIECE_DET_IDLE_TIME;
...
PRIVATE BOOL g_pmVMICActive = TRUE;
#endif
...
PRIVATE UINT8 pm_PmdChargerStatus2Bcs(PMD_CHARGER_STATUS_T status)
{
        ...
        && (status > PMD_CHARGER_UNPLUGGED) )
    {
        ...
        return CHARGING_ADAPTER_IS_CONNECTED;
    }
    ...
    switch (status)
    {
        ...
        case PMD_CHARGER_UNPLUGGED:
            return CHARGING_ADAPTER_NO_CONNECTED;
        ...
        case PMD_CHARGER_PULSED_CHARGE_WAIT:
            return CHARGING_ADAPTER_CONNECTED_AND_CHRGING;
        case PMD_CHARGER_FULL_CHARGE:
            return CHARGING_ADAPTER_CONNECTED_AND_FINISHED;
        case PMD_CHARGER_ERROR:
        case PMD_CHARGER_TIMED_OUT:
            return CHARGING_ERROR_OR_INTERRUPTED;
        case PMD_CHARGER_TEMPERATURE_ERROR:
            return CHARGING_ERROR_FOR_FALSE_TEMPERATURE;
        default:
            return CHARGING_ERROR_OR_INTERRUPTED;
    }
}
...
PRIVATE INT pm_GetStableBatteryLevel(UINT8 * pPercent)
{
    ...
    UINT16 batLevel = pmd_GetBatteryLevel(&bcl);
 ...
 BOOL pm_BattChargeRemind(PMD_CHARGER_STATUS_T status)
{
    ...
    if (bcs == CHARGING_ADAPTER_NO_CONNECTED)
    {
    ...
    // when charge full : bcl = BATTERY_PERCENT_LEVEL_100
    // also force histeresys to 100%
    ...
    {
      if (CHARGING_ADAPTER_NO_CONNECTED == bcs)
      {
...
{
    if ((PMD_CHARGER_OPAL2_FULL == status)||(PMD_CHARGER_OPAL2_FAST == status))
    {
        if (PMD_CHARGER_OPAL2_FULL == status)
        {
          pm_BattChargeRemind(PMD_CHARGER_FULL_CHARGE);
        }
        else
        {
          pm_BattChargeRemind(PMD_CHARGER_FAST_CHARGE);
        }
...
extern BOOL pmd_Gpadc_stable(VOID);

BOOL pm_flit_ear_detect(UINT16 level)
{
    return TRUE;
#if 0
    CSW_TRACE(CFW_PM_TS_ID, "pm_flit_ear_detect level =%dmv,cycle %d",level,16384/g_pmEarPieceDetTime);
    UINT8 i = 0;
    ...
    if (g_pmEarDetect.levels[1] == 0)
    {
      for (i =0;i<PM_AVERAGE_EAR_NB;i++)
      {
        g_pmEarDetect.levels[i] = level;
        g_pmEarDetect.sum += level;
      }
      g_pmEarDetect.idx = PM_AVERAGE_EAR_NB -1;
      return TRUE;
    }
    
    g_pmEarDetect.sum -= g_pmEarDetect.levels[g_pmEarDetect.idx];
    g_pmEarDetect.sum += level;
    g_pmEarDetect.levels[g_pmEarDetect.idx] = level;
    g_pmEarDetect.idx++;
    if (g_pmEarDetect.idx >= PM_AVERAGE_EAR_NB)
    {
        g_pmEarDetect.idx = 0;
    }

    CSW_TRACE(CFW_PM_TS_ID, "pm: ear sum= %d,idx = %d,level =%dmv,",g_pmEarDetect.sum,g_pmEarDetect.idx,level);
    if(level >= g_pmEarDetect.sum /PM_AVERAGE_EAR_NB )
    {
       if ((level - g_pmEarDetect.sum /PM_AVERAGE_EAR_NB) > PM_EARPIECE_DET_DEBOUNCE_VALUE)
       {  
       ...
       {
           g_pmEarPieceDetTime = PM_EARPIECE_DET_INTER_TIME;
          return TRUE;
       }
       CSW_TRACE(CFW_PM_TS_ID, "pm: -diff %d",level - g_pmEarDetect.sum /PM_AVERAGE_EAR_NB);
    }
    else
    {
       if ((g_pmEarDetect.sum /PM_AVERAGE_EAR_NB - level) > PM_EARPIECE_DET_DEBOUNCE_VALUE)
       {  
           g_pmEarPieceDetTime = PM_EARPIECE_DET_DEBOUNCE_TIME;
           return FALSE;
       }
       ...
       CSW_TRACE(CFW_PM_TS_ID, "pm: diff- %d",g_pmEarDetect.sum /PM_AVERAGE_EAR_NB - level);
    }
...
PRIVATE VOID pm_EarPieceDetect(BOOL fromLowPowerEvent)
{
    ...
    // 0: VMIC always on, direct measure
    {
        ...
        if (ret != HAL_ANA_GPADC_BAD_VALUE) {
           ...
           if ((pm_flit_ear_detect(ret) == TRUE)&&(pmd_Gpadc_stable()==TRUE))
           {
...
PRIVATE VOID pm_doMonitoring(BOOL fromLowPowerEvent)
{
    ...
    if ((now - g_pmEarPieceDetLastDate) >= g_pmEarPieceDetTime)
    {
    ...
    {   // called from lp event, assuming not in call then ;)
        callStatus = 0x00 /*CC_STATE_NULL*/;
    }
    else
    {
#ifdef CFW_MULTI_SIM
        UINT8 nTmp = CFW_SIM_0;
        callStatus = 0x00;

        for( ;nTmp < CFW_SIM_COUNT; nTmp++)

        {
            callStatus |= CFW_CcGetCallStatus(nTmp) /*CC_STATE_NULL*/;
        }
#else
    ...
    {   // not in call
        ...
        if (g_pmEarDetect.mode == PMD_EAR_DETECT_POLLING)
        {
            if (g_pmEarPieceDetTime != PM_EARPIECE_DET_DEBOUNCE_TIME)
            g_pmEarPieceDetTime = PM_EARPIECE_DET_IDLE_TIME;
        }
        else
        {
            if (g_pmEarDetect.earon == FALSE) //earphone plug out
            {
...
// from pm_mmi.c
// The following functions are called by coolmmi/adaptation


BOOL DM_StartGpadcDetect(UINT16 nMillisecondes)
{
    return FALSE;
}

BOOL DM_StopGpadcDetect(VOID)
{
    return FALSE;
}
...
BOOL PM_GetBatteryInfo(UINT8* pBcs, UINT8* pBcl, UINT16* pMpc)
{
    ...
    // 5: OVERBATTEMP
    {
        ...
        {
            *pBcs = CHARGING_ADAPTER_IS_CONNECTED;
        }
        else
        {
            *pBcs = CHARGING_ADAPTER_NO_CONNECTED;
        }
...
VOID pm_ear_detect_inter(BOOL ear_status)
{
  CSW_TRACE(CFW_PM_TS_ID, "pm: ear int = %d", ear_status); 
  COS_EVENT ev;
  g_pmEarDetect.earon = ear_status;
  ev.nEventId = EV_DM_GPADC_IND;
  if (ear_status == TRUE)
  {
      ev.nParam1  = 2000;
  ...
  if (ear_status == TRUE)
  {
    ...
    // start monitoring right away -- earpiece key detection
  UINT8 mbx = ((TASK_HANDLE*)BAL_TH(BAL_DEV_MONITOR_TASK_PRIORITY))->nMailBoxId;
...
/*         UINT16 batState = 0; */
    ...
    {
        g_pmEarDetect.earon = pmd_GetEarModeStatus();
        if (g_pmEarDetect.earon == 0) // plug in
        {
            pm_ear_detect_inter(TRUE);
            g_pmEarDetect.earon = 1;
        }
        else
        {
          g_pmEarDetect.earon = 0;
        }
...
UINT16 PM_GetGpadcValue(PM_GPADC_CHAN_T channel)
{
    ...
    CONST UINT32 vmicChannel = pmd_GetEarpieceGpadcChannel();
    UINT16 offset = 0;
    // skip earpiece and battery measurement channels
    if (channel != vmicChannel && channel != vbatChannel)
    {
        hal_AnaGpadcOpen(channel, HAL_ANA_GPADC_ATP_1MS);
    }
    if (g_pmEarDetect.mode != PMD_EAR_DETECT_POLLING)
    {
        if (g_pmEarDetect.earon == 0)
        {
    ...
    UINT16 ret  = hal_AnaGpadcGet(channel);
    if (ret == 0xFFFF)
    {
        COS_Sleep(5);
    ...
    // battery channel
    if (vbatChannel == channel)
    {
      offset = pmd_GetBackLightOffsetVolt();
      ret = ret*3 + offset;
    }
    CSW_TRACE(CFW_PM_TS_ID, "pm: PM_GetGpadcValue*2 = %dmv,offset = %dmv channel =%d", ret*2,offset,channel);
    return ret;
}
