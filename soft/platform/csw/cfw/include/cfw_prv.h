...
/*                                                                            */
/* Description:                                                               */
/*   cfw private procedure declarations, constant definitions and macros.     */
/*                                                                            */
/******************************************************************************/
...
UINT32 SRVAPI CFW_PostNotifyEvent (
    UINT32 nEventId,
    UINT32 nParam1,
    UINT32 nParam2,
    UINT32 nUTI,
...
UINT32 SRVAPI CFW_SendSclMessage (UINT32 nMessageId,VOID* pMessasgeStruct);
#else
...
#define  CFW_AO_PROC_CODE_SINGLE            1 // the event are processed one time, and don't need to process again in this state.
#define  CFW_AO_PROC_CODE_MULTIPLE          2 // continue process AO proc.
#define  CFW_AO_PROC_CODE_CONTINUE          2 // continue process AO proc.
#define  CFW_AO_PROC_CODE_CONSUMED          4 // consume the current event.
#define  CFW_AO_PROC_CODE_NOT_CONSUMED      8 // don't consume the current event.
...
#ifdef CFW_MULTI_SIM
UINT32 SRVAPI CFW_SimGetSmsParameters (
									   UINT8 nIndex,
									   UINT16 nUTI,
									   CFW_SIM_ID nSimID
									   );
...
UINT32 SRVAPI CFW_SmsMoInit(UINT16);
UINT32 SRVAPI CFW_SimInit(VOID);
...
UINT32 SRVAPI CFW_CfgSetTmParam(
    TM_PARAMETERS *pTmParam
);

UINT32 SRVAPI CFW_CfgGetTmParam(
    TM_PARAMETERS *pTmParam
);

#ifdef _DEBUG
PVOID  SRVAPI CSW_MallocDbg (UINT32 nSize, CONST UINT8* pszFileName, UINT32 nLine);
BOOL SRVAPI CSW_FreeDbg (VOID* pMemAddr, CONST UINT8* pszFileName, UINT32 nLine);
#define CFW_MALLOC(size) CSW_MallocDbg((UINT32)size, __FILE__, __LINE__)
#define CFW_FREE(p)      CSW_FreeDbg((PVOID)p, __FILE__, __LINE__)
#else
#define CFW_MALLOC(size) CSW_Malloc((UINT32)size)
#define CFW_FREE(p)      CSW_Free((PVOID)p)
#endif
...
//[[hameina[+]2006.8.30:add this struct to perform emod cfg functions
