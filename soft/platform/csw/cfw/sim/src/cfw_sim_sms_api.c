...
#else
  hAo = CFW_RegisterAo(CFW_SIM_SRV_ID, pReadSMSInfo, CFW_SimPatchProc);
#endif
...
#  else
UINT32 CFW_SimReadMessage(UINT16 nLocation, UINT32 nIndex, UINT16 nUTI)
#  endif
{
  HAO hAo;
  ...
  {
    CSW_TRACE(CFW_SIM_TS_ID, "Error Invlaid parameter nLocation 0x%x nIndex 0x%x\n", nLocation, nIndex);
...
#  else
  hAo = CFW_RegisterAo(CFW_SIM_SRV_ID, pReadSMSInfo, CFW_SimReadMessageProc);
#  endif
  CFW_SetUTI(hAo, nUTI, 1);
...
#  else
UINT32 CFW_SimWriteMessage(UINT16 nLocation, UINT32 nIndex, UINT8* pData, UINT8 nDataSize, UINT16 nUTI)
#  endif
{
  // 
  ...
  default:
    {
      CSW_TRACE(CFW_SIM_TS_ID, "Error Invlaid parameter pData[0] 0x%x\n", pData[0]);
  ...
  {
    CSW_TRACE(CFW_SIM_TS_ID, "Error Invlaid parameter nDataSize 0x%x\n", nDataSize);
    CSW_PROFILE_FUNCTION_EXIT(CFW_SimWriteMessage);
    return ERR_INVALID_PARAMETER;
  }
...
#  else
  hAo = CFW_RegisterAo(CFW_SIM_SRV_ID, pWriteSMSInfo, CFW_SimWriteMessageProc);
#  endif
  CFW_SetUTI(hAo, nUTI, 1);
...
UINT32 CFW_SimListMessage(UINT16 nLocation, UINT8 nStatus,  // The status of message in SIM,Just as below                                      
                          UINT16 nCount,  // Specify the counter message to list
                          UINT16 nStartIndex, UINT16 nUTI)
#  endif
{
  HAO hAo;
  ...
  if (nLocation != CFW_SMS_STORAGE_SM)
  {
    ...
    // 
    CSW_TRACE(CFW_SIM_TS_ID, "Error Invlaid parameter nLocation 0x%x\n", nLocation);
...
#  else
  hAo = CFW_RegisterAo(CFW_SIM_SRV_ID, pListSmsInfo, CFW_SimListMessageProc);
#  endif
  CFW_SetUTI(hAo, nUTI, 1);
...
#  else
  hAo = CFW_RegisterAo(CFW_SIM_SRV_ID, pDeleSmsInfo, CFW_SimDeleMessageProc);
#  endif
  CFW_SetUTI(hAo, nUTI, 1);
...
#  else
  hAo = CFW_RegisterAo(CFW_SIM_SRV_ID, pReadBinary, CFW_SimReadBinaryProc);
#  endif
  CFW_SetUTI(hAo, nUTI, 1);
...
#  else
  hAo = CFW_RegisterAo(CFW_SIM_SRV_ID, pUpdateBinary, CFW_SimUpdateBinaryProc);
#  endif
  CFW_SetUTI(hAo, nUTI, 1);
...
#  else
UINT32 CFW_SimSetMR(UINT8 nMR, UINT16 nUTI)
#  endif
{
  HAO hAo;
...
#  else
  hAo = CFW_RegisterAo(CFW_SIM_SRV_ID, pSetMR, CFW_SimSetMRProc);
#  endif
  CFW_SetUTI(hAo, nUTI, 1);
...
#  else
UINT32 CFW_SimGetMR(UINT16 nUTI)
#  endif
{
  HAO hAo;
...
#  else
  hAo = CFW_RegisterAo(CFW_SIM_SRV_ID, pGetMR, CFW_SimGetMRProc);
#  endif
  CFW_SetUTI(hAo, nUTI, 1);
...
#endif
{
  HAO hAo;
#ifdef CFW_MULTI_SIM
  CSW_TRACE(CFW_SIM_TS_ID, "CFW_SimGetSmsTotalNum nSimID[%d]\n",nSimID);
#else
  CSW_TRACE(CFW_SIM_TS_ID, "CFW_SimGetSmsTotalNum\n");
#endif
...
#else
  hAo = CFW_RegisterAo(CFW_SIM_SRV_ID, pGetSmsInfo, CFW_SimGetSmsTotalNumProc);
#endif
  CFW_SetUTI(hAo, nUTI, 1);
...
#  else
  hAo = CFW_RegisterAo(CFW_SIM_SRV_ID, pGetSmsInfo, CFW_SimGetSmsStorageInfoProc);
#  endif
  CFW_SetUTI(hAo, nUTI, 1);
  SUL_ZeroMemory32(pGetSmsInfo, SIZEOF(SIM_INFO_GETSMSINFO));
...
#  else
UINT32 CFW_SimGetSmsParameters(UINT8 nIndex, UINT16 nUTI)
#  endif
{
  // 
...
#  else
  hAo = CFW_RegisterAo(CFW_SIM_SRV_ID, pGetSmsParam, CFW_SimGetSmsParametersProc);
#  endif
  CFW_SetUTI(hAo, nUTI, 1);
...
#  else
UINT32 CFW_SimSetSmsParameters(CFW_SIM_SMS_PARAMETERS* pSmsParamArray, UINT8 nArrayCount, UINT8 nIndex, UINT16 nUTI)
#  endif
{
  HAO hAo;
...
#  else
  hAo = CFW_RegisterAo(CFW_SIM_SRV_ID, pSetSmsParam, CFW_SimSetSmsParametersProc);
#  endif
...
#  else
UINT32 CFW_SimSmsInit(VOID)
#  endif
{
  CSW_TRACE(CFW_SIM_TS_ID, "Initing CFW_SimSmsInit\n");
...
#  else
  nUTI = (CFW_APP_UTI_MIN + CFW_APP_UTI_MAX) / 2;
#  endif
...
#  else
  hMyAO = CFW_RegisterAo(CFW_SIM_SRV_ID, pUserData, CFW_SimSmsInitProc);
#  endif
  RET = CFW_SetUTI(hMyAO, nUTI, 0);
  if (RET != ERR_SUCCESS)
  {
    CSW_TRACE(CFW_SIM_TS_ID, "Error CFW_SetUTI return 0x%x \n", RET);
  ...
  if (RET != ERR_SUCCESS)
  {
    CSW_TRACE(CFW_SIM_TS_ID, "Error CFW_SetAoProcCode return 0x%x \n", RET);
    CSW_SIM_FREE(pUserData);
    CSW_PROFILE_FUNCTION_EXIT(CFW_SimSmsInit);
    return (RET);
  }
...
#  if g_SMS_ValueCheck
  ...
  {
    CSW_TRACE(CFW_SIM_TS_ID, "Error pbuf[1] Len 0x%x \n", pbuf[1]);
    return ERR_INVALID_PARAMETER;
  }
#  endif
  else
  ...
  {
    CSW_TRACE(CFW_SIM_TS_ID, "Error SMS is a Command or StatusReport\n");
    CSW_PROFILE_FUNCTION_EXIT(CFW_DecomposePDU);
    return ERR_INVALID_PARAMETER;
  }
  ...
  if (nMTI & 0x01)  // For Submit, there will be not read or unread statue
  {
    ...
    {
      CSW_TRACE(CFW_SIM_TS_ID, "Error Submit status read or unread\n");
  ...
  else if (nMTI & 0x00) // Fpr DELIVER,  there will be not send or unsend statue
  {
    ...
    {
      CSW_TRACE(CFW_SIM_TS_ID, "Error Submit status send or unsend\n");
      CSW_PROFILE_FUNCTION_EXIT(CFW_DecomposePDU);
      return ERR_INVALID_PARAMETER;
    }
...
#  if g_SMS_ValueCheck
    ...
    {
      CSW_TRACE(CFW_SIM_TS_ID, "Error Originating telephone number Len 0x%x \n", pbuf[tmp]);
      return ERR_INVALID_PARAMETER;
    }
#  endif
    nTmp = pbuf[tmp];
...
#  endif
    ...
    else
    {
      ...
      if (dcs == GSM_7BIT)
      {
        ...
        {
          CSW_TRACE(CFW_SIM_TS_ID, "Error it is a EMS\n");
          CSW_PROFILE_FUNCTION_EXIT(CFW_DecomposePDU);
          return ERR_INVALID_PARAMETER;
        }
        else
...
#  if 0
        CSW_TRACE(CSW_TS_ID, "=========>length is 0x%x\r\n", length);
        CSW_TRACE(CSW_TS_ID, "=========>nUDHL is 0x%x\r\n", nUDHL);
#  endif
        SUL_MemCopy8(data, &(nTmpForDate[nUDHL + 2]), (length = length - nUDHL - 2));
#  if 0
        CSW_TRACE(CSW_TS_ID, "=========>data[0] is 0x%x\r\n", data[0]);
        CSW_TRACE(CSW_TS_ID, "=========>data[1] is 0x%x\r\n", data[1]);
        CSW_TRACE(CSW_TS_ID, "=========>data[2] is 0x%x\r\n", data[2]);
        CSW_TRACE(CSW_TS_ID, "=========>data[3] is 0x%x\r\n", data[3]);
        CSW_TRACE(CSW_TS_ID, "=========>length is 0x%x\r\n", length);
#  endif
      }
      else if (dcs == GSM_UCS2)
...
#  if g_SMS_ValueCheck
    ...
    {
      CSW_TRACE(CFW_SIM_TS_ID, "Error DA Len 0x%x \n", nTmp);
      return ERR_INVALID_PARAMETER;
    }
#  endif
    if (nTmp != 0x00)
...
#  if g_SMS_ValueCheck
      if (nTpUdl > 140)
      {
        CSW_TRACE(CFW_SIM_TS_ID, "Error nTpUdl 0x%x \n", nTpUdl);
        return ERR_INVALID_PARAMETER;
      }
#  endif
      pTmp->nTpUdl    = nTpUdl;
...
#  if g_SMS_ValueCheck
      if (nTpUdl > 160)
      {
        CSW_TRACE(CFW_SIM_TS_ID, "Error nTpUdl Len 0x%x \n", nTpUdl);
        return ERR_INVALID_PARAMETER;
      }
#  endif
...
#  endif
    ...
    {
      CSW_TRACE(CFW_SIM_TS_ID, "Error MTI Value 0x%x\n", nMTI);
      CSW_PROFILE_FUNCTION_EXIT(CFW_DecomposePDU);
      return (ERR_INVALID_PARAMETER);
    }
  ...
  {
    CSW_TRACE(CFW_SIM_TS_ID, "Error nType Value 0x%x\n", nType);
    CSW_PROFILE_FUNCTION_EXIT(CFW_DecomposePDU);
