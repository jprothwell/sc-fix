	...
	switch (FileType)
	{
	    ...
	    case MCI_TYPE_WAV:
		{
			...
			if(readlen!=sizeof(WAVDataHeader))
			{
		       ...
		       else if(memcmp(WAVDataHeaderInfo.szData,"data",4)!=0)
			{
				MCI_TRACE (MCI_AUDIO_TRC|TSTDOUT,0,"[MMC_AUDIO_WAV]data header error [%d][%d][%d][%d]\n",
							WAVDataHeaderInfo.szData[0],WAVDataHeaderInfo.szData[1],WAVDataHeaderInfo.szData[2],WAVDataHeaderInfo.szData[3]);
				return MCI_ERR_BAD_FORMAT;
			}
			...
			else
			{
				...
				return MCI_ERR_ERROR;
                  ...
                  if(WAVHeaderInfo.wFormatTag !=1){ //ADPCM 
