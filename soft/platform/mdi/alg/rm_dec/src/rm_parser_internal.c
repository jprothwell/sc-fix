/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: rm_parser_internal.c,v 1.9 2008/02/23 05:06:41 ehyche Exp $
 * 
                                            ...
                                            struct rm_media_props_hdr* h)
{
    ...
    if (pInt && h)
    {
        ...
        if (ulLen >= 41)
        {
            ...
            {
				//not unpack non video or non audio MDPR .sheen
				if(rm_stream_is_realvideo_mimetype(pBuf+h->stream_name_sz+1)!=TRUE &&
					rm_stream_is_realaudio_mimetype(pBuf+h->stream_name_sz+1)!=TRUE)
					return HXR_IGNORE;
				//sheen end
...
UINT32 rm_parseri_get_num_logical_fileinfo_hdrs(rm_parser_internal* pInt)
{
    UINT32      ulRet       = 0;
    UINT32      i           = 0;
    const char* pszMimeType = HXNULL;

    if (pInt && pInt->pMediaPropsHdr)
    {
        for (i = 0; i < pInt->ulNumMediaPropsHdrs; i++)
        {
            pszMimeType = (const char*) pInt->pMediaPropsHdr[i].mime_type;
                                                ...
                                                UINT32 ulLFIndx, UINT32* pulMPIndx)
{
    ...
    if (pInt && pInt->pMediaPropsHdr && pulMPIndx)
    {
        for (i = 0; i < pInt->ulNumMediaPropsHdrs; i++)
        {
            pszMimeType = (const char*) pInt->pMediaPropsHdr[i].mime_type;
            if (rm_parseri_is_logical_fileinfo_mime_type(pszMimeType))
            {
                /* Is this the logical-fileinfo we're looking for? */
                if (ulLF == ulLFIndx)
                {
                    *pulMPIndx = i;
                    retVal     = HXR_OK;
                ...
                /* Increment the logical-fileinfo index */
                ulLF++;
            }
        }
    }
...
{
    if (pInt && pInt->ulNumStreamLogicalFileInfoHdrs && pInt->pStreamLogicalFileInfo)
    {
        UINT32 i = 0;
        for (i = 0; i < pInt->ulNumStreamLogicalFileInfoHdrs; i++)
        {
            /* Clean up the i-th stream level logical-fileinfo struct */
            rm_parseri_cleanup_logical_stream_hdr(pInt, &pInt->pStreamLogicalFileInfo[i]);
        }
        /* Clean up the array memory */
        rm_parseri_free(pInt, pInt->pStreamLogicalFileInfo);
        /* NULL out the pointer */
        pInt->pStreamLogicalFileInfo = HXNULL;
        /* Set the number of stream-level logical-fileinfo headers to zero */
        pInt->ulNumStreamLogicalFileInfoHdrs = 0;
    }
}
...
HX_RESULT rm_parseri_unpack_all_logical_stream_hdrs(rm_parser_internal* pInt)
{
    ...
    if (pInt)
    {
        ...
        UINT32 ulIndx       = 0;
        UINT32 i            = 0;
        UINT32 ulNumLogical = 0;
        /* Get the number of media props headers that are logical streams */
        ulNumLogical = rm_parseri_get_num_logical_streams(pInt);
        if (ulNumLogical)
        {
        ...
        if (retVal == HXR_OK)
        {
            ...
            /* Clean up any existing stream-level logical-fileinfo headers */
            rm_parseri_cleanup_stream_logical_fileinfo_hdrs(pInt);
            /* Do we have any logical-fileinfo headers? */
            ulNumLogical = rm_parseri_get_num_logical_fileinfo_hdrs(pInt);
            if (ulNumLogical)
            {
                /*
                 * The first logical-fileinfo header is for file-level meta-data.
                 * logical-fileinfo headers after the first are for stream-level meta-data.
                 * So we will put the file-level info in pLogicalFileInfo, and put
                 * the stream-level info in pStreamLogicalFileInfo.
                 */
                retVal = rm_parseri_get_logical_fileinfo_index(pInt, 0, &ulIndx);
                if (retVal == HXR_OK)
                {
                    ...
                    if (pInt->pLogicalFileInfo)
                    {
                        ...
                        {
                            /* Call back to the error interface */
                            rm_parseri_error(pInt, retVal, "Failed to parse file-level logical-fileinfo header.");
                        }
                    }
                    else
                    {
                ...
                if (retVal == HXR_OK)
                {
                    /* Compute the number of stream-level logical-fileinfo headers */
                    pInt->ulNumStreamLogicalFileInfoHdrs = ulNumLogical - 1;
                    if (pInt->ulNumStreamLogicalFileInfoHdrs)
                    {
                        /* Compute the size of our logical-fileinfo array */
                        ulSize = sizeof(struct rm_logical_stream_hdr) * pInt->ulNumStreamLogicalFileInfoHdrs;
                        /* Allocate space for the array */
                        pInt->pStreamLogicalFileInfo =
                            (struct rm_logical_stream_hdr*) rm_parseri_malloc(pInt, ulSize);
                        if (pInt->pStreamLogicalFileInfo)
                        {
                            /* Zero out the memory */
                            memset(pInt->pStreamLogicalFileInfo, 0, ulSize);
                            /* Loop through each stream-level logical-fileinfo header and unpack it */
                            for (i = 0; i < pInt->ulNumStreamLogicalFileInfoHdrs && retVal == HXR_OK; i++)
                            {
                                /*
                                 * Get the index of the i-th stream-level logical-fileinfo header. We
                                 * have to add one since the first logical-fileinfo header contains
                                 * file-level (not stream-level) meta-data.
                                 */
                                retVal = rm_parseri_get_logical_fileinfo_index(pInt, i + 1, &ulIndx);
                                if (retVal == HXR_OK)
                                {
                                    /* Unpack the logical-fileinfo struct */
                                    retVal = rm_parseri_unpack_logical_stream_hdr(pInt,
                                                                                  &pInt->pMediaPropsHdr[ulIndx],
                                                                                  &pInt->pStreamLogicalFileInfo[i]);
                                    if (retVal != HXR_OK)
                                    {
                                        /* Call back to the error interface */
                                        rm_parseri_error(pInt, retVal, "Failed to parse stream-level logical-fileinfo header.");
                                    }
                                }
                            }
                        }
...
HX_RESULT rm_parseri_setup_interleaved_streams(rm_parser_internal* pInt)
{
    ...
    if (pInt && pInt->pMediaPropsHdr && pInt->ulNumMediaPropsHdrs)
    {
        ...
        if (pInterleaved)
        {
            ...
            if (ulNumInterleaved)
            {
#if 0//same to pMediaPropsHdr, no need realloc. sheen
                ...
                {
                    /*
                     * Copy the media props headers which are interleaved. If
                     * a media props header is not interleaved, then clean
                     * it up.
                     */
                    j = 0;
                    for (i = 0; i < pInt->ulNumMediaPropsHdrs; i++)
...
HX_RESULT rm_parseri_read_all_headers(rm_parser_internal* pInt)
{
    ...
    if (pInt)
    {
        ...
        /* Init the allocator */
        retVal = rm_allocator_init(&pInt->allocator,
                                   pInt->pUserMem,
                                   pInt->fpMalloc,
                                   pInt->fpFree);
        if (retVal == HXR_OK)
                        ...
                        */
                            ...
                            (pInt->fileHdr.file_version == 0 || pInt->fileHdr.file_version == 1))
                        {
                            ...
                            */
                                        ...
                                        case RM_PROPERTIES_OBJECT:
                                            {
                                                ...
                                                if (retVal == HXR_OK)
                                                {
                                                    ...
                                                    if (retVal == HXR_OK)
                                                    {
                                                        ...
                                                        pInt->ulNumMediaPropsHdrs = 0;
                                                        /* Do we have a non-zero max packet size? */
                                                        if (pInt->propHdr.max_pkt_size)
                                                        {
                                                            /* Pre-allocate one buffer of maximum packet size */
                                                            retVal = rm_allocator_preallocate(&pInt->allocator, 2,
                                                                                              pInt->propHdr.max_pkt_size);//change 2 max packet buffer for rv and ra read the same time. sheen
                                                        }
                                                    }
                                                }
                                            }
                                        ...
                                        case RM_MEDIA_PROPERTIES_OBJECT:
                                            {
                                                ...
                                                */
                                                ...
                                                if (ulStreamNum < pInt->ulNumMediaPropsHdrsAlloc)
                                                {
													...
													else if(retVal == HXR_IGNORE)
													{//skip none video or audio stream. sheen
														retVal = HXR_OK;
													}
                                                    else
                                                    {
                            ...
                            {
								/*
								in RM_MEDIA_PROPERTIES_OBJECT skip non video or audio stream,
								so no logical here. sheen
								*/
    ...
    if (pInt && pProp && ulNumProp)
    {
        ...
        if (retVal == HXR_OK)
        {
            retVal = rm_parseri_set_rm_property(pInt, &pProp[ulIndex++], "Flags",
                                                RM_PROPERTY_TYPE_UINT32,
                                                (void*) (UINT32) pInt->propHdr.flags, 0);
        }
        if (retVal == HXR_OK &&
                ...
                for (i = 0; i < pInt->ulNumStreams && retVal == HXR_OK; i++)
                {
                    ...
                    if (pInt->pMediaPropsHdr[i].mime_type)
                    {
                        ...
                        if (rm_stream_is_realaudio_mimetype(pszStr))
                        {
                            ...
                            pInt->pStreamInfo[i].bIsRealAudioLossless = FALSE;
                            pInt->bIsRealDataType                     = TRUE;
                            if (rm_stream_is_realaudio_lossless_mimetype(pszStr))
                            {
                                pInt->pStreamInfo[i].bIsRealAudioLossless = TRUE;
                            }
                        }
                                                ...
                                                rm_property* pProp, UINT32 ulNumProps)
{
    ...
    if (pInt && i < pInt->ulNumStreams && hdr)
    {
            ...
            pInt->pMediaPropsHdr[i].type_spec_sz)
        {
            if (pProp)
            {
                if (pInt->pStreamInfo[i].bIsRealVideo)
                {
                     ...
                     */
                    BYTE*  pBuf_in  = &ucRuleToFlag[0];
                    UINT32 ulLen_in = 8;
                    rm_pack16(3,                &pBuf_in, &ulLen_in);
                    rm_pack16(HX_KEYFRAME_FLAG, &pBuf_in, &ulLen_in);
                    rm_pack16(0,                &pBuf_in, &ulLen_in);
                    rm_pack16(HX_KEYFRAME_FLAG, &pBuf_in, &ulLen_in);
                    /* Set the property */
        ...
        /* Do we have a stream-level logical-fileinfo header for this stream? */
        if (i < pInt->ulNumStreamLogicalFileInfoHdrs && pInt->pStreamLogicalFileInfo)
        {
            /*
             * We do have stream-level meta-data in a logical-fileinfo header
             * for this stream, so we need to count all these properties.
             */
            for (j = 0; j < (UINT32) pInt->pStreamLogicalFileInfo[i].num_props; j++)
            {
                struct rm_name_value_map* pMap = &pInt->pStreamLogicalFileInfo[i].props[j];
                UINT32 ulLength = pMap->value_sz;
                if (pMap->type != RM_PROPERTY_TYPE_BUFFER)
                {
                    ulLength = 0;
                ...
                if (pProp)
                {
                    rm_parseri_set_rm_property(pInt, &pProp[ulRet],
                                               (const char*) pMap->name,
                                               pMap->type, pMap->value, ulLength);
                }
                ulRet++;
...
//add usStreamNum for audio or video.sheen
HX_RESULT rm_parseri_read_next_packet(rm_parser_internal* pInt, rm_packet** ppPacket, UINT16 usStreamNum)
{
    ...
    if (pInt && ppPacket)
    {
        /* Set the return value */
        retVal = HXR_OUTOFMEMORY;
        /* Set the default out param */
        *ppPacket = HXNULL;
        /* Allocate an rm_packet struct */
        pPacket = (rm_packet*) rm_parseri_malloc(pInt, sizeof(rm_packet));
        if (pPacket)
        {
            /* Null out the packet members */
            memset(pPacket, 0, sizeof(rm_packet));
            /* Now call rm_parseri_read_next_packet2() */
            ...
            {
                /* Free the rm_packet struct memory */
...
HX_RESULT rm_parseri_read_next_packet2(rm_parser_internal* pInt, rm_packet* pPacket, UINT16 usStreamNum)
{
    ...
    if (pInt && pPacket)
    {
        /* Cleanup the incoming packet struct */
        ...
        if (pInt->ulCurFileOffset < pInt->propHdr.data_offset + pInt->dataHdr.size)
        {
            ...
            do
            {
                ...
                if (retVal == HXR_OK)
                {
                    ...
                    retVal = HXR_FAIL;
                    /* Translate the stream number */
                    ulStreamNum2 = rm_parseri_translate_stream_number(pInt, hdr.stream_num);

                    if (ulStreamNum2 == usStreamNum)//find usStreamNum packet. change sheen
                    {
                        /* Get the rm_stream_info for this stream */
                        struct rm_stream_info* pInfo = &pInt->pStreamInfo[ulStreamNum2];
                        if (pInfo)
                        {
                            /* Clear the return value */
                            retVal = HXR_OK;
					...
					else if(ulStreamNum2 != RM_NO_STREAM_SET)//skip none usStreamNum packet. change sheen
					{
						retVal = HXR_OK;
						bSkipPacket = TRUE;
						/* Seek past the data of this packet */
            ...
            if (HX_SUCCEEDED(retVal))
            {
                ...
                ulLastPacketTime = pInt->pStreamInfo[ulStreamNum2].ulLastTimeStamp;
                ulLastRule       = pInt->pStreamInfo[ulStreamNum2].ulLastRule;
                /* Determine the ASM rules and flags */
                if (pInt->pStreamInfo[ulStreamNum2].bIsRealVideo)
                {
                    /*
                ...
                else if (pInt->pStreamInfo[ulStreamNum2].bIsRealAudio &&
                         !pInt->pStreamInfo[ulStreamNum2].bIsRealAudioLossless)
                {
                    /*
                ...
                else
                {
                    ...
                    if (pInt->pStreamInfo[ulStreamNum2].bIsRealEvent ||
                ...
                retVal = rm_allocator_get_buffer(&pInt->allocator,
                                                 ulDataSize,
                                                 FALSE,
                                                 &pPacket->pData);
                if (retVal == HXR_OK)
                {
                    ...
                    if (ulBytesRead == ulDataSize)
                    {
                        ...
                        pPacket->usStream   = (UINT16) ulStreamNum2;
                        ...
                        pInt->pStreamInfo[ulStreamNum2].ulLastTimeStamp = hdr.timestamp;
                        pInt->pStreamInfo[ulStreamNum2].ulLastRule      = ulASMRule;
...
{
    if (pInt && pInt->fpFree && pPacket)
    {
        /* Do we have packet data? */
        if (pPacket->pData)
        {
            /* Release the packet buffer back to the allocator */
            rm_allocator_release_buffer(&pInt->allocator, pPacket->pData);
        }
        /* Null out the packet members */
        memset(pPacket, 0, sizeof(rm_packet));
    }
...
#if 0//sheen, need return big than seektime		.
...
#else
//rm_printf("ulTableIdx=%d pTable->ulNumEntries=%d time=%d ", ulTableIdx, pTable->ulNumEntries, pTable->pEntry[ulTableIdx ].ulTime);

        /* Scan forward in the table */
        while ((ulTableIdx < (pTable->ulNumEntries - 1)) &&
	       (pTable->pEntry[ulTableIdx ].ulTime < ulSeekTime))
        {
	    ulTableIdx++;
//rm_printf("1 time=%d",pTable->pEntry[ulTableIdx].ulTime);
        }
        /* Scan backwards in the table */
        while ((ulTableIdx > 0) &&
	       (pTable->pEntry[ulTableIdx-1].ulTime >= ulSeekTime))
        {
	    ulTableIdx--;
//rm_printf("2 time=%d",pTable->pEntry[ulTableIdx].ulTime);
        }
#endif
        /* Report results */
        ...
        *pulFoundOffset = pTable->pEntry[ulTableIdx].ulOffset;
        /* Set the return value */
        //retVal = (ulSeekTime > pTable->ulRangeTime ? HXR_AT_END : HXR_OK);//change sheen
		retVal = (ulSeekTime > *pulFoundTime ? HXR_AT_END : HXR_OK);

//rm_printf("ulSeekTime=%d pTable->ulRangeTime=%d retVal=%d lFoundTime=%d", ulSeekTime, pTable->ulRangeTime, retVal, *pulFoundTime);
    }
...
HX_RESULT rm_parseri_seek(rm_parser_internal* pInt, UINT32 ulSeekTime)
{
    ...
    if (pInt && pInt->ulNumStreams && pInt->pStreamInfo)
    {
        ...
        else if (pInt->propHdr.index_offset /*&& ulSeekTime*/)//change sheen.
        {
            /*
