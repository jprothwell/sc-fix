/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: rv_depack_internal.c,v 1.5 2007/05/07 14:50:25 ehyche Exp $
 * 
                                  ...
                                  rv_frame**          ppFrame)
{
        ...
        pFrameHdr->ulFrameSize && pFrameHdr->ulNumPackets)
    {
        ...
        if (pFrame)
        {
            ...
            /*
             * Pad the size of the encoded frame buffer by 3 bytes.
             * This is because the codecs do reads 32 bits at a time,
             * regardless of how many bytes are left in the buffer.
             */
            ulPadFrameSize = pFrameHdr->ulFrameSize + 3;
            /* Allocate enough space for the data */
            pFrame->pData = rv_depacki_malloc(pInt, ulPadFrameSize);
            if (pFrame->pData)
            {
                /* NULL out the memory */
                memset(pFrame->pData, 0, ulPadFrameSize);
