...
int32 MMF_Avi_Enc_Frame(JpegAmr_Avi_Enc* pAvi_Enc)
{
  ...
  //printf("shenh jpg size res=%d\n",pAvi_Enc->Jpeg_Frame_Size);
	...
	//avi_auidx[audio_i].audio_size = pAvi_Enc->Amr_Data_Size;
	...
	idx_audio_offset = idx_video_offset + avi_viidx[video_i-1].video_size + 8;
	audio_i ++;
...
//	long movi_sz;
	...
	riff_sz_64 = 4 + sizeof(struct AVI_list_hdrl) + 12 + 8*frames + jpg_sz_64 + 8*frames + amr_block*frames + 6 
  ...
  hdrl.strl_video.strh.rate = LILEND4(VIDREC_AVI_VIDEO_FRAME_RATE);       //bao debug: frame per second
	...
	print_quartet((uint8 *)&pOutBuf,jpg_sz_64 + 8*frames + 4 + frames * (amr_block + 8) + 6);	
...
int16 MMF_Avi_Enc_End(JpegAmr_Avi_End* pAvi_End, Avi_End_Return* pAvi_End_Re)
{
	...
	for (; frame_pos < i; frame_pos++) {
		...
		print_quartet((uint8 *)&pOutBuf, a_size);
	}
...
void video_record_test()
{
	...
	jpg_size=FS_Seek((int32)fjpg, 0,FS_SEEK_END );
	...
	jpg2_size=FS_Seek((int32)fjpg, 0,FS_SEEK_END );
	...
	result=FS_Seek((int32)favi, 0, FS_SEEK_SET);
