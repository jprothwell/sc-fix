			   ...
			   int bs_version)
{
	...
	const uint32_t edge_size2 = EDGE_SIZE2;
	uint32_t i;
	...
	for(i = height2 ; i > 0; i--)
	{
		left_y_s = (int32_t)(*(p_left_y_d + EDGE_SIZE));
		right_y_s = (int32_t)(*(p_right_y_d - 1));
		memset(p_left_y_d,  (int32_t)left_y_s  , EDGE_SIZE);
		memset(p_right_y_d, (int32_t)right_y_s , EDGE_SIZE);
	...
	//top, bottom
	p_top_y_s = image->y - EDGE_SIZE;
	p_top_y_d = p_top_y_s - EDGE_SIZE * edged_width;		
	p_bottom_y_s = image->y + edged_width * (height - 1) - EDGE_SIZE;	
