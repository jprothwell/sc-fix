...
--------------------------------------------------------------------------------
  ...
  Apr 22 04  |  FCH  | Add variable sxr_NbUsedBlk for memory check in the
             |       | scenarios (simulation)
  Sep 13 04  |  ADA  | Cluster in Heap.
  Dec 20 04  |  ADA  | - Memory tracker.
             |       | - Checksum functions -> defines.
  Mar 23 05  |  ADA  | HMalloc :
             |       | - Fill proprely NextBlock -> H.Index to free.
             |       | - Cluster match when required size < cluster size.
  Aug 15 05  |  ADA  | Update the number of allocated blocks in cluster context.
  Jan 27 06  |  ADA  | Function sxr_GetLink.
  Feb 08 06  |  ADA  | _sxr_HMalloc: allocation in Heap 0 when cluster allocation
             |       | fails because of a lack of memory.
  Mar 27 06  |  ADA  | - HMalloc / HFree: invoke the memory level call back
             |       | when a cluster allocation is propagated in the heap
             |       | for the first time (MEMORY_LEVEL_LOW) and when all
             |       | propagated blocks are released (MEMORY_LEVEL_OK).
             |       | - Cluster for big blocks useless.
  Mar 31 06  |  ADA  | - Heap allocation of more than 64k.
  May 20 06  |  ADA  | - Change criteria to exit flow control.
================================================================================
*/
...
#endif
{
    ...
    if (!sxr_CheckSum (&Header -> CheckSum))
    {
        Header = (sxr_MemHead_t *)( ((u8 *)_Ptr) - sizeof (sxr_HMemHead_t));
    }
