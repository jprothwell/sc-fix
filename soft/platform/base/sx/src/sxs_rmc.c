...
--------------------------------------------------------------------------------
 ...
 Jan 27 2006 |  ADA  | Function sxs_InitRmc sends the trace Id in a flow INIT_RMC.
================================================================================
*/
...
//  sxs_SendToUdp (Id, Data, Size);
  ...
  case SXS_RPC_RMC :
  {
   ...
   {
    sxs_FlwRpcRsp_t *FlwRpcRsp = (sxs_FlwRpcRsp_t *)sxs_SendToRemote (SXS_RPC_RSP_RMC, NIL, (u16)(SXS_GET_RPC_RET_SZE(FlwRpc -> Id) + sizeof (sxs_FlwRpcRsp_t)));
  ...
  case SXS_READ_RMC :
  {
   ...
   sxs_FlwDmp_t *OutFlwDmp = (sxs_FlwDmp_t *)sxs_SendToRemote (SXS_DUMP_RMC, NIL, (u16)(InFlwRW -> Size + sizeof (sxs_FlwDmp_t)));
   u16 SizeRW, i;
   u8  readId;
...
//    sxs_SendToUdp (Id, Data, Size);
...
#endif
   {
    ...
    u8 *Msg = (u8 *)sxr_Malloc (Size);

    memcpy (Msg, (u8 *)RmtMbx -> Data, Size); // - sizeof (sxs_RmtMbx_t) + 4);
...
#ifdef __TARGET__
  {
   sxs_FlwSerial_t *sxs_FlwSerial = (sxs_FlwSerial_t *)Data;
   u16 Setting;

   if (sxs_FlwSerial -> Id == SXS_RMC_REQ)
   {
    Setting = sxs_GetSerialCfg (sxs_FlwSerial -> SerialIdx);
    sxs_SetSerialCfg ((Setting & ~SXS_RATE_MSK) | SXS_SET_RATE(sxs_FlwSerial -> Speed), sxs_FlwSerial -> SerialIdx);
    sxs_FlwSerial = (sxs_FlwSerial_t *)sxs_SendToRemote (SXS_SERIAL_RMC, NIL, sizeof (sxs_FlwSerial_t));
    sxs_FlwSerial -> Id = SXS_RMC_IND;
   }
  }
#endif
