...
int sxr_CheckStackUsage(unsigned char TaskId)
{
	u32 *stackMem = (u32 *)sxr_Task.Ctx [TaskId].StackTop;
	int i=0;
#ifdef __SXR_STACK_CHECK__
	while (stackMem[i++] == SXR_DW_MEM_PATTERN );
#endif	
	return sxr_Task.Ctx [TaskId].Desc -> StackSize-i;
}
// =============================================================================
// sxr_FreeTask
// -----------------------------------------------------------------------------
...
#ifdef __NATIVE_JAVA__	
    gcj_TraceOut(TSTDOUT,"sxr_CheckStackUsage task is %s,id=%d, max is %d, usage is %d\n",sxr_Task.Ctx [Id].Desc -> Name,Id,sxr_Task.Ctx [Id].Desc -> StackSize,sxr_CheckStackUsage(Id));
#endif
...
{
    {
        if ( sxr_Task.Ctx [TkNb].Id != SXR_NO_TASK)
        {
            ...
            switch (sxr_Task.Ctx [TkNb].State)
            {
                ...
                SXS_FPRINTF ((_SXR|TIDU|TSTDOUT|TDB,TSTR("is pending.\n",0x06c30007)));
                break;
                ...
                SXS_FPRINTF ((_SXR|TIDU|TSTDOUT|TDB,TSTR("is suspended.\n",0x06c30009)));
                break;

                case SXR_WAIT_MSG_TSK :
                SXS_FPRINTF ((_SXR|TIDU|TSTDOUT|TDB,TSTR("wait for a message.\n",0x06c3000a)));
                break;
...
#ifdef __SXR_STACK_CHECK__
            {
                ...
                while (((u32 *)sxr_Task.Ctx [TkNb].StackTop) [j++] == SXR_DW_MEM_PATTERN);

                SXS_FPRINTF ((_SXR|TSTDOUT|TIDU|TDB|TNB_ARG(4),TSTR("Stack %3i/%3i Top 0x%lx <- 0x%lx\n",0x06c3000c),
                             (SXR_SET_STACK(sxr_Task.Ctx [TkNb].Desc -> StackSize) << 2) - (j << 2),
