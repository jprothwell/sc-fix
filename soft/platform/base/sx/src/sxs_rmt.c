...
u8 *_sxs_SendToRemote (u8 Id, u8 *Data, u16 Size, u8 RmtCpuIdx)
{
    ...
    if (!sxs_Rmt [RmtCpuIdx].StampStarted)
    {
        ...
        if (((sxs_Rmt [RmtCpuIdx].FnStamp != NIL)
        &&  (Id == SXS_RMC_YIELD)
        &&  (sxs_Rmt [RmtCpuIdx].Fn != *sxs_Rmt [RmtCpuIdx].FnStamp))
        ||  (sxs_Rmt [RmtCpuIdx].DataLost == TRUE))
        {
...
//   sxs_Rmt [RmtCpuIdx].Buf [sxs_Rmt [RmtCpuIdx].NextIdx ] = 0; 
...
//sxs_Rmt [RmtCpuIdx].Buf [WIdx] = SXS_RMT_START_FRM;
