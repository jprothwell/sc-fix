...
/*                                                                       */
/* DESCRIPTION                                                           */
/*      This file contains PA and RF switch related functions.           */
/*                                                                       */
/*************************************************************************/
...
// For HAL PA control
//PRIVATE UINT8 g_rampIdx = 0;
//PRIVATE HAL_ANA_PA_RAMP_T g_ramp[HAL_ANA_PA_MAX_RAMP_QTY];
CONST HAL_TCU_EVENT_ID_T g_paEvent[HAL_ANA_PA_MAX_RAMP_QTY] =
{
...
typedef UINT32             my_fxd_t;
    ...
    for(i=0;i<1024;i++)
    {
        if(g_paProfileTmpArray[i].x>=0 && g_paProfileTmpArray[i].x<=1023 )
        {
		...
		case INTERP_PADAC:
            ...
            *NextDAC = 0;
        	...
        	for (   paTabIdx = 0;
                    ...
                    paTabIdx++)
        	{
        		dacArray[paTabIdx].x = 1023 * (dacArray[paTabIdx].x - min)
                                              / (max - min);
        	}
