...
#define CONFIG_CLASS_K_AUDIO_PA 1
#else
#define CONFIG_CLASS_K_AUDIO_PA 0
#endif
...
typedef union
{
    struct {
        ...
        POWER_STATE_T  reserved1:1;
        POWER_STATE_T  reserved2:1;
        ...
        POWER_STATE_T  PFM_MODE_SEL_ACT:1;
        BOOL           VRtcHigh:1;
    };
    UINT16 reg;
} OPAL_PROFILE_CFG_T;
...
PRIVATE OPAL_PROFILE_CFG_T g_pmdProfileRegs[OPAL_PROFILE_MODE_QTY] =
{
    {{  // profile 1 : OPAL_PROFILE_MODE_NORMAL
        ...
        .Vmemory = POWER_ON,
        .VDcdcCore = POWER_ON,
        .PFM_MODE_SEL_ACT = FALSE,
        .VRtcHigh = TRUE,
    }},
    {{  // profile 2 : OPAL_PROFILE_MODE_LOWPOWER
        .mode = LP_MODE,
        .VMic = POWER_OFF,//600uA
        .VUSB = POWER_OFF,
        .Vibr = POWER_OFF,
        .Vmmc = POWER_OFF,
        .Vlcd = POWER_OFF,
...
// SIM setting
//  RDA_ADDR_SIM_INTERFACE2                    == 0x00000027
PRIVATE UINT16 g_pmdSimSelect = 
                RDA1203_PULLUP_ENABLE_1|RDA1203_UNSEL_CLK_VAL_1|
                RDA1203_UNSEL_RST_VAL_1|RDA1203_PULLUP_ENABLE_2|
                RDA1203_UNSEL_CLK_VAL_2|RDA1203_UNSEL_RST_VAL_2|
                RDA1203_PULLUP_ENABLE_3|RDA1203_UNSEL_CLK_VAL_3|
                RDA1203_UNSEL_RST_VAL_3|RDA1203_PULLUP_ENABLE_4|
                RDA1203_UNSEL_CLK_VAL_4|RDA1203_UNSEL_RST_VAL_4;
              

PRIVATE UINT16 g_pmdSimSelectTime = RDA1203_TIMER_BB_RESETN_SEL_60MS;
...
PRIVATE UINT32 g_pmdLdoActiveSetting5 = 0;

/// value of the Opal register LDO_Out_V_Ctrl
//  RDA_ADDR_LDO_LP_SETTING2 
//  RDA_ADDR_LDO_ACTIVE_SETTING2
...
PRIVATE CONST CALIB_PMD_T g_pmdCalibDefault = {0x2901, 0};
...
{
    //PMD_TRACE(PMD_INFO_TRC, 0, "Write REG 0x%x: 0x%0x", regIdx, value);
...
#endif // SPI_REG_DEBUG
//return;
    PMD_ASSERT(PMD_POWER_ID_QTY < 31, "Expected PMD_POWER_ID_QTY < 31, implementation of g_pmdMapAccess.powerInfo must be updated to support more IDs", PMD_POWER_ID_QTY);
    PMD_ASSERT(pmdConfig != NULL, "pmdConfig must be not NULL.");
    g_pmdConfig = pmdConfig;
    g_pmdInitDone = FALSE;

    // LDO VCORE value->voltage (estimated):
    // 7->0.94, 6->1.07, 5->1.21, 4->1.34, 3->1.47, 2->1.60, 1->1.70, 0->1.80
    // NOTE:
    ...
    g_pmdLdoVcoreValueLp = 0;

    // DCDC VCORE value->voltage:
    // 2->1.38, 3->1.54, 4->1.69, 5->1.86, 6->2.00, 7->xxx
#if (DYNAMIC_DCDC_VCORE_ADJUSTMENT == 1)
    ...
    g_pmdDcdcVcoreValueActiveAry[PMD_CORE_VOLTAGE_LOW]  = 4;
//    g_pmdDcdcVcoreValueActiveAry[PMD_CORE_VOLTAGE_LOW]  = 1;
#endif // DYNAMIC_DCDC_VCORE_ADJUSTMENT != 1

    // 4->1.55, 7->2.0
#ifdef USE_HIGH_CORE_VAL
    g_pmdDcdcVcoreValueActiveAry[PMD_CORE_VOLTAGE_HIGH]  = 7;
#else
    g_pmdDcdcVcoreValueActiveAry[PMD_CORE_VOLTAGE_HIGH]  = 4;
#endif
    g_pmdDcdcVcoreValueLp = 0;
    ...
    extern const HAL_RFSPI_CFG_T g_spiCfg;
    hal_RfspiOpen(&g_spiCfg, HAL_RFSPI_19_5M_CLK);
...
#endif
    UINT32 chipId = GALLITE_CHIP_ID_VERSION_5;
    xcvReadReg(0, (UINT8*)&chipId);
#endif


    if (chipId == GALLITE_CHIP_ID_VERSION_5)
    {
    ...
    else if (chipId == GALLITE_CHIP_ID_VERSION_6)
    {
...
//    g_pmdProfileRegs[OPAL_PROFILE_MODE_LOWPOWER].VRF = POWER_OFF;
//    g_pmdProfileRegs[OPAL_PROFILE_MODE_LOWPOWER].Vpad = POWER_OFF;
    ...
    // Init SIM setting
    pmd_OpalSpiWrite(RDA_ADDR_SIM_INTERFACE1, g_pmdSimSelectTime);
    pmd_OpalSpiWrite(RDA_ADDR_SIM_INTERFACE2, g_pmdSimSelect);
    // Init voltage selection

//    hal_HstSendEvent(0x22000000);
//    hal_HstSendEvent(g_pmdLDO_Out_V_Ctrl[OPAL_PROFILE_MODE_NORMAL]);
    ...
    pmd_OpalSpiWrite(RDA_ADDR_LDO_LP_SETTING4, 0x493c);
...
#if (DYNAMIC_DCDC_VCORE_ADJUSTMENT == 1)
    hal_SysRequestSetCallback(_pmd_inc_CoreVoltage, HAL_SYS_FREQ_LEVEL_RISE);
    hal_SysRequestSetCallback(_pmd_dec_CoreVoltage, HAL_SYS_FREQ_LEVEL_FALL);   
#endif // DYNAMIC_DCDC_VCORE_ADJUSTMENT == 1
...
PUBLIC VOID pmd_EnableDcdcPower(BOOL on)
{
    if (FALSE == on)
    {
        pmd_RegWrite(RDA_ADDR_THERMAL_CALIBRATION, 0x7a00);
        hal_TimDelay(5);
    }
    ...
    if (on)
    {
        g_pmdProfileRegs[OPAL_PROFILE_MODE_NORMAL].VDcdcCore = POWER_ON;
    }
    else
    {
        g_pmdProfileRegs[OPAL_PROFILE_MODE_NORMAL].VDcdcCore = POWER_OFF;
    }
    ...
    {
        hal_TimDelay(5);
        pmd_RegWrite(RDA_ADDR_THERMAL_CALIBRATION, 0xfa00);
    }
...
PUBLIC VOID PMD_FUNC_INTERNAL pmd_SetPowerMode(PMD_POWERMODE_T powerMode)
{
    ...
    switch (powerMode)
    {
        case PMD_LOWPOWER:
            ...
            // LDO VCORE will be powered on and set to the highest voltage once entering active mode.
//            g_pmdProfileRegs[OPAL_PROFILE_MODE_NORMAL].PFM_MODE_SEL_ACT = TRUE;
#ifdef SLEEP_USE_LDO
            pmd_RegRead(RDA_ADDR_THERMAL_CALIBRATION, &ret);
            ret = (ret &
                                        ~RDA1203_ACT_LDO_CORE);
            pmd_RegWrite(RDA_ADDR_THERMAL_CALIBRATION, ret);
#endif
            break;
        case PMD_IDLEPOWER:
#if (DYNAMIC_DCDC_VCORE_ADJUSTMENT == 0)
...
//            g_pmdProfileRegs[OPAL_PROFILE_MODE_NORMAL].PFM_MODE_SEL_ACT = FALSE;
#ifdef __PRJ_WITH_SPIFLSH__
            memd_FlashWakeup();
#endif
#ifdef SLEEP_USE_LDO
...
//    pmd_RDAWrite(RDA_ADDR_LDO_LP_SETTING3, g_pmdLdoActiveSetting3);
#endif // DYNAMIC_DCDC_VCORE_ADJUSTMENT == 0

    // LDO VCORE on/off
    pmd_RDAWrite(RDA_ADDR_LDO_ACTIVE_SETTING1, g_pmdProfileRegs[OPAL_PROFILE_MODE_NORMAL].reg);
//    pmd_RDAWrite(RDA_ADDR_LDO_LP_SETTING1, g_pmdProfileRegs[OPAL_PROFILE_MODE_NORMAL].reg);

    switch (powerMode)
    {
        case PMD_LOWPOWER:
//            pmd_RDAWrite(RDA_ADDR_LDO_LP_SETTING1, 0xe770);
//            pmd_RDAWrite(RDA_ADDR_LDO_LP_SETTING2, 0xf203);
//            pmd_RDAWrite(RDA_ADDR_LDO_LP_SETTING3, 0x4124);            
#ifdef __PRJ_WITH_SPIFLSH__
            memd_FlashSleep();
#endif
            break;
...
PRIVATE VOID pmd_SimSetVoltageClass(PMD_SIM_VOLTAGE_CLASS_T voltClass)
{
    ...
    g_pmdLdoActiveSetting5 = pmd_OpalSpiRead(RDA_ADDR_LDO_ACTIVE_SETTING5);
    g_pmdLdoActiveSetting5 &= ~(RDA1203_ACT_SIM1_SEL_MASK|RDA1203_ACT_SIM2_SEL_MASK);
    g_pmdLdoActiveSetting5 |= simVoltSel;

    pmd_SimSPIWrite(RDA_ADDR_LDO_ACTIVE_SETTING5, g_pmdLdoActiveSetting5); 
    pmd_SimSPIWrite(RDA_ADDR_LDO_LP_SETTING5, g_pmdLdoActiveSetting5); 

    hal_SysExitCriticalSection(scStatus);


    SIM1LDO = (enableLDO) ? RDA1203_ACT_VSIM4 : 0;
    SIMLDO = pmd_OpalSpiRead(RDA_ADDR_SIM_INTERFACE3);
    SIMLDO = SIMLDO & (~RDA1203_ACT_VSIM4) & (~RDA1203_ACT_VSIM3) & (~RDA1203_ACT_VSIM2) & (~RDA1203_ACT_VSIM1);
//    SIMLDO  |= SIM1LDO;
//    SIMLDO  |= (RDA1203_ACT_VSIM1 | RDA1203_ACT_VSIM2 | RDA1203_ACT_VSIM3 | RDA1203_ACT_VSIM4);
//    SIMLDO  |= (RDA1203_ACT_VSIM1 | RDA1203_ACT_VSIM4);
    SIMLDO  |= (RDA1203_ACT_VSIM3 | RDA1203_ACT_VSIM4);

    pmd_SimSPIWrite(RDA_ADDR_SIM_INTERFACE3, SIMLDO);


    SIM1LDO = (enableLDO) ? RDA1203_LP_LDO_VSIM4 : 0;
    SIMLDO = pmd_OpalSpiRead(RDA_ADDR_SIM_INTERFACE4);
    SIMLDO = SIMLDO & (~RDA1203_LP_LDO_VSIM1) & (~RDA1203_LP_LDO_VSIM2) & (~RDA1203_LP_LDO_VSIM3) & (~RDA1203_LP_LDO_VSIM4);
//    SIMLDO  |= SIM1LDO;
//    SIMLDO  |= (RDA1203_LP_LDO_VSIM1 | RDA1203_LP_LDO_VSIM2 | RDA1203_LP_LDO_VSIM3 | RDA1203_LP_LDO_VSIM4);
//    SIMLDO  |= (RDA1203_ACT_VSIM1 | RDA1203_ACT_VSIM4);
    SIMLDO  |= (RDA1203_ACT_VSIM3 | RDA1203_ACT_VSIM4);

    pmd_SimSPIWrite(RDA_ADDR_SIM_INTERFACE4, SIMLDO);

}

DUALSIM_ERR_T pmd_FourSimSetVoltageClass(DUALSIM_SIM_VOLTAGE_T voltClass0,DUALSIM_SIM_VOLTAGE_T voltClass1, 
												DUALSIM_SIM_VOLTAGE_T voltClass2,DUALSIM_SIM_VOLTAGE_T voltClass3)
{
    ...
    BOOL enableSim3LDO = TRUE, enableSim4LDO = TRUE;	
    UINT16 SIM1LDO, SIM2LDO, SIM3LDO, SIM4LDO;	
    UINT16 SIMLDO = 0;	
    UINT32 simVoltSel = 0;
    UINT16 LdoActiveSetting5;

    //UINT32 VoltErr_statusEDGE = 0x55eeeeee;
    ...
    switch (voltClass2)
    {
        case DUALSIM_SIM_VOLTAGE_NULL:
            enableSim3LDO = FALSE;
            simVoltSel |= RDA1203_ACT_SIM3_SEL1V8;    
            PMD_TRACE(PMD_INFO_TRC, 0, "opal_set_sim2_votage:0V");    
            break;

        case DUALSIM_SIM_VOLTAGE_CLASS_C:
            simVoltSel |= RDA1203_ACT_SIM3_SEL1V8;    
            PMD_TRACE(PMD_INFO_TRC, 0, "opal_set_sim2_votage:1V8");           
            break;

        case DUALSIM_SIM_VOLTAGE_CLASS_B:
            simVoltSel |= RDA1203_ACT_SIM3_SEL2V8;        
            PMD_TRACE(PMD_INFO_TRC, 0, "opal_set_sim2_votage:3V");
            break;

        case DUALSIM_SIM_VOLTAGE_CLASS_A:
            simVoltSel |= RDA1203_ACT_SIM3_SEL2V8;        
            PMD_TRACE(PMD_INFO_TRC, 0, "opal_set_sim2_votage:5V");
            break;

        default:
            PMD_ASSERT(FALSE, "opal_set_sim_votage invalid voltage class %d", voltClass2);                    
        break;
    }
    ...
    switch (voltClass3)
    {
        case DUALSIM_SIM_VOLTAGE_NULL:
            enableSim4LDO = FALSE;
            simVoltSel |= RDA1203_ACT_SIM4_SEL1V8;    
            PMD_TRACE(PMD_INFO_TRC, 0, "opal_set_sim3_votage:0V");    
            break;

        case DUALSIM_SIM_VOLTAGE_CLASS_C:
            simVoltSel |= RDA1203_ACT_SIM4_SEL1V8;    
            PMD_TRACE(PMD_INFO_TRC, 0, "opal_set_sim3_votage:1V8");           
            break;

        case DUALSIM_SIM_VOLTAGE_CLASS_B:
            simVoltSel |= RDA1203_ACT_SIM4_SEL2V8;        
            PMD_TRACE(PMD_INFO_TRC, 0, "opal_set_sim3_votage:3V");
            break;

        case DUALSIM_SIM_VOLTAGE_CLASS_A:
            simVoltSel |= RDA1203_ACT_SIM4_SEL2V8;        
            PMD_TRACE(PMD_INFO_TRC, 0, "opal_set_sim3_votage:5V");
            break;

        default:
            PMD_ASSERT(FALSE, "opal_set_sim_votage invalid voltage class %d", voltClass3);                    
        break;
    }

    UINT32 scStatus = hal_SysEnterCriticalSection();

    SIM1LDO = (enableSim1LDO) ? RDA1203_LDO_SIM1 : 0;
    SIM2LDO = (enableSim2LDO) ? RDA1203_LDO_SIM2 : 0;
    SIM3LDO = (enableSim3LDO) ? RDA1203_LDO_SIM3 : 0;
    SIM4LDO = (enableSim4LDO) ? RDA1203_LDO_SIM4 : 0;

    SIMLDO = pmd_OpalSpiRead(RDA_ADDR_LDO_SETTINGS);
    SIMLDO = SIMLDO & (~RDA1203_LDO_SIM1) & (~RDA1203_LDO_SIM2) & (~RDA1203_LDO_SIM3) & (~RDA1203_LDO_SIM4);
    SIMLDO  |= (SIM1LDO | SIM2LDO | SIM3LDO | SIM4LDO);

    SIMLDO  |= (RDA1203_LDO_SIM1 | RDA1203_LDO_SIM2 | RDA1203_LDO_SIM3 | RDA1203_LDO_SIM4);

    pmd_SimSPIWrite(RDA_ADDR_LDO_SETTINGS, SIMLDO);


    SIM1LDO = (enableSim1LDO) ? 0 : RDA1203_ACT_VSIM1 ;
    SIM2LDO = (enableSim2LDO) ? 0 : RDA1203_ACT_VSIM2;
    SIM3LDO = (enableSim3LDO) ? 0 : RDA1203_ACT_VSIM3;
    SIM4LDO = (enableSim4LDO) ? 0 : RDA1203_ACT_VSIM4;
...
//    SIMLDO  |= (RDA1203_ACT_VSIM3 | RDA1203_ACT_VSIM4);
    ...
    SIM1LDO = (enableSim1LDO) ? 0 : RDA1203_LP_LDO_VSIM1 ;
    SIM2LDO = (enableSim2LDO) ? 0 : RDA1203_LP_LDO_VSIM2;
    SIM3LDO = (enableSim3LDO) ? 0 : RDA1203_LP_LDO_VSIM3;
    SIM4LDO = (enableSim4LDO) ? 0 : RDA1203_LP_LDO_VSIM4;
...
//    g_pmdLdoActiveSetting5 = pmd_OpalSpiRead(RDA_ADDR_LDO_SETTINGS);
//    hal_HstSendEvent(1);
//    hal_HstSendEvent(g_pmdLdoActiveSetting5);
//    hal_HstSendEvent(voltClass0);
//    hal_HstSendEvent(voltClass1);
//    hal_HstSendEvent(voltClass2);
//    hal_HstSendEvent(voltClass3);
//    g_pmdLdoActiveSetting5 &= ~(0x3f8);
//    pmd_SimSPIWrite(RDA_ADDR_LDO_SETTINGS, g_pmdLdoActiveSetting5); 


    g_pmdLdoActiveSetting5 = pmd_OpalSpiRead(RDA_ADDR_LDO_ACTIVE_SETTING5);
    g_pmdLdoActiveSetting5 &= ~(RDA1203_ACT_SIM1_SEL_MASK|RDA1203_ACT_SIM2_SEL_MASK|RDA1203_ACT_SIM3_SEL_MASK|RDA1203_ACT_SIM4_SEL_MASK);
    g_pmdLdoActiveSetting5 |= simVoltSel;


    LdoActiveSetting5 = pmd_OpalSpiRead(RDA_ADDR_LDO_LP_SETTING5);
    LdoActiveSetting5 &= ~(RDA1203_ACT_SIM1_SEL_MASK|RDA1203_ACT_SIM2_SEL_MASK|RDA1203_ACT_SIM3_SEL_MASK|RDA1203_ACT_SIM4_SEL_MASK);
    LdoActiveSetting5 |= simVoltSel;
    pmd_SimSPIWrite(RDA_ADDR_LDO_LP_SETTING5, LdoActiveSetting5); 
...
{
    g_pmdSimSelect = g_pmdSimSelect & (~RDA1203_SIM_SELECT_MASK);

#ifdef CT1129_OLD_SIM
    if (sim_card == 0)
    {
        g_pmdSimSelect = g_pmdSimSelect | RDA1203_SIM_SELECT_SIM4;
    }
    if (sim_card == 1)
    {
         g_pmdSimSelect = g_pmdSimSelect | RDA1203_SIM_SELECT_SIM3;
    }
    if (sim_card == 2)
    {
        g_pmdSimSelect = g_pmdSimSelect | RDA1203_SIM_SELECT_SIM2;
    }
    if (sim_card == 3)
    {
         g_pmdSimSelect = g_pmdSimSelect | RDA1203_SIM_SELECT_SIM1;
    }
#else
    if (sim_card == 0)
    {
    ...
    if (pmd_SimSPIWrite(RDA_ADDR_SIM_INTERFACE2, g_pmdSimSelect) == FALSE)
    {
        return FALSE;
    }
...
#if 0
VOID tsd_EnableRdaPen()
{
    if (FALSE == pmd_TSDGetSPI())
    {
        sxr_StartFunctionTimer(1 * HAL_TICK1S,tsd_EnableRdaPen,(VOID*)NULL,0x03);
    }
...
PUBLIC VOID pmd_SetLevel(PMD_LEVEL_ID_T id, UINT32 level)
{
    ...
    if (id == PMD_LEVEL_LCD)
    {
        ...
        else
        {
            ...
            if (g_pmdChargerDcOnAtInit)
            {
                g_pmdChargerDcOnAtInit = FALSE;
                pmd_ChargerPlugIn();
            }
        }
    }
...
{ 
    ...
    ret = ret & (~RDA1203_LDO_TSC);
    ret = ret | RDA1203_LDO_SIM2 | RDA1203_LDO_SIM1 |RDA1203_LDO_MIC | RDA1203_LDO_VIBRATE|RDA1203_LDO_USB;
...
//    pmd_RegWrite(RDA_ADDR_LDO_LP_SETTING3, g_pmdLdoActiveSetting3);

    vcore = g_pmdLdoVcoreValueActive;
    g_pmdLDO_Out_V_Ctrl[OPAL_PROFILE_MODE_NORMAL] = (g_pmdLDO_Out_V_Ctrl[OPAL_PROFILE_MODE_NORMAL] &
                                ~RDA1203_LDO_VCORE_CURRENT_MASK) |
                             RDA1203_LDO_VCORE_CURRENT(vcore);

    pmd_OpalSpiWrite(RDA_ADDR_LDO_ACTIVE_SETTING2, g_pmdLDO_Out_V_Ctrl[OPAL_PROFILE_MODE_NORMAL]);
//    pmd_OpalSpiWrite(RDA_ADDR_LDO_LP_SETTING2, g_pmdLDO_Out_V_Ctrl[OPAL_PROFILE_MODE_NORMAL]);
...
//    vcore = 4;
    ...
    g_pmdLDO_Out_V_Ctrl[OPAL_PROFILE_MODE_LOWPOWER] = (g_pmdLDO_Out_V_Ctrl[OPAL_PROFILE_MODE_LOWPOWER] &
                                ~RDA1203_LDO_VCORE_CURRENT_MASK) |
                             RDA1203_LDO_VCORE_CURRENT(vcore);
    ...
    // Enter active mode
    g_pmdIrqSettingLatest = 0;
    pmd_RegIrqSettingSet(RDA1203_EOMIRQ_MASK|RDA1203_PENIRQ_MASK|RDA1203_PD_MODE_SEL);
...
{
    pmd_RegIrqSettingSet(RDA1203_EOMIRQ_CLEAR);
}
...
{
    pmd_RegIrqSettingSet(RDA1203_EOMIRQ_CLEAR|RDA1203_PENIRQ_CLEAR);
}
...
BOOL pmd_TsdEnableIrq(VOID)
{
    ...
    // unmask pen irq
    pmd_RegIrqSettingClr(RDA1203_PENIRQ_MASK);
...
BOOL pmd_TsdDisableIrq(VOID)
{
    ...
    pmd_RegIrqSettingSet(RDA1203_PENIRQ_MASK);
...
PUBLIC VOID pmd_IrqHandler(UINT8 interruptId)
{
    ...
    PMD_TRACE(PMD_DBG_TRC | TSTDOUT, 0, "pmd_IrqHandler");
    return;
}
...
PROTECTED RDA1203_CHR_STATUS_T pmd_GetChargerHwStatus(VOID)
{
    ...
    { 
        ...
        pmd_RegIrqSettingClr(RDA1203_INT_CHR_MASK);
    }
    else
    {
...
UINT32 g_pmdSpiRegValue[PMU_REG_NUM] PMD_SPI_REG_VALUE_LOCATION;
