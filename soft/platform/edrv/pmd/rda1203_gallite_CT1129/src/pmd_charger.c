...
extern UINT32 g_pmdIrqSettingLatest;
...
PRIVATE VOID pmd_TuneCharger(CHG_TUNING_STAGE_T stage)
{
    ...
    {
        ret = pmd_OpalSpiRead(RDA_ADDR_SIM_INTERFACE1);
        ret = ret | RDA1203_SELF_CAL_ENABLE_DR | RDA1203_SELF_CAL_ENABLE_REG;
        pmd_OpalSpiWrite(RDA_ADDR_SIM_INTERFACE1, ret);
...
PRIVATE VOID pmd_ForceChargerFinished(BOOL stop)
{
    if (stop)
    {
        pmd_RegIrqSettingSet(RDA1203_CHR_DISABLE);
    }
    else
    {
        pmd_RegIrqSettingClr(RDA1203_CHR_DISABLE);
    }
}
...
// ============================================================================
// pmd_GetEffuseGpadcLow
// ----------------------------------------------------------------------------
/// Read effuse value of GPADC calib low sample.
/// 
// ============================================================================
PRIVATE VOID pmd_GetEffuseGpadcLow(UINT32 *pData)
{
    UINT16 ret;

    ret = pmd_OpalSpiRead(RDA_ADDR_THERMAL_CALIBRATION);
    
    ret = (ret & ~0xc0);
    ret = (ret |0x40);
    pmd_OpalSpiWrite(RDA_ADDR_THERMAL_CALIBRATION, ret);
    pmd_OpalSpiWrite(RDA_ADDR_EFUSE_OPT_SETTING2, 0x7800);
    pmd_OpalSpiWrite(RDA_ADDR_EFUSE_OPT_SETTING1, 0x9000);
    pmd_OpalSpiWrite(RDA_ADDR_EFUSE_OPT_SETTING1, 0x9800);
    pmd_OpalSpiWrite(RDA_ADDR_EFUSE_OPT_SETTING1, 0x9000);
    
    *pData = pmd_OpalSpiRead(RDA_ADDR_EFUSE_OPT_SETTING4);
    pmd_OpalSpiWrite(RDA_ADDR_EFUSE_OPT_SETTING2, 0x7801);
...
// ============================================================================
// pmd_GetEffuseGpadcHigh
// ----------------------------------------------------------------------------
/// Read effuse value of GPADC calib high sample.
/// 
// ============================================================================
PRIVATE VOID pmd_GetEffuseGpadcHigh(UINT32 *pData)
{
    UINT16 ret;    
    ret = pmd_OpalSpiRead(RDA_ADDR_THERMAL_CALIBRATION);   
    ret = (ret & ~0xc0);
...
{
    pmd_GetEffuseGpadcHigh(hal_AnaGpadcGetCalibHigh());
    pmd_GetEffuseGpadcLow(hal_AnaGpadcGetCalibLow());
}
...
PUBLIC VOID pmd_InitCharger(VOID)
{
    ...
    chr_vfb_sel = 0;
#endif
    pmd_OpalSpiWrite(RDA_ADDR_EFUSE_OPT_SETTING2, 0x3200);
