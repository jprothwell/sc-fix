...
////////////////////////////////////////////////////////////////////////////////
//                                                                            //
//  $HeadURL: http://svn.coolsand-tech.com/svn/developing1/Sources/edrv/trunk/lcdd/s6d0154/ebc/src/drv_lcd_s6d0154.c $ //
//  $Author: pengzg $                                                        // 
...
//PRIVATE CONST void* g_lcddConfig = NULL;
#ifdef __ADS__

#pragma arm section zidata ="lcdd.globalvars.cbss"
PRIVATE CONST LCDD_CONFIG_T* g_lcddConfig = NULL;
//        __attribute__((section (".lcdd.globalvars.c")));
        
/// Main screen base address
PRIVATE UINT32 g_lcddCsBase  = 0;
//PRIVATE UINT32 g_lcddCsBase __attribute__((section (".lcdd.globalvars.c"))) = 0;

/// Main screen offset address to access the second register.
PRIVATE UINT32 g_lcddCsOffset  = 0;
//PRIVATE UINT32 g_lcddCsOffset __attribute__((section (".lcdd.globalvars.c"))) = 0;

/// Subscreen base address
/// Let public to be usable by the util driver
 UINT32 g_lcddCsSubBase  = 0;
// UINT32 g_lcddCsSubBase __attribute__((section (".lcdd.globalvars.c"))) = 0;

/// Sub screen offset address to access the second register.
PRIVATE UINT32 g_lcddCsSubOffset  = 0;
//PRIVATE UINT32 g_lcddCsSubOffset __attribute__((section (".lcdd.globalvars.c"))) = 0;
...
//PRIVATE UINT32 g_lcddLock __attribute__((section (".lcdd.globalvars.c"))) = 0;

/// Lock to protect the access to the LCD SUB screen during a DMA access.
/// When 1, access is allowed. When 0, it is denied.
PRIVATE UINT32 g_lcddSubLock  = 0;
//PRIVATE UINT32 g_lcddSubLock __attribute__((section (".lcdd.globalvars.c"))) = 0;
...
// =============================================================================
void lcdd_MutexFree(UINT32* mutex)
...
char gouda_memory[LCDD_DISP_X*LCDD_DISP_Y*2];
extern void gouda_init();
...
{
    g_lcddConfig = (LCDD_CONFIG_T*)tgt_GetLcddConfig();
    g_lcddCsBase     = (UINT32) gouda_memory;
//    g_lcddCsBase     = (UINT32) hal_EbcCsOpen(g_lcddConfig->main.csNum, &g_lcddConfig->main.csConfig);
    ...
    {
        memset(gouda_memory, 0xff, LCDD_DISP_X*LCDD_DISP_Y*2);
        gouda_init();
...
// Power On Sequence
    ...
    LCM_WR_REG(0xb4, 0x0000);
    LCM_WR_REG(0x12, 0x00b3);
    LCM_WR_REG(0x13, 0x0808);
    LCM_WR_REG(0x14, 0x7fe0);
    LCM_WR_REG(0x10, 0x1a6c);

    // Delay 80ms
    hal_TimDelay(80 MS_WAITING);

    LCM_WR_REG(0x13, 0x081e);

    // Delay 20ms
    hal_TimDelay(20 MS_WAITING);

    LCM_WR_REG(0x01, 0x0114);
    LCM_WR_REG(0x02, 0x0100);
    LCM_WR_REG(0x03, 0x0030);
    LCM_WR_REG(0x08, 0x0202);
    LCM_WR_REG(0x0b, 0x0000);
    LCM_WR_REG(0x0c, 0x0001);
    LCM_WR_REG(0x30, 0x0303);
    LCM_WR_REG(0x31, 0x0303);
    LCM_WR_REG(0x32, 0x0303);
    LCM_WR_REG(0x33, 0x0000);
    LCM_WR_REG(0x34, 0x0404);
    LCM_WR_REG(0x35, 0x0404);
    LCM_WR_REG(0x36, 0x0404);
    LCM_WR_REG(0x37, 0x0000);
    LCM_WR_REG(0x38, 0x0707);
    LCM_WR_REG(0x40, 0x0000);
    LCM_WR_REG(0x42, 0x9f00);
    LCM_WR_REG(0x43, 0x0000);
    LCM_WR_REG(0x44, 0x7f00);
    LCM_WR_REG(0x45, 0x9f00);
    LCM_WR_REG(0x61, 0x001D);
    LCM_WR_REG(0x69, 0x0000);
    LCM_WR_REG(0x70, 0x0000);
    LCM_WR_REG(0x71, 0x0000);
...
//          LCM_WR_REG(0x44, (g_lcddActiveWin.hea << 8) | g_lcddActiveWin.hsa);
//          LCM_WR_REG(0x45, (g_lcddActiveWin.vea << 8) | g_lcddActiveWin.vsa);
...
//            LCM_WR_REG(0x45, (g_lcddActiveWin.vea << 8) | g_lcddActiveWin.vsa);
            ...
            if (frameBufferWin->roi.width == frameBufferWin->fb.width)
            {
                ...
                UINT16 curLine=0;
                UINT16 startLine = startY;
                UINT16 endLine = startY+frameBufferWin->roi.height-1;
                ...
                UINT16* curBuf = (UINT16*)frameBufferWin->fb.buffer;
                UINT16* desBuf = (UINT16*)(g_lcddCsBase
                    +(startY * 240*2)
                    +(startX*2));
                ...
                for (curLine=startLine; curLine<=endLine; curLine++)
                {
                    ...
                    if (1)
                    {
                    	memcpy(desBuf,curBuf,frameBufferWin->fb.width*2);
...
//                        lcdd_TransferData(curBuf, frameBufferWin->roi.width, FALSE);
            ...
            if (frameBufferWin->roi.width == frameBufferWin->fb.width)
            {
                // The source buffer and the roi in the buffer have the same width, we can
...
// here too.       
//        pmd_SetLevel(PMD_LEVEL_LCD, 0x80);
        hal_SysResetOut(FALSE);
...
PUBLIC LCDD_ERR_T lcdd_SubSetStandbyMode(BOOL standbyMode)
{
    ...
    // #warning This function may not be implemented yet: we need the stand
    //by mode ...
}
