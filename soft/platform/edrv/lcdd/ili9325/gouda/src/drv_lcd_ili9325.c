...
////////////////////////////////////////////////////////////////////////////////
//                                                                            //
//  $HeadURL: http://svn.coolsand-tech.com/svn/developing1/Sources/edrv/trunk/lcdd/ili9325/gouda/src/drv_lcd_ili9325.c $ //
//	$Author: qiff $                                                        // 
...
PRIVATE VOID lcddp_Init(VOID)
{
...
#else
	LCM_WR_REG(0x00E5, 0x78F0); // Set internal timing
	LCM_WR_REG(0x0001, 0x0100); // set SS and SM bit
	LCM_WR_REG(0x0002, 0x0700); // set 1 line inversion
	LCM_WR_REG(0x0003, 0x1030); // set GRAM write direction and BGR=1.
	LCM_WR_REG(0x0004, 0x0000); // Resize register
	...
	LCM_WR_REG(0x0009, 0x0000); // set non-display area refresh cycle ISC[3:0]
	LCM_WR_REG(0x000A, 0x0000); // FMARK function
    LCM_WR_REG(0x000C, 0x0000);  //0110// RGB interface setting
	LCM_WR_REG(0x000D, 0x0000); // Frame marker Position
	LCM_WR_REG(0x000F, 0x0000); // RGB interface polarity
	//*************Power On sequence ****************//
	LCM_WR_REG(0x0010, 0x0000); // SAP, BT[3:0], AP, DSTB, SLP, STB
	...
	LCM_WR_REG(0x0012, 0x0000); // VREG1OUT voltage
	LCM_WR_REG(0x0013, 0x0000); // VDV[4:0] for VCOM amplitude
	sxr_Sleep(200); // Dis-charge capacitor power voltage
	...
	sxr_Sleep(50); // Delay 50ms
	LCM_WR_REG(0x0012, 0x008E); // External reference voltage= Vci;
	sxr_Sleep(50); // Delay 50ms
	LCM_WR_REG(0x0013, 0x1E00); // R13=1200 when R12=009D;VDV[4:0] for VCOM amplitude
	LCM_WR_REG(0x0029, 0x0008); // R29=000C when R12=009D;VCM[5:0] for VCOMH
	LCM_WR_REG(0x002B, 0x000D); // Frame Rate = 91Hz
	sxr_Sleep(50); // Delay 50ms
	LCM_WR_REG(0x0020, 0x0000); // GRAM horizontal Address
	LCM_WR_REG(0x0021, 0x0000); // GRAM Vertical Address
	// ----------- Adjust the Gamma Curve ----------//
	LCM_WR_REG(0x0030, 0x0000);
	LCM_WR_REG(0x0031, 0x0405);
	LCM_WR_REG(0x0032, 0x0103);
	LCM_WR_REG(0x0035, 0x0107);
	LCM_WR_REG(0x0036, 0x0004);
	LCM_WR_REG(0x0037, 0x0406);
	LCM_WR_REG(0x0038, 0x0203);
	LCM_WR_REG(0x0039, 0x0707);
...
PRIVATE BOOL lcddp_CheckProductId()
{
    ...
    SXS_TRACE(TSTDOUT, "ili9325(0x%x): lcd read id is 0x%x ", LCD_ILI9325_ID, productId);
