...
static UINT32 base_id = MSG_ID_BT_NOTIFY_EVM_IND;
const char * const bt_debug_sap[] = {
"MSG_ID_BT_NOTIFY_EVM_IND = BT_MSG_CODE_BEGIN",
"MSG_ID_BT_POWERON_REQ",//1060737
...
void Debug_BT_Sap(BOOL dir,module_type src_mod,UINT32 msgid)
{
	if (msgid <= base_id )//|| msgid > (base_id + (sizeof (bt_debug_sap))>>2))
	{
	//	mmi_trace(1,"message id = %d\n",msgid);
		return;
	}
	if (dir)
		mmi_trace(1,"module(%d) send message id = %d is %s at  tick %d \n",src_mod,msgid,bt_debug_sap[msgid-base_id],hal_TimGetUpTime());
	else
		mmi_trace(1,"recive message id = %d is %s at  tick %d form module(%d)\n",msgid,bt_debug_sap[msgid-base_id],hal_TimGetUpTime(),src_mod);
}
