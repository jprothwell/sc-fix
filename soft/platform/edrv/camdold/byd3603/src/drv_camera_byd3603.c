...
PRIVATE void camerap_PowerOn(CAM_SIZE_T CamOutSize, UINT16 IspOutWidth, UINT16 IspOutHeight, CAM_FORMAT_T Format)
{
    ...
    CamConfig.camClkDiv = 9; // 156MHz/6 = 26MHz
...
{
   static char byd3603_id_str[] = "byd3603\n"; 
   return byd3603_id_str; 
}
// ============================================================================
...
PRIVATE BOOL camerap_CheckProductId()
{
   ...
   // Disable Camera Interface Module
    ...
    SXS_TRACE(TSTDOUT,"byd3603(0x%02x): read id is 0x%02x", CAM_BYD3603_ID, cameraID);
    if(cameraID == CAM_BYD3603_ID)
        return TRUE;
    else
        return FALSE;
}
...
/// @return #TRUE, #FALSE
// ============================================================================
PUBLIC BOOL camera_byd3603_RegInit(CAM_DRV_T  *pCamDrv)
{
