 ...
 *
 * SOURCE CONTROL: $Id: gpio_iic.c 1532 2010-08-23 05:47:34Z huazeng $
 *
 * LICENSE:
...
BOOL pas_gpio_i2c_init_sensor(VOID)
{
  if (init_first == FALSE)
  {
    init_first = TRUE;
 ...
 // SCL_PIN = (25 << 1) + 0;
 // SDA_PIN = (24 << 1) + 0;
    SCL_PIN = ((tgt_GetGpioi2cConfig()->scl_i2c_gpio) & 0xffff) << 1;
    SDA_PIN = ((tgt_GetGpioi2cConfig()->sda_i2c) & 0xffff) << 1;
...
//  UINT32 scStatus = hal_SysEnterCriticalSection();
  ...
  if(1==	gpio_i2c_TxByte(addr<<1)) { return FALSE;}//hal_SysExitCriticalSection(scStatus);}//chip adress


	for(i=addrlen;i>0;i--,regaddr++)//addr
	{
		if(gpio_i2c_TxByte(*regaddr) == 1) { return FALSE;}//hal_SysExitCriticalSection(scStatus);}
	}


	for( i=datalen;i>0;i--,data++)//data
	{
		if(gpio_i2c_TxByte(*data) == 1){ return FALSE;}//hal_SysExitCriticalSection(scStatus);}
	}
	gpio_i2c_Stop();
...
//  UINT32 scStatus = hal_SysEnterCriticalSection();
	 ...
	 gpio_i2c_Start();//start
	if(1== gpio_i2c_TxByte((addr<<1)+0))
	{
         //   hal_DbgAssert("no ack");
	}
	if(1== gpio_i2c_TxByte(*regaddr))
       {  
         //       hal_DbgAssert("no ack");
	}
        gpio_i2c_Stop();
