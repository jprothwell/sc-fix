...
PRIVATE BOOL camera_load_drv(VOID)
{
    ...
    if ((*pFS).moduleId.cam_id[gCameraID] < CAM_REGINIT_QTY)
    {// Valid ID
        if(g_CamRegInit[(*pFS).moduleId.cam_id[gCameraID]](pCamDrv))
        {        
            return TRUE;
        }
    }
    ...
    {	  
        ...
        {		
            ...
            factorySettings = *pFS;
            factorySettings.moduleId.cam_id[gCameraID] = i;
...
PUBLIC VOID camera_GetImageInfo(UINT32 * pcamout,UINT16 *pwidth,UINT16 *pheight)
{	
	if ((pCamDrv->camera_GetImageInfo != NULL))
	{
		return (pCamDrv->camera_GetImageInfo(pcamout,pwidth,pheight));
	}
}
