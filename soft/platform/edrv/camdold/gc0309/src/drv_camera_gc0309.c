...
PRIVATE void camerap_PowerOn(CAM_SIZE_T CamOutSize, UINT16 IspOutWidth, UINT16 IspOutHeight, CAM_FORMAT_T Format)
{
	...
	{
		camerap_WriteOneReg(0x14, 0x12);
	}
	else
	{
		camerap_WriteOneReg(0x14,0x13 );
	}
#endif
    // Sensor Init seq
}
...
{         
    ...
    {
        case 1:
            camerap_WriteOneReg(0xb3, 0x60);	      
            break;
        case 2:
            camerap_WriteOneReg(0xb3, 0x54);	   
            break;
        case 3:
            camerap_WriteOneReg(0xb3, 0x46);	      
            break;
        case 4:
            camerap_WriteOneReg(0xb3, 0x38);	      
            break;
        case 5:
            camerap_WriteOneReg(0xb3, 0x34);	     
            break;
        default: break;
    }
}
...
{//Actually this is the same with camera_SetBrightnessL
     switch(Exposure)
    {
        ...
        case CAM_EXPOSURE_NEGATIVE_2:   //-2
            //camera_WriteOneReg(0x7a, 0xe0);
            //camera_WriteOneReg(0xd1, 0x40);
            camerap_WriteOneReg(0xb5, 0xf0); 
            camerap_WriteOneReg(0xd3, 0x48); 
            break;
        case CAM_EXPOSURE_NEGATIVE_1: //-1
            // camera_WriteOneReg(0x7a, 0xf0);
            //camera_WriteOneReg(0xd1, 0x48);
            camerap_WriteOneReg(0xb5, 0x00); 
            camerap_WriteOneReg(0xd3, 0x70); 
            break;
...
PRIVATE void camerap_SetWhiteBalanceL(CAM_WHITEBALANCE_T WhiteBalance)
{
    ...
    switch(WhiteBalance)
    {
        case CAM_WB_AUTO:  /*Auto */
            //camerap_WriteOneReg(0x5a,0x56); //for AWB can adjust back
            camerap_WriteOneReg(0x5a,0x4c); 
            camerap_WriteOneReg(0x5b,0x40);
            camerap_WriteOneReg(0x5c,0x4a);			
            //camerap_WriteOneReg(0x22,TempReg|0x02);
            camerap_WriteOneReg(0x22,0x57);
            break;
        case CAM_WB_DAYLIGHT:   /*ÇçÌìÌ«Ñô¹â*/
            camerap_WriteOneReg(0x22,TempReg&~0x02);   
            camerap_WriteOneReg(0x5a,0x74);  //  50 45 40
            camerap_WriteOneReg(0x5b,0x52);
        ...
        case CAM_WB_CLOUDY:  /*ÒõÌì*/
            camerap_WriteOneReg(0x22,TempReg&~0x02);   // Disable AWB 
            camerap_WriteOneReg(0x5a,0x8c); //WB_manual_gain // 5a 42 40
            camerap_WriteOneReg(0x5b,0x50);
            camerap_WriteOneReg(0x5c,0x40);
            break;		
        case CAM_WB_TUNGSTEN:  /*ÎÙË¿µÆ*/
            camerap_WriteOneReg(0x22,TempReg&~0x02); 
            camerap_WriteOneReg(0x5a,0x40);
            camerap_WriteOneReg(0x5b,0x54);
            camerap_WriteOneReg(0x5c,0x70);
            break;
        default : break;
    }
 }
...
PRIVATE VOID camerap_SetSpecialEffect(CAM_SPECIALEFFECT_T SpecialEffect)
{
    ...
    switch(SpecialEffect)
    {
        case CAM_SPE_NORMAL	:
            ...
            camerap_WriteOneReg(0x2d,0x0a); 
            camerap_WriteOneReg(0x20,0x7f);
            camerap_WriteOneReg(0xd2,0x90);
            camerap_WriteOneReg(0x73,0x00);
            camerap_WriteOneReg(0x77,0x38);
			
            camerap_WriteOneReg(0xb3,0x40);
            camerap_WriteOneReg(0xb4,0x80);
        ...
        case CAM_SPE_ANTIQUE	:	
            ...
            camerap_WriteOneReg(0x20,0x7f);
            camerap_WriteOneReg(0xd2,0x90);
            camerap_WriteOneReg(0x77,0x88);

            camerap_WriteOneReg(0xb3,0x40);
            camerap_WriteOneReg(0xb4,0x80);
            camerap_WriteOneReg(0xba,0x80);
            camerap_WriteOneReg(0xbb,0x20);
#endif
            break;
        case CAM_SPE_REDISH	:
            camerap_WriteOneReg(0x23,0x02);
        ...
        case CAM_SPE_GREENISH:	
            ...
            camerap_WriteOneReg(0xb3,0x40);
            camerap_WriteOneReg(0xb4,0x80);
            camerap_WriteOneReg(0xba,0xc0);
            camerap_WriteOneReg(0xbb,0xc0);
            break;
...
PRIVATE VOID camerap_SetNightMode(UINT32 NightMode)
{
    #if 0
     INT8 readregval;
    readregval=camerap_ReadReg(0x20);

    if(NightMode==0)
    {
		camerap_WriteOneReg(0xec  ,0x30);
		camerap_WriteOneReg(0x20  ,readregval&0x5f);   // close cc
		camerap_WriteOneReg(0x3c  ,0x08);
		camerap_WriteOneReg(0x3d  ,0x08);
		camerap_WriteOneReg(0x3e  ,0x08);
		camerap_WriteOneReg(0x3f  ,0x08);
    }	
    else
    {
		camerap_WriteOneReg(0xec  ,0x20);
		camerap_WriteOneReg(0x20  ,readregval|0x20);
		camerap_WriteOneReg(0x3c  ,0x02); 
		camerap_WriteOneReg(0x3d  ,0x02); 
		camerap_WriteOneReg(0x3e  ,0x02);
		camerap_WriteOneReg(0x3f  ,0x02); 
    }
   #endif 
...
{
   static char gc0309_id_str[] = "gc0309\n"; 
   return gc0309_id_str; 
}
// ============================================================================
...
PRIVATE BOOL camerap_CheckProductId()
{
    ...
    SXS_TRACE(TSTDOUT,"gc0309(0x%02x): read id is 0x%02x", CAM_GC0309_ID, cameraID);

    if(cameraID == CAM_GC0309_ID)
        return TRUE;
    else
        return FALSE;
}
PUBLIC BOOL camera_gc0309_RegInit(CAM_DRV_T  * pCamDrv)
{	
