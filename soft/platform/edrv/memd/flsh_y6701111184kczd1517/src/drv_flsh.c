...
// a11-a0 : 555 a21-a12:0021
// a11-a0 : 555 a21-a12:0022
//#define FLASH_CRD 0x22555
//#define FLASH_CRD 0x42555

//#define FLASH_CRD 0x02555

#define FLASH_CRD 0x01555
...
memd_FlashConfigureBurstMode(REG16* csBase, REG32* pCRE)
{
    ...
    // FLASH_CRD[13:11] : X Latency 2 = 30MHz / 3 = 40MHz / 4 = 54MHz / 5 = 66MHz? / 6 = 86MHz
    ...
    *(csBase+FLASH_CRD) = CMD_SET_CFG_REG_2;
    ...
    A[12:14] : wait count 000=reserved,001=3count,011=5count,111=9count
    A[15:16] : brust length , 00 continuous, 01 8words,10 16words.
    A[17] : 1 burst state and data is output on the rising edge of clk.
    A[18] : RDY 1 RDY active one clock cycle before data. 0 RDY active withe data
    A[19] : 0 synchronous ,1 asynchronous
  #endif
     // wait count register , may 3 count is 4 clk ,please take care on it
    *(csBase+0x555) = 0xaa;  
    *(csBase+0x2aa) = 0x55;
    *(csBase+FLASH_CRD) = 0xc0;
...
#ifdef MEMD_FLASH_IS_BURST
// SXS_TRACE(TSTDOUT, "memd_FlashOpen busrst");
...
//#define PSRAM_BCR 0x08510f
#define PSRAM_BCR 0x08591f

PRIVATE VOID HAL_FUNC_INTERNAL 
memd_RamConfigureBurstMode(REG16* csBase, REG32* pCRE)
{

   //  hal_HstSendEvent((UINT32)csBase);  // a0000000
UINT32 status;
 ...
 //UINT16 read;
    ...
    *(REG16*)(csBase + 0x1fffff)= 0x0001;
    *(REG16*)(csBase + 0x1fffff)= 0x510f;
...
//extern VOID test_vpp_flash_dma(VOID);
...
#ifdef MEMD_RAM_IS_BURST
 //SXS_TRACE(TSTDOUT, "memd_RamOpen busrst sort");
//hal_HstSendEvent(ssssss);
    hal_EbcSramOpen(&cfg->csConfig, memd_RamConfigureBurstMode);
 
  //  hal_HstSendEvent(0x99997);
...
PUBLIC MEMD_ERR_T memd_FlashErase(UINT8 *startFlashAddress, UINT8 *endFlashAddress)
{
    ...
    while (phy_Start != phys_end_add)
    {
	 ...
	 // Re-enable EBC write
        ...
        //*(ptr) = CMD_BLOCK_ERASE_2;//block adrress
        // Sector Erase command
        *(BankAddr+0x555) = 0xaa;
        *(BankAddr+0x2aa) = 0x55;
        *(BankAddr+0x555) = 0x80;
...
/// whether the operation succeeded or failed. ytt
// =============================================================================
        ...
        CONST UINT8* buffer)
{
  ...
  //UINT16 rdstatus;
   ...
   //SXS_TRACE(TSTDOUT, "memd_FlashWrite");
SXS_TRACE(TSTDOUT, "memd_FlashWriete prtr = 0x%x",ptr);
   // fast program set
    status = hal_SysEnterCriticalSection();
    *(BankAddr+0x555) = 0xaa;
    *(BankAddr+0x2aa) = 0x55;
