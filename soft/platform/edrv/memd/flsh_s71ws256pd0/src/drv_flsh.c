...
////////////////////////////////////////////////////////////////////////////////
//                                                                            //
//  $HeadURL: http://svn.coolsand-tech.com/svn/developing1/Sources/edrv/trunk/memd/flsh_s71ws256pd0/src/drv_flsh.c $ //
//	$Author: huazeng $                                                        // 
...
//                                                                            //
/// @file drv_flsh.c                                                          //
/// That file provides the driver implementation for the flsh_s71ws256pd0     //
/// flash.                                                                    //
//                                                                            //
////////////////////////////////////////////////////////////////////////////////
...
// Bank Number = Addr[23:21]
// Page Number = Addr[23:22]

// S71WS256PD0 is 32M Bytes flash.
// Unit of flash operation is byte. So the 32M bytes flash range is 0x000_0000 -> 0x1FF_FFFF
#define FLASHADDR_MASK    0xFE000000 // 0x0000_0000 to 0x0200_0000, ignore the 0x81000000 or 0xA1000000
#define FLASHBANK_MASK    0xFFE00000 // Bank is 2M byte -> 0x1F_FFFF, or 0xFFF<<(21)
...
// Page Size is 4M bytes, 2^22=4M. And the total access size is 32M, so the page number is 0->7.
...
PUBLIC MEMD_FLASH_LAYOUT_T* memd_FlashOpen(CONST MEMD_FLASH_CONFIG_T* cfg)
{
    ...
    // Open Flash, and store logical base address.
#ifdef MEMD_FLASH_IS_BURST
...
memd_RamConfigureBurstMode(REG16* csBase, REG32* pCRE)
{
    ...
    *pCRE=1; // MEM_FIFO_CRE
    *(csBase+0x08710f)=read; // read burst with latency = 6 fixed
...
PUBLIC MEMD_ERR_T memd_FlashErase(UINT8 *startFlashAddress, UINT8 *endFlashAddress)
{
    ...
    MEMD_ASSERT(phys_start_add <= FLASH_SIZE, 
        "flashAddress should be physical address and expected to be a byte offset within the flash chip!")
    ...
    else
    {
        ...
        MEMD_ASSERT(phys_end_add <= FLASH_SIZE, 
            "flashAddress should be physical address and expected to be a byte offset within the flash chip!")
...
/// 
/// @param  flashAddress The physical address and byte offset within the flash chip.
/// (Take care not to overwrite the code present at the beginning of the flash)
...
/// whether the operation succeeded or failed.
// =============================================================================
PUBLIC MEMD_ERR_T memd_FlashWrite(
    UINT8 *flashAddress,
    UINT32 byteSize, 
    ...
    CONST UINT8* buffer)
{
    ...
    //SXS_TRACE(TSTDOUT,"hal_flash: WriteAddr 0x%08x, WriteBytes: %d\n", flashAddress, byteSize);
    ...
    *pWrittenbyteSize=0;
    ...
    //SXS_TRACE(TSTDOUT,"hal_flash: Write ptr 0x%08x \n", ptr);
    if (!g_memdDybOverride)
    {
        if (memd_DYBCheck(BankAddr, (UINT16*)ptr))
        {
...
///
/// @param flashAddress The physical address and the byte offset within the flash chip
/// (Take care of the positions of the code in the flash)
...
/// @return #MEMD_ERR_NO
// =============================================================================
PUBLIC MEMD_ERR_T memd_FlashRead(
    UINT8  *flashAddress,
    UINT32  byteSize,
...
{
    MEMD_ASSERT((UINT32)startFlashAddress <= FLASH_SIZE, 
        "startflashAddress should be physical address and expected to be a byte offset within the flash chip!");
    MEMD_ASSERT((UINT32)endFlashAddress <= FLASH_SIZE,
        "endFlashAddress should be physical address and expected to be a byte offset within the flash chip!");
