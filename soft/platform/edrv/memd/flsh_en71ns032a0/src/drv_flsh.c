...
// -------------------------------
#define FLASH_SIZE  4*MBYTES // 16MB
...
memd_RamConfigureBurstMode(REG16* csBase, REG32* pCRE)
{
  ...
  // read=*(csBase+0x08590f);//spansion
   ...
   #if 0
   UINT16 bcrread;
    read=*(csBase);//latency 6 code. samsung
    *pCRE=1; // MEM_FIFO_CRE
    bcrread=*(csBase+0x80000); // read burst with latency = 6 fixed
   ...
   hal_HstSendEvent(0x2221443); hal_HstSendEvent((UINT32)read); hal_HstSendEvent((UINT32)bcrread); 
   hal_HstSendEvent(0x4444444);
   #endif
                 ...
                 if (suspend_time - now > max_time) 
                  {
                      ...
                      *(ptr) = 0xb0;
                      ...
                      do
                      {
                         rdstatus_old = (*ptr);
                         rdstatus = (*ptr);
                         rdstatus_old &= flash_uspend_flag;
                         rdstatus &= flash_uspend_flag;
                        
                      }while (!(rdstatus_old == flash_uspend_flag && rdstatus == flash_uspend_flag));	// Wait for suspend active
                      (*flashaddr) = 0xf0;
