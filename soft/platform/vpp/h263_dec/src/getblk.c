...
static VLCtab INTRA_DCT3Dtab0_list[] = 
#else
    static VLCtab INTRA_DCT3Dtab0[] = 
#endif
{
...
static VLCtab INTRA_DCT3Dtab1_list[] = 
#else
    static VLCtab INTRA_DCT3Dtab1[] = 
#endif
{
...
static VLCtab INTRA_DCT3Dtab2_list[] = 
#else
    static VLCtab INTRA_DCT3Dtab2[] = 
#endif
{
...
unsigned char zig_zag_scan_list[64]=
#else
...
char MQ_chroma_QP_table_list[32] = 
#else
    char MQ_chroma_QP_table[32] = 
#endif
{
...
#else
    store_code=(CodeCoeff*)h263_myMalloc(64*sizeof(CodeCoeff));
#endif
}
...
//                        diag_printf ( "DCT coeff index (i) out of bounds\n");
                ...
                /*
                  ...
                  j = alternate_horizontal_scan[i];//change shenh
                  //j = scan_tables[1][i];
                  break;
                  case INTRA_MODE_HORI_AC:
                  j = alternate_vertical_scan[i];//change shenh
                  //   j = scan_tables[2][i];
                  break;
                  default:
...
#if 0 //shenh
                if (comp >= 6)
                    QP = mmax (1, mmin (31, (/*bquant_tab[bquant]*/ (bquant+5)* quant) >> 2));
                else
                    QP = quant;
#endif
                QP = quant;
                ...
                */
                /* TMN3 dequantization */
                if ((QP % 2) == 1)
                    *qval = (signID ? -(QP * ( (val<<1) + 1)) : QP * ((val<<1) + 1));
                else
                    *qval = (signID ? -(QP * ((val<<1) + 1) - 1) : QP * ((val<<1) + 1) - 1);
                //}
