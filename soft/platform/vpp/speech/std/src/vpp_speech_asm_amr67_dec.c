...
Dec_amr_LoopStart_67:
	VoC_bne16_rd(Dec_amr_LoopEnd_671,REG5,CONST_160_ADDR);  	//for (i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR)
	VoC_jump(Dec_amr_LoopEnd_67);
Dec_amr_LoopEnd_671:
	...
	VoC_bne16_rd(Dec_amr_L14_67,REG5,CONST_80_ADDR);	//   if (sub (i_subfr, L_FRAME_BY2) == 0)
...
Dec_amr_L14_67:
	...
	VoC_lw16i_short(REG2,5,IN_PARALLEL);   		//     delta_frc_low = 5;
    	...
    	VoC_bnlt16_rd(Dec_amr_L18_67,REG6,CONST_20_ADDR);  	//     if (sub(t0_min, PIT_MIN) < 0)
    	VoC_lw16i_short(REG6,20,DEFAULT);       		//     {           t0_min = PIT_MIN;
Dec_amr_L18_67:          					//     }
    	VoC_add16_rr(REG7,REG6,REG1,DEFAULT);      		//     t0_max = add(t0_min, delta_frc_range);
    	VoC_bngt16_rd(Dec_amr_L19_67,REG7,CONST_143_ADDR);     //     if (sub(t0_max, PIT_MAX) > 0)
    	VoC_lw16i(REG7,143);		      		//     {        t0_max = PIT_MAX;
    	VoC_sub16_rr(REG6,REG7,REG1,DEFAULT);      		//        t0_min = sub(t0_max, delta_frc_range);
Dec_amr_L19_67:			          		//     }

   	VoC_jal(CII_Dec_lag3_67);      			//     Dec_lag3 (index, t0_min, t0_max, pit_flag, st->old_T0,&T0, &T0_frac, flag4);
   	VoC_sw16_d(REG6,STRUCT_DECOD_AMRSTATE_T0_LAGBUFF_ADDR); 	//     st->T0_lagBuff = T0;

   	VoC_bez16_d(Dec_amr_L20_67,DEC_AMR_BFI_ADDRESS);   	//  if (bfi != 0)
   	VoC_bnlt16_rd(Dec_amr_L20_67A,REG4,CONST_143_ADDR);   	//  {      if (sub (st->old_T0, PIT_MAX) < 0)
...
Dec_amr_L20_67:     					//  }
	...
	VoC_lw16inc_p(REG6,REG2,DEFAULT);		// {   MR67
	VoC_sw16_sd(REG2,3,DEFAULT);	 		//    index = *parm++;
	   						//    i = *parm++;
	VoC_jal(CII_decode_3i40_14bits);   		//    decode_3i40_14bits (i, index, code);
       							//    pit_sharp = shl (st->sharp, 1);  }
        VoC_lw16_d(REG5,STRUCT_DECOD_AMRSTATE_SHARP_ADDR);
	VoC_jal(CII_amr_dec_com_sub1);

	// begin of  dec_amr_not795122_s1_67    			// M475 ,515 ,59 ,67 ,74 ,102
	VoC_bez16_d(Dec_amr_L35_67A,DEC_AMR_BFI_ADDRESS);    	//   if (bfi == 0)
	VoC_jump(Dec_amr_L35_67);
Dec_amr_L35_67A:
...
// begin of dec_gain
	...
	VoC_lw16i_short(REG7,3,IN_PARALLEL);		// mode
...
Dec_amr_L36_67:
    	...
    	VoC_bngt16_rr(Dec_amr_L37_67,REG6,REG5);    		//   if (sub (pit_sharp, SHARPMAX) > 0)
    	VoC_movreg16(REG6,REG5,DEFAULT);    		//   {       pit_sharp = SHARPMAX;    }
Dec_amr_L37_67: 						//}
	// end of  dec_amr_not795122_s1_67
	...
	VoC_bngt16_rd(Dec_amr_L52_67,REG6,CONST_0x00004000L_ADDR); 	//if (sub (pit_sharp, 16384) > 0)
...
Dec_amr_L52_67:        						//}
	...
	VoC_bngt16_rr(Dec_amr_L54_67,REG7,REG0);   		//    if (sub (st->sharp, SHARPMAX) > 0)
...
Dec_amr_L54_67:						// }
	...
	VoC_bnez16_d(Dec_amr_L56_67,DEC_AMR_BFI_ADDRESS);   	//if ( bfi == 0 )
...
Dec_amr_L56_67:
	...
	VoC_jal(CII_Int_lsf);		   		//Int_lsf(prev_lsf, st->lsfState->past_lsf_q, i_subfr, lsf_i);
   	...
   	VoC_bgt16_rd(CGAver_L5_67,REG5,CONST_5325_ADDR);		//if (sub(diff, 5325) > 0)
   	VoC_lw16i_short(REG7,0,DEFAULT);			//{ st->hangVar = add(st->hangVar, 1);}
CGAver_L5_67:						// else{st->hangVar = 0; }

   	VoC_bngt16_rd(CGAver_L6_67,REG7,CONST_10_ADDR);	//if (sub(st->hangVar, 10) > 0)
   	VoC_lw16i_short(REG6,1,DEFAULT);			//{  st->hangCount = 0;
CGAver_L6_67:
   	VoC_lw16_sd(ACC1_HI,0,DEFAULT);			//bgMix = 8192; cbGainMix = gain_code;

	VoC_lw16i(REG4,3277);
	//  begin of dec_amr_not74795122_s1_67
   	 	 						//   if (sub(2048, tmp1) < 0)
   	VoC_sub16_rr(REG3,REG5,REG4,DEFAULT);		        //   { bgMix = 8192; }
   	VoC_bgtz16_r(CGAver_L11_67,REG3);   			//   else   {bgMix = shl(tmp1, 2);             } }
   	VoC_lw16i_short(REG3,0,DEFAULT);  			//else {
CGAver_L11_67:   						        //   tmp_diff = sub(diff, 3277);
   	VoC_shr16_ri(REG3,-2,DEFAULT);                          //   if (tmp_diff > 0)
   	VoC_bgt16_rd(CGAver_L17_67,REG3,CONST_0x2000_ADDR);   	//   {tmp1 = tmp_diff;  }
	VoC_blt16_rd(CGAver_L17_67,REG6,CONST_41_ADDR);		//if ((sub(st->hangCount, 40) < 0) || (sub(diff, 5325) > 0))
   	VoC_bngt16_rd(CGAver_L10_67,REG5,CONST_5325_ADDR);
CGAver_L17_67:
   	VoC_lw16i(REG3,8192);   				//   else {tmp1 = 0; }
CGAver_L10_67:    				                        //   if (sub(2048, tmp1) < 0)
                                                                //   {bgMix = 8192;  }
   ...
   //	VoC_pop32(ACC0,IN_PARALLEL);
   	//  end of dec_amr_not74795122_s1_67
   ...
   //return cbGainMix;  ->ACC1_HI
	...
	VoC_blt16_rd(Phdisp_L16_67_go,REG4,CONST_2_ADDR);		//   sub(mode, MR74) != 0 && sub(impNr, 2) < 0)
   	VoC_jump(Phdisp_L16_67);
Phdisp_L16_67_go:
   	// begin of dec_Phdisp_not74102122_s1_67
	...
	VoC_startloop0					//    {
   		...
   		VoC_bez16_r(Phdisp_L12_67,REG6);
...
Phdisp_L12_67:
   	...
   	VoC_bnez16_r(Phdisp_L14_67,REG4);			//        if (impNr == 0)
...
Phdisp_L14_67:   					//        }
   	...
   	VoC_loop_r(1,REG7);					//    for (nPulse = 0; nPulse < nze; nPulse++)    {
   		...
   		VoC_bez16_r(Phdisp_L20_67,REG6);
...
void CII_Dec_lag3_67(void)
{
	...
	VoC_bnez16_d(Dec_lag3_L3_67,DEC_AMR_PIT_FLAG_ADDRESS);	//if (i_subfr == 0) {
...
//	VoC_sub16_rd(REG6,REG3,CONST_112_ADDR);		//      *T0 = sub(index, 112);
	...
	VoC_bnlt16_rd(Dec_lag3_L2_67,REG3,CONST_197_ADDR); //	   if (sub(index, 197) < 0) {
...
Dec_lag3_L2_67:
	VoC_NOP();
	VoC_jump(Dec_lag3_LEnd_67);			//} else {
Dec_lag3_L3_67:    					// if (flag4 == 1)  {
	...
	VoC_bngt16_rr(Dec_lag3_L4_67,REG5,REG2);
	VoC_movreg16(REG5,REG2,DEFAULT);			//         tmp_lag = add (t0_min, 5);
Dec_lag3_L4_67:
	VoC_sub16_rd(REG7,REG7,CONST_4_ADDR);		//      if ( sub( sub(t0_max, tmp_lag), 4) > 0)
	VoC_bngt16_rr(Dec_lag3_L5_67,REG7,REG5);		//         tmp_lag = sub (t0_max, 4);
	VoC_movreg16(REG5,REG7,DEFAULT);
Dec_lag3_L5_67:
	VoC_blt16_rd(Dec_lag3_L8_67,REG3,CONST_4_ADDR);	//      if (sub(index, 4) < 0)
	//      { i = sub(tmp_lag, 5);
	//         *T0 = add(i, index);
	//         *T0_frac = 0;
	//      }
 	//      else
	VoC_bnlt16_rd(Dec_lag3_L7_67,REG3,CONST_12_ADDR);	//      {if (sub(index, 12) < 0)
...
/**************************************************************
 function   CII_decode_3i40_14bits
...
{
    VoC_lw16i_short(REG5,7,DEFAULT);			//i = index & 7; REG5-> i
    VoC_lw16i_short(REG3,5,IN_PARALLEL);
    VoC_and16_rr(REG5,REG1,DEFAULT);			//pos[0] = add(i, shl(i, 2));   REG4->pos[0]
    VoC_mult16_rr(REG4,REG5,REG3,DEFAULT);
    VoC_shr16_ri(REG1,3,IN_PARALLEL);			//index = shr(index, 3);
    VoC_lw16i_short(REG5,1,DEFAULT);			//j = index & 1;
    VoC_and16_rr(REG5,REG1,DEFAULT);
    VoC_shr16_ri(REG1,1,DEFAULT);			//index = shr(index, 1);
    VoC_lw16i_short(REG7,7,IN_PARALLEL);			//i = index & 7;
    VoC_and16_rr(REG7,REG1,DEFAULT);
    VoC_mult16_rr(REG7,REG7,REG3,DEFAULT);		//i = add(i, shl(i, 2));
    VoC_shr16_ri(REG5,-1,DEFAULT);			//j = shl(j, 1);
    VoC_add16_rd(REG7,REG7,CONST_1_ADDR);		//i = add(i, 1);
    VoC_add16_rr(REG5,REG7,REG5,DEFAULT);		//pos[1] = add(i, j);
    ...
    VoC_lw16i_short(REG5,1,IN_PARALLEL);			//j = index & 1;

    VoC_and16_rr(REG5,REG1,DEFAULT);
    VoC_shr16_ri(REG1,1,DEFAULT);			//index = shr(index, 1);
    VoC_and16_ri(REG1,7);	    			//i = index & 7;
    VoC_mult16_rr(REG1,REG1,REG3,DEFAULT);		//i = add(i, shl(i, 2));
	VoC_shr16_ri(REG5,-1,DEFAULT);			//j = shl(j, 1);
	VoC_add16_rd(REG1,REG1,CONST_2_ADDR);		//i = add(i, 2);

    VoC_add16_rr(REG4,REG1,REG5,DEFAULT);		//pos[2] = add(i, j);
    VoC_lw16i_short(REG3,0,DEFAULT);
    VoC_sw16_d(REG4,DEC_AMR_D2PF_POS_ADDRESS+2);
    VoC_loop_i_short(40,DEFAULT);             	        //for (i = 0; i < L_SUBFR; i++) {
    VoC_startloop0                                      //    cod[i] = 0;
        VoC_sw16inc_p(REG3,REG0,DEFAULT);
    VoC_endloop0                                  	//}

    VoC_loop_i_short(3,DEFAULT);             		//for (j = 0; j < NB_PULSE; j++) {
    ...
    VoC_startloop0
        ...
        VoC_lw16i(REG4,-8192);				//    if (i > 0) {
        VoC_bngtz16_r(D314pf_lab1,REG1);                //        cod[pos[j]] = 8191;
        VoC_lw16i(REG4,8191);                           //    } else {
D314pf_lab1:                                         	//        cod[pos[j]] = -8192;
