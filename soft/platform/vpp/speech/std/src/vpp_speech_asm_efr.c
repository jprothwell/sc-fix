...
void CII_dtx_efr_dec_reset(void)
{
	...
	VoC_startloop0;
		...
		VoC_sw32_d(ACC1,GLOBAL_GAIN_CODE_OLD_CN_DEC_ADDR);
...
void CII_interpolate_CN_lsf(void) //fts optimised 1114
{
		...
		VoC_sub16_dr(REG6,CONST_0x00007FFF_ADDR,REG5);//reg6 sub (0x7fff, interp_factor[rx_dtx_state]);
...
void CII_interpolate_CN_param (void)
{
		...
		VoC_sub16_dr(REG5,CONST_0x00007FFF_ADDR,REG4);//reg5 sub (0x7fff, interp_factor[rx_dtx_state]);
 ...
 //REG5 input new_gain_code  
{
		...
		VoC_bne16_rd(UP_RX001,REG2,(2+STATIC_CONST_dhf_MR475_ADDR)); //const_28
...
void CII_rx_dtx(void)//new
{
		...
		VoC_lw16i_short(REG1,2,DEFAULT );    //reg1 value of SID_flag
		VoC_bne16_rd(RX_DTX101, REG1, GLOBAL_SID_ADDR);  //if SID_flag != 2
	...
	RX_DTX101:
		...
		VoC_bnez16_d(RX_DTX107, GLOBAL_SID_ADDR);  //if SID_flag != 0
		    //if bfi == 0
		...
		VoC_movreg16(REG0,REG7,DEFAULT);
			...
			VoC_bez16_r(RX_DTX150, REG0);//if ((*rxdtx_ctrl & RX_CONT_SID_UPDATE) != 0)
				...
				VoC_bne16_rd(RX_DTX142,REG3,CONST_1_ADDR);
					...
					VoC_lw16i_short(REG2,0,DEFAULT);  //rx_dtx_state = 0
					VoC_jump(RX_DTX150);
...
//////
	...
	RX_DTX160://rxdtx_N_elapsed = add (rxdtx_N_elapsed, 1);
		...
		VoC_sw16_d(REG2,GLOBAL_RX_DTX_STATE_ADDR);			//reg2:rxdtx_state
            ...
            VoC_lw16i_short(REG2,1,DEFAULT);                //reg2:rxdtx_N_elapsed
            VoC_add16_rd(REG2,REG2,STATIC_RXDTX_N_ELAPSED_ADDR);  //rxdtx_N_elapsed = add (rxdtx_N_elapsed, 1);
			...
			VoC_bez16_r(RX_DTX161,REG1);
				...
				VoC_lw16i_short(REG4,DTX_HANGOVER,DEFAULT);  //rxdtx_aver_period = DTX_HANGOVER;
				VoC_jump(RX_DTX170);
		RX_DTX161:
			...
			VoC_bngt16_rd(RX_DTX162,REG2,CONST_30_ADDR)  //if (sub (rxdtx_N_elapsed, DTX_ELAPSED_THRESHOLD) > 0)
                ...
                VoC_lw32_d(REG01,CONST_0x70816958_ADDR);
				//rxdtx_N_elapsed = 0; 
				VoC_lw16i_short(REG2,0,DEFAULT);
				//rxdtx_aver_period = 0;
...
void CII_efr_dtx_func1(void)
{
	...
	VoC_push16(RA,DEFAULT);
		...
		VoC_lw16i_short(REG2,RX_UPD_SID_QUANT_MEM,DEFAULT);
       	VoC_and16_rd(REG2,GLOBAL_RXDTX_CTRL_ADDR);      
...
void CII_efr_dtx_func2(void)
{
		...
		VoC_and16_rd(REG4,GLOBAL_RXDTX_CTRL_ADDR);
		...
		VoC_bnez16_r(Dplsf5_LEnd_EFR10,REG4);
			...
			VoC_startloop0    		 
				// REG4 for lsf_p_CN[i] 		
				VoC_lw16inc_p(REG7,REG2,DEFAULT);  	//        temp = add (mean_lsf[i], mult (st->past_r_q[i],LSP_PRED_FAC_MR122));			
...
void CII_efr_dtx_func3 (void)
{
	VoC_push16(RA,DEFAULT);
       ...
       VoC_lw16i_short(REG2,RX_FIRST_SID_UPDATE,DEFAULT);
	   VoC_and16_rr(REG2,REG6,DEFAULT);      //(rxdtx_ctrl & RX_FIRST_SID_UPDATE)	
	...
	D_PLSF200:
      ...
      VoC_lw16i_short(REG2,RX_CONT_SID_UPDATE,DEFAULT);
	...
	D_PLSF201: //if ((rxdtx_ctrl & RX_SP_FLAG) != 0)
       ...
       VoC_lw16i_short(REG2,RX_SP_FLAG,DEFAULT);
	   VoC_and16_rr(REG2,REG6,DEFAULT);  
...
void CII_efr_dtx_func4(void)
{
		 ...
		 VoC_and16_rd(REG7,GLOBAL_RXDTX_CTRL_ADDR);
...
void CII_efr_dtx_func5(void)
{
       ...
       VoC_lw16i_short(REG7,RX_UPD_SID_QUANT_MEM,DEFAULT);
       VoC_and16_rd(REG7,GLOBAL_RXDTX_CTRL_ADDR); //reg7 value of (rxdtx_ctrl & RX_UPD_SID_QUANT_MEM)
	...
	D_GAIN200:
       ...
       which past valid SID frames are repeated */
		...
		VoC_movreg16(REG1,REG7,DEFAULT);
       	...
       	VoC_and16_ri(REG1,64);//CONST_RX_NO_TRANSMISSION_ADDR
	...
	//	VoC_and16_rd(REG2,CONST_RX_INVALID_SID_FRAME_ADDR);
       	VoC_and16_ri(REG2,RX_INVALID_SID_FRAME);//CONST_RX_INVALID_SID_FRAME_ADDR
		VoC_bnez16_r(D_GAIN202,REG2);//reg2:rxdtx_ctrl&RX_INVALID_SID_FRAME

		VoC_movreg16(REG3,REG7,DEFAULT);
	//	VoC_and16_rd(REG3,CONST_RX_LOST_SID_FRAME_ADDR);  CONST_16_ADDR
       VoC_and16_ri(REG3,16); 
	...
	D_GAIN202:
		...
		D_GAIN210:
			...
			VoC_and16_ri(REG1,128);  //reg1 value of (rxdtx_ctrl & RX_DTX_MUTING)
...
void CII_efr_dtx_func6(void)
{
			...
			VoC_bnez16_r(d_gain_code_EFR10,REG7);
				...
				VoC_and16_rd(REG1,GLOBAL_RXDTX_CTRL_ADDR);//reg1 value of (rxdtx_ctrl & RX_FIRST_SID_UPDATE)
...
void CII_acf_averaging (void)
{
	...
	VoC_startloop0
		...
		VoC_multf32inc_rp(ACC0,REG6,REG1,DEFAULT);
		VoC_lw16inc_p(RL6_HI,REG0,DEFAULT);
		VoC_lw16i_short(RL6_LO,0,IN_PARALLEL);
		VoC_add32_rr(ACC0,RL6,ACC0,DEFAULT);
 
		VoC_shr32_rr(ACC0,REG5,DEFAULT);			//L_temp	
	
		VoC_add32inc_rp(RL6,ACC0,REG3,DEFAULT);
		VoC_add32inc_rp(RL6,RL6,REG3,DEFAULT);
		VoC_add32_rp(RL6,RL6,REG3,DEFAULT);	
...
/*** Compute the division of the partial sum by sav0[0] ***/
	...
	VoC_bnltz32_r(SPECTRAL_COMPARISON104,ACC0);
		VoC_sub32_dr(REG45,CONST_0_ADDR,REG45);
...
VoC_NOP();
	...
	VoC_bnlt16_rd(THRESHOLD_ADAPTATION109,REG4,CONST_0x00004000L_ADDR)
...
COMPUTE_CN00:
	...
	VoC_add32_rd(ACC0,ACC0,CONST_1_ADDR);	// ACC0 for L_temp
...
//if (sub (buf_p_tx, (4 * DTX_HANGOVER - 1)) == 0)
  ...
  //buf_p_tx = add (buf_p_tx, 1);
	VoC_bnez16_r(UPDATE_GAIN_CODE_HISTORY_TX104,REG3);
// buf_p_tx = 0; 
	VoC_lw16i_short(REG0,0,DEFAULT);
	VoC_NOP();
...
{
	VoC_lw16i_short(REG7,TX_SID_UPDATE,DEFAULT);
	VoC_lw16_d(REG6,GLOBAL_TXDTX_CTRL_ADDR);
 
	VoC_and16_rr(REG7,REG6,DEFAULT);
	VoC_bez16_r(CII_CN_encoding01,REG7)	
...
Q_PLSF_5_01:
	VoC_bez16_d(Q_PLSF_5_02,Q_PLSF_5_DTX_AND_SID_ADDR);
		...
		VoC_lw16i_set_inc(REG0,Q_PLSF_5_LSF1_ADDR,1); 
	...
	VoC_movreg16(RL6_LO,REG2,DEFAULT);
	
    VoC_lw16i(REG4,0x3fff);
	...
	//	VoC_NOP();
		...
		VoC_add32_rd(RL6,RL6,CONST_1_ADDR);
		VoC_movreg16(REG2,RL6_LO,DEFAULT);
...
Q_PLSF_5_02:
	VoC_bnez16_d(Q_PLSF_5_03,Q_PLSF_5_DTX_AND_SP_ADDR);
		...
		VoC_lw16i(RL6_LO,Q_PLSF_5_LSF1_ADDR);
		VoC_lw16i(RL6_HI,Q_PLSF_5_LSF2_ADDR);
...
cod_amr1117:
	...
	VoC_sw32_d(ACC0,GLOBAL_L_PN_SEED_TX_ADDR);
	VoC_and16_rd(REG1,GLOBAL_TXDTX_CTRL_ADDR);       //REG1:addr of txdtx_ctrl
...
//*gain = CN_excitation_gain; 
	...
	VoC_lw16i_short(REG0,TX_SID_UPDATE,DEFAULT);

	VoC_sw16_d(REG7,COD_AMR_GAIN_CODE_ADDRESS);    
	VoC_and16_rd(REG0,GLOBAL_TXDTX_CTRL_ADDR);  // if ((txdtx_ctrl & TX_SID_UPDATE) != 0)
...
void CII_EFR_LEVINSON_PART (void)
{
	   ...
	   VoC_multf32_rp(ACC0, REG7, REG2,DEFAULT);
		...
		VoC_multf32inc_pd(REG45,REG0,CONST_0x00007FFF_ADDR);
		VoC_multf32_rp(ACC0, REG7, REG2,DEFAULT);
...
{
 // ACC1 is 0 when this function be called.

/*
   //This part have been cleared in CII_encoder_reset
...
*/	
    ...
    VoC_lw32_d(REG45,CONST_0x69cb0014_ADDR);
