...
Dec_amr_LoopStart_515:
	VoC_bne16_rd(Dec_amr_LoopEnd_5151,REG5,CONST_160_ADDR);  	//for (i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR)
	VoC_jump(Dec_amr_LoopEnd_515);
Dec_amr_LoopEnd_5151:
	...
	// begin of dec_amr_not122_s1_515
	   		//        flag4 = 1;
	VoC_lw16i_short(REG2,5,IN_PARALLEL);   		//     delta_frc_low = 5;
    	...
    	VoC_bnlt16_rd(Dec_amr_L18_515,REG6,CONST_20_ADDR);  	//     if (sub(t0_min, PIT_MIN) < 0)
    	VoC_lw16i_short(REG6,20,DEFAULT);       		//     {           t0_min = PIT_MIN;
Dec_amr_L18_515:          					//     }
    	VoC_add16_rr(REG7,REG6,REG1,DEFAULT);      		//     t0_max = add(t0_min, delta_frc_range);
    	VoC_bngt16_rd(Dec_amr_L19_515,REG7,CONST_143_ADDR);     //     if (sub(t0_max, PIT_MAX) > 0)
    	VoC_lw16i(REG7,143);		      		//     {        t0_max = PIT_MAX;
    	VoC_sub16_rr(REG6,REG7,REG1,DEFAULT);      		//        t0_min = sub(t0_max, delta_frc_range);
Dec_amr_L19_515:			          		//     }

   	VoC_jal(CII_Dec_lag3_515);      			//     Dec_lag3 (index, t0_min, t0_max, pit_flag, st->old_T0,&T0, &T0_frac, flag4);
   	VoC_sw16_d(REG6,STRUCT_DECOD_AMRSTATE_T0_LAGBUFF_ADDR); 	//     st->T0_lagBuff = T0;

   	VoC_bez16_d(Dec_amr_L20_515,DEC_AMR_BFI_ADDRESS);   	//  if (bfi != 0)
   	VoC_bnlt16_rd(Dec_amr_L20_515A,REG4,CONST_143_ADDR);   	//  {      if (sub (st->old_T0, PIT_MAX) < 0)
...
Dec_amr_L20_515A:						//  }
    	...
    	VoC_bez16_d(Dec_amr_L20_515,STRUCT_DECOD_AMRSTATE_INBACKGROUNDNOISE_ADDR);
    	VoC_bnlt16_rd(Dec_amr_L20_515,REG1,STRUCT_DECOD_AMRSTATE_VOICEDHANGOVER_ADDR);
...
Dec_amr_L20_515:     					//  }
	...
	VoC_jal(CII_decode_2i40_9bits_515);			//    decode_2i40_9bits (subfrNr, i, index, code);
       							//    pit_sharp = shl (st->sharp, 1);  }
        VoC_lw16_d(REG5,STRUCT_DECOD_AMRSTATE_SHARP_ADDR);
	VoC_jal(CII_amr_dec_com_sub1);

	// begin of  dec_amr_not795122_s1_515    			// M475 ,515 ,59 ,67 ,74 ,102
	VoC_bez16_d(Dec_amr_L35_515A,DEC_AMR_BFI_ADDRESS);    	//   if (bfi == 0)
    VoC_jump(Dec_amr_L35_515);
Dec_amr_L35_515A:
...
// begin of dec_gain
	...
	VoC_lw16i_short(REG7,1,IN_PARALLEL);		// mode
	...
	VoC_sub16_dr(REG7,CONST_10_ADDR,REG7);//515c
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
	VoC_pop16(REG1,IN_PARALLEL);
    	...
    	VoC_jump(Dec_amr_L36_515);    				//   }
Dec_amr_L35_515:        					//   else
...
Dec_amr_L36_515:
    	...
    	VoC_bngt16_rr(Dec_amr_L37_515,REG6,REG5);    		//   if (sub (pit_sharp, SHARPMAX) > 0)
    	VoC_movreg16(REG6,REG5,DEFAULT);    		//   {       pit_sharp = SHARPMAX;    }
Dec_amr_L37_515: 						//}
	// end of  dec_amr_not795122_s1_515
	...
	VoC_bngt16_rd(Dec_amr_L52_515,REG6,CONST_0x00004000L_ADDR); 	//if (sub (pit_sharp, 16384) > 0)
...
Dec_amr_L52_515:        						//}
	...
	VoC_bngt16_rr(Dec_amr_L54_515,REG7,REG0);   		//    if (sub (st->sharp, SHARPMAX) > 0)
...
Dec_amr_L54_515:						// }
	...
	VoC_bnez16_d(Dec_amr_L56_515,DEC_AMR_BFI_ADDRESS);   	//if ( bfi == 0 )
...
Dec_amr_L56_515:
	...
	VoC_bez16_d(Dec_amr_L60_515,STRUCT_DECOD_AMRSTATE_INBACKGROUNDNOISE_ADDR);// && st->inBackgroundNoise != 0 &&
	//     ((sub(mode, MR475) == 0) ||(sub(mode, MR515) == 0) ||(sub(mode, MR59) == 0)) )
	VoC_bnez16_d(Dec_amr_L57_515,DEC_AMR_BFI_ADDRESS);
	VoC_bez16_d(Dec_amr_L60_515,STRUCT_DECOD_AMRSTATE_PREV_BF_ADDR);
Dec_amr_L57_515:        					//{
	VoC_lw16i(REG0,12288);
	VoC_bngt16_rr(Dec_amr_L58_515,REG2,REG0);   		//   if ( sub (gain_pit, 12288) > 0)
...
Dec_amr_L58_515:
	...
	VoC_bngt16_rr(Dec_amr_L60_515,REG2,REG0);		//   if ( sub (gain_pit, 14745) > 0)
...
Dec_amr_L60_515:						//   }
	...
	VoC_jal(CII_Int_lsf);		   		//Int_lsf(prev_lsf, st->lsfState->past_lsf_q, i_subfr, lsf_i);
   	...
   	VoC_bgt16_rd(CGAver_L5_515,REG5,CONST_5325_ADDR);		//if (sub(diff, 5325) > 0)
   	VoC_lw16i_short(REG7,0,DEFAULT);			//{ st->hangVar = add(st->hangVar, 1);}
CGAver_L5_515:						// else{st->hangVar = 0; }

   	VoC_bngt16_rd(CGAver_L6_515,REG7,CONST_10_ADDR);	//if (sub(st->hangVar, 10) > 0)
...
CGAver_L6_515:
	...
	VoC_lw32_sd(REG23,0,IN_PARALLEL);
   	...
   	VoC_lw16i(REG4,3277);
   	VoC_bez16_r(CGAver_L8_515,REG2); 				//if ((((pdfi != 0) && (prev_pdf != 0)) || (bfi != 0) || (prev_bf != 0)) &&
   	VoC_bngt16_rd(CGAver_L8_515,REG3,CONST_1_ADDR);                //   (sub(voicedHangover, 1) > 0) && (inBackgroundNoise != 0) &&
   	VoC_bnez16_d(CGAver_L9_515,DEC_AMR_BFI_ADDRESS);               //   ((sub(mode, MR475) == 0) ||(sub(mode, MR515) == 0) ||  (sub(mode, MR59) == 0)) )
   	VoC_bnez16_d(CGAver_L9_515,STRUCT_DECOD_AMRSTATE_PREV_BF_ADDR);
   	VoC_bez16_d(CGAver_L8_515,DEC_AMR_PDFI_ADDRESS);
   	VoC_bez16_d(CGAver_L8_515,STRUCT_DECOD_AMRSTATE_PREV_PDF_ADDR);//{       tmp_diff = sub(diff, 4506);
CGAver_L9_515:						        //tmp1->reg3
   	VoC_lw16i(REG4,4506);                                        //   if (tmp_diff > 0)
   	                                       //   { tmp1 = tmp_diff;    }
CGAver_L8_515:                                                    //   else{ tmp1 = 0; }
			                                                   //   else{ tmp1 = 0; }

   	//  begin of dec_amr_not74795122_s1_515
   	 	 						//   if (sub(2048, tmp1) < 0)
   	VoC_sub16_rr(REG3,REG5,REG4,DEFAULT);		        //   { bgMix = 8192; }
   	VoC_bgtz16_r(CGAver_L11_515,REG3);   			//   else   {bgMix = shl(tmp1, 2);             } }
   	VoC_lw16i_short(REG3,0,DEFAULT);  			//else {
CGAver_L11_515:   						        //   tmp_diff = sub(diff, 3277);
   	VoC_shr16_ri(REG3,-2,DEFAULT);                          //   if (tmp_diff > 0)
   	VoC_bgt16_rd(CGAver_L17_515,REG3,CONST_0x2000_ADDR);   	//   {tmp1 = tmp_diff;  }
   	VoC_blt16_rd(CGAver_L17_515,REG6,CONST_41_ADDR);	//if ((sub(st->hangCount, 40) < 0) || (sub(diff, 5325) > 0))
   	VoC_bngt16_rd(CGAver_L10_515,REG5,CONST_5325_ADDR);    	//   else {tmp1 = 0; }
CGAver_L17_515:
   	VoC_lw16i(REG3,8192);	   				//  { bgMix = 8192;  }     //   if (sub(2048, tmp1) < 0)
CGAver_L10_515:                                                 //   {bgMix = 8192;  }
   	...
   	VoC_lw16i(REG4,4681);            		 	//cbGainMean = round(L_sum);
   	                       //if (((bfi != 0) || (prev_bf != 0)) && (inBackgroundNoise != 0) &&
   	VoC_bez16_r(CGAver_L19_515,REG0);      			//    ((sub(mode, MR475) == 0) ||(sub(mode, MR515) == 0) ||(sub(mode, MR59) == 0)) )
   	VoC_bnez16_d(CGAver_L20_515,DEC_AMR_BFI_ADDRESS);        //{   L_sum = L_mult(4681, st->cbGainHistory[0]);
   	VoC_bnez16_d(CGAver_L20_515,STRUCT_DECOD_AMRSTATE_PREV_BF_ADDR);	//   for (i = 1; i < L_CBGAINHIST; i++)
CGAver_L19_515:                                             	//   {
...
CGAver_L20_515:
	...
	VoC_shr32_ri(ACC0,-2,DEFAULT);				//   cbGainMix = round(L_shl(L_sum, 2));
  	...
  	//  end of dec_amr_not74795122_s1_515
	...
	VoC_jal(CII_amr_dec_com_sub3);
     ...
     VoC_blt16_rd(Phdisp_L16_515_go,REG4,CONST_2_ADDR);		//   sub(mode, MR74) != 0 && sub(impNr, 2) < 0)
   	  VoC_jump(Phdisp_L16_515);
Phdisp_L16_515_go:

   	// begin of dec_Phdisp_not74102122_s1_515
   ...
   VoC_lw16i_short(INC3,1,IN_PARALLEL);
	...
	VoC_startloop0					//    {
   		...
   		VoC_bez16_r(Phdisp_L12_515,REG6);
...
Phdisp_L12_515:
   	...
   	VoC_bnez16_r(Phdisp_L14_515,REG4);			//        if (impNr == 0)
...
Phdisp_L14_515:   					//        }
   	...
   	VoC_loop_r(1,REG7);					//    for (nPulse = 0; nPulse < nze; nPulse++)    {
   		...
   		VoC_multf16inc_rp(REG7,REG2,REG3,DEFAULT); //515c
   		...
   		VoC_bez16_r(Phdisp_L20_515,REG6);
...
Phdisp_L20_515:
	...
	VoC_endloop1						// }
   	// end of dec_Phdisp_not74102122_s1_515

Phdisp_L16_515:
   	VoC_jal(CII_amr_dec_com_sub4);

   	// begin of dec_amr_47551559_s1_515
	VoC_bez16_r(Dec_amr_L66S1a_515,REG6);   			//&& sub(st->voicedHangover, 5) > 0 && st->inBackgroundNoise != 0 && sub(st->state, 4) < 0 &&
     	VoC_bgt16_rd(Dec_amr_L66S1b_515,REG7,CONST_5_ADDR)   	//     ( (pdfi != 0 && st->prev_pdf != 0) ||bfi != 0 ||st->prev_bf != 0) )
Dec_amr_L66S1a_515:
	VoC_jump(Dec_amr_L66S1_515);
Dec_amr_L66S1b_515:
	VoC_lw16i_short(REG6,4,DEFAULT);
     	VoC_bngt16_rd(Dec_amr_L66S1c_515,REG6,STRUCT_DECOD_AMRSTATE_STATE_ADDR);
     	VoC_bnez16_d(Dec_amr_L65_515,DEC_AMR_BFI_ADDRESS);
     	VoC_bnez16_d(Dec_amr_L65_515,STRUCT_DECOD_AMRSTATE_PREV_BF_ADDR);
     	VoC_bez16_d(Dec_amr_L66S1c_515,DEC_AMR_PDFI_ADDRESS);
     	VoC_bnez16_d(Dec_amr_L65_515,STRUCT_DECOD_AMRSTATE_PREV_PDF_ADDR);
Dec_amr_L66S1c_515:
	VoC_jump(Dec_amr_L66S1_515);
Dec_amr_L65_515:        					//{
     	VoC_lw16i_short(REG7,0,DEFAULT);   		//   carefulFlag = 0;
     	VoC_bez16_d(Dec_amr_L67_515,DEC_AMR_PDFI_ADDRESS);    	//   if ( pdfi != 0 && bfi == 0 )
     	VoC_bnez16_d(Dec_amr_L67_515,DEC_AMR_BFI_ADDRESS);   	//   {
...
//  begin of ex_ctrl
	VoC_lw16i_short(REG5,9,IN_PARALLEL);			//avgEnergy = gmed_n(exEnergyHist, 9);
   	...
   	VoC_bngt16_rr(Exctrl_Lab1_515,REG6,REG7);			//if ( sub (exEnergyHist[8], prevEnergy) < 0)
   	VoC_movreg16(REG6,REG7,DEFAULT);			//{  prevEnergy = exEnergyHist[8];}
Exctrl_Lab1_515:
   	VoC_bngt16_rr(Exctrl_LaEnd_515_go,REG1,REG5);		//if ( sub (excEnergy, avgEnergy) < 0 && sub (excEnergy, 5) > 0)
   	VoC_bgt16_rd(Exctrl_LaEnd_515_no,REG5,CONST_5_ADDR);
Exctrl_LaEnd_515_go:
	VoC_jump(Exctrl_LaEnd_515)
Exctrl_LaEnd_515_no:
	VoC_movreg16(REG4,REG6,DEFAULT);			//{  testEnergy = shl(prevEnergy, 2);
   	...
   	VoC_blt16_rd(Exctrl_Lab2_515,REG3,CONST_7_ADDR);		//   if ( sub (voicedHangover, 7) < 0 || prevBFI != 0 )
   	VoC_bez16_d(Exctrl_Lab3_515,STRUCT_DECOD_AMRSTATE_PREV_BF_ADDR);
Exctrl_Lab2_515:   						//   {
   	VoC_sub16_rr(REG4,REG4,REG6,DEFAULT);		// testEnergy = sub (testEnergy, prevEnergy);   }
Exctrl_Lab3_515:
   	VoC_bngt16_rr(Exctrl_Lab4_515,REG7,REG4);		//   if ( sub (avgEnergy, testEnergy) > 0)
...
Exctrl_Lab4_515:
   	...
   	VoC_bngt32_rd(Exctrl_Lab5_515,REG67,CONST_0x00007FFF_ADDR);//   if ( L_sub(t0, 32767) > 0 )
...
Exctrl_Lab5_515:
   	...
   	VoC_bez16_r(Exctrl_Lab6_515,REG7);
   	VoC_bngt16_rr(Exctrl_Lab6_515,REG6,REG4);
...
Exctrl_Lab6_515:
    ...
    VoC_multf32inc_rp(ACC0,REG6,REG0,DEFAULT);//515c
   	VoC_loop_i_short(40,DEFAULT); 			//   for (i = 0; i < L_SUBFR; i++)
...
Dec_amr_L66S1_515:
	// end of dec_amr_47551559_s1_515
	VoC_pop32(ACC0,DEFAULT);


	VoC_jal(CII_amr_dec_com_sub5);
	VoC_jump(Dec_amr_LoopStart_515); 	//  }
Dec_amr_LoopEnd_515:
...
/*************************************
 Function  void CII_Dec_lag3_515(void)
...
void CII_Dec_lag3_515(void)
{
	...
	VoC_bnez16_d(Dec_lag3_L3_515,DEC_AMR_PIT_FLAG_ADDRESS);	//if (i_subfr == 0) {
...
//	VoC_sub16_rd(REG6,REG3,CONST_112_ADDR);		//      *T0 = sub(index, 112);
	...
	VoC_bnlt16_rd(Dec_lag3_L2_515,REG3,CONST_197_ADDR); //	   if (sub(index, 197) < 0) {
	VoC_add16_rr(REG2,REG3,REG0,DEFAULT);		//      *T0 = add(mult(add(index, 2), 10923), 19);
	//VoC_lw16i_short(REG5,19,DEFAULT);
	VoC_multf16_rr(REG6,REG2,REG1,DEFAULT);//515c	//      i = add(add(*T0, *T0), *T0);
	VoC_lw16i_short(REG5,19,DEFAULT);//515c
...
Dec_lag3_L2_515:
//	VoC_NOP();
	VoC_jump(Dec_lag3_LEnd_515);			//} else {
Dec_lag3_L3_515:    					// if (flag4 == 1)  {
	...
	VoC_bngt16_rr(Dec_lag3_L4_515,REG5,REG2);
	VoC_movreg16(REG5,REG2,DEFAULT);			//         tmp_lag = add (t0_min, 5);
Dec_lag3_L4_515:
	VoC_sub16_rd(REG7,REG7,CONST_4_ADDR);		//      if ( sub( sub(t0_max, tmp_lag), 4) > 0)
	VoC_bngt16_rr(Dec_lag3_L5_515,REG7,REG5);		//         tmp_lag = sub (t0_max, 4);
	VoC_movreg16(REG5,REG7,DEFAULT);
Dec_lag3_L5_515:
	VoC_blt16_rd(Dec_lag3_L8_515,REG3,CONST_4_ADDR);	//      if (sub(index, 4) < 0)
	//      { i = sub(tmp_lag, 5);
	//         *T0 = add(i, index);
	//         *T0_frac = 0;
	//      }
 	//      else
	VoC_bnlt16_rd(Dec_lag3_L7_515,REG3,CONST_12_ADDR);	//      {if (sub(index, 12) < 0)
	...
	VoC_jump(Dec_lag3_LEnd_515);						//         }
Dec_lag3_L7_515:    					//         else
...
/**************************************************************
 function void CII_decode_2i40_9bits_515(void)
 Input: Word16 subNr,    i -> REG7
    	Word16 sign,       i -> REG6
    	Word16 index,      i -> REG1
    	Word16 cod[]       o <- REG0  push REG0

 Version 1.0 Created by Kenneth       06/30/2004
 Version 1.1 Optimized by Kenneth     08/16/2004
**************************************************************/
void CII_decode_2i40_9bits_515(void)
{
    ...
    VoC_startloop0
        ...
        VoC_lw16i(REG4,-8192);
        VoC_bez16_r(d_pf_sub_L11_515,REG1);           //    if (i != 0) {
        VoC_lw16i(REG4,8191);                     // cod[pos[j]] = 8191;
d_pf_sub_L11_515:                                     //    } else { cod[pos[j]] = -8192;
