...
void CII_amr_dec_475(void)
{
	...
	VoC_lw16_sd(REG7,1,IN_PARALLEL);		//   parm += 3;
...
//	VoC_multf16_rr(REG6,REG2,REG1,IN_PARALLEL);	//      i = add(add(*T0, *T0), *T0);
	VoC_multf16_rr(REG6,REG2,REG1,DEFAULT);
	VoC_lw16i_short(REG5,19,DEFAULT);
...
Dec_lag3_L5_475:
	...
	//      }
 	//      else
	VoC_bnlt16_rd(Dec_lag3_L7_475,REG3,CONST_12_ADDR);	//      {if (sub(index, 12) < 0)
...
Dec_amr_L57_475:        					//{
	...
	VoC_bngt16_rr(Dec_amr_L58_475,REG2,REG0);   		//   if ( sub (gain_pit, 12288) > 0)
...
Phdisp_L16_475:
	...
	VoC_bez16_r(Dec_amr_L66S1a_475,REG6);   			//&& sub(st->voicedHangover, 5) > 0 && st->inBackgroundNoise != 0 && sub(st->state, 4) < 0 &&
     	VoC_bgt16_rd(Dec_amr_L66S1b_475,REG7,CONST_5_ADDR)   	//     ( (pdfi != 0 && st->prev_pdf != 0) ||bfi != 0 ||st->prev_bf != 0) )
Dec_amr_L66S1a_475:
