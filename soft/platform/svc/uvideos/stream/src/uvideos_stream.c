...
#define WEBCAM_SENSOR_BUF_WIDTH_QQVGA 160
#define WEBCAM_SENSOR_BUF_HEIGHT_QQVGA 120
#define WEBCAM_JPEG_BUF_WIDTH_QQVGA 160
#define WEBCAM_JPEG_BUF_HEIGHT_QQVGA 120


#define WEBCAM_SENSOR_BUF_WIDTH_QVGA 320
#define WEBCAM_SENSOR_BUF_HEIGHT_QVGA 240
#define WEBCAM_JPEG_BUF_WIDTH_QVGA 320
#define WEBCAM_JPEG_BUF_HEIGHT_QVGA 240
...
static struct usb_video_probe_commit_ctr_struct video_probe_commit_ctr = {
  ...
  .bFormatIndex = 0x01,
  .bFrameIndex = 0x02,
  .dwFrameInterval = 0x0007a120<<1,//50ms
...
PRIVATE VOID uvideos_CompleteJpegBuff(VOID)
{
	...
	pHeader->bmHeaderInfo = (1<<USBVIDEO_PAYLOAD_PTS)|(1<<USBVIDEO_PAYLOAD_SCR)|(g_frame_id<<USBVIDEO_PAYLOAD_FRAME_ID);
...
PRIVATE VOID uvideos_ImageEncode(VOID)
{
	...
	if(SensorFrameSize == CAM_NPIX_QQVGA)
	{
		VocImgEncIn.SampleRate  = 0;
		VocImgEncIn.imag_width  = WEBCAM_JPEG_BUF_WIDTH_QQVGA;
		VocImgEncIn.imag_height = WEBCAM_JPEG_BUF_HEIGHT_QQVGA;	
	}
	else if (SensorFrameSize == CAM_NPIX_QVGA)
	{
		VocImgEncIn.SampleRate  = 0;
		VocImgEncIn.imag_width  = WEBCAM_JPEG_BUF_WIDTH_QVGA;
		VocImgEncIn.imag_height = WEBCAM_JPEG_BUF_HEIGHT_QVGA;
	}
	...
	if(SensorFrameSize == CAM_NPIX_QQVGA)
	{
		VPP_WRITE_JPEGHeadr(VocImgEncIn.imag_quality, WEBCAM_JPEG_BUF_WIDTH_QQVGA,WEBCAM_JPEG_BUF_HEIGHT_QQVGA,g_jpegbuf);
	}
	else if (SensorFrameSize == CAM_NPIX_QVGA)
	{
		VPP_WRITE_JPEGHeadr(VocImgEncIn.imag_quality, WEBCAM_JPEG_BUF_WIDTH_QVGA,WEBCAM_JPEG_BUF_HEIGHT_QVGA,g_jpegbuf);
	}
...
UINT32 uvideos_CameraPowerUp (VOID)
{
	...
	if (SensorFrameSize == CAM_NPIX_VGA)
	{
		ispOutWidth = 640;
		ispOutHeight = 480;
	}
	...
	else if (SensorFrameSize == CAM_NPIX_QQVGA)
	{
		ispOutWidth = 160;
		ispOutHeight = 120;    
	}
	...
	{
		UVIDEOS_TRACE(UVIDEOS_INFO_TRC, 0, "uvideos_CameraPowerUp: Unsupported ISP Out Size");
		return 0;
	}
	...
	if(SensorFrameSize == CAM_NPIX_QQVGA)
	{
		g_sensorbuf = (UINT16*)convertaddr(mmc_MemMalloc(WEBCAM_SENSOR_BUF_WIDTH_QQVGA*WEBCAM_SENSOR_BUF_HEIGHT_QQVGA*2));
		g_jpegbuf = (UINT8*)convertaddr(mmc_MemMalloc(WEBCAM_JPEG_BUF_WIDTH_QQVGA*WEBCAM_JPEG_BUF_HEIGHT_QQVGA*2));
	}
	else if (SensorFrameSize == CAM_NPIX_QVGA)
	{
		g_sensorbuf = (UINT16*)convertaddr(mmc_MemMalloc(WEBCAM_SENSOR_BUF_WIDTH_QVGA*WEBCAM_SENSOR_BUF_HEIGHT_QVGA*2));
		g_jpegbuf = (UINT8*)convertaddr(mmc_MemMalloc(WEBCAM_JPEG_BUF_WIDTH_QVGA*WEBCAM_JPEG_BUF_HEIGHT_QVGA*2));
	}
	
	camera_Reserve(uvideos_CameraFrameReadyCB);

#if (CHIP_ASIC_ID == CHIP_ASIC_ID_GREENSTONE)
	 camera_PowerOn(SensorFrameSize, 0);
