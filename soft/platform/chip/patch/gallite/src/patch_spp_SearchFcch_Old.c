...
//
//-----------------------------------------------------------------------------
/// @file patch_spp_SearchFcch_Old.c
///
/// This file contains the patch functions for the FCCH Search
///
//    $Author: Chunjiang TU $                                                        // 
///  $Date: 2010-12-21 $ //
//-----------------------------------------------------------------------------
...
// PATCH_SPP_SEARCHFCCH_ALGO:0: greenstone algo. 1:gallite algo
#define     PATCH_SPP_SEARCHFCCH_ALGO       0

//------------------------------------------------------------------------------
// Global variables
//------------------------------------------------------------------------------
PRIVATE SPP_FCCH_PARAM_T SPAL_DATA_INTERNAL g_sppFcchParam;
PRIVATE UINT16 SPAL_DATA_INTERNAL g_sppFcchBurstNb = 0;
PRIVATE UINT32* SPAL_DATA_INTERNAL g_sppPtrNew;

#if 0
UINT32 g_tcj_FCCHCnt = 0;
UINT32 g_tcj_FCCHINIT = 0x01;
UINT32 g_tcj_FCCHBuf[142*9] = {0xABABABAB};
#endif

#if (PATCH_SPP_SEARCHFCCH_ALGO == 1)
// The following is the same as spp_SearchFcch in gallite proj. just for debug. no use.

//======================================================================
// FCCH Detection function debug @ afc locked
//----------------------------------------------------------------------
...
// .
//======================================================================
PUBLIC SPP_FCCH_RETURN_T SPAL_FUNC_INTERNAL patch_spp_SearchFcch_Old(UINT32* RxBuffer, BOOL Init, SPP_FCCH_MODE_T Mode, SPP_FCCH_RESULT_T* Res)
{
...
//#if 1    Off no use. This will cause no base station camping
//    g_sppGlobalParams.fcch_sliding_energy = TRUE;
//#endif
...
#else
//
// The following is the same as spp_SearchFcch in greenstone proj. just for debug

PRIVATE UINT32 SPAL_DATA_INTERNAL g_spalRkm1 = 0;
PRIVATE UINT32 SPAL_DATA_INTERNAL g_spalRkm2 = 0;
PROTECTED VOID spal_InvalidateDcache(VOID);
//=============================================================================
// patch_spal_XcorFchxc_Old
//-----------------------------------------------------------------------------
...
// @param init UINT8
//=============================================================================
PRIVATE VOID SPAL_FUNC_INTERNAL spal_XcorFchxc_Old(UINT32* symb_ptr, UINT16 Offset, UINT16 size, UINT8 init)
{
    //SPAL_PROFILE_FUNCTION_ENTER(spal_XcorFchxc);
//    static UINT32 g_spalRkm1 = 0;
//    static UINT32 g_spalRkm2 = 0;
...
#define BB_FCCH_NB_WIN                           (((625*9)/(BB_FCCH_SIZE*4) + 1))
    ...
    // for I-part correlation and one for Q-part correlation)
    //
    // Each window is divided in 3 parts of BB_FCCH_SIZE symbols, one part
    // Wx is processed at every call to this function(x being 0->1->2->0->1...).
    //
    //   ----------------------------------------------------------------
    //   |      |      |      |      |      |      |      |      |      |
    ...
    if (Init) // First call for this FCCH search
    {
        ...
        // Init pointers
        g_sppPtrNew = MEM_ACCESS_CACHED(RxBuffer);
        temp_ptr = g_sppPtrNew + 3*BB_FCCH_SIZE; // point to CI
        ...
        // Run DCOC (and get Accumulated Energy)
        g_sppFcchParam.Acc_Energy= spal_XcorDcoc((UINT32*)MEM_ACCESS_CACHED(RxBuffer),
                                          BB_FCCH_SIZE,
                                          FALSE,// No derotation
                                          BYPASS_DCOC,
                                          &Dco);
        ...
        // Run XCOR in FCCH mode to generate Power, CI and CQ terms
        // This function flushes the Dcache
        spal_XcorFchxc_Old( MEM_ACCESS_CACHED(RxBuffer),
                        3*BB_FCCH_SIZE, // offset
                        BB_FCCH_SIZE, // size
                        TRUE); // Init

        // Calculate initial accumulated CI and CQ
        temp_accu = 0;
        for(i=0;i<BB_FCCH_SIZE;i++)
            temp_accu += *(temp_ptr++);
        g_sppFcchParam.Acc_Corr_I = temp_accu;
        temp_ptr += 2*BB_FCCH_SIZE; // point to CQ
        temp_accu = 0;
        for(i=0;i<BB_FCCH_SIZE;i++)
            temp_accu += *(temp_ptr++);
        g_sppFcchParam.Acc_Corr_Q = temp_accu;
    ...
    // Update pointers
    Ptr_old = g_sppPtrNew;
    g_sppPtrNew += BB_FCCH_SIZE;
    g_sppPtrNew = (g_sppPtrNew==MEM_ACCESS_CACHED(RxBuffer+3*BB_FCCH_SIZE))?
                                        MEM_ACCESS_CACHED(RxBuffer):
                                        g_sppPtrNew;
    ...
    // Run XCOR in FCCH mode to generate Power, CI and CQ terms
    // This function flushes the Dcache
    spal_XcorFchxc_Old( g_sppPtrNew,
                    3*BB_FCCH_SIZE, // offset
                    BB_FCCH_SIZE, // size
                    FALSE); // Init
    ...
    // Prevents the search mechanism to wait for a next IT
    // when we are in the last search window (slice mode only).
    if ((Mode == FCCH_SLICED) && (g_sppFcchBurstNb==(BB_FCCH_NB_WIN-1)))
    {
        g_sppFcchParam.lastIT = TRUE;
    }

    if (spp_FcchDetectAsm(Ptr_old,g_sppPtrNew,&g_sppFcchParam,Res))
    {
#define SYMB_MARGIN 11
#define CI_PTR_OFFSET 3*BB_FCCH_SIZE
#define CQ_PTR_OFFSET 2*3*BB_FCCH_SIZE
        INT32 register temp_CI;
        INT32 register temp_CQ;
        INT32* temp_ptr;
        INT32* fcch_start;
        ...
        // Discard detections too close from the window's border
        if (((g_sppFcchBurstNb == 1) && // window's beginning
             (Res->TOf <= (BB_BURST_ACTIVE_SIZE-BB_FCCH_SIZE)))
            &&
            ( g_sppFcchParam.lastIT && // End of sliced mode window
             (Res->TOf >= (2*BB_FCCH_SIZE-BB_BURST_ACTIVE_SIZE)))
           )
        {
            return FCCH_INVALID;
        }
        ...
        if (Res->TOf < 0)
        {
            end_cnt =(BB_FCCH_SIZE - SPP_ABS(Res->TOf) - SYMB_MARGIN);
            start_cnt = (SPP_ABS(Res->TOf) - SYMB_MARGIN);

            if (SPP_ABS(Res->TOf) < SYMB_MARGIN)
                    end_cnt = BB_FCCH_SIZE-(2*SYMB_MARGIN);

            if (g_sppPtrNew==MEM_ACCESS_CACHED(RxBuffer))
                fcch_end += 3*BB_FCCH_SIZE;
            if (Ptr_old==MEM_ACCESS_CACHED(RxBuffer))
                fcch_start += 3*BB_FCCH_SIZE;
        }
        else
        {
            start_cnt =(BB_FCCH_SIZE - (Res->TOf) - SYMB_MARGIN);
            end_cnt = ((Res->TOf) - SYMB_MARGIN);

            if ((Res->TOf) < SYMB_MARGIN)
                start_cnt = BB_FCCH_SIZE-(2*SYMB_MARGIN);
        }
        ...
        // ---------------
        // Calculate overall offset since the window's start
        Res->TOf = (INT16) ((g_sppFcchBurstNb-1)*BB_FCCH_SIZE // passed bursts
                            + (INT16) Res->TOf // offset found
                            - ((BB_BURST_ACTIVE_SIZE-BB_FCCH_SIZE)/2 - 1)); // win bias
        return FCCH_FOUND;
    }
