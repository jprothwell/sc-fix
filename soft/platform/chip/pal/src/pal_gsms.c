...
////////////////////////////////////////////////////////////////////////////////
//                                                                            //
//  $HeadURL: http://svn.coolsand-tech.com/svn/developing1/Sources/chip/branches/gallite441/pal/src/pal_gsms.c $ //
//  $Author: huazeng $                                                        // 
...
//                                                                            //
/// @file pal_gsms.c
/// This file contains the PAL API & IRQ functions
//                                                                            //
////////////////////////////////////////////////////////////////////////////////
...
#if (RESET_XCV_AT_ASYNC_PAL_INIT)
BOOL g_palResetXcvAtAsyncPalInit = TRUE;
#endif // RESET_XCV_AT_ASYNC_PAL_INIT
...
pal_Initialisation (PAL_INIT_T InitType, UINT8 BandMap)
{
    ...
    switch(InitType)
    {
        case PAL_INIT_GLOBAL:
            // clear the active part of the TCU also
            hal_TcuClrAllEvents();
            // flush the Rfspi Fifo now
            hal_RfspiFlushFifo();
            hal_RfspiSetupFifo();
        ...
        case PAL_INIT_ASYNCHRO_WITH_RESETXCV:
           ...
           // of the TCU also
            ...
            hal_TcuStop(); 
            hal_TcuSetWrap(PAL_TCU_DEFAULT_WRAP_COUNT); 
            // lps is not allowed during xcv reset
            hal_SysRequestFreq(HAL_SYS_FREQ_PAL, PAL_STACK_SYS_FREQ_DEFAULT, NULL);
            hal_RfspiFlushFifo();
            hal_RfspiSetupFifo();
            rfd_XcvClose();
           ...
           // postpone the Rfspi FiFo flush to the next Frame Start
            g_palDebug.RfspiFlushFifo=TRUE;
            PAL_TRACE(PAL_LEV(8), 0, "PAL_INIT_ASYNCHRO_WITH_RESETXCV.");
          break;
...
pal_GprsCiphering(pal_GprsCiphering_t *GprsCiphering)
{
    ...
    if (GprsCiphering->Length > 512)
    {
        // flush cache
        hal_GdbFlushCacheData();
    } else {
        // do a memcpy from uncached to cached to insure coherency of cache
        // FIXME with hal_SysInvalidateCache ?
        memcpy( (UINT8*) GprsCiphering->DataBlock, // destination
                (UINT8*) UNCACHED((UINT32) GprsCiphering->DataBlock), // source
                GprsCiphering->Length); // size
    }
...
pal_FintSuspend (UINT16 frameQty)
{
    ...
    PAL_TRACE(PAL_LEV(15), 0, "FintSuspend FrameQty=%d",
                        frameQty);
#endif
...
pal_GetElapsedTime (VOID)
{
...
#ifdef __LPS_ENABLED__
    UINT16 skippedFrames = hal_LpsGetElapsedFrames();
    ...
    PAL_TRACE(PAL_LEV(15), 0, "GetElapsedTime : %d frames",skippedFrames);
#endif
    ...
    PAL_TRACE(PAL_LEV(15), 0, "GetElapsedTime : 1 frames");
#endif
    PAL_PROFILE_FUNCTION_EXIT(pal_GetElapsedTime);
    return (1);
#endif
...
VOID palStartBcpu (VOID)
{
    ...
    {
        UINT32* ptr=(UINT32*)(CACHED((UINT32)&_bb_sram_romed_end));
    ...
    hal_SysStartBcpu((VOID*)bcpu_main, (VOID*)bcpuStackBase);

    // Wait until other cpu is done to stop: read at mailbox top.
    while ((g_mailbox.spc2pal.debug[0] == MBOX_XCPU_HELLO_WORD) && (counter < PAL_DEADLOCK_COUNTER))
    {
        counter++;
    }
...
palSetRamps_calib (UINT16 dacValueIdx, GSM_RFBAND_T band)
{
    ...
    {
        dacValueHigh = dacValueIdx & 0x3FF; // 10 bits
    }
    else // This is a DAC value index
    {
        ...
        if (GSM_BAND_DCS1800 == band || GSM_BAND_PCS1900 == band)
        {
            dacValueHigh = g_palCalibration->pa->palcust.profileInterpDp[dacValueIdx];
        }
        else
        {
            dacValueHigh = g_palCalibration->pa->palcust.profileInterpG[dacValueIdx];
        }
    }
...
palHwInit (VOID)
{
    ...
    hal_SysRequestFreq(HAL_SYS_FREQ_PAL,  PAL_STACK_SYS_FREQ_DEFAULT, NULL);
    hal_SysRequestFreq(HAL_SYS_FREQ_BCPU, PAL_BCPU_SYS_FREQ_DEFAULT,  NULL);
