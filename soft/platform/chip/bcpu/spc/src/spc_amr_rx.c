...
/*                               All Rights Reserved                          */
/******************************************************************************/
/* Filename:  spc_amr_x.c                                                    */
/* Description:                                                               */
/*   Reception functions of the SPC Layer (AMR)                               */
/******************************************************************************/
...
VOID spc_AmrDecode(SPC_RX_LOCALS_T *spRx){
    ...
    // in case of dtx downlink the sidupdate_ctr is incremented form 1 to 8 
    switch (spRx->RxFrameType){
        case SPP_RX_NO_DATA: 
            if (g_spcCtx->sidupdate_ctr>0){
                g_spcCtx->sidupdate_ctr++;
                if (g_spcCtx->sidupdate_ctr==9) g_spcCtx->sidupdate_ctr=1;
            }
            break;
    ...
    switch (FrameType){
        ...
        case SPP_AFS_SID_FIRST:
            ...
            *RxFrameType=SPP_RX_SID_FIRST;
            if (((T2==3)||(T2==11)||(T2==20))&&(id1<AMR_Cfg.nbActiveCodecSet)){
                if (AMR_Cfg.phase==ODD_PHASE) *RxCodecMode=cm1; // odd phase CMI received
                else                          *TxCodecMode=cm1; //even phase CMC received   
            }
            else{
    ...
    switch (FrameType){
        ...
        case SPP_AHS_SID_FIRST_INH:
            ...
            if (((T2==2)||(T2==10)||(T2==19)||(T2==3)||
                (T2==11)||(T2==20))&&(id1<AMR_Cfg.nbActiveCodecSet)){
                if (AMR_Cfg.phase==ODD_PHASE) *RxCodecMode=cm1; // odd phase CMI received
                else                          *TxCodecMode=cm1; //even phase CMC received   
            }
            else{
