...
////////////////////////////////////////////////////////////////////////////////
//                                                                            //
//  $HeadURL: http://svn.coolsand-tech.com/svn/developing1/Sources/chip/branches/gallite441/hal/src/hal_ebc.c $ //
//    $Author: admin $                                                        // 
...
PRIVATE HAL_SYS_FREQ_T g_halEbcFreq =  0;
...
#else
    hwp_memFifo->EBC_Flash_Block_Remap = physAddr;
#endif
}
...
PUBLIC VOID* hal_EbcCsOpen(HAL_EBC_CS_T cs, CONST HAL_EBC_CS_CFG_T* csConfig)
{
    ...
    {
        case HAL_EBC_FLASH :
            HAL_ASSERT(FALSE, "Improper use of the hal_EbcCsOpen function."
...
// ============================================================================
PUBLIC HAL_SYS_FREQ_T  hal_EbcGetFrequency(VOID)
{
    HAL_ASSERT(g_halEbcFreq != 0, "EBC frequency not set by a flash open yet");
    return g_halEbcFreq;  
}
...
PROTECTED VOID hal_EbcCsSetup(HAL_SYS_MEM_FREQ_T ebcFreq)
{
    ...
    // TODO
    // Cf. hal_ebcFlashOpen.
    // HAL_ASSERT(hwp_sysCtrl->Sel_Clock & SYS_CTRL_PLL_LOCKED, "pll not enabled before setting ebc clock");
    // IMPLEMENT
#else
    // FPGA
#if ((CHIP_ASIC_ID == CHIP_ASIC_ID_GALLITE) || (CHIP_ASIC_ID == CHIP_ASIC_ID_ESPERITE))
    ...
    switch (ebcFreq)
    {
        ...
        case HAL_SYS_MEM_FREQ_13M:
            reg = SYS_CTRL_FREQ_13M;
            break;

        case HAL_SYS_MEM_FREQ_26M:
            reg = SYS_CTRL_FREQ_26M;
            break;

        case HAL_SYS_MEM_FREQ_39M:
            reg = SYS_CTRL_FREQ_39M;
            break;

        case HAL_SYS_MEM_FREQ_52M:
            reg = SYS_CTRL_FREQ_52M;
            break;

        case HAL_SYS_MEM_FREQ_78M:
            reg = SYS_CTRL_FREQ_78M;
            break;
...
#else
 #error "Unsupported CHIP_ASIC_ID in hal_ebc.c"
#endif

        default:
            HAL_ASSERT(FALSE, "Configuring the EBC clock with a bad frequency: %d",
                    (UINT32)ebcFreq);
            reg = 0;
            break;
    }
