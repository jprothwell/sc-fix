...
////////////////////////////////////////////////////////////////////////////////
//                                                                            //
//  $HeadURL: http://svn.coolsand-tech.com/svn/developing1/Sources/chip/branches/gallite441/hal/src/hal_rfspi.c $ //
//    $Author: huazeng $                                                        // 
//    $Date: 2011-05-06 15:02:49 +0800 (Fri, 06 May 2011) $                     //   
//    $Revision: 7440 $                                                          //   
//                                                                            //
////////////////////////////////////////////////////////////////////////////////
//                                                                            //
/// @file hal_rfspi                                                           //
/// RF-SPI driver implementation
//                                                                            //
//////////////////////////////////////////////////////////////////////////////// 
...
// =============================================================================
PRIVATE HAL_RFSPI_SW_FIFO_T HAL_DATA_INTERNAL g_spiDataFifo;
...
// In 8808, TCU is async but RFSPI is still sync
...
// =============================================================================
PROTECTED VOID hal_RfspiUpdateDivider(HAL_SYS_FREQ_T sysFreq);
...
PRIVATE VOID HAL_FUNC_INTERNAL hal_RfspiLockMcp(VOID)
{
    ...
    hal_AbbRegWrite(CODEC_MISC_SETTING,
                    ABB_SYNC_CLK_FREQ(g_halRxDcdcFreqDivValue[curFrame])|
...
PUBLIC VOID hal_RfspiOpen(CONST HAL_RFSPI_CFG_T* cfg, HAL_RFSPI_CLK_T reqFreq)
{
    ...
    // Get a minimum system frequency ? (Use PAL's one)
#if 1 //(!CHIP_HAS_ASYNC_TCU)
    // Register the divider updating function and don't take any resource
    // as this stuff uses PAL clock requirement, and PAL knows how
    // to handle its stuff.
    hal_SysRequestFreq(HAL_SYS_FREQ_RF_SPI, HAL_SYS_FREQ_32K, hal_RfspiUpdateDivider);
    ...
    UINT8  spiDivider = 7;
    // In Emerald we don't care about system clock.
    // Rf Spi clock is 52M/(2*(1+spiDivider[2:0]))
    switch(g_rfspiReqFreq)
    {    
        case HAL_RFSPI_26M_CLK:    spiDivider= 0;break; 
        case HAL_RFSPI_19_5M_CLK:  spiDivider= 1;break; // Actually 13MHz.
        case HAL_RFSPI_13M_CLK:    spiDivider= 1;break; 
        case HAL_RFSPI_6_5M_CLK:   spiDivider= 3;break; 
        case HAL_RFSPI_3_25M_CLK:  spiDivider= 7;break;
    }    
    hwp_sysCtrl->Cfg_Clk_RfSpi = spiDivider;
#endif
}
...
PUBLIC VOID hal_RfspiForceRead(UINT8 rdSize, UINT8* dataOut)
{
    ...
    if (hal_SysGetRequestFreq(HAL_SYS_FREQ_RF_SPI) == HAL_SYS_FREQ_32K)
    {
...
PUBLIC VOID HAL_FUNC_INTERNAL hal_RfspiPushData(HAL_RFSPI_CMD_T* cmd, CONST UINT8* data, UINT8 byteQty)
{
    ...
    HAL_ASSERT((g_spiCmdQueue.dataIdx <= RFSPI_DATA_BUF_SIZE),
                  "CmdQueue Data Overflow, with index=%d",
                  g_spiCmdQueue.dataIdx);
    HAL_ASSERT((cmd->byteQty >= byteQty),
                "Spi command too large, size=%d", 255+cmd->byteQty);
}
...
PUBLIC VOID HAL_FUNC_INTERNAL hal_RfspiSetupFifo(VOID)
{
    // Reset SW Fifo
    g_spiDataFifo.end = 0;
    g_spiDataFifo.start = RFSPI_SW_FIFO_SIZE-1;
    g_spiDataFifo.addedData = 0;
#if  (CHIP_ASIC_ID == CHIP_ASIC_ID_GREENSTONE) || defined(RFSPI_IFC_WORKAROUND)
...
PUBLIC VOID HAL_FUNC_INTERNAL hal_RfspiFillFifo(BOOL nextFrame)
{
    ...
    UINT32 spaceLeft = (g_spiDataFifo.end < g_spiDataFifo.start) ?
                     g_spiDataFifo.start - g_spiDataFifo.end :
                     RFSPI_SW_FIFO_SIZE - (g_spiDataFifo.end - g_spiDataFifo.start);


    // Flag the start of the new frame
    if (nextFrame)
    {
        g_spiDataFifo.start = g_spiDataFifo.end;
    }
    ...
    UINT32 cmdLeft = CMD_FIFO_SPACE;

    while (g_spiCmdQueue.firstCmd)
    {
        // Only fill data of a full command
        spaceLeft -= g_spiCmdQueue.firstCmd->byteQty;
        cmdLeft--;
        if ((spaceLeft >= 0) && (cmdLeft >= 0)) 
        {
            UINT32 i;
            // Fill Data
            for (i=0; i < g_spiCmdQueue.firstCmd->byteQty; i++)
            {
                g_spiDataFifo.data[g_spiDataFifo.end] = g_spiCmdQueue.firstCmd->data[i];
                if (++g_spiDataFifo.end == RFSPI_SW_FIFO_SIZE)
                {
                    g_spiDataFifo.end = 0;
                }
            }
            // Fill Cmd
            if (g_spiCmdQueue.firstCmd->maskable)
            {
                hwp_rfSpi->Cmd_Size = g_spiCmdQueue.firstCmd->byteQty |
                                      RF_SPI_CMD_MARK;
            }
            else
            {
            ...
            g_spiDataFifo.addedData += g_spiCmdQueue.firstCmd->byteQty;
        ...
        {
            // Remaining commands in queue at FINT -> error
            HAL_ASSERT(nextFrame, "Remaining commands in queue at FINT:%d",
                                    (UINT32)g_spiCmdQueue.firstCmd);
        }
    }
...
PUBLIC BOOL hal_RfspiIsActive()
{
    // Use it to pool until the end of an Spi command
     return ((hwp_rfSpi->Status & RF_SPI_ACTIVE_STATUS) == 1);
}
