...
////////////////////////////////////////////////////////////////////////////////
//                                                                            //
//  $HeadURL: http://svn.coolsand-tech.com/svn/developing1/Sources/chip/branches/gallite441/hal/src/hal_trace.c $ //
//    $Author: lijy $                                                        // 
//    $Date: 2010-12-13 11:33:34 +0800 (Mon, 13 Dec 2010) $                     //
//    $Revision: 4557 $                                                         //   
//                                                                            //
////////////////////////////////////////////////////////////////////////////////
//                                                                            //
...
PUBLIC VOID hal_HstTraceOpen(UINT16 setting, UINT8 portIdx)
{
    ...
    case HAL_HST_TRACE_PORT_HOST:
        ...
        // Uart already open
        HAL_ASSERT(!((hwp_debugUart->ctrl) & DEBUG_UART_ENABLE),
        ...
        // to send even more...
        hwp_debugUart->irq_mask = DEBUG_UART_RX_DATA_AVAILABLE 
...
// By default use host hard coded config, but !
        ...
        hwp_debugUart->ctrl     = DEBUG_UART_ENABLE
                        ...
                        | DEBUG_UART_DMA_MODE ;
#else
        hwp_debugUart->ctrl     = DEBUG_UART_ENABLE | DEBUG_UART_DMA_MODE | DEBUG_UART_DATA_BITS_8_BITS | DEBUG_UART_AUTO_FLOW_CONTROL_ENABLE;
#endif
...
PUBLIC VOID hal_HstTraceDisable(VOID)
{
    ...
    case HAL_HST_TRACE_PORT_HOST:
        ...
        hwp_debugUart->irq_cause = DEBUG_UART_TX_DMA_DONE;
        hal_IfcChannelRelease(HAL_IFC_DEBUG_UART_TX, g_hstTraceTxIfcCh);
        g_hstTraceTxIfcCh    = HAL_UNKNOWN_CHANNEL;
        g_halHstTraceEnabled = FALSE;
       ...
       //  Check if the uart is open
        ...
        if (g_halHstTraceEnabled)
        {
            ...
            else
            {
                ...
                // Adding TC while transfering bytes is boguous on Greenstone,
                // so we cleverly disable this feature, keeping the proper
                // implementation for later chips. (eg, for Gallite, we directly
                // update the TC, even if not 0).
...
PUBLIC UINT32 hal_HstTraceGetData(UINT8 portIdx, UINT8 *destAddress, UINT32 length)
{
    ...
    case HAL_HST_TRACE_PORT_HOST:
        ...
        nbAvailable = GET_BITFIELD(hwp_debugUart->status,
        ...
        {
            destAddress[i] = hwp_debugUart->rxtx_buffer;
        }
        break;
