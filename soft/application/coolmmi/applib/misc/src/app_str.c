 ...
 *  which may be the null character _T_MMI'\0'.
 * PARAMETERS
...
kal_int8 *app_ucs2_strupr(kal_int8 *string)
{
    ...
    {
        if ((*(cursor+1)<<8|(kal_uint8)(*cursor)) >= _T_MMI'a' && 
            (*(cursor+1)<<8|(kal_uint8)(*cursor)) <= _T_MMI'z' )
        {
            *cursor = *cursor - ('a' - 'A');
        }
        cursor += 2;
...
kal_int32 app_ucs2_stricmp(const kal_int8 *str_src, const kal_int8 *str_dst)
{
    ...
    /*----------------------------------------------------------------*/
    do {
        first = (((*(src+1)<<8|(kal_uint8)(*src)) <= _T_MMI'Z') && 
                 ((*(src+1)<<8|(kal_uint8)(*src)) >= _T_MMI'A'))
            ? (kal_wchar)(*src - 'A' + 'a')
            : (kal_wchar)*src;
        last = (((*(dst+1)<<8|(kal_uint8)(*dst)) <= _T_MMI'Z') && 
                ((*(dst+1)<<8|(kal_uint8)(*dst)) >= _T_MMI'A'))
            ? (kal_wchar)(*dst - 'A' + 'a')
            : (kal_wchar)*dst;
        src += 2;
        dst += 2;
...
kal_wchar *app_ucs2_wcslwr(kal_wchar *string)
{
    ...
    {	
        if ((*cursor >= (kal_wchar)_T_MMI'A') && (*cursor <= (kal_wchar)_T_MMI'Z') )
        {	
            *cursor = *cursor - _T_MMI'A' + _T_MMI'a';
        }
    }
    return string;
}
...
kal_int32 app_ucs2_wcsicmp(const kal_wchar *str_src, const kal_wchar *str_dst)
{
    ...
    /*----------------------------------------------------------------*/
    do {
        first = ((*str_dst <= _T_MMI'Z') && (*str_dst >= _T_MMI'A'))
            ? *str_dst + _T_MMI'a' - _T_MMI'A'
            : *str_dst;
        last = ((*str_src <= _T_MMI'Z') && (*str_src >= _T_MMI'A'))
            ? *str_src + _T_MMI'a' - _T_MMI'A'
            : *str_src;
        str_dst++;
        str_src++;
    } while (first && (first == last));
                            ...
                            kal_uint32 count)
{
    ...
    {
        do {
            first = ((*str_src <= _T_MMI'Z') && (*str_src >= _T_MMI'A'))
                ? *str_src - _T_MMI'A' + _T_MMI'a'
                : *str_src;
            last = ((*str_dst <= _T_MMI'Z') && (*str_dst >= _T_MMI'A'))
                ? *str_dst - _T_MMI'A' + _T_MMI'a'
                : *str_dst;
            str_src++;
            str_dst++;
