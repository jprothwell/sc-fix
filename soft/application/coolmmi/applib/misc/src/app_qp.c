...
#define QP_DECODE_STATE_PADCRLF 4
...
#define QP_NEED_DECD_SKIP_LEADING_SPACE \
            ((cntx->opt & QPOPT_DECODE_SKIP_LEADING_SPACE) > 0)
...
static kal_int32 decode_state_padcrlf(applib_qprint_context* cntx,
                    kal_char buf[], kal_char c);
            ...
            kal_int32 dstl)
{
    ...
    while(i_input < *srcl)
    {
        ...
        case QP_DECODE_STATE_PADCRLF:
            cnt = decode_state_padcrlf(cntx,buf,c);
            break;
                    ...
                    kal_char buf[], kal_char c)
{
    ...
    else if (QP_IS_LF(c))
    {
        ...
        cntx->state = QP_DECODE_STATE_PADCRLF;
    }
    else if (QP_IS_HEX(c))
    {
                    ...
                    kal_char buf[], kal_char c)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/    
    if (QP_NEED_DECD_SKIP_LEADING_SPACE && QP_IS_LWSP(c))
    {
        /* Leading SPACE after Soft Line Break */
        cntx->state = QP_DECODE_STATE_PADCRLF;
    }
    else
    {
        if (cntx->state2 == QP_DECODE_STATE_PADHEX || 
            cntx->state2 == QP_DECODE_STATE_PAD)
        {
