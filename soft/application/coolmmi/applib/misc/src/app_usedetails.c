...
static kal_bool applib_is_valid_phoneno_wchar(kal_wchar ch, kal_bool is_written_char_valid)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    kal_wchar phoneno_special_chars[] = {0x2b00, 0x7700, 0x7000};//{L'+',L'w',L'p'};
    kal_wchar phoneno_written_chars[] = {0x2d00, 0x2800, 0x2900};//{L'-',L'(',L')'};
    ...
    if (ch != 0)
    {
        if (ch >= AnsiiToUnicode('0') && ch <= AnsiiToUnicode('9'))
        {
            return KAL_TRUE;
        }
...
static kal_bool applib_is_valid_email_local_part_wchar(kal_wchar ch)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    U8 phoneno_special_chars[] = {'.', '!', '#', '$', '%', '&', '\\', '*', '+','-','/', '=', '?', '^','_', '`', '{', '|', '}', '~' , '0'};
    U8 i = 0;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if ((ch >= AnsiiToUnicode('a') && ch <= AnsiiToUnicode('z')) ||
        (ch >= AnsiiToUnicode('A') && ch <= AnsiiToUnicode('Z')) || (ch >= AnsiiToUnicode('0') && ch <= AnsiiToUnicode('9')))
    {
        return KAL_TRUE;
    }

    while(ch != AnsiiToUnicode('0'))
    {   
        if(ch == AnsiiToUnicode(phoneno_special_chars[i]))
        {
            return KAL_TRUE;
        }
        i++;
...
kal_bool applib_is_valid_email_local_part_unicode(const kal_wchar *str, int cnt)
{
    ...
    if (*str == AnsiiToUnicode('.') || ch == AnsiiToUnicode('.'))  /* '.' should not be the first or last character in the local-part */
    {
        return KAL_FALSE;
    }
...
static kal_bool applib_is_valid_email_domain_name_wchar(kal_wchar ch)
{
    ...
    /*----------------------------------------------------------------*/ 
    if (!((ch >= AnsiiToUnicode('a') && ch <= AnsiiToUnicode('z')) ||
          (ch >=AnsiiToUnicode('A') && ch <= AnsiiToUnicode('Z')) ||
          (ch >= AnsiiToUnicode('0') && ch <= AnsiiToUnicode('9')) || 
          (ch == AnsiiToUnicode('-')) || (ch == AnsiiToUnicode('.'))))
    {
        return KAL_FALSE;
    }
...
kal_bool applib_is_valid_url_wchar(kal_wchar ch)
{
    ...
    if (ch != 0)
    {
        ...
        else
        {
            for (i = 0; i < num_url_special_chars; i++)
            {
                if (ch == AnsiiToUnicode(url_special_chars[i]))
                {
                    return KAL_TRUE;
                }
            }
...
static kal_bool applib_is_lp_paired_wchar(const kal_wchar *lp_pos, kal_int32 length)
{
    ...
    while (length > 0)
    {
        if (*ch == AnsiiToUnicode('('))
        {
            lp_count++;
        }
        else if (*ch == AnsiiToUnicode(')'))
        {
            lp_count--;
        }
        length--;
...
/*                    while((start - 1 >= data) && applib_is_valid_url_scheme_char(*(start - 1)))
            ...
            if (end == strstr(data, pattern[type]) + strlen(pattern[type]))  /* address string is NULL */
            {
                if ((end - data) < len)
                {
        ...
        kal_bool *is_stopped)
{
    ...
    kal_wchar *data = NULL;
    applib_address_node_struct *address_list = NULL;  
    S8 *patternansii[APPLIB_USE_DETAILS_URL_PATTERN_NO] = {URL_PATTERN_1, URL_PATTERN_2, URL_PATTERN_3, URL_PATTERN_4};
    S8 *schemeansii[APPLIB_USE_DETAILS_URL_SCHEME_NO] = {URL_SCHEME1, URL_SCHEME2, URL_SCHEME3};
    kal_wchar *pattern[APPLIB_USE_DETAILS_URL_PATTERN_NO]={NULL};
    kal_wchar *scheme[APPLIB_USE_DETAILS_URL_SCHEME_NO]={NULL}; 
    int i = 0, type = 0;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/   
    for(i = 0; i < APPLIB_USE_DETAILS_URL_PATTERN_NO; i++)
    {
        AnsiiToUnicodeString((S8*)pattern[i], (S8*)patternansii[i]);
        AnsiiToUnicodeString((S8*)scheme[i], (S8*)schemeansii[i]);
    }
    ...
    while ((data - input_data) < dataLen)
    {
        kal_wchar *start = NULL, *end = NULL, *data_lc = NULL;
        i = 0;
        type = 0;
        data_lc = mem_alloc_fn(len + 2);

        memset(data_lc, 0x00, len + 2);
        memcpy((char*)data_lc, (char*)data, len);
        ...
        else
        {
            ...
            if (type == 0 || type == 1)  /* "://" or ":\\\\" */
            {
                ...
                for(i = 0 ; i < APPLIB_USE_DETAILS_URL_SCHEME_NO; i++)
                {
                    len = app_ucs2_strlen((kal_int8 *)scheme[i]);
...
/*                    while((start - 1 >= data) && applib_is_valid_url_scheme_wchar(*(start - 1)))
            ...
            if (end == app_ucs2_strstr(data, pattern[type]) + app_ucs2_strlen((kal_int8 *)pattern[type]))  /* address string is NULL */
            {
                if ((end - data) < len)
                {
                    data = end;  /* continue from the following char */
        ...
        kal_bool *is_stopped)        
{
    ...
    AnsiiToUnicodeString((S8*)Atstr, "@");
    data = input_data;
    len = dataLen;
    while (data - input_data < dataLen)
    {
        start = (kal_wchar *)app_ucs2_strchr((const kal_int8 *)data, (kal_wchar)AnsiiToUnicode('@'));
        ...
        if (start == NULL)  /* not found */
        {
            ...
            *(start - 1) == AnsiiToUnicode('.'))  /* '.' should not be the last character in the local-part */
        {
            data = start + 1;   //app_ucs2_strlen((kal_int8*)L"@");  /* continue from the end of the pattern */
            len = dataLen - (data - input_data);
            continue;
        }
        else
        {
            ...
            while (*start == AnsiiToUnicode('.'))  /* '.' should not be the first character in the local-part */
            {
                start++;
            }
            ...
            while (*(end - 1) == AnsiiToUnicode('.'))  /* removing trailing dot(s)*/
            {
                end--;
            }

            if (end == app_ucs2_strstr(data, Atstr) + app_ucs2_strlen((kal_int8*)Atstr))  /* host string is NULL */
            {
                if ((end - data) < len)
                {
                    data = end;  /* continue from the following char */
        ...
        kal_bool *is_stopped)        
{
    ...
    while (data - input_data < dataLen)
    {
        ...
        for(i = 0; i < len; i++)
        {
            ...
            else if((*end >= AnsiiToUnicode('0')) && (*end <= AnsiiToUnicode('9')))
            {
                if(no_length > APPLIB_MAX_PHONE_NUM_LEN)  /* Phone number is too long */
                {
                    break;  /* search stop */
            ...
            else if((*end == AnsiiToUnicode('(')) || (*end == AnsiiToUnicode(')')) || (*end == AnsiiToUnicode('-')))
            {
                str_length++;
                /* part of the number but not add to length */
            }
            else if((*end == AnsiiToUnicode('w')) || (*end == AnsiiToUnicode('p')))
            {
                if(no_length == 0)
                {
                }
        ...
        /* remove invalid trailing char */
        while (no_length >= APPLIB_MIN_PHONE_NUM_LEN && (*(end - 1) == 'w' || *(end - 1) == 'p' || 
            *(end - 1) == AnsiiToUnicode('(') || *(end - 1) == AnsiiToUnicode(')') || *(end - 1) == AnsiiToUnicode('-')))
        {
            if (*(end - 1) == AnsiiToUnicode('w') || *(end - 1) == AnsiiToUnicode('p'))
            {
                no_length -= 1;
            }
            str_length -= 1;
        ...
        /* remove invalid heading char */
        while (no_length >= APPLIB_MIN_PHONE_NUM_LEN && (*start == AnsiiToUnicode('-') || *start == AnsiiToUnicode(')') ||
...
kal_bool applib_is_valid_email_address_unicode(const kal_wchar *str)
{
    ...
    if (*(str) == AnsiiToUnicode('.') || *(str + pos - 1) == AnsiiToUnicode('.'))
    {
    ...
    if (*(str + pos + 1) == AnsiiToUnicode('.') || *(str + len - 1) == AnsiiToUnicode('.'))
    {
