...
#define wbxml_generate_get_buffer     OslMalloc
#define wbxml_generate_free_buffer    OslMfree
...
kal_int32 wbxml_generate_empty_element(WG_HANDLE hd, const kal_char* el, xg_attr_struct* attrlist, kal_uint32 num_attr)
{
    ...
    return wbxml_generate_stag(hd, el, attrlist, num_attr, 0);
}
...
kal_int32 wbxml_generate_inline_element(WG_HANDLE hd, const kal_char* el, kal_char* data, kal_uint32 datalen, kal_uint8 datatype)
{
    ...
    if ((ret = wbxml_generate_stag(hd, el, NULL, 0, 1)) < 0)
    {
        return ret;
    }
    if ((ret = wbxml_generate_data(hd, data, datalen, datatype)) < 0)
    {
        return ret;
    }
