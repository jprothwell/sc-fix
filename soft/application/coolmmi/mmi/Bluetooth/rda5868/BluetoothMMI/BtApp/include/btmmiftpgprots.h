...
typedef enum
{
    ...
    MMI_FTPC_NOTIFY_NO_CARD,                  /* Device not found*/
...
#define FTP_FOLDER_PATH                 (L"E:/@ftp/") /* working folder */
#define FTP_SHARED_FOLDER_PATH_FILE     (L"E:/@ftp/shared_file_path") /* for saving shared folder */
#define FTP_RECIVED_FOLDER_PATH_FILE	  (L"E:/@ftp/recived_file_path")  /* for saving reviced folder */		

/* for saving folder content */
#define FTPS_MAKE_FOLDER_CONTENT(x, i)              \
do{\
   csd_wsprintf( (kal_uint16 *)x, "E:/@ftp/folder_content_%d", i);\
}while(0)
/* for saving unparsed folder content */
#define FTPC_MAKE_FOLDER_OBJECT(x, i)              \
do{\
   csd_wsprintf( (kal_uint16 *)x, "E:/@ftp/folder_object_%d", i);\
}while(0)
/* for saving parsed folder content */
#define FTPC_MAKE_FOLDER_PARSED(x, i)              \
do{\
   csd_wsprintf( (kal_uint16 *)x, "E:/@ftp/fc.tmp_%d", i);\
}while(0)
...
#define MMI_FTPC_LOG_STATE(i, x)                                                                            \
   {                                                                                                        \
    FTP_DEBUG(kal_print("FTPC STATE_TRANS TO STATE: (act_client_cntx_p+i)->ftpc_state) =%d",(act_client_cntx_p+i)->ftpc_state););\
   }
...
#define FTPC_MAKE_RECV_PATH(x)    do{                      \
   ...
   UCS2Strcat((PS8)x, (PS8)FTPC_RECV_OBJ_FILEPATH);                 \
}while(0)
...
#define MMI_FTP_MAKE_RECV_TMP_OBJNAME(x, d) do{       \
    U8 temp_name[32];                                                              \
    UCS2Strcpy((PS8)x, (act_client_cntx_p + i)->received_folder); \
    csd_wsprintf( (kal_uint16 *)temp_name, "%s%02d.%s",   \
        MMI_FTP_RECV_TMP_OBJNAME, d, MMI_FTP_RECV_TMP_OBJEXT );    \
    UCS2Strcat((PS8)x, (PS8)temp_name);                 \
}while(0)
...
extern void mmi_bt_ftp_event_hdlr(msg_type_t msg_id, void *msg);  /* protocol event hdlr */
