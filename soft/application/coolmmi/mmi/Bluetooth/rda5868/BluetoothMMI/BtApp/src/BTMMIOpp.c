 ...
 * Description:
 * ------------
 *   This file is for COOLSAND bluetooth MMI OPP client/server
 *
 * Author:
 * -------
 * -------
...
extern void av_bt_connect_hfp_callback(void);//zhou siyou 20100414//use for pause music if need
extern void av_bt_conhfp_cnf_callback(void);//zhou siyou 091104 use for resume music if need
/***************************************************************************** 
* External Variable
*****************************************************************************/
...
static void mmi_opp_init_oppc_cntx(void)
{
    ...
    else    /* Error handling */
    {
        ...
        if (ret < 0)
        {
            mmi_trace (1,"error for create ftp dir");    
        }
        return ;
    }
}
...
static void mmi_opp_init_opps_cntx(void)
{
    ...
    MMI_OPP_CTX(h_recv) = (FS_HANDLE) -1;
...
static void mmi_opp_init_fs_recover(void)
{
    ...
    MMI_OPP_MAKE_RECV_PATH(MMI_OPP_CTX(recived_file_path));
    ret = MMI_FS_CreateDir((const UINT8*) MMI_OPP_CTX(recived_file_path));
...
static void mmi_opp_init_received_folder(void)
{
    ...
    */
	...
	{
		MMI_OPP_MAKE_RECV_PATH_FLASH(MMI_OPP_CTX(recived_file_path));
	}
    /* Folder Path Init */
    ...
    if (h >= FS_NO_ERROR)
    {
        FTP_DEBUG(kal_print("FS_OPEN NO ERROR\n");)
            ...
            res = MMI_FS_Read(h, (void *)MMI_OPP_CTX(recived_file_path), FMGR_MAX_PATH_LEN * ENCODING_LENGTH, (INT32 *)&read_length);
        MMI_FS_Close(h);

        if (res == FS_NO_ERROR)
        {
	     if(!UCS2Strncmp((S8 *)(MMI_OPP_CTX(recived_file_path)),(S8 *)L"D:",2) && !g_hasMountedTFlashFS)
    	     {
	    	     MMI_OPP_MAKE_RECV_PATH(MMI_OPP_CTX(recived_file_path));
		     h = MMI_FS_Open((const UINT8*) FTP_RECIVED_FOLDER_PATH_FILE, (FS_CREATE_ALWAYS));
	    	     if (h >= FS_NO_ERROR )
	    	     {
	    	        res = MMI_FS_Write(h, MMI_OPP_CTX(recived_file_path), (UCS2Strlen(MMI_OPP_CTX(recived_file_path)) * 2 + 2), &read_length);
	    	        MMI_FS_Close(h);
	    	     }     
    	     }
		 else if( UCS2Strncmp((S8 *)(MMI_OPP_CTX(recived_file_path)),(S8 *)L"D:",2) &&(data == FALSE))
	 	{
			MMI_OPP_MAKE_RECV_PATH(MMI_OPP_CTX(recived_file_path));
			MMI_FS_Close(h);
    ...
    else    /* h<FS_NO_ERROR */
    {
        ...
        FTP_DEBUG(kal_print("FS_OPEN ERROR\n");)
	     ...
	     {
		 	mmi_trace(1,"mmi_opp_init_received_folder create_always fails");
	     }
    }

    //MMI_OPP_MAKE_RECV_PATH(nPrintableStr);
    if( g_hasMountedTFlashFS == TRUE )
    {
      h = MMI_FS_Open((const UINT8*) MMI_OPP_CTX(recived_file_path), FS_READ_ONLY | FS_OPEN_DIR);
      mmi_trace(1,"MMI_BT_OPP_INIT_RECEIVED_FOLDER_OPEN_DIR, h=%d\n",h);
      if (h >= FS_NO_ERROR)
      {
          MMIOPP_RESET_FLAG(MMIOPP_MASK_FOLDER_ERR);
    ...
    else    /* Error handling */
    {
        ...
        if (h == FS_ERROR_RESERVED ||h == FS_PATH_NOT_FOUND || h == FS_FILE_NOT_FOUND)
        {
            ret = MMI_FS_CreateDir((const UINT8*) MMI_OPP_CTX(recived_file_path));
...
static S8 *mmi_opp_get_file_name_ext(S8 *pathfileName, MMI_OPP_GET_FILE_ENUM type)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    U16 NameLength = (U16) UCS2Strlen(pathfileName);
...
static U32 mmi_opp_retrieve_obj_type(S8 *obj_name)
{
    ...
    AnsiiNToUnicodeString(ext_buf, p, 5 * ENCODING_LENGTH);
 ...
 *  RETURNS:(?)
 *****************************************************************************/
void mmi_opp_get_folder_name()
{
    U16 folderlength = (U16) UCS2Strlen((S8 *)MMI_OPP_CTX(recived_file_path));
    S16* pName = NULL ;
 
    
    UCS2Strcpy((S8*)MMI_OPP_CTX(file_stored_folder_name),(S8*)MMI_OPP_CTX(recived_file_path));
    pName  = (S16*)MMI_OPP_CTX(file_stored_folder_name);
    
    if(UCS2Strcmp((PS8) &pName[folderlength-1],(PS8) L"\\") == 0 || UCS2Strcmp((PS8) &pName[folderlength-1], (PS8)L"/")== 0)
    {
...
static void mmi_opp_notify_register_app(U16 *path)
{
    ...
    switch (obj_type)
    {
        ...
        case FMGR_TYPE_VBM:
        #endif
#if  defined(__MMI_FTP_SUPPORT__)
...
static void mmi_bt_opps_connect_ind_handler(void *msg)
{
    ...
    UCS2Strcpy((PS8) MMI_OPP_CTX(dev_name), (PS8) goep_connect_ind->dev_name);
...
static void mmi_bt_opps_disconnect_rsp_handler(U8 rsp_code)
{
    ...
    {
        MMI_FS_Close(MMI_OPP_CTX(h_recv));
        MMI_OPP_CTX(h_recv) = (FS_HANDLE) -1;
    }
...
static void mmi_bt_opps_first_push_ind_handler(void *msg)
{
    ...
    if (UCS2Strlen((const S8*)goep_push_ind->obj_name) * 2 > (sizeof(MMI_OPP_CTX(obj_name)) - 2))
    {
        int name_length = UCS2Strlen((const S8*)goep_push_ind->obj_name) * 2;
    ...
    mmi_trace(1,"MMI_BT_OPP_OPPS_FIRST_PUSH_IND_FS_OPEN, h=%d\n",h);
    ...
    mmi_trace(1,"MMI_BT_OPP_OPPS_FIRST_PUSH_IND_FRAG_LEN, goep_push_ind->frag_len=%d, goep_push_ind->total_obj_len=%d\n",goep_push_ind->frag_len,goep_push_ind->total_obj_len);
    ...
    if (goep_push_ind->frag_len > 0)
    {
        ...
        if (MMI_OPP_CTX(h_recv) >= 0)
        {
            ret = MMI_FS_Write(MMI_OPP_CTX(h_recv), goep_push_ind->frag_ptr, goep_push_ind->frag_len, (INT32*) & len_written);

            MMI_OPP_LOG1(MMI_BT_OPP_OPPS_FIRST_PUSH_IND_FS_WRITE, ret);
    	     mmi_trace(1,"MMI_BT_OPP_OPPS_FIRST_PUSH_IND_FS_WRITE, ret=%d\n",ret);
    ...
    if (goep_push_ind->pkt_type == GOEP_SINGLE_PKT ||goep_push_ind->pkt_type == GOEP_FINAL_PKT)
    {
        U8 *ppath_buf;
        ...
        ppath_buf = OslMalloc(FMGR_MAX_PATH_LEN * ENCODING_LENGTH);
        MMI_OPP_ASSERT(ppath_buf != NULL);

        MMI_OPP_MAKE_RECV_FILENAME(ppath_buf, MMI_OPP_CTX(fcnt));

        mmi_opp_notify_register_app((U16*) ppath_buf);
...
static void mmi_bt_opps_next_push_ind_handler(void *msg)
{
 ...
 //  mmi_trace(1,"MMI_BT_OPP_OPPS_NEXT_PUSH_IND MMI_OPP_CTX(ops_state)=%d,MMI_OPP_CTX(h_recv)=%d ",MMI_OPP_CTX(ops_state),MMI_OPP_CTX(h_recv));
    if ((MMI_OPP_CTX(ops_state) == MMI_OPS_DISCONNECTING) || (MMI_OPP_CTX(ops_state) == MMI_OPS_DEACTIVATING))
    {
        /* Do Nothing */
        MMI_OPS_LOG_STATE_ERR();
...
static void mmi_bt_opps_push_ind_handler(void *msg)
{
    ...
    mmi_trace(1,"mmi_bt_opps_push_ind_handler,goep_push_ind->put_type=%d,goep_push_ind->pkt_type = %d,MMIOPP_GET_FLAG(MMIOPP_MASK_MULTI_RECEIV) =%d",goep_push_ind->put_type,goep_push_ind->pkt_type,MMIOPP_GET_FLAG(MMIOPP_MASK_MULTI_RECEIV));
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    ...
    if (goep_push_ind->pkt_type == GOEP_FIRST_PKT || goep_push_ind->pkt_type == GOEP_SINGLE_PKT ||
		(goep_push_ind->pkt_type == GOEP_FINAL_PKT && MMI_OPP_CTX(ops_state) != MMI_OPS_RECEIVING))
    {
#ifdef __BQB_PTS__
 ...
 *  void
 *****************************************************************************/
#define BT_SEND_COSMSG(MOD_DEST,P_COS_EVENT) \
	sxr_Send(P_COS_EVENT,((TASK_HANDLE *)GetMmiTaskHandle(MOD_DEST))->nMailBoxId, SXR_SEND_EVT)

#define bt_msg_send_ext_queue(ilm_ptr) \
do \
{ \
	COS_EVENT ev; \
	ev.nEventId = ilm_ptr->msg_id; \
	ev.nParam1 = (UINT32)ilm_ptr; \
	BT_SEND_COSMSG(ilm_ptr->dest_mod_id, &ev); \
} while (0)

#define BT_SEND_COSMSG_HEAD(MOD_DEST,P_COS_EVENT) \
	sxr_Send(P_COS_EVENT,((TASK_HANDLE *)GetMmiTaskHandle(MOD_DEST))->nMailBoxId, SXR_SEND_EVT|SXR_QUEUE_FIRST)

#define bt_msg_send_ext_queue_to_head(ilm_ptr) \
do \
{ \
	COS_EVENT ev; \
	ev.nEventId = ilm_ptr->msg_id; \
	ev.nParam1 = (UINT32)ilm_ptr; \
	BT_SEND_COSMSG_HEAD(ilm_ptr->dest_mod_id, &ev); \
} while (0)
...
static void mmi_bt_oppc_push_req(U8 goep_conn_id, goep_pkt_type_enum pkt_type, U32 total_obj_len, PS8 obj_name, PS8 obj_mime, U32 *frag_ptr, U16 frag_len)
{
    ...
    mmi_trace(1,"mmi_bt_oppc_push_req total_obj_len = %d\n",total_obj_len);
    /* compose ILM */
    ilm_ptr = allocate_ilm(MOD_MMI);
    ...
    bt_msg_send_ext_queue_to_head(ilm_ptr);
}
...
volatile UINT32 BtSendTime;
volatile UINT32 BtSendFileSize;
extern void sxr_CheckSbx (void);
...
void mmi_bt_oppc_connect_rsp_handler(void *msg)
{
    ...
    if (goep_connect_rsp->rsp_code == GOEP_STATUS_SUCCESS)
    {
        ...
        switch (MMI_OPP_CTX(opc_state))
        {
            ...
            {
		  BtSendTime = csw_TMGetTick ();
		  BtSendFileSize = MMI_OPP_CTX(total_obj_len);
...
static void mmi_bt_oppc_push_rsp_handler(void *msg)
{
    ...
    mmi_trace(g_sw_BT,"mmi_bt_oppc_push_rsp_handler MMI_OPP_CTX(oppc_goep_conn_id)=%d, MMI_OPP_CTX(opc_state)=%d\n",MMI_OPP_CTX(oppc_goep_conn_id),MMI_OPP_CTX(opc_state));
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    ...
    if (MMI_OPP_CTX(remain_put_len) > 0)
    {
		...
		mmi_trace(1," MMI_OPP_CTX(h_snd)  =%d\n",MMI_OPP_CTX(h_snd) );
            if (MMI_OPP_CTX(h_snd) >= 0)
            {
                errno = MMI_FS_Read(MMI_OPP_CTX(h_snd), buf_p, send_len, &read_len);
		...
		mmi_trace(1," MMI_OPP_CTX(h_snd)  =%d\n",MMI_OPP_CTX(h_snd) );
            if (MMI_OPP_CTX(h_snd) >= 0)
            {
                errno = MMI_FS_Read(MMI_OPP_CTX(h_snd), buf_p, MMI_OPP_CTX(remain_put_len), &read_len);
    ...
    else
    {
        ...
        MMI_OPC_STATE_TRANS(MMI_OPC_DISCONNECTING);
        mmi_bt_opp_disconnect_req(goep_push_rsp->goep_conn_id);
	BtSendTime = (csw_TMGetTick()-BtSendTime)/16384;
	if (BtSendTime>0)
		mmi_trace(1,"------------------------------BT BYTERATE = %d KByte/S------------------------------------\n",(BtSendFileSize/BtSendTime)/1024);
	#ifdef MMI_ON_HARDWARE_P
	sxr_CheckSbx();
	#endif
    }
...
void mmi_bt_oppc_abort_rsp_handler(void *msg)
{
	...
	BtSendTime = (csw_TMGetTick()-BtSendTime)/16384;
	if (BtSendTime > 0)
		mmi_trace(1,"------------------------------BT BYTERATE = %d KByte/S------------------------------------\n",((BtSendFileSize-MMI_OPP_CTX(remain_put_len))/BtSendTime)/1024);
	#ifdef MMI_ON_HARDWARE_P
	 sxr_CheckSbx();
	#endif
        MMI_OPC_STATE_TRANS(MMI_OPC_DISCONNECTING);
...
void mmi_opp_send_file_obj(void)
{
    ...
    h = MMI_FS_Open((const UINT8*) MMI_OPP_CTX(fwd_file_path), FS_READ_ONLY);
    ...
    MMI_FS_GetFileSize(MMI_OPP_CTX(h_snd), &len);
    ...
    if (filename_p)
    {
        ...
        {
        if (UCS2Strlen((S8*) filename_p) * 2 > (sizeof(MMI_OPP_CTX(sending_obj_name)) - 2))
        {
            memcpy(
                MMI_OPP_CTX(sending_obj_name),
                filename_p + (UCS2Strlen((S8*) filename_p) * 2) - (sizeof(MMI_OPP_CTX(sending_obj_name)) - 2),
    ...
    {
        csd_wsprintf((U16*) MMI_OPP_CTX(sending_obj_name), "%s", MMI_OPP_SND_DEFAULT_OBJNAME);
    }
...
extern  u8 need_deactive_profile_flag;
void mmi_opp_ops_connection_terminated(U8 cause)
{
    ...
    MMI_OPP_CTX(opps_cm_conn_id) = 0xff;       
    MMI_OPP_CTX(fcnt) = 0;
    if (!g_NeedRecheckOppRecv)	
    	MMI_OPP_CTX(cur_recv_file_cnt) = 0;
    MMI_OPP_CTX(disconnflag) = MMI_FALSE;

    MMI_OPS_STATE_TRANS(MMI_OPS_ACTIVE);    
		if(need_deactive_profile_flag)
	{
		need_deactive_profile_flag=0;
...
void mmi_opp_opc_connection_terminated(U8 cause)
{
    ...
    if (MMI_OPP_CTX(h_snd)>=0)
    {
        MMI_FS_Close(MMI_OPP_CTX(h_snd));
        MMI_OPP_CTX(h_snd) = (FS_HANDLE) -1;
    }
...
MMI_OPP_SND_ERR_TYPE mmi_opp_discovery_device(S8 *path, mmi_opp_sendobj_callback_fptr fp)
{
    ...
    {
        UCS2Strcpy(MMI_OPP_CTX(fwd_file_path), path);
    }
 ...
 *  void
 *****************************************************************************/
void mmi_bt_opp_event_hdlr(msg_type_t msg_id, void* msg)
{
    MMI_OPP_LOG_STATE();
        ...
        void *dst_dev)
{
    ...
    {
        UCS2Strcpy(MMI_OPP_CTX(fwd_file_path), file_name_with_path);
    }
    /* First, store the callback functionif any */
    /* In case send fail, clean up function will be able to callback */
...
u8 opps_state(void)//zhou siyou 20100429
{
	if(MMI_OPP_CTX(ops_state)==MMI_OPS_RECEIVING)
		return 1;
	else return 0;
