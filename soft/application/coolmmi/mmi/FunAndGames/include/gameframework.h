...
typedef struct {
	...
	U16	menu_help_str_id;				/* help string ID		 [OPTIONAL] */				
       U16 menu_soundswitch_str_id;    /* game sound switch string ID [OPTIONAL]*/
...
extern __align(2) const U8 				g_gfx_audio_gameover[862];
extern __align(2) const U8 				g_gfx_audio_complete[310];


#ifdef __MMI_GAME_MULTICHANNEL_SOUND__
	extern S32 	dummy_background_midi;  ///for dummy background
	extern __align(2) const U8 g_gfx_Dummy_background_midi[94];
...
extern void mmi_gx_store_setting(void);
extern void mmi_gx_load_setting(void);
...
do{\
	if((TRUE == GFX.is_vib_on) &&(TRUE == GFX.is_first_time_enter_gameover) )\
	{\
		PlayVibratorOnce();\
	}\
}while(0)
...
#define GFX_OPEN_MULTICHANNEL_MIDI(raw_data, size, repeats,handleno) \
	...
	//MIDI PLAY
#define GFX_PLAY_MULTICHANNEL_MIDI(audio_str, len, handle,style) \
		do{\
		if( (GFX.is_background_music_on==TRUE) || (GFX.is_sound_effect_on==TRUE) )\
		{\
			mdi_audio_play_string((void*) audio_str, len,MDI_FORMAT_MID,style,handle,NULL );\
		}\
	}while(0)
...
#define GFX_PLAY_BACKGROUND_SOUND(audio_str, len, handle,style) \
	do{	\
		if(GFX.is_background_music_on !=FALSE)	\
		{	\
			GFX_PLAY_MULTICHANNEL_MIDI(audio_str, len, handle,style);	\
		}	\
	}while(0)
...
#define GFX_PLAY_SOUND_EFFECTS_MIDI(audio_str, len, handle,style) \
	do{	\
		if(GFX.is_sound_effect_on !=FALSE)	\
		{	\
			GFX_PLAY_MULTICHANNEL_MIDI(audio_str, len, handle,style);	\
