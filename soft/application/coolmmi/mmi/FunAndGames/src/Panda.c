...
* Define
*****************************************************************************/
#if defined(__MMI_GAME_PANDA_128x160__) || defined(__MMI_GAME_MONKEY_128x160__)
	#define MMI_GX_PANDA_LCD_HEIGHT			(160)
	#define MMI_GX_PANDA_LCD_WIDTH			(128)
#elif defined(__MMI_GAME_PANDA_128x128__) || defined(__MMI_GAME_MONKEY_128x128__)
...
__align(2) const U8 Up_Down_Midi[] = {
...
#define PANDABOUNCE_MIDI 173

__align(2) const U8 PandaBounce_Midi[] = {
    0x4D,0x54,0x68,0x64,0x00,0x00,0x00,0x06,
    0x00,0x00,0x00,0x01,0x03,0xC0,0x4D,0x54,
    0x72,0x6B,0x00,0x00,0x00,0x97,0x00,0xFF,
    ...
    0x00,0xC0,0x7F,0x00,0xB0,0x07,0x64,0x00,
    0x90,0x5D,0x6F,0x00,0x5B,0x6F,0x00,0x59,
    0x67,0x00,0x58,0x5A,0x00,0xB9,0x07,0x50,
    0x00,0x99,0x26,0x5A,0x78,0x27,0x7D,0x28,
    0x90,0x5D,0x27,0x00,0x5B,0x27,0x00,0x59,
    0x25,0x00,0x58,0x20,0x83,0x38,0x99,0x27,
    0x00,0x82,0x68,0x90,0x58,0x00,0x00,0x5D,
    0x00,0x00,0x59,0x00,0x00,0x99,0x26,0x00,
    0x00,0x90,0x5B,0x00,0x81,0x20,0x59,0x00,
    0x00,0x58,0x00,0x00,0x5D,0x00,0x00,0x5B,
...
__align(2) const U8 PandaHit_Midi[] = {
    0x4D,0x54,0x68,0x64,0x00,0x00,0x00,0x06,
    0x00,0x00,0x00,0x01,0x03,0xC0,0x4D,0x54,
...
__align(2) const U8  BackGround_Panda_Midi[] = {
    0x4D,0x54,0x68,0x64,0x00,0x00,0x00,0x06,
    0x00,0x00,0x00,0x01,0x03,0xC0,0x4D,0x54,
...
__align(2) const U8 PandaDown_wav[] = {
...
__align(2) const U8 PandaFall_wav[] = {
...
__align(2) const U8 PandaEating_wav[] = {
...
__align(2) const U8 PandaBounce_dvi[1074] = {
...
__align(2) const U8 PandaDown_dvi[4155] = {
...
__align(2) const U8 PandaFall_dvi[1976] = {
...
__align(2) const U8 PandaHit_dvi[1528] = {
...
__align(2) const U8 PandaUp_down_dvi[1232] = {
...
__align(2) const U8 PandaEat_dvi[6164] = {
...
{
	TRACE_AUD_FUNCTION();
	//GFX_PLAY_AUDIO_GAMEOVER();//chenhe fix bug 11364.we don't support play buffer temporarily.

	/* call this function to draw standard gameover screen */
	mmi_gfx_draw_gameover_screen(IMG_ID_GX_PANDA_GOTEXT, IMG_ID_GX_PANDA_GRADESMAP, IMG_ID_GX_PANDA_GOPIC, g_gx_panda_context.game_grade);
}
...
{
	mmi_trace(TRUE, "Excute func : mmi_gx_panda_enter_game()");
...
#else
        TRACE_EF(g_sw_GAME, "MMI_PANDA: GFX.is_aud_on=%d",GFX.is_aud_on);
		if(TRUE == GFX.is_aud_on)//added by jinzh:20070724
		{
			mdi_audio_stop_all();				//added by jinzh:20070724
			AudioPlayReq(FNG_PANDA_SOUND,(U8)DEVICE_AUDIO_PLAY_INFINITE);	//added by jinzh:20070724
		}
#endif //__MMI_GAME_MULTICHANNEL_SOUND__	
	...
	mmi_trace(TRUE, "Excute func : mmi_gx_panda_draw_static_background() before");
	mmi_gx_panda_draw_static_background();
	mmi_trace(TRUE, "Excute func : mmi_gx_panda_draw_static_background() after");
	...
	mmi_trace(TRUE, "Excute func : mmi_gx_panda_cyclic_timer() after");
}
...
void	mmi_gx_panda_draw_static_background(void)
{
	...
	coolsand_UI_set_text_color(coolsand_UI_color(255,255,255));
	coolsand_UI_set_text_border_color(coolsand_UI_color(0,0,0));


	g_gx_panda_context.score_str_width = (S16)coolsand_UI_get_string_width(str_ptr);

	if(r2lMMIFlag)
	{
		#if defined(__MMI_GAME_PANDA_128x160__) || defined(__MMI_GAME_PANDA_128x128__) || defined(__MMI_GAME_MONKEY_128x160__) || defined(__MMI_GAME_MONKEY_128x128__)
			coolsand_UI_move_text_cursor(125, 3);
		#elif defined(__MMI_GAME_PANDA_176x220__) || defined(__MMI_GAME_MONKEY_176x220__)
			coolsand_UI_move_text_cursor(171, 3);
		#endif
			coolsand_UI_print_bordered_text(str_ptr);	
	...
	{
		#if defined(__MMI_GAME_PANDA_128x160__) || defined(__MMI_GAME_PANDA_128x128__) || defined(__MMI_GAME_MONKEY_128x160__) || defined(__MMI_GAME_MONKEY_128x128__)
			coolsand_UI_move_text_cursor(3, 3);
		#elif defined(__MMI_GAME_PANDA_176x220__) || defined(__MMI_GAME_MONKEY_176x220__)
			coolsand_UI_move_text_cursor(5, 3);
		#endif
			coolsand_UI_print_bordered_text(str_ptr);	
...
void mmi_gx_panda_exit_game(void)
{
	...
	coolsand_UI_cancel_timer(mmi_gx_panda_cyclic_timer);	/* cancle the looping timer */
      #ifdef __MMI_GAME_MULTICHANNEL_SOUND__	
...
void	mmi_gx_panda_init_enemy_list(void)
{
	...
	for (i = LEFT_TREE_LEFT; i <= RIGHT_TREE_RIGHT; i++)
	{
		...
		for (j = 0; j < MMI_GX_PANDA_ENEMY_COUNT; j++)
		{
			if (j == 0)
			{
				g_gx_panda_context.tree[i].enemy[j].position_y = MMI_GX_PANDA_LCD_HEIGHT + MMI_GX_PANDA_ENEMY_HEIGHT * (rand() % RIGHT_TREE_RIGHT);
...
void mmi_gx_panda_framemove(void)
{
	...
	if (	((g_gx_panda_context.tick <= 1800 && g_gx_panda_context.tick % 10 == 0) || (g_gx_panda_context.tick > 1800 && g_gx_panda_context.tick % 5 == 0)) && g_gx_panda_context.tick != 0)
	{
	...
	if ((g_gx_panda_context.game_grade + 5) % 50 == 0 )
	{
		...
		g_gx_panda_context.clock_x_position = (S16)mmi_gx_panda_get_x_position(g_gx_panda_context.clock_pos_state, MMI_GX_PANDA_IS_CLOCK);
	}
	...
	if (g_gx_panda_context.state != FLYING && g_gx_panda_context.old_state != FLYING)
	{
		...
		while (current_node != g_gx_panda_context.tree[g_gx_panda_context.pos_state].empty)
		{
			...
			{
				if ((current_node->position_y >= g_gx_panda_context.position.y && current_node->position_y < g_gx_panda_context.position.y + MMI_GX_PANDA_ROLE_HEIGHT) ||
					(g_gx_panda_context.position.y >= current_node->position_y && g_gx_panda_context.position.y < current_node->position_y + MMI_GX_PANDA_ENEMY_HEIGHT))
				{
					/* in attack mode */
...
void mmi_gx_panda_render(void)
{
	UI_image_ID_type			role_image = 0;
	S16							i=0;
	S16							enemy_x_position=0;
	S16							old_enemy_x_position=0;
	gx_panda_node_struct		*current_node =NULL;
	gx_panda_node_struct		*next_node = NULL;
	gx_panda_node_struct		*tmp_node = NULL;

	gdi_layer_set_active(g_gx_panda_context.foreground);
	
	gdi_layer_clear_background(MMI_GX_PANDA_TRANSPARENT_COLOR);
//	gdi_image_draw_id(0, 0, IMG_ID_GX_PANDA_BACKGROUND); //added by zhoumn@2007/0205
	...
	if (g_gx_panda_context.clock_falling != FALSE)
	{
		...
		gdi_image_draw_id(	g_gx_panda_context.clock_x_position,
	...
	for (i = LEFT_TREE_LEFT; i <= RIGHT_TREE_RIGHT; i++)
	{
		...
		while (current_node != g_gx_panda_context.tree[i].empty)
		{
			...
			if (i % 2 == 0)
			{
				gdi_image_draw_id(	enemy_x_position,
			...
			if (current_node->position_y + MMI_GX_PANDA_ENEMY_HEIGHT + 1 < 0 || current_node->position_y > MMI_GX_PANDA_LCD_HEIGHT)
			{
				/* normal case */
				if (current_node->is_dying != TRUE)
				{
		...
		if (g_gx_panda_context.state == FLYING)
		{
			...
			if (g_gx_panda_context.clock_falling != FALSE)
			{
				...
				if (	(g_gx_panda_context.clock_pos_state == LEFT_SPACE &&
						((g_gx_panda_context.old_pos_state == LEFT_TREE_RIGHT && g_gx_panda_context.pos_state == MID_TREE_LEFT) || (g_gx_panda_context.pos_state == LEFT_TREE_RIGHT && g_gx_panda_context.old_pos_state == MID_TREE_LEFT))) ||
						(g_gx_panda_context.clock_pos_state == RIGHT_SPACE &&
						((g_gx_panda_context.old_pos_state == RIGHT_TREE_LEFT && g_gx_panda_context.pos_state == MID_TREE_RIGHT) || (g_gx_panda_context.pos_state == RIGHT_TREE_LEFT && g_gx_panda_context.old_pos_state == MID_TREE_RIGHT)))
					)
				{
					/* the role does touch the clock */
					if (	(g_gx_panda_context.position.y >= g_gx_panda_context.clock_position_y && g_gx_panda_context.position.y < g_gx_panda_context.clock_position_y + MMI_GX_PANDA_CLOCK_HEIGHT) ||
							(g_gx_panda_context.clock_position_y >= g_gx_panda_context.position.y && g_gx_panda_context.clock_position_y < g_gx_panda_context.position.y + MMI_GX_PANDA_ROLE_HEIGHT))
					{
						/* extend 20 seconds */
	...
	gdi_layer_blt(	g_gx_panda_context.background,	/* layer 0 */
...
*	Drawing digits		
* PARAMETERS
*	x			X position for images
*	y			Y position for images
...
void	mmi_gx_panda_draw_digits(U16 x, U16 y, U16 number)
{
	...
	coolsand_UI_measure_string((UI_string_type)buf_USC2, &str_width, &str_height);

	coolsand_UI_move_text_cursor(x, y);
	
	coolsand_UI_print_bordered_text((UI_string_type)buf_USC2);	
...
* PARAMETERS
*	action	indicate the input is left-pressing or right-pressing
* RETURNS
*	void
* GLOBALS AFFECTED
*	NIL
...
void	mmi_gx_panda_turn_or_fly(U8 action)
{
	...
	if (g_gx_panda_context.state == FLYING)
	{
		g_gx_panda_context.position.x = (g_gx_panda_context.position.x + mmi_gx_panda_get_x_position(g_gx_panda_context.old_pos_state, MMI_GX_PANDA_IS_ROLE)) / 2;
...
void mmi_gx_panda_cyclic_timer(void)
{
	...
	coolsand_UI_start_timer(g_gx_panda_context.timer_elapse, mmi_gx_panda_cyclic_timer);
...
* PARAMETERS
*	state		Current position-state of object.
*	type		Type of object
* RETURNS
