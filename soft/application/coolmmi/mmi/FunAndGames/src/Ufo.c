...
UINT8 ufo_flag = 0;    //panxu add used to cs621 game 20070915
...
* Define
*****************************************************************************/
#if defined(__MMI_GAME_UFO_128x160__)
	#define MMI_GX_UFO_LCD_HEIGHT						(160)
	#define MMI_GX_UFO_LCD_WIDTH						(128)
#elif defined(__MMI_GAME_UFO_128x128__) 
	...
	#define MMI_GX_UFO_LCD_HEIGHT						(176)
	#define MMI_GX_UFO_LCD_WIDTH						(220)
#endif

#if defined(__MMI_GAME_UFO_128x160__) || defined(__MMI_GAME_UFO_128x128__)
...
#define MMI_GX_UFO_SCORE_X 4
#define MMI_GX_UFO_SCORE_Y 4
#define MMI_GX_UFO_ENERGY_X 92
#define MMI_GX_UFO_ENERGY_Y 4
#endif

#define MMI_GX_UFO_ENERGY_UNIT_COUNT			(4)
#define MMI_GX_UFO_TRANSPARENT_COLOR			(GDI_COLOR_TRANSPARENT)
...
__align(2) const U8 UFOExplode_Midi[] = {
...
__align(2) const U8 BackGround_UFO_Midi[] = {
    0x4D,0x54,0x68,0x64,0x00,0x00,0x00,0x06,
    0x00,0x00,0x00,0x01,0x03,0xC0,0x4D,0x54,
...
__align(2) const U8 UfoEat_midi[] = {
    0x4D,0x54,0x68,0x64,0x00,0x00,0x00,0x06,
    0x00,0x00,0x00,0x01,0x03,0xC0,0x4D,0x54,
...
__align(2) const U8 UfoExplode_wav[] = {
...
__align(2) const U8 UfoHit_midi[] = {
...
__align(2) const U8 UfoShoot_wav[] = {
...
__align(2) const U8 UfoEat_dvi[6164] = {
...
__align(2) const U8 UfoExplode_dvi[1965] = {
...
__align(2) const U8 UfoHit_dvi[1646] = {
...
__align(2) const U8 UfoShoot_dvi[1850] = {
...
{
/*Pan Xu Del Start For for test Mp4, this will be deleted in future Ver: rel 1.10  on 2007-2-1 15:17 */
	#if 0
	GFX_PLAY_AUDIO_GAMEOVER();
	#endif
/*Pan Xu Del End  For for test Mp4, this will be deleted in future Ver: rel 1.10  on 2007-2-1 15:17 */

	/* call this function to draw standard gameover screen */
	mmi_gfx_draw_gameover_screen(IMG_ID_GX_UFO_GOTEXT, IMG_ID_GX_UFO_GRADESMAP, IMG_ID_GX_UFO_GOPIC, g_gx_ufo_context.game_grade);
}
...
{
	TRACE_AUD_FUNCTION();
	TRACE_EF(g_sw_GAME, "MMI_UFO: g_gx_ufo_context.is_new_game = %d", g_gx_ufo_context.is_new_game);
...
/*QPAD doesn't need multi-layer.*/
	...
	GFX_PLAY_BACKGROUND_SOUND(BackGround_UFO_Midi, BACKGROUND_UFO_MIDI,&(g_gx_ufo_context.background_midi),DEVICE_AUDIO_PLAY_INFINITE);
...
#else
    TRACE_EF(g_sw_GAME, "MMI_UFO: GFX.is_aud_on=%d",GFX.is_aud_on);
	if(TRUE == GFX.is_aud_on)  
	{
		mdi_audio_stop_all();				//added by jinzh:20070724
#ifdef __MMI_SUPPORT_JME__
extern void VM_mmi_disable_qq_tone(void);
		VM_mmi_disable_qq_tone();
#endif
		AudioPlayReq(FNG_UFO_SOUND,(U8)DEVICE_AUDIO_PLAY_INFINITE);	//added by jinzh:20070724
	}
#endif //__MMI_GAME_MULTICHANNEL_SOUND__
...
void	mmi_gx_ufo_draw_static_background(void)
{
	...
	/*QPAD doesn't draw the background using image. It show the score and energy using text.*/
#ifndef __MMI_MAINLCD_220X176__
	#if defined(__MMI_GAME_UFO_128x160__) || defined(__MMI_GAME_UFO_128x128__)
...
void	mmi_gx_ufo_draw_score(void)
{
/*QPAD doesn't draw the background using image. It shows the score and energy using text.*/
#ifdef __MMI_MAINLCD_220X176__
	S8	buffer[32];
	S8	buffer_UCS2[32];
	S32	str_width = 0;
	...
	memset(buffer_UCS2, 0x00, sizeof(buffer_UCS2));	
	sprintf(buffer,"SCORE: %d",g_gx_ufo_context.game_grade);
	AnsiiToUnicodeString((PS8)buffer_UCS2, (PS8)buffer);		

	/*Clear the background of the score as black.*/
	coolsand_UI_measure_string((UI_string_type)buffer_UCS2, &str_width, &str_height);	
	coolsand_UI_set_clip(MMI_GX_UFO_SCORE_X, MMI_GX_UFO_SCORE_Y, MMI_GX_UFO_ENERGY_X - 1, MMI_GX_UFO_ORIGIN - 1);	
	coolsand_UI_set_text_clip(MMI_GX_UFO_SCORE_X, MMI_GX_UFO_SCORE_Y, MMI_GX_UFO_ENERGY_X - 1, MMI_GX_UFO_ORIGIN - 1);	
	coolsand_UI_fill_rectangle(MMI_GX_UFO_SCORE_X, MMI_GX_UFO_SCORE_Y, MMI_GX_UFO_ENERGY_X - 1, MMI_GX_UFO_ORIGIN - 1,*(current_MMI_theme->UI_black_color));	
	
	coolsand_UI_move_text_cursor(MMI_GX_UFO_SCORE_X, MMI_GX_UFO_SCORE_Y);
...
/*QPAD doesn't draw the background using image. It shows the score and energy using text.*/
	...
	memset(buffer, 0x00, sizeof(buffer));
	memset(buffer_UCS2, 0x00, sizeof(buffer_UCS2));
	sprintf(buffer,"ENERGY: %d",g_gx_ufo_context.energy);
	AnsiiToUnicodeString((PS8)buffer_UCS2, (PS8)buffer);

	/*Clear the background of the energy as black.*/	
	coolsand_UI_measure_string((UI_string_type)buffer_UCS2, &str_width, &str_height);		
	coolsand_UI_set_clip(MMI_GX_UFO_ENERGY_X, MMI_GX_UFO_ENERGY_Y, MMI_GX_UFO_LCD_WIDTH - 1, MMI_GX_UFO_ORIGIN - 1);	
	coolsand_UI_set_text_clip(MMI_GX_UFO_ENERGY_X, MMI_GX_UFO_ENERGY_Y, MMI_GX_UFO_LCD_WIDTH - 1, MMI_GX_UFO_ORIGIN - 1);	
	coolsand_UI_fill_rectangle(MMI_GX_UFO_ENERGY_X, MMI_GX_UFO_ENERGY_Y, MMI_GX_UFO_LCD_WIDTH - 1, MMI_GX_UFO_ORIGIN - 1,*(current_MMI_theme->UI_black_color));	

	coolsand_UI_move_text_cursor(MMI_GX_UFO_ENERGY_X, MMI_GX_UFO_ENERGY_Y);
	coolsand_UI_print_text((UI_string_type)buffer_UCS2);		
	...
	#if defined(__MMI_GAME_UFO_128x160__) || defined(__MMI_GAME_UFO_128x128__)
	gdi_draw_solid_rect(position_x, 2, position_x + MMI_GX_UFO_ENERGY_UNIT_COUNT * (MMI_GX_UFO_ENERGY_UNIT_WIDTH + 1), 10, GDI_COLOR_BLACK);
	#elif defined(__MMI_GAME_UFO_176x220__)
	gdi_draw_solid_rect(position_x, 2, position_x + MMI_GX_UFO_ENERGY_UNIT_COUNT * (MMI_GX_UFO_ENERGY_UNIT_WIDTH + 2), 10, GDI_COLOR_BLACK);
	#elif defined(__MMI_GAME_UFO_220x176__)	
	gdi_draw_solid_rect(position_x, 2, position_x + MMI_GX_UFO_ENERGY_UNIT_COUNT * (MMI_GX_UFO_ENERGY_UNIT_WIDTH + 2), 10, GDI_COLOR_BLACK);
	#endif
...
{
	coolsand_UI_cancel_timer(mmi_gx_ufo_cyclic_timer);	/* cancle the looping timer */
      #ifdef __MMI_GAME_MULTICHANNEL_SOUND__	
...
void mmi_gx_ufo_init_background_list(void)
{
	...
	g_gx_ufo_context.last_background_image = &g_gx_ufo_context.background_image_list[MMI_GX_UFO_BACKGROUND_LIST_NUMBER - 1];
}
...
void	mmi_gx_ufo_draw_background(void)
{
	...
	gdi_layer_set_clip(	0,
								MMI_GX_UFO_ORIGIN,
								MMI_GX_UFO_LCD_WIDTH - 1,
								MMI_GX_UFO_LCD_HEIGHT - 1);
...
void mmi_gx_ufo_framemove(void)
{
	...
	if (g_gx_ufo_context.is_boss_on != TRUE)
	{
		g_gx_ufo_context.first_background_position_y = g_gx_ufo_context.first_background_position_y + MMI_GX_UFO_BACKGROUND_SPEED;
	}
	else
	{
		#if defined(__MMI_GAME_UFO_128x160__) || defined(__MMI_GAME_UFO_128x128__)
		#elif defined(__MMI_GAME_UFO_176x220__)
		g_gx_ufo_context.first_background_position_y = g_gx_ufo_context.first_background_position_y + MMI_GX_UFO_BACKGROUND_SPEED + 2;
		#elif defined(__MMI_GAME_UFO_220x176__)	
		g_gx_ufo_context.first_background_position_y = g_gx_ufo_context.first_background_position_y + MMI_GX_UFO_BACKGROUND_SPEED + 2;
		#endif
	}
	...
	if (g_gx_ufo_context.is_treasure_on != FALSE && g_gx_ufo_context.fighter_state == MMI_GX_UFO_NORMAL_STATE)
	{
		if (	(g_gx_ufo_context.treasure_position_x >= g_gx_ufo_context.fighter_position_x && g_gx_ufo_context.treasure_position_x < g_gx_ufo_context.fighter_position_x + MMI_GX_UFO_ROLE_WIDTH && g_gx_ufo_context.treasure_position_y >= g_gx_ufo_context.fighter_position_y && g_gx_ufo_context.treasure_position_y < g_gx_ufo_context.fighter_position_y + MMI_GX_UFO_ROLE_HEIGHT) ||
				(g_gx_ufo_context.treasure_position_x + MMI_GX_UFO_TREASURE_WIDTH - 1 >= g_gx_ufo_context.fighter_position_x && g_gx_ufo_context.treasure_position_x + MMI_GX_UFO_TREASURE_WIDTH - 1 < g_gx_ufo_context.fighter_position_x + MMI_GX_UFO_ROLE_WIDTH && g_gx_ufo_context.treasure_position_y >= g_gx_ufo_context.fighter_position_y && g_gx_ufo_context.treasure_position_y < g_gx_ufo_context.fighter_position_y + MMI_GX_UFO_ROLE_HEIGHT) ||
				(g_gx_ufo_context.treasure_position_x >= g_gx_ufo_context.fighter_position_x && g_gx_ufo_context.treasure_position_x < g_gx_ufo_context.fighter_position_x + MMI_GX_UFO_ROLE_WIDTH && g_gx_ufo_context.treasure_position_y + MMI_GX_UFO_TREASURE_HEIGHT - 1 >= g_gx_ufo_context.fighter_position_y && g_gx_ufo_context.treasure_position_y + MMI_GX_UFO_TREASURE_HEIGHT - 1 < g_gx_ufo_context.fighter_position_y + MMI_GX_UFO_ROLE_HEIGHT) ||
				(g_gx_ufo_context.treasure_position_x + MMI_GX_UFO_TREASURE_WIDTH - 1 >= g_gx_ufo_context.fighter_position_x && g_gx_ufo_context.treasure_position_x + MMI_GX_UFO_TREASURE_WIDTH - 1 < g_gx_ufo_context.fighter_position_x + MMI_GX_UFO_ROLE_WIDTH && g_gx_ufo_context.treasure_position_y + MMI_GX_UFO_TREASURE_HEIGHT - 1 >= g_gx_ufo_context.fighter_position_y && g_gx_ufo_context.treasure_position_y + MMI_GX_UFO_TREASURE_HEIGHT - 1 < g_gx_ufo_context.fighter_position_y + MMI_GX_UFO_ROLE_HEIGHT))
		{
...
/*Pan Xu Del End  For for test Mp4, this will be deleted in future Ver: rel 1.10  on 2007-2-1 15:17 */
			...
			gdi_draw_solid_rect(g_gx_ufo_context.treasure_old_position_x, g_gx_ufo_context.treasure_old_position_y, g_gx_ufo_context.treasure_old_position_x + MMI_GX_UFO_TREASURE_WIDTH - 1, g_gx_ufo_context.treasure_old_position_y + MMI_GX_UFO_TREASURE_HEIGHT - 1, MMI_GX_UFO_TRANSPARENT_COLOR);
			gdi_layer_pop_clip();
			...
			else if (g_gx_ufo_context.treasure_type == MMI_GX_UFO_BOMB_TREASURE)
			{
				...
				for (enemy = g_gx_ufo_context.first_enemy; enemy != NULL && enemy != g_gx_ufo_context.first_empty_enemy_list; enemy = enemy->next)
				{
...
#else //__MMI_GAME_MULTICHANNEL_SOUND__	
/*Pan Xu Del Start For for test Mp4, this will be deleted in future Ver: rel 1.10  on 2007-2-1 15:18 */
					#if 0
					GFX_PLAY_AUDIO_DVI(UfoExplode_dvi, UFOEXPLODE_DVI, DEVICE_AUDIO_PLAY_ONCE);
					#endif
/*Pan Xu Del End  For for test Mp4, this will be deleted in future Ver: rel 1.10  on 2007-2-1 15:18 */
#endif //__MMI_GAME_MULTICHANNEL_SOUND__		
	...
	for (i = g_gx_ufo_context.first_bullet; i != NULL && i != g_gx_ufo_context.first_empty_bullet_list; i = i->next)
	{
		...
		{
/*panxu Del Start For for game Ver: rel1.10  on 2007-2-5 9:51 */
			#if 1
			REMOVE_BULLET_FROM_LIST(i);
			#endif
/*panxu Del End  For for game Ver: rel1.10  on 2007-2-5 9:51 */
		}
	}


	/* update positions of bullets of enemies */
	for (bullet = g_gx_ufo_context.first_enemy_bullet; bullet != NULL && bullet != g_gx_ufo_context.first_empty_enemy_bullet_list; bullet = bullet->next)
	{
		...
		if (bullet->old_x < -MMI_GX_UFO_ENEMY_BULLET_WIDTH || bullet->old_x > MMI_GX_UFO_LCD_WIDTH || bullet->old_y < MMI_GX_UFO_ORIGIN - MMI_GX_UFO_ENEMY_BULLET_HEIGHT || bullet->old_y > MMI_GX_UFO_LCD_HEIGHT)
		{
/*panxu Del Start For for game Ver: rel1.10  on 2007-2-5 9:50 */
			#if 1
			REMOVE_ENEMY_BULLET_FROM_LIST(bullet);
			#endif
/*panxu Del End  For for game Ver: rel1.10  on 2007-2-5 9:50 */
		}

		/* check if the bullet hit the fighter */
		if (	(g_gx_ufo_context.fighter_state == MMI_GX_UFO_NORMAL_STATE) && (
				(bullet->x >= g_gx_ufo_context.fighter_position_x && bullet->x < g_gx_ufo_context.fighter_position_x + MMI_GX_UFO_ROLE_WIDTH && bullet->y >= g_gx_ufo_context.fighter_position_y && bullet->y < g_gx_ufo_context.fighter_position_y + MMI_GX_UFO_ROLE_HEIGHT) ||
				(bullet->x + MMI_GX_UFO_ENEMY_BULLET_WIDTH - 1 >= g_gx_ufo_context.fighter_position_x && bullet->x + MMI_GX_UFO_ENEMY_BULLET_WIDTH - 1 < g_gx_ufo_context.fighter_position_x + MMI_GX_UFO_ROLE_WIDTH && bullet->y >= g_gx_ufo_context.fighter_position_y && bullet->y < g_gx_ufo_context.fighter_position_y + MMI_GX_UFO_ROLE_HEIGHT) ||
				(bullet->x >= g_gx_ufo_context.fighter_position_x && bullet->x < g_gx_ufo_context.fighter_position_x + MMI_GX_UFO_ROLE_WIDTH && bullet->y + MMI_GX_UFO_ENEMY_BULLET_HEIGHT - 1 >= g_gx_ufo_context.fighter_position_y && bullet->y + MMI_GX_UFO_ENEMY_BULLET_HEIGHT - 1 < g_gx_ufo_context.fighter_position_y + MMI_GX_UFO_ROLE_HEIGHT) ||
				(bullet->x + MMI_GX_UFO_ENEMY_BULLET_WIDTH - 1 >= g_gx_ufo_context.fighter_position_x && bullet->x + MMI_GX_UFO_ENEMY_BULLET_WIDTH - 1 < g_gx_ufo_context.fighter_position_x + MMI_GX_UFO_ROLE_WIDTH && bullet->y + MMI_GX_UFO_ENEMY_BULLET_HEIGHT - 1 >= g_gx_ufo_context.fighter_position_y && bullet->y + MMI_GX_UFO_ENEMY_BULLET_HEIGHT - 1 < g_gx_ufo_context.fighter_position_y + MMI_GX_UFO_ROLE_HEIGHT))
			)
		{
			...
			gdi_layer_push_clip();
			gdi_layer_set_clip(0, MMI_GX_UFO_ORIGIN, MMI_GX_UFO_LCD_WIDTH - 1, MMI_GX_UFO_LCD_HEIGHT - 1);
			gdi_draw_solid_rect(bullet->old_x, bullet->old_y, bullet->old_x + MMI_GX_UFO_ENEMY_BULLET_WIDTH - 1, bullet->old_y + MMI_GX_UFO_ENEMY_BULLET_HEIGHT - 1, MMI_GX_UFO_TRANSPARENT_COLOR);
			gdi_layer_pop_clip();
...
#else //__MMI_GAME_MULTICHANNEL_SOUND__	
/*Pan Xu Del Start For for test Mp4, this will be deleted in future Ver: rel 1.10  on 2007-2-1 15:21 */
			#if 0
			GFX_PLAY_AUDIO_DVI(UfoHit_dvi, UFOHIT_DVI, DEVICE_AUDIO_PLAY_ONCE);
			#endif
/*Pan Xu Del End  For for test Mp4, this will be deleted in future Ver: rel 1.10  on 2007-2-1 15:21 */
#endif //__MMI_GAME_MULTICHANNEL_SOUND__
	...
	for (enemy = g_gx_ufo_context.first_enemy; enemy != NULL && enemy != g_gx_ufo_context.first_empty_enemy_list; enemy = enemy->next)
	{
		...
		if (enemy->old_x < -enemy_width || enemy->old_x > MMI_GX_UFO_LCD_WIDTH || enemy->old_y < MMI_GX_UFO_ORIGIN - enemy_height /* 0 */ || enemy->old_y > MMI_GX_UFO_LCD_HEIGHT)
		{
		...
		for (i = g_gx_ufo_context.first_bullet; enemy->state == MMI_GX_UFO_NORMAL_STATE && i != NULL && i != g_gx_ufo_context.first_empty_bullet_list; i = i->next)
		{
			if (	(i->x >= enemy->x && i->x < enemy->x + enemy_width && i->y >= enemy->y && i->y < enemy->y + enemy_height) ||
					(i->x + i->type * MMI_GX_UFO_BULLET_WIDTH - 1>= enemy->x && i->x + i->type * MMI_GX_UFO_BULLET_WIDTH - 1 < enemy->x + enemy_width && i->y >= enemy->y && i->y < enemy->y + enemy_height) ||
					(i->x >= enemy->x && i->x < enemy->x + enemy_width && i->y + MMI_GX_UFO_BULLET_HEIGHT - 1 >= enemy->y && i->y + MMI_GX_UFO_BULLET_HEIGHT - 1 < enemy->y + enemy_height) ||
					(i->x + i->type * MMI_GX_UFO_BULLET_WIDTH - 1>= enemy->x && i->x + i->type * MMI_GX_UFO_BULLET_WIDTH - 1 < enemy->x + enemy_width && i->y + MMI_GX_UFO_BULLET_HEIGHT - 1 >= enemy->y && i->y + MMI_GX_UFO_BULLET_HEIGHT - 1 < enemy->y + enemy_height))
			{
				...
				gdi_layer_push_clip();
				gdi_layer_set_clip(0, MMI_GX_UFO_ORIGIN, MMI_GX_UFO_LCD_WIDTH - 1, MMI_GX_UFO_LCD_HEIGHT - 1);
				gdi_draw_solid_rect(i->old_x, i->old_y, i->old_x + i->type * MMI_GX_UFO_BULLET_WIDTH - 1, i->old_y + MMI_GX_UFO_BULLET_HEIGHT - 1, MMI_GX_UFO_TRANSPARENT_COLOR);
				gdi_layer_pop_clip();
/*panxu Del Start For for game Ver: rel1.10  on 2007-2-5 9:50 */
				#if 1
				REMOVE_BULLET_FROM_LIST(i);
...
#else //__MMI_GAME_MULTICHANNEL_SOUND__	
/*Pan Xu Del Start For for test Mp4, this will be deleted in future Ver: rel 1.10  on 2007-2-1 15:22 */
					#if 0
					GFX_PLAY_AUDIO_DVI(UfoExplode_dvi, UFOEXPLODE_DVI, DEVICE_AUDIO_PLAY_ONCE);
					#endif
/*Pan Xu Del End  For for test Mp4, this will be deleted in future Ver: rel 1.10  on 2007-2-1 15:22 */
#endif //__MMI_GAME_MULTICHANNEL_SOUND__
...
/*Pan Xu Del End  For for test Mp4, this will be deleted in future Ver: rel 1.10  on 2007-2-1 15:22 */
		...
		if (	(	
					...
					(
						(g_gx_ufo_context.fighter_state == MMI_GX_UFO_NORMAL_STATE) && 
						(
							(enemy->x >= g_gx_ufo_context.fighter_position_x && enemy->x < g_gx_ufo_context.fighter_position_x + MMI_GX_UFO_ROLE_WIDTH && enemy->y >= g_gx_ufo_context.fighter_position_y && enemy->y < g_gx_ufo_context.fighter_position_y + MMI_GX_UFO_ROLE_HEIGHT) ||
							(enemy->x + MMI_GX_UFO_ENEMY_WIDTH - 1 >= g_gx_ufo_context.fighter_position_x && enemy->x + MMI_GX_UFO_ENEMY_WIDTH - 1 < g_gx_ufo_context.fighter_position_x + MMI_GX_UFO_ROLE_WIDTH && enemy->y >= g_gx_ufo_context.fighter_position_y && enemy->y < g_gx_ufo_context.fighter_position_y + MMI_GX_UFO_ROLE_HEIGHT) ||
							(enemy->x >= g_gx_ufo_context.fighter_position_x && enemy->x < g_gx_ufo_context.fighter_position_x + MMI_GX_UFO_ROLE_WIDTH && enemy->y + MMI_GX_UFO_ENEMY_HEIGHT - 1 >= g_gx_ufo_context.fighter_position_y && enemy->y + MMI_GX_UFO_ENEMY_HEIGHT - 1 < g_gx_ufo_context.fighter_position_y + MMI_GX_UFO_ROLE_HEIGHT) ||
							(enemy->x + MMI_GX_UFO_ENEMY_WIDTH - 1 >= g_gx_ufo_context.fighter_position_x && enemy->x + MMI_GX_UFO_ENEMY_WIDTH - 1 < g_gx_ufo_context.fighter_position_x + MMI_GX_UFO_ROLE_WIDTH && enemy->y + MMI_GX_UFO_ENEMY_HEIGHT - 1 >= g_gx_ufo_context.fighter_position_y && enemy->y + MMI_GX_UFO_ENEMY_HEIGHT - 1 < g_gx_ufo_context.fighter_position_y + MMI_GX_UFO_ROLE_HEIGHT)
						)
					)
				) ||
				(
					...
					(
						(g_gx_ufo_context.fighter_state == MMI_GX_UFO_NORMAL_STATE) && 
						(
							(g_gx_ufo_context.fighter_position_x >= enemy->x && g_gx_ufo_context.fighter_position_x < enemy->x + enemy_width && g_gx_ufo_context.fighter_position_y >= enemy->y && g_gx_ufo_context.fighter_position_y < enemy->y + enemy_height) ||
							(g_gx_ufo_context.fighter_position_x + MMI_GX_UFO_ROLE_WIDTH - 1 >= enemy->x && g_gx_ufo_context.fighter_position_x + MMI_GX_UFO_ROLE_WIDTH - 1 < enemy->x + enemy_width && g_gx_ufo_context.fighter_position_y >= enemy->y && g_gx_ufo_context.fighter_position_y < enemy->y + enemy_height) ||
							(g_gx_ufo_context.fighter_position_x >= enemy->x && g_gx_ufo_context.fighter_position_x < enemy->x + enemy_width && g_gx_ufo_context.fighter_position_y + MMI_GX_UFO_ROLE_HEIGHT - 1 >= enemy->y && g_gx_ufo_context.fighter_position_y + MMI_GX_UFO_ROLE_HEIGHT - 1 < enemy->y + enemy_height) ||
							(g_gx_ufo_context.fighter_position_x + MMI_GX_UFO_ROLE_WIDTH - 1 >= enemy->x && g_gx_ufo_context.fighter_position_x + MMI_GX_UFO_ROLE_WIDTH - 1 < enemy->x + enemy_width && g_gx_ufo_context.fighter_position_y + MMI_GX_UFO_ROLE_HEIGHT - 1 >= enemy->y && g_gx_ufo_context.fighter_position_y + MMI_GX_UFO_ROLE_HEIGHT - 1 < enemy->y + enemy_height)
						)
					)
				)
			)
...
/*Pan Xu Del End  For for test Mp4, this will be deleted in future Ver: rel 1.10  on 2007-2-1 15:22 */
		...
		if (g_gx_ufo_context.is_boss_on != TRUE && ((g_gx_ufo_context.tick - enemy->starting_time) % (20 + (rand() % 3)) == 0))
		{
			/* new() a new bullt of enemy */
			NEW_ENEMY_BULLET();
		}
		/* when the enemy is the boss, it will shoot for every 0.4~0.6 seconds */
		else if (g_gx_ufo_context.is_boss_on != FALSE && ((g_gx_ufo_context.tick - enemy->starting_time) % (4 + (rand() % 3)) == 0))
		{
			NEW_ENEMY_BULLET();
		}
	}
	...
	if ((g_gx_ufo_context.tick + 1) % MMI_GX_UFO_BULLET_INTERVAL == 0 && (g_gx_ufo_context.fighter_state == MMI_GX_UFO_NORMAL_STATE || g_gx_ufo_context.fighter_state == MMI_GX_UFO_HIT_STATE))
	{
		NEW_FIGHTER_BULLET();
#ifdef __MMI_GAME_MULTICHANNEL_SOUND__	
	...
	if ((g_gx_ufo_context.tick + 1) % MMI_GX_UFO_ENEMY_INTERVAL == 0 && (g_gx_ufo_context.tick < MMI_GX_UFO_STAGE_INTERVAL * 4))
	{
		NEW_ENEMY();
	}
	/* after 127 seconds, the boss appears */
	else if ((g_gx_ufo_context.tick > g_gx_ufo_context.boss_appearing_time) && (g_gx_ufo_context.is_boss_on != TRUE) && (g_gx_ufo_context.tick < g_gx_ufo_context.boss_appearing_time + 2))
	{
		/* make sure that all enemies are out of screen */
		if (g_gx_ufo_context.first_enemy != NULL && g_gx_ufo_context.first_enemy != g_gx_ufo_context.first_empty_enemy_list)
		{
...
void mmi_gx_ufo_render(void)
{
	gx_ufo_bullet_struct			*bullet = NULL;
	gx_ufo_enemy_struct			*enemy = NULL;
	gx_ufo_enemy_bullet_struct	*enemy_bullet = NULL;
	S16								i = 0;
	UI_image_ID_type				image_type = 0;
	TRACE_AUD_FUNCTION();
	...
	if (g_gx_ufo_context.is_treasure_on != FALSE)
	{
		...
		gdi_image_draw_id(	g_gx_ufo_context.treasure_position_x,
	...
	for (enemy = g_gx_ufo_context.first_enemy; enemy != NULL && enemy != g_gx_ufo_context.first_empty_enemy_list; enemy = enemy->next)
	{
		...
		else
		{
			...
			{
				TRACE_AUD_FUNC("enemy is explorying!");
				TRACE_EF(g_sw_GAME, "MMI_UFO: enymy is explorying, enemy state = %d", enemy->state );
				/* exploding frame 1 */
	...
	/* draw enemy bullets */
	for (enemy_bullet = g_gx_ufo_context.first_enemy_bullet; enemy_bullet != NULL && enemy_bullet != g_gx_ufo_context.first_empty_enemy_bullet_list; enemy_bullet = enemy_bullet->next)
	{
		gdi_image_draw_id(enemy_bullet->x, enemy_bullet->y, (U16)(IMG_ID_GX_UFO_ENEMY_BULLET_1 + enemy_bullet->image_offset));
	}
	/* draw the fighter */
	/* in normal state */
	if (g_gx_ufo_context.fighter_state == MMI_GX_UFO_NORMAL_STATE)
	{
		gdi_image_draw_id(	g_gx_ufo_context.fighter_position_x,
	...
	else
	{
		...
		else
		{
			...
			{
				mmi_gx_ufo_draw_energy();
				gdi_image_draw_id(g_gx_ufo_context.fighter_position_x, g_gx_ufo_context.fighter_position_y, IMG_ID_GX_UFO_FLAME_1);
			...
			{
				gdi_image_draw_id(g_gx_ufo_context.fighter_position_x, g_gx_ufo_context.fighter_position_y, IMG_ID_GX_UFO_FLAME_2);
			...
			{
				gdi_image_draw_id(g_gx_ufo_context.fighter_position_x, g_gx_ufo_context.fighter_position_y, IMG_ID_GX_UFO_FLAME_3);
	...
	for (bullet = g_gx_ufo_context.first_bullet; bullet != NULL && bullet != g_gx_ufo_context.first_empty_bullet_list; bullet = bullet->next)
	{
		for (i = 0; i < bullet->type; i++)
		{
			gdi_image_draw_id(	bullet->x + i * MMI_GX_UFO_BULLET_WIDTH,
										bullet->y,
	...
	ufo_flag = 1;   //panxu add used to cs621 game 20070915
		gdi_layer_blt(	g_gx_ufo_context.background,	/* layer 0 */
						/*g_gx_ufo_context.foreground*/0,	/* layer 1 */ //changed by panxu 2007-8-4
						0,										/* layer 2 */
						0,										/* layer 3 */
						0,										/* offset x */
						0,										/* offset y */
	...
	ufo_flag = 0;     //panxu add used to cs621 game 20070915
...
void	mmi_gx_ufo_update_enemy_vector(gx_ufo_enemy_struct *enemy)
{
	...
	{
		enemy->vector_y = y_velocity;
		if (enemy->x + enemy->vector_x > MMI_GX_UFO_LCD_WIDTH - MMI_GX_UFO_ENEMY_WIDTH || enemy->x + enemy->vector_x < 0)
		{
...
/*****************************************************************************
* FUNCTION
*	mmi_gx_ufo_get_enemy_positon_x
* DESCRIPTION
*	Initialization of enemy position in x direction
* PARAMETERS
*	type			the moving type of the enemy
* RETURNS
...
void mmi_gx_ufo_cyclic_timer(void)
{
	...
	coolsand_UI_start_timer(g_gx_ufo_context.timer_elapse, mmi_gx_ufo_cyclic_timer);
	
	COS_Sleep(2); // to avoid draw ufo without reset watchDog,chenhe
