...
/*wuzc Del Start For GDI/GUI Portting Ver:    on 2008-3-10 11:14 */
#if 0
...
#endif
/*wuzc Del End  For GDI/GUI Portting Ver:    on 2008-3-10 11:14 */
...
{
    /* wtch_coor_type typ_timer */ {(LCD_WIDTH >> 1), 70},
    /* wtch_coor_type typ_milsec */ {(LCD_WIDTH >> 1), 85},
    /* typ_history_height, 2 line */ {MMI_MENUITEM_HEIGHT << 1},

    /* U16 separator_height */ {0},
    /* nWay_maintimer_start */ {0, 0},
    /* nWay_maintimer_end */ {LCD_WIDTH, MMI_MENUITEM_HEIGHT},
    /* nWay_dialog_start */ {0, MMI_MENUITEM_HEIGHT},
    /* nWay_dialog_end   */ {LCD_WIDTH, LCD_HEIGHT - MMI_BUTTON_BAR_HEIGHT - STPWACH_NW_FRAM_BORDER},
};
...
wtchSkinResStruct wtch_pos = 
{
    /* wtch_coor_type typ_timer */ {(LCD_WIDTH >> 1), 88},
    /* wtch_coor_type typ_milsec */ {(LCD_WIDTH >> 1), 105},
    /* typ_history_height, 2 line */ MMI_MENUITEM_HEIGHT << 1,

    /* U16 separator_height */ MMI_MENUITEM_HEIGHT,
    /* nWay_maintimer_start */ {0, 0},
    /* nWay_maintimer_end */ {LCD_WIDTH, MMI_MENUITEM_HEIGHT},
    /* nWay_dialog_start */ {0, MMI_MENUITEM_HEIGHT},
    /* nWay_dialog_end   */ {LCD_WIDTH, LCD_HEIGHT - MMI_BUTTON_BAR_HEIGHT - STPWACH_NW_FRAM_BORDER},
...
wtchSkinResStruct wtch_pos = 
{
    /* wtch_coor_type typ_timer */ {(LCD_WIDTH >> 1), (MMI_MENUITEM_HEIGHT << 2) + 61},
    /* wtch_coor_type typ_milsec */ {(LCD_WIDTH >> 1), (MMI_MENUITEM_HEIGHT << 2) + 73},
    /* typ_history_height, 4 line */ (MMI_MENUITEM_HEIGHT << 2),

    /* U16 separator_height */ 30,
    /* nWay_maintimer_start */ {10, 10},
    /* nWay_maintimer_end */ {LCD_WIDTH - 10, 40},
    /* nWay_dialog_start */ {10, 40},
    /* nWay_dialog_end   */ {LCD_WIDTH - 10, LCD_HEIGHT - MMI_BUTTON_BAR_HEIGHT - 10},
};
...
wtchSkinResStruct wtch_pos = 
{
    ...
    /* wtch_coor_type typ_timer */ {(LCD_WIDTH >> 1), (MMI_MENUITEM_HEIGHT << 2) + 61},
    /* wtch_coor_type typ_milsec */ {(LCD_WIDTH >> 1), (MMI_MENUITEM_HEIGHT << 2) + 73},
    /* typ_history_height, 4 line */ (MMI_MENUITEM_HEIGHT *5),
...
wtchSkinResStruct wtch_pos = 
{
    {(LCD_WIDTH >> 1) + 10, MAIN_LCD_DEVICE_HEIGHT - MMI_BUTTON_BAR_HEIGHT - 48},    /* wtch_coor_type typ_timer */ 
    {(LCD_WIDTH >> 1) + 100, MAIN_LCD_DEVICE_HEIGHT - MMI_BUTTON_BAR_HEIGHT - 48},    /* wtch_coor_type typ_milsec */
    {MAIN_LCD_DEVICE_HEIGHT - MMI_BUTTON_BAR_HEIGHT - 82},                             /* typ_history_height, 4 line */ 

    {30},                                                   /* U16 separator_height */ 
    {10, 10},                                               /* nWay_maintimer_start */ 
    {LCD_WIDTH - 10, 40},                                   /* nWay_maintimer_end */     
    {10, 40},                                               /* nWay_dialog_start */ 
    {LCD_WIDTH - 10, LCD_HEIGHT - MMI_BUTTON_BAR_HEIGHT - 10}, /* nWay_dialog_end */ 
};
/*- zhouqin 2011-02-16 modify for q6 320X240 */
#else 

// #error "LCD size not support!"
...
/*wuzc Del Start For GDI/GUI Portting Ver:    on 2008-3-10 11:15 */
#if 0
#ifdef MMI_ON_HARDWARE_P
...
#endif /* MMI_ON_HARDWARE_P */
#endif
extern U32 L1I_GetTimeStamp(void);
/*wuzc Del End  For GDI/GUI Portting Ver:    on 2008-3-10 11:15 */
...
void display_stop_watch_list_header(S32 x1, S32 y1, S32 x2, S32 y2)
{
    ...
    if (g_current_screen == MP_TIMER_SCR)
    {
        ...
        gui_fill_rectangle(
            ...
            UI_COLOR_DARK_GREY);
        gui_fill_rectangle(
            x1 + STPWACH_NW_FRAM_BORDER /* *2 */ ,
            y1 + STPWACH_NW_FRAM_BORDER /* *2 */ ,
            x2 - STPWACH_NW_FRAM_BORDER /* *2 */ ,
            y2 - STPWACH_NW_FRAM_BORDER /* *2 */ ,
            UI_COLOR_GREY);
        ...
        if (r2lMMIFlag)
        {
            /* gui_move_text_cursor((x1 + (((x2 - x1) >>1) - (width>>1))) +width, y1 + (row_height>>1)-(height>>1)); */
        ...
        {
            /* gui_move_text_cursor((x1 + (((x2 - x1) >>1) - (width>>1))) , y1 + (row_height>>1)-(height>>1)); */
            gui_move_text_cursor(
...
void increment_time_stopwatch(UI_stop_watch *stop_watch)
{
    ...
    /*----------------------------------------------------------------*/
    {
		...
		/*wangrui modify begin 2008.11.1 for fixbug 10117, 10119*/
//			TotalTime=TotalTime*4.6153846 ;
			TotalTime=TotalTime*0.06103515625 ;/*1 second = 16384*/
		/*wangrui modify 2008.11.1end*/
        if (TotalTime >= 10)
        {
 ...
 *  stop_watch_list         [?]         Stopwatch list
        ...
        S32 highlight_item)
{
    ...
    /* title_side_message = (UI_string_type) GetString(title_side_msg); */
...
/* extern U8    category89_string_list[]; */
...
void print_center_time(void)
{
    ...
    color_t text_col = gui_color(0, 0, 0);
    S32 w, h;
    ...
    change_stopwatchtime_to_string(0, center_hr, center_min, center_sec, time_str, DISPLAY_MILISEC_FALSE);

    /* sprintf((S8*)mili_string, "%02d", center_mili); */
    sprintf((S8*) mili_string, "%02d", center_mili);
    AnsiiToUnicodeString((PS8) uni_mili_string, (PS8) mili_string);

    gui_measure_string((UI_string_type) time_str, &w, &h);
    ...
    gui_print_text((UI_string_type) time_str);
...
U8 gbuffer_millitime[600 /* 550 */ ];   /* 32(row bytes) * 16 (y offset) = 512 */
U8 gbuffer_maintime[1200 /* 1100 */ ];  /* 102(row bytes) * 11(y offset) = 1122 */
    ...
    if (g_current_screen == TYPICAL_TIMER_SCR)
    {
        ...
        else
        {
            if ((gMainTimeUpdatedFlag == 1) || (gResetTimer))
            {
                ...
                w = gui_get_string_width((UI_string_type) time_str);
                if (r2lMMIFlag)
                {
                    gui_move_text_cursor(wtch_pos.typ_timer.x + w, wtch_pos.typ_timer.y);
                }
...
UI_image_ID_type  stopwatch_scr_bg_ID;
...
void ExitCategory307Screen(void)
{
    ...
    dm_set_scr_bg_image_no_draw(
        stopwatch_scr_bg_ID,
        stopwatch_scr_bg_filename,
        stopwatch_scr_bg_x,
        stopwatch_scr_bg_y,
        stopwatch_scr_bg_opacity);
#endif /* defined (__MMI_WALLPAPER_ON_BOTTOM__) */ 
...
void stop_watch_animation_processor()
{
    ...
    /* gdi_image_draw_animation_single_frame(stop_watch_x,stop_watch_y,stop_watch_img,(S16)stop_watch_n_frame); */
   ...
   /***********************************************************/
    ...
    _get_bitmap(
        ...
        (wtch_pos.typ_timer.x + 79),
        (wtch_pos.typ_timer.y + 15),
        &gdata_maintimer);
#else /* defined (__MMI_MAINLCD_240X320__) */ 
    _get_bitmap(
        wtch_pos.typ_timer.x,
        wtch_pos.typ_timer.y,
        (wtch_pos.typ_timer.x + 50),
        (wtch_pos.typ_timer.y + 10),
        &gdata_maintimer);
#endif /* defined (__MMI_MAINLCD_240X320__) */ 
   ...
   /***********************************************************/
    ...
    _get_bitmap(
        ...
        (wtch_pos.typ_milsec.x + 24) /* 70 */ ,
        (wtch_pos.typ_milsec.y + 22) /* 120 */ ,
        &gdata_millitimer);
#else /* defined (__MMI_MAINLCD_240X320__) */ 
    _get_bitmap(
        wtch_pos.typ_milsec.x,
        wtch_pos.typ_milsec.y,
        (wtch_pos.typ_milsec.x + 15) /* 70 */ ,
        (wtch_pos.typ_milsec.y + 15) /* 120 */ ,
        &gdata_millitimer);
#endif /* defined (__MMI_MAINLCD_240X320__) */ 
...
/*- zhouqin 2011-02-16 modify for q6 320X240 */
    ...
    dm_get_scr_bg_image(
        ...
        &stopwatch_scr_bg_y,
        &stopwatch_scr_bg_opacity);
    dm_set_scr_bg_image(IMG_TYPICALSW_LIST_BACKGROUND, NULL, 0, 0, 70);
#endif /* defined (__MMI_WALLPAPER_ON_BOTTOM__) */ 
    ...
    for (i = 0; i < number_of_items; i++)
    {
        ...
        change_stopwatchtime_to_string(
            lst_watch[i].mili_sec,
            lst_watch[i].hour,
            lst_watch[i].min,
            lst_watch[i].sec,
            lst_watch[i].watch_string,
            DISPLAY_MILISEC_TRUE);
        add_fixed_icontext_list_item_text(i, 1, (UI_string_type) lst_watch[i].watch_string);
    }

    register_fixed_list_keys();
//qiff modify for bug:10571
    resize_fixed_icontext_list_menuitems(0, 19) ;//MMI_menuitem_height);
...
void change_stopwatchtime_to_string(S32 milisec, S32 hour, S32 min, S32 sec, U8 *str_val, U8 display_milisec_flag)
{
    ...
    AnsiiToUnicodeString((PS8) str_val, (PS8) temp_str);
...
void show_typical_stop_watch(void)
{
   ...
   /***********************************************************/
    ...
    gui_measure_string((UI_string_type) time_str, &width, &height);
    wtch_pos.typ_timer.x -= (width >> 1);
#if defined (__MMI_MAINLCD_240X320__)
    _get_bitmap(wtch_pos.typ_timer.x, wtch_pos.typ_timer.y, (wtch_pos.typ_timer.x + 79), (wtch_pos.typ_timer.y + 22), &gdata_maintimer);        
#else /* defined (__MMI_MAINLCD_240X320__) */ 
    _get_bitmap(
        wtch_pos.typ_timer.x,
        wtch_pos.typ_timer.y,
   ...
   /***********************************************************/
    ...
    wtch_pos.typ_milsec.x -= (width >> 1);
#if defined (__MMI_MAINLCD_240X320__)
    _get_bitmap(wtch_pos.typ_milsec.x, wtch_pos.typ_milsec.y, (wtch_pos.typ_milsec.x + 24) /* 70 */ , (wtch_pos.typ_milsec.y + 22) /* 120 */ , &gdata_millitimer);
#else /* defined (__MMI_MAINLCD_240X320__) */ 
    _get_bitmap(
        wtch_pos.typ_milsec.x,
        wtch_pos.typ_milsec.y,
...
/*wuzc Add Start For GDI/GUI Portting Ver:    on 2008-3-12 13:34 */
#ifdef MMI_ON_HARDWARE_P
U32 L1I_GetTimeStamp(void)
{
	/*wangrui modify begin 2008.11.1 for fixbug 10117, 10119*/
//	return TM_GetTime();
...
#endif /* MMI_ON_HARDWARE_P */
/*wuzc Add End  For GDI/GUI Portting Ver:    on 2008-3-12 13:34 */
