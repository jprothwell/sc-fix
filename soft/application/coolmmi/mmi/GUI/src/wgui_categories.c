...
//qiff modify for bug 9077
 ...
 #if 0  //temp delete 20080618 by guojian  去掉触摸屏的vkeypad
...
gdi_layer_unlock_frame_buffer();
    ...
    dm_data.S32flags = DM_CLEAR_SCREEN_BACKGROUND;
#ifdef __MMI_TOUCH_SCREEN__
 //   dm_data.S32flags |= DM_SHOW_VKPAD;  //guojian deleted for jassmine 20080527 不显示vkpad
 //   dm_register_vkpad_callback(wgui_paging_virtual_keypad_callback);  //guojian deleted for jassmine 20080527 不显示vkpad
#endif /* __MMI_TOUCH_SCREEN__ */ 
    dm_setup_data(&dm_data);
    dm_redraw_category_screen();
...
///
	 ...
	 U8 *history_buffer)
 {
    ...
    MMI_fixed_list_menu.n_items = number_of_items;
       ...
       ////////////////// ShowStatusIconsTitle();
        //show_status_icon_bar(1);    //HEZHIPENG 2008-10-20 Bug #6349 。当水平的状态栏显示满行时,垂直的状态栏生效,但编辑任一通话记录时，垂直的状态栏的图标与电话记录显示重叠 。
	application_present = MMI_FALSE;
