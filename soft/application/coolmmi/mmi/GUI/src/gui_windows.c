...
/**********************************************************************************
   Filename:      gui_windows.c
   Date Created:  July-22-2002
   Contains:      UI routines for User Interface Windows
**********************************************************************************/
...
void gui_pop_up_description_scroll_handler(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    /* CSD added by JL 031128 for scrolling time can't stop */
...
void gui_show_pop_up_description(UI_pop_up_description *b)
{
    ...
    color_t text_color = b->text_color;
    ...
    for (i = 0; i < b->n_strings; i++)
    {
        if (b->text_strings[i] != NULL)
        {
            ...
            if (b->flags & UI_POP_UP_DESCRIPTION_MARQUEE_SCROLL)
            {
                ...
                if (r2lMMIFlag)
                {
                    gui_move_text_cursor(x1 + text_x - (text_width + UI_pop_up_description_scroll_gap), y1 + text_y);
                }
                else
                {
                    gui_move_text_cursor(x1 + text_x + text_width + UI_pop_up_description_scroll_gap, y1 + text_y);
