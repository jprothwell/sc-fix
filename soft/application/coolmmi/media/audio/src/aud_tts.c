...
#include "app_ltlcom.h"         /* Task message communiction */
    /* Declaration of buffer management API */
...
extern HANDLE g_TTSSem;//wangrui
...
//    aud_context_p->tts_event = kal_create_event_group("tts_events");
	g_TTSSem = COS_CreateSemaphore(0);
 ...
 *  void
 *****************************************************************************/
void aud_tts_play_event_ind( TTS_Event event, void *data)//TTS_Handle *hdl,
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
//    ind_p->handle = (kal_uint32)hdl;
 ...
 //       ind_p->data = (void *)get_ctrl_buffer(sizeof(TTS_PROCESS_DATA));
   ...
   //     ind_p->data = (void *)get_ctrl_buffer(sizeof(TTSPROGRESS));
    ...
    {
       kal_mem_set( ind_p->data, 0, sizeof(TTS_PROCESS_DATA));
    }
...
void aud_tts_pause_req_hdlr(ilm_struct *ilm_ptr)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    Media_Status result=MEDIA_FAIL;
