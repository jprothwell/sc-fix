...
const char * const id3_tag_v2_2_frame_id_list[NUM_OF_ID3_FRAME] = { "TT2", "TP1", "TAL", "TCO", "TYE" };
const char *const id3_tag_v2_3_frame_id_list[NUM_OF_ID3_FRAME] = { "TIT2", "TPE1", "TALB", "TCON", "TYER" };
const char *const id3_tag_v2_4_frame_id_list[NUM_OF_ID3_FRAME] = { "TIT2", "TPE1", "TALB", "TCON", "TDRC" };
const id3_tag_v1_field_struct id3_tag_v1_field_list[NUM_OF_ID3_FRAME] = { {3, 30}, {33, 30}, {63, 30}, {127, 1}, {93, 4} };
const  char * const id3_tag_genres[] =
{
	"Blues",
	"Classic Rock",
	"Country",
...
*     kal_uint8* src
*     kal_uint32 src_size (>=1)
* RETURNS
*     kal_int32
* GLOBALS AFFECTED
*     
*****************************************************************************/
kal_int32 id3_tag_utf16_to_ucs2( kal_uint8 byte_order, kal_uint8 *dest, kal_uint32 dest_size, kal_uint8* src, kal_uint32 src_size )
{
	kal_uint16 utf16;
...
*     
* PARAMETERS
*     kal_wchar *text: UCS2 and its format is *((0-9)+)*, i.e. (12) or (123), ....etc.
*     kal_uint32 text_size
* RETURNS
*     kal_int32
* GLOBALS AFFECTED
*     
...
kal_int32 id3_tag_parse_number( kal_wchar *text, kal_uint32 text_size )
{
	...
	{
		if( *text >= (kal_wchar)'0' && *text <= (kal_wchar)'9' )
		{
			if( val < 0 ) // check if it's the first time
				val = 0;
			val = val * 10 + (kal_int32)(*text - (kal_wchar)'0');
		}
		else if( *text == (kal_wchar)'(' || *text == (kal_wchar)')' )
		{
			if( val >= 0 )
				break; // finish
		}
...
*     
* PARAMETERS
*     id3_tag_text_pool_struct* id3_text_pool
* RETURNS
*     void
* GLOBALS AFFECTED
*     
...
void id3_text_pool_init( id3_tag_text_pool_struct* id3_text_pool )
{
	...
	id3_text_pool->available_words = ID3_TAG_POOL_SIZE;
}
...
*     
* PARAMETERS
*     id3_tag_frame_struct* id3_frame
* RETURNS
*     void
* GLOBALS AFFECTED
*     
...
*     
*****************************************************************************/
void id3_tag_init( id3_tag_struct* id3_tag_para )
{
	kal_int32 i;
	
	// init frames
	for( i = 0; i < NUM_OF_ID3_FRAME; i++ )
		id3_frame_init( &id3_tag_para->frames[i] );
		
	// init text_pool
	id3_text_pool_init( &id3_tag_para->text_pool );
}
...
*     
*****************************************************************************/
kal_bool id3_tag_parse_v2_tag_header( kal_uint8* data, id3_tag_struct* id3_tag_para )
{
	kal_uint8 flags, version;
	mmi_trace(1,"chenhe,id3_tag_parse_v2_tag_header,data[0] is %c,%c,%c",data[0],data[1],data[2]);
	...
	// init flags
	id3_tag_para->v2_major_ver = version;
	id3_tag_para->v2_unsync = KAL_FALSE;
	id3_tag_para->v2_exthdr = KAL_FALSE;
	id3_tag_para->v2_footer = KAL_FALSE;
	mmi_trace(1,"chenhe,id3_tag_parse_v2_tag_header,version is %d,flags is %d",version,flags);
	switch( version )
	{
	case 4:
		if( flags & 0x0F )
		...
		if( flags & 0x10 ) // check footer flag
			id3_tag_para->v2_footer = KAL_TRUE;
	case 3:
		if( flags & 0x1F )
			return KAL_FALSE;
		if( flags & 0x20 ) // check extended header flag
			id3_tag_para->v2_exthdr = KAL_TRUE;
		break;
	case 2:
		if( flags & 0x3F )
			return KAL_FALSE;
	...
	if( flags & 0x80 ) // check unsynchronisation flag
		id3_tag_para->v2_unsync = KAL_TRUE;


	// get tag size
	id3_tag_para->v2_size = id3_tag_get_unsync_uint32( data + 6, 4 ); // 6, 7, 8, 9
	mmi_trace(1,"chenhe,id3_tag_parse_v2_tag_header,version is %d,id3_tag_para->v2_size is %d",version,id3_tag_para->v2_size);
...
*     
*****************************************************************************/
kal_bool id3_tag_parse_v2_tag_extended_header( kal_uint8* data, id3_tag_struct* id3_tag_para, kal_bool data_full_load )
{
	...
	if( id3_tag_para->v2_unsync )
		tag_size = id3_tag_para->v2_reverse_unsync_size;
	else
		tag_size = id3_tag_para->v2_size;
	switch( id3_tag_para->v2_major_ver )
	{
	case 4:
	...
	id3_tag_para->v2_exthdr_size = extended_header_size;
	id3_tag_para->v2_crc = crc_data_present;
...
*     
*****************************************************************************/
kal_int8 id3_tag_parse_v2_frame_header( kal_uint8* data, kal_int32 size, id3_tag_frame_header_struct* frame_header, id3_tag_struct* id3_tag_para )
{
	kal_uint8 flags;
...
*     
*****************************************************************************/
kal_int8 id3_tag_match_v2_frame_id_from_list( kal_uint8* id, const char* const id_list[], kal_uint8 size )
{
	kal_int8 i;
	for( i = 0; i < NUM_OF_ID3_FRAME; i++ )
	{
...
*     
*****************************************************************************/
kal_bool id3_tag_parse_v2_frame_data( kal_uint8* data, id3_tag_frame_header_struct* frame_header, id3_tag_struct* id3_tag_para )
{
	kal_int8 frame_id_index = 0;
	...
	ptr = (kal_uint8*)id3_tag_para->text_pool.next_p;
	max_text_size = id3_tag_para->text_pool.available_words << 1;
	if( max_text_size < 4 )
		return KAL_FALSE;
	...
	{
		if( kal_wstrcmp( (const kal_wchar*)ptr, (kal_wchar *)L"RX" ) == 0 )
		{
			if( id3_tag_para->text_pool.available_words >= 6 ) // check available space in pool
			{
				kal_wstrcpy( (kal_wchar*)ptr, (kal_wchar *)L"Remix" );
				text_size = 12; // include UCS2 null terminator
			}
		}
		else if( kal_wstrcmp( (const kal_wchar*)ptr, (kal_wchar *)L"CR" ) == 0 )
		{
			if( id3_tag_para->text_pool.available_words >= 6 ) // check available space in pool
			{
				kal_wstrcpy( (kal_wchar*)ptr, (kal_wchar *)L"Cover" );
				text_size = 12; // include UCS2 null terminator
			}
		}
		else
		{
			genre_index = id3_tag_parse_number( (kal_wchar*)ptr, text_size ); // get genre number (index)
			if( genre_index >= 0 && genre_index < ID3_TAG_GENRE_NUM ) // check if the index is valid
			{
				...
				if( max_text_size <= (id3_tag_para->text_pool.available_words << 1) ) // check available space in pool
					text_size = (kal_uint32) mmi_chset_text_to_ucs2( (kal_uint8*)id3_tag_para->text_pool.next_p, max_text_size, ptr );
			}
		}
	}
	id3_tag_para->frames[frame_id_index].text = id3_tag_para->text_pool.next_p;
	id3_tag_para->frames[frame_id_index].size = text_size;
	id3_text_pool_consume( &id3_tag_para->text_pool, (text_size+1) >> 1 );
...
*     
*****************************************************************************/
kal_bool id3_tag_parse_v2_tag( audio_input_stream_struct* input_stream, id3_tag_struct* id3_tag_para )
{
	kal_bool data_full_load;
	kal_int8 parse_result;
	...
	id3_tag_frame_header_struct frame_header;
	mmi_trace(1,"chenhe,id3_tag_parse_v2_tag,");
	if( audio_input_stream_read( input_stream, 10 ) != 10 )
		return KAL_FALSE;
		mmi_trace(1,"chenhe,id3_tag_parse_v2_tag,111");
	 ...
	 * (4) v2_footer
	 */
	if( id3_tag_parse_v2_tag_header( ptr, id3_tag_para ) )
	{
			mmi_trace(1,"chenhe,id3_tag_parse_v2_tag,22222");

		// check if tag size <= our read buffer size
		if( (v2_tag_size = (kal_int32)id3_tag_para->v2_size) > buf_size )
		{
				mmi_trace(1,"chenhe,id3_tag_parse_v2_tag,v2_tag_size is %d,buf_size is %d",v2_tag_size,buf_size);
		...
		else
		{
			...
			mmi_trace(1,"chenhe,id3_tag_parse_v2_tag,else,line is %d",__LINE__);
		...
		if( id3_tag_para->v2_unsync )
		{
			...
			v2_tag_size = id3_tag_para->v2_reverse_unsync_size = (kal_int32)id3_tag_reverse_unsync_process( ptr, v2_tag_size );
		}
		
		// target size (v2_tag_size) not including 10 bytes v2 tag header
		target_size = v2_tag_size;
		if( id3_tag_para->v2_exthdr )
		{
			/*
			 ...
			 * (2) v2_crc
			 */
			if( !id3_tag_parse_v2_tag_extended_header( ptr, id3_tag_para, data_full_load ) )
				return KAL_FALSE;
			
			// point to frame start position
			offset += id3_tag_para->v2_exthdr_size;
			ptr += id3_tag_para->v2_exthdr_size;
			target_size -= id3_tag_para->v2_exthdr_size;
		}
		
		// parse frames
		for( frame_size = 0; target_size > 0 && ptr + 15 <= input_stream->tail && // 15 is the maximal frame header size
			(parse_result = id3_tag_parse_v2_frame_header( ptr, target_size, &frame_header, id3_tag_para )) != PARSE_ID3_FRAME_NO_SPACE;
		...
		if( !data_full_load )
		{
			...
			do
			{
				...
				parse_result = id3_tag_parse_v2_frame_header( ptr, target_size, &frame_header, id3_tag_para );
				...
				if( parse_result != PARSE_ID3_FRAME_ABORT && (kal_int32)frame_data_size <= buf_size )
				{
					...
					// parse frame data
					if( frame_header.data_size > 1 )
						id3_tag_parse_v2_frame_data( ptr, &frame_header, id3_tag_para );
				}
...
*     
*****************************************************************************/
kal_bool id3_tag_parse_v1_tag( audio_input_stream_struct* input_stream, id3_tag_struct* id3_tag_para )
{
	 ...
	 * and parse header and set the following 5 frames in id3_tag_para:
	 * (1) title
	 * (2) artist
	 * (3) album
	 * (4) year
	...
	for( i = 0; i < NUM_OF_ID3_FRAME; i++ )
	{
		...
		if( frame_p->text == NULL || frame_p->size < len )
		{
			...
			if( id3_tag_para->text_pool.available_words << 1 >= len )
			{
				// alloc space from pool
				frame_p->text = id3_tag_para->text_pool.next_p;
		...
		frame_p->size = len;
		if( need_alloc_pool )
			id3_text_pool_consume( &id3_tag_para->text_pool, (len+1) >> 1 );
	}
	return KAL_TRUE;
}
...
*     
*****************************************************************************/
kal_bool id3_tag_parse( audio_input_stream_struct* input_stream, id3_tag_struct* id3_tag_para )
{
	kal_int32 data_size;
	 ...
	 */
	v2_result = id3_tag_parse_v2_tag( input_stream, id3_tag_para );
	...
	if( (data_size = audio_input_stream_get_data_size( input_stream )) >= 128 )
	{
		 ...
		 * and parse & save data to frames
		 */
		v1_result = id3_tag_parse_v1_tag( input_stream, id3_tag_para );
	}
	else
		v1_result = KAL_FALSE;
