...
{
   player->active_tempo = 1024;
   player->active_tempo = player->active_tempo * player->tempo / (uint32)player->division;
   player->active_tempo = player->active_tempo * player->playRate / 100;
}
...
static int32 Ticks2ms( MIDI_Player *player, int32 ticks )
{
   ...
   return (ticks * 1000) / (((uint32)-(int8)(((uint16)player->division >> 8) & 0xFF)) * (uint32)((uint16)player->division & 0xFF));
}
...
static void SetMIP( MIDI_Player *player, uint8 *mip )
{
   ...
   for( I = 0; I < MAX_MIDI_CHANNEL; I++ ) {
      if( player->channel[I] != (MidiChannel*)0 )
         player->channel[I]->flag |= CHANNEL_FLAG_MUTE;
   }

   for( I = 0; ; I++ ) {
      ch = mip[I*2];
      if( ch >= MAX_MIDI_CHANNEL )
         break;
      if( mip[I*2+1] <= MAX_POLYPHONY ) {
         if( player->channel[ch] != (MidiChannel*)0 )
            player->channel[ch]->flag &= ~CHANNEL_FLAG_MUTE;
      }
   }
}
...
static void ME_NoteOff( MidiEvent *me, uint16 *cmd, Media_Handle *handle )
{
   ...
   if( chan == (MidiChannel *)0 || chan->flag & CHANNEL_FLAG_MUTE )
      return;
...
static void midiDspEnd( Media_Handle *handle )
{
   ...
   if( handle->state == MIDI_STATE_ABORT )
...
static void midiGetCmd( Media_Handle *handle, uint16 *dtime, uint16 cmd[4] )
{
   ...
   if( (player->eof && EventQ_Count(player) == 0) || handle->current_time >= handle->stop_time ) {
   ...
   while( EventQ_Count(player) > 0 && cmd[0] == DSPSYN_CMD_NO_DATA ) {
      ...
      if( note->dtime <= *dtime ) {
         if( handle->current_time < handle->start_time ) {
            if( note->type != ME_NOTEON && note->type != ME_NOTEOFF && note->type != ME_KEY_PRESSURE ) {
...
Media_Handle *JSmf_Open( void (*handler)( Media_Handle *handle, Media_Event event ), Media_SMF_Param **param )
{
   ...
   player = (MIDI_Player*)OslMalloc( sizeof(MIDI_Player) );
...
Media_Status JSmf_Close( Media_Handle *handle )
{
   ...
   OslMfree( (MIDI_Player*)handle->data );
...
int32 JSmf_GetMidiDuration( const uint8 *mid )
{
   ...
   tracks = (TrkParam*)OslMalloc( MAX_TRACK * sizeof(TrkParam) );
   ...
   while( 1 ) {
      ...
      delta = ( division > 0 )? delta * active_tempo >> 10 :
         (delta * 1000) / (((uint32)-(int8)(((uint16)division >> 8) & 0xFF)) * (uint32)((uint16)division & 0xFF));
      temp_dura += delta;
      trk = &tracks[next_trk];
