...
#if MT6238_SERIES
#include "Camera_webcam_v2.h"
#define __MMI_WEBCAM_NEW_IDP__
#else
...
kal_int32 webcam_capture_mem[2] = {122880, 0};
    ...
    kal_int32 webcam_preview_mem[2] = {38016, 0};
    kal_int32 webcam_capture_mem[2] = {38016, 0};
    #else
...
void webcam_start_cnf_hdlr(ilm_struct *ilm_ptr)
{
    ...
    if (WEBCAM_IN_STATE(WEBCAM_START_PREVIEW))
    {
        ...
        #ifdef __SW_JPEG_CODEC_SUPPORT__ 
            #ifdef __MMI_WEBCAM_NEW_IDP__
                usbvideo_param.jpeg_enc_y_buffer_addr = (kal_uint8*)webcam_context_p->jpeg_y_buf_p;
                usbvideo_param.jpeg_enc_y_buffer_size = webcam_context_p->jpeg_y_buf_size;
                usbvideo_param.jpeg_enc_u_buffer_addr = (kal_uint8*)webcam_context_p->jpeg_u_buf_p;
                usbvideo_param.jpeg_enc_u_buffer_size = webcam_context_p->jpeg_u_buf_size;
                usbvideo_param.jpeg_enc_v_buffer_addr = (kal_uint8*)webcam_context_p->jpeg_v_buf_p;
                usbvideo_param.jpeg_enc_v_buffer_size = webcam_context_p->jpeg_v_buf_size;         
                usbvideo_param.cam_capture_jpeg_cb = webcam_send_encode_request;
            #else           
...
void webcam_stop_req_hdlr(ilm_struct *ilm_ptr)
{
    ...
    if (WEBCAM_IN_STATE(WEBCAM_START_PREVIEW))
    {
        ...
        //webcam_send_usb_stop_video_req(NULL);
		USBVideo_Send_Msg(UVIDEOS_MSG_MED_STOP_CNF, (kal_uint32)KAL_TRUE, 0);
        WEBCAM_ENTER_STATE(WEBCAM_STOPPING);
...
void webcam_pause_req_hdlr(ilm_struct *ilm_ptr)
{
    ...
    if (WEBCAM_IN_STATE(WEBCAM_START_PREVIEW))
    {
        ...
        USBVideo_Send_Msg(UVIDEOS_MSG_MED_PAUSE_CNF, (kal_uint32)KAL_TRUE, 0);
...
#endif
        USBVideo_Send_Msg(UVIDEOS_MSG_MED_RESUME_SUCCESS_CNF, (kal_uint32)KAL_TRUE, 0);
		WEBCAM_ENTER_STATE(WEBCAM_START_PREVIEW);
...
void webcam_encode_req_hdlr(ilm_struct *ilm_ptr)
{
    ...
    {
    #ifdef __MMI_WEBCAM_NEW_IDP__    
        usbvideo_camera_frame_ready(JPEG_CODEC_STATE_COMPLETE);
    #else
