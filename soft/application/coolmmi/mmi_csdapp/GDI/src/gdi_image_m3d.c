 ...
 *==============================================================================
 *******************************************************************************/
#if defined(M3D_MMI_SUPPORT) && !defined(MMI_ON_WIN32)
#include "m3d_object3d.h"
#include "m3d_parser.h"
#include "m3d_graphics3d.h"
#include "m3d_class.h"
#endif /* defined(M3D_MMI_SUPPORT) && !defined(MMI_ON_WIN32) */ 
...
static BOOL gdi_image_m3d_rendering = FALSE;
static BOOL gdi_image_m3d_loaded = FALSE;
static BOOL gdi_image_m3d_inited = FALSE;
static BOOL gdi_image_m3d_drawing = FALSE;
#ifdef GDI_IMAGE_M3D_SUPPORT_COLOR_FORMAT_16_ONLY
    static gdi_handle gdi_image_m3d_output_layer_handle = GDI_NULL_HANDLE;
    static U8* gdi_image_m3d_output_layer_ptr = NULL;
#endif
...
static U8 gdi_image_m3d_pbFSALBuffer[FSAL_BUFFER_SIZE];
 ...
 * static U32 gdi_image_m3d_oob_image_data_size;            /// size in U8 of OOB data
...
GDI_RESULT gdi_image_m3d_mem_init(void)
{
    ...
    {
        gdi_image_m3d_mem_cntx.buf_size = m3d_mem;
    ...
    if (handle_oob)
    {
        ...
        if (gdi_image_m3d_oob_cntx.image_type != M3D_IMAGE_TYPE_NONE)
        {
            ...
            if (is_file)
            {
            	...
            	UCS2Strcat((S8*)gdi_m3d_file_name,(const S8*)gdi_image_ext_name[image_identifier]);
...
void gdi_image_m3d_show_profile(int x, int y, int w, int h)
{
    ...
    color_t text_color = gui_color(255, 255, 255);
    ...
    AnsiiToUnicodeString((char*)str_buf, str_tmp[2]);
            ...
            U32 img_size)
{
    ...
    if (is_file)
    {
        gdi_m3d_file_handle = DRM_open_file((U16*) data_ptr, FS_READ_ONLY | FS_OPEN_NO_DIR | FS_OPEN_SHARED, 0);
...
BOOL gdi_image_is_m3d_file(U8 *file_name)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    f = DRM_open_file((U16*) file_name, FS_READ_ONLY | FS_OPEN_NO_DIR, 0);
    if (f < 0)
    {
        return FALSE;
    }
