...
void mmi_data_unit_test(void)
{
    ...
    /*
     * // add records
     * pfnUnicodeStrcpy( (S8*)data, (S8*)L"data record store:(1) add a record done!" );
     * result = mmi_data_add_record( &unit_test_record_store, &record_id, data );
     * pfnUnicodeStrcpy( (S8*)data, (S8*)L"data record store:(2) add a record done!" );
     * result = mmi_data_add_record( &unit_test_record_store, &record_id, data );
     * pfnUnicodeStrcpy( (S8*)data, (S8*)L"data record store:(3) add a record done!" );
     * result = mmi_data_add_record( &unit_test_record_store, &record_id, data );
     * pfnUnicodeStrcpy( (S8*)data, (S8*)L"data record store:(4) add a record done!" );
     * result = mmi_data_add_record( &unit_test_record_store, &record_id, data );
     * 
     * // get records
     * memset( data, 0, 128 );
    ...
    /* set records */
    record_id = 0;
    pfnUnicodeStrcpy((S8*) data, (S8*) L"data record store:(01) set a record done!");
    result = mmi_data_set_record(&unit_test_record_store, record_id, data, TRUE);
    record_id = 1;
    pfnUnicodeStrcpy((S8*) data, (S8*) L"data record store:(02) set a record done!");
    result = mmi_data_set_record(&unit_test_record_store, record_id, data, TRUE);
    record_id = 2;
    pfnUnicodeStrcpy((S8*) data, (S8*) L"data record store:(03) set a record done!");
    result = mmi_data_set_record(&unit_test_record_store, record_id, data, TRUE);
    record_id = 3;
    pfnUnicodeStrcpy((S8*) data, (S8*) L"data record store:(04) set a record done!");
    result = mmi_data_set_record(&unit_test_record_store, record_id, data, TRUE);
...
static S32 mmi_data_scan_db_file(mmi_data_record_store_type *context)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	mmi_trace(g_sw_DataMgr,"entry mmi_data_scan_db_file");
    /* open the db file */
    if ((fd = MMI_FS_Open((UINT8 *)context->db_file, FS_READ_WRITE | FS_CREATE | FS_ATTR_HIDDEN)) >= 0)
    {
        INT32 size;
        U16 i;
		...
		mmi_trace(g_sw_DataMgr,"mmi_data_scan_db_file open success");
        ...
        for (i = 0; i < MAX_RECORD_SIZE && MMI_FS_Read(fd, context->buff, context->record_size + 1, &size) == FS_NO_ERROR;
             i++)
        {
    ...
    else
    {
		...
		mmi_trace(g_sw_DataMgr,"mmi_data_scan_db_file can't open the db file fd=%d",fd);
...
S32 mmi_data_init_record_store(mmi_data_record_store_type *context, S8 *db_file, S32 record_size)
{
	...
	mmi_trace(g_sw_DataMgr,"MMI_DATAMGR: mmi_data_init_record_store");
    if (record_size > MAX_RECORD_SIZE)
    {
        return DB_OUT_OF_LIMIT;
    }

/*Guojian Modify Start For phb_video Ver: Jassmine  on 2008-6-24 10:35 */
//   pfnUnicodeStrncpy((S8*) context->db_file, (S8*) db_file, FMGR_MAX_PATH_LEN);
   memcpy((S8*) context->db_file, (S8*) db_file, FMGR_MAX_PATH_LEN * ENCODING_LENGTH);
/*Guojian Modify End  For phb_video Ver: Jassmine  on 2008-6-24 10:35 */
...
S32 mmi_data_add_record(mmi_data_record_store_type *context, U16 *record_id, void *data)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	mmi_trace(g_sw_DataMgr," mmi_data_add_record");
    ...
    /* open the db file */
    if ((fd = MMI_FS_Open((UINT8 *)context->db_file, FS_READ_WRITE)) < 0)
    {
        /* can't open the db file, something wrong with it, so set it as inactive */
        context->is_active = 0;
        return (S32) fd;
    ...
    if ((result = MMI_FS_Seek(fd, offset, FS_FILE_BEGIN)) >= 0)
    {
        ...
        if (result == offset)
        {
            ...
            /* write to db file */
            if ((result = MMI_FS_Write(fd, context->buff, context->record_size + 1,  &size)) == FS_NO_ERROR)
            {
                *record_id = index;
...
S32 mmi_data_get_record(mmi_data_record_store_type *context, U16 record_id, void *data)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	mmi_trace(g_sw_DataMgr,"mmi_data_get_record record_id:%d context->is_active:%d, context->record_num:%d", record_id, context->is_active, context->record_num );
    /* check if it is active, otherwise perform a db file scan */
    if (!context->is_active)
    {
        if ((result = mmi_data_scan_db_file(context)) != 0)
    ...
    /* open the db file */
    mmi_trace(g_sw_DataMgr,"Func: %s open file context->db_file:%s", __FUNCTION__, context->db_file);
    if ((fd = MMI_FS_Open((UINT8 *)context->db_file, FS_READ_WRITE)) < 0)
    {
        /* can't open the db file, something wrong with it, so set it as inactive */
	    mmi_trace(g_sw_DataMgr,"Func: %s open file fd:0x%x", __FUNCTION__, fd);
        context->is_active = 0;
        return (S32) fd;
    }
    ...
    if ((result = MMI_FS_Seek(fd, offset, FS_FILE_BEGIN)) >= 0)
    {
        ...
        else if ((result = MMI_FS_Read(fd, context->buff, context->record_size + 1, &size)) == FS_NO_ERROR)
        {
...
S32 mmi_data_set_record(mmi_data_record_store_type *context, U16 record_id, void *data, BOOL always_add)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
mmi_trace(g_sw_DataMgr," mmi_data_set_record");
    ...
    if ((result = MMI_FS_Seek(fd, offset, FS_FILE_BEGIN)) >= 0)
    {
        ...
        if (result == offset)
        {
            ...
            memcpy(context->buff + 1, data, context->record_size);
            /* write to db file */
            result = MMI_FS_Write(fd, context->buff, context->record_size + 1, &size);
 ...
 *  S32
 *****************************************************************************/
extern INT32 FS_Truncate(INT32 fd, UINT32 nFileSize);
S32 mmi_data_delete_record(mmi_data_record_store_type *context, U16 record_id)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
mmi_trace(g_sw_DataMgr," mmi_data_delete_record");
                ...
                if (record_id == context->max_record_num - 1)
                {
                  ...
                  finish_find_last_used_slot:
                    ...
                    if ((result = MMI_FS_Seek(fd, offset, FS_FILE_BEGIN)) >= 0)
                    {
                        if (result == offset)
                        {
                            if (FS_Truncate(fd, offset) == FS_NO_ERROR)
                            {
...
S32 mmi_data_check_record(mmi_data_record_store_type *context, U16 record_id, BOOL check_db_file)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	mmi_trace(g_sw_DataMgr," mmi_data_check_record");
...
S32 mmi_data_check_space_for_set_record(mmi_data_record_store_type *context, U16 record_id)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	mmi_trace(g_sw_DataMgr," mmi_data_check_space_for_set_record");
...
/*Guojian Modify Start For phb_video Ver: Jassmine  on 2008-6-24 10:36 */
//    pfnUnicodeStrncpy((S8*) dir, (const S8*)context->db_file, 3);     /* copy "X:\" */
    memcpy((S8*) dir, (const S8*)context->db_file, 3 * ENCODING_LENGTH);     /* copy "X:\" */
/*Guojian Modify End  For phb_video Ver: Jassmine  on 2008-6-24 10:36 */
...
S32 mmi_data_check_space_for_add_record(mmi_data_record_store_type *context)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	mmi_trace(g_sw_DataMgr," mmi_data_check_space_for_add_record");
    ...
    if (!context->is_active)
    {
        if ((result = mmi_data_scan_db_file(context)) != 0)
        {
		mmi_trace(g_sw_DataMgr," mmi_data_check_space_for_add_record mmi_data_scan_db_file result=%d",result);
    ...
    {
        offset = (S64) context->record_size;
    }
    else
    {
        return 0;
...
/*Guojian Modify Start For phb_video Ver: Jassmine  on 2008-6-24 10:37 */
//    pfnUnicodeStrncpy((S8*) dir, (const S8*)context->db_file, 3);     /* copy "X:\" */
    memcpy((S8*) dir, (const S8*)context->db_file, 3 * ENCODING_LENGTH);     /* copy "X:\" */
/*Guojian Modify End  For phb_video Ver: Jassmine  on 2008-6-24 10:37 */
    /* check the available free space */
    result = MMI_FS_GetDiskInfo((UINT8 *)dir, &disc_info, FS_DI_BASIC_INFO | FS_DI_FREE_SPACE);
    if (result > 0 && (result & (FS_DI_BASIC_INFO | FS_DI_FREE_SPACE)))
    {
        if ((disc_info.FreeClusters * disc_info.SectorsPerCluster * disc_info.BytesPerSector) <= (U64) offset)
        {
            return FS_DISK_FULL;
        }
        else
