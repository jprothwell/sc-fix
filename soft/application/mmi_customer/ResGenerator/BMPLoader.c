...
U8 Load_BMP(bytestream *file)
{
    ...
    printf("The first color in palette is (%d,%d,%d)\n", palette[0][0], palette[0][1], palette[0][2]);
}

#if(!WINGUI_DIRECT_IMAGE_DISPLAY)

/* file:       input bitmap file (ROM)
   b:          output bitmap
   color_depth:    output bitmap color depth   */
...
U8 BMP_load(bytestream *file, bitmap *b, S8 color_depth)
{
    ...
    else if (info_header.bits_per_pixel == 32)//070306 Alpha layer
    {
    ...
    for (j = height - 1; j >= 0; j--)
    {
        ...
        //BMP_write_line(b, buffer, row_bytes, (S8) info_header.bits_per_pixel, width, j, palette);//070306 Alpha layer
        BMP_write_line(b, buffer, row_bytes, (S8) info_header.bits_per_pixel, color_depth, width, j, palette);//070306 Alpha layer
    }
    return (1);
}
...
int BMPLoader(char *in_filename, char *out_filename, S8 color_depth)
{
...
#if 1
    //ret = BMP_load(&inimage, &outimage, DEVICE_COLOR_DEPTH);
    ret = BMP_load(&inimage, &outimage, color_depth/*__MMI_DEVICE_BMP_FORMAT__*/);
    //ret = BMP_load(&inimage, &outimage, 16);
    if (ret == 0)
    {
    ...
    if (toolFlag == 1)  /* Write to file in binary */
    {
        fprintf(dest_file, "%c", 0x04);
        fprintf(dest_file, "%c", 0x00);
        total_data_size = bitmap_size + 13;
        fprintf(
            dest_file,
            "%c%c%c%c",
    ...
    else if (enfbFlag == 1) /* write ENFB header to CustImgDataxxx.h and image header/data to ENFB image data file */
    {
        ...
        fprintf(enfb_img_data_file, "%c", 0x00);
        total_data_size = bitmap_size + 13;
        fprintf(
            enfb_img_data_file,
            "%c%c%c%c",
