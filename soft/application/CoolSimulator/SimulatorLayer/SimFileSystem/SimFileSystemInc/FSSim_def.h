...
*  permission of Coolsand Inc. (C) 2009
*
 ...
 * NOTE: MoDIS will *NOT* define __COOLSAND_TARGET__ global option.
 */
#if defined(__CSD_TARGET__)
#error "__CSD_TARGET__ defined, wrong environment!"
#endif 
...
/* buffer size for Virtual File (include NULL tail) */
#define FS_GenVFN_SIZE 92
...
INLINE void fssim_printf(TCHAR* fmt, ...)
{
  TCHAR tch[1024];
  va_list var;
  va_start(var, fmt);
  _vstprintf(tch,fmt,var);
  va_end(var);
  
  _tcsncat(tch,_T("\n"),1024);
  WriteConsole(GetStdHandle(STD_OUTPUT_HANDLE),tch,_tcslen(tch),NULL,NULL);
}
...
typedef struct FSSIM_DRVINFO_STRUCT
{
    ...
    TCHAR dir_name[MAX_PATH];   /* the current directory in the drive */
} FSSIM_DRVINFO_T;
