...
/*
  ...
  thus be inaccurate.
*/
void dlmallinfo(unsigned char isAllMsg);
#endif /* NO_MALLINFO */
...
static struct mallinfo internal_mallinfo(mstate m,unsigned char isAllMsg) {
  int max_freeSpace=0;
  int prev_size=0;
  ensure_initialization();
  if (!PREACTION(m)) {
    ...
    if (is_initialized(m)) {
      ...
      while (s != 0) {
	    ...
	    if (max_freeSpace < sz)
            ...
            ++nfree;
	   if (isAllMsg)
	    	;//fprintf(stderr, "taskId=%d,free chunk %d;addr 0x%x,size %d,inuse sign: p %d,c %d,prev_size=%d\n",sxr_GetCurrentTaskId(),nfree,q,sz,pinuse(q),cinuse(q),prev_size);
          }
	  prev_size = chunksize(q);
          q = next_chunk(q);
        }
        s = s->next;
      }
    ...
    fprintf(stderr, "taskId=%d,non-mmapped space allocated from system =%d\n",sxr_GetCurrentTaskId(), nm.arena);
    fprintf(stderr, "taskId=%d,number of free chunks =%d\n",sxr_GetCurrentTaskId(), nm.ordblks);
    fprintf(stderr, "taskId=%d,space in mmapped regions = %d \n",sxr_GetCurrentTaskId(),nm.hblkhd);
    fprintf(stderr, "taskId=%d,maximum total allocated space = %d \n",sxr_GetCurrentTaskId(),nm.usmblks);
    fprintf(stderr, "taskId=%d,total allocated space = %d \n",sxr_GetCurrentTaskId(),nm.uordblks);
    fprintf(stderr, "taskId=%d,total free space = %d \n",sxr_GetCurrentTaskId(),nm.fordblks);
    fprintf(stderr, "taskId=%d,max_freeSpace = %d \n",sxr_GetCurrentTaskId(),max_freeSpace);
    fprintf(stderr, "taskId=%d,releasable (via malloc_trim) space = %d \n",sxr_GetCurrentTaskId(),nm.keepcost);
    POSTACTION(m);
  }
  return nm;
}
...
static void internal_malloc_stats(mstate m) {
  ...
  if (!PREACTION(m)) {
    ...
    fprintf(stderr, "taskId=%d,max system bytes = %10lu\n", sxr_GetCurrentTaskId(),(unsigned long)(maxfp));
    fprintf(stderr, "taskId=%d,system bytes     = %10lu\n", sxr_GetCurrentTaskId(), (unsigned long)(fp));
    fprintf(stderr, "taskId=%d,in use bytes     = %10lu\n", sxr_GetCurrentTaskId(), (unsigned long)(used));
  ...
  */
  ...
  if (MORECORE_CONTIGUOUS && !use_noncontiguous(m)) {
    ...
    if (ss == 0) {  /* First time through or recovery */
	  ...
	  //gcj_TraceOut(0,"base=0x%x LINE=%d",base,__LINE__);
      if (base != CMFAIL) {
        asize = granularity_align(nb + SYS_ALLOC_PADDING);
        /* Adjust to end on a page boundary */
        if (!is_page_aligned(base))
          asize += (page_align((size_t)base) - (size_t)base);
		//gcj_TraceOut(0,"base=0x%x LINE=%d asize=%d",base,__LINE__,asize);
	...
	//gcj_TraceOut(0,"tbase=0x%x br=0x%x,LINE=%d",tbase,br,__LINE__);
...
static int sys_trim(mstate m, size_t pad) {
  ...
  if (released >0)
  	gcj_TraceOut(0,"sys_trim released=%d\n",released);
...
#if !NO_MALLINFO
void dlmallinfo(unsigned char isAllMsg) {
   struct mallinfo  nm = internal_mallinfo(gm,isAllMsg);
}
#endif /* NO_MALLINFO */
