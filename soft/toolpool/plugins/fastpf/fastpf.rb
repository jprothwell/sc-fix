...
=end
    ...
    def checkFPCBufferSize(connection,flashProgAccess)
        ...
        begin
            ...
            if !((fpcSizeFlashProg == 32*1024) || (fpcSizeFlashProg == 4*1024))
                ...
                fpcSizeFlashProg = fpcSizeScript
            end
        rescue
        ...
        if (fpcSizeScript != fpcSizeFlashProg)
            sleep(0.5)
            crestart(connection, false)
            raise "FPC buffer size mismatch!"
        end
    end
    ...
    def dofastpfXfer(connection, flashProgrammerLodPackets, codeFileLodPackets, writeBootSector, verify)
...
puts Benchmark.measure{                  
        ...
        checkFPCBufferSize(connection,flashProgAccess)
...
puts Benchmark.measure {
    codeFileLodPackets.each { |packet|
            ...
            while( dataStart < packet.data.size )
                ...
                data = packet.data.slice(dataStart,@@C::FPC_BUFFER_SIZE)
           ...
           # Do the verify
            ...
            codeFileLodPackets.each{ |p| 
                ...
                while(datadone < p.data.size)
                    ...
                    datalen = (p.data.size - datadone>@@C::FPC_BUFFER_SIZE)?(@@C::FPC_BUFFER_SIZE):(p.data.size - datadone)
...
#######################################
    ...
    def dofastpfVerifyOnlyXfer(connection, flashProgrammerLodPackets, codeFileLodPackets, writeBootSector, indepth)
...
puts Benchmark.measure {
    ...
    def dofastpf( flash_programmer_filename , lod_filename, writeBootSector, verify)
        ...
        lodPackets = LOD_Read( lod_filename, @@C::FPC_BUFFER_SIZE )
        
        dofastpfPackets( LOD_Read( flash_programmer_filename ),  lodPackets, writeBootSector, verify) { |prog|
    ...
    def dofastpfVerifyOnly( flash_programmer_filename , lod_filename, writeBootSector, indepth )
        ...
        begin
            ...
            dofastpfVerifyOnlyXfer(connection, LOD_Read( flash_programmer_filename ), lodPackets, writeBootSector, indepth)
        ensure
            connection.close()
        end         
    ...
    def dofastpfPackets( flash_programmer_packets , lod_packets, writeBootSector, verify)
        begin
            #Don't use directly the $CURRENTCONNECTION, it could polute other apps
            connection = $CURRENTCONNECTION.copyConnection()
            connection.open(false)
            dofastpfXfer(connection, flash_programmer_packets, lod_packets, writeBootSector, verify) { |prog|
                yield(prog) if(block_given?())       
            }
        ensure
            connection.close()
...
addHelpEntry("fastpf", "fastpfVerify", "flash_programmer_filename, lod_filename, writeBootsector=FastPf::NOBOOTSECTORFASTPF, indepth=true, disable_event_sniffer = true","", "Do a fastpf verify with a specified flash programmer and lod file.")
def fastpfVerify( flash_programmer_filename, lod_filename, writeBootsector=FastPf::NOBOOTSECTORFASTPF, indepth=true, disable_event_sniffer = true )
    include FastPf
...
addHelpEntry("fastpf", "fastpfPackets", "flash_programmer_packets, lod_packets, writeBootsector=FastPf::NOBOOTSECTORFASTPF, verify=true, disable_event_sniffer = true","", "Do a fastpf from a LODPacket object.")
def fastpfPackets( flash_programmer_packets, lod_packets, writeBootsector=FastPf::NOBOOTSECTORFASTPF, verify=true, disable_event_sniffer = true )
    include FastPf
    ...
    begin
        ...
        dofastpfPackets(flash_programmer_packets , lod_packets, writeBootSector, verify) { |prog|
            if(block_given?())
                yield(prog)
            else 
                fastpfProgress(prog)
...
def fastSectorEraser( flash_programmer_filename, sector_list, disable_event_sniffer = true )
    ...
    sector_list.each { |sectorAddress|
        p = LODPacket.new
        p.address = sectorAddress.to_i
        p.data = []
        p.fcsChunkSize = @@C::FPC_BUFFER_SIZE
        p.rehash
        eraserLodPackets << p
    }
    
    # Never squash the boot sector
    fastpfPackets( LOD_Read(flash_programmer_filename), eraserLodPackets, FastPf::NOBOOTSECTORFASTPF, disable_event_sniffer) { |prog|
        yield(prog) if(block_given?())       
    }
