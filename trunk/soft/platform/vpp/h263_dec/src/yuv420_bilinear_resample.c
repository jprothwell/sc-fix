...
bilinear yuv420 image size resample
...
extern char * h263_myMalloc(int32 nsize);
...
int init_pixeltab(unsigned int ImgInw, unsigned int ImgInh, unsigned int ImgOutw,unsigned int ImgOuth)
{
	...
	PixelTab=h263_myMalloc(PIXEL_VALUE*maxScale);
	LimitTab=h263_myMalloc(PIXEL_VALUE*2);
...
void initResample(short lcd_width)
{
	...
	#else
	pResampleM=(uint8*)h263_myMalloc(lcd_width*5);
	#endif
}
/*****************
...
int Img_Resample_Yuv2Rgb_Embed(yuvImage * image, unsigned int ImgInw, unsigned int ImgInh, unsigned int extr_width, char* LcdBuf, short LcdBufWidth, short StartX, short StartY,  unsigned int ImgOutw,unsigned int ImgOuth)
{
	...
	//mmi_trace(TRUE, " LcdBufWidth=%d  StartX=%d   StartY=%d", LcdBufWidth,  StartX,  StartY);
...
#ifdef YUV2RGBLOOKUPTABLE
	L_tab = (int *) h263_myMalloc(256 * sizeof (int));
	Cr_r_tab = (int *) h263_myMalloc (256 * sizeof (int));
	Cr_g_tab = (int *) h263_myMalloc (256 * sizeof (int));
	Cb_g_tab = (int *) h263_myMalloc (256 * sizeof (int));
	Cb_b_tab = (int *) h263_myMalloc (256 * sizeof (int));
#endif  

#ifdef USE_INTERNAL_SRAM
	r_2_pix_alloc = (short *) YUV2RGB_START;//h263_myMalloc (1024 * sizeof (short));
	g_2_pix_alloc = (short *) (YUV2RGB_START+2048);//h263_myMalloc (1024 * sizeof (short));
	b_2_pix_alloc = (short *) (YUV2RGB_START+4096);//h263_myMalloc (1024 * sizeof (short));
#else
	r_2_pix_alloc = (short *) h263_myMalloc (1024 * sizeof (short));
	g_2_pix_alloc = (short *) h263_myMalloc (1024 * sizeof (short));
	b_2_pix_alloc = (short *) h263_myMalloc (1024 * sizeof (short));
#endif
