...
Prm2bits_MODE0:
	...
	VoC_add16_rd(REG1,REG1,	(6+TABLE_ERR_psrLevelMaxIncrease_ADDR));//4
...
void CII_avgGsHistQntz_gsQuant_opt(void)
{
	...
	VoC_startloop0;
		...
		VoC_bnltz32_r(CII_gsQuant_opt_abs_pos,RL6)
			// L_diff = - L_diff
			VoC_sub32_dr(RL6, (0+TABLE_ERR_psrLevelMaxIncrease_ADDR )  ,RL6);//CONST_0_ADDR
...
void CII_getPnBits_opt(void)
{
	...
	VoC_startloop0
		...
		VoC_and32_rd(RL6,(4+TABLE_SFRM_pswModNextBitUV_ADDR));//    CONST_0x00000005L_ADDR
...
void CII_lpcZsFir_opt(void)
{
	...
	//	VoC_lw32_d(ACC0,CONST_0x00000800L_ADDR);//    L_Sum = LPC_ROUND
		...
		VoC_bgt16_rd(CII_lpcZsFir_100,REG7,(12+TABLE_ERR_psrLevelMaxIncrease_ADDR));//10
...
CII_lpcZsFir_100:
    ...
    /* ------------ */
        ...
        VoC_lw16i(REG5,0x8000);
//		VoC_multf32inc_pd(RL7,REG0,CONST_0x8000_ADDR);	//L_Sum = L_msu(L_Sum, pswInput[siSmp], 0x8000);
      VoC_multf32inc_rp(RL7,REG5,REG0,DEFAULT);	//L_Sum = L_msu(L_Sum, pswInput[siSmp], 0x8000);
...
//	VoC_lw16i_short(REG5,10,DEFAULT);
    ...
    VoC_loop_i(1,2);
		...
		//	VoC_be16_rd(getNWCoefs_LABEL1,REG4,CONST_0x400_ADDR);
           VoC_be16_rr(getNWCoefs_LABEL1,REG4,REG5);
...
getNWCoefs_LABEL2:
	...
	//	VoC_sub16_rr(REG0,REG0,REG5,DEFAULT);
       VoC_sub16_rd(REG0,REG0,(12+TABLE_ERR_psrLevelMaxIncrease_ADDR));//10
...
//		VoC_lw32_d(ACC0,CONST_0x00000800L_ADDR);
		VoC_movreg32(ACC0,RL6,IN_PARALLEL);
		VoC_bgt16_rd(CII_lpcIrZsIir_100,REG1,(12+TABLE_ERR_psrLevelMaxIncrease_ADDR));//10
...
WEIGHTSPEECHFRAME_LOOP_START:
    ...
    VoC_be16_rd(WEIGHTSPEECHFRAME_go, REG5, (6+TABLE_ERR_psrLevelMaxIncrease_ADDR));	//4
...
*/
       VoC_blt16_rd(PERIODICITYUPDATE103, REG3,(4+TABLE_ERR_psrLevelMaxIncrease_ADDR));//2
			VoC_bnlt16_rd(PERIODICITYUPDATE104, REG5, (4+TABLE_ERR_psrLevelMaxIncrease_ADDR) );//2
...
//   VoC_sw16_d(REG5, HR_STATIC_VAD_swVeryOldLagCount_ADDR);
   ...
   VoC_bnlt16_rd(PERIODICITYUPDATE105, REG6, (9+TABLE_ERR_psrLevelMaxIncrease_ADDR));//7
	   VoC_lw16i_short(REG4, 0,DEFAULT);
...
exit_on_warnings=ON;
		   ...
		   // REG0 = 10*giSfrmCnt
		   VoC_mult16_rd(REG0, REG7, (12+TABLE_ERR_psrLevelMaxIncrease_ADDR));//10
...
//          }
    ...
    VoC_be16_rd(EN_LOOP_END,REG7,(6+TABLE_ERR_psrLevelMaxIncrease_ADDR));//4
...
EN_CODE_200://else
		...
		VoC_startloop0
			...
			VoC_mult16_rd(REG6, REG0, (4+TABLE_ERR_psrLevelMaxIncrease_ADDR));//2
            VoC_lw32_p(ACC0, REG5, DEFAULT);
            VoC_add16_rr(REG7,REG2,REG6, DEFAULT);

			VoC_inc_p(REG0, DEFAULT);
		    VoC_bngt16_rd(EN_LOOP1_100, REG0,(102+TABLE_SP_ROM_pppsrUvCodeVec_ADDR) );   //CONST_27_ADDR
...
/////////////////////////////////////////////////////////
  ...
  /*-----------------------------------*/
         ...
         VoC_lw16i_set_inc(REG0,GLOBAL_EN_pswSpeech_ADDR,1);
         VoC_add16_rd(REG1, REG0, 	(28+  TABLE_SP_ROM_psrLagTbl_ADDR));//160
...
SWCOMFORTNOISE_opt_100:
	...
	VoC_bez16_r(SWCOMFORTNOISE_opt_RETURN1,REG6);
		...
		VoC_mult16_rd(REG3,REG4,  	(7+   TABLE_ERR_psrR0RepeatThreshold_ADDR) );  //CONST_11_ADDR
...
exit_on_warnings=ON;
		...
		VoC_bne16_rd(updateCNHist_no_modulo,REG4,	(15+  TABLE_ERR_psrLevelMaxIncrease_ADDR) );//CONST_16_ADDR
		VoC_lw16i_short(REG4,0,DEFAULT);
updateCNHist_no_modulo:
		...
		VoC_lw16i_short(REG4,0,DEFAULT);
		VoC_bnlt16_rd(SWCOMFORTNOISE_opt_101,REG5,	(39+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR) );//CONST_24_ADDR
		VoC_lw16i_short(REG4,1,DEFAULT);
...
SWCOMFORTNOISE_opt_101:
	...
	//  	VoC_lw32_d(RL7,CONST_0x1091988L_ADDR);
		...
		// (swShortBurst == 0)
		VoC_bne16_rd(SWCOMFORTNOISE_opt_no_1st_SID,REG6,(10+TABLE_ERR_psrLevelMaxIncrease_ADDR));//8
...
SWCOMFORTNOISE_opt_no_1st_SID:
		...
		VoC_bnlt16_rd(SWCOMFORTNOISE_opt_106,REG6,(10+TABLE_ERR_psrLevelMaxIncrease_ADDR));//8
		// (swVadFrmCnt < OVERHANG)
...
void CII_aflatRecursionLoop_opt(void)
{
	...
	VoC_shr16_ri(REG2,-2,DEFAULT);
        VoC_lw16i(REG3,	( -4 + TABLE_SP_ROM_psvqIndex_ADDR) );
	// &psvqIndex[iSeg - 1].l
	VoC_add16_rr(REG2,REG2,REG3,DEFAULT);
...
CII_aflatRecurLoop_skipL1:
		...
		exit_on_warnings=ON;
			...
			// if (sub(pswPNew[0], 0x4000) >= 0) iLimit = 1
          		...
          		VoC_blt16_rd(CII_aflatRecurLoop_PNew,REG2,CONST_0x00004000L_ADDR);
...
//		VoC_msu32_pd(REG2,CONST_0x00008000_ADDR);

		// L_sum = L_msu(L_sum, pswPNew[0], SW_MIN);
		VoC_msu32_rp(REG0,REG2,DEFAULT);
...
exit_on_warnings = ON;
	...
	VoC_lw32_d(RL7,CONST_0x00008000_ADDR);
...
CII_lpcCorrQntz_opt_iVec_loop:
            ...
            VoC_lw16i(REG2,66);
        	VoC_push16(REG4,DEFAULT);
			VoC_mult16_rr(REG4,REG4,REG2,IN_PARALLEL);
	...
	//		VoC_mult16_rd(REG4,REG4,CONST_66_ADDR);
			...
			VoC_lw16i_short(REG3,2,IN_PARALLEL);
			 VoC_lw16i(REG0,66);
	       	VoC_mult16_rr(REG6,REG2,REG0,DEFAULT);
			// &bestQl[iSeq]
		...
		// &bestQl[0]
		  ...
		  VoC_lw16i_short(INC1,2,IN_PARALLEL);
			...
			VoC_be16_rd(CII_lpcCorrQntz_opt_copy,REG3,(6+TABLE_ERR_psrLevelMaxIncrease_ADDR));//4
...
CII_lpcCorrQntz_opt_no_copy:
		...
		VoC_blt16_rd(CII_lpcCorrQntz_opt_no_NewRecur1,REG7,	(5+TABLE_ERR_psrLevelMaxIncrease_ADDR));//3
...
CII_lpcCorrQntz_opt_no_NewRecur1:
		// pL_PBar & pL_VBar
			...
			// &psvqIndex[iSeg - 1].l
			VoC_mult16_rd(REG6,REG7,(6+TABLE_ERR_psrLevelMaxIncrease_ADDR));//4
			// &psvqIndex[-1].l
	...
	// for (j = 0; j < psvqIndex[iSegment - 1].len; j++)
		...
		VoC_loop_r(1,REG5)
			...
			VoC_bez32_r(CII_afNBR_swNShift_cmpt,RL7)
				...
				VoC_pop16(REG2,DEFAULT);
				VoC_add16_rd(REG4,REG0,(4+   TABLE_ERR_psrLevelMaxIncrease_ADDR));//2
				VoC_pop16(REG0,DEFAULT);
...
CII_afNBR_incr_invert:
		...
		VoC_sub16_rd(REG6,REG6,(4+   TABLE_ERR_psrLevelMaxIncrease_ADDR));//2
...
CII_lpcCorrQntz_opt_no_NewRecur:
		...
		// 4-iSeg
		VoC_sub16_dr(REG3,(6+TABLE_ERR_psrLevelMaxIncrease_ADDR),REG2);	//4
...
exit_on_warnings=ON;
	...
	//   REG0 psqlBestOutList[bstIndex].pswPredErr[0]
	 VoC_add16_rd(REG0,REG0,(4+   TABLE_ERR_psrLevelMaxIncrease_ADDR));//2
...
void CII_setupPreQ(void)
{
	...
	//     iWordHalfPtr = HIGH;
	VoC_be16_rd(SETUPPREQ_100, REG0, (4+   TABLE_ERR_psrLevelMaxIncrease_ADDR));  //  2 }
    	VoC_lw16i_short(REG7, 0,DEFAULT);                 //else if(iSeg==3)
SETUPPREQ_100:

	VoC_lw16i_short(REG4, 1,DEFAULT);
	VoC_movreg16(REG5,RL7_LO,IN_PARALLEL);
	VoC_be16_rd(SETUPPREQ_2, REG2, (4+   TABLE_ERR_psrLevelMaxIncrease_ADDR));     //  2 iWordPtr = iVector * 2;
	VoC_be16_rd(SETUPPREQ_3, REG2, (5+TABLE_ERR_psrLevelMaxIncrease_ADDR));     // 3  iWordHalfPtr = HIGH;
    // case 1:
	VoC_movreg16(REG5,REG6,DEFAULT);
...
SETUPPREQ_2://case2
    ...
    VoC_mult16_rd(REG3, REG3, (5+TABLE_ERR_psrLevelMaxIncrease_ADDR));//3
	// here REG4 == 1
...
void CII_flat_opt(void)
{
	...
	VoC_add16_rd(REG1,REG0,	(28+  TABLE_SP_ROM_psrLagTbl_ADDR));//160 pswIn[F_LEN+i-1] in REG1  CONST_00A0_ADDR
	VoC_movreg32(RL6,ACC0,DEFAULT);
	VoC_loop_i_short(10,DEFAULT);
	VoC_startloop0
		VoC_lw16inc_p(REG6,REG0,DEFAULT);
	...
	VoC_sub16_dr(REG1,(8+TABLE_ERR_psrLevelMaxIncrease_ADDR),REG1);//6
...
LAP1:
	   ...
	   VoC_bnlt16_rd(cov32kk, REG5, (11+TABLE_ERR_psrLevelMaxIncrease_ADDR));//9
...
LAP2:
	// reg1 & pswInScale[NP + F_LEN - kk]
       VoC_lw16i_short(INC1,-1,DEFAULT);
	   VoC_sub16_dr(REG4, (12+TABLE_ERR_psrLevelMaxIncrease_ADDR), REG5);//10
...
LAP8:
	   ...
	   VoC_startloop0;
		   ...
		   VoC_mac32inc_rp(REG7, REG3, DEFAULT);
		   VoC_add16_rd(REG6,REG6,(4+   TABLE_ERR_psrLevelMaxIncrease_ADDR));//2
		   VoC_or32_rr(RL6,ACC0,DEFAULT);
	   ...
	   VoC_bnez16_r(LAP3,REG5);
		 ...
		 // VoC_add16_rd(REG7, REG1,CONST_1_ADDR);//-swTemp in REG7
           ...
           VoC_movreg32(ACC1,RL6,DEFAULT);
		   VoC_and32_rd(ACC1, CONST_0x40000000_ADDR);
...
cov32and:
       ...
       VoC_sub16_dr(REG7, (4+   TABLE_ERR_psrLevelMaxIncrease_ADDR), REG1);//2  -swNorm in REG4
...
LAP3:
	   ...
	   VoC_add16_rd(REG3,REG2,(32+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR)); //  40 reg3 &pppL_C[1][0][0]

	   VoC_mult16_rd(REG7,REG5,(6+TABLE_ERR_psrLevelMaxIncrease_ADDR));//4
	   ...
	   VoC_mult16_rd(REG7,REG5,(32+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR));//40
	   VoC_sub16_rd(REG2,REG2,(6+TABLE_ERR_psrLevelMaxIncrease_ADDR));//4
...
LAP6:
	...
	VoC_sub16_dr(REG4, (10+TABLE_ERR_psrLevelMaxIncrease_ADDR), REG5);//8
...
EN_FLAT_opt_LOOP_START:
       ...
       VoC_lw16i_short(INC3,1,DEFAULT);
		...
		VoC_mult16_rd(REG3,REG6,(178+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR));//44
		// general purpose 1
		VoC_lw16i(REG5,396);
		VoC_lw16i_short(REG4,1,DEFAULT);
...
exit_on_warnings=ON;
		...
		* other used regs : none
		...
		// general purpose constant in REG45
   		VoC_lw32_d(REG45,CONST_0x00008000_ADDR);
...
EN_FLAT_opt_ABS_SWNUM:
		...
		VoC_bgt16_rr(EN_FLAT_opt_NOT_NULL_DIV,REG2,REG0);
			...
			VoC_sub16_dr(REG7,(12+TABLE_ERR_psrLevelMaxIncrease_ADDR),REG7);//10
			VoC_lw16i_short(REG6,0,DEFAULT);
			VoC_loop_r_short(REG7,DEFAULT);
			VoC_startloop0
...
exit_on_warnings=ON;
			 ...
			 ******************************/
      			...
      			VoC_jal(CII_NORM_L_ACC0);
			VoC_sub16_dr(REG7,(4+   TABLE_ERR_psrLevelMaxIncrease_ADDR),REG1);//2
...
void CII_hnwFilt_opt(void)
{
	...
	VoC_loop_r(1,REG7)
	    ...
	    VoC_add16_rd(REG7,REG2,(108+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR)  );//_39_ADDR
...
//		VoC_mac32_pd(REG7,CONST_0x4000_ADDR);//L_temp in ACC0
    	VoC_mac32_rp(REG0,REG7,DEFAULT);//L_temp in ACC0
...
*/
   	...
   	VoC_lw16_d(REG2, 	(1+  TABLE_SP_ROM_psrLagTbl_ADDR));//128
	VoC_lw16i_short(REG1, 0, DEFAULT);
...
void CII_get_ipjj_opt_paris(void)
{
    ...
    VoC_NOP();
   	...
   	VoC_multf32_rd(ACC0, REG0, (8+TABLE_ERR_psrLevelMaxIncrease_ADDR));//6
	VoC_NOP();
   	VoC_add32_rd(ACC0, ACC0, CONST_0x00008000_ADDR);
   	VoC_movreg16(REG0, ACC0_HI, DEFAULT);

	VoC_bne16_rd(GETIPJJ100, REG0, (8+TABLE_ERR_psrLevelMaxIncrease_ADDR));//6
   	VoC_lw16i_short(REG0, 0, DEFAULT);
...
fp_ex0:
	...
	VoC_blt16_rd(fp_ex03_go,REG7, (32+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR));//40
//	VoC_jump(fp_ex03);
     	VoC_return
fp_ex03_go:
		VoC_push16(RA,DEFAULT);
		...
		VoC_sub16_dr(REG6, (32+TABLE_SP_ROM_pppsrUvCodeVec_ADDR) ,REG7);//40

		VoC_bnlt16_rd(fp_ex00,REG1,(32+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR)); //40
...
MAXCCOVERGVI103:
			...
			VoC_sub16_dr(REG3,  (0+TABLE_ERR_psrLevelMaxIncrease_ADDR )   ,REG3);//CONST_0_ADDR
MAXCCOVERGVI100:
...
void CII_b_con_opt(void)
{
	...
	VoC_lw32_d(REG23,CONST_0x40000000_ADDR);
...
void CII_norm_s_opt(void)
{
	...
	VoC_startloop0
		VoC_bnlt16_rd(NORM_S_RETURN,REG2,CONST_0x00004000L_ADDR)
		VoC_shr16_ri(REG2,-1,DEFAULT);
		VoC_inc_p(REG0,IN_PARALLEL);
	VoC_endloop0
NORM_S_RETURN:
...
void CII_sqroot_opt(void)
{
	...
	// L_shr(L_SqrtIn, 1);
	VoC_sub32_rd(REG67, RL7, CONST_0x40000000_ADDR);
	// swTemp in REG7
	VoC_add32_rd(RL7, RL7, CONST_0x40000000_ADDR);
	// L_temp1 in RL7
...
// swSqrtOut = mac_r(L_Temp1, TERM6_MULTIPLER, swTemp2);
   VoC_add32_rr(ACC0, RL7, ACC0,DEFAULT);
   VoC_lw16i(REG5,0x5000);
   VoC_msu32_rr(REG7, REG5,DEFAULT);//L_Temp0 in ACC0
   //VoC_msu32_rd(REG7, CONST_0x5000_ADDR);//L_Temp0 in ACC0
   VoC_lw16i(REG5,0x7000);
...
rs_rrNs00:
	...
	VoC_shr16_ri(REG5,1,DEFAULT);
	// output REG1
    VoC_sub16_dr(REG1,  (0+TABLE_ERR_psrLevelMaxIncrease_ADDR )  ,REG1);//CONST_0_ADDR
	...
	VoC_add32_rd(REG01,REG67,CONST_0x00008000_ADDR);
...
exit_on_warnings=ON;
		...
		VoC_be16_rd(lpcZsIir_NP_IS_MIN,REG6,(12+TABLE_ERR_psrLevelMaxIncrease_ADDR));//10
...
// 1
	...
	VoC_startloop0
		...
		VoC_bnltz32_r(v_srch02,ACC0)
			VoC_sub32_dr(ACC0,CONST_0_ADDR,ACC0);//0
v_srch02:
...
exit_on_warnings = ON;
	...
	VoC_sub16_dr(REG1,(0+TABLE_ERR_psrLevelMaxIncrease_ADDR ),REG1);//0
...
exit_on_warnings=ON;
	...
	// nb to copy
	VoC_mult16_rd(REG0,REG4, (103+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR)  );//CONST_20_ADDR
...
exit_on_warnings=ON;
	...
	VoC_add16_rd(REG3,REG3,(32+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR));//40
...
//    VoC_NOP();
	...
	//siShiftCnt in reg1
	VoC_sub16_dr(REG1,(6+TABLE_ERR_psrLevelMaxIncrease_ADDR),REG1);//4
...
exit_on_warnings=ON;
        ...
        VoC_sub16_rd(REG2,REG2,(4+   TABLE_ERR_psrLevelMaxIncrease_ADDR));//2
		// here (after last write) :
...
// 3
	...
	VoC_be16_rd(v_srch_siEBits,REG4, (11+TABLE_ERR_psrLevelMaxIncrease_ADDR));//9
...
COMPILED_NEW_LABEL7:
		...
		VoC_lw16inc_p(REG6,REG3,DEFAULT);
		VoC_sub16_dr(REG6,(0+TABLE_ERR_psrLevelMaxIncrease_ADDR ),REG6);//0
...
//	VoC_lw16i_set_inc(REG2,TABLE_SFRM_pswBitIndexV_ADDR,1);
	...
	VoC_be16_rd(v_srch_psw_other,REG4, (11+TABLE_ERR_psrLevelMaxIncrease_ADDR));//9
...
void CII_sfrmAnalysis_opt(void)
{
	...
	VoC_lw16i(REG3,GLOBAL_SFRM_pswLtpStateBase_ADDR);
      	VoC_add16_rd(REG3,REG3, (15+TABLE_SP_ROM_psrLagTbl_ADDR));//   CONST_147_ADDR
...
lpcZiIir_skip_1st:
		...
		// restitute &pswCoef[0]
		VoC_sub16_rd(REG3,REG3,(12+TABLE_ERR_psrLevelMaxIncrease_ADDR));//10
...
exit_on_warnings=ON;
	...
	//	VoC_lw32_d(ACC0,CONST_0x001dff4cL_ADDR);
        ...
        VoC_lw16i(REG1,0x7ff);
   		VoC_bngt16_rr(sfrmAnalysis_000, REG5, REG1);
   		VoC_lw16i_short(REG6, 1, DEFAULT);
...
closedLoopLagSearch_loopstart:
			...
			VoC_add16_rr(REG0,REG4,REG0,DEFAULT);
      		VoC_add16_rd(REG5, REG5, (15+TABLE_SP_ROM_psrLagTbl_ADDR));  //CONST_147_ADDR
...
*********************************************/
		...
		VoC_pop32(REG45,DEFAULT);
	  	...
	  	VoC_mult16_rd(REG2,REG4,(32+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR)); //40
...
//  in STK16, &pVect, RA
	...
	//swLTPEnergy in REG5
   	...
   	VoC_lw16i_short(REG4, 0,DEFAULT);
	VoC_lw16i(REG2,0x7ff);
   	VoC_bngt16_rr(CLOSEDLOOPLAG100, REG5, REG2);
   	VoC_lw16i_short(REG4, 1, DEFAULT);
 ...
 ***********************************************************/
   	VoC_jal(CII_NORM_L_ACC0);
      	VoC_movreg32(ACC0, ACC1, DEFAULT);
		VoC_sub16_dr(REG5,(0+TABLE_ERR_psrLevelMaxIncrease_ADDR ),REG1);//0
//	VoC_movreg16(REG5,REG1,DEFAULT);//swCCShiftCnt in REG5

   	VoC_jal(CII_NORM_L_ACC0);
   //	VoC_movreg16(REG4,REG1,DEFAULT);//swCGShiftCnt in REG4
		VoC_sub16_dr(REG4,(0+TABLE_ERR_psrLevelMaxIncrease_ADDR ),REG1);//0
	// restitute initial values
	VoC_movreg32(REG01, RL6, DEFAULT);
...
// A
		...
		VoC_bez16_r(sfrmAnalysis_003,REG1);
			...
			VoC_add16_rd(REG2,REG2,(10+TABLE_ERR_psrLevelMaxIncrease_ADDR));//8
...
sfrmAnalysis_003:
		...
		VoC_startloop0
			...
			VoC_sub16_dr(REG6,(0+TABLE_ERR_psrLevelMaxIncrease_ADDR ),REG7);//0
...
sfrmAnalysis_loop0:
		...
		VoC_mult16_rd(REG1,REG0,(32+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR));//40
...
sfrmAnalysis_LOOP_100_START:
			...
			VoC_mult16_rd(REG1,REG4,(32+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR));//40
...
CII_g_quant_vl_Lable1:
		VoC_be16_rd(CII_g_quant_vl_Lable2,REG6,(4+   TABLE_ERR_psrLevelMaxIncrease_ADDR));  //2
//  From 3 to 5 loop, we only need one arg in reg0, if it is not
//  stocked there, we done a mov
		VoC_bne16_rd(CII_g_quant_vl_Lable3,REG6,(5+TABLE_ERR_psrLevelMaxIncrease_ADDR));//3
			VoC_movreg16(REG0,REG1,DEFAULT);
CII_g_quant_vl_Lable3:
		VoC_bne16_rd(CII_g_quant_vl_Lable4,REG6,(6+TABLE_ERR_psrLevelMaxIncrease_ADDR));//4
...
//  g_corr1, otherwise, g_corr2 is used.
		VoC_bgt16_rd(CII_g_quant_vl_Lable10,REG6,(4+   TABLE_ERR_psrLevelMaxIncrease_ADDR));//2
...
//  after that, it is the common part of round and save
		VoC_add32_rd(REG45,ACC1,CONST_0x00008000_ADDR);
		VoC_pop16(REG2,DEFAULT);
		VoC_sw16inc_p(REG5,REG3,DEFAULT);
		VoC_sw16inc_p(REG1,REG3,DEFAULT);
		VoC_pop32(REG01,DEFAULT);
	...
	VoC_sub16_rd(REG7,REG3,	(13+TABLE_ERR_psrLevelMaxIncrease_ADDR));//12
exit_on_warnings=OFF;
	VoC_push16(REG7,DEFAULT);
exit_on_warnings=ON;
...
//  if we map three different arguments to reg0,reg1,reg2
	...
	VoC_lw16i_short(REG6,2,DEFAULT);
	 ...
	 VoC_lw16i_short(INC1,1,IN_PARALLEL);
		...
		VoC_add16_rd(REG0,REG7,(8+TABLE_ERR_psrLevelMaxIncrease_ADDR));//6
...
//  reg2: &psErrorTerm[4].man
		...
		VoC_add16_rd(REG0,REG7,(6+TABLE_ERR_psrLevelMaxIncrease_ADDR));//4
        VoC_movreg16(REG1,REG0,DEFAULT);
		VoC_add16_rd(REG2,REG7,(12+TABLE_ERR_psrLevelMaxIncrease_ADDR));//10
//  if loop_counter == 1
//  reg0: &psErrorTerm[2].man
//  reg1: &psErrorTerm[2].man
//  reg2: &psErrorTerm[5].man
LOOP_ADDR2:
		VoC_bne16_rd(LOOP_ADDR3,REG5,(4+   TABLE_ERR_psrLevelMaxIncrease_ADDR));//2
		VoC_add16_rr(REG0,REG7,REG6,DEFAULT);
        VoC_movreg16(REG1,REG0,DEFAULT);
		VoC_add16_rd(REG2,REG7,(8+TABLE_ERR_psrLevelMaxIncrease_ADDR));//6
//  if loop_counter == 2
//  reg0: &psErrorTerm[1].man
//  reg1: &psErrorTerm[1].man
//  reg2: &psErrorTerm[3].man
LOOP_ADDR3:
		VoC_bne16_rd(LOOP_ADDR4,REG5,(5+TABLE_ERR_psrLevelMaxIncrease_ADDR));//3
...
LOOP_ADDR4:
		...
		VoC_bne16_rd(LOOP_ADDR5,REG5,(6+TABLE_ERR_psrLevelMaxIncrease_ADDR));//4
		VoC_movreg16(REG0,REG7,DEFAULT);
        VoC_movreg16(REG2,REG7,IN_PARALLEL);
		VoC_add16_rd(REG1,REG7,(10+TABLE_ERR_psrLevelMaxIncrease_ADDR));//8
...
//  save the values in REG5 & REG67 inorder to use them after
        ...
        VoC_lw16_sd(REG6,0,DEFAULT);
		VoC_bne16_rd(LOOP_END1,REG6,(5+TABLE_ERR_psrLevelMaxIncrease_ADDR));//3
            VoC_sub16_rd(REG5,REG5,CONST_1_ADDR);
LOOP_END1:
...
//  swNum = sub(terms[0].man, terms[1].man);
	...
	VoC_jal(CII_NORM_L_ACC0);//swShift in reg1
	VoC_sub16_dr(REG1,(0+TABLE_ERR_psrLevelMaxIncrease_ADDR ),REG1);//0
	VoC_sub16_dr(REG2,CONST_0x0000FFFF_ADDR,REG1);//siNormShift in reg2
...
gainTweak99:
	...
	VoC_add32_rd(REG45,RL6,CONST_0x00008000_ADDR);//swGainTweak in reg5


	VoC_lw16i(REG4,0x5a82);
//  siNormShift>-1
	VoC_bgt16_rd(gainTweak_end,REG2,CONST_0x0000FFFF_ADDR);

	VoC_bgt16_rd(gainTweak003,REG5,CONST_0x5a82_ADDR);
	VoC_blt16_rd(gainTweak003,REG2,CONST_0x0000FFFF_ADDR);
		VoC_movreg16(REG4,REG5,DEFAULT);
gainTweak003:
...
//  only the 1er,3th,4th loop need to move the args from reg to reg.
		VoC_bez16_r(CII_g_quant_vl_Lable21,REG6);
		VoC_bne16_rd(CII_g_quant_vl_Lable23,REG6,(5+TABLE_ERR_psrLevelMaxIncrease_ADDR));//3
CII_g_quant_vl_Lable21:
			VoC_movreg16(REG4,REG5,DEFAULT);
CII_g_quant_vl_Lable23:
		VoC_bne16_rd(CII_g_quant_vl_Lable24,REG6,(6+TABLE_ERR_psrLevelMaxIncrease_ADDR));//4
			VoC_movreg16(REG5,REG4,DEFAULT);
...
//  the following part is not done by the first and second loop,
		VoC_bngt16_rd(CII_g_quant_vl_Lable28,REG6,CONST_1_ADDR);
			...
			VoC_sub16_pr(REG0,REG2,REG1,DEFAULT);
			 VoC_lw16i_short(INC1,1,IN_PARALLEL);
			VoC_add32_rd(RL6,RL6,CONST_0x00008000_ADDR);
...
//	ErrorTerm[2].sh = add(ErrorTerm[2].sh, swShift);
		...
		VoC_add32_rd(RL6,RL6,CONST_0x00008000_ADDR);//ErrorTerm[1].man in reg7
		VoC_movreg16(REG7,RL6_HI,DEFAULT);
...
g_quant_vl_111:
    ...
    VoC_pop16(REG7,DEFAULT);
	VoC_mult16_rd(REG2,REG7,	(28+  TABLE_SP_ROM_psrLagTbl_ADDR));//160
...
// H
	...
	VoC_mult16_rd(REG0,REG6,	(28+  TABLE_SP_ROM_psrLagTbl_ADDR));//160
...
//	VoC_lw16d_set_inc(REG3,decorr_pswVects_ADDR_ADDR,1);//address of pswVects[] in reg3
   ...
   VoC_lw16i_short(INC0,1,DEFAULT);
	...
	VoC_loop_r(1,REG5);
		...
		VoC_sub32_dr(ACC0,(0+TABLE_ERR_psrLevelMaxIncrease_ADDR ),ACC1);//0
decorr01:
    	VoC_shr32_ri(ACC0,1,DEFAULT);
...
// SHIRT_R
		...
		VoC_bngt16_rd(decorr03,REG0, (0+TABLE_ERR_psrR0RepeatThreshold_ADDR)); //CONST_15_ADDR
...
decorr04:
		...
		VoC_lw32_d(REG67,CONST_0x00008000_ADDR);
		VoC_movreg32(ACC0,REG67,DEFAULT);
		VoC_lw16inc_p(REG4,REG1,IN_PARALLEL);
...
//  inc1=1;
	...
	VoC_jal(CII_NORM_L_ACC0);
	VoC_pop16(RA,DEFAULT);
    VoC_sub16_dr(REG2,(0+TABLE_ERR_psrLevelMaxIncrease_ADDR ),REG1);//0
...
void CII_resetDec(void)
{
	...
	VoC_lw16i_set_inc(REG2,GLOBAL_ERR_pswLastGood_ADDR,2);
    VoC_lw32z(ACC0,DEFAULT);
    VoC_loop_i(1,3)
    VoC_loop_i(0,178)
    VoC_sw32inc_p(ACC0,REG2,DEFAULT);
    VoC_endloop0
    VoC_lw16i_set_inc(REG0,hr_reset_value_ADDR,2);
    VoC_endloop1
...
exit_on_warnings=ON;
     ...
     VoC_lw16i_short(REG3,11,IN_PARALLEL);
     VoC_lw32z(ACC0,DEFAULT);
     VoC_sw16_d(REG4,GLOBAL_DE_swDecoMode_ADDR);
     VoC_sw16_d(REG2,GLOBAL_ERR_swState_ADDR);
     VoC_sw16_d(REG3,GLOBAL_DTX_DE_swRxDTXState_ADDR);
...
void CII_resetHREnc(void)
{
    ...
    VoC_lw16i_set_inc(REG2,GLOBAL_EN_swOldR0_ADDR,2);
    VoC_lw32z(ACC0,DEFAULT);
    VoC_loop_i(1,4)
    VoC_loop_i(0,139)
    VoC_sw32inc_p(ACC0,REG2,DEFAULT);
    VoC_endloop0
    VoC_sw16_d(ACC0_LO,GLOBAL_DTX_swVadFrmCnt_ADDR);
    VoC_sw16_d(ACC0_LO,GLOBAL_DTX_siUpdPointer_ADDR);
    VoC_endloop1

	VoC_lw16i_short(REG2,1,DEFAULT);
    VoC_lw16i_short(REG4,7,IN_PARALLEL);
    VoC_lw16i(ACC0_LO,24576);
    VoC_lw16i_short(REG5,21,DEFAULT);
    VoC_lw16i_short(REG6,-1,IN_PARALLEL);
    VoC_lw16i(ACC0_HI,21875);


	VoC_sw16_d(REG2, GLOBAL_EN_swPtch_ADDR);
 	VoC_sw16_d(ACC0_LO, HR_STATIC_VAD_pswRvad_ADDR);
	VoC_sw16_d(REG4, HR_STATIC_VAD_swNormRvad_ADDR);
	VoC_sw16_d(REG5, HR_STATIC_VAD_swE_thvad_ADDR);

     VoC_lw16i(ACC0_LO,50);

	VoC_sw16_d(ACC0_HI, HR_STATIC_VAD_swM_thvad_ADDR);
	VoC_sw16_d(REG6, HR_STATIC_VAD_swHangCount_ADDR);
	VoC_sw16_d(REG5, HR_STATIC_VAD_swOldLag_ADDR);
 	VoC_sw16_d(ACC0_LO, GLOBAL_DTX_swNElapsed_ADDR);
...
//	VoC_add16_rr(REG5, REG3, REG1,DEFAULT);
	...
	//swShift in REG1
      ...
      VoC_sub16_dr(REG1,(5+TABLE_ERR_psrLevelMaxIncrease_ADDR), REG1);//3
	VoC_loop_i_short(9,DEFAULT);
	//L_temp=0
	VoC_lw32z (ACC0,IN_PARALLEL);
	VoC_startloop0
		...
		VoC_bne16_rd(ENERGY_COMPUTATION_First,REG4,  	(392+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR)  );//CONST_32_ADDR		//L_temp = L_mac(L_temp, pswSacf[i], pswRvad[i]);
...
/****************************************************/
	...
	VoC_sub16_dr (REG5,(12+TABLE_ERR_psrLevelMaxIncrease_ADDR),REG1);				//10 scale
...
exit_on_warnings=ON;
		...
		VoC_add16_rd(REG4,REG4,(4+   TABLE_ERR_psrLevelMaxIncrease_ADDR));//2
		VoC_sw32inc_p(RL7,REG2,DEFAULT);
...
**********************************************/
	...
	VoC_endloop0
 	...
 	VoC_loop_i(1, 8);
		...
		VoC_sub16_dr(REG7,(0+TABLE_ERR_psrLevelMaxIncrease_ADDR ),REG5);//0
...
schur_recursion_End1:
		VoC_movreg32(REG23,ACC1,DEFAULT);
		VoC_sub16_dr(REG2, (10+TABLE_ERR_psrLevelMaxIncrease_ADDR), REG2);//8
...
/*********************/
		...
		VoC_bngtz16_r(SCHUR_RECURSION_210, REG5);		//  if (pswPp[1] > 0)
	        VoC_sub16_dr(REG2, (0+TABLE_ERR_psrLevelMaxIncrease_ADDR ), REG2);     //0    	//  pswVpar[n] = negate(pswVpar[n]);
SCHUR_RECURSION_210:
		...
		VoC_be16_rd(schur_recursion_End, REG2, (9+TABLE_ERR_psrLevelMaxIncrease_ADDR));  //if (n == 7)     return;

	    VoC_sub16_dr(REG6,(9+TABLE_ERR_psrLevelMaxIncrease_ADDR),REG2);//7
		...
		VoC_add16_rd(REG2,REG1,(4+   TABLE_ERR_psrLevelMaxIncrease_ADDR));	//2			// pswPp[1 + m] in REG2
 ...
 ***********************************************************/
    ...
    VoC_sub16_dr(REG3, (5+TABLE_ERR_psrLevelMaxIncrease_ADDR), REG1);//3
    //REG5   pswSav0[0]
	VoC_shr32_rr(REG45, REG3, DEFAULT);
	//L_sump in ACC0
    VoC_lw32z(ACC0,IN_PARALLEL);
...
/*********************/
		...
		VoC_shr32_ri (REG67, -1, DEFAULT);
 		VoC_bnltz32_r(SPECTRAL_COMPARISON106, ACC1);/*if (ACC1 < 0L)*/
 			VoC_sub32_dr(REG67,(0+TABLE_ERR_psrLevelMaxIncrease_ADDR ),REG67);//0
...
/*** Re-normalization and final computation of L_dm ***/
	...
	VoC_shr32_rr (REG67, REG4, DEFAULT);/* reg23 = L_shr (reg23, reg4);*/
 	...
 	VoC_bnltz32_r(SPECTRAL_COMPARISON113,REG45)
 		VoC_sub32_dr(REG45,(0+TABLE_ERR_psrLevelMaxIncrease_ADDR ),REG45);//0L_temp in REG45
...
TONE_DETECTION_101:						//}
   ...
   VoC_lw16i(REG7, 0x7FFF);				//swPredErr = 0x7fff;   ;swPredErr in REG7
   ...
   VoC_startloop0						//{
	    ...
	    VoC_NOP();
        VoC_sub16_dr(REG5, CONST_0x00007FFF_ADDR, REG5);		//swPredErr = mult(swPredErr, swTemp);
		VoC_multf16_rr(REG7, REG7, REG5, DEFAULT);
   VoC_endloop0							//}

   VoC_lw16i_short(REG3, 1,DEFAULT);			//*pswTone = 1;
   VoC_lw16i(REG0,1464);
   VoC_bgt16_rr(TONE_DETECTION_120, REG0, REG7);
TONE_DETECTION_End:
	   VoC_lw16i_short(REG3, 0,DEFAULT);
TONE_DETECTION_120:
...
****************************************************************/
	...
	// REG3 for swTone
 	VoC_be16_rd(THRESHOLD_ADAPTATION107,REG3,CONST_1_ADDR);
		...
		VoC_bgt16_rd(THRESHOLD_ADAPTATION108, REG5,(10+TABLE_ERR_psrLevelMaxIncrease_ADDR));    	//8 if (swAdaptCount <= 8) return;
...
THRESHOLD_ADAPTATION108:
	...
	//pswE_thvad in REG7
	VoC_bnlt16_rd(THRESHOLD_ADAPTATION109,REG6,CONST_0x00004000L_ADDR)
		VoC_shr16_ri(REG6,-1,DEFAULT);
...
//	VoC_multf32_rd (REG45, REG0, CONST_20889_ADDR);
	...
	VoC_bngt32_rd(THRESHOLD_ADAPTATION110,REG45,CONST_0x00007FFF_ADDR)	//if (L_temp > 0x7fffL)
...
THRESHOLD_ADAPTATION110:
	...
	VoC_bngt32_rr(THRESHOLD_ADAPTATION117,REG01,REG67);
		...
		VoC_bngt32_rd(THRESHOLD_ADAPTATION113,REG23, CONST_0x00007FFF_ADDR)//if
...
VAD_HANDOVER101:
   	VoC_pop16(RA, DEFAULT);
	VoC_blt16_rd(VAD_HANDOVER103,REG6,(5+TABLE_ERR_psrLevelMaxIncrease_ADDR))//3
	VoC_lw16i_short(REG7,5,DEFAULT);
...
CII_aFlatRcDp202:
		...
		VoC_add32_rd(REG67,REG67,CONST_0x00008000_ADDR);	// REG7 for swRcSq
		// VoC_pop16(REG6,DEFAULT);
		...
		VoC_sub16_dr(REG5, (11+TABLE_ERR_psrLevelMaxIncrease_ADDR),REG6);//9
...
CII_aFlatRcDp301:
		...
		VoC_shr16_ri(REG6,-1,DEFAULT);
		 VoC_lw16i_short(INC0,-2,IN_PARALLEL);
		// REG5: 14-j*2
		VoC_sub16_dr(REG5, 	(14+TABLE_ERR_psrLevelMaxIncrease_ADDR), REG6);  //14
		...
		VoC_add16_rd(REG5,REG5,(5+TABLE_ERR_psrLevelMaxIncrease_ADDR));//3
		...
		VoC_bne16_rd(CII_aFlatRcDp105,REG5,CONST_1_ADDR)
			VoC_add16_rd(REG0,REG6,(4+   TABLE_ERR_psrLevelMaxIncrease_ADDR));//2
...
rcToADp100:
       ...
       VoC_lw16i(REG4, 0x0800);
		// REG0 &pswRc[i]
	//	VoC_multf32inc_pd(ACC1,REG0,CONST_0x0800_ADDR);
       VoC_multf32inc_rp(ACC1,REG4 ,REG0,DEFAULT);
	...
	VoC_loop_i_short(10,DEFAULT)
	VoC_startloop0
	//	VoC_add32inc_pd(REG45,REG2,CONST_0x00008000_ADDR);
        VoC_add32inc_rp(REG45,ACC1,REG2,DEFAULT);
		VoC_be16_rd(isSwLimit2,REG5,CONST_0x00008000_ADDR);
		VoC_bne16_rd(isSwLimit1,REG5,CONST_0x00007FFF_ADDR);
isSwLimit2:
			VoC_lw16i_short(REG7,1,DEFAULT);
isSwLimit1:
...
exit_on_warnings=ON;
	...
	VoC_loop_i(1,9);
		...
		VoC_be16_rd(isSwLimit_opt1a,REG5,CONST_0x00008000_ADDR);
		VoC_bne16_rd(isSwLimit_opt1b,REG5,CONST_0x00007FFF_ADDR);
isSwLimit_opt1a:
		VoC_lw16i_short(REG7,1,DEFAULT);
isSwLimit_opt1b:
		...
		// if (pswRc[i] == SW_MIN)
		VoC_bne16_rd(aToRc_opt0,REG0,CONST_0x00008000_ADDR);
...
aToRc_opt0:
   ...
   //    VoC_sub32_dr(ACC0,CONST_0_ADDR,ACC0);
			// reg1 -swNormShift
       	   ...
       	   VoC_add16_rd(REG6,REG1, 	(24+  TABLE_SP_ROM_pppsrGsp0_ADDR));// -2
...
aToRc_opt1:
		...
		// if not equal limits, do nothing
		VoC_be16_rd(isSwLimit_opt2a,REG5,CONST_0x00008000_ADDR);
		VoC_bne16_rd(isSwLimit_opt2b,REG5,CONST_0x00007FFF_ADDR);
isSwLimit_opt2a:
		VoC_lw16i_short(REG7,1,DEFAULT);
isSwLimit_opt2b:
		...
		VoC_startloop0
			...
			VoC_add32_rd(REG01,ACC0,CONST_0x00008000_ADDR);

			// CII_isSwLimit inline...
			// if not equal limits, do nothing
			VoC_be16_rd(isSwLimit_opt3a,REG1,CONST_0x00008000_ADDR);
			VoC_bne16_rd(isSwLimit_opt3b,REG1,CONST_0x00007FFF_ADDR);
isSwLimit_opt3a:
			VoC_lw16i_short(REG7,1,DEFAULT);
isSwLimit_opt3b:
			// &pswA[j+1] & swDiv
...
exit_on_warnings=ON;
	...
	// CII_isSwLimit inline...
	// if not equal limits, do nothing
	VoC_be16_rd(isSwLimit_opt4a,REG1,CONST_0x00008000_ADDR);
	VoC_bne16_rd(isSwLimit_opt4b,REG1,CONST_0x00007FFF_ADDR);
isSwLimit_opt4a:
	VoC_lw16i_short(REG7,1,DEFAULT);
isSwLimit_opt4b:
...
Decoder_101:
	...
	VoC_add16_rd(REG0,REG0,(10+TABLE_ERR_psrLevelMaxIncrease_ADDR));//8
...
PARA_CONC_opt_101:
		...
		VoC_bnlt16_rd(PARA_CONC_opt_100,REG3,	(14+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR))//30
			// swMutePermit = 1
//			VoC_lw16i_short(REG7,1,DEFAULT);
...
exit_on_warnings = ON;
	...
	// if (pswErrorFlag[0] <= 0)
		...
		VoC_blt16_rd(PARA_CONC2_opt_104,REG1,(8+TABLE_ERR_psrLevelMaxIncrease_ADDR))//6
...
PARA_CONC2_opt_else:
	...
	// if (pswErrorFlag[0] > 0)
		...
		VoC_inc_p(REG1,DEFAULT);
		// if (sub(swState, 6) > 0)
		VoC_bngt16_rd(PARA_CONC2_opt_end,REG1,(8+TABLE_ERR_psrLevelMaxIncrease_ADDR))//6
        // swState = 6
...
PARA_CONC3_opt_100:
	...
	// (swState != 0)
		...
		VoC_blt16_rd(PARA_CONC3_opt_101,REG1,(5+TABLE_ERR_psrLevelMaxIncrease_ADDR))//3
		VoC_bgt16_rd(PARA_CONC3_opt_101,REG1,CONST_5_ADDR)
			...
			VoC_sub16_pd(REG4,REG2,(4+   TABLE_ERR_psrLevelMaxIncrease_ADDR));//2
			VoC_bltz16_r(PARA_CONC3_opt_101,REG4)
	//		VoC_lw16i_short(REG4,0,DEFAULT);

		VoC_blt16_rd(PARA_CONC3_opt_103,REG1,(8+TABLE_ERR_psrLevelMaxIncrease_ADDR)) //6
PARA_CONC3_opt_101:
			// pswLastGood[0] = 0
			VoC_lw16i_short(REG4,0,DEFAULT);
PARA_CONC3_opt_103:
...
level_calc_opt0:
	...
	VoC_bez32_r(level_calc_opt1,ACC0);
		VoC_add16_rd(REG7,REG7,(4+   TABLE_ERR_psrLevelMaxIncrease_ADDR));//2
level_calc_opt1:
...
//	VoC_bgt16_rd(level_calc_opt2,REG6,CONST_neg1_ADDR);
	VoC_bgt16_rr(level_calc_opt2,REG6,REG1);
...
level_calc_opt4:
	...
	VoC_add32_rd(REG67,REG67,CONST_0x00008000_ADDR);
    VoC_bgt16_rr(level_calc_opt5,REG7,REG0);
...
void CII_lookupVq_opt(void)
{
	...
	VoC_loop_i(0,3)
		...
		// if (siSeg == 2)
		// mult_factor = 4
		VoC_bne32_rd(LOOKUP_opt_100,RL7, (218+TABLE_SP_ROM_pppsrVcdCodeVec_ADDR )  )      //CONST_0x2L_ADDR
...
exit_on_warnings=ON;
		...
		VoC_be32_rd(LOOKUP_opt_103,RL7, (218+TABLE_SP_ROM_pppsrVcdCodeVec_ADDR ) )     //  CONST_0x2L_ADDR
...
LOOKUP_opt_102:
		...
		VoC_be32_rd(LOOKUP_opt_END1,RL7,(218+TABLE_SP_ROM_pppsrVcdCodeVec_ADDR )  )    //  CONST_0x2L_ADDR
...
void  CII_extra_HR_bfi_filter(void)
{
	...
	VoC_bnlt16_rd(CII_extra_HR_bfi_filter_100,REG3, 	(35+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR)  )//  CONST_25_ADDR
	VoC_sw16_d(REG2,GLOBAL_swOldR0_ADDR);

	VoC_bne16_rd(CII_extra_HR_bfi_filter_exit,REG1, 	(348+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR)   )  // 33_ADDR branch if no speech
    VoC_bez16_d(CII_extra_HR_bfi_filter_exit,GLOBAL_swPreBfi_ADDR)
	VoC_lw16_d(REG4,(STATIC_DE_pswSpeechPara_ADDR+5));
    VoC_blt16_rd(CII_extra_HR_bfi_filter_exit,REG4,(5+TABLE_ERR_psrLevelMaxIncrease_ADDR))//3
...
DEC_FRAME_CLASS_100:
	...
	// if (swSID == 2)
	VoC_bne16_rd(DEC_FRAME_CLASS_101,REG4,(4+   TABLE_ERR_psrLevelMaxIncrease_ADDR));//2
	// if (swBfiDtx_L == 0)
...
DEC_FRAME_CLASS_101:
   ...
   VoC_jal(CII_extra_HR_bfi_filter);
	...
	VoC_bne16_rd(DEC_UPDATE_STATE_100,REG2,GLOBAL_DE_swDecoMode_ADDR)
		...
		VoC_bgt16_rd(DEC_UPDATE_STATE_END,REG1, 	(45+TABLE_SP_ROM_pppsrUvCodeVec_ADDR))//22
...
DEC_UPDATE_STATE_100:
	...
	// else
		...
		VoC_lw16i_short(REG3,1,DEFAULT);
		// jump if (swFrameType == GOODSPEECH)
		VoC_be16_rd(DEC_UPDATE_STATE_END,REG1, 	(348+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR)   )//33_ADDR
		// else swDecoMode = CNIBFI
		VoC_lw16i_short(REG3,4,DEFAULT);
		// jump if (swFrameType == UNUSABLE)
		VoC_be16_rd(DEC_UPDATE_STATE_END,REG1,(178+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR))//44
		// else (swFrameType == VALIDSID) || (swFrameType == INVALIDSID)
		// swDecoMode = CNICONT;
...
DEC_NOT_SPEECH_MODE100:
	...
	// if (swDecoMode != SPEECH)
		...
		VoC_be16_rd(DEC_NO_CNIFIRSTSID_100_10,REG3,(4+   TABLE_ERR_psrLevelMaxIncrease_ADDR))//2
...
DEC_NO_CNIFIRSTSID_100_10:
			...
			VoC_bne16_rd(DEC_NO_CNIFIRSTSID_101,REG1, (7+TABLE_ERR_psrR0RepeatThreshold_ADDR))  //CONST_11_ADDR
...
DEC_NO_CNIFIRSTSID_100:
		...
		VoC_bne16_rd(DEC_NO_CNICONT_200,REG3,(5+TABLE_ERR_psrLevelMaxIncrease_ADDR))//3

			VoC_sw16_d(REG4,GLOBAL_DE_swDtxBfiCnt_ADDR);
			VoC_sw16_d(REG4,GLOBAL_DE_swDtxMuting_ADDR);

			// if (swFrameType == VALIDSID)
			VoC_bne16_rd(DEC_NO_CNIFIRSTSID_201,REG1,   (7+TABLE_ERR_psrR0RepeatThreshold_ADDR)  ) //CONST_11_ADDR
...
DEC_NO_CNICONT_203:
		...
		VoC_be16_rd(DEC_NO_CNICONT_204,REG3,(6+TABLE_ERR_psrLevelMaxIncrease_ADDR))//4
...
DEC_NO_CNIFIRSTSID_302:
			...
			VoC_blt16_rd(DEC_NO_CNIFIRSTSID_304,REG0, 	(35+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR) )//  25
				// swDtxMuting = 1
...
DEC_NO_CNICONT_END:
		...
		// if (swDecoMode == CNIFIRSTSID)
		VoC_bne16_rd(DEC_NO_FIRST_SID,REG6,(4+   TABLE_ERR_psrLevelMaxIncrease_ADDR))//2
...
//	VoC_push16(RA,DEFAULT);
	...
	// if (swDecoMode == CNIFIRSTSID)
	VoC_bne16_rd(RxInterpR0Lpc_opt_100,REG6,(4+   TABLE_ERR_psrLevelMaxIncrease_ADDR))//2
...
RxInterpR0Lpc_opt_100:
	...
	VoC_bne16_rd(RxInterpR0Lpc_opt_else,REG6,(5+TABLE_ERR_psrLevelMaxIncrease_ADDR))//3
	VoC_bne16_rd(RxInterpR0Lpc_opt_else,REG7, (7+TABLE_ERR_psrR0RepeatThreshold_ADDR)   )  //CONST_11_ADDR
...
DEC_PROCESS_200_END:
		...
		VoC_bez16_d(DEC_PROCESS_300,GLOBAL_DE_SP_swVoicingMode_ADDR)
			...
			VoC_sub16_rd(REG0,REG0,(10+TABLE_ERR_psrLevelMaxIncrease_ADDR));//8
			// swLag = add(swLag, swLastLag);
...
//		BEGIN OF CII_rs_rr_opt
	...
	VoC_sub16_dr(REG5,(5+TABLE_ERR_psrLevelMaxIncrease_ADDR),REG3);     //3
	//  if (-swEnergy > 0)
...
DEC_PROCESS_500:
		...
		VoC_mult16_rd(REG5,REG4,	(28+  TABLE_SP_ROM_psrLagTbl_ADDR));//160
		VoC_add16_rr(REG1,REG1,REG3,DEFAULT);
		VoC_add16_rr(REG1,REG1,REG5,DEFAULT);
...
pitchPreFilt_opt_00:
		...
		// else
			...
			VoC_shr16_ri(REG7,-1,DEFAULT);
       		VoC_sub16_dr(REG5,(5+TABLE_ERR_psrLevelMaxIncrease_ADDR),REG7);  //3
...
pitchPreFilt_opt_03:
			...
			VoC_multf32_rd(REG67,REG0, 	(0+  TABLE_SP_ROM_psrNewCont_ADDR)  );//CONST_0x2666_ADDR
			// swLag
...
//			BEGIN OF CII_signal_conceal_sub_opt
{
   ...
   VoC_lw16i_short(FORMAT16,(15-16),DEFAULT);
	...
	// if (sub(swIndex, 15) > 0) swIndex = 15;
 	VoC_bngt16_rd(signal_conceal_opt00,REG0, (0+TABLE_ERR_psrR0RepeatThreshold_ADDR) );//
...
signal_conceal_opt_go:
	...
	//	VoC_bngt16_rr(signal_conceal_opt_exit2,REG2,REG0);
        VoC_bgt16_rr(signal_conceal_opt_exit2,REG2,REG0);
	  //  VoC_bngt16_rd(signal_conceal_opt_exit2,REG2,CONST_neg45_ADDR);
		...
		VoC_bngt16_rd(signal_conceal_opt04,REG2, (0+TABLE_ERR_psrR0RepeatThreshold_ADDR)  );//CONST_15_ADDR
			// swMute = 15;
...
//	VoC_push16(RA,DEFAULT);
	...
	// snsOrigEnergy
	VoC_add32_rd(RL7,ACC0,CONST_0x00008000_ADDR);
...
//	VoC_pop16(RA,DEFAULT);
	...
	VoC_sw16_d(REG6,GLOBAL_DE_gswPostFiltAgcGain_ADDR);
		...
		VoC_add16_rd(REG1,REG0,(32+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR));//40
...
void CII_agcGain_opt(void)
{
	...
	VoC_bez32_r(agcGain_opt_END,ACC0);
		...
		VoC_blt16_rd(agcGain_opt_END,REG0, 	(24+  TABLE_SP_ROM_pppsrGsp0_ADDR)); //-2
...
void CII_compute_s2_s3_new(void)
{
	...
	VoC_bngt16_rd(COMPUTES2S3_S2LIMIT,REG4,(101+  TABLE_SP_ROM_psrLagTbl_ADDR))//256
		// si2 + 1 = (1 << L_BITS) - 1 + 1;
		VoC_lw16i(REG4,256);
COMPUTES2S3_S2LIMIT:
...
void CII_r0BasedEnergyShft_new(void)
{
	...
	VoC_lw16i_short(REG5,0,DEFAULT);
	VoC_bngt16_rd(R0Based_END_CASE, REG4,	(366+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR)  )//18
	VoC_lw16i_short(REG5,2,DEFAULT);
	VoC_bngt16_rd(R0Based_END_CASE, REG4, (36+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR)  )// 21
	VoC_lw16i_short(REG5,3,DEFAULT);
	VoC_bngt16_rd(R0Based_END_CASE, REG4, (30+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR))// 23
	VoC_lw16i_short(REG5,4,DEFAULT);
	VoC_bngt16_rd(R0Based_END_CASE, REG4, (39+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR) )//24
	VoC_lw16i_short(REG5,5,DEFAULT);
	VoC_bngt16_rd(R0Based_END_CASE, REG4, (100+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR))//26
	VoC_lw16i_short(REG5,6,DEFAULT);
	VoC_bngt16_rd(R0Based_END_CASE, REG4, (121+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR)   )//28
	VoC_lw16i_short(REG5,7,DEFAULT);
	VoC_bngt16_rd(R0Based_END_CASE, REG4, (171+TABLE_SP_ROM_pppsrUvCodeVec_ADDR)) // 29
	VoC_lw16i_short(REG5,8,DEFAULT);
R0Based_END_CASE:
...
FINDPEAK100:
	...
	VoC_bnlt16_rd(FINDPEAK101,REG5, (103+TABLE_SP_ROM_pppsrUvCodeVec_ADDR));//CONST_20_ADDR
...
FINDPEAK101:
	...
	VoC_sub16_rd(REG6,REG5,(366+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR));//18
...
//	VoC_be16_rd(FINDPEAKRETURN,REG3,CONST_neg1_ADDR);
	VoC_be16_rr(FINDPEAKRETURN,REG3,REG7);
		VoC_add16_rr(REG3,REG3,REG5,DEFAULT);

		VoC_be16_rr(FINDPEAKRETURN,REG3,REG4);
		VoC_be16_rr(FINDPEAKRETURN,REG3,REG5);

			VoC_multf32_rd(REG67,REG3,(8+TABLE_ERR_psrLevelMaxIncrease_ADDR));//6
			 VoC_NOP();
			VoC_shr16_ri(REG6,1,DEFAULT);
...
void CII_CGInterp_new(void)
{
	...
	// swLoIntLag update
	VoC_add16_rd(REG5,REG5,(4+   TABLE_ERR_psrLevelMaxIncrease_ADDR));//2
...
//		VoC_multf32inc_pd(REG67,REG0,CONST_0x1555_ADDR);
       VoC_lw16i(REG6,0x1555);

     	VoC_multf32inc_rp(REG67,REG6,REG0,DEFAULT);
...
void CII_CGInterpValid_new(void)
{
	...
	VoC_sub16_rd(REG6,REG7,(8+TABLE_ERR_psrLevelMaxIncrease_ADDR));	//6
...
CGINTERPVALID100:
	...
	VoC_add16_rd(REG6,REG7,(8+TABLE_ERR_psrLevelMaxIncrease_ADDR));//6
	VoC_push16(REG2,DEFAULT);
...
void CII_pitchLags_new(void)
{
	...
	VoC_multf32_rd(REG45,REG4,(8+TABLE_ERR_psrLevelMaxIncrease_ADDR));//6
	// pswLBuf
...
PITCHLAGS300:
	...
	VoC_blt16_rd(PITCHLAGS200_go, REG7,  (36+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR)  );  //CONST_21_ADDR
...
PITCHLAGS400:
		...
		VoC_add16_rr(REG5,REG5,REG2,DEFAULT);
        VoC_lw16_d(REG6,LOCAL_PITCHLAGS_pswLPeaks_ADDR);
       	VoC_multf32_rp(REG67,REG6,REG5,DEFAULT);
...
PITCHLAGSSTART: //while start
	VoC_lw16i(REG6,142);
    VoC_bngt16_rr(PITCHLAGSYIYI_go,REG7,REG6)
...
PITCHLAGSYIYI: //WHILEEND
	...
	// -si2 in REG3,
	VoC_movreg16(REG3,REG0,DEFAULT);
    VoC_lw16i(REG0,0x3333);
	// L_1 = L_mult(sw1, PW_FRAC);
	VoC_multf32_rr(ACC0,REG2,REG0,DEFAULT);//L_1 in ACC0
...
void CII_openLoopLagSearch_new(void)
{
	...
	VoC_loop_i(1,4)
		...
		VoC_sub16_rd(REG0,REG0,(4+   TABLE_ERR_psrLevelMaxIncrease_ADDR));//2
...
OPENLOOP302:
		...
		VoC_bngt16_rd(OPENLOOP302,REG7,(5+TABLE_ERR_psrLevelMaxIncrease_ADDR));//3
...
exit_on_warnings=ON;
			...
			VoC_bngtz32_r(GETCCTHRESHOLD101,REG67);
				...
				VoC_bngt16_rd(GETCCTHRESHOLD101,REG2, (348+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR)  );//33_ADDR
...
GETCCTHRESHOLD101:
			...
			// siShiftCnt = negate(add(siShiftCnt, 1)) << 10;
   	        VoC_sub16_dr(REG1,CONST_0x0000FFFF_ADDR,REG1);
   		...
   		//	VoC_multf32_rd(ACC0,REG7,CONST_neg0x2b2a_ADDR);
         	...
         	VoC_lw16i(REG0,0x7fc5);
		//	VoC_mac32_rd(REG2,CONST_0x7fc5_ADDR);
        	VoC_mac32_rr(REG2,REG0,DEFAULT);
			VoC_lw16i_short(REG0,0,DEFAULT);
			VoC_add32_rr(ACC0,ACC0,RL7,DEFAULT);
			...
			VoC_add32_rd(REG01, ACC0, CONST_0x00008000_ADDR);
            	VoC_lw16i(REG0,0x6000);

			// sw1 in REG1
			// L_1 = L_mult(sw1, swPGainScale);
    		VoC_multf32_rr(REG01,REG1,REG0,DEFAULT);
			...
			// swTemp2 in REG1
			// L_Input in REG01
			VoC_multf32_rd(REG01,REG1,(392+TABLE_SP_ROM_pppsrUvCodeVec_ADDR));//CONST_32_ADDR
...
OPENLOOP305:
		...
		// update pointers
		VoC_add16_rd(REG0,REG0,	(1+  TABLE_SP_ROM_psrLagTbl_ADDR));//128
		VoC_sub16_rd(REG1,REG1,(32+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR));//40
...
OPENLOOP404:
		...
		VoC_bngt16_rd(OPENLOOPWHILESTART,REG5,(4+   TABLE_ERR_psrLevelMaxIncrease_ADDR))//2
		VoC_lw16i_short(REG5,2,DEFAULT);
...
exit_on_warnings=ON;
			...
			VoC_add32_rd(ACC1,ACC1,CONST_0x00008000_ADDR);
...
exit_on_warnings=ON;
			...
			// test if 0 loops
			VoC_bnlt16_rd(OPENLOOP412,REG4,(5+TABLE_ERR_psrLevelMaxIncrease_ADDR))//3
...
OPENLOOP413:
				...
				// for (siFIndex = siAnchorIndex + 1; siFIndex < N_SUB; siFIndex++)
				VoC_blt16_rd(OPENLOOP413,REG4,(5+TABLE_ERR_psrLevelMaxIncrease_ADDR))//3
...
OPENLOOP418:
            ...
            VoC_lw16i_short(INC2,-1,DEFAULT);
				...
				VoC_sub16_rd(REG0,REG0,	(1+  TABLE_SP_ROM_psrLagTbl_ADDR));//128
				VoC_add16_rd(REG1,REG1,(32+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR));//40
...
OPENLOOP2001:
				...
				VoC_bgtz16_r(OPENLOOP2003, REG4)
					VoC_sub16_dr(REG4,(0+TABLE_ERR_psrLevelMaxIncrease_ADDR ),REG4);//0
OPENLOOP2003:
...
OPENLOOP2005:
				...
				VoC_bgtz16_r(OPENLOOP2002ADD,REG3)
					VoC_sub16_dr(REG2,(0+TABLE_ERR_psrLevelMaxIncrease_ADDR ),REG2);//0
...
OPENLOOPCONTINUE:
		...
		VoC_add16_rd(REG2,REG2,	(1+  TABLE_SP_ROM_psrLagTbl_ADDR));//128
		VoC_sub16_rd(REG3,REG3,(32+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR));//40
		...
		// if (siAnchorIndex == 4)
		VoC_be16_rd(OPENLOOPPITCHLAGLOOPEND,REG7,(6+TABLE_ERR_psrLevelMaxIncrease_ADDR));//4
		// else
...
OPENLOOPPITCHLAGLOOPEND:
	...
	VoC_bez16_r(OPENLOOPUNVOICED,REG7)
		...
		VoC_startloop0
			...
			VoC_multf32_rd(ACC0,REG5,  	(619+TABLE_SP_ROM_psrQuant2_ADDR)  );  // CONST_neg0x6676_ADDR
			  ...
			  ppswPFDenomAs[siSfrm][i] = mac_r(L_Temp2, pswPrevFrmPFDenom[i],
				...
				VoC_startloop0
					...
					VoC_bez16_r(getSfrmLpcTx0, REG7);
						VoC_add32_rd(ACC1,ACC0,CONST_0x00008000_ADDR);
	getSfrmLpcTx0:
					VoC_multf32inc_rp(ACC0,REG4,REG0,DEFAULT);
					VoC_sw16inc_p(ACC1_HI,REG1,DEFAULT);
				VoC_endloop0
...
exit_on_warnings=ON;
   ...
   VoC_be16_rd(GETSFRMLPCTX_LOOP_END, REG6, (5+TABLE_ERR_psrLevelMaxIncrease_ADDR)); //3
...
//     swNewR0
	...
	VoC_add16_rd(REG0,REG0,(8+TABLE_ERR_psrLevelMaxIncrease_ADDR));//6
	VoC_NOP();
	// save the result
...
//	 VoC_push16(RA,DEFAULT);
	 ...
	 VoC_startloop0;
		 VoC_lw32inc_p(REG45,REG0,DEFAULT);
		 VoC_bngt16_rd(compResidEnergy0,REG4,(0+TABLE_ERR_psrLevelMaxIncrease_ADDR ));//0
...
compResidEnergy0:
	...
	VoC_endloop0;
	 ...
	 VoC_bnlt16_rd(compResidEnergy3,REG6,CONST_1_ADDR);
		...
		VoC_lw16i_short(REG7,1,DEFAULT);
		VoC_blt16_rd(compResidEnergy3,REG6,CONST_0x0000FFFF_ADDR);

		// elsif REG6 == -1 then REG5 = 0x2000, REG7 = 1
		VoC_shr16_ri(REG5,-1,DEFAULT);
		VoC_be16_rd(compResidEnergy3,REG6,CONST_0x0000FFFF_ADDR);
...
LAP10_new:
    ...
    VoC_bnez16_r(LAP11,REG0);
		...
		VoC_lw16_sd(REG3,1,DEFAULT);
		VoC_mult16_rd(REG4,REG6,(12+TABLE_ERR_psrLevelMaxIncrease_ADDR));//10
...
LAP11:
    ...
    // reg0 &speech[i*S_LEN]
    VoC_lw16_sd(REG0,2,DEFAULT);
	VoC_mult16_rd(REG4,REG6,(32+  TABLE_SP_ROM_pppsrUvCodeVec_ADDR));//40
...
compResidEnergy790:
    ...
    VoC_be16_rd(LAP13_end,REG6,(6+TABLE_ERR_psrLevelMaxIncrease_ADDR));//4
    VoC_jump(LAP13);
LAP13_end:
	...
	VoC_add16_rr(REG0,REG0,REG1,DEFAULT);
	VoC_sub32_dr(RL6,(0+TABLE_ERR_psrLevelMaxIncrease_ADDR ),RL6);//0
	VoC_push16(REG0,DEFAULT);
