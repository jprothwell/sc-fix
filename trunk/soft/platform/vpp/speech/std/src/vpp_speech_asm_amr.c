...
DtxDec_L8:
	VoC_bez16_d(DtxDec_L5,STRUCT_DTX_DECSTATE_DTXGLOBALSTATE_ADDR);					//    (((sub(st->dtxGlobalState, DTX) == 0) ||
...
DtxDec_L3:   										//	 {
    ...
    VoC_lw16i_short(REG3,2,DEFAULT);
   	...
   	VoC_add16_rd(REG2,REG4,STRUCT_DTX_DECSTATE_SINCE_LAST_SID_ADDR); //   st->since_last_sid = add(st->since_last_sid, 1);
	
	VoC_bne16_rd(DtxDec_L7,REG3,STRUCT_DTX_DECSTATE_DTXGLOBALSTATE_ADDR);		//   if ((sub(st->dtxGlobalState, DTX_MUTE) == 0) &&
	VoC_be16_rd(DtxDec_L7,REG7,CONST_3_ADDR);		//       ((sub(frame_type, RX_SID_BAD) == 0) ||
	VoC_be16_rd(DtxDec_L7,REG7,CONST_4_ADDR);		//        (sub(frame_type, RX_SID_FIRST) ==  0) ||
 													//   {
	VoC_lw16i_short(REG4,2,DEFAULT);				//      newState = DTX_MUTE;   }
DtxDec_L7:
   
	VoC_be16_rd(DtxDec_L5,REG7,CONST_5_ADDR);		//   if((sub(frame_type, RX_SID_UPDATE) != 0) &&
	VoC_bngt16_rd(DtxDec_L5,REG2,(346+STATIC_CONST_table_gain_highrates_ADDR));	// 50     (sub(st->since_last_sid, DTX_MAX_EMPTY_THRESH) > 0))
...
DtxDec_L14:   
	...
	VoC_sw32_d(REG23,STRUCT_DTX_DECSTATE_DTXHANGOVERCOUNT_ADDR);
  	VoC_bez16_r(DtxDec_L19,REG4);		//if (sub(newState, SPEECH) != 0)
		VoC_lw16i_short(REG3,0,DEFAULT);			//    st->valid_data = 0;  
...
void CII_dtx_dec(void) 
{
	...
	VoC_add16_rd(REG0,REG0,(33+TABLE_MR515_ADDR) );//CONST_102_ADDR
...
DTX_DEC_L004:   						//         else
		...
		VoC_bngt16_rr(DTX_DEC_L006,REG6,REG1);	//         if (sub(st->lsf_hist_mean[i+j*M], 655) > 0)
			...
			VoC_shr16_ri(REG4,2,DEFAULT);		//            st->lsf_hist_mean[i+j*M] =  add(655, shr(sub(st->lsf_hist_mean[i+j*M], 655), 2));
			VoC_add16_rr(REG6,REG4,REG1,DEFAULT);	//         }
DTX_DEC_L006:   
			
			VoC_bngt16_rd(DTX_DEC_L007,REG6,CONST_1310_ADDR);	//         if (sub(st->lsf_hist_mean[i+j*M], 1310) > 0)
	
            VoC_lw16i(REG6,1310);   	//         {     st->lsf_hist_mean[i+j*M] = 1310;   }
DTX_DEC_L007:
...
DTX_DEC_L012:
	...
	VoC_lw16i(REG2,0x4000);	
 	VoC_blt16_rd(DTX_DEC_L013,REG1,(218+TABLE_DICO2_LSF_compressed_ADDR) );	//  CONST_0x0800_ADDR    if (sub(tmp_int_length, 2) >= 0)
...
DTX_DEC_L010:      		
	...
	VoC_multf16_rd(REG4,REG4,(0+STATIC_CONST_F_GAMMA1_ADDR));	// 29491    shr(mult(shl(dtx_log_en_adjust[mode],5),3277),5));
	VoC_multf16_rd(REG5,REG5,(1+STATIC_CONST_QUA_GAIN_PITCH_ADDR));	
	...
	VoC_bngt16_rd(DTX_DEC_L020,REG3, (163+TABLE_DICO1_LSF_3_compressed_ADDR)  );//CONST_0x400_ADDR if (sub(int_fac, 1024) > 0)
...
DTX_DEC_L020:
	...
	VoC_sub16_dr(REG7,STRUCT_DTX_DECSTATE_LOG_PG_MEAN_ADDR,REG5);//lsf_variab_factor = sub(st->log_pg_mean,2457);
	VoC_multf16_rd(REG7,REG7,(4+STATIC_CONST_PDOWN_ADDR)); // 0x2666  

    VoC_lw16i_set_inc(REG1,dtx_dec_LSF_INT_ADDR,-1);

	VoC_sub16_dr(REG7,(2+STATIC_CONST_INTERP_FACTOR_ADDR),REG7);		// 4096  lsf_variab_factor = sub(4096, mult(lsf_variab_factor, 9830)); 
	
	VoC_bngt16_rd(DTX_DEC_L021,REG7,(2+STATIC_CONST_INTERP_FACTOR_ADDR));	//if (sub(lsf_variab_factor, 4096) > 0)
			
	VoC_lw16i(REG7,4096);   	//{ lsf_variab_factor = 4096;}
DTX_DEC_L021:
...
A_refl_L0:    
       ...
       VoC_shr16_ri(REG7,-3,DEFAULT);		//   refl[i] = shl(aState[i], 3);   
      VoC_blt16_rd(A_refl_L1,REG6,(89+STATIC_CONST_table_gain_lowrates_ADDR));	//	4096
...
//	  VoC_sub16_dr(REG3,CONST_15_ADDR,REG1);		//   scale = sub(15, normShift);  
	  VoC_lw16i_short(REG3,15,DEFAULT);
...
/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
    ...
    VoC_shr16_ri(REG5,3,DEFAULT);		//log_pg = shr(sub(0,add(log_pg, shr(log_pg_m, 15-12))), 1); 
    ...
    VoC_multf16_rd(REG5,REG4,(1+STATIC_CONST_QUA_GAIN_PITCH_ADDR));	//3277//st->log_pg_mean = add(mult(29491,st->log_pg_mean), mult(3277, log_pg));                  
    ...
    VoC_add32_rd(ACC0,ACC0,CONST_0x00040000L_ADDR);   	//L_log_en_int = L_add(L_log_en_int, 4 * 65536L);                 
  

    VoC_shru32_ri(REG45,-16,DEFAULT);	
    VoC_shr32_ri(REG45,12,DEFAULT);  //L_log_en_int = L_sub(L_log_en_int, L_shl(L_deposit_l(log_pg), 4));
    ...
    VoC_shru32_ri(REG23,-16,DEFAULT);	
    VoC_shr32_ri(REG23,11,DEFAULT)   //L_log_en_int = L_add(L_log_en_int,L_shl(L_deposit_l(st->log_en_adjust), 5));  
...
DTX_DEC_L027: 
    ...
    VoC_sub16_rd(REG5,REG4,(50+STATIC_CONST_WIND_200_40_compressd_ADDR));	//256      st->log_en = sub(st->log_en, 256);                         
...
void CII_vad1(void)            
{
	...
	VoC_lw16i(REG7, 0x3fff);

	VoC_lw16_d(REG6, STRUCT_VADSTATE1_PITCH_ADDR);//REG6  ,st->pitch   REG7   st->complex_low
   	VoC_and16_rr(REG6, REG7,DEFAULT);
	VoC_and16_rd(REG7, STRUCT_VADSTATE1_COMPLEX_LOW_ADDR);
         
   VoC_bnlt32_rd(VAD_L2,  ACC0,CONST_0x53c00_ADDR);
...
//    CII_first_filter_stage
	...
	VoC_loop_i(1, 80)	
		...
		VoC_multf16_rd(REG5, REG6, CONST_21955_ADDR);	// mult(COEFF5_1, data0)
		VoC_shr16_ri(REG2, 2, DEFAULT);
		VoC_sub16_rr(REG5, REG2, REG5, DEFAULT);//temp0 in REG5
		
		VoC_multf16_rd(REG4,  REG5, CONST_21955_ADDR);	//mult(COEFF5_1, temp0) in REG5
 	
		VoC_multf16_rd(REG2,  REG7, CONST_6390_ADDR);	// mult(COEFF5_2, data1)
		VoC_shr16_ri(REG3, 2, DEFAULT);
		VoC_sub16_rr(REG2, REG3, REG2, DEFAULT);//temp3 in REG2
		VoC_multf16_rd(REG3,  REG2, CONST_6390_ADDR);	//mult(COEFF5_2, temp3) in REG3
...
FILTER5_LOOP_START:
	...
	VoC_loop_i(1, 2)
	    ...
	    VoC_lw16i(REG4,21955);  ////COEFF5_1 in  REG4
		...
		VoC_startloop0
            ...
            VoC_lw16i(REG4,6390);   	////COEFF5_2 in  REG4
	...
	VoC_blt16_rd(FILTER5_LOOP_START, REG2, CONST_160_ADDR)
   	 ...
   	 VoC_lw16i_short(INC1,4,DEFAULT);	
...
level_calculation_abs1:			  
		...
		VoC_sub16_rr(REG5, REG5, REG4,IN_PARALLEL);	 //sub(16, scale)
		VoC_startloop0			
...
// level -> REG1 inc=1
	...
	VoC_multf16_rd(REG7, REG7, CONST_3641_ADDR);//snr_sum in reg7
...
COMPLEX_ESTIMATE_110: 
	...
	VoC_add32_rd(REG23, ACC0, CONST_0x00008000_ADDR);// st->corr_hp_fast in reg3
//begin of CII_complex_vad
	

	VoC_blt16_rd(COMPLEX_ESTIMATE_130,REG3, CONST_13106_ADDR);
	VoC_bez16_r(COMPLEX_ESTIMATE_140,REG6);
COMPLEX_ESTIMATE_130: 

    VoC_lw16i(REG3, 13106);
...
COMPLEX_VAD_MM:
	...
	VoC_be16_rd(COMPLEX_VAD_END, REG1, CONST_0x00007FFF_ADDR);
	VoC_lw16i_short(REG4, 0, DEFAULT);
COMPLEX_VAD_END: 
...
UPDATE_CNTRL_JUMP_100:
	...
	VoC_be16_rd(UPDATE_CNTRL_JUMP_200_go, REG5, CONST_0x7c00_ADDR);	
 	VoC_be16_rd(UPDATE_CNTRL_JUMP_200_go, REG4, 	(17+STATIC_CONST_INTERP_FACTOR_ADDR));//CONST_0x6000_ADDR
...
UPDATE_CNTRL_JUMP_230:	
        VoC_push32(REG01, DEFAULT);
		VoC_lw16i(REG0,184);
		VoC_bngt16_rr(UPDATE_CNTRL_JUMP_240, REG0, REG4);
		VoC_movreg16(REG4,REG0,DEFAULT);  //	VoC_lw16i(REG4,184);
UPDATE_CNTRL_JUMP_240:
		VoC_bngt16_rr(UPDATE_CNTRL_JUMP_250, REG0, REG3);
		VoC_movreg16(REG3,REG0,DEFAULT); 	//VoC_lw16i(REG3,184);
...
UPDATE_CNTRL_JUMP_200:
   	VoC_lw32z(ACC0,DEFAULT);  // bckr_add in ACC0_HI
	...
	VoC_lw16i(REG5, 0x3fff);   //CONST_ALPHA5_ADDR 
	VoC_bez16_r(UPDATE_CNTRL_JUMP_280,REG6);    //REG6   st->vadreg & 0x4000
    VoC_lw16i(REG5, 0x0ccc);//CONST_ALPHA4_ADDR // alpha in reg5
...
// void CII_update_cntrl (void)  end
	...
	VoC_and16_rr(REG4, REG3,DEFAULT);
 	VoC_and16_rd(REG3, STRUCT_VADSTATE1_PITCH_ADDR);
	...
	VoC_movreg16(REG6,REG7,DEFAULT);  // alpha_up in reg6
   
    VoC_lw16i(REG7, 0x831);   // alpha_down in reg7
	      
	VoC_jump(NOISE_ESTIMATE_JZGL2);
...
NOISE_ESTIMATE_ZGL:
  ...
  // 	VoC_lw16i(REG6, 0x01eb);// CONST_ALPHA_UP2_ADDR
   	//VoC_lw16i(REG7, 0x74b);   //CONST_ALPHA_DOWN2_ADDR
    VoC_lw32_d(REG67, CONST_ALPHA_DOWN2_ALPHA_UP2_ADDR);

NOISE_ESTIMATE_JZGL2:
...
NOISE_ESTIMATE_JZGL: 
	...
	VoC_startloop0
		...
		VoC_multf32_rr(RL7, REG6, REG4, DEFAULT);
		
		VoC_bnltz16_r(NOISE_ESTIMATE_LX, REG4);//if
		VoC_add32_rr(REG45, REG23, ACC1,DEFAULT);
...
NOISE_ESTIMATE_LX: //else
		...
		VoC_add32_rr(REG45, REG23, RL7,DEFAULT);
		VoC_add32_rr(REG45, ACC0, REG45,DEFAULT);
		
		VoC_bngt16_rd(NOISE_ESTIMATE_JLX, REG5, (18+TABLE_DICO5_LSF_compressed_ADDR));//16000
        VoC_lw16i(REG5, 16000);
		VoC_NOP();
NOISE_ESTIMATE_JLX:
...
HANDOVER_F2:
	...
	VoC_bnlt16_rd(HANDOVER_F3,  REG3, (334+TABLE_DICO3_LSF_3_compressed_ADDR));//250
	VoC_lw16i(REG3, 250);
HANDOVER_F3:
...
COMPUTE_PARA:
	...
	VoC_loop_i(1,10)
		...
		VoC_sub16_rd(REG0,REG0,(57+STATIC_CONST_INTER_6_ADDR));//89
		VoC_lw32z(REG45,DEFAULT);																
		VoC_lw32z(REG67,DEFAULT);
...
NLT0:
	...
	VoC_sw16_d(REG7,STRUCT_DTX_ENCSTATE_LOG_EN_INDEX_ADDR);
   	exit_on_warnings = ON;
	VoC_sub16_rd(REG7,REG7,(299+STATIC_CONST_table_gain_MR475_ADDR));//2560
...
GET_PARA:
	...
	VoC_lw16_d(REG1,STRUCT_DTX_ENCSTATE_LSP_INDEX_ADDR+2);
	VoC_lw16_d(REG2,STRUCT_DTX_ENCSTATE_LOG_EN_INDEX_ADDR);
...
//   Copy(&st->old_speech[L_FRAME], &st->old_speech[0], L_TOTAL - L_FRAME);
   ...
   VoC_lw16i_set_inc(REG0,STATIC_CONST_bitno_MR475_ADDR,1);
   VoC_bez16_r(CII_sid_mode,REG7)
   VoC_lw16i_set_inc(REG0,STATIC_CONST_bitno_MR515_ADDR,1);
   VoC_be16_rd(CII_sid_mode,REG7,CONST_1_ADDR)
   VoC_lw16i_set_inc(REG0,STATIC_CONST_bitno_MR59_ADDR,1);
   VoC_be16_rd(CII_sid_mode,REG7,CONST_2_ADDR)
   VoC_lw16i_set_inc(REG0,STATIC_CONST_bitno_MR67_ADDR,1);
   VoC_be16_rd(CII_sid_mode,REG7,CONST_3_ADDR)
   VoC_lw16i_set_inc(REG0,STATIC_CONST_bitno_MR74_ADDR,1);
   VoC_be16_rd(CII_sid_mode,REG7,CONST_4_ADDR)
   VoC_lw16i_set_inc(REG0,STATIC_CONST_bitno_MR795_ADDR,1);
   VoC_be16_rd(CII_sid_mode,REG7,CONST_5_ADDR)
   VoC_lw16i_set_inc(REG0,STATIC_CONST_bitno_MR102_ADDR,1);
   VoC_be16_rd(CII_sid_mode,REG7,CONST_6_ADDR)
   VoC_lw16i_set_inc(REG0,STATIC_CONST_BITNO_ADDR,1);
    VoC_be16_rd(CII_sid_mode,REG7,CONST_7_ADDR)
   VoC_lw16i_set_inc(REG0,STATIC_CONST_bitno_MRDTX_ADDR,1);
CII_sid_mode:
...
/************************************************
  ...
  Rewrote   by Cui     28/02/2008 
       used the compressed lsf table
************************************************/


void CII_Vq_subvec3_lsf_compressed(void) 
{
    ...
    VoC_rbinc_i(REG7,13,DEFAULT);
	   		// create sign bit
	VoC_shru16_ri(REG7,(-3),DEFAULT);
    VoC_shr16_ri (REG7,(3),DEFAULT);
	
   	VoC_lw32_d(ACC1,CONST_0x7FFFFFFF_ADDR);

   	VoC_sub16inc_pr(REG7,REG0,REG7,DEFAULT);
       ...
       VoC_shr16_ri (REG6,(3),IN_PARALLEL);	 
		...
		VoC_sub16inc_pr(REG6,REG0,REG6,DEFAULT);    //temp = mult(wf1[0], temp);     
	   ...
	   VoC_shru16_ri(REG7,(-3),DEFAULT);
        VoC_shr16_ri (REG7,(3),DEFAULT);
		VoC_sub16_pr(REG7,REG0,REG7,DEFAULT);           //temp = mult(wf1[1], temp);                
		...
		VoC_bngt32_rr(CII_Vq_subvec_new_02,ACC1,ACC0);   //if (L_sub(dist, dist_min) < (Word32) 0) {                                
			VoC_movreg32(ACC1,ACC0,DEFAULT);         //   dist_min = dist;                      
			VoC_movreg16(REG4,REG2,IN_PARALLEL);    //   index = i;                            
CII_Vq_subvec_new_02:                                           //}
     	VoC_lw16i_short(REG5,1,DEFAULT);
	VoC_bez32_r(Vq_subvec3_L32,RL7);
       	VoC_lw16i_short(REG5,4,DEFAULT);
...
Vq_subvec3_L32:
    ...
    VoC_mult16_rd(REG6,REG6,CONST_13_ADDR);
	...
	VoC_add16_rd(REG1,REG1,CONST_3_ADDR);
   	...
   	VoC_lbinc_p(REG3);
    VoC_rbinc_r(REG5,REG5,DEFAULT);
    VoC_lbinc_p(REG3);
     
	
	VoC_loop_i_short(3,DEFAULT);
	VoC_startloop0
        VoC_rbinc_i(REG5,13,DEFAULT);
	    VoC_shru16_ri(REG5,(-3),DEFAULT);
...
Q_PLSF_3_PRED_END:
	...
	VoC_lw32_sd(REG45,1,IN_PARALLEL);		 
      //       temp_3[0]=DICO1_SIZE_3;  
        
       VoC_lw16i_set_inc(REG3,TABLE_DICO1_LSF_3_compressed_ADDR,1);
	...
	VoC_lw16i(ACC1_HI,TABLE_DICO2_LSF_3_compressed_ADDR);           //  	temp_3[1]=DICO2_SIZE_3;	temp_ptr_1=dico2_lsf_3;         
    VoC_lw16i(RL6_HI,TABLE_DICO3_LSF_3_compressed_ADDR);            //  	temp_3[2]=DICO3_SIZE_3;	temp_ptr_2=dico3_lsf_3;         
	...
	VoC_bgt16_rd(Vq_subvec3_L1a,REG4,CONST_1_ADDR);	     //      {   // MR475, MR515 /                                                   
     	VoC_lw16i(RL6_HI,STATIC_mr515_3_lsf_compressed_ADDR);           //        temp_ptr_2=mr515_3_lsf;
...
Vq_subvec3_L1a:                                               //      else if (sub (mode, MR795) == 0) 
	...
	VoC_lw16i_set_inc(REG3,STATIC_mr795_1_lsf_compressed_ADDR,1);   //        temp_ptr_0=mr795_1_lsf;                                               
...
Vq_subvec3_L2:                                               //      }    
	...
	VoC_push32(RL6,DEFAULT);	                     //      {   // MR59, MR67, MR74, MR102 , MRDTX 
   	...
   	VoC_jal(CII_Vq_subvec3_lsf_compressed);   
...
Vq_subvec3_L3:                                               //      {   // MR475, MR515                                                      
	...
	VoC_jal(CII_Vq_subvec3_lsf_compressed);                             //  	  temp_3[1]=temp_3[1]/2;                                                
	VoC_pop32(REG23,DEFAULT);                            //      }                                                                       
                //       indice[1] = Vq_subvec3(&lsf_r1[3], temp_ptr_1, &wf1[3], temp_3[1], i); 
                                                                                                                 
    VoC_lw16i_short(RL6_LO,2,DEFAULT);
	...
	VoC_push32(REG01,DEFAULT);
 				//       indice[2] = Vq_subvec4(&lsf_r1[6], temp_ptr_2, &wf1[6], temp_3[2]); 

    VoC_jal(CII_Vq_subvec_lsf_compressed);	
...

{
	...
	VoC_multf16_rr(REG5, REG5,  REG2,DEFAULT);
   	...
   	VoC_lw16i_set_inc(REG2, 32,1);
	VoC_mac32_rd(REG5,CONST_1_ADDR);                

	VoC_lw16i(REG0, 32588);

	VoC_be16_rd(label_tes_end, REG7, CONST_4_ADDR)

	VoC_lw16i(REG0, 32268);

	VoC_be16_rd(label_tes_end, REG7, CONST_3_ADDR) 

//	VoC_lw16i_set_inc(REG2, 64,1);	// MR59, MR515, MR475,MR102 guosz	
    VoC_shr16_ri(REG2,-1,DEFAULT);  //32*2

    VoC_lw16i(REG0,16678);

	VoC_bne16_rd(label_tes_end, REG7, CONST_5_ADDR)   	// ACC1_HI : frac_en
  
     VoC_lw16i_short(REG0, -11,DEFAULT);  
	VoC_sub16_rr(REG1, REG0, REG1,DEFAULT);		//   REG1:         exp_en
...
//REG7   dist_min2    REG6    dist_min1
    ...
    VoC_lw32_d(REG45,CONST_600_32000_ADDR);

	VoC_blt16_rd(CHECK_LSP_03,REG4,STRUCT_LSPSTATE_LSP_OLD_ADDR+1)

   VoC_lw32_d(REG45,CONST_800_30500_ADDR);
...
//REG5    dist_th
	...
	VoC_blt16_rd(CHECK_LSP_04,REG6,CONST_1500_ADDR)
...
CHECK_LSP_05:
	...
	VoC_lw16i_short(REG6,0,DEFAULT);	//return value
...
Bgnscd_LA1:   							//      frameEnergyMin = st->frameEnergyHist[i];          
   ...
   VoC_lw16_sd(REG0,0,DEFAULT);				//maxEnergy = st->frameEnergyHist[0];      
...
Bgnscd_LA3:   							//   }  	REG4->maxEnergyLastPart  	
   ...
   VoC_bgt16_rr(Bgnscd_LA5,REG7,REG2);				//     (sub(currEnergy, LOWERNOISELIMIT) > 0) &&                	<1953>
...
Bgnscd_LA9:   							//	REG6->ltpLimit
   ...
   VoC_lw16i_short(REG7,0,DEFAULT);
   VoC_bgt16_rr(Bgnscd_LA15,REG1,REG4);		//   prevVoiced = 1;     
...
Bgnscd_Lab11:   					//}                                                         
   ...
   VoC_lw16i_short(REG7,0,DEFAULT);
   VoC_bgt16_rr(Bgnscd_LA15,REG1,REG4); 
Bgnscd_Lab15:   
...
{    
	...
	VoC_sub16_rd(REG7,REG7,(61+  STATIC_CONST_table_gain_highrates_ADDR));  //9000      //  ma_pred_init = sub(ma_pred_init, 9000);        
...
void COOSAND_lsf_compressed_read(void) 
{
  ...
  VoC_mult16_rd(REG4,REG4,CONST_13_ADDR);
   ...
   VoC_lw16i_short(REG5,0xf,DEFAULT);
   VoC_and16_rr(REG5,REG4,DEFAULT);
   VoC_shr16_ri(REG4,4,DEFAULT);
 
   VoC_add16inc_rp(REG0,REG4,REG1,DEFAULT);
...
Dplsf3_lab1:    						//else { 
	...
	VoC_lw16i(REG5,TABLE_DICO2_LSF_3_compressed_ADDR);  
	VoC_lw16i_set_inc(REG1,DEC_AMR_TEMP20_ADDRESS,1);  
	VoC_lw16i(REG4,TABLE_DICO1_LSF_3_compressed_ADDR);  
  
	VoC_lw16i(REG6,TABLE_DICO3_LSF_3_compressed_ADDR); 

	VoC_bne16_rd(Dplsf3_lab4,REG7,CONST_5_ADDR);   		//{    
	VoC_lw16i(REG4,STATIC_mr795_1_lsf_compressed_ADDR);         		//p_cb1 = mr795_1_lsf;                             
	                                              		//p_cb2 = dico2_lsf;                               
                                        	       		//p_cb3 = dico3_lsf;  }                      
Dplsf3_lab4:      
	...
	VoC_lw16i(REG6,STATIC_mr515_3_lsf_compressed_ADDR);  			//{   p_cb1 = dico1_lsf;                             
	                                     			//   p_cb2 = dico2_lsf;                             
	                                    		  	//   p_cb3 = mr515_3_lsf;   
...
Dplsf3_lab3:       						//else if (sub (mode, MR795) == 0) 
	...
	VoC_lw16i_set_inc(REG3,DEC_AMR_TEMP10_ADDRESS,1); 	 
	        //lsf1_r[0] = *p_dico++; 
                          //lsf1_r[1] = *p_dico++; 
                                     //lsf1_r[2] = *p_dico++; 
  VoC_jal(COOSAND_lsf_compressed_read);
...
void CII_Post_Process(void)
{
    ...
    //    VoC_lw16i(RL7_LO,7699);
    //   VoC_lw16i(RL7_HI, -15398);
    //      VoC_lw32_d(RL7, CONST_0xc3da1e13_ADDR);
		...
		VoC_loop_i(1,160); 				//for (i = 0; i < 160; i++) 
           ...
           VoC_lw16i(REG2,-7667 );
			...
			VoC_loop_i_short(2,DEFAULT) 
			VoC_startloop0 
    				VoC_mac32_rr(REG7,REG2,DEFAULT);	//    L_tmp = L_mult (st->y1_hi, a[1]);                
    				VoC_multf16_rr(REG5,REG6,REG2,DEFAULT); 
    			   
					 VoC_lw16i(REG2,15836);
          ...
          //  VoC_movreg32(REG23,RL7,IN_PARALLEL);
            VoC_lw32_d(REG23, CONST_0xc3da1e13_ADDR);
    		...
    		VoC_bimac32_rr(REG67,REG23);	//    L_tmp = L_mac (L_tmp, st->x0, b[0]); 
    		 
    		VoC_mac32_rr(REG5,REG2,DEFAULT);	//    L_tmp = L_mac (L_tmp, x2, b[2]); 
