...
*************************************************************************/
   ...
   voc_short STATIC_CONST_INTERP_FACTOR_FR_ADDR ,24,x
#endif
...
PRM2BIT_L2:
	...
	VoC_loop_i(1,17)
		...
		VoC_startloop0
           ...
           VoC_bngt16_rr(PRM2BIT_001,REG3,REG6)
...
//	   	   VoC_sub16_rd(REG6,REG7,CONST_0x2b33_ADDR);	//	{ 	assert( temp >= 11059 );
        VoC_lw16i(REG3,0x2b33);
         VoC_sub16_rr(REG6,REG7,REG3,DEFAULT);
...
tx_dtx_100:
		...
		VoC_bngt16_rr(tx_dtx_end, REG3 , REG2  )
...
LARp_to_rp_L1:						//	else
		...
		VoC_bgt16_rr(LARp_to_rp_L5, REG3, REG4)     //	{ 	temp  = *LARp; }
...
//	 *s->reg0
	...
	VoC_jal(CII_LARp_to_rp);
	VoC_lw16i_short(REG2,27,DEFAULT);
	VoC_lw16_sd(REG0,5,DEFAULT);		//reg0->*s
	VoC_lw16i_short(REG7,13,IN_PARALLEL);
...
TONE_DETECTION_L2:
   ...
   VoC_lw16i_short(REG0,-1,DEFAULT);
	VoC_bngt16_rr(TONE_DETECTION_L4,REG5,REG0);
...
TONE_DETECTION_L10:
	...
	VoC_lw16i_short(REG3,0,DEFAULT);
	VoC_lw16i(REG6,1464);
	VoC_bngt16_rr(TONE_DETECTION_End,REG6,REG7);	//temp = gsm_sub (prederr, PREDTH);
	VoC_lw16i_short(REG3,1,DEFAULT);
TONE_DETECTION_End:
	VoC_pop16(RA,DEFAULT);
...
/*******************************************/
	...
	VoC_sub16_rr(REG1, REG6,REG1,IN_PARALLEL);
	//swShift in REG1
...
//	VoC_sub16_dr(REG1,CONST_19_ADDR,REG1);//        shift = gsm_sub  (gsm_norm (L_av0[0]), 3);
    VoC_lw16i_short(REG6,19,DEFAULT);
    VoC_sub16_rr(REG1,REG6,REG1,DEFAULT);//        shift = gsm_sub  (gsm_norm (L_av0[0]), 3);
...
gsm_decode_130:
	...
	VoC_lw16i_set_inc(REG3,STATIC_CONST_INTERP_FACTOR_FR_ADDR,1);//reg2 addr of interp_factor
...
//send
	...
	VoC_lw16i(ACC0_LO,0x6958);
    VoC_lw16i(ACC0_HI,0x7081);
...
RX_DTX160://rxdtx_N_elapsed = add (rxdtx_N_elapsed, 1);
	...
	VoC_jump(RX_DTX170);
RX_DTX161:
	VoC_lw16i_short(REG0,27,DEFAULT);
	VoC_bngt16_rr(RX_DTX162,REG2,REG0)  //if (sub (rxdtx_N_elapsed, DTX_ELAPSED_THRESHOLD) > 0)
...
void reset_fr_table(void)
{
  ...
  #define  interp_factor0    0x0555
  #define  interp_factor1    0x0aaa
  #define  interp_factor2    0x1000
  #define  interp_factor3    0x1555

  #define  interp_factor4    0x1aaa
  #define  interp_factor5    0x2000
  #define  interp_factor6    0x2555
  #define  interp_factor7    0x2aaa

  #define  interp_factor8    0x3000
  #define  interp_factor9    0x3555
  #define  interp_factor10   0x3aaa
  #define  interp_factor11   0x4000

  #define  interp_factor12    0x4555
  #define  interp_factor13    0x4aaa
  #define  interp_factor14    0x5000
  #define  interp_factor15    0x5555

  #define  interp_factor16    0x5aaa
  #define  interp_factor17    0x6000
  #define  interp_factor18    0x6555
  #define  interp_factor19    0x6aaa

  #define  interp_factor20    0x7000
  #define  interp_factor21    0x7555
  #define  interp_factor22    0x7aaa
  #define  interp_factor23    0x7fff


  VoC_lw16i_set_inc(REG0,STATIC_CONST_INTERP_FACTOR_FR_ADDR,2);

  VoC_lw16i(REG6,interp_factor0);
  VoC_lw16i(REG7,interp_factor1);
  VoC_lw16i(ACC0_LO,interp_factor2);
  VoC_lw16i(ACC0_HI,interp_factor3);
  VoC_sw32inc_p(REG67,REG0,DEFAULT);
  VoC_sw32inc_p(ACC0,REG0,DEFAULT);

  VoC_lw16i(REG6,interp_factor4);
  VoC_lw16i(REG7,interp_factor5);
  VoC_lw16i(ACC0_LO,interp_factor6);
  VoC_lw16i(ACC0_HI,interp_factor7);
  VoC_sw32inc_p(REG67,REG0,DEFAULT);
  VoC_sw32inc_p(ACC0,REG0,DEFAULT);


  VoC_lw16i(REG6,interp_factor8);
  VoC_lw16i(REG7,interp_factor9);
  VoC_lw16i(ACC0_LO,interp_factor10);
  VoC_lw16i(ACC0_HI,interp_factor11);
  VoC_sw32inc_p(REG67,REG0,DEFAULT);
  VoC_sw32inc_p(ACC0,REG0,DEFAULT);

  VoC_lw16i(REG6,interp_factor12);
  VoC_lw16i(REG7,interp_factor13);
  VoC_lw16i(ACC0_LO,interp_factor14);
  VoC_lw16i(ACC0_HI,interp_factor15);
  VoC_sw32inc_p(REG67,REG0,DEFAULT);
  VoC_sw32inc_p(ACC0,REG0,DEFAULT);

  VoC_lw16i(REG6,interp_factor16);
  VoC_lw16i(REG7,interp_factor17);
  VoC_lw16i(ACC0_LO,interp_factor18);
  VoC_lw16i(ACC0_HI,interp_factor19);
  VoC_sw32inc_p(REG67,REG0,DEFAULT);
  VoC_sw32inc_p(ACC0,REG0,DEFAULT);


  VoC_lw16i(REG6,interp_factor20);
  VoC_lw16i(REG7,interp_factor21);
  VoC_lw16i(ACC0_LO,interp_factor22);
  VoC_lw16i(ACC0_HI,interp_factor23);
  VoC_sw32inc_p(REG67,REG0,DEFAULT);
  VoC_sw32inc_p(ACC0,REG0,DEFAULT);
