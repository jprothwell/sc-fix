...
******************************************************************/
	...
	VoC_loop_i(0,2);
    		...
    		VoC_lw16i(REG5,124);
		VoC_bngt16_rr(decompress10_L1,REG4,REG5); 	//if (sub(MSBs, 124) > 0) //{
	//	VoC_lw16i(REG4,124);     //   MSBs = 124;
        VoC_movreg16(REG4,REG5,DEFAULT);
decompress10_L1:		   		//}

		VoC_lw16i(REG0,1311);
		VoC_multf16_rr(REG5,REG4,REG0,DEFAULT);	//ia = mult(MSBs, 1311);        REG5
		VoC_lw16i_short(REG0,25,DEFAULT);//change
		VoC_multf32_rr(REG01,REG5,REG0,DEFAULT);	//ia = sub(MSBs, extract_l(L_shr(L_mult(ia, 25), 1)));
		VoC_NOP();//change
		...
		VoC_multf16_rd(REG7,REG4,CONST_0x199a_ADDR);  	//REG7->mult(ia, 6554)
		VoC_NOP();//change
...
******************************************************************/
	...
	VoC_multf32_rr(REG45,REG4,REG5,DEFAULT);	//MSBs0_24 = shr(add(extract_l(L_shr(L_mult(MSBs, 25), 1)), 12), 5);
	...
	VoC_multf16_rd(REG5,REG4,CONST_0x199a_ADDR);	//ia = mult(MSBs0_24, 6554) & 1;
...
Dec_amr_L39_102:	        					//    }
	...
	VoC_bngt16_rd(Dec_amr_L52_102,REG6,CONST_0x00004000L_ADDR); 	//if (sub (pit_sharp, 16384) > 0)
