...
*/
	...
	voc_alias STATIC_BUF_P_RX_ADDR                              	STRUCT_POST_PROCESSSTATE_X1_ADDR       ,x
      ...
      voc_short VAD_FLAG_ADDR_P                                         ,x      // 1  seems not in use!!
      voc_short TX_SP_FLAG_ADDR                                         ,x     // 1   seems not in use!!
	...
	voc_short   GLOBAL_GAIN_CODE_OLD_TX_ADDR                      ,6,y
...
SEARCH_COPY_label_100:
	...
	VoC_sub16_rr(REG1, REG4, REG2,DEFAULT);
	exit_on_warnings=OFF;
...
RESET_AMR:
    ...
    VoC_sw16_d(REG6,STRUCT_PITCHOLWGHTSTATE_OLD_T0_MED_ADDR);// maybe can be removed.

    VoC_lw16i_set_inc(REG0, STRUCT_COD_AMRSTATE_OLD_LAGS_ADDR,2);
    VoC_lw16i(REG5,4096);
  	VoC_loop_i_short(3,DEFAULT)
	VoC_startloop0
   	   VoC_sw32inc_p(REG67,REG0,DEFAULT);
    VoC_endloop0
    VoC_sw16_d(REG5,STRUCT_LEVINSONSTATE_OLD_A_ADDR);


   VoC_lw16i_set_inc(REG2, STATIC_CONST_lsp_init_data_ADDR,2);
  
   VoC_lw16i_set_inc(REG0, STRUCT_LSPSTATE_LSP_OLD_ADDR,2);
   VoC_lw16i_set_inc(REG1, STRUCT_LSPSTATE_LSP_OLD_Q_ADDR,2);
   VoC_lw16i_set_inc(REG3, STRUCT_DTX_ENCSTATE_LSP_HIST_ADDR,2);
   ...
   VoC_lw32_d(ACC0,CONST_D_0xc800_ADDR) ; //#define MIN_ENERGY       -14336       // 14                 Q10
   VoC_lw32_d(ACC1,CONST_D_0xf6b3_ADDR) ;   //#define MIN_ENERGY_MR122  -2381       // 14 / (20*log10(2)) Q10

     VoC_lw16i_set_inc(REG0, STRUCT_GC_PREDST_ADDR,2);
     VoC_lw16i_set_inc(REG1, STURCT_GC_PREDUNQST_ADDR,2);
     VoC_lw16i_set_inc(REG2, STRUCT_GC_PREDST_PAST_QUA_EN_MR122_ADDR,2);
     VoC_lw16i_set_inc(REG3, STRUCT_GC_PREDUNQST_PAST_QUA_EN_MR122_ADDR,2);
 ...
 // 	VoC_lw16i(REG4,40);
   ...
   VoC_lw32_d(ACC0,CONST_D_150_ADDR) ;  //#define NOISE_INIT 150
 //  VoC_sw16_d(REG4,STRUCT_PITCHOLWGHTSTATE_OLD_T0_MED_ADDR);// maybe can be removed.

    VoC_lw16i_set_inc(REG0, STRUCT_VADSTATE1_BCKR_EST_ADDR,2);

  	VoC_lw16i(REG5,  0x3332 );    //#define CVAD_LOWPOW_RESET        (Word16) (0.40 * (Word16)0x7fff)
    ...
    VoC_sw16_d(REG5,STRUCT_VADSTATE1_BEST_CORR_HP_ADDR);
    VoC_sw16_d(REG5,STRUCT_VADSTATE1_CORR_HP_FAST_ADDR);
    VoC_lw16i_short(REG4,7,DEFAULT);
  	VoC_lw16i(REG5,0x7FFF);
    VoC_sw16_d(REG4,STRUCT_DTX_ENCSTATE_DTXHANGOVERCOUNT_ADDR);
    VoC_sw16_d(REG5,STRUCT_DTX_ENCSTATE_DECANAELAPSEDCOUNT_ADDR);
...
void CII_dtx_com_s2(void)
{
	...
	VoC_sub16_rd(REG4,REG4,CONST_8521_ADDR);		// log_en = sub(log_en, 8521);
	VoC_return;
}
...
void CII_DIV_32(void)
{
	...
	VoC_bnltz32_r(DIV_32_L_ABS_IN,REG45)
		VoC_sub32_dr(REG45,CONST_0_ADDR,REG45);   //    t1 = L_abs (t0);
DIV_32_L_ABS_IN:
	...
	VoC_bngtz32_r(DIV_32_L_ABS_OUT,ACC1)					//   if (t0 > 0)
	   VoC_sub32_dr(REG67,CONST_0_ADDR,REG67);  //    t2 = L_negate (t2);         /* K =-t0/Alpha                */
DIV_32_L_ABS_OUT:
...
void CII_Inv_sqrt(void)
{
	...
	VoC_bgt32_rd(INV_SQRT101,REG01,CONST_0_ADDR)
...
INV_SQRT102:
    ...
    VoC_shru16_ri(REG0,1,DEFAULT); 	//REG0 for a		
...
void CII_Levinson (void)
{
	...
	VoC_add32_rd(ACC0,REG67,CONST_0x00008000_ADDR);


	VoC_shru16_ri(REG6,1,DEFAULT);
	VoC_shr32_ri(REG45,4,IN_PARALLEL);  //   L_Extract (t0, &Kh, &Kl);           /* K in DPF         */
	...
	exit_on_warnings=ON;
	VoC_mac32_rr(REG0,REG3,DEFAULT);              //  REG3      1
	VoC_mac32_rr(REG0,REG3,DEFAULT);           // t0 = Mpy_32 (Kh, Kl, Kh, Kl);       /* K*K             */
...
LEVINSON_L_ABS_2:
	...
	VoC_sub32_dr(REG45,CONST_0x7FFFFFFF_ADDR,ACC0); // REG23 = 0X7FFFFFFF
	...
	VoC_loop_i(1,9)
		...
		VoC_shru16_ri(REG6,1,IN_PARALLEL);   //Kh   Kl
...
LEVINSON_ABS_S_1:
		...
		VoC_bngt16_rd(LEVINSON_04,REG5,CONST_32750_ADDR)
...
LEVINSON_101:
		...
		VoC_shru16_ri(REG4,1,DEFAULT);   //   L_Extract (t2, &Anh[i], &Anl[i]);
		...
		VoC_bnltz32_r(LEVINSON_L_ABS_4,ACC0)
			VoC_sub32_dr(ACC0,CONST_0_ADDR,ACC0);//  t0 = L_abs (t0);                        /* Some case <0 !! */
LEVINSON_L_ABS_4:

 
		VoC_sub32_dr(REG23,CONST_0x7FFFFFFF_ADDR,ACC0);// t0 = L_sub ((Word32) 0x7fffffffL, t0);  /* 1 - K*K        */
		/* DPF format */
		VoC_shru16_ri(REG2,1,DEFAULT);        //    L_Extract (t0, &hi, &lo);               /* DPF format      */
...
Get_lsp_pol_L1:
        ...
        VoC_msu32inc_rp(REG3,REG1,DEFAULT);
...
void CII_Lsp_Az(void)
{
	...
	VoC_startloop0
		...
		VoC_add32_rp(ACC0,RL7,REG1,DEFAULT);
        VoC_sub32inc_rp(ACC1,RL7,REG1,IN_PARALLEL);
...
*/
  	VoC_lw32z(REG45,DEFAULT);
	VoC_bngtz32_r(LOG1002,ACC0);    /* if (L_x <= (Word32) 0) goto */
	...
	VoC_sub16_dr(REG1,CONST_30_ADDR,REG1);//reg1:exponent
 	VoC_shr32_ri(REG23,9,DEFAULT);   //reg2:a
...
void CII_Lsf_wt(void)
{
	...
	VoC_sub16_dr(REG6,CONST_0x4000_ADDR,REG6); //wf[9] = sub (16384, lsf[8])

	VoC_lw32_d(REG45,CONST_28160_ADDR);//CONST_28160_ADDR and CONST_1843_ADDR

	VoC_loop_i_short(10,DEFAULT);
	VoC_startloop0

        VoC_multf16_rp(REG0,REG4,REG3,DEFAULT);
       	VoC_sub16_pr(REG7,REG3,REG5,DEFAULT);	//REG1: temp

        VoC_sub16_dr(REG0,CONST_3427_ADDR,REG0);//REG0 for wf[i]
        VoC_multf16_rd(REG2,REG7,CONST_6242_ADDR);

		VoC_bltz16_r(Lsf_wt101,REG7);
        VoC_sub16_rr(REG0,REG5,REG2,DEFAULT);
Lsf_wt101:
...
void CII_Residu_new (void)
{
	...
	VoC_movreg16(RL7_LO,REG0,DEFAULT);   	// REG0 for a[j]
	 VoC_lw16i_short(INC1,-1,IN_PARALLEL);
...
subframePostProc102:
    ...
    VoC_lw16i_short(FORMAT32,-2,DEFAULT);  	// tempShift = 1;
...
void CII_Pow2 (void)
{
	...
	VoC_shru16_ri(REG2,1,DEFAULT);      //reg2:a
	...
	VoC_bngt16_rr(POW2101,REG0,REG4);
...
POW2101:
	...
	VoC_bngt16_rd(POW2102,REG0,CONST_1_ADDR);
		VoC_add32_rd(REG23,REG23,CONST_1_ADDR);
...
void CII_sqrt_l_exp (void)
{
	...
	VoC_jal(CII_NORM_L_ACC0);
   	VoC_pop16(RA,DEFAULT);
	VoC_lw32z(ACC0,DEFAULT);
	VoC_bngtz32_r(sqrt_l_exp_go, REG67)
	 
	VoC_and16_ri(REG1,0xFFFE);    //	 REG1     e    // e = norm_l (L_x) & 0xFFFE;  ;
	VoC_sub16_dr(REG0,CONST_0_ADDR,REG1);
	VoC_shr32_rr(REG67,REG0,DEFAULT);
          // REG1    exp
	VoC_shr32_ri(REG67,9, DEFAULT);
	VoC_lw16i_set_inc(REG0, (TABLE_SQRT_L_ADDR-16), 1);
...
void CII_gc_pred_update(void)
{
	...
	VoC_add16_rd(REG0,REG5,CONST_2_ADDR);
...
void CII_gmed_n(void) //new  //return REG1
{
		...
		VoC_movreg16(ACC0_HI,REG1,DEFAULT);//reg4 addr of tmp2[i]
		...
		VoC_endloop0;
     	...
     	VoC_movreg16(REG1,ACC0_HI,DEFAULT);//addr tmp2[j]
     	VoC_lw16i(ACC0_LO,-16384);
		...
		VoC_loop_r(1,REG5);
			...
			VoC_endloop0;
          	...
          	VoC_movreg16(REG1,ACC0_HI,DEFAULT);//addr tmp2[j]
			VoC_sub16_rr(REG2,REG6,REG1,DEFAULT);//&tmp2[ix]

            VoC_sw16_p(ACC0_LO,REG6,DEFAULT); //tmp2[ix] = -16384;
...
void CII_Autocorr(void)
{
	...
	voc_short	 Windows_table_ADDR                  ,240,y                          //~241
#endif
	...
	/* Windowing of signal */
	VoC_push16(RA,DEFAULT);
   	VoC_lw16i_short(RL7_LO,4,DEFAULT);
...
//	VoC_NOP();
	VoC_lw16i_set_inc(REG0,AUTOCORR_Y_ADDR,2);	
...
AUTOCORR_01_END:
	...
	VoC_add32_rd (ACC0, ACC0,CONST_1_ADDR);	//s+=lL
...
void CII_Chebps (void)
{
	...
	// b1 = 2*x + f[1]
	VoC_multf32_rd(REG45,REG3,(STATIC_CONST_WIND_200_40_compressd_ADDR+41));
...
exit_on_warnings = ON;
	...
	VoC_bimac32inc_pp(REG6,REG7);			//-b2
   	...
   	VoC_mac32_rp(REG4,REG2,DEFAULT);		//+f[i]/2
	VoC_multf32_rr(REG67,REG5,REG3,IN_PARALLEL);//b1_h*x
	VoC_NOP();
...
void CII_cor_h( void)
{
	...
	VoC_bne16_rd(COR_H101,REG1,CONST_0x00007FFF_ADDR)					/* if (j == 0) */
...
//	  VoC_lw16i_short(INC0,1,DEFAULT);
		VoC_lw16i_set_inc(REG0,32440,1);
...
exit_on_warnings=ON;
    ...
    VoC_lw16i_short(RL7_LO,38,IN_PARALLEL);	
	...
	VoC_loop_i(1,39)
		...
		VoC_lw32_sd(REG01,0,DEFAULT);        //REG0:&h2[0]
...
exit_on_warnings=ON;
		...
		VoC_pop32(RL6,DEFAULT);
		VoC_add32_rd(RL7,RL7,CONST_0x0000FFFF_ADDR);//dec++;i--
...
G_PITCH_01_END:
	...
	VoC_bltz16_r(G_PITCH_03_END1,REG1);			//i<0 ?
		...
		VoC_bngt16_rd(G_PITCH_03_END,REG2,(0+STATIC_CONST_F_GAMMA2_ADDR));			//?gain>19661
...
PRED_IT_3OR6_001:
         VoC_lw16i_short(REG4,1,DEFAULT);
	   	 VoC_lw16i_short(REG3,6,DEFAULT);
		...
		VoC_bnltz16_r(PRED_IT6_003,REG1)//if (frac < 0)   REG4   frac
			...
			VoC_sub16_rr(REG6,REG6,REG4,DEFAULT);//x0--;
PRED_IT6_003:

	 
               VoC_add16_rr(REG2,REG7,REG1,DEFAULT);//c1 = &inter_6[frac]
               VoC_sub16_rr(REG3,REG3,REG1,DEFAULT);   // sub (UP_SAMP_MAX, frac)
...
//  begin of ph_disp
   	...
   	VoC_lw16i_short(REG3,1,IN_PARALLEL);
	   	...
	   	VoC_bnlt16_rd(Phdisp_L1,REG7,( 9 + STATIC_CONST_QUA_GAIN_PITCH_ADDR)  );	// CONST_14746_ADDR if (sub(ltpGain, PHDTHR2LTP) < 0) {
   		VoC_lw16i_short(REG4,0,DEFAULT);			// } else
   		VoC_bngt16_rd(Phdisp_L1,REG7,( 4 + STATIC_CONST_QUA_GAIN_PITCH_ADDR) );	// CONST_9830_ADDR   if (sub(ltpGain, PHDTHR1LTP) > 0)
   			VoC_lw16i_short(REG4,1,DEFAULT);			//    {  impNr = 1;

Phdisp_L1:    						//{impNr = 2; }  REG2->impNr
    VoC_lw16i_short(INC2,1,DEFAULT);
   				//tmp1 = round(L_shl(L_mult(state->prevCbGain, ONFACTPLUS1), 2));
   	VoC_multf32_rd(REG67,REG5,CONST_0x00004000L_ADDR);
...
Phdisp_L4:   						//else
   	...
   	VoC_bnez16_r(Phdisp_L8,REG1);			//if (state->onset == 0)
   								//{   i1 = 0;
   	...
   	VoC_startloop0	
	   	...
	   	VoC_bnlt16_rd(Phdisp_L7,REG7,(STATIC_CONST_QUA_GAIN_PITCH_ADDR+4));			// CONST_9830_ADDR       if (sub(state->gainMem[i], PHDTHR1LTP) < 0)
...
Dec_amr_L68:         			//}
	...
	VoC_bngt16_rd(Dec_amr_L69,REG4,CONST_0x00004000L_ADDR); //if (sub (pit_sharp, 16384) > 0)
...
void CII_AMR_Decode(void)
{
	...
	VoC_push16(RA,DEFAULT);
    	...
    	VoC_lw32z(RL6,DEFAULT);
   //the REG6 is  0x177
	VoC_bgt16_rr(DEC_MODE_IS_AMR,REG6,REG7);
		// clear tx_frame_type in case of EFR
		VoC_sw16_d(RL6_HI,DEC_AMR_FRAME_TYPE_ADDR);
DEC_MODE_IS_AMR:
...
/*
    VoC_lw16i_set_inc(REG2,STATIC_CONST_dhf_MR122_ADDR,1);

    VoC_be16_rd(efr_label7,REG7,CONST_7_ADDR)
...
*/
      ...
      VoC_bnez16_d(efr_label6,GLOBAL_EFR_FLAG_ADDR);            // EFR
      VoC_lw16i_set_inc(REG2,STATIC_CONST_dhf_MR475_ADDR,1);
      VoC_bez16_r(amr_dec_main_mode,REG7)

      VoC_lw16i_set_inc(REG2,STATIC_CONST_dhf_MR515_ADDR,1);
      VoC_be16_rd(amr_dec_main_mode,REG7,CONST_1_ADDR)

      VoC_lw16i_set_inc(REG2,STATIC_CONST_dhf_MR59_ADDR ,1);
      VoC_be16_rd(amr_dec_main_mode,REG7,CONST_2_ADDR)

      VoC_lw16i_set_inc(REG2,STATIC_CONST_dhf_MR67_ADDR,1);
      VoC_be16_rd(amr_dec_main_mode,REG7,CONST_3_ADDR)

      VoC_lw16i_set_inc(REG2,STATIC_CONST_dhf_MR74_ADDR,1);
      VoC_be16_rd(amr_dec_main_mode,REG7,CONST_4_ADDR)

     VoC_lw16i_set_inc(REG2,STATIC_CONST_dhf_MR795_ADDR,1);
      VoC_be16_rd(amr_dec_main_mode,REG7,CONST_5_ADDR)
     VoC_lw16i_set_inc(REG2,STATIC_CONST_dhf_MR102_ADDR,1);
      VoC_be16_rd(amr_dec_main_mode,REG7,CONST_6_ADDR)
     VoC_lw16i_set_inc(REG2,STATIC_CONST_dhf_MR122_ADDR,1);
amr_dec_main_mode:
efr_label6:
		// EFR
...
SpDec_Fr_L2:  					//} else { Bits2prm (mode, serial, parm);}
    ...
    VoC_lw16_d(REG7,DEC_AMR_FRAME_TYPE_ADDR);
...
Post_Filter_L1:
    ...
    VoC_lw16i(REG7,STATIC_CONST_F_GAMMA4_ADDR);
    ...
    VoC_bgt16_rd(Post_Filter_L4,REG6,CONST_5_ADDR);		//  if (sub(mode, MR122) == 0 || sub(mode, MR102) == 0)
       ...
       VoC_lw16i(REG7,STATIC_CONST_F_GAMMA3_ADDR); 	//      Weight_Ai (Az, gamma4, Ap4);  }
Post_Filter_L4:
	...
	VoC_loop_i(1,2);
		...
		VoC_jal(CII_Weight_Ai);  	//AP4->	DEC_AMR_TEMP11_ADDRESS
...
exit_on_warnings=ON;
    ...
    VoC_bngtz16_r(Post_Filter_L6,REG0);	//   if (temp2 <= 0)
				//   else
    			//   {
		VoC_multf16_rd(REG0,REG0, (3+ STATIC_CONST_PDOWN_ADDR) );	//   CONST_26214_ADDR   temp2 = mult (temp2, MU);
...
Dec_amr_L4_go:
   ...
   VoC_lw16i_set_inc(REG1,STATIC_CONST_bitno_MR475_ADDR,1);
   VoC_bez16_r(Dec_amr_mode,REG7)
   VoC_lw16i_set_inc(REG1,STATIC_CONST_bitno_MR515_ADDR,1);
   VoC_be16_rd(Dec_amr_mode,REG7,CONST_1_ADDR)
   VoC_lw16i_set_inc(REG1,STATIC_CONST_bitno_MR59_ADDR,1);
   VoC_be16_rd(Dec_amr_mode,REG7,CONST_2_ADDR)
   VoC_lw16i_set_inc(REG1,STATIC_CONST_bitno_MR67_ADDR,1);
   VoC_be16_rd(Dec_amr_mode,REG7,CONST_3_ADDR)
   VoC_lw16i_set_inc(REG1,STATIC_CONST_bitno_MR74_ADDR,1);
   VoC_be16_rd(Dec_amr_mode,REG7,CONST_4_ADDR)
   VoC_lw16i_set_inc(REG1,STATIC_CONST_bitno_MR795_ADDR,1);
   VoC_be16_rd(Dec_amr_mode,REG7,CONST_5_ADDR)
   VoC_lw16i_set_inc(REG1,STATIC_CONST_bitno_MR102_ADDR,1);
   VoC_be16_rd(Dec_amr_mode,REG7,CONST_6_ADDR)
   VoC_lw16i_set_inc(REG1,STATIC_CONST_BITNO_ADDR,1);
    VoC_be16_rd(Dec_amr_mode,REG7,CONST_7_ADDR)
   VoC_lw16i_set_inc(REG1,STATIC_CONST_bitno_MRDTX_ADDR,1);
Dec_amr_mode:
	...
	//  begin of build_CN_param
	   ...
	   VoC_multf32_rd(ACC0,REG4,CONST_31821_ADDR); //*seed = extract_l(L_add(L_shr(L_mult(*seed, 31821), 1), 13849L));
		   ...
		   VoC_add32_rd(REG45,ACC0,CONST_13849L_ADDR);

		      VoC_lw16i_set_inc(REG1,STATIC_CONST_WIND_200_40_compressd_ADDR,1);
		   ...
		   VoC_and16_ri(REG6,0x7F); 		    	//p = &window_200_40[*seed & 0x7F];
		   VoC_lw16i_short(REG2,0,DEFAULT);
		   VoC_add16_rr(REG3,REG6,REG1,DEFAULT);
...
void CII_dec_reset(void)
{
	...
	VoC_lw16i_set_inc(REG0,STRUCT_DECOD_AMRSTATE_OLD_EXC_ADDR,2);
		 ...
		 VoC_sw32inc_p(ACC0,REG0,DEFAULT);
	     VoC_endloop0
      VoC_sw16_d(ACC0_LO,STRUCT_DECOD_AMRSTATE_SHARP_ADDR);
      VoC_sw32_d(ACC0,STRUCT_DECOD_AMRSTATE_PREV_BF_ADDR);
      VoC_sw16_d(ACC0_LO,STRUCT_DECOD_AMRSTATE_PREV_BF_ADDR+2);
      VoC_sw16_d(ACC0_LO,STRUCT_EC_GAIN_PITCHSTATE_PAST_GAIN_PIT_ADDR);
      VoC_sw16_d(ACC0_LO,STRUCT_EC_GAIN_CODESTATE_PAST_GAIN_CODE_ADDR);


      VoC_lw16i_set_inc(REG0,STRUCT_CB_GAIN_AVERAGESTATE_ADDR,2);
      VoC_lw16i_set_inc(REG1,STRUCT_D_PLSFSTATE_PAST_R_Q_ADDR,2);
      VoC_lw16i_set_inc(REG2,STRUCT_PH_DISPSTATE_GAINMEM_ADDR,1);
      	 VoC_loop_i(0,5)
		 VoC_sw32inc_p(ACC0,REG0,DEFAULT);
        VoC_sw32inc_p(ACC0,REG1,DEFAULT);
		VoC_sw16inc_p(ACC0_LO,REG2,DEFAULT);
      ...
      VoC_lw16i_set_inc(REG3,STRUCT_DECOD_AMRSTATE_LTPGAINHISTORY_ADDR,1);
       VoC_lw16i_set_inc(REG1,STRUCT_BGN_SCDSTATE_FRAMEENERGYHIST_ADDR,1);
     ...
     //   VoC_lw16i(REG4,40);
      //  VoC_lw16i(REG5,0x4000);
        VoC_lw32_d(REG45, CONST_0x40000028_ADDR) ;
		
       // VoC_lw16i_short(REG6,1,DEFAULT);
        //VoC_lw16i(REG7,21845);//MR_RESET_VALUE_TABLE_ADDR+34,
        VoC_lw32_d(REG67, CONST_0x55550001_ADDR) ;
	

   //     VoC_lw16i_short(ACC1_LO,7,DEFAULT);// AMR_RESET_VALUE_TABLE_ADDR+44,
     //   VoC_lw16i(ACC1_HI,32767);     //  //AMR_RESET_VALUE_TABLE_ADDR+46  is DTX,mean 1
           VoC_lw32_d(ACC1, CONST_0x7fff0007_ADDR) ;
       

        VoC_sw16_d(REG4,STRUCT_DECOD_AMRSTATE_OLD_T0_ADDR);
        VoC_sw16_d(REG4,STRUCT_DECOD_AMRSTATE_T0_LAGBUFF_ADDR);
        VoC_sw16_d(REG5,STRUCT_EC_GAIN_PITCHSTATE_PREV_GP_ADDR);
        VoC_sw16_d(REG6,STRUCT_EC_GAIN_CODESTATE_PREV_GC_ADDR);

        VoC_lw16i(REG4,1640);  //AMR_RESET_VALUE_TABLE_ADDR+24 is 1640
		VoC_lw16i_set_inc(REG0,TABLE_MEAN_LSF_ADDR,2);
      	VoC_lw16i_set_inc(REG1,STRUCT_D_PLSFSTATE_PAST_LSF_Q_ADDR,2);

		VoC_lw16i_set_inc(REG2,STRUCT_EC_GAIN_PITCHSTATE_ADDR,1);
      	VoC_lw16i_set_inc(REG3,STRUCT_EC_GAIN_CODESTATE_ADDR,1);

        VoC_loop_i_short(5,DEFAULT)
		   VoC_startloop0
         VoC_lw32inc_p(RL7,REG0,DEFAULT);
	     VoC_sw16inc_p(REG4,REG2,DEFAULT);
	     VoC_sw16inc_p(REG6,REG3,DEFAULT);   // AMR_RESET_VALUE_TABLE_ADDR+29 is 1
         VoC_sw32inc_p(RL7,REG1,DEFAULT);
		   VoC_endloop0

    	VoC_lw32_d(RL7,CONST_0x70816958_ADDR);
 
        VoC_sw32_d(ACC0,STRUCT_PH_DISPSTATE_PREVSTATE_ADDR);  //state->prevState = 0;state->prevCbGain = 0;
        VoC_sw32_d(ACC0,STRUCT_PH_DISPSTATE_LOCKFULL_ADDR);   //state->lockFull = 0;state->onset = 0;
        VoC_sw16_d(REG7,STRUCT_DECOD_AMRSTATE_NODATASEED_ADDR);
			   
      VoC_bnez32_r(dec_reset_Lable22,RL6)
		 ...
		 VoC_sw32inc_p(ACC0,REG0,DEFAULT);
	     VoC_endloop0
          VoC_sw32_d(ACC0,STRUCT_DTX_DECSTATE_LOG_EN_HIST_PTR_ADDR);
         VoC_endloop1


		VoC_sw16_d(REG7,STRUCT_DECOD_AMRSTATE_NODATASEED_ADDR);
		VoC_sw32_d(RL7,STRUCT_DTX_DECSTATE_L_PN_SEED_RX_ADDR);
		VoC_sw32_d(ACC1,STRUCT_DTX_DECSTATE_DTXHANGOVERCOUNT_ADDR);
		VoC_sw16_d(REG6,STRUCT_DTX_DECSTATE_DTXGLOBALSTATE_ADDR);

		VoC_lw16i_set_inc(REG0,STATIC_CONST_lsp_init_data_ADDR,2);
		VoC_lw16i_set_inc(REG1,STRUCT_DECOD_AMRSTATE_LSP_OLD_ADDR,2);
		VoC_lw16i_set_inc(REG2,STRUCT_DTX_DECSTATE_LSP_ADDR,2);
		VoC_lw16i_set_inc(REG3,STRUCT_DTX_DECSTATE_LSP_OLD_ADDR,2);
       ...
       VoC_lw32_d(REG67,CONST_D_0xc800_ADDR);  //#define MIN_ENERGY       -14336       // 14                 Q10 //
       VoC_lw32_d(ACC0,CONST_D_0xf6b3_ADDR);//#define MIN_ENERGY_MR122  -2381       // 14 / (20*log10(2)) Q10 //

       VoC_sw32_d(REG67,STRUCT_GC_PREDSTATE_PAST_QUA_EN_ADDR);
       VoC_sw32_d(REG67,STRUCT_GC_PREDSTATE_PAST_QUA_EN_ADDR+2);
...
//	  VoC_lw16i(REG6,3500);
//      VoC_movreg16(REG7,REG6,DEFAULT);
         VoC_lw32_d(REG67, CONST_D_3500_ADDR) ;
    

      VoC_sw16_d(REG5,STRUCT_DTX_DECSTATE_TRUE_SID_PERIOD_INV_ADDR);
      VoC_sw32_d(REG67,STRUCT_DTX_DECSTATE_TRUE_SID_PERIOD_INV_ADDR+1);

     	 VoC_lw16i_set_inc(REG0,STRUCT_DTX_DECSTATE_LOG_EN_HIST_ADDR,2);
	       VoC_loop_i_short(4,DEFAULT)
			VoC_startloop0
			VoC_sw32inc_p(REG67,REG0,DEFAULT);
	    	VoC_endloop0
           ...
           VoC_lw16i_set_inc(REG1,STRUCT_LSP_AVGSTATE_LSP_MEANSAVE_ADDR,2);
           VoC_lw16i_set_inc(REG2,STRUCT_DTX_DECSTATE_LSF_HIST_ADDR,2);
           ...
           VoC_loop_i(1,7)
	    	...
	    	VoC_endloop0
            VoC_movreg16(REG0,REG7,DEFAULT);
        VoC_sw32_d(ACC0,STRUCT_GC_PREDSTATE_PAST_QUA_EN_MR122_ADDR);
         	VoC_endloop1
    
        VoC_sw32_d(ACC0,STRUCT_GC_PREDSTATE_PAST_QUA_EN_MR122_ADDR+2);
dec_reset_Lable22:
...
void CII_agc(void)
{
   ...
   VoC_add32_rd(REG23,REG01,CONST_0x00008000_ADDR);
AGC_LA5:
...
// opti: two branch are deleted here by loading default values
   ...
   VoC_startloop0 					//{
	 ...
	 VoC_multf16_rd(REG6,REG5,(0+STATIC_CONST_F_GAMMA1_ADDR));	// 29491 agc: gain = mult (gain,agc_fac); 
   
    VoC_bnez16_r(AGC_LA6,REG4);
	VoC_add16_rr(REG5,REG6,REG3,DEFAULT); 		//agc: gain = add (gain, g0);
AGC_LA6:
...
*  Input: enum Mode mode ->REG7
	...
	ADD to decode the IF1 format AMR    by Cui    June 16, 2008
***************************************************************************/
...
#if 0
	voc_short 	AMR_DEC_BUFIN_ADDR                ,256 	,x
	voc_short 	AMR_IF1_DEC_BUFIN_ADDR            ,256 	,x
#endif


    VoC_push16(RA,DEFAULT);

    VoC_lw16i_set_inc(REG0, INPUT_BITS2_ADDR+8,2);
	VoC_lw16i_set_inc(REG1,AMR_DEC_BUFIN_ADDR,1);

    VoC_jal(Coolsand_If1_table_selet);

    VoC_bez16_d(CII_bits2prm_amr_efr_no_IF1, GLOBAL_IF1_flag_ADDR)

    VoC_lw16i_short(INC0,1,DEFAULT);
//input: REG0  source parameters     //(Coolsand_If1format2amr);
//       REG7   mode
//       REG2 table
//       REG6   Bit numbers
    ...
    VoC_lw16i_set_inc(REG3,AMR_IF1_DEC_BUFIN_ADDR+8,1);
    VoC_loop_i(1,16)
		VoC_lw32z(REG45,DEFAULT);
		VoC_lw16inc_p(REG4,REG0,DEFAULT);
    ...
    VoC_add16_rd(REG3,REG3,CONST_16_ADDR);
	VoC_endloop1

//      for(i = 0; i < bits_no[mode]; i++)
//	  {
//		j = amr_IF1[mode][i];
//		destinbit[j] = sorcerbit[i];
//	  }	

      VoC_lw16i_set_inc(REG3,AMR_IF1_DEC_BUFIN_ADDR+8,1);//discard the frame header 
      VoC_lw16i_set_inc(REG1,AMR_DEC_BUFIN_ADDR,1);

	  VoC_add16inc_rp(REG5,REG1,REG2,DEFAULT);
      VoC_loop_r(0,REG6)
    ...
    VoC_jump(CII_bits2prm_amr_efr_no_IF1_end);


CII_bits2prm_amr_efr_no_IF1:
...
CII_bits2prm_amr_efr_no_IF1_end:
 ...
 *************************************************************************/
    ...
    VoC_endloop0					//}
         ...
         VoC_shr16_ri(REG6,2,DEFAULT);      //av_pred_en = mult (av_pred_en, 8192);
/*
    VoC_lw16i(REG5,-2381);
...
*/
    ...
    VoC_bgt16_rd(Gcpred_L1,REG7,CONST_D_0xf6b3_ADDR);			//if (sub (av_pred_en, MIN_ENERGY_MR122) < 0)
 	
     VoC_lw16i(REG7,-2381);    	//{  av_pred_en = MIN_ENERGY_MR122; }
Gcpred_L1:    	  					//av_pred_en = 0;
...
*/
    ...
    VoC_bgt16_rd(Gcpred_L2,REG6,CONST_D_0xc800_ADDR);			//if (sub (av_pred_en, MIN_ENERGY) < 0)
 	 VoC_lw16i(REG6,-14336);   	//{av_pred_en = MIN_ENERGY;
Gcpred_L2:    
...
CII_Cb_gain_La2:
       		...
       		VoC_shr32_ri(ACC0,1,DEFAULT);
       		VoC_movreg16(REG5,REG1,DEFAULT);     //   tmp1 = shl(tmp1, shift1);
            ...
            VoC_sub16_rr(REG5,REG5,REG1,DEFAULT);//   shift = sub(add(2, shift1), shift2);
...
void CII_pseudonoise (void)
{
	...
	//if_else_1: if ((*shift_reg & 0x00000001) != 0) {Sn = 1; }    else {Sn = 0; }
		...
		VoC_and32_rd(ACC1,CONST_1_ADDR);
...
PSEUDONOISE103:
		...
		VoC_movreg32(REG45,ACC0,DEFAULT);
		VoC_and32_rd(REG45,CONST_1_ADDR);
...
Reord_L:
     // VoC_add16inc_pd(REG5,REG0,CONST_205_ADDR);	//    lsf_min = add (lsf[i], min_dist);
      VoC_add16inc_rp(REG5,REG3, REG0,DEFAULT);	//    lsf_min = add (lsf[i], min_dist);
   VoC_endloop0
   VoC_return;					            //}
}
	 ...
	 //  VoC_push16(REG0,DEFAULT);
       ...
       VoC_loop_i(1,10);
            ...
            VoC_add16_rr(REG6,REG1,REG4,DEFAULT); //i= add (i, k);
 			...
 			VoC_add16_rr(REG2,REG3,REG6, DEFAULT);        //REG2:addr of cod[i]
...
void CII_Vq_subvec_lsf_compressed(void)
{
	...
	VoC_loop_i(0,32);			//in loop REG5 for sign
	   VoC_lbinc_p(REG3);
       VoC_rbinc_i(REG6,13,DEFAULT);
	   VoC_lbinc_p(REG3);
       VoC_rbinc_i(REG7,13,DEFAULT);
	   		// create sign bit
		...
		VoC_sub16inc_pr(REG6,REG0,REG6,DEFAULT);	//  temp = sub (lsf_r1[0], *p_dico++);
		VoC_sub16inc_pr(REG7,REG0,REG7,IN_PARALLEL);    //  temp = mult (wf1[0], temp);
        VoC_push32(REG67,DEFAULT);
		...
		VoC_lw32_sd(REG01,1,DEFAULT);                   //  temp = mult (wf1[1], temp);
       ...
       VoC_rbinc_i(REG7,13,DEFAULT);
		...
		VoC_sub16inc_pr(REG6,REG0,REG6,DEFAULT);	//  temp = mult (wf2[0], temp);
		VoC_sub16inc_pr(REG7,REG0,REG7,IN_PARALLEL);    //  dist = L_mac (dist, temp, temp);
	    VoC_push32(REG67,DEFAULT);
		...
		VoC_bimac32_rr(REG67,REG67);                    //  dist = L_mac (dist, temp, temp);
		VoC_lw32_sd(REG01,3,DEFAULT);                   //   if (L_sub (dist, dist_min) < (Word32) 0)
		VoC_bngt32_rr(Vq_subvec_s_new_01,ACC1,ACC0);     //   {
		    	VoC_lw16i_short(REG5,0,DEFAULT);        //       dist_min = dist;
		    	VoC_movreg32(ACC1,ACC0,DEFAULT);         //       index = i;
...
Vq_subvec_s_new_01:                                             //    }
		...
		VoC_add16inc_rp(REG6,REG6,REG0,DEFAULT);	 // temp = mult (wf1[0], temp);
		VoC_add16inc_rp(REG7,REG7,REG0,IN_PARALLEL);     // dist = L_mult (temp, temp);
		...
		VoC_lw32_sd(REG01,2,DEFAULT);                    // dist = L_mac (dist, temp, temp);
		VoC_bimac32_rr(REG67,REG67);                     // temp = add (lsf_r2[0], *p_dico++);
                                                        // temp = mult (wf2[0], temp);
	    VoC_lw32_sd(REG67,0,DEFAULT);

		VoC_add16inc_rp(REG6,REG6,REG0,DEFAULT);	 // dist = L_mac (dist, temp, temp);
		VoC_add16inc_rp(REG7,REG7,REG0,IN_PARALLEL);     // temp = add (lsf_r2[1], *p_dico++);
		...
		VoC_lw32_sd(REG01,3,DEFAULT);                    // {
                                                                 //     dist_min = dist;
		VoC_bngt32_rr(Vq_subvec_s_new_02,ACC1,ACC0);      //     index = i;
...
Vq_subvec_s_new_02:
	...
	VoC_mult16_rd(REG4,REG4,CONST_13_ADDR);
...
Vq_subvec_LEnd:
	...
	VoC_bnez16_r(Vq_subvec_s_new_05,REG2); 		//if (sign == 0)
	 	//{
...
{
#if 0
	voc_short	 BIT_SERIAL_ADDR_RAM    ,244,x
    voc_short	 BIT_SERIAL_TRANS_ADDR    ,244+12,y 
#endif
    	VoC_push16(RA,DEFAULT);
...
COD12D2_NO_SID_ENCODING:
	...
	VoC_add16_rd(REG1,REG1,CONST_4_ADDR);
      ...
      VoC_lw16_d(REG7,GLOBAL_ENC_MODE_ADDR);
     VoC_jal(Coolsand_If1_table_selet);
       VoC_pop16(RA,DEFAULT);
	   
    VoC_bnez16_d(prm2bits_amr_not_IF1_format,GLOBAL_IF1_flag_ADDR)

	VoC_add16_rd(REG3,REG0,CONST_15_ADDR);	//REG3:&bits_short[i+15]
	VoC_loop_i(1,16)
		VoC_lw32z(REG45,DEFAULT);
		...
		VoC_endloop0
			VoC_add16_rd(REG3,REG3,CONST_32_ADDR);
			VoC_sw16inc_p(REG4,REG1,DEFAULT);
	VoC_endloop1
	
 
    VoC_jump(prm2bits_amr_efr_end);
 prm2bits_amr_not_IF1_format:

  //  VoC_jal(Coolsand_amr2If1format);
//input: REG0  source bit serial
//       REG7   mode
//       REG3 table
//       REG6   Bit numbers
     VoC_lw16i_set_inc(REG3, BIT_SERIAL_TRANS_ADDR,1);
 /*
	  // bit reverse!
    	for(i = 0; i < bits_no[mode]; i++)
		{
		j = amr_IF1[mode][i];
		destinbit[i] = sorcerbit[j];
		}		
*/
	  VoC_add16inc_rp(REG5,REG0,REG2,DEFAULT);
      VoC_loop_r(0,REG6)
      ...
      VoC_lw32z(REG45,DEFAULT);
	   ...
	   VoC_startloop0
         VoC_sw16inc_p(REG4,REG3,DEFAULT)   //clear the bit buffer,
       VoC_endloop0

     VoC_lw16i(REG6,Frame_header_ADDR);
	 VoC_add16_rr(REG6,REG6,REG7,DEFAULT);
     VoC_lw16i_set_inc(REG0, BIT_SERIAL_TRANS_ADDR,1);
 	...
 	VoC_loop_i(1,16)
		...
		VoC_startloop0
		    VoC_shr16_ri(REG4,-1,DEFAULT);
			VoC_lw16inc_p(REG7,REG0,IN_PARALLEL);	//reg6: bits_short
			VoC_and16_ri(REG7,1);
		...
		VoC_startloop0
	        VoC_shr16_ri(REG6,-1,DEFAULT);
			VoC_lw16inc_p(REG3,REG0,IN_PARALLEL);	//reg6: bits_short
			VoC_and16_ri(REG3,1);
			VoC_or16_rr(REG6,REG3,DEFAULT);
		VoC_endloop0
      	VoC_lw32z(REG45,DEFAULT);
	VoC_endloop1

prm2bits_amr_efr_end:
...
//input : REG7 mode
//output: REG2 table  REG6   Bit numbers
void Coolsand_If1_table_selet(void)
{
	VoC_lw16i_short(REG3,1,DEFAULT);

	VoC_lw16i(REG6,Bits_no_ADDR);
    VoC_add16_rr(REG6,REG6,REG7,DEFAULT);

    VoC_lw16i_set_inc(REG2,TABLE_MR475_ADDR,1);
    VoC_lw16i(RL6_HI,STATIC_CONST_bitno_MR475_ADDR);
    VoC_bez16_r(mode_selet_end,REG7)
    VoC_lw16i(REG2,TABLE_MR515_ADDR);
    VoC_lw16i(RL6_HI,STATIC_CONST_bitno_MR515_ADDR);
    
	VoC_be16_rr(mode_selet_end,REG7,REG3)

    VoC_lw16i(REG2,TABLE_MR59_ADDR);
	VoC_lw16i(RL6_HI,STATIC_CONST_bitno_MR59_ADDR);
    VoC_lw16i_short(REG3,2,DEFAULT);

	VoC_be16_rr(mode_selet_end,REG7,REG3)
    VoC_lw16i(REG2,TABLE_MR67_ADDR);
    VoC_lw16i(RL6_HI,STATIC_CONST_bitno_MR67_ADDR);
	VoC_be16_rd(mode_selet_end,REG7,CONST_3_ADDR)
    VoC_lw16i(REG2,TABLE_MR74_ADDR);
    VoC_lw16i(RL6_HI,STATIC_CONST_bitno_MR74_ADDR);
	VoC_be16_rd(mode_selet_end,REG7,CONST_4_ADDR)
    VoC_lw16i(REG2,TABLE_MR795_ADDR);
    VoC_lw16i(RL6_HI,STATIC_CONST_bitno_MR795_ADDR);
    VoC_be16_rd(mode_selet_end,REG7,CONST_5_ADDR)
    VoC_lw16i(REG2,TABLE_MR102_ADDR);
    VoC_lw16i(RL6_HI,STATIC_CONST_bitno_MR102_ADDR);
    VoC_be16_rd(mode_selet_end,REG7,CONST_6_ADDR)
    VoC_lw16i(REG2,TABLE_MR122_ADDR);
    VoC_lw16i(RL6_HI,STATIC_CONST_BITNO_ADDR);
    VoC_be16_rd(mode_selet_end,REG7,CONST_7_ADDR)
    VoC_lw16i(RL6_HI,STATIC_CONST_bitno_MRDTX_ADDR);
mode_selet_end:
   VoC_lw16_p(REG6,REG6,DEFAULT);
   VoC_return;
}
