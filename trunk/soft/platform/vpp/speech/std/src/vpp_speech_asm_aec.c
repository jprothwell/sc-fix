...
//add new
	...
	voc_short r_PF_SpkPowerBuf[SUBBANDFILTER_NUM],x			// Spk Power Buf for PF


	voc_short r_PF_CCErrSpkVecBufI[SUBBANDFILTER_NUM*FILTER_LEN],x		// Cross Corr between Err and Spk (PF) Vec
	voc_short r_PF_CCErrSpkVecBufQ[SUBBANDFILTER_NUM*FILTER_LEN],x		// Cross Corr between Err and Spk (PF) Vec
...
Coolsand_AEC_L0:		
		...
		VoC_lw16_d(REG0,DEC_OUTPUT_ADDR_BAK_ADDR);
...
//		VoC_lw32z(RL6,DEFAULT);
  ...
  //     	VoC_sw32_d(RL6,CONST_32bit0_addr);		
   //     VoC_sw16_d(REG0,CONST_1_addr); 
		...
		VoC_sw16_d(ACC0_LO,r_PF_CL_MicNoiseFloorDevCnt);
		VoC_sw16_d(ACC0_LO,r_PF_CL_SpkNoiseFloorDevCnt);
		...
		VoC_lw16i_set_inc(REG0,r_PF_SpkPowerBuf,1);
