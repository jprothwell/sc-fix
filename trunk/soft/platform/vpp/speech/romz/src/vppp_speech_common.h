...
#if (defined(MDF_TEST) && defined(SDF_TEST))
 #undef MDF_TEST // both won't run simultaneously
#endif
...
#if ((defined(MSDF_TEST) || defined(RAMP_TEST) || defined(ECHO_TEST)) && defined(BINARY_SIM))
 #undef BINARY_SIM
#endif


#define MAX_SPECCH_FRAME_SIZE                   18

/// Structure used for vocoder AudioCfg
typedef struct
{
    UINT16 echoOn;
    UINT16 echoExpMu;
...
/// Structure containing the code initial configuration

typedef struct
{
	int codeState[2];    // code reload state
	int codePtrs[21];    /// code pointers
	short reset;
	short irqGen;
} VPP_SPEECH_INIT_T;
...
typedef struct
{
    UINT16 dtxOn;								/// DTX allowed by the network, active->1
    UINT16 codecMode;								/// mode of the next encoder speech frame output
    UINT16 decFrameType;						/// Type of the coded input frame (AMR only)
    UINT16 bfi;									/// BFI flag (all vocoders except AMR)
    UINT16 sid;									/// SID flag (all vocoders except AMR)
    UINT16 taf;									/// TAF flag (all vocoders except AMR)
    UINT16 ufi;									/// UFI flag (HR only)
    UINT16 reserved;
    UINT16 decInBuf[MAX_SPECCH_FRAME_SIZE];     /// Decoder input buffer (coded frame in dec_mode)
}VPP_DEC_IN_T;
...
typedef struct
{
    UINT16 encMode;								/// Mode of the voice coded output frame
	UINT16 encFrameType;						/// Type of the coded output frame (AMR only)
	UINT16 vad;									/// Voice Activity Detection flag (all except AMR)
	UINT16 sp;									/// Speech flag (all except AMR)
	UINT16 encOutBuf[MAX_SPECCH_FRAME_SIZE];	/// Encoder output buffer (coded frame in enc_mode)
...
typedef enum
{
	...
	VPP_SPEECH_WAKEUP_NO = 0,      	/// No wakeup enabled
	VPP_SPEECH_WAKEUP_SW_ENC,      	/// Record on software event (XCPU)
	VPP_SPEECH_WAKEUP_SW_DEC,      	/// Play on software event (XCPU)
	VPP_SPEECH_WAKEUP_SW_ENCDEC,   	/// Speech on software event (XCPU)
	VPP_SPEECH_WAKEUP_HW_ENC,      	/// Record on hardware event (IFC)
	VPP_SPEECH_WAKEUP_HW_DEC,      	/// Play on hardware event (IFC)
	VPP_SPEECH_WAKEUP_HW_ENCDEC     /// Record on hardware event (IFC)
...
typedef enum
{
	...
	VPP_SPEECH_IRQGEN_ENC,
	VPP_SPEECH_IRQGEN_DEC,
	VPP_SPEECH_IRQGEN_DECENC
} VPP_SPEECH_IRQGEN_T;
...
#define AMR_EFR_RESET_Offset_X                            	(144-56)
