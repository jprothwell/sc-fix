...
PITCH_OL_795_01:
	...
	VoC_lw16i_short(REG5,0x10,DEFAULT);	//1048576
...
//	VoC_lw16i_short(REG4,40,DEFAULT);
...
//	VoC_lw32_d(REG23,STATIC_CONST_mode_dep_parm_MR795_ADDR+4);
    VoC_lw16i_short(REG2,3,DEFAULT);
...
//	VoC_lw32_d(REG23,STATIC_CONST_mode_dep_parm_MR795_ADDR+6);
...
//	VoC_lw16_d(REG4,STATIC_CONST_mode_dep_parm_MR795_ADDR);	// REG4 for max_frac_lag

	VoC_bnez16_r(AMR_PITCH_FR105,REG5)
	VoC_bgt16_rr(AMR_PITCH_FR106,REG1,REG4)	    // if ((delta_search == 0) && (sub (lag, max_frac_lag) > 0))
AMR_PITCH_FR105:
	VoC_lw16_sd(REG2,0,DEFAULT);
//	VoC_lw32_d(REG45,STATIC_CONST_mode_dep_parm_MR795_ADDR+2);
...
//	VoC_jal(CII_Enc_lag3_795);
	...
	VoC_bnez16_d(ENC_LAG3_100,Pitch_delta_search_addr)
   	VoC_lw16i(REG2,112);
	VoC_add16_rr(REG2,REG1,REG2,DEFAULT);  	//index = add (T0, 112);
    	VoC_lw16i(REG6,85);
	VoC_bgt16_rr(ENC_LAG3_102,REG1,REG6)		//   if (sub (T0, 85) <= 0)
//	VoC_bgt16_rd(ENC_LAG3_102,REG1,CONST_0x55_ADDR)		//   if (sub (T0, 85) <= 0)
...
//	VoC_jal(CII_build_code_MR74_795);
   ...
   VoC_lw16i_short(INC0,2,IN_PARALLEL);
	...
	VoC_lw32z(ACC0,IN_PARALLEL);
	VoC_loop_i(1,4);
//  		VoC_jal(CII_cbseach_cala_track_795);
...
MR475_gain_quant_120:
	...
	VoC_loop_i_short(5, DEFAULT);
	VoC_lw16_sd(REG3, 11, IN_PARALLEL);        //  gainQuant_frac_coeff_ADDRESS
	VoC_startloop0
...
//	 begin of CII_gain_adapt		//	REG7     alpha
	...
	VoC_lw16i_short(REG1,1,IN_PARALLEL);
	VoC_lw16i(REG5, 2721);
	VoC_bngt16_rr(gain_adapt_100, REG7, REG5)         //  #define LTP_GAIN_THR2 5443 /* 5443 Q13 = 0.6644 ~= 2.0 / (10*log10(2)) */
	VoC_lw16i_short(REG6, 1, DEFAULT);
	VoC_lw16i(REG4,5443);
	VoC_bngt16_rr(gain_adapt_100, REG7, REG4)
...
gain_adapt_100:
    ...
    VoC_lw16i(REG5,200);
	VoC_bngt16_rr(gain_adapt_200, REG0,REG5)
...
gain_adapt_300:
	...
	VoC_pop16(REG6, DEFAULT);
	VoC_lw16i_short(REG7, 0,IN_PARALLEL); //REG7   result

	VoC_bnez16_r(gain_adapt_400, REG6)
    VoC_lw16i(REG6, 5443);
	VoC_bgt16_rr(gain_adapt_400, REG1, REG6)
...
//	VoC_bgt16_rd(gain_adapt_400, REG1, CONST_5443_ADDR)
	...
	VoC_multf16_rr(REG1, REG1, REG6,DEFAULT);
	VoC_NOP();
...
MR795_gain_quant_L10:
	...
	VoC_sub16_dr(REG5, CONST_fu10_ADDR, REG5);
...
//  begin of CII_MR795_gain_code_quant_mod
	...
	VoC_add32_rd(REG23,REG23,CONST_0x00020002_ADDR);
	VoC_movreg16(REG4,RL7_HI, DEFAULT); 	//	REG4   	alpha
	...
	VoC_sub16_dr(REG7, CONST_0x7FFF_ADDR, REG4);
	...
	VoC_add16_rd(REG5, REG1, CONST_31_ADDR);  	//	REG5  e_max
...
//	VoC_movreg16(REG0,RL7_HI,IN_PARALLEL);
	...
	VoC_sub16_rd(REG5, REG5, CONST_31_ADDR);
