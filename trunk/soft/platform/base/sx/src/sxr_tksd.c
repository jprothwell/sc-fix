...
--------------------------------------------------------------------------------
  ...
  Scope      : Systeme executif real time tasks scheduler.

  History    :
--------------------------------------------------------------------------------
  Jun 13 2003   |  ADA  | Creation
  Dec 05 2004   |  ADA  | Leave an dw free in bottom of task stack.
  Nov 10 2005   |  ADA  | Check Task at deletion (_SXR level 3).
================================================================================
*/
...
void sxr_StartTask (u8 Id, void *DataCtx)
{
    ...
    {
        SXS_RAISE ((_SXR|TABORT|TDB|TNB_ARG(2),TSTR("Task Id unconsistency %i/%i\n",0x06ab0001), sxr_Task.Ctx [Id].Id, Id));
    }

    if (!(sxr_Task.Ctx [Id].State & (SXR_ALLOCATED_TSK | SXR_STOPPED_TSK)))
    {
        SXS_RAISE ((_SXR|TABORT|TDB|TNB_ARG(2),TSTR("Bad state for to start a task 0x%x (%i)\n",0x06ab0002), sxr_Task.Ctx [Id].State, Id));
    }
...
void gcj_page_spy_task_stack(unsigned char Id)
{
	gcj_pageSpay0((u8 *)sxr_Task.Ctx [Id].StackTop,(u8 *)sxr_Task.Ctx [Id].StackTop+((SXR_SET_STACK(sxr_Task.Ctx [Id].Desc -> StackSize)) << 2));
}
// =============================================================================
// sxr_StopTask
// -----------------------------------------------------------------------------
    ...
    if (sxr_Task.Ctx [Id].State == SXR_PENDING_TSK)
    {
        ...
        if (NextIndex != Id)
        {
            if (Id == sxr_Task.Active && sxr_Task.ScheduleDisable == 1)
            {
                ...
                {
                    SXS_RAISE ((_SXR|TABORT|TNB_ARG(1), "No pending task except active task %i", Id));
                }
