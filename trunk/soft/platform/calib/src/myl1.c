...
{
	enum {NO_STATE, FCH_SEARCH_STATE, FCH_READ_STATE, SCH_STATE};
	PRIVATE UINT8 SynchState = NO_STATE;
	...
	switch (SynchState)
	{
		case NO_STATE:
			...
			pal_FrequencyTuning(TabfOf[SearchIdx++],
												g_calibStubGlobalCtxPtr->arfcn[0]);
			...
			FrameCount = 0;
			FchFound = FALSE;
			SynchState = FCH_SEARCH_STATE;
			break;

		case FCH_SEARCH_STATE:

			FrameCount++;
			SynchState = FCH_READ_STATE;
			break;
