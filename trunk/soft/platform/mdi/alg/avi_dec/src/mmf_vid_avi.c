...
INT32 avi_fs_seek(avi_struct *avi, INT32 iOffset, UINT8 iOrigin)
{
	...
	if(avi->avi_fs_mode==0)
	{
		...
		res=ftell(avi->fdes);
#else
		res=FS_Seek((INT32)avi->fdes, iOffset, iOrigin);
#endif
	}
	else
	{
...
INT32 avi_fs_getFileSize(avi_struct *avi)
{
	...
	if(avi->avi_fs_mode==0)
	{
		...
		res=ftell(avi->fdes);
#else
		res=FS_GetFileSize((INT32)avi->fdes);
#endif
	}
	else
	{
...
uint32 avi_extend_to_cluster_size(FS_HANDLE FileHandle, const WCHAR *FileName)
{
   ...
   FS_GetFileSize(FileHandle, &fileLength);   
   ...
   FS_Seek(FileHandle, (int)0, FS_SEEK_END);      
...
int32 avi_write_idx_header(avi_struct *avi)
{
	...
	FSErrorCode = FS_GetFileSize(avi->idx_fdes, &idx_length);	
	  
	if ( (idx_length > 0) && (FSErrorCode>=0) )  //
	{
   ...
   while(1)
   {
      ...
      else if(strncasecmp(data,avi->audio_tag,4) == 0)
      {
         *len = n;
         ...
         if(avi_seek(avi->fdes,n,FS_SEEK_CUR)<0)  return 0;
   }
}
#endif
...
void main(void)
{
	...
	pIn=fopen("F:\\测试文件\\media_test_case\\AVI\\康熙来了.avi","rb");
	pOut=fopen("F:\\测试文件\\test.m4v","wb");
	pBuf=malloc(40*1024);
	...
	while(1)
	{
		...
		res=avi_read_frame(avi_p, pBuf, 40*1024,  &key_frame);
