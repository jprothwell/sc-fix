...
int init_tab_map ()//add shenh
{
	...
	/*
	if(sizeof(int)*(MBR + 1)+sizeof(int)*(MBR + 1)*(MBC + 1)>mmc_MemGetFree()-32)
	{
	  	return -1;
	}
	coded_map=mmc_MemMalloc(sizeof(int)*(MBR + 1)+sizeof(int)*(MBR + 1)*(MBC + 1));
	if(!coded_map )
		return -1;
	...
	*/
	...
	quant_map=mmc_MemMalloc(sizeof(int)*(MBR + 1)+sizeof(int)*(MBR + 1)*(MBC + 1));
	if( !quant_map)
		return -1;
	...
	*/
...
#if 0
	if((sizeof(int)*(2+2*6+2*6*(MBR + 1))+sizeof(short)*2*6*(MBR + 1)*(MBC + 2))>mmc_MemGetFree()-32)
	{
	  	return -1;
	}
	MV=mmc_MemMalloc(sizeof(int)*(2+2*6+2*6*(MBR + 1))+sizeof(short)*2*6*(MBR + 1)*(MBC + 2));
	if(!MV )
		return -1;
...
#else
	if((sizeof(short)*2*6*(MBR + 1)*(MBC + 2))>mmc_MemGetFree()-32)
	{
	  	return -1;
	}
	MV=(short*)mmc_MemMalloc(sizeof(short)*2*6*(MBR + 1)*(MBC + 2));
	if(!MV )
		return -1;
	...
	/*
	if((sizeof(int)*(2+2*5+2*5*(MBR + 1))+sizeof(short)*2*5*(MBR + 1)*(MBC + 2))>mmc_MemGetFree()-32)
	{
	  	return -1;
	}
	true_B_direct_mode_MV=mmc_MemMalloc(sizeof(int)*(2+2*5+2*5*(MBR + 1))+sizeof(short)*2*5*(MBR + 1)*(MBC + 2));
	...
	*/
...
#if 0
	if((sizeof(int)*(MBR + 1)+sizeof(short)*(MBR + 1)*(MBC + 2))>mmc_MemGetFree()-32)
	{
	  	return -1;
	}
	modemap=mmc_MemMalloc(sizeof(int)*(MBR + 1)+sizeof(short)*(MBR + 1)*(MBC + 2));
	if(!modemap )
		return -1;
...
#else
	if(sizeof(short)*(MBR + 1)*(MBC + 2)>mmc_MemGetFree()-32)
	{
	  	return -1;
	}
	modemap=(short*)mmc_MemMalloc(sizeof(short)*(MBR + 1)*(MBC + 2));
	if(!modemap )
		return -1;
	...
	anchorframemodemap=mmc_MemMalloc(sizeof(int)*(MBR + 1)+sizeof(short)*(MBR + 1)*(MBC + 2));
	...
	*/
	...
	predictionmap=mmc_MemMalloc(sizeof(int)*(MBR + 1)+sizeof(short)*(MBR + 1)*(MBC + 2));
	if(!predictionmap )
		return -1;
...
void getpicture (unsigned char *outbuf, unsigned short LcdBufWidth, unsigned short StartX, unsigned short StartY, int gob, zoomPar *pZoom)
{
	...
	switch (pict_type)
	{
		...
		//enhance_pict = NO; change shenh
			 ...
			 if (0 == UFEP /*&& plus_type*/) 
			{
				...
				{
					if (!quiet)
						diag_printf ("error: advanced_intra_coding is not supported for mmc_MemMalloc. shenh\n");
						//exit (-1);
					errorstate=-1;
					return;
				}
	...
	case PCT_B:
		...
		#if 0//change shenh. not support for mmc_MemMalloc
...
extern void h263_simple_idct_put(uint8 *dest, int line_size, int16 *block);
extern void h263_simple_idct_add(uint8 *dest, int line_size, int16 *block);
...
reconstruct_mb:
		...
		if (MBA > 0 /*&& !dont_reconstruct_next_mb*/)
		{
			...
			if ((ModeBit&0x18)!=0)
			{
				...
				h263_simple_idct_put(dest, coded_picture_width, ld->block);
				h263_simple_idct_put(dest+8, coded_picture_width, ld->block+64);
				h263_simple_idct_put(dest+(coded_picture_width<<3), coded_picture_width, ld->block+128);
				h263_simple_idct_put(dest+(coded_picture_width<<3)+8, coded_picture_width, ld->block+192);
				pos= chrom_width * (by>>1) + (bx>>1);
				h263_simple_idct_put(current_frame[1]+pos, chrom_width, ld->block+256);
				h263_simple_idct_put(current_frame[2]+pos, chrom_width, ld->block+320);
			...
			else
			{
				...
				dest=current_frame[0] + coded_picture_width * by + bx;
				if (pCBP & 0x20)
					h263_simple_idct_add(dest, coded_picture_width, ld->block);
				if (pCBP & 0x10)
					h263_simple_idct_add(dest+8, coded_picture_width, ld->block+64);
				if (pCBP & 0x08)
					h263_simple_idct_add(dest+(coded_picture_width<<3), coded_picture_width, ld->block+128);
				if (pCBP & 0x04)
					h263_simple_idct_add(dest+(coded_picture_width<<3)+8, coded_picture_width, ld->block+192);
				pos= chrom_width * (by>>1) + (bx>>1);
				if (pCBP & 0x02)
					h263_simple_idct_add(current_frame[1]+pos, chrom_width, ld->block+256);
				if (pCBP & 0x01)
					h263_simple_idct_add(current_frame[2]+pos, chrom_width, ld->block+320);
			}
		...
		*/
		...
		if(fillAudioData && MBA>=fillAudMbScale)//read audio when image big than qcif
		{
			fillAudioData();
...
static void init_enhancement_layer (int layer)
{
  ...
  for (cc = 0; cc < 3; cc++)
  {
    ...
    if (!(prev_enhancement_frame[layer][cc] = (unsigned char *) mmc_MemMalloc (size)))
      error ("mmc_MemMalloc failed\n");
    
    if (!(current_enhancement_frame[layer][cc] = (unsigned char *) mmc_MemMalloc (size)))
      error ("mmc_MemMalloc failed\n");

    if (!(tmp_enhance_fwd[layer][cc] = (unsigned char *) mmc_MemMalloc (size)))
      error ("mmc_MemMalloc failed\n");

    if (!(tmp_enhance_up[layer][cc] = (unsigned char *) mmc_MemMalloc (size)))
      error ("mmc_MemMalloc failed\n");
  ...
  for (cc = 0; cc < 3; cc++)
  {
    if (cc == 0)
    {
      ...
      if (!(enhance_edgeframeorig[layer][cc] = (unsigned char *) mmc_MemMalloc (size)))
        error ("mmc_MemMalloc failed\n");
...
void UpsampleReferenceLayerPicture(void)
{
  ...
  for (cc = 0; cc < 3; cc++)
  {
    ...
    if (!(upsampled_reference_frame[cc] = (unsigned char *) mmc_MemMalloc (size)))
      error ("mmc_MemMalloc failed\n");
...
void initdecoder ()
{
...
#else
	if(1024>mmc_MemGetFree()-32)
	{
		errorstate=-1;
	  	return ;
	}
	if (!(clp = (unsigned char *) mmc_MemMalloc (1024)))
	{
		//error ("mmc_MemMalloc failed\n");
		errorstate=-1;
		return;
	...
	*/
	...
	size = coded_picture_width * coded_picture_height;
	if((size*3/2)>mmc_MemGetFree()-32)
	{
		errorstate=-1;
		return ;
	}
	if (!(next_I_P_frame[0] = (unsigned char *) mmc_MemMalloc (size*3/2)))//next yuv frame will putin. shenh
	{
		//error ("mmc_MemMalloc failed\n");
		errorstate=-1;
		return;
	...
	if (!(current_frame[0] = (unsigned char *) mmc_MemMalloc (size*3/2)))//switch address with next_I_P_frame for output yuv.shenh
	{
		//error ("mmc_MemMalloc failed\n");
		errorstate=-1;
		return;
  	...
  	for (cc = 0; cc < 3; cc++)
	{
		...
		/* Used for bidirectional and direct prediction mode for true B
		* pictures, one for forward and one for backward. */
		if(size>mmc_MemGetFree()-32)
		{
			errorstate=-1;
		  	return ;
		}
		if (!(next_I_P_frame[cc] = (unsigned char *) mmc_MemMalloc (size)))//next yuv frame will putin. shenh
		{
			//error ("mmc_MemMalloc failed\n");
			errorstate=-1;
			return;
		...
		/* change shenh
		if (!(prev_I_P_frame[cc] = (unsigned char *) mmc_MemMalloc (size)))
		{
			//error ("mmc_MemMalloc failed\n");
			errorstate=-1;
			return;
		...
		if (!(current_frame[cc] = (unsigned char *) mmc_MemMalloc (size)))//switch address with next_I_P_frame for output yuv.shenh
		{
			//error ("mmc_MemMalloc failed\n");
			errorstate=-1;
			return;
		...
		/* change shenh
		if (!(bframe[cc] = (unsigned char *) mmc_MemMalloc (size)))
		{
			//error ("mmc_MemMalloc failed\n");
			errorstate=-1;
			return;
		...
		*/
		...
		/* change shenh
		if (!(tmp_f[cc] = (unsigned char *) mmc_MemMalloc (size)))
		{
			//error ("mmc_MemMalloc failed\n");
			errorstate=-1;
			return;
		...
		if (!(tmp_b[cc] = (unsigned char *) mmc_MemMalloc (size)))
		{
			//error ("mmc_MemMalloc failed\n");
			errorstate=-1;
			return;
		...
		*/
		...
		/*change shenh
		if (!(buffered_frame[cc] = (unsigned char *) mmc_MemMalloc (size)))
		{
			//error ("mmc_MemMalloc failed\n");
			errorstate=-1;
			return;
  ...
  for (cc = 0; cc < 3; cc++)
  {
	if (cc == 0)
	{
		...
		/* Stores framed version of previous inter-picture, luminance. */
		if (!(edgeframeorig[cc] = (unsigned char *) mmc_MemMalloc (size)))
		{
			//error ("mmc_MemMalloc failed\n");
			errorstate=-1;
			return;
		...
		* prediction. Stores future inter-picture, luminance */
		/* change shenh
		if (!(nextedgeframeorig[cc] = (unsigned char *) mmc_MemMalloc (size)))
		{
			//error ("mmc_MemMalloc failed\n");
			errorstate=-1;
			return;
  ...
  if (expand)
  {
    for (cc = 0; cc < 3; cc++)
    {
      ...
      if (!(exnewframe[cc] = (unsigned char *) mmc_MemMalloc (size)))
        error ("mmc_MemMalloc failed\n");
    }
  }*/
