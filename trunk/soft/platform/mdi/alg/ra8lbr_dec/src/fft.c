...
static void R8FirstPass(int *x, int bg)
{
	...
	for (; bg != 0; bg--) {
		...
		MULSHIFT32hx(SQRT1_2, ar - ai, xr);
		MULSHIFT32hx(SQRT1_2, ar + ai, xi);
		MULSHIFT32hx(SQRT1_2, cr - ci, zr);
		//MULSHIFT32hx(SQRT1_2, cr + ci, zi);
	...
	for (; bg != 0; gp <<= 2, bg >>= 2) {
		...
		for (i = bg; i != 0; i--) {
			...
			for (j = gp; j != 0; j--) {
				...
				bi = xptr[1];
				wd = ws + 2*wi;
				MULSHIFT32hx(wi, br + bi,tr);
				MULSHIFT32hx(wd, br,br);
				br = br - tr;
				MULSHIFT32hx(ws, bi, bi);
				bi =  bi+ tr;
				xptr += step;
				...
				ci = xptr[1];
				wd = ws + 2*wi;
				MULSHIFT32hx(wi, cr + ci,tr);
				MULSHIFT32hx(wd, cr, cr);
				cr =  cr - tr;
				MULSHIFT32hx(ws, ci, ci);
				ci =  ci + tr;
				xptr += step;
				...
				di = xptr[1];
				wd = ws + 2*wi;
				MULSHIFT32hx(wi, dr + di,tr);
				MULSHIFT32hx(wd, dr, dr);
				dr =  dr - tr;
				MULSHIFT32hx(ws, di, di);
				di =  di + tr;
				wptr += 6;
