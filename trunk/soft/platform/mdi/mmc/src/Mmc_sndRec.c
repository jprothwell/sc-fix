//chenhe create for greenstone sound recorder 20080807
//coolsand tech.
...
ARS_REC_MODE_T mmc_amrcodec= 0xff;
...
INT32 Mmc_MicStartRecord(UINT32 *pBuffer, UINT32 nBufferSize,ARS_REC_MODE_T codec,BOOL loop_mode,ARS_USER_HANDLER_T recHanlder)
{
	...
	if(pBuffer == NULL)
		return FALSE;
	if((nBufferSize == 0) || (codec > ARS_REC_MODE_AMR_RING))
	{
		return FALSE;
	}
	mmc_amrcodec = codec;
...
#else	
    ret_val = (ars_Record(AUD_ITF_RECEIVER, &stream, &arsAudioCfg, loop_mode) == ARS_ERR_NO);
#endif
#endif
	Delay_for_audio(1);
...
INT32 Mmc_FmStartRecord(UINT32 *pBuffer, UINT32 nBufferSize,ARS_REC_MODE_T codec,BOOL loop_mode,ARS_USER_HANDLER_T recHanlder)
{
    ...
    arsAudioCfg.spkLevel = AUD_SPK_VOL_6;
    arsAudioCfg.filter = NULL;
    ...
    ars_err_temp=(int32)ars_Record(itf, &stream, &arsAudioCfg, loop_mode);
   ...
   ret_val = (ars_err_temp == ARS_ERR_NO);
	...
	if(ret_val)
		...
		{
		MCI_TRACE (MCI_AUDIO_TRC|TSTDOUT,0,"----Mmc_FmStartRecord is started: failure and error is %d---!",ars_err_temp);
		return -1;
...
INT32 Mmc_sndRecStart(HANDLE fhd,U8 quality, mci_type_enum format)
{
	INT32 result;
	U8 snd_quality;
//	UINT8 waveHeader[WAVE_FILE_HEADER_SIZE]={0};
	recHandle = fhd;
	snd_quality = quality;
...
MCI_TRACE (MCI_AUDIO_TRC,0,"sndRec_msgHandle!");
     if(g_RecordPlayOpen==1)
     	{
       		AMRRing_frame = (sndRecBufferLength*2)/Voc_frame_bytes[mmc_amrcodec];
		    MCI_TRACE (MCI_AUDIO_TRC,0,"sndRec_msgHandle,part is %d,mmcRecFormat is %d",part, mmcRecFormat);
			
		    MCI_TRACE (MCI_AUDIO_TRC,0,"######sndRecBufferLength = %d",sndRecBufferLength);
		    int32 result = -1;
		    if(part == SNDREC_DATA_PART_HALF)
		    {
		        switch(mmcRecFormat)
		        {
		            case MCI_TYPE_AMR:
				{
					WriteFramebuffLen = AMRRing_frame*(frame_bytes[mmc_amrcodec]+1);
				Amr2AmrRing(mmc_amrcodec,(UINT8 *)sndRecBuffer,(UINT8 *)sndRecRingBuffer,AMRRing_frame);
		    ...
		    else
		    {
				...
				{
					WriteFramebuffLen = AMRRing_frame*(frame_bytes[mmc_amrcodec]+1);
				Amr2AmrRing(mmc_amrcodec,(UINT8 *)(sndRecBuffer+sndRecBufferLength/2),(UINT8 *)sndRecRingBuffer,AMRRing_frame);
				...
				{
		       	  WriteFramebuffLen = sndRecBufferLength*2;
		                memcpy(sndRecRingBuffer,(UINT8 *)(sndRecBuffer+sndRecBufferLength/2),sndRecBufferLength*2);
		                int32 timebegin=hal_TimGetUpTime();
		                result=FS_Write(recHandle, (UINT8 *)sndRecRingBuffer, sndRecBufferLength*2);
		                int32 timeend=hal_TimGetUpTime();
