    ...
    BOOL loop)
{
   ...
   case MCI_PLAY_MODE_STREAM_PCM:
    ...
    case MCI_PLAY_MODE_WAV:
    {
        ...
        if (WAVHeaderInfo.dwFmtSize==18)
        {
            memcpy((UINT8 *)OptionalInfo, WAVbufferStart, sizeof(WAVHeader));
            WAVbufferStart+=2;
        ...
        MCI_TRACE(MCI_AUDIO_TRC,0,"[APBS] wBitsPerSample: %d\n", WAVHeaderInfo.wBitsPerSample);
        memset(g_apbsPcmBuf,0,MP3PCMBUFSIZE);//set max

        if (WAVHeaderInfo.wBitsPerSample == 8)
        {
            g_apbsFrameSize = WAVPCMBUFSIZE/4;
            g_apbsBitsPerSample= 8;
        }
...
PUBLIC VOID apbs_UserMsgHandle(COS_EVENT *pnMsg)
{
    ...
    {
    case  APBS_STOP:
        MCI_TRACE(MCI_ANALOGTV_TRC|TSTDOUT, 0,"[[APBS]]STOP");
    ...
    case APBS_NO_MORE_DATA:
        MCI_TRACE(MCI_ANALOGTV_TRC|TSTDOUT, 0,"[[APBS]]APBS_NO_MORE_DATA");
