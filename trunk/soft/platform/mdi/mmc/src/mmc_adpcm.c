...
static const uint8 AdaptCoeff1[] = {
        64, 128, 0, 48, 60, 115, 98
};

static const int8 AdaptCoeff2[] = {
        0, -64, 0, 16, 0, -52, -58
};
 ...
 */
static  int16 av_clip_int16(int a)
{
...
int adpcm_decode_init(WAV_INPUT * input )
{
    ...
    ADPCMContext *c = input->priv_data;
    int max_channels = 2;

    switch(input->codec_id) {
   ...
   default:
    ...
    case CODEC_ID_ADPCM_IMA_WS:
        if (input->extradata && input->extradata_size == 2 * 4) {
            c->status[0].predictor = AV_RL32(input->extradata);
            c->status[1].predictor = AV_RL32(input->extradata + 4);
        }
        break;
    default:
        break;
...
int16 bytestream_get_le16(const uint8 **src)
{
 ...
 int16 bytestream_get_be16(const uint8 **src)
{
...
int32 bytestream_get_le32(const uint8 **src)
{
...
int32 bytestream_get_be32(const uint8 **src)
{
...
int adpcm_decode_frame(WAV_INPUT * avctx,/*AVCodecContext *avctx,*/
                            ...
                            const uint8 *buf, int buf_size)
{
    ...
    int32 previous_left_sample, previous_right_sample;
    int32 current_left_sample, current_right_sample;
    int32 next_left_sample, next_right_sample;
    int32 coeff1l, coeff2l, coeff1r, coeff2r;
    uint8 shift_left, shift_right;
    int count1, count2;
    ...
    *data_size= 0;
    ...
    case CODEC_ID_ADPCM_IMA_QT:
        ...
        for (channel = 0; channel < avctx->channels; channel++) {
            ...
            if (cs->step_index > 88){
         	  MCI_TRACE (MCI_AUDIO_TRC,0, "ERROR: step_index = %i\n", cs->step_index);		
                cs->step_index = 88;
            }
...
//        samples_per_block= (block_align-4*chanels)*8 / (bits_per_sample * chanels) + 1;
        ...
        for(i=0; i<avctx->channels; i++){
            cs = &(c->status[i]);
            cs->predictor = *samples++ = (int16)bytestream_get_le16(&src);         
            cs->step_index = *src++;
            if (cs->step_index > 88){
 		  MCI_TRACE (MCI_AUDIO_TRC,0, "ERROR: step_index = %i\n", cs->step_index);	
                cs->step_index = 88;
            }
            if (*src++) MCI_TRACE (MCI_AUDIO_TRC,0, "unused byte should be null but is %d!!\n", src[-1]);	/* unused */
        }
    ...
    case CODEC_ID_ADPCM_4XM:
        cs = &(c->status[0]);
        c->status[0].predictor= (int16)bytestream_get_le16(&src);
        if(st){
            c->status[1].predictor= (int16)bytestream_get_le16(&src);
        }
        c->status[0].step_index= (int16)bytestream_get_le16(&src);
        if(st){
            c->status[1].step_index= (int16)bytestream_get_le16(&src);
        }
    ...
    case CODEC_ID_ADPCM_MS:
        ...
        c->status[0].idelta = (int16)bytestream_get_le16(&src);
        if (st){
            c->status[1].idelta = (int16)bytestream_get_le16(&src);
        }
    ...
    case CODEC_ID_ADPCM_IMA_DK3:
        ...
        c->status[0].predictor  = (int16)AV_RL16(src + 10);
        c->status[1].predictor  = (int16)AV_RL16(src + 12);
    ...
    case CODEC_ID_ADPCM_IMA_ISS:
        c->status[0].predictor  = (int16)AV_RL16(src + 0);
        c->status[0].step_index = src[2];
        src += 4;
        if(st) {
            c->status[1].predictor  = (int16)AV_RL16(src + 0);
    ...
    case CODEC_ID_ADPCM_EA:
        ...
        src += 4;
        current_left_sample   = (int16)bytestream_get_le16(&src);
        previous_left_sample  = (int16)bytestream_get_le16(&src);
        current_right_sample  = (int16)bytestream_get_le16(&src);
        previous_right_sample = (int16)bytestream_get_le16(&src);
        ...
        for (count1 = 0; count1 < samples_in_chunk/28;count1++) {
            ...
            for (count2 = 0; count2 < 28; count2++) {
                next_left_sample  = (int32)((*src & 0xF0) << 24) >> shift_left;
                next_right_sample = (int32)((*src & 0x0F) << 28) >> shift_right;
                src++;
    ...
    case CODEC_ID_ADPCM_EA_MAXIS_XA:
        ...
        for (count1 = 0; count1 < (buf_size - avctx->channels) / avctx->channels; count1++) {
            for(i = 4; i >= 0; i-=4) { /* Pairwise samples LL RR (st) or LL LL (mono) */
                for(channel = 0; channel < avctx->channels; channel++) {
                    int32 sample = (int32)(((*(src+channel) >> i) & 0x0F) << 0x1C) >> shift[channel];
                    sample = (sample +
    ...
    case CODEC_ID_ADPCM_EA_R3: {
        ...
        const int big_endian = avctx->codec_id == CODEC_ID_ADPCM_EA_R3;
        int32 previous_sample, current_sample, next_sample;
        int32 coeff1, coeff2;
        uint8 shift_tmp;
        unsigned int channel_tmp;
        uint16 *samplesC;
        const uint8 *srcC;
        const uint8 *src_end = buf + buf_size;
        ...
        for (channel_tmp=0; channel_tmp<avctx->channels; channel_tmp++) {
            int32 offset = (big_endian ? bytestream_get_be32(&src)
                                         : bytestream_get_le32(&src))
                           + (avctx->channels-channel_tmp-1) * 4;

            if ((offset < 0) || (offset >= src_end - src - 4)) break;
            srcC  = src + offset;
            samplesC = samples + channel_tmp;

            if (avctx->codec_id == CODEC_ID_ADPCM_EA_R1) {
                current_sample  = (int16)bytestream_get_le16(&srcC);
                previous_sample = (int16)bytestream_get_le16(&srcC);
            } else {
                current_sample  = c->status[channel_tmp].predictor;
                previous_sample = c->status[channel_tmp].prev_sample;
            }
            ...
            for (count1=0; count1<samples_in_chunk; count1++) {
                if (*srcC == 0xEE) {  /* only seen in R2 and R3 */
                    ...
                    current_sample  = (int16)bytestream_get_be16(&srcC);
                    previous_sample = (int16)bytestream_get_be16(&srcC);

                    for (count2=0; count2<28; count2++) {
                        *samplesC = (int16)bytestream_get_be16(&srcC);
                        samplesC += avctx->channels;
                    }
                } else {
                    coeff1 = ea_adpcm_table[ *srcC>>4     ];
                    coeff2 = ea_adpcm_table[(*srcC>>4) + 4];
                    shift_tmp = (*srcC++ & 0x0F) + 8;

                    if (srcC > src_end - 14) break;
                    for (count2=0; count2<28; count2++) {
                        if (count2 & 1)
                            next_sample = (int32)((*srcC++ & 0x0F) << 28) >> shift_tmp;
                        else
                            next_sample = (int32)((*srcC   & 0xF0) << 24) >> shift_tmp;
            ...
            if (avctx->codec_id != CODEC_ID_ADPCM_EA_R1) {
                c->status[channel_tmp].predictor   = current_sample;
                c->status[channel_tmp].prev_sample = previous_sample;
            }
        }
    ...
    case CODEC_ID_ADPCM_EA_XAS:
        ...
        for (channel=0; channel<avctx->channels; channel++) {
            int coeff_t[2][4], shift_t[4];
            short *s2, *s = &samples[channel];
            for (n=0; n<4; n++, s+=32*avctx->channels) {
                for (i=0; i<2; i++)
                    coeff_t[i][n] = ea_adpcm_table[(src[0]&0x0F)+4*i];
                shift_t[n] = (src[2]&0x0F) + 8;
            ...
            for (m=2; m<32; m+=2) {
                ...
                for (n=0; n<4; n++, src++, s+=32*avctx->channels) {
                    for (s2=s, i=0; i<8; i+=4, s2+=avctx->channels) {
                        int level = (int32)((*src & (0xF0>>i)) << (24+i)) >> shift_t[n];
                        int pred  = s2[-1*avctx->channels] * coeff_t[0][n]
                                  + s2[-2*avctx->channels] * coeff_t[1][n];
    ...
    case CODEC_ID_ADPCM_IMA_SMJPEG:
        ...
        if (avctx->codec_id == CODEC_ID_ADPCM_IMA_AMV)
            src+=4;
    ...
    case CODEC_ID_ADPCM_SBPRO_2:
        ...
        if (avctx->codec_id  == CODEC_ID_ADPCM_SBPRO_4) {
            while (src < buf + buf_size) {
        ...
        } else if (avctx->codec_id  == CODEC_ID_ADPCM_SBPRO_3) {
...
*/
    ...
    case CODEC_ID_ADPCM_THP:
    {
        ...
        if (buf_size < 80) {
  		   MCI_TRACE (MCI_AUDIO_TRC,0,"frame too small\n");
            return -1;
        }
        ...
        for (i = 0; i < 32; i++)
            table[0][i] = (int16)bytestream_get_be16(&src);

        /* Initialize the previous sample.  */
        for (i = 0; i < 4; i++)
            prev[0][i] = (int16)bytestream_get_be16(&src);

        if (samplecnt >= (samples_end - samples) /  (st + 1)) {
          	 MCI_TRACE (MCI_AUDIO_TRC,0,"allocated output buffer is too small\n");	
            return -1;
        }
