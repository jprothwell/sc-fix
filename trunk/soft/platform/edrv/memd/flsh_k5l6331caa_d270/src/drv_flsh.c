...
////////////////////////////////////////////////////////////////////////////////
//                                                                            //
//  $HeadURL: http://svn.coolsand-tech.com/svn/developing1/Sources/edrv/trunk/memd/flsh_k5l6331caa_d270/src/drv_flsh.c $ //
//	$Author: yangtt $                                                        // 
//	$Date: 2011-01-21 15:00:51 +0800 (Fri, 21 Jan 2011) $                     //   
//	$Revision: 5578 $                                                          //   
//                                                                            //
////////////////////////////////////////////////////////////////////////////////
//                                                                            //
...
PUBLIC  MEMD_ERR_T memd_FlashErase_First(UINT8 *startFlashAddress, UINT8 *endFlashAddress,UINT32 time)
{
    ...
    } else
    {
        ...
        if (phys_end_add != FLASH_SIZE)
        {
            ...
            {
                MEMD_ASSERT(0, "erase error 1,phys_end(0x%x) != start(0x%x).",phys_end_add,Start);
    ...
    while (Start != phys_end_add)
    {
        ...
        if (!g_memdLockOverride)
        {
            if (memd_LockCheck(BankAddr, (UINT16*)ptr))
            {
                ...
                MEMD_ASSERT(0, "erase error 2,lock check error.");
        ...
        *(ptr) = 0x30;
        ...
        while(1){
            ...
            if ((rdstatus & 0x20) == 0x20) {
                ...
                (*BankAddr) = 0xf0;
                hal_EbcFlashWriteEnable(FALSE);
                MEMD_ASSERT(0, "erase error 3,rdstatus(0x%x) error.",rdstatus);
...
PUBLIC  MEMD_ERR_T memd_FlashErase_Continue(UINT8 *startFlashAddress, UINT8 *endFlashAddress,UINT32 time)
{
    ...
    UINT32 suspend_time = 0,max_time = 0;
    #if 0
    VOLATILE UINT8 * start_test = NULL;
    UINT32 kk = 0;
    start_test = (VOLATILE UINT8 *)(g_memdFlashBaseAddress + (UINT32)startFlashAddress);
    #endif
    //UINT8 data_test = 0;
    //hal_HstSendEvent(0x998877);
    max_time = time MILLI_SECONDS;
    UINT32 now = hal_TimGetUpTime();
    phys_start_add = (UINT32)startFlashAddress;
    ...
    // erase done
    #if 0
    hal_HstSendEvent(0x345678);
    hal_HstSendEvent((UINT32)start_test);
    hal_HstSendEvent((UINT32)(*(start_test+0x16b)));
    for (kk = 0; kk<64*1024;kk++)
    {
        if((*start_test)!=0xff)
        {
           MEMD_ASSERT(0,"kk=0x%x,startFlashAddress= 0x%x",kk,startFlashAddress);
        // hal_HstSendEvent(0x44444445); hal_HstSendEvent(kk);hal_HstSendEvent((UINT32)start_test);
          //hal_HstSendEvent((UINT32)(*start_test));
          break;
        }
        start_test ++;
...
PUBLIC MEMD_ERR_T memd_FlashErase(UINT8 *startFlashAddress, UINT8 *endFlashAddress)
{
    ...
    while (Start != phys_end_add)
    {
        ...
        if (!g_memdLockOverride)
        {
            ...
            {
                hal_EbcFlashWriteEnable(FALSE);
                MEMD_ASSERT(0, "flash erase error 3.");
                return MEMD_ERR_PROTECT;
            }
        }
        ...
        *(ptr) = 0x30;
        ...
        while(1){
            ...
            if ((rdstatus & 0x20) == 0x20) {
                ...
                (*BankAddr) = 0xf0;
                hal_EbcFlashWriteEnable(FALSE);
                MEMD_ASSERT(0, "flash erase error 4,rdstatus = 0x%x.",rdstatus);
                return MEMD_ERR_ERASE;
            }
        }
        if (End != FLASH_SIZE)
...
UINT32 kk_count = 0;
PUBLIC MEMD_ERR_T memd_FlashWrite_3(UINT8 *flashAddress,
        UINT32 byteSize, 
        ...
        CONST UINT8* buffer)
{
    ...
    *(BankAddr) = 0xf0;
    ...
    if (!g_memdLockOverride)
    {
        ...
        {
            hal_EbcFlashWriteEnable(FALSE);
            MEMD_ASSERT(0, "write error 1,lock check error.");
            return MEMD_ERR_PROTECT;
        }
    }
    if ((phys_add&1) == 1)
    ...
    #if 1
    if((((*(ptr))^0xffff)&(data^0x0000)) != 0) {hal_HstSendEvent(0x01010101);MEMD_ASSERT( 0, "illegal data 1");}
    #endif
    test_data = (*(ptr));
    ...
    while(bsize<byteSize) {
        ...
        while(1)
        {
           // if(write_ok == 1) break;
            ...
            if ((rdstatus & 0x20) == 0x20)
            {
                ...
                if (((rdstatus ^ tdata) & 0x80) == 0)
                    break;
                MEMD_ASSERT(0, "rw2,rdstatus=0x%x.tdata=0x%x,test_data=0x%x,tptr=0x%x",rdstatus,tdata,test_data,tptr);
        ...
        /* 16b data ready write it to flash*/
    #if 0
    if((((*(ptr))^0xffff)&(data^0x0000)) != 0) {hal_HstSendEvent(0x01010101);MEMD_ASSERT( 0, "illegal data 2");}
    #endif
    ...
    } else
    {
        ...
        while(1)
        {
            ...
            if ((rdstatus & 0x20) == 0x20)
            {
                ...
                if (((rdstatus ^ data) & 0x80) == 0)
                    break;
                MEMD_ASSERT(0, "rw3,rdstatus = 0x%x. data =0x%x test_data=0x%x,ptr=0x%x",rdstatus,data,test_data,ptr);
                errorcode=MEMD_ERR_WRITE;
                break;
            }
        }
...
PUBLIC MEMD_ERR_T memd_FlashWrite_2(UINT8 *flashAddress,
        UINT32 byteSize, 
        UINT32 * pWrittenbyteSize, 
        CONST UINT8* buffer)
{
        ...
        CONST UINT8* buffer)
{
    ...
    kk_count++;
    if((kk_count%50)==1)
    hal_HstSendEvent(0x23456789);
    #endif
    UINT32 * pWrittenbyteSize1 = 0;
    *pWrittenbyteSize=0;
    if (byteSize==0)
    {
        return MEMD_ERR_NO;
        ...
        while(1)
        {
           // if(write_ok == 1) break;
            ...
            if ((rdstatus & 0x20) == 0x20)
            {
                ...
                if (((rdstatus ^ tdata) & 0x80) == 0)
                    break;
               // MEMD_ASSERT(0, "w2,rdstatus=0x%x.tdata=0x%x,test_data=0x%x,tptr=0x%x",rdstatus,tdata,test_data,tptr);
                errorcode=MEMD_ERR_WRITE;
                break;
            }
        }
    ...
    } else
    {
        ...
        while(1)
        {
            ...
            if ((rdstatus & 0x20) == 0x20)
            {
                ...
                if (((rdstatus ^ data) & 0x80) == 0)
                    break;
               // MEMD_ASSERT(0, "w3,rdstatus = 0x%x. data =0x%x test_data=0x%x,ptr=0x%x",rdstatus,data,test_data,ptr);
                errorcode=MEMD_ERR_WRITE;
                break;
            }
        }
   ...
   *(BankAddr) = 0x90;
    ...
    *(BankAddr) = 0xf0;
      errorcode= memd_FlashWrite_2(flashAddress,byteSize,pWrittenbyteSize1,buffer);
      *pWrittenbyteSize=*pWrittenbyteSize1;
   ...
   {  
      ...
      errorcode= memd_FlashWrite_3(flashAddress,byteSize,pWrittenbyteSize1,buffer);
      *pWrittenbyteSize=*pWrittenbyteSize1;
