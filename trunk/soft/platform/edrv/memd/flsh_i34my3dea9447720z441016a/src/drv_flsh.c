...
////////////////////////////////////////////////////////////////////////////////
//                                                                            //
//  $HeadURL: http://svn.coolsand-tech.com/svn/developing1/Sources/edrv/trunk/memd/flsh_i34my3dea9447720z441016a/src/drv_flsh.c $ //
//	$Author: huazeng $                                                        // 
...
//                                                                            //
/// @file drv_flsh.c                                                          //
/// That file provides the driver implementation for the flsh_i34my3dea9447720z441016a     //
/// flash.                                                                    //
//                                                                            //
////////////////////////////////////////////////////////////////////////////////
...
#define FLASHBANK_MASK      (0x0F << 21)     // Actually only need 3 bits, but have to use 4 due to shift
#define FLASH_ADDR_MASK     0x00FFFFFF      // 16M flash (no flash base address)
#define MY_MASK 0x00FFFFFF      // FIX MY_MASK is only used for masking address to Block Lock function.  Do it properly?

#define INVALID_CONTROL_MODE_ADDR(addr)     (((UINT32)(addr) & 0x10) != 0x00)
...
#define CMD_PROG                    0x41
#define CMD_BP_PROG_ST1     0xE9
#define CMD_BP_PROG_ST2     0xD0
...
#define FLASH_CONTROL_MODE_USE_BYTES_16IN32       16
#define FLASH_CONTROL_MODE_USE_BYTES_32IN32       32
...
PRIVATE UINT32 g_memdFlashFSSectorCTX_Size = 0;//(5 * 1024);//5Kbytes
PRIVATE UINT32 g_memdFlashFSSectorSize = 0x40000;//256kbytes
// -------------------------------
// Local Declaration
// -------------------------------
PRIVATE MEMD_ERR_T memd_BlockLock(UINT8 *phy_startFlshAddr, UINT8 *phy_endFlshAddr, BOOL Lock);
PRIVATE UINT32  memd_FlashRemap_FSSectorCTX_ControlModeAddr(UINT32 flashAddress);
PRIVATE MEMD_INTEL_FLASH_SECTOR_T  memd_Flash_Check_FSSectorCTX_Addr(UINT32 flashAddress);
PUBLIC MEMD_ERR_T memd_FlashWrite_ObjectMode_Program(UINT8 *flashAddress,
        UINT32 byteSize, 
        UINT32 * pWrittenbyteSize, 
        CONST UINT8* buffer);
PUBLIC MEMD_ERR_T memd_FlashWrite_ControlMode_Program(UINT8 *flashAddress,
        UINT32 byteSize, 
        UINT32 * pWrittenbyteSize, 
        CONST UINT8* buffer);

#ifdef MEMD_FLASH_IS_BURST
#define FLASH_CRD  0x2907//0x4107//0x2907

PRIVATE VOID HAL_FUNC_INTERNAL 
memd_FlashConfigureBurstMode(REG16* csBase, REG32* pCRE)
{
    // FLASH_CRD[15] : 0 = synch / 1 = asynch
    // FLASH_CRD[14:11] : X Latency 2 = 30MHz / 3 = 40MHz / 4 = 54MHz / 5 = 66MHz? / 6 = 86MHz
...
#ifdef MEMD_RAM_IS_BURST
#define PSRAM_BCR  0x08590f// 0x08610f//0x08710f//0x08590f
...
PUBLIC MEMD_ERR_T memd_FlashErase(UINT8 *startFlashAddress, UINT8 *endFlashAddress)
{
    ...
    MEMD_ASSERT((int)(phys_start_add -FLASH_SIZE) <= 0, 
        "start flash_address to erase is out of flash size!");

    if (endFlashAddress != NULL)
    {
        MEMD_ASSERT((int)(phys_start_add -((UINT32)endFlashAddress)) < 0, 
            "end flash_address to erase is samller than start flash_address!");
    }   
...
PUBLIC MEMD_ERR_T memd_FlashWrite(UINT8 *flashAddress,
        ...
        CONST UINT8* buffer)
{    
    UINT32 uTemp_Flash_Addr;
    MEMD_ERR_T errorcode = MEMD_ERR_NO;   
    UINT32 uTemp_remapped_flashAddress;
    MEMD_INTEL_FLASH_SECTOR_T uTemp_WriteSector_Type;

    uTemp_Flash_Addr = (UINT32)flashAddress;
    uTemp_WriteSector_Type = memd_Flash_Check_FSSectorCTX_Addr(uTemp_Flash_Addr);

    if(uTemp_WriteSector_Type == MEMD_FLASH_OBJECTMODE_CODE_SECTOR)    
    {
        errorcode = memd_FlashWrite_ObjectMode_Program(flashAddress, byteSize, pWrittenbyteSize, buffer);
    }
    else
    if(uTemp_WriteSector_Type == MEMD_FLASH_CONTROLMODE_USERDATA_SECTOR)
    {  
      //  hal_HstSendEvent(0x00ffee15);
      //  hal_HstSendEvent((UINT32)flashAddress);
        uTemp_remapped_flashAddress = memd_FlashRemap_FSSectorCTX_ControlModeAddr(uTemp_Flash_Addr);        
      //  hal_HstSendEvent((UINT32)uTemp_remapped_flashAddress);        
        errorcode = memd_FlashWrite_ControlMode_Program((UINT8*)uTemp_remapped_flashAddress, byteSize, pWrittenbyteSize, buffer);
    }    
    else
    if(uTemp_WriteSector_Type == MEMD_FLASH_OBJECTMODE_USERDATA_SECTOR) 
    {       
       // hal_HstSendEvent(0x00ffee16);
       // hal_HstSendEvent((UINT32)flashAddress);
        uTemp_remapped_flashAddress = (UINT32)flashAddress + g_memdFlashFSSectorCTX_Size;//offset one FSSectorCTX_Size     
       // hal_HstSendEvent((UINT32)uTemp_remapped_flashAddress);
        errorcode = memd_FlashWrite_ObjectMode_Program((UINT8*)uTemp_remapped_flashAddress, byteSize, pWrittenbyteSize, buffer);
    }
        ...
        UINT8*  buffer)
{
    ...
    uTemp_WriteSector_Type = memd_Flash_Check_FSSectorCTX_Addr(phys_add);
   if(uTemp_WriteSector_Type == MEMD_FLASH_OBJECTMODE_CODE_SECTOR)    
   {
   ...
   if(uTemp_WriteSector_Type == MEMD_FLASH_CONTROLMODE_USERDATA_SECTOR) 
   {
//        hal_HstSendEvent(0x00ffee19);
//        hal_HstSendEvent((UINT32)flashAddress);
//        hal_HstSendEvent((UINT32)byteSize);
//        hal_HstSendEvent((UINT32)pReadbyteSize);
//        hal_HstSendEvent((UINT32)buffer);       
        u_Tempsize = byteSize;        
        phys_add = memd_FlashRemap_FSSectorCTX_ControlModeAddr(phys_add);        
       
        ptr = (VOLATILE UINT8 *)(g_memdFlashBaseAddress + phys_add);
        while(u_Tempsize > 0)
        {
            while(INVALID_CONTROL_MODE_ADDR(ptr)) ptr++; 
            *buffer++ = *ptr++;              
            u_Tempsize--;
        }        
   }
   ...
   if(uTemp_WriteSector_Type == MEMD_FLASH_OBJECTMODE_USERDATA_SECTOR) 
   {
        phys_add = (UINT32)flashAddress + g_memdFlashFSSectorCTX_Size;//offset one FSSectorCTX_Size      
        ptr = (VOLATILE UINT8 *)(g_memdFlashBaseAddress + phys_add);
        /* could do aligned read from flash to improve bus accesses as it is uncached */
        memcpy(buffer, (UINT8 *)ptr, byteSize);
   }
...
{   
    MEMD_ERR_T error_code = MEMD_ERR_NO;
    ...
    error_code = memd_BlockLock(startFlashAddress, endFlashAddress, TRUE);   
...
{    
    ...
    error_code = memd_BlockLock(startFlashAddress, endFlashAddress, FALSE);
    /* FIX Must enable Flash for writing in EBC */
    return  error_code;
}
...
PUBLIC VOID memd_FlashGetUserFieldInfo(UINT32 *userFieldBase, UINT32 *userFieldSize, UINT32 *sectorSize, UINT32 *blockSize)
{
    ...
    *blockSize = g_memdFlashLayout.userDataSectorLogicalBlockSize;
    //hal_HstSendEvent(0x00ffee18);
    //hal_HstSendEvent((UINT32)(g_memdFlashLayout.userDataSectorLogicalBlockSize));
    g_memdFlashFSSectorSize = *sectorSize;
}

PUBLIC VOID memd_FlashSetFSSectorCTXSizeInfo(UINT32 sectorCTXSize)
{      
    g_memdFlashFSSectorCTX_Size = sectorCTXSize;
    //hal_HstSendEvent(0x00ffee17);
    //hal_HstSendEvent(g_memdFlashFSSectorCTX_Size);
}

PUBLIC UINT32 memd_FlashGetFSSectorCTXSizeInfo(VOID)
{
   return g_memdFlashFSSectorCTX_Size;
}

PUBLIC VOID memd_FlashGetDevInfo(void)
{
    VOLATILE UINT16 * BankAddr;
    VOLATILE UINT16 * BlockAddr;
    UINT16 DeviceCode;
    ...
    BlockAddr = (VOLATILE UINT16 *)(g_memdFlashBaseAddress);
...
//    hal_HstSendEvent(0x9999999);
//    hal_HstSendEvent(DeviceCode);
...
//    UINT32 owsize = 0;
    ...
    UINT32 status;
    UINT32 phys_add;
    UINT32 phys_add_stop;
    UINT32 phy_SectAddr;
    UINT32 phy_End;
    BOOL isLocked;
    VOLATILE UINT16 rdstatus;  

    VOLATILE UINT16 * tptr;
    UINT32 buffer_program_num;
    UINT32 buffer_program_last_region_bytes;
    UINT32 buffer_program_bytes_needed_write_once;
    UINT32 u_temp_bsize = 0;
    ...
    *pWrittenbyteSize = 0;
    ...
    MEMD_ASSERT((byteSize & 0x1) == 0, 
                   " INTEL_flash object mode , invalid program data length!");

    buffer_program_num = byteSize / g_memdFlashLayout.userDataSectorLogicalBlockSize;    
    buffer_program_last_region_bytes = byteSize - buffer_program_num * g_memdFlashLayout.userDataSectorLogicalBlockSize;
  
    hal_EbcFlashWriteEnable(TRUE);

    phys_add = (UINT32)flashAddress;
    phys_add_stop = phys_add + byteSize;
    MEMD_ASSERT((phys_add & 0xe0000000) == 0,
        "flash_address is expected to be a byte offset within the flash chip, not an absolute address");

while(phys_add < phys_add_stop)
{   
    
    MEMD_ASSERT((phys_add & (g_memdFlashLayout.userDataSectorLogicalBlockSize - 1)) == 0,
        "flash_address is expected to be USER_DATA_BLOCK_SIZE bytes offset within the flash chip, not an absolute address");   

    memd_FlashGetSectorLimits(phys_add, &phy_SectAddr, &phy_End);
    ...
    if(buffer_program_num > 0)
    {
        buffer_program_bytes_needed_write_once = g_memdFlashLayout.userDataSectorLogicalBlockSize;        
        buffer_program_num--;
    }
    else
    {
        buffer_program_bytes_needed_write_once = buffer_program_last_region_bytes;
    }    

    u_temp_bsize = buffer_program_bytes_needed_write_once;
    ...
    if(isLocked == TRUE)
        memd_BlockLock((UINT8*)((UINT32)phy_SectAddr & MY_MASK),NULL,FALSE);
            ...
            status = hal_SysEnterCriticalSection();
            *(BankAddr) = CMD_BP_PROG_ST1;
            *(ptr) = (buffer_program_bytes_needed_write_once >> 1) - 1;

            while(u_temp_bsize > 0)
            {                
                 data = (*buffer) | ~0x00ff;                  
                 buffer ++;
                 bsize ++;     
            ...
            *BankAddr = CMD_READ_ARRAY;      
             ...
             if(errorcode != MEMD_ERR_NO)
            {
             ...
             phys_add += buffer_program_bytes_needed_write_once;            
        
    }
    
    
    *pWrittenbyteSize = bsize;
        ...
        CONST UINT8* buffer)
{
    ...
    *pWrittenbyteSize = 0;
    ...
    while(INVALID_CONTROL_MODE_ADDR(ptr))
        ptr++;
...
{    
    UINT32 phys_add;
    UINT32 uTemp_sector_div_num;
    UINT32 uTemp_sector_mode_num;
    UINT32 uTemp_offset_div_num;
    UINT32 uTemp_offset_mode_num;

     MEMD_ASSERT((flashAddress - g_memdFlashLayout.userDataBase) >= 0,
        "flash_address is not an USER DATA SECTOR address");         

    phys_add = flashAddress - g_memdFlashLayout.userDataBase;    

    MEMD_ASSERT((int)(phys_add - g_memdFlashLayout.userDataSize) <= 0,
        "flash_address is out of  USER DATA SECTOR address");     
    
    MEMD_ASSERT(g_memdFlashFSSectorSize > 0,
        "g_memdFlashFSSectorSize is 0!!!!!!!!!!");     

    uTemp_sector_div_num = phys_add / g_memdFlashFSSectorSize;
    uTemp_sector_mode_num = phys_add % g_memdFlashFSSectorSize;
    
    uTemp_offset_div_num = uTemp_sector_mode_num / FLASH_CONTROL_MODE_USE_BYTES_16IN32;
    uTemp_offset_mode_num = uTemp_sector_mode_num % FLASH_CONTROL_MODE_USE_BYTES_16IN32;

    phys_add = uTemp_offset_div_num * FLASH_CONTROL_MODE_USE_BYTES_32IN32 + uTemp_offset_mode_num;
    phys_add += uTemp_sector_div_num * g_memdFlashFSSectorSize + g_memdFlashLayout.userDataBase;       
...
{
    UINT32 phys_add;    
    UINT32 uTemp_mode_num;
    UINT32 uTemp_div_num0, uTemp_div_num1;
    MEMD_INTEL_FLASH_SECTOR_T uTemp_Sector_Type;
    
    if(flashAddress < g_memdFlashLayout.userDataBase)
    {
        uTemp_Sector_Type = MEMD_FLASH_OBJECTMODE_CODE_SECTOR;
    }
    else
    {
        MEMD_ASSERT(g_memdFlashFSSectorSize > 0,
        ...
        phys_add = flashAddress - g_memdFlashLayout.userDataBase; 
        uTemp_div_num0 = phys_add / g_memdFlashFSSectorSize;
        uTemp_mode_num = phys_add % g_memdFlashFSSectorSize;

        if(uTemp_mode_num < g_memdFlashFSSectorCTX_Size)
        {
            uTemp_Sector_Type = MEMD_FLASH_CONTROLMODE_USERDATA_SECTOR;
        }
        else
        {
            uTemp_div_num1 = (flashAddress + g_memdFlashFSSectorCTX_Size - g_memdFlashLayout.userDataBase) / g_memdFlashFSSectorSize;
            MEMD_ASSERT((uTemp_div_num1 - uTemp_div_num0) == 0, "write data address is out of sector range!");     
            
            uTemp_Sector_Type = MEMD_FLASH_OBJECTMODE_USERDATA_SECTOR;
        }        
    }

    return uTemp_Sector_Type;
}
