...
int rftest_sendua(prh_t_rfcomm_dlci dlci, u_int8 fbit, u_int8 cresp)
{
  t_DataBuf *frame;
  int ret;
  ret=L2_GetWriteBuffer(test_l2_inst->cid, TEST_BUF_SIZE, &frame);
  if (ret==BT_NOERROR)
    {
...
int rftest_sendmsc_reply(prh_t_rfcomm_dlci dlci, u_int8 cresp, char *inframe)
{
  t_DataBuf *frame;
  int ret;  
  ret=L2_GetWriteBuffer(test_l2_inst->cid, 20, &frame);
  if (ret==BT_NOERROR)
    {
      ...
      prh_rftest_create_uih(frame, 0,0, rftest_get_crbit());
      L2_Write(test_l2_inst->cid, frame);
    }
  return 0;
}
...
int rftest_sendrpnc(prh_t_rfcomm_dlci dlci, u_int8 cresp, prh_t_rfcomm_port_params *port_params)
{
  t_DataBuf *frame;
  int ret;
 ret=L2_GetWriteBuffer(test_l2_inst->cid, 50, &frame);
  if (ret==BT_NOERROR)
    {
