...
{
  int data_len=frame->dataLen;
  
  if (data_len>0x7f)
    rftest_data_buf_release_header(frame, 3);
  else
    rftest_data_buf_release_header(frame, 2);
...
int prh_rftest_cc_create_msc(t_DataBuf *frame, u_int8 cresp, prh_t_rfcomm_dlci dlci, prh_t_rfcomm_control_params *ctrl_params)
{
  ...
  *ptr++ = (dlci << 2) | PRH_RFCOMM_CR_BIT | PRH_RFCOMM_EA_BIT;
  ...
  frame->dataLen=ptr-frame->data;
  return 0;
}

/* 
 * prh_rftest_cc_create_rls
...
int prh_rftest_cc_create_pn(t_DataBuf *frame, u_int8 cresp, prh_t_rfcomm_dlci dlci, prh_t_rfcomm_dlc_params *dlc_params)
{
  ...
  *ptr++ = 0; /* From RFCOMM SPEC 5.5.3, I1-I4, CL1-CL4 must be 0 */
...
/* 
 * prh_rftest_cc_create_credit_pn
 * -- Creates a Port Negotiation Frame
 *
 * This function uses default values from the RFCOMM Spec.
 */
