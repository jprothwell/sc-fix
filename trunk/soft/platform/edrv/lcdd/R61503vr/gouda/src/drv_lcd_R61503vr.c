...
////////////////////////////////////////////////////////////////////////////////
//                                                                            //
//  $HeadURL: http://svn.coolsand-tech.com/svn/developing1/Sources/edrv/trunk/lcdd/R61503vr/gouda/src/drv_lcd_R61503vr.c $ //
//	$Author: pengzg $                                                        // 
...
#else
    LCM_WR_REG(0x0007, 0x0202); // Set GON=0, DTE=0, D1=1, D0=0
    LCM_WR_REG(0x0012, 0x0137); // Set VON=0
    sxr_Sleep(50 MS_WAITING);
    LCM_WR_REG(0x0007, 0x0200); // Set GON=0, DTE=0, D1=0, D0=0
    sxr_Sleep(50 MS_WAITING);
    LCM_WR_REG(0x0010, 0x11B1); // Enter Standby mode
#endif
...
#if 0
    LCM_WR_REG(0x0010, 0x11B0); // Exit Standby mode
    LCM_WR_REG(0x0007, 0x0201); // Set GON=0, DTE=0, D1=0, D0=1
    sxr_Sleep(50 MS_WAITING);
    LCM_WR_REG(0x0007, 0x0221); // Set GON=1, DTE=0, D1=0, D0=1
    sxr_Sleep(10 MS_WAITING);
    LCM_WR_REG(0x0012, 0x1137); // Set VON=1, PON=1
    sxr_Sleep(10 MS_WAITING);
    LCM_WR_REG(0x0007, 0x0233); // Set GON=1, DTE=0, D1=1, D0=1
#else
    lcddp_Init();
#endif
...
PRIVATE LCDD_ERR_T lcddp_SetPixel16(UINT16 x, UINT16 y, UINT16 pixelData)
{
    ...
    else
    {
        ...
        LCM_WR_REG(0x0052,LCDD_DISP_Y-1-y);
        LCM_WR_REG(0x0053,LCDD_DISP_Y-1-y);
...
PRIVATE VOID lcddp_BlitRoi2Win(CONST HAL_GOUDA_WINDOW_T* pRoi, CONST HAL_GOUDA_WINDOW_T* pActiveWin)
{
    ...
    LCM_WR_REG(0x0050,LCDD_DISP_X-1-pActiveWin->brPX);//Window Horizontal RAM Address Start                                                                                         
    LCM_WR_REG(0x0051,LCDD_DISP_X-1-pActiveWin->tlPX);//Window Horizontal RAM Address End                                                                                           
    LCM_WR_REG(0x0052,LCDD_DISP_Y-1-pActiveWin->brPY);//Window Vertical RAM Address Start                                                                                           
    LCM_WR_REG(0x0053,LCDD_DISP_Y-1-pActiveWin->tlPY);//Window Vertical RAM Address End 

    LCM_WR_REG(0x0020,LCDD_DISP_X-1-pActiveWin->brPX);
    LCM_WR_REG(0x0021,LCDD_DISP_Y-1-pActiveWin->brPY);
...
{
   static char R61503vr_id_str[] = "R61503vr\n"; 
   return R61503vr_id_str; 
}
PRIVATE BOOL lcddp_CheckProductId()
{
    ...
    SXS_TRACE(TSTDOUT, "R61503vr(0x%x): lcd read id is 0x%x ", LCD_R61503V_ID, productId);
    hal_HstSendEvent(0x1cd00007);
