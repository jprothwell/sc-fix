...
////////////////////////////////////////////////////////////////////////////////
//                                                                            //
//  $HeadURL: http://svn.coolsand-tech.com/svn/developing1/Sources/edrv/trunk/gsensor/mma7660fc/src/gsensor.c $ //
//	$Author: huazeng $                                                        // 
...
//       
/// @file gsensor.c
/// That file implement the gsensor driver for MMA766FC chip.
//
///////////////////////////////////////////
...
//INT8 Xavg8, Yavg8, Zavg8;

UINT8 ShakeSenseStep;
INT8 Xfst, Yfst, Zfst;
INT8 Xsnd, Ysnd, Zsnd;
//UINT8 X_Sk_Step, Y_Sk_Step, Z_Sk_Step;
//UINT8 ShakeDelay;
...
GSENSOR_ERR_T gsensor_WriteSingle(UINT32 addr, UINT8 data)
{
    HAL_ERR_T sensor_err = HAL_ERR_NO;
    sensor_err = hal_I2cSendByte(g_gsensorConfig->i2cBusId,GSENSOR_CHIP_ADRESS,addr,data);
...
GSENSOR_ERR_T gsensor_ReadSingle(UINT32 addr, UINT8* data)
{
    HAL_ERR_T sensor_err = HAL_ERR_NO;
    sensor_err = hal_I2cGetByte(g_gsensorConfig->i2cBusId,GSENSOR_CHIP_ADRESS,addr,data);
    if (sensor_err != HAL_ERR_NO)
    {
...
GSENSOR_ERR_T gsensor_Write(UINT32 addr, UINT8* data,UINT8 length)
{
    HAL_ERR_T sensor_err = HAL_ERR_NO;
    gsensor_I2cOpen();
    sensor_err = hal_I2cSendData(g_gsensorConfig->i2cBusId,GSENSOR_CHIP_ADRESS, addr,data,length);
    gsensor_I2cClose();
    if (sensor_err != HAL_ERR_NO)
    {
...
GSENSOR_ERR_T gsensor_Read_xyz(UINT8* x_adc, UINT8* y_adc,UINT8* z_adc)
{
    ...
    {
        sensor_err = hal_I2cGetByte(g_gsensorConfig->i2cBusId,GSENSOR_CHIP_ADRESS,MMA7660FC_ADDR_XOUT,x_adc);
    ...
    while(sensor_err != HAL_ERR_NO)
    {
        sensor_err = hal_I2cGetByte(g_gsensorConfig->i2cBusId,GSENSOR_CHIP_ADRESS,MMA7660FC_ADDR_YOUT,y_adc);
    ...
    while(sensor_err != HAL_ERR_NO)
    {
        sensor_err = hal_I2cGetByte(g_gsensorConfig->i2cBusId,GSENSOR_CHIP_ADRESS,MMA7660FC_ADDR_ZOUT,z_adc);
...
GSENSOR_ERR_T gsensor_Read(UINT32 addr, UINT8* data,UINT8 length)
{
    HAL_ERR_T sensor_err = HAL_ERR_NO;
    gsensor_I2cOpen();
    sensor_err = hal_I2cGetData(g_gsensorConfig->i2cBusId,GSENSOR_CHIP_ADRESS,addr,data,length);
    gsensor_I2cClose();
    if (sensor_err != HAL_ERR_NO)
    {
...
VOID gsensor_monitor(VOID)
{
   ...
   GSS_TRACE(GSS_INFO_TRC, 0, "gss:  hi  x = %d  ",g_acc_data.x_adc);
...
GSENSOR_ERR_T gsensor_Filt_LP(UINT8 count)
{
 ...
 gsensor_I2cOpen();
 if (GSENSOR_ERR_NO != gsensor_ReadSingle(MMA7660FC_ADDR_SR,&reg_data))
 {
     sxr_Sleep(1 MILLI_SECONDS);
     ret =  gsensor_ReadSingle(MMA7660FC_ADDR_SR,&reg_data);
 ...
 if (GSENSOR_ERR_NO != gsensor_WriteSingle(MMA7660FC_ADDR_SR,reg_data))
 {
     sxr_Sleep(1 MILLI_SECONDS);
     ret =  gsensor_WriteSingle(MMA7660FC_ADDR_SR,reg_data);
     if (ret != GSENSOR_ERR_NO)
     {
        gsensor_I2cClose();
        return GSENSOR_ERR_RESOURCE_BUSY;
...
VOID test_sleep_count_write()
{
    ...
    GSS_TRACE(GSS_INFO_TRC, 0, "gss:  test_sleep_count_  write ");
