...
#define CDAC_REG_VALUE_BASE (0x010)
...
INT16 RFD_FUNC_INTERNAL Pcl2dBm (GSM_RFBAND_T band,UINT16 arfcn,UINT8 Pcl)
{
    ...
    if (band == GSM_BAND_GSM850)
    {
        if (Pcl == 1)
        {
            dBm_level = dBm_level - 100;
        }
        else if (Pcl == 2)
        {
            dBm_level = dBm_level - 150;
        }
	 else if (Pcl >= 3)
        {
            dBm_level = dBm_level - 200;
        }
    }
#endif
...
UINT8 RFD_FUNC_INTERNAL xcvBuildRxSpiCmd(UINT16 arfcn, GSM_RFBAND_T band, UINT8 gain, HAL_RFSPI_CMD_T *spiCmd)
{
    ...
    xcvBuildSpiCmd (spiCmd, RDA6220_CDAC_REG, CDAC_REG_VALUE_BASE | ((adjustedCdac&0xff) << 10));
...
INLINE VOID xcvConfigure(VOID)
{
    ...
    // ;=====DCXO Setting=====
    UINT16 adjustedCdac = g_CDAC + g_CdacDelta;
    xcvBuildSpiCmd (confCmd, RDA6220_CDAC_REG, CDAC_REG_VALUE_BASE| ((adjustedCdac&0xff) << 10));
    // ;=====PLL_GSM Setting=====
    xcvBuildSpiCmd (confCmd, 0x14, 0x104a1); // Rxpll_sink_mode=100
    xcvBuildSpiCmd (confCmd, 0x15, 0x23c00); // Rxmix 900 seting
    xcvBuildSpiCmd (confCmd, 0x3F, 0x00001); // page=1
...
/*
    ...
    xcvBuildSpiCmd (confCmd, 0x0, 0x10 | ((page3_dac&0xff) << 9));
*/
    xcvBuildSpiCmd (confCmd, 0x3F, 0x00003); // page=3
    //0x000 for power off currrent is 300uA
    xcvBuildSpiCmd (confCmd, 0x0, 0x000| ((adjustedCdac&0xff) << 10));    
    xcvBuildSpiCmd (confCmd, 0x3F, 0x00000); // page=0
    // ;=====PLL_Edge Setting=====
#ifdef FPGA
...
#ifdef FPGA
#else
    xcvBuildSpiCmd (confCmd, 0x04, 0x2783f); // gsm_mode, tmx_gain_bit=0x08
...
#else
    xcvBuildSpiCmd (confCmd, 0x1D, 0x28358); // TxPLL OPT:Edge_tx_dvdd_bit=1111,Edge_tx_vcom_bit=0011,Edge_tx_reg_buf_bit=1111
#endif
...
#else
#ifdef __RF_RX1234_RECFG__
    xcvBuildSpiCmd (confCmd, 0x3A, 0x078af); // GPIO BS1,BS0 Setting for PCB6220 
    xcvBuildSpiCmd (confCmd, 0x2A, 0x007ff); // GPIO SWEN_lowband 
    xcvBuildSpiCmd (confCmd, 0x2C, 0x08000); // GPIO SWEN_highband 
#else
...
#else
    xcvBuildSpiCmd (confCmd, 0x1A, 0x146d8); // 0x146FF
    xcvBuildSpiCmd (confCmd, 0x1B, 0x2e5c0); // 0x2e000
#endif
...
VOID RFD_FUNC_INTERNAL rfd_XcvWakeUp (VOID)
{
    if (g_lastSleepMode == RFD_POWER_OFF)
    {
        ...
        HAL_RFSPI_CMD_T* confCmd = hal_RfspiNewSingleCmd();
        UINT32 bitfield = CDAC_REG_VALUE_BASE | (((g_CDAC + g_CdacDelta)&0xff) << 10);
...
VOID RFD_FUNC_INTERNAL rfd_XcvSleep (RFD_LP_MODE_T lp_mode)
{
   ...
   // INT32 afc_offset_ppm = 13.1+(g_afcDacVal-(RDA6220_AFC_MID-RDA6220_AFC_HRANGE))*XCV_PARAM(AFC_GAIN(GSM_BAND_GSM900))/(1000*900);
    INT32 afc_offset_ppm;// = g_afcDacVal * XCV_PARAM(AFC_GAIN(GSM_BAND_GSM900)) / (1000*900) - 5;
    UINT16 step_offset_lp = 0;
    if(g_afcDacVal <= (RDA6220_AFC_MID - RDA6220_AFC_HRANGE))
    {
        afc_offset_ppm = (g_afcDacVal * 2 / 3) * XCV_PARAM(AFC_GAIN(GSM_BAND_GSM900)) / (1000 * 900);
    }
    else
    if(g_afcDacVal <= (RDA6220_AFC_MID + RDA6220_AFC_HRANGE))
    {
        afc_offset_ppm = ((RDA6220_AFC_MID - RDA6220_AFC_HRANGE) * 2 / 3 + (g_afcDacVal -(RDA6220_AFC_MID -RDA6220_AFC_HRANGE))) * XCV_PARAM(AFC_GAIN(GSM_BAND_GSM900)) / (1000 * 900);
    }
    else
        afc_offset_ppm = ((RDA6220_AFC_MID - RDA6220_AFC_HRANGE) * 2 / 3 + RDA6220_AFC_HRANGE * 2 + (g_afcDacVal -(RDA6220_AFC_MID + RDA6220_AFC_HRANGE)) * 4 / 3) * XCV_PARAM(AFC_GAIN(GSM_BAND_GSM900)) / (1000 * 900);
    ...
    xcvBuildSpiCmd (confCmd, 0x3F, 0x00003); // page=3
    xcvBuildSpiCmd (confCmd, 0x00, 0x000| (((g_CDAC + g_CdacDelta)&0xff) << 10));    
    xcvBuildSpiCmd (confCmd, 0x03, 0x0);    
    xcvBuildSpiCmd (confCmd, 0x02, (step_offset_lp*(-1))&0xff);    
...
VOID rfd_XcvCalibResetValues(VOID)
{
    ...
    if (g_CDAC != RDA6220_CDAC_DEFAULT)
    {
        ...
        // Xtal triming capacitor register to default (calibrated)
        g_CDAC = RDA6220_CDAC_DEFAULT;
        UINT32 bitfield = CDAC_REG_VALUE_BASE | ((g_CDAC&0xff) << 10);
...
{ 
    ...
    UINT32 bitfield = CDAC_REG_VALUE_BASE | (((g_CDAC+RDA6220_CDAC_OFFSET)&0xff) << 10);
    xcvBuildSpiCmd (spiCmd, RDA6220_CDAC_REG, bitfield);
