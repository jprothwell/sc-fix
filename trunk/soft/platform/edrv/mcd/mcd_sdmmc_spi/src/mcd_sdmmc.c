...
PUBLIC MCD_ERR_T mcd_sdmmc_Open(MCD_CSD_T* mcdCsd, MCD_CARD_VER mcdVer,MCD_SDMMC_SPI_IF* mcdConfig)
{
    ...
    MCD_PROFILE_FUNCTION_ENTER(mcd_sdmmc_Open);
    MCD_TRACE(MCD_INFO_TRC, 0, "mcd_sdmmc_Open starts ...");

    // Check concurrency. Only one mcd_sdmmc_Open.
    ...
    if(g_mcdConfig->cardDetectGpio != HAL_GPIO_NONE)
    {
        ...
        {
            MCD_TRACE(MCD_INFO_TRC, 0, "mcd_sdmmc_Open: GPIO detection - no card");
            MCD_CS_EXIT;
            MCD_PROFILE_FUNCTION_EXIT(mcd_sdmmc_Open);
    ...
    if (g_mcdStatus == MCD_STATUS_OPEN)
    {
        ...
        MCD_TRACE(MCD_INFO_TRC, 0, "mcd_sdmmc_Open: Already opened");
...
///         MCD_ERR_CARD_TIMEOUT if the card timedout during procedure.
// =============================================================================
PUBLIC MCD_ERR_T mcd_sdmmc_Close()
{
    MCD_TRACE(MCD_INFO_TRC, 0, "mcd_sdmmc_Close");

    if (g_mcdSemaphore == 0xffffffff)
    {
        MCD_TRACE(MCD_INFO_TRC, 0, "mcd_sdmmc_Close: Never opened before");
        return MCD_ERR_NO;
    }
    ...
    MCD_PROFILE_FUNCTION_ENTER(mcd_sdmmc_Close);
    ...
    MCD_CS_EXIT;
    MCD_PROFILE_FUNCTION_EXIT(mcd_sdmmc_Close);
    return errStatus;
}
...
PUBLIC MCD_ERR_T mcd_sdmmc_Write(UINT32 startAddr, CONST UINT8* blockWr, UINT32 size)
{
    ...
    MCD_ASSERT(g_mcdBlockLen != 0, "mcd_Write called before a successful mcd_sdmmc_Open");
    MCD_ASSERT(startAddr % g_mcdBlockLen == 0,
