...
////////////////////////////////////////////////////////////////////////////////
//
//  $HeadURL: http://10.10.100.14/svn/developing/Sources/svc/branches/Total_BJ/cams/include/cams_m.h $
//  $Author: lilian $
//  $Date$
//  $Revision: 20008 $
//
...
//
/// @file cams_m.h
/// @mainpage Camera Service (CAMS)
/// @par
/// The CAMera Service (CAMS) is the high level service handling the task
/// of performing camera related operations, namely:
/// - aiming
/// - shoot of picture
/// - capture of videos
...
/// 
/// @par
/// The API of this service is designed in order to be directly usable
/// by a MMI code, and therefore offers callback with the taken picture
/// set as parameter to the upper layer. MMI code can then at its own
/// will use the image taken to display it on the screen, taking into
/// account other OSD information to display, for example. 
/// The CAMS API is THE ONE offering 
/// access to the camera for user application. CAMD shall not be used 
/// by any other code than CAMS's.
///
/// @par
/// CAMS is implemented as a standalone task, started by the #cams_Open 
/// function, and stopped by #cams_Close.
///
/// @par Aiming
/// The preview of the picture to shoot is started by the #cams_StartPreview
/// function. This will display what is seen by the camera into the frame 
/// buffer window passed to the function as a parameter, and will be 
/// automatically refreshed until the #cams_StopPreview function is called
/// to stop it. The preview is done at a given resolution (parameter to the
/// function). Every time a capture is done, a user callback is called with
/// the provided frame buffer window as a parameter. The user can then display
/// it on the screen or do whatever operation required. This callback's type
/// is #CAMS_PREVIEW_HANDLER_T.
///
/// @par Shooting
/// Once the aiming is satisfying enough, the capture is done with
/// #cams_Capture. Parameters are the same as for the preview function
/// (#cams_StartPreview), plus a full size frame buffer window that receive
/// the full resolution picture. The handler is of a different type:
