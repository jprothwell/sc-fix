...
void iput( struct inode * inode )
{
    if( NULL == inode)
    {
        SVC_FSS_TRACE(DL_VFSERROR, "in iput, inode is NULL\n");
        return;
    }
    ...
    if( 0 == inode->i_count)
    {
        SVC_FSS_TRACE(DL_VFSERROR, "in iput, inode->i_count = %d\n",inode->i_count);
        return;
    }
    
    if ( !inode->i_dev )             /*should be an empty inode in g_fs_inode_table*/
    {
        SVC_FSS_TRACE(DL_VFSWARNING, "in iput, inode->i_dev = %d\n",inode->i_dev);
        inode->i_count--;
        return;
    }
    inode->i_count--;
...
struct inode* get_empty_inode( void )
{
    ...
    inode = (struct inode *)sxr_Malloc(SIZEOF(*inode));
    if( NULL == inode)
    {
        return NULL;
    }
...
struct inode* inode_list_add(struct inode **ppsInodeList, struct inode *psInode)
{
    ...
    {
        SVC_FSS_TRACE(DL_FATERROR, "inode_list_add: _ERR_FAT_PARAM_ERROR, ppsInodeList(0x%x), psInode(0x%x)", ppsInodeList, psInode);
        return NULL;
    }
...
INT32 inode_list_del(struct inode **ppsInodeList, struct inode *psInode)
{
    ...
    if (NULL == ppsInodeList || NULL == psInode)
    {
        SVC_FSS_TRACE(DL_FATERROR, "inode_list_del: _ERR_FAT_PARAM_ERROR, ppsInodeList(0x%x), psInode(0x%x)", ppsInodeList, psInode);
    ...
    */
    ...
    while (inode != NULL)
    {
        ...
        if ( inode->i_is_mountpoint )
        {
            ...
            {
                SVC_FSS_TRACE(DL_VFSERROR, "in iget, Mounted inode hasn't got sb, inode->i_dev = %d, inode->i_ino  = %d\n",inode->i_dev,inode->i_ino);
                return NULL;
            }
            iput( inode );
            dev_nr = sb->s_dev;
    ...
    {
        SVC_FSS_TRACE(DL_VFSWARNING, "in iget, inode table is full\n");
        return NULL;
    }
    ...
    {
        SVC_FSS_TRACE(DL_VFSERROR, "in iget, fat_read_inode error,lRet = %d\n",lRet);
        iput( inode );
        return NULL;
    }
    return inode;
...
struct super_block * get_super( UINT32 devnr )
{
    ...
    SVC_FSS_TRACE(DL_VFSWARNING, "in get_super,  no superblock is found\n");
    return NULL;
}
