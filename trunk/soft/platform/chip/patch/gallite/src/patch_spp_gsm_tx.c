...
//
//-----------------------------------------------------------------------------
/// @file patch_spp_gsm_tx.c
///
/// This file contains the patch functions for the transmission
/// functions of the Signal Processing Package
///
//  $HeadURL: http://svn.coolsand-tech.com/svn/developing1/Sources/chip/branches/gallite441/patch/gallite/src/patch_spp_gsm_tx.c $ //
//    $Author: huazeng $                                                        // 
...
#define NEXT_SLOT(n) ((n) & 0x4)
    ...
    for (bst = 0; bst < 4; bst++) // loop on bursts
    {
        if (CURR_SLOT(Bitmap)) // Active slot
        {
            ...
            InBuf = (Lshift > remBits)?STUFFING_WORD : // All data bits already in LastBuf
                                       BufferIn[BufIdx++];
