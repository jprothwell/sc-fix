...
//                       !!! PLEASE DO NOT EDIT !!!                             
//                                                                              
//  $HeadURL: http://svn.coolsand-tech.com/svn/developing1/Sources/chip/branches/gallite441/regs/esperite/include/sci.h $                                                                   
//  $Author: admin $                                                                    
...
/// 
// =============================================================================
#define REG_SCI_BASE                0x01A0F000
...
typedef volatile struct
{
    ...
    /// This register is a <b>READ ONLY</b> register that returns the logical <b>and</b>
    /// of the SCI_INT_STATUS register and the SCI_INT_MASK. If any of these bits
    /// is '1', the SCI module will generate an interrupt. Bits 25:16 return the
    /// <u>status</u> of the interrupt which is the interrupt state before the mask
    /// is applied. These bits should only be used for debugging.
    REG32                          Int_Cause;                    //0x00000020
    /// This is a WRITE ONLY register that is used to clear an SCI interrupt. Write
    /// a '1' to the interrupt that is to be cleared. Writing '0' has no effect.
    REG32                          Int_Clr;                      //0x00000024
    /// This register is READ/WRITE register that enables the desired interrupt.
    /// A '1' in a bit position indicates that the corresponding interrupt is enabled
    /// and if the interrupt occurs, the SCI will generate a hardware interrupt.
    REG32                          Int_Mask;                     //0x00000028
    REG32                          BWT_WWT;                      //0x0000002C
} HWP_SCI_T;
...
#define SCI_EDC_ERR                 (1<<12)
#define SCI_RX_FIFO_LEVEL(n)        (((n)&3)<<14)
...
#define SCI_MAINDIV(n)              (((n)&0x7F)<<24)
#define SCI_MAINDIV_MASK            (0x7F<<24)
...
#define SCI_CWT(n)                  (((n)&0x3F)<<16)
#define SCI_TX_PERT(n)              (((n)&15)<<24)
...
#define SCI_TX_DMA_DONE             (1<<6)
#define SCI_RX_DMA_DONE             (1<<7)
#define SCI_CWT_TIMEOUT             (1<<8)
#define SCI_BWT_TIMEOUT             (1<<9)
...
//BWT_WWT
#define SCI_BWT_WWT(n)              (((n)&0xFFFFFFFF)<<0)
