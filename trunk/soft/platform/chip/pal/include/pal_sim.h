...
--------------------------------------------------------------------------------
  ...
  Scope      : SIM physical abstraction header file.

  History    :
--------------------------------------------------------------------------------
  Sep 29 03  |  ABA   | Creation
================================================================================
*/
#ifndef PAL_SIM_H
#define PAL_SIM_H
...
#if (NUMBER_OF_SIM==1) 
#define	PAL_MAX_SIM	   1        //The number of SIM stack can supports

#elif (NUMBER_OF_SIM==2) 
#define PAL_MAX_SIM           2


#elif (NUMBER_OF_SIM==3) 
#define PAL_MAX_SIM           3
...
// Used to set the sim  voltage
//extern void pal_SetSimVoltage(UINT8 Level);
...
//extern void pal_SetChGuard(UINT8 Ch_Guard);
#define pal_SetChGuard hal_SimSetChGuard
// Ch_Guard is the number of ETUs _after_ the parity bit is sent
// Default (and minimum) value is 2

//extern void pal_SetTurnaroundGuard(UINT8 Turnaround_Guard);
#define pal_SetTurnaroundGuard hal_SimSetTurnaroundGuard
// Turnaround_Guard is the number of ETUs between characters
// sent in opposite directions.  Default = 6
...
PUBLIC VOID pal_GetSimIndex(UINT8 *sim_Index);
PUBLIC BOOL pal_EnableSimCard(UINT8 sim_Index);
PUBLIC BOOL  pal_SelectSwitch(UINT8 number);
