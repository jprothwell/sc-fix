...
////////////////////////////////////////////////////////////////////////////////
//                                                                            //
//  $HeadURL: http://svn.coolsand-tech.com/svn/developing1/Sources/chip/branches/gallite441/boot/greenstone/src/boot_uart_monitor.c $ //
//    $Author: admin $                                                         //
...
////////////////////////////////////////////////////////////////////////////////
//                                                                            //
/// @file hal_uart_monitor.c                                                  //
/// That file implement the uart monitor...                                   //
/// That file is ROMED.
//                                                                            //
////////////////////////////////////////////////////////////////////////////////
...
PROTECTED BOOL g_bootUartMonitorBrDetected
  = FALSE;
...
{
    hwp_extApb->GPIO_Mode        &=
        ~(REGS_MODE_PIN_UART1_RTS | REGS_MODE_PIN_UART1_CTS);
...
PROTECTED BOOL boot_UartMonitorTimeOut(BOOL start, UINT32* chrono)
{
    ...
    {
        return (hwp_timer->HWTimer_CurVal > *chrono);
    }
    return FALSE;
}
    ...
    while (tryFaster)
    {
        ...
        switch (cmdId)
        {
            ...
            default:
                ...
                baudRate *= 2;
                if (baudRate >= HAL_UART_BAUD_RATE_1843200) // Max rate, too fast
                {
                    // Exit
                    tryFaster = FALSE;
                }
...
PROTECTED BOOT_UM_ERR_T boot_UartMonitorWriteProcess(UINT32 length)
{
    ...
    switch (writeHeader.access)
    {
        ...
        case 2:
        {
            ...
            for (i = 0 ; i < size; i++)
            {
                if (BOOT_UM_ERR_NO != boot_UartMonitorGet(value.byte, 2, &checksum))
                {
                    if (boot_UartMonitorAck("COM_ERR", 7) != BOOT_UM_ERR_NO)
                    {
                        return BOOT_UM_ERR_TX_FAILED;
        ...
        case 4:
        {
            ...
            for (i=0; i<size; i++)
            {
                if (BOOT_UM_ERR_NO != boot_UartMonitorGet(value.byte, 4, &checksum))
                {
                    if (boot_UartMonitorAck("COM_ERR", 7) != BOOT_UM_ERR_NO)
                    {
                        return BOOT_UM_ERR_TX_FAILED;
...
PROTECTED BOOT_UM_ERR_T boot_UartMonitorReadProcess(UINT32 length)
{
    ...
    switch (readHeader.access)
    {
        ...
        case 2:
        {
            ...
            for (i = 0; i<size; i++)
            {
                value.halfWord = *address++ ;
                if (BOOT_UM_ERR_NO != boot_UartMonitorSend(value.byte, 2, &sendCrc))
                {
                    return BOOT_UM_ERR_TX_FAILED; // Send ERROR
                }
            }
        ...
        case 4:
        {
            ...
            {
                value.word = *address++ ;
                if (BOOT_UM_ERR_NO != boot_UartMonitorSend(value.byte, 4, &sendCrc))
                {
                    return BOOT_UM_ERR_TX_FAILED; // Send ERROR
                }
            }
