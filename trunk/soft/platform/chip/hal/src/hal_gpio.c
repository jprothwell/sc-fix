...
////////////////////////////////////////////////////////////////////////////////
//                                                                            //
//  $HeadURL: http://svn.coolsand-tech.com/svn/developing1/Sources/chip/branches/gallite441/hal/src/hal_gpio.c $//
//  $Author: admin $                                                        // 
...
PRIVATE HAL_GPIO_IRQ_HANDLER_T g_halGpioRegistry[8]
...
PUBLIC VOID hal_GpioOpen(HAL_GPIO_GPIO_ID_T gpio, CONST HAL_GPIO_CFG_T* cfg)
{
    ...
    HAL_ASSERT(apo.type == HAL_GPIO_TYPE_IO, "Using hal_GpioOpen for the wrong gpio type");
    ...
    if( cfg->irqMask.rising || cfg->irqMask.falling )
    {
        UINT32 ctrlSet = 0;
        UINT32 ctrlClr = 0;
        HAL_ASSERT( gpio >= HAL_GPIO_0 && gpio <= HAL_GPIO_7, "Trying to set interrupt mask on non interruptible gpio (number not between 0 and 7.");

        if(cfg->irqMask.rising)
        {
    ...
    //Set the irq handler
    hal_GpioIrqSetHandler(gpio,cfg->irqHandler);
}
...
PUBLIC VOID hal_GpioClose(HAL_GPIO_GPIO_ID_T gpio)
{
    ...
    HAL_ASSERT(apo.type == HAL_GPIO_TYPE_IO, "Using hal_GpioClose for the wrong gpio type");
...
PUBLIC VOID hal_GpioSetOut(HAL_GPIO_GPIO_ID_T gpio)
{
    ...
    HAL_ASSERT(apo.type == HAL_GPIO_TYPE_IO, "Using hal_GpioSetOut for the wrong gpio type");
...
PUBLIC VOID hal_GpioSetIn(HAL_GPIO_GPIO_ID_T gpio)
{
    ...
    HAL_ASSERT(apo.type == HAL_GPIO_TYPE_IO, "Using hal_GpioSetIn for the wrong gpio type");
...
PUBLIC UINT32 hal_GpioGet(HAL_GPIO_GPIO_ID_T gpio)
{
    ...
    HAL_ASSERT(apo.type == HAL_GPIO_TYPE_IO, "Using hal_GpioGet for the wrong gpio type");
...
PUBLIC VOID hal_GpioResetIrq(HAL_GPIO_GPIO_ID_T gpio)
{
    ...
    HAL_ASSERT(apo.type == HAL_GPIO_TYPE_IO, "Using hal_GpioResetIrq for the wrong gpio type");
    HAL_ASSERT( gpio >= HAL_GPIO_0 && gpio <= HAL_GPIO_7, "Trying to reset interrupt on non interruptible gpio (number not between 0 and 7.");
...
PUBLIC VOID hal_GpioIrqSetHandler(HAL_GPIO_GPIO_ID_T gpio, HAL_GPIO_IRQ_HANDLER_T handler)
{
    ...
    HAL_ASSERT(apo.type == HAL_GPIO_TYPE_IO, "Using hal_GpioIrqSetHandler for the wrong gpio type");
    HAL_ASSERT( gpio >= HAL_GPIO_0 && gpio <= HAL_GPIO_7, "Trying to reset interrupt on non interruptible gpio (number not between 0 and 7.");

    g_halGpioRegistry[apo.id] = handler ;
}
...
{
    hal_GpioIrqSetHandler(HAL_GPIO_CHARGER_DETECT, handler);
}
