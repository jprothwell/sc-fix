...
////////////////////////////////////////////////////////////////////////////////
//                                                                            //
//  $HeadURL: http://svn.coolsand-tech.com/svn/developing1/Sources/chip/branches/gallite441/hal/src/hal_spi.c $ //
//    $Author: caoxh $                                                        // 
//    $Date: 2011-06-17 12:26:22 +0800 (Fri, 17 Jun 2011) $                     //   
//    $Revision: 8319 $                                                          //   
//                                                                            //
////////////////////////////////////////////////////////////////////////////////
//                                                                            //
...
/// Gallite introduces a new naming for the hwp_spi pointer to the SPI
/// hardware register configuration
#define hwp_spi1    hwp_spi
#endif
...
PRIVATE HWP_SPI_T* g_halSpiHwpArray[HAL_SPI_QTY] = 
#if (CHIP_SPI2_CS_QTY == 0)
// Just one SPI
    {hwp_spi1};
#else
...
// =============================================================================
PRIVATE VOID hal_SpiDoConfigCs(HAL_SPI_ID_T id,  HAL_SPI_CFG_T* spiConfigPtr)
{
...
#else // GREENSTONE
#if (CHIP_SPI_HAS_DIO == 0)
            | SPI_INPUT_SEL
            // There is enough room to put those registers,
            // otherwise they are deported in the irqMask thing
#else
            | ((spiConfigPtr->oeRatio == 0)?SPI_INPUT_SEL:0)
            | SPI_OE_DELAY(spiConfigPtr->oeRatio)
            );
...
PUBLIC VOID hal_SpiFlushFifos(HAL_SPI_ID_T id)
{
    HAL_ASSERT(id < HAL_SPI_QTY, "Improper SPI id %d ! \n"
                                " Your system only has %d SPI", id, HAL_SPI_QTY);
    g_halSpiHwpArray[id]->status = SPI_FIFO_FLUSH;
}
...
{
    g_halSpiHwpArray[id]->status = SPI_RX_DMA_DONE;
}
...
{
    g_halSpiHwpArray[id]->status = SPI_TX_DMA_DONE;
}
...
PROTECTED VOID hal_SpiUpdateDivider(HAL_SPI_ID_T id, HAL_SYS_FREQ_T sysFreq)
{
    ...
    // Apply change
    g_halSpiHwpArray[id]->cfg = g_halSpiPropArray[id].cfg[cs].cfg;
}
...
PUBLIC BOOL hal_SpiActivateCs(HAL_SPI_ID_T id, HAL_SPI_CS_T csNum)
{
    ...
    if (doActivate)
    {
        ...
        hal_SysRequestFreq(HAL_SYS_FREQ_SPI, suitedSysFreq, hal_SpiUpdateAllDividers);
        ...
        g_halSpiHwpArray[id]->cfg  = g_halSpiPropArray[id].cfg[csNum].cfg;
        g_halSpiHwpArray[id]->ctrl = g_halSpiPropArray[id].cfg[csNum].ctrl;
#if (CHIP_SPI_HAS_DIO == 1)
        g_halSpiHwpArray[id]->irq = g_halSpiPropArray[id].cfg[csNum].irqMask;
#endif
...
{
    hal_IfcChannelRelease(HAL_IFC_SPI_RX, g_halSpiPropArray[id].rxIfcCh);
}
...
{
    hal_IfcChannelRelease(HAL_IFC_SPI_TX, g_halSpiPropArray[id].rxIfcCh);
}
...
PUBLIC VOID hal_SpiDeActivateCs(HAL_SPI_ID_T id, HAL_SPI_CS_T csNum)
{
    ...
    if (doDeactivate)
    {
        ...
        {
            hal_IfcChannelRelease(HAL_IFC_SPI_TX, g_halSpiPropArray[id].txIfcCh);
            //hal_SpiTxIfcChannelRelease(HAL_IFC_SPI_TX, g_halSpiTxIfcCh);
        }

        // Clear all status
        g_halSpiHwpArray[id]->status = 0xffffffff;
                ...
                UINT32 length)
{
    ...
    // DIRECT TRANSFER
    // -----------------------------
        freeRoom = GET_BITFIELD(g_halSpiHwpArray[id]->status, SPI_TX_SPACE);
        ...
        {
            g_halSpiHwpArray[id]->rxtx_buffer = *(startAddress + i);
        }
    ...
    else
    {
        ...
        if (id == HAL_SPI_2)
        {
            g_halSpiPropArray[id].txIfcCh=hal_IfcTransferStart(HAL_IFC_SPI2_TX,
                                        (UINT8*)startAddress,
                                        length, HAL_IFC_SIZE_8_MODE_AUTO);
        }
        else
        {
            g_halSpiPropArray[id].txIfcCh=hal_IfcTransferStart(HAL_IFC_SPI_TX,
                                        (UINT8*)startAddress,
                                        length, HAL_IFC_SIZE_8_MODE_AUTO);
        }
...
PRIVATE UINT8 hal_SpiTxFifoLevel(HAL_SPI_ID_T id, HAL_SPI_CS_T csNum)
{
    ...
    txLevel = SPI_TX_FIFO_SIZE
            - GET_BITFIELD(g_halSpiHwpArray[id]->status, SPI_TX_SPACE);
...
//  Check if this CS has been activated 
    ...
    if (g_halSpiHwpArray[id]->status & SPI_TX_DMA_DONE)
    {
        // clear done
        g_halSpiPropArray[id].txIfcCh = HAL_UNKNOWN_CHANNEL;
        g_halSpiHwpArray[id]->status = SPI_TX_DMA_DONE;
                ...
                )
{
    ...
    g_halSpiHwpArray[id]->ctrl = g_halSpiPropArray[id].cfg[csNum].ctrl;
    //g_halSpiHwpArray[id]->cfg = g_halSpiCsCfg[csNum].cfg;
    ...
    if (!IS_DMA(g_halSpiPropArray[id].txMode))
    {
        ...
        // DIRECT TRANSFER
        // -----------------------------
        nbAvailable = GET_BITFIELD(g_halSpiHwpArray[id]->status, SPI_RX_LEVEL);
        ...
        {
            *(destAddress + i) = g_halSpiHwpArray[id]->rxtx_buffer;
        }
    ...
    else
    {
       // -----------------------------
        ...
        if (id == HAL_SPI_2)
        {
            g_halSpiPropArray[id].rxIfcCh = hal_IfcTransferStart( HAL_IFC_SPI2_RX,
                                             destAddress,
                                             length, HAL_IFC_SIZE_8_MODE_AUTO);
        }
        else
        {
            ...
            g_halSpiPropArray[id].rxIfcCh = hal_IfcTransferStart( HAL_IFC_SPI_RX,
                                             destAddress,
                                             length, HAL_IFC_SIZE_8_MODE_AUTO);
        }
...
PUBLIC UINT8 hal_SpiRxFifoLevel(HAL_SPI_ID_T id, HAL_SPI_CS_T csNum)
{
    ...
    // Get level 
    rxLevel = GET_BITFIELD(g_halSpiHwpArray[id]->status, SPI_RX_LEVEL);
...
PUBLIC BOOL hal_SpiRxDmaDone(HAL_SPI_ID_T id, HAL_SPI_CS_T csNum)
{
    ...
    if (g_halSpiHwpArray[id]->status & SPI_RX_DMA_DONE) 
    {
        // clear done
        g_halSpiPropArray[id].rxIfcCh = HAL_UNKNOWN_CHANNEL;
        g_halSpiHwpArray[id]->status = SPI_RX_DMA_DONE;
...
#endif
    g_halSpiHwpArray[id]->pattern = patternReg;

    g_halSpiHwpArray[id]->stream = SPI_TX_STREAM_MODE_ENABLED
...
PUBLIC VOID hal_SpiStopInfiniteWriteMode(HAL_SPI_ID_T id, HAL_SPI_CS_T csNum)
{
    ...
    g_halSpiHwpArray[id]->stream = SPI_TX_STREAM_MODE_DISABLED;
}
...
PUBLIC VOID hal_SpiForcePin(HAL_SPI_ID_T id, HAL_SPI_PIN_T pin,  HAL_SPI_PIN_STATE_T state)
{
    ...
    switch (pin)
    {
        case HAL_SPI_PIN_CS0  :
            g_halSpiHwpArray[id]->pin_control &= ~SPI_CS0_CTRL_MASK;
            g_halSpiHwpArray[id]->pin_control |= SPI_CS0_CTRL(state);
            break;

        case HAL_SPI_PIN_CS1  :
            g_halSpiHwpArray[id]->pin_control &= ~SPI_CS1_CTRL_MASK;
            g_halSpiHwpArray[id]->pin_control |= SPI_CS1_CTRL(state);
            break;

        case HAL_SPI_PIN_CS2  :
            g_halSpiHwpArray[id]->pin_control &= ~SPI_CS2_CTRL_MASK;
            g_halSpiHwpArray[id]->pin_control |= SPI_CS2_CTRL(state);
            break;

        case HAL_SPI_PIN_CS3  :
            g_halSpiHwpArray[id]->pin_control &= ~SPI_CS3_CTRL_MASK;
            g_halSpiHwpArray[id]->pin_control |= SPI_CS3_CTRL(state);
            break;

        case HAL_SPI_PIN_CLK  :
            g_halSpiHwpArray[id]->pin_control &= ~SPI_CLK_CTRL_MASK;
            g_halSpiHwpArray[id]->pin_control |= SPI_CLK_CTRL(state);
            break;

        case HAL_SPI_PIN_DO   :
            g_halSpiHwpArray[id]->pin_control &= ~SPI_DO_CTRL_MASK;
            g_halSpiHwpArray[id]->pin_control |= SPI_DO_CTRL(state);
            break;
...
#if (CHIP_SPI_HAS_DIO == 1)
    u.reg = GET_BITFIELD(g_halSpiHwpArray[id]->irq, SPI_IRQ_MASK);
...
#else
    u.reg = GET_BITFIELD(g_halSpiHwpArray[id]->status, SPI_IRQ_MASK);
...
PROTECTED VOID hal_SpiIrqHandler(UINT8 interruptId)
{
...
#if (CHIP_ASIC_ID == CHIP_ASIC_ID_GREENSTONE)
    HAL_SPI_ID_T id = interruptId - SYS_IRQ_SPI;
    ...
    cause.reg = g_halSpiHwpArray[id]->status;
    // clear all cause 
    g_halSpiHwpArray[id]->status = cause.reg;
