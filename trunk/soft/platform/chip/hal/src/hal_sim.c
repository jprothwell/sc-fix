...
////////////////////////////////////////////////////////////////////////////////
//
//  $HeadURL: http://svn.coolsand-tech.com/svn/developing1/Sources/chip/branches/gallite441/hal/src/hal_sim.c $
//    $Author: kongdq $
//    $Date: 2011-06-14 19:02:34 +0800 (Tue, 14 Jun 2011) $
//    $Revision: 8229 $
//
////////////////////////////////////////////////////////////////////////////////
//
...
#define sim_HstSendEvent
//#define sim_HstSendEvent    hal_HstSendEvent
#endif
// ============================================================================
//  MACROS
// ============================================================================
...
#else
	g_halSciMainDivVal = (newSysFreq/(2*SCI_CLK_MAX));
#endif  
...
PUBLIC VOID hal_SimSetDataConvFormat(UINT32 Ts)
{
    ...
    // Test if power down process has been done before (in case of previous SIM ARG failure)
    if((hwp_sci->Config & SCI_ENABLE) == 0)
    {
        // Recover to ready state.
        hwp_sci->Config |= SCI_RESET |
                           SCI_TX_RESEND_EN_H |
                           SCI_PARITY_EN |
                           SCI_PEGEN_LEN |
                           SCI_ENABLE;
                                ...
                                BOOL useIfc, BOOL stopClock)
{
    ...
    {
        channel =  HAL_UNKNOWN_CHANNEL; //=0x7 // 0xff;
    }
...
INLINE VOID hal_SimAutoReset(VOID)
{
    ...
    hwp_sci->Config &= ~(0x00ffffff & ~(SCI_RESET|SCI_LLI|SCI_MSBH_LSBL));
...
PUBLIC UINT8 hal_SimSetDbg(VOID)
{
...
//       hal_HstSendEvent(hwp_sci->Status & SCI_AUTOSTOP_RXCOUNTER_MASK );


}

PUBLIC UINT8 hal_SimStopClk(VOID)
{
	hwp_sci->Config &= ~SCI_CLOCKSTOP;

//       hal_HstSendEvent(0x33330000 );

}


PUBLIC UINT8 hal_SimStopSysClk(VOID)
{
	hwp_sysCtrl->Clk_Sys_Mode |= SYS_CTRL_MODE_SYSD_SCI_MANUAL;
}

PUBLIC UINT8 hal_SimResetSys(VOID)
{
...
{
    hal_SimClrConfigBit(0x00ffffff);
}
...
{
    // Disable everything, but keep the upper bits and SCI_RESET/SCI_LLI/SCI_MSBH_LSBL
    hal_SimClrConfigBit(0x00ffffff & ~(SCI_RESET|SCI_LLI|SCI_MSBH_LSBL));
    hal_SimSetConfigBit(SCI_CLOCKSTOP);  // Shut down Clk_SCI
}
...
#ifdef GALLITE_IS_CT1129
    UINT32 cri_status = 0;
//     static    UINT32 tx_count = 0;
#endif
//	sim_HstSendEvent(0x11100000 + nRxByte);
//    	hal_SimStopClk();  
...
//        sim_HstSendEvent(0x99900000 + tx_count);
...
{
 	hwp_sci->Config &= ~(0x00000fff); // clear all but upper bits
