...
/// @file spc_tx.c
///
/// This file contains Signal Processing Control Tx related functions.
///
/// @date 28/05/07
/// @author jba
//------------------------------------------------------------------------------
...
//=============================================================================
PRIVATE VOID spc_ResetCodec(VOID)
{
    UINT8 i=0;
...
//=============================================================================
PRIVATE VOID spc_DataEncode(UINT32 *InputBuffer)
{
    UINT32 *ptr32;
...
//=============================================================================
PRIVATE VOID spc_DataFacch(UINT32 *InputBuffer)
{
    UINT32 *ptr32;
    UINT8 TxBurstOffset=0;
    UINT8 i,j;
...
//=============================================================================
// spc_EgprsCCHTxProcess()
//-----------------------------------------------------------------------------
/// This function executes the CCH Tx processing for the current frame  
//=============================================================================
PROTECTED VOID spc_EgprsCCHTxProcess(UINT8 buffIdx)
{
#if (CHIP_EDGE_SUPPORTED==1)
    volatile SPC_MBOX_STATWIN_TX_CTX_T *BuffCtx;
    UINT32 *ItlvBuffAdd;    
    UINT32 *BuiltOutput;
    UINT32 BurstToSend[4*15];
    UINT8 bitMap=g_mailbox.pal2spc.win[g_spcCtx->currentSnap].tx.bitMap;
    UINT8 slotIdx;
    UINT8 slotNb=0;
    UINT8 burst_offset;
    ...
    SPC_PROFILE_FUNCTION_ENTER(spc_EgprsCCHTxProcess);
    ...
    for (slotIdx=0; slotIdx<slotNb; slotIdx++)
    {
        ...
        if ((g_mailbox.pal2spc.win[g_spcCtx->currentSnap].tx.bstIdx==0)&&
                (BuffCtx->encDone)&&
                (g_mailbox.pal2spc.statWin.loopMode==MBOX_LOOP_OPEN))
        {
            UINT8* ptr=(UINT8*) (g_spcCtx->NBlockDataIn[buffIdx]);
            UINT8 size=spcSizeFromMCS[BuffCtx->cs];
            BuffCtx->encDone=0;
            switch (BuffCtx->cs)
            {
                case EGPRS_MCS1:
                ...
                case EGPRS_MCS3:
                case EGPRS_MCS4:
                    g_spcCtx->tx_modulation=SPP_GMSK_MOD;
                    g_spcCtx->tx_headertype[slotIdx]=SPP_HEADER_TYPE3;
                    break;
                case EGPRS_MCS5:
                case EGPRS_MCS6:
                    g_spcCtx->tx_modulation=SPP_8PSK_MOD;
                    g_spcCtx->tx_headertype[slotIdx]=SPP_HEADER_TYPE2;
                    break;
                case EGPRS_MCS7:
                case EGPRS_MCS8:
                case EGPRS_MCS9:
                    g_spcCtx->tx_modulation=SPP_8PSK_MOD;
                    g_spcCtx->tx_headertype[slotIdx]=SPP_HEADER_TYPE1;
                    break;
                default:
                    g_spcCtx->tx_modulation=SPP_GMSK_MOD;
                    g_spcCtx->tx_headertype[slotIdx]=SPP_HEADER_TYPE3;
            ...
            // rewrite the RLC block from the format defined in PAL Emerald
            //  interface to the format supported by spp
            for (i=6;i<size+6;i++)
            {
                 ptr[i]=(ptr[i]&0x03)||(ptr[i+1]<<2);   
                 ptr[i+1] >>= 0x06;
            }
            if (g_spcCtx->tx_headertype[slotIdx] == SPP_HEADER_TYPE1)
            {
                ptr[6+size] |= (ptr[6+size+1]&0x03)<<2;
                ptr[6+size+1]=ptr[6+size+2];
                ptr[6+size] |= (ptr[6+size+1]&0x0F)<<4;
                ptr[6+size+1] >>= 0x04;
                
                for (i=6+size+1;i<6+size+size;i++)
                {
                    ptr[i+1]=ptr[i+2];
                    ptr[i]=(ptr[i]&0x0F)||(ptr[i+1]<<4);
                    ptr[i+1] >>= 0x04;  
                }
            }
            
            spp_EgprsEncodeBlock(g_spcCtx->NBlockDataIn[buffIdx]+6,
                            g_spcCtx->NBlockDataIn[buffIdx]+6+size+1,
                            g_spcCtx->NBlockDataIn[buffIdx],         // input
                            g_spcCtx->tx_headertype[slotIdx],
                            g_spcCtx->ItlvBufTxCtrl+slotIdx*SPC_ITLV_TX_BUFF_OFFSET);   // output
        ...
        g_spcCtx->Tx_off = FALSE;
        if(g_spcCtx->tx_modulation==SPP_GMSK_MOD)
        {
            burst_offset=4;
            BuiltOutput=&(BurstToSend[slotIdx*4]);
        }
        else
        {
            burst_offset=11;
            BuiltOutput=&(BurstToSend[slotIdx*15]);
        }

        if(g_mailbox.pal2spc.statWin.loopMode==MBOX_LOOP_CLOSE_EGPRS)
        {
            ItlvBuffAdd=(UINT32 *)((UINT32 *)(g_spcCtx->loopC_buffer) + 
            (g_mailbox.pal2spc.win[g_spcCtx->currentSnap].tx.bstIdx)*burst_offset);            
        }
        else
        { 
            ItlvBuffAdd=(UINT32 *)(g_spcCtx->ItlvBufTxCtrl + 
        ...
        spp_EgprsBuildNBurst(ItlvBuffAdd + SPC_ITLV_TX_BUFF_OFFSET*slotIdx,       // In
                        g_mailbox.pal2spc.win[g_spcCtx->currentSnap].tx.tsc,     // Tsc
                        g_spcCtx->tx_headertype[slotIdx],
                        BuiltOutput);     // Out
    ...
    spp_Modulation( BurstToSend,
                    ...
                    g_mailbox.pal2spc.rf.txBstMap.guardStart,
                    g_spcSwTxFifo,
                    g_spcCtx->tx_modulation,
                    TRUE);

    //------------------------------ MODULATION ---------------------------------   

    SPC_PROFILE_FUNCTION_EXIT(spc_EgprsCCHTxProcess);
#endif
}
...
PROTECTED VOID spc_TxProcess(VOID)
{
    ...
    {   
        case MBOX_CCH_CHN :
        case MBOX_PDCH_CHN: 
            if((cs==GSM_CS1)||(cs==GSM_CS2)||(cs==GSM_CS3)||(cs==GSM_CS4))
            {
                spc_CCHTxProcess(buffIdx);
            }
            else
            {
                spc_EgprsCCHTxProcess(buffIdx);
            }
        break;
        case MBOX_TCHF_CHN : 
        case MBOX_TCHH_CHN :
