...
#if (CHIP_ASIC_ID == CHIP_ASIC_ID_JADE)
	 hal_spi_Open((struct hal_SpiCfg*)spicfg);
#else
...
VOID DRV_SpiIsr(UINT16 status)
{
...
#if (CHIP_ASIC_ID == CHIP_ASIC_ID_JADE)
  if (status == HAL_SPI_IRQ_MSK_TX_TH)
#else
    union
    {
  ...
  {
#if (CHIP_ASIC_ID == CHIP_ASIC_ID_JADE)
    hal_SetUsrVectorMsk(HAL_SPI_IRQ, 0);
#else
...
#if (CHIP_ASIC_ID == CHIP_ASIC_ID_JADE)
  if (status == HAL_SPI_IRQ_MSK_RX_TH || status == HAL_SPI_IRQ_MSK_RX_OVF)
#else
...
{
#if (CHIP_ASIC_ID == CHIP_ASIC_ID_JADE)
  hal_SetUsrVector(HAL_SPI_IRQ, DRV_SpiIsr);
  hal_SetUsrVectorMsk(HAL_SPI_IRQ, mask);
#else
    union
    {
        HAL_SPI_IRQ_STATUS_T bfStatus;
