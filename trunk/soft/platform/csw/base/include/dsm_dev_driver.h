...
typedef INT32 (*PF_DEV_OPEN)( PCSTR pszDevName, INT32 *pDevHandle ); 
...
#define DRV_BLOCK_READ( dev_nr, blk_nr, buf  ) \
    g_pDevDescrip[ dev_nr ].bread( g_pDevDescrip[ dev_nr ].dev_handle, blk_nr, buf )

// Block write.
#define DRV_BLOCK_WRITE( dev_nr, blk_nr, buf  ) \
    g_pDevDescrip[ dev_nr ].bwrite( g_pDevDescrip[ dev_nr ].dev_handle, blk_nr, buf )
