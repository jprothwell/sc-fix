...
/*建立字符集{'0'~'9','A'~'F','a'~'f'}到整数子集[0~15]的映射*/
#define HTOB( h ) (( h ) <= '9' ? (( h ) - '0' ) : (( h ) <= 'F' ? (( h ) - 'A' + 10 ) : (( h ) - 'a' + 10 )) )

/*建立整数子集[0~15]到字符集{'0'~'9','A'~'F'}的映射*/
#define BTOH( b ) (( b ) <= 9 ? (( b ) + '0' ) : (( b ) - 10 + 'A' ))

#define CHAR_IS_HEX( c ) ( ('0' <= (c) && (c) <= '9') || ('A' <= (c) && (c) <= 'F') || ('a' <= (c) && (c) <= 'f'))
...
UINT32 vds_CRC32( CONST VOID *pvData, INT32 iLen )
{
    UINT32 iVal = 0;
...
BOOL vds_IsCRCChecked( UINT8* pBuff, UINT32 iLen, UINT32 iCRC, UINT32 iMode )
{
    ...
    if ( VDS_FLAG_CHECK == iMode )
    {
        iResult = vds_CRC32( pBuff, iLen );
        return ( iResult == iCRC ) ? TRUE : FALSE;
    }
...
BOOL vds_GetLineFromStr( char *line, int n, char *str, int *offset )
{
    ...
    if ( NULL_CHAR == *substr )
    {
        D( ( DL_WARNING, "terminal character of the sting encountered" ) );
        return FALSE;
    }

    p = DSM_StrChr( substr, '\n' );
    if ( !p )
        p = DSM_StrChr( substr, '\0' );

    line_len = p - substr; /* newline charactor is NOT included */
    DSM_MemCpy( line, substr, MIN( line_len, n - LEN_FOR_NULL_CHAR ) );
    line[ MIN( line_len, n - LEN_FOR_NULL_CHAR ) ] = NULL_CHAR;
    *offset += line_len + 1;
...
// ASSUME that the next field of the VDS_SECT_REF{} is initialized as
//VDS_INVALID_POINTER(0xFFFFFFFF)
...
void vds_str_to_hex( char *str, long *len, char *buf )
{
    ...
    for ( hex_counter = 0, q = str; NULL_CHAR != *q; q++ )
    {
        if ( CHAR_IS_HEX( *q ) )
            hex_counter++;
    }
    ...
    for ( q = str, low = high = -1; NULL_CHAR != *q; q++ )
    {
        if ( CHAR_IS_HEX( *q ) )
        {
            if ( -1 == high )
...
void vds_hex_to_str( char *hex, long len, char *buf, char separator )
{
    ...
    while ( len )
    {
        *p = (char)BTOH( ( ( UINT8 ) * hex ) >> 4 );
        *( p + 1 ) = (char)BTOH( ( ( UINT8 ) * hex ) & 0xf );
        len--;
        hex++;
        if ( NULL_CHAR != separator )
        {
            *( p + 2 ) = separator;
            p += 3;
        }
