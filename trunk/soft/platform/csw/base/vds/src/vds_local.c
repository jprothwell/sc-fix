...
INT32 vds_SetSectStatus(UINT16 iSN, UINT32 iStatus )
{
    ...
    if ( status != iStatus )
    {
        D( ( DL_ERROR, "In vds_SetSectStatus, _ERR_VDS_WRITE_CHECK_FAILED!\n" ) );
...
INT32 vds_SetPBStatus( VDS_PARTITION *psPart, UINT16 iSN, UINT16 iPBN, UINT16 iStatus )
{
    ...
    {
        D( ( DL_ERROR, "vds_SetPBStatus: iSN = %d, iPBN = %d", iSN, iPBN ) );
        return _ERR_VDS_PARAM_ERROR;
    }
    ...
    {       
        D( ( DL_ERROR, "vds_SetPBStatus: Call  DRV_WriteFlash failed,iRet = %d,write size = %d,written size = %d.",iRet,VDS_SZ_PBD_STATUS,iWriteSize) );
        D( ( DL_ERROR, "vds_SetPBStatus: Write status failed: original status 0x%x, status to set: 0x%x.", iOriginalStatus, iStatus ) );
        return _ERR_VDS_WRITE_FLASH_FAILED;
    }
    ...
    if ( iRet != ERR_SUCCESS || iReadSize != VDS_SZ_PBD_STATUS )
    {        
         D( ( DL_ERROR, "vds_SetPBStatus: call DRV_ReadFlash failed. addr = 0x%x, iRet = %d,read size= %d, read out  = %d.", 
                  iPbdtAddr + iPbdOffset + VDS_POS_PBD_STATUS,
                 iRet,
                 VDS_SZ_PBD,
                 iReadSize ));    
    ...
    {
        D( ( DL_ERROR, "vds_SetPBStatus: write check failed: original status 0x%x, status to set: 0x%x, status readback: 0x%x,addr = 0x%x.", 
                 ...
                 VDS_PBD *pPBD, UINT16 iSectNo, UINT16 iPBN)
{
    ...
    {
        D( ( DL_ERROR, "In vds_SetMT, vbn overflow( %d, %d ).\n", pPBD->iVBN, VDS_NR_MT_VALID_ENTRY( psCP )) );
    ...
    { 
        ...
        else            
        {
            ...
            else
            {
                ...
                {
                    D( ( DL_ERROR, "In vds_SetMT, vds_ReadPBD return failed, ErrCode = %d.\n", iRet ) );
                    iRet = iRet;
                }
            }
        }
...
INT32 vds_SetMTByScanSec( VDS_PARTITION *psCP,UINT16 iSectNo,UINT16 *iNrBlkScaaned)
{
    ...
    while ( iPbn < VDS_NR_PB_PER_SECT( psCP ) )
    {
        if ( VDS_NR_PBD_PER_GROUP <= iIndexIngrp )
        {
            ...
            if ( iRet < 0 )
            {
                D( ( DL_ERROR, "In vds_SetMTByScanSec, vds_ReadPBDT return failed, ErrCode = %d\n", iRet ) );
                return iRet;
            }
            iIndexIngrp = 0;
        }
        ...
        {         
            ...
            {                     
                ...
                if ( ERR_SUCCESS != iRet )
                {
                    D( ( DL_ERROR, "In vds_SetMTByScanSec, vds_SetMT return failed, ErrCode = %d\n", iRet ) );
                    return iRet;
                }
            }
        }
...
INT32 vds_MoveCurrentPos( VDS_PARTITION* psVDSPart )
{
    ...
    if ( NULL != psVDSPart->pCurrentSect && psVDSPart->pCurrentSect->iCurrentPBN == ( UINT16 ) ( lNrPbPerSec - 1 ) )
    {
        ...
        {
            D( ( DL_ERROR, "In MoveCurrentPos, SetSectStatus1 ERR,ErrCode = %d,wSN = %d\n", lErrCode,wSN) );
            return _ERR_VDS_PARAM_ERROR;
        }
    ...
    {       
        while ( NULL == psVDSPart->pCurrentSect )
        {
            ...
            {
                D( ( DL_ERROR, "In MoveCurrentPos, vds_Gc ERR,ErrCode = %d.\n", lResult) );
...
INT32 vds_Pop( VDS_PARTITION* psVDSPart, VDS_SECT_REF** list )
{
    ...
    if ( ERR_SUCCESS != lErrCode )
    {
        D( ( DL_ERROR, "In vds_GetPos,ReadSH erro, ErrCode = %d,", lErrCode ) );
        D( ( DL_ERROR, "sSh.iInvalidBlockCount=%d,sSh. iMagic=0x%x, sSh.iSectorStatus=0x%x\n", sSH.iEraseCounter, sSH.iMagic, sSH.iSectorStatus ) );
    ...
    if ( ERR_SUCCESS != lErrCode )
    {
        D( ( DL_ERROR, "In vds_GetPos,WriteSH erro, ErrCode = %d,", lErrCode ) );
        D( ( DL_ERROR, "sSh.iInvalidBlockCount=%d,sSh. iMagic=0x%x, sSh.iSectorStatus=0x%x\n", sSH.iEraseCounter, sSH.iMagic, sSH.iSectorStatus ) );
...
INT32 vds_ReadPBD( VDS_PARTITION* psCP, UINT16 iSN, UINT16 iPBN, VDS_PBD*pPBD )
{
         ...
         (NULL != pPBD))
    {
        ...
        {
            D( ( DL_ERROR, "vds_ReadPBD: call DRV_ReadFlash failed. addr = 0x%x, iRet = %d,read size= %d, read out  = %d.", 
                 lAddr,
                 lRet,
                 VDS_SZ_PBD,
                 lRSize ));    
    ...
    {
        D( ( DL_ERROR, "In vds_ReadPBD,_ERR_VDS_PARAM_ERROR" ) );
...
INT32 vds_WritePBD( VDS_PARTITION* psCP, UINT16 iSN, UINT16 iPBN, VDS_PBD* pPBD )
{
         ...
         ( NULL != pPBD ))
    {
        ...
        {
            D( ( DL_ERROR, "vds_WritePBD: Call  DRV_WriteFlash failed,iRet = %d,write size = %d,written size = %d.",lRet,VDS_SZ_PBD,lWSize) );
...
INT32 vds_ReadPB( VDS_PARTITION *psCP, UINT16 iSN, UINT16 iPBN, UINT8* pBuff )
{
    ...
    else
    {
        ...
        {
            D( ( DL_ERROR, "vds_ReadPB: call DRV_ReadFlash failed. addr = 0x%x, iRet = %d,read size= %d, read out  = %d.", 
                 uAddr,
                 lResult,
                 uLen,
                 uReadLen ));    
...
INT32 vds_WritePB( VDS_PARTITION *psCP, UINT16 iSN, UINT16 iPBN, UINT8 *pBuff )
{
    ...
    else
    {
        ...
        */
        ...
        {
            D( ( DL_ERROR, "vds_WritePB: Call  DRV_WriteFlash failed,iRet = %d,write size = %d,written size = %d.",lResult,uLen,uWriteLen) );
            lRet = _ERR_VDS_WRITE_FLASH_FAILED;
        }
    }
    return lRet;
...
INT32 vds_ReadPBDT( VDS_PARTITION* psCP, UINT16 iSN, UINT16 iBegIndex, UINT16 iNum, VOID* pBuf )
{
  ...
  //if ( ( NULL == psCP ) || ( iSN >= VDS_NR_SECT( psCP ) ) )
	if ( NULL == psCP  )
    {
        D( ( DL_ERROR, "In vds_ReadPBDT, _ERR_VDS_PARAM_ERROR.\n") );
        return _ERR_VDS_PARAM_ERROR;
    }
    ...
    {
         D( ( DL_ERROR, "vds_ReadPBDT: call DRV_ReadFlash failed. addr = 0x%x, iRet = %d,read size= %d, read out  = %d.", 
...
INT32 vds_ReadSHWithCrc( VDS_PARTITION *psCP, UINT16 iSN, VDS_SH *psSH )
{
    ...
    {
        D( ( DL_ERROR, "vds_ReadSHWithCrc: call DRV_ReadFlash failed. addr = 0x%x, iRet = %d,read size= %d, read out  = %d.", 
                 saddr,
                 err_code,
                 VDS_SZ_SH,
                 rsize ));             
    ...
    if ( iCrc != psSH->iCRC )
    {
        D( ( DL_ERROR, "vds_ReadSHWithCrc: saddr = 0x%x, crc = 0x%x, calculated crc: 0x%x\n", 
...
INT32 vds_WriteSH( VDS_PARTITION *psCP, UINT16 iSN, VDS_SH *psSH )
{
   ...
   */
    ...
    {    
        D( ( DL_ERROR, "vds_WriteSH:call DRV_WriteFlash failed.error code = %d,saddr = %d,read size = %d, out size = %d.\n", 
             err_code,saddr,VDS_SZ_SH,rsize));
        return _ERR_VDS_WRITE_FLASH_FAILED;
    }
    rsize = 0;
    ...
    {
        D( ( DL_ERROR, "Checking write no pass.mem magic = 0x%x,flash magic = 0x%x,mem status = 0x%x,flash status = 0x%x.\n", 
...
INT32 vds_WritePBDAndPB( VDS_PARTITION *psCP, UINT16 iSN, UINT16 iPBN, VDS_PBD* psPBD, UINT8* pPB )
{
 ...
 //   UINT8 szWBefor[512]; 
    ...
    {
        D( ( DL_ERROR, "In vds_WritePBDAndPB,PARA_ERR3,\n" ) );
        return _ERR_VDS_PARAM_ERROR;
    }
    if (FALSE == vds_IsValidSn(psCP,iSN) )
    {
        D( ( DL_ERROR, "In vds_WritePBDAndPB,sn is invalid.iSN = 0x%x.", iSN) );
        return _ERR_VDS_PARAM_ERROR;
    }
    ...
    if ( iPBN >= lPBCount )
    {
        D( ( DL_ERROR, "In vds_WritePBDAndPB,PARA_ERR1,iPBN = 0x%x, iPBCount = 0x%x\n", iPBN, lPBCount ) );
        return _ERR_VDS_PARAM_ERROR;
    }
    ...
    {
        D( ( DL_ERROR, "In vds_WritePBDAndPB,SETPBStatus ERR1,iSN = 0x%x, iPBN = 0x%x,lStatus = 0x%x\n", iSN, iPBN, lStatus ) );
    ...
    if ( ERR_SUCCESS != lResult )
    {
       // sxr_Sleep(50);
        //
        D( ( DL_ERROR, "In vds_WritePBDAndPB,write pb failed,err code = %d,SN = 0x%x, iPBN = 0x%x\n", lResult,iSN, iPBN ) );
        lStatus = VDS_STATUS_PB_INVALID;
        // Todo ..to dubug download smm no enough space!.
        lResult = vds_SetPBStatus( psCP, iSN, iPBN, lStatus );
        if(lResult != ERR_SUCCESS)
        {
             D( ( DL_ERROR, "In vds_WritePBDAndPB,set pb status failed.err code = %d,iSN = 0x%x, iPBN = 0x%x\n", lResult,iSN, iPBN ) );    
        }
      //  sxr_Sleep(50);
        return _ERR_VDS_WRITE_FLASH_FAILED;
    }
    ...
    if ( ERR_SUCCESS != lResult )
    {
        D( ( DL_ERROR, "In vds_WritePBDAndPB,WritePBD ERR, ErrCode = %d,iSN = 0x%x, iPBN = 0x%x\n",lResult, iSN, iPBN ) );
...
INT32 vds_ListScan( VDS_PARTITION* psCP, UINT32 uStatus )
{
    ...
    if ( ( NULL == psCP ) || ( ( uStatus != VDS_STATUS_SECT_CURRENT ) && ( uStatus != VDS_STATUS_SECT_DIRTY ) &&(uStatus != VDS_STATUS_SECT_GCING )) )
    {
        ...
        D( ( DL_ERROR, "In vds_ListScan, ErrCode = %d\n", lErrCode ) );
        return lErrCode;
    }
    ...
    while ( NULL != psRef )
    {
        ...
        if ( ERR_SUCCESS != lErrCode )
        {
            D( ( DL_ERROR, "In vds_ListScan, Update map table by scanning sector failed, ErrCode = %d\n", lErrCode ) );
            return lErrCode;
        }
        if ( VDS_STATUS_SECT_DIRTY == uStatus )
        {
...
INT32 vds_SetPBDCRC( VDS_PARTITION *psCP, UINT16 iSN, UINT16 iPBN, UINT32 iCRC )
{
    ...
    {
        D( ( DL_ERROR, "vds_SetPBDCRC:call DRV_WriteFlash failed.error code = %d,read size = %d, out size = %d.\n", 
    ...
    if ( iCRCRead != iCRC )
    {
         D( ( DL_ERROR, "vds_SetPBDCRC: call DRV_ReadFlash failed. addr = 0x%x, iRet = %d,read size= %d, read out  = %d.", 
...
VOID vds_SetFlushAlloc(VOID)
{
    g_iFlushAlloc = 1;
}


VOID vds_SetFlushUnAlloc(VOID)
{
    g_iFlushAlloc = 0;
}


BOOL vds_IsFlushAlloc(VOID)
{
    return g_iFlushAlloc == 1 ? TRUE:FALSE;
}
