...
INT32 VDS_GetFlashInfo( VDS_FLASH_INFO *psInfo )
{
    ...
    {
       D( ( DL_ERROR,"DRV_FlashRegionMap failing,return:0x%x.",iResult));
...
{    
    ...
    if ( ERR_SUCCESS != iResult )
    {
        D( ( DL_ERROR, "vds init: get flash information failed err code = %d.", iResult ));
    ...
    for(i = 0; i < pDsmConfig->dsmPartitionNumber; i++)
    {
        ...
        {
            D( ( DL_ERROR, "vds_InitEntry: partition configure error. rsv_blk_cnt > all_blk_cnt." ));
        ...
        if(NULL == psCP->psMT )
        {
            D( ( DL_ERROR, "VDS_PartitionInit: malloc error,psCP->psMT = NULL." ));
    ...
    if ( NULL == g_psSectRef)
    {
        D( ( DL_ERROR, "vds init:sec_ref malloc failed size = %d.",VDS_NR_SECT( psCP ) * sizeof( VDS_SECT_REF ) ));
        iRet = _ERR_VDS_NO_MORE_MEM;
    ...
    for(i = 0; i < iSectorCnt; i++)
    {
        ...
        {
            D( ( DL_ERROR, "vds init:  call DRV_ReadFlash failed return. addr = 0x%x, iRet = %d,read size= %d, readed size  = %d.", 
        ...
        if(NULL == psRef)
        {
            D( ( DL_ERROR, "vds init: alloc a  sec_ref failed,index = %d, addr = 0x%x.", 
                i,
                iSecAddr       
                ));
...
INT32 vds_EraseSec(UINT32 iSecAddr)
{
    ...
    if(ERR_SUCCESS == iResult && VDS_SZ_SH == iRSize)
    {
        ...
        if(ERR_SUCCESS == iResult)
        {
            ...
            {
                D( ( DL_ERROR, "Erase sector:write sector failed!err code = %d,addr = 0x%x.",iResult,iSecAddr));
        ...
        {
            D( ( DL_ERROR, "Erase sector:erase sector failed!err code = %d,addr = 0x%x.",iResult,iSecAddr));
...
INT32 vds_FormatSec(UINT32 iSecAddr,VDS_PARTITION_INFO sPartInfo)
{
    ...
    {
        D( ( DL_ERROR, "vds_format_sector: DRV_ReadFlash failed.iRet = %d, sect_addr = 0x%x,reading size = %d,readed size = %d.", iRet, iSecAddr,VDS_SZ_SH,rsize));
    ...
    {
        D( ( DL_ERROR, "vds_format_sector: DRV_WriteFlash failed.iRet = %d, sect_addr = 0x%x,writing size = %d,written size = %d.", iRet, iSecAddr,VDS_SZ_SH,wsize));
        return _ERR_VDS_WRITE_FLASH_FAILED;
    }
    else
    {
