...
INT32 VDS_Init(VOID)
{
    ...
    {
        D( ( DL_ERROR, "Initiate VDS failed! errorcode = %d.", iErrCode));
        return iErrCode;
    }
    else 
    {      
...
INT32 VDS_Open(PCSTR pszPartName, INT32 *pDevHandle )
{
    ...
    D( ( DL_ERROR, "VDS_Open: No Such partition(%s)", pszPartName ));
...
INT32 VDS_WriteBlock( UINT32 iPartId, UINT32 iVBNum, UINT8 *pBuff )
{
    ...
    {
        D( ( DL_ERROR, "In VDS_WriteBlock,PARA_ERR.DevHd = 0x%x\n", iPartId ) );
        return ERR_VDS_PARA_ERROR;
    }
    psCP = &g_VDS_partition_table[ iPartId ];
    if ( psCP->sPartInfo.iFlashSectorCount != psCP->iRefCount )
    {
        D( ( DL_ERROR, "In VDS_ReadBlock, partition[%d] not initialized.\n", iPartId ) );
    ...
    {
        D( ( DL_ERROR, "In VDS_WriteBlock, VBNum overflow(%d, %d)\n", iVBNum, VDS_NR_MT_VALID_ENTRY( psCP ) ) );
        return ERR_VDS_PARA_ERROR;
    }
    ...
    if ( ERR_SUCCESS != lResult )
    {
        ...
        {
            D( ( DL_ERROR, "In VDS_WriteBlock,GetCurrentPos_ERR, ErrCode = %d.\n", lResult ) );
            VDS_ReleaseSemaphore();
    ...
    if ( ( VDS_SECT_NONE != wOldSN ) && ( VDS_BLOCK_NONE != wOldPBN ) )
    {
        ...
        if ( ERR_SUCCESS != lResult )
        {
            D( ( DL_ERROR, "In VDS_WriteBlock,ReadPBD_ERR1 ERR_CODE=%d, wOldSN=%d, wOldPBN=%d\n", lResult, wOldSN, wOldPBN ) );
            D( ( DL_ERROR, "PBCRC=0x%x, PbdCRC=0x%x, sPBD.iStatus=0x%x, sPBD.iVBN=0x%x, sPBD.iVersion=%d\n", sPBD.iPBCRC, sPBD.iPBCRC, sPBD.iStatus, sPBD.iVBN, sPBD.iVersion ) );
            D( ( DL_ERROR, "vb number: %d\n", iVBNum ) );
            VDS_ReleaseSemaphore();
    ...
    if ( ERR_SUCCESS != lResult )
    {
        D( ( DL_ERROR, "In VDS_WriteBlock,WriteBlock_ERR, ErrCode = %d,wNewSN = %d,wNewPBN=%d\n", lResult, wNewSN, wNewPBN ) );
        D( ( DL_ERROR, "sPBD.iPbCRC=%d, sPBD.iPbdCRC=%d, sPBD.iStatus=0x%x, sPBD.iVBN=0x%x, sPBD.iVersion=%d\n", sPBD.iPBCRC, sPBD.iPBDCRC, sPBD.iStatus, sPBD.iVBN, sPBD.iVersion ) );
        VDS_ReleaseSemaphore();
        return ERR_VDS_FLASH_DRV_WRITE;
    }
    if ( ( VDS_SECT_NONE == wOldSN ) || ( VDS_BLOCK_NONE == wOldPBN ) )
    ...
    else
    {
        ...
        {
            D( ( DL_ERROR, "In VDS_Init, SetPBStatus ERR, ErrCode = %d, sn =%d, pbn=%d\n", lResult, wOldSN, wOldPBN ) );
            VDS_ReleaseSemaphore();
            return ERR_VDS_FLASH_DRV_WRITE;
        }
        else
...
INT32 VDS_ReadBlock( UINT32 iPartId, UINT32 iVBNum, UINT8* pBuff )
{
    ...
    if ( iPartId >= VDS_GetPartitionCount() )
    {
        D( ( DL_ERROR, "In VDS_ReadBlock,PARA_ERR.DevHd = 0x%x\n", iPartId ) );
        return ERR_VDS_PARA_ERROR;
    }
    psCP = &g_VDS_partition_table[ iPartId ];
    if ( psCP->sPartInfo.iFlashSectorCount != psCP->iRefCount )
    ...
    {
        D( ( DL_ERROR, "In VDS_ReadBlock,PARA_ERR.VBNum = %d\n", iVBNum ) );
        VDS_ReleaseSemaphore();
        return ERR_VDS_PARA_ERROR;
    }
    if ( NULL == pBuff )
    ...
    if ( lResult != ERR_SUCCESS )
    {
        D( ( DL_ERROR, "In VDS_ReadBlock,ReadPBDERR.ErrCode = %d,SN=%d, PBN=%d\n", lResult, wSN, wPBN ) );
        VDS_ReleaseSemaphore();
        return ERR_VDS_FLASH_DRV_READ;
    }
    if ( VDS_STATUS_PB_VALID != sPBD.iStatus )
    {
        D( ( DL_ERROR, "In VDS_ReadBlock,PBD_STATUS_ERR. sPBD.iStatus = 0x%x.(iPartId = %d,iVBNum = %d,wSN = %d, wPBN = %d.)\n", sPBD.iStatus,iPartId,iVBNum, wSN, wPBN) );
    ...
    if ( ERR_SUCCESS != lResult )
    {
        D( ( DL_ERROR, "In VDS_ReadBlock,ReadPB_ERR. ErrCode = %d,wSN=%d, wPBN=%d\n", lResult, wSN, wPBN ) );
        VDS_ReleaseSemaphore();
        return ERR_VDS_FLASH_DRV_READ;
    }
    ...
    if ( !bCRCResult )
    {
        D( ( DL_ERROR, "In VDS_ReadBlock,PB CRC check failed. SN = %d, PBN = %d\n", wSN, wPBN ) );
        VDS_ReleaseSemaphore();
...
INT32 VDS_RevertBlock( UINT32 iPartId, UINT32 iVBNum)
{
    ...
    if ( iPartId >= VDS_GetPartitionCount() )
    {
        D( ( DL_ERROR, "In VDS_RevertBlock,PARA_ERR.DevHd = 0x%x\n", iPartId ) );
        return ERR_VDS_PARA_ERROR;
    }
    psCP = &g_VDS_partition_table[ iPartId ];
    if ( psCP->sPartInfo.iFlashSectorCount != psCP->iRefCount )
    {
        D( ( DL_ERROR, "In VDS_RevertBlock, partition[%d] not initialized.\n", iPartId ) );
        return ERR_VDS_UNINIT;
    }
    ...
    if ( iVBNum >= VDS_NR_MT_VALID_ENTRY( psCP )  )
    {
        D( ( DL_ERROR, "In VDS_RevertBlock, VBNum overflow(%d, %d)\n", iVBNum, VDS_NR_MT_VALID_ENTRY( psCP ) ) );
        return ERR_VDS_PARA_ERROR;
    }
    ...
    if ( ( VDS_SECT_NONE != wSN ) && ( VDS_BLOCK_NONE != wPBN ) )
    {
        ...
        if ( ERR_SUCCESS != lResult )
        {
            D( ( DL_ERROR, "In VDS_RevertBlock,vds_WritePBD ERR_CODE=%d, wOldSN=%d, wOldPBN=%d\n", lResult, wSN, wPBN ) );
            D( ( DL_ERROR, "vb number: %d\n", iVBNum ) );
            VDS_ReleaseSemaphore();
            return ERR_VDS_FLASH_DRV_WRITE;
        }        
