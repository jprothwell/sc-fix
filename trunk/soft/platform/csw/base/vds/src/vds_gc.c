...
VDS_SECT_REF* vds_GetRemainSector( VDS_PARTITION *psPart )
{
    ...
    {                
        ...
        if ( ERR_SUCCESS != iRet )
        {
            D( ( DL_ERROR, "vds get remain sector:change sector status failed, iRet = %d\n", iRet ) );
            return NULL;
        }
    }
    else
    ...
    if ( VDS_BLOCK_NONE == remain->iCurrentPBN )
    {
        ...
        {
            D( ( DL_ERROR, "In vds_GetRemainSector, call vds_GetFirstBlockPos return failed, iRet = %d\n", iRet ) );
            return NULL;
        }
    }
    return remain;
                                   ...
                                   VDS_SECT_REF *psSect, UINT32 iOldStatus, UINT32 iNewStatus )
{
    ...
    switch ( iNewStatus )
    {
        case VDS_STATUS_SECT_FREE:
            ...
            if ( ERR_SUCCESS != iRet )
            {
                D( ( DL_ERROR, "change sect statues: read SH failed,err code = %d.", iRet  ) );
            ...
            if ( ERR_SUCCESS != iRet )
            {
                D( ( DL_ERROR, "change sect statues:write sh failed.err code = %d,sn = %d.", iRet,psSect->iSN) );
            ...
            if ( ERR_SUCCESS != iRet )
            {
                D( ( DL_ERROR, "change sect statues:(->free)erase sector failed.err code = %d,addr = %d.", iRet,iSecAddr) );
        ...
        case VDS_STATUS_SECT_GCING:
            if ( VDS_STATUS_SECT_DIRTY == iOldStatus )
            {
                ...
                if ( ERR_SUCCESS != iRet )
                {
                    D( ( DL_ERROR, "change sect statues:set the sector statues(dirty -> gcing) failed.err code = %d,sn = %d.", iRet,psPart->pGcingSect->iSN) );                    
        ...
        case VDS_STATUS_SECT_CURRENT:
            ...
            if ( ERR_SUCCESS != iRet )
            {
                D( ( DL_ERROR, "change sect statues:set the sector statues(gcdest -> current) failed.err code = %d,sn = %d.", iRet,psPart->pGcingSect->iSN) );                    
                return _ERR_VDS_CHANGE_STATUS_FAILED;
            }
            psSect->iCurrentPBN--;
        ...
        case VDS_STATUS_SECT_DIRTY:
            ...
            if ( ERR_SUCCESS != iRet )
            {
                D( ( DL_ERROR, "change sect statues:set the sector statues(remain -> dirty) failed.err code = %d,sn = %d.", iRet,psPart->pGcingSect->iSN) );
                return _ERR_VDS_CHANGE_STATUS_FAILED;
            }
       ...
       case VDS_STATUS_SECT_TEMP:
            ...
            if ( ERR_SUCCESS != iRet )
            {
                D( ( DL_ERROR, "Change sector status failed*********, %d", iRet  ) );
                return _ERR_VDS_WRITE_SH_FAILED;
            }
            psSect->iCurrentPBN = VDS_BLOCK_NONE;      
        ...
        case VDS_STATUS_SECT_REMAIN:
            ...
            if ( ERR_SUCCESS != iRet )
            {
                D( ( DL_ERROR, "change sect statues:wite sh failed.err code = %d,sn = %d.", iRet,psSect->iSN) );
                return _ERR_VDS_WRITE_SH_FAILED;
            }               
            ...
            if ( ERR_SUCCESS != iRet )
            {
                D( ( DL_ERROR, "change sect statues:(->remain)erase sector failed.err code = %d,addr = %d.", iRet,iSecAddr) );
                return _ERR_VDS_ERASE_FLASH_FAILED;
            } 
...
INT32 vds_GcInit(VDS_PARTITION *psPart)
{
    ...
    if ( NULL == psPart )
    {    
        D( ( DL_ERROR, "In vds_Gc, \"psPart\" paramater null." ) );     
    ...
    {
        D( ( DL_ERROR, "In vds_Gc, partition not initialized." ) );
    ...
    {        
        D( ( DL_ERROR, "In vds_Gc, get gcing sector failed!" ) );
    ...
    {
        D( ( DL_ERROR, "In vds_Gc, get remain sector failed!" ) );
...
INT32 vds_GcCopy(VDS_PARTITION *psPart,UINT32 iCount)
{
    ...
    UINT32 i;
     ...
     else
     {
         ...
         D( ( DL_ERROR, "In vds_Gc: gc buffer = NULL.") );
     ...
     do
     {
        ...
        if(iResult < 0)
        {
            D( ( DL_ERROR, "In vds_Gc, Read  PBDT failed,err code = %d.",iResult ) );
        ...
        for(i = 0; i < iGroupCount; i ++)
        {
            ...
            else if(VDS_STATUS_PB_VALID == pTmpPbd->iStatus)
            {
                 ...
                 {
                    D( ( DL_ERROR, "In vds_Gc, Read  PB failed,err code = %d.",iResult ) );
                    iRet = _ERR_VDS_READ_FLASH_FAILED;
                ...
                (VDS_STATUS_PB_FREE == pTmpPbd->iStatus))
            {
                ...
                {
                    D( ( DL_ERROR, "In vds_Gc, write PBD failed,err code = %d.",iResult ) );
                ...
                {
                    D( ( DL_ERROR, "In vds_Gc, write PB failed,err code = %d.",iResult ) );
                    iRet = _ERR_VDS_WRITE_FLASH_FAILED;
...
INT32 vds_GcSetMt(VDS_PARTITION *psPart)
{
    ...
    {        
        ...
        if ( ERR_SUCCESS != iRet )
        {
            D( ( DL_ERROR, "vds_GcSetMt: Change Temp2Current failed.sn = 0x%x,cur_pbn = 0x%x.",g_GcScane.psRemSect->iSN,g_GcScane.psRemSect->iCurrentPBN));
            return _ERR_VDS_CHANGE_STATUS_FAILED;
        }
    }
    else
    {
        D( ( DL_ERROR, "vds_GcSetMt: cur_pbn is error.sn = 0x%x,cur_pbn = 0x%x.",g_GcScane.psRemSect->iSN,g_GcScane.psRemSect->iCurrentPBN) );
    ...
    if (ERR_SUCCESS != iResult)
    {
        ...
        D(( DL_ERROR, "vds_GcSetMt: update map-table failed!sn = 0x%x,err_code = %d.",g_GcScane.psRemSect->iSN,iRet));
    ...
    else 
    {
        if(( UINT16 ) ( g_GcScane.psRemSect->iCurrentPBN + 1) != iScannedNr)
        {
            ...
            D( ( DL_ERROR, "vds_GcSetMt:vds_SetMTByScanSec temp sector is full.") );
            iRet =  _ERR_VDS_UPDATE_MT_FAILED;
        }
        else
        {
...
INT32 vds_GcEraseBegin(VDS_PARTITION *psPart)
{
   ...
   // UINT32 iWSize = 0;
    ...
    if ( ERR_SUCCESS != iRet )
    {
        D( ( DL_ERROR, "gc erase begin: read SH failed,err code = %d.", iRet  ) );
        return _ERR_VDS_READ_SH_FAILED;
    }
    ...
    if ( ERR_SUCCESS != iRet )
    {
        D( ( DL_ERROR, "Erase begin:wite sh failed.err code = %d,sn = %d.", iRet,g_GcScane.psGcSect->iSN) );
        return _ERR_VDS_WRITE_SH_FAILED;
    }              
    ...
    else
    {
        ...
        else
        {
            ...
            D( ( DL_ERROR, "Erase begin:erase sector failed!err code = %d,addr = 0x%x.",iResult,iSecAddr));
        }
...
INT32 vds_GcEraseResume(VDS_PARTITION *psPart)
{
   ...
   // UINT32 iWSize = 0;
    ...
    else
    {
        ...
        {
            iRet = _ERR_VDS_GC_ERASE;
            D( ( DL_ERROR, "Erase resume:erase sector failed!err code = %d,addr = 0x%x.",iResult,iSecAddr));
        }
...
INT32 vds_GcEraseFinish(VDS_PARTITION *psPart)
{
    ...
    {
        D( ( DL_ERROR, "Erase finish:write sector failed!err code = %d,addr = 0x%x,iWSize = 0x%x.",iResult,iSecAddr,iWSize));
        iRet = _ERR_VDS_GC_ERASE;
    }
    g_GcScane.psGcSect->iCurrentPBN = VDS_BLOCK_NONE;
    psPart->pGcingSect = NULL;
...
INT32 vds_Gc( VDS_PARTITION *psPart )
{
    ...
    {
        D( ( DL_ERROR, "GC<c>: change remain-sector(#%d) as dirty-sector.\n", psRemSect->iSN ) );
        ...
        // 强制转换是因为如果该扇区空的话，它的iCurrentPBN已经设成VDS_BLOCK_NONE。
    {
        ...
        {
            D( ( DL_ERROR, "In vds_Gc, vds_SetMTByScanSec update map-table return failed!iRet = %d, psRemSect->iSN = 0x%x, "\
