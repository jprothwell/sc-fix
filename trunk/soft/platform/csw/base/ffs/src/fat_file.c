                    ...
                    struct file * filp, INT8 * buf, INT32 len)
{
    ...
    while(!fat_is_last_cluster(sb, clunum))
    {
        ...
        secnum = secnum2;
  	 DSM_ASSERT(clunum >= 2,"clunum = %d.",clunum);	
...
INT32 fat_relate_inode_with_fatcache(FAT_CACHE *psFATCache, struct inode *psInode)
{
    ...
    if (NULL == (psTmpNode = DSM_MAlloc(sizeof(*psTmpNode))))
    {
        DSM_ASSERT(0,"fat_relate_inode_with_fatcache: 1.malloc(0x%x) failed.",(sizeof(*psTmpNode))); 
        return _ERR_FAT_MALLOC_FAILED;
    }
...
INT32 fat_synch_fat_cache(FAT_SB_INFO *sb_info, FAT_CACHE *psFATCache)
{
    ...
    if(DSM_GetDevType(sb_info->sb->s_dev) == DSM_MEM_DEV_TFLASH)    {
	 ...
	 CSW_TRACE(BASE_DSM_TS_ID,"#debug FE_ERR: fat_synch_fat_cache:sb->dev = 0x%x, psFATCache:dev = 0x%x,FATSecNum = %d,bDirty = %d!\n", 
              sb_info->sb->s_dev,
	       psFATCache->dev,
	 	psFATCache->FATSecNum, 
	 	psFATCache->bDirty);
	// sxr_Sleep(100);
    }
    #endif	 
                                                                ...
                                                                BOOL bSynchFlag, struct inode *psInode)
{
	...
	#if 0	  
	if(bufvalue32 == 0 && DSM_GetDevType(sb_info->sb->s_dev) == DSM_MEM_DEV_TFLASH)
	{
		//D((DL_FATERROR, "fat_set_fat_entry_value: fatent = 0x%x, bufvalue32 = 0,bSynchFlag = %d!\n", fatent,bSynchFlag));
		CSW_TRACE(BASE_DSM_TS_ID,"#debug FE_ERR: fat_set_fat_entry_value1:sb->dev = 0x%x,psFatBuf:dev = 0x%x, bufvalue32 = 0,FATSecNum = %d,bDirty = %d!\n", 
			sb_info->sb->s_dev,
			psFatCache->iDevNo,
			psFatCache->iSecNum,
			psFatCache->iDirty
...
INT32 fat_file_truncate(struct inode *psInode, UINT32 nFileSize)
{
    ...
    if (nPreClu != 0)
    {       
        DSM_ASSERT(nPreClu >= 2,"nPreClu= %d.",nPreClu);
