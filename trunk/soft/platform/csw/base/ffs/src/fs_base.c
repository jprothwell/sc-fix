...
struct fs_cfg_info g_fs_cfg = {FS_DEFAULT_NR_SUPER, FS_NR_TASK_OPEN_FILE_MAX};
...
INT32 fs_GlobalVarInit()
{
    ...
    g_FatCahcheNum = n;
    g_fs_super_block_table = DSM_MAlloc((SIZEOF(*g_fs_super_block_table)) * g_fs_cfg.uNrSuperBlock);
    if (NULL == g_fs_super_block_table)
    {
        D((DL_FATERROR, "fs_GlobalVarInit: malloc g_fs_super_block_table failed!"));
        DSM_ASSERT(0,"fs_GlobalVarInit: 3.malloc(0x%x) failed.",((SIZEOF(*g_fs_super_block_table)) * g_fs_cfg.uNrSuperBlock)); 
        return ERR_FS_NO_MORE_MEMORY;
    }
    DSM_MemZero(g_fs_super_block_table, (SIZEOF(*g_fs_super_block_table)) * g_fs_cfg.uNrSuperBlock);
    ...
    DSM_MemSet( g_fs_current.filp, 0x00, (UINT16)((sizeof( struct file * ) ) * g_fs_cfg.uNrOpenFileMax));
