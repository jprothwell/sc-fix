...
/*   Name            Date             Remaks                                  */
/*                                                                            */
/*   li xingpeng      2005/06/12         version 1.0                          */
/*                    time reference function                                 */
/*     chenwf           2006.6.25          version 1.0.1 with alarm
*/
/******************************************************************************/
#include "chip_id.h"
#include <cswtype.h>
...
INT32  REG_OpenKey(HKEY hKey,PCSTR pcSubKey,PHKEY phKey)
{
    hKey = hKey;
    pcSubKey = pcSubKey;
    phKey = phKey;
...
BOOL TM_SetTimeFormat(UINT16 timeformat)
{
    ...
    #ifdef TIM_REG_FFS_USED
    if(!TIM_WriteRegData("TM_FORMAT", (UINT8*)(&timeFormat), 4))
    {
...
PRIVATE BOOL TM_IsValidTime(CONST TM_SYSTEMTIME* pSysTime)
{
    ...
    {
        CSW_TRACE(BASE_TM_TS_ID, "=====SUL_TM Time Valid Year failed----\n");
	    CSW_PROFILE_FUNCTION_EXIT(TM_IsValidTime);
    ...
    {
        CSW_TRACE(BASE_TM_TS_ID, "=====SUL_TM Time Valid failed----\n");
	    CSW_PROFILE_FUNCTION_EXIT(TM_IsValidTime);
    ...
    CSW_TRACE(BASE_TM_TS_ID, "=====The time is valid----\n");
                               ...
                               UINT8 uTimeStringSize)
{
    ...
    {
        CSW_TRACE(BASE_TM_TS_ID,"TM_FormatSystemTime fuckly format ERR!!! \n");
        CSW_PROFILE_FUNCTION_EXIT(TM_FormatSystemTime);
        return FALSE;
    }
    if(nResFormatID == 0)
    ...
    if((pDateString != NULL) && (uDateStringSize != 0))
    {
        ...
        {
            //SUL_RETURN(ERR_INVALID_PARAMETER,(UINT32)-1);
            CSW_TRACE(CFW_SIM_TS_ID,"=============uDateStringSize < 26 failed \n");
            return FALSE;
        }
        #endif
        //return ERR_INVALID_PARAMETER;
    ...
    if((pTimeString != NULL) && (uTimeStringSize != 0))
    {
        ...
        {
            //SUL_RETURN(ERR_INVALID_PARAMETER,(UINT32)-1);
            CSW_TRACE(CFW_SIM_TS_ID,"=============Because uDateStringSize < 14 failed \n");
	        CSW_PROFILE_FUNCTION_EXIT(TM_FormatSystemTime);
            return FALSE;
        }
        ...
        {
            CSW_TRACE(BASE_TM_TS_ID,"TM_FormatSystemTime TM_FMT_TWELVE_TIME \n");
    ...
    {
        //SUL_RETURN(ERR_INVALID_PARAMETER,(UINT32)-1);
        CSW_TRACE(CFW_SIM_TS_ID,"=============Because pTimeString == NULL .... failed \n");
	    CSW_PROFILE_FUNCTION_EXIT(TM_FormatSystemTime);
        return FALSE;
    }
    //return ERR_INVALID_PARAMETER;
                             ...
                             UINT8 uTimeStringSize)
{
    ...
    if(TRUE != RET)
    {
        CSW_TRACE(CSW_TS_ID,"=============TM_FileTimeToSystemTime failed \n");
	    CSW_PROFILE_FUNCTION_EXIT(TM_FormatFileTime);
        return RET;
    }
    ...
    if(TRUE != RET)
    {
        CSW_TRACE(CSW_TS_ID,"=============TM_FormatSystemTime failed \n");
	    CSW_PROFILE_FUNCTION_EXIT(TM_FormatFileTime);
        return RET;
    }
...
BOOL TM_SetTimeZone(INT8 TimeZone)
{
    ...
    #ifdef TIM_REG_FFS_USED
    if(!TIM_WriteRegData("TM_ZONE", &TimeZone, 1))
    {
        TIM_SetTimePara(&paraBak);
        CSW_PROFILE_FUNCTION_EXIT(TM_SetTimeZone);
        return FALSE;
...
UINT32 TM_GetTime (VOID)
{
    ...
    CSW_PROFILE_FUNCTION_ENTER(TM_GetTime);
    if(!TIM_GetTimePara(&TM_Parameters))
    {
    ...
    RET = TM_Parameters.iRtcBase + hal_timRTC_GetTime();
#else
...
BOOL TM_GetSystemTime(TM_SYSTEMTIME * pSystemTime)
{
	...
	nFileTime.DateTime = TM_Parameters.iBase + hal_timRTC_GetTime ()/TIM_CLOCKS_PER_SECOND;
    RET = TM_FileTimeToSystemTime(nFileTime, pSystemTime);
    if(TRUE != RET)
    {
        CSW_PROFILE_FUNCTION_EXIT(TM_GetSystemTime);
...
)
{
	...
	pFileTime->DateTime= TM_Parameters.iBase + hal_timRTC_GetTime ()/TIM_CLOCKS_PER_SECOND;
...
#if (CHIP_ASIC_ID == CHIP_ASIC_ID_JADE)
    TM_FILETIME nFileTime;
    if(TRUE != TM_SystemTimeToFileTime(pSystemTime,&nFileTime))
    {
        CSW_PROFILE_FUNCTION_EXIT(TM_SetSystemTime);
        return FALSE;
    }

    if(!TM_ResetRTC((UINT32)(nFileTime.DateTime)))
	{
...
#endif
    CSW_TRACE(BASE_TM_TS_ID,"TM_SetSystemTime()==OK==\n");
    CSW_PROFILE_FUNCTION_EXIT(TM_SetSystemTime);
    return(TRUE);
}
...
#if (CHIP_ASIC_ID == CHIP_ASIC_ID_JADE)
            if((TimeTmp < 0) || (TimeTmp > 70))
            {
                CSW_PROFILE_FUNCTION_EXIT(TM_FormatDateTime);
                return(FALSE);
            }
            while(YearTmp < (TimeTmp + 30))
            {
                if(TM_IsLeapYear((UINT16)(YearTmp + TM_STARTYEAR)))
                {
                    TotalSecond = TotalSecond + 366 * 24 * 60 * 60;
...
#if (CHIP_ASIC_ID == CHIP_ASIC_ID_JADE)
    CSW_PROFILE_FUNCTION_EXIT(TM_GetTimeZoneTime);
    return(TM_Parameters.iBase + TM_Parameters.iZone * 60 * 60);
#else
     TM_FILETIME nFileTime;
...
BOOL TIM_OpenKey(HKEY* pHkey)
{

    CSW_PROFILE_FUNCTION_ENTER(TIM_OpenKey);

    if(REG_OpenKey(REG_HKEY_SYSTEM_ROOT, TEXT("Time"), pHkey) != ERR_SUCCESS)
    {
        CSW_TRACE(DEBUG_PRINT_SWITCH,"SUL_GetLastError()0x0%x \n",SUL_GetLastError());

        CSW_PROFILE_FUNCTION_EXIT(TIM_OpenKey);
        TIM_ERR_RETURN(TM_OPEN_REG_ERROR);
    }

    CSW_TRACE(DEBUG_PRINT_SWITCH,"TIM_OpenKey()==OK== \n");
    CSW_PROFILE_FUNCTION_EXIT(TIM_OpenKey);
    return TRUE;
}
...
BOOL TIM_CloseKey(HKEY hkey)
{
    CSW_PROFILE_FUNCTION_ENTER(TIM_CloseKey);
    if(REG_CloseKey(hkey) != ERR_SUCCESS)
    {
        CSW_PROFILE_FUNCTION_EXIT(TIM_CloseKey);
        TIM_ERR_RETURN(TM_CLOSE_REG_ERROR);
    }

    CSW_TRACE(DEBUG_PRINT_SWITCH,"REG_CloseKey()==OK== \n");
    CSW_PROFILE_FUNCTION_EXIT(TIM_CloseKey);
    return TRUE;
}
...
BOOL TIM_WriteBinSet(HKEY hkey,
                     INT8*  Ptext,
                     UINT8* stringtxt,
                     UINT32 length)
{
	...
	{
        UINT32 ret = SUL_GetLastError();
        CSW_TRACE(DEBUG_PRINT_SWITCH,"TIM_WriteBinSet()SUL_GetLastError ret = 0x0%x\n",ret);
        CSW_PROFILE_FUNCTION_EXIT(TIM_WriteBinSet);
        TIM_ERR_RETURN(TM_WRITE_REG_ERROR);
	}
    CSW_TRACE(DEBUG_PRINT_SWITCH,"TIM_WriteBinSet()==OK==\n");
    CSW_PROFILE_FUNCTION_EXIT(TIM_WriteBinSet);
                    ...
                    UINT32* plength)
{
    ...
    {
        UINT32 ret = SUL_GetLastError();
        CSW_TRACE(DEBUG_PRINT_SWITCH,"TIM_ReadBinSet()SUL_GetLastError ret = 0x0%x\n",ret);
        CSW_PROFILE_FUNCTION_EXIT(TIM_ReadBinSet);
        TIM_ERR_RETURN(TM_READ_REG_ERROR);
    }
    CSW_TRACE(DEBUG_PRINT_SWITCH,"TIM_ReadBinSet()==OK==\n");
...
BOOL TIM_WriteFlash(VOID)
{
    ...
    CSW_TRACE(DEBUG_PRINT_SWITCH,"TIM_WriteFlash()==111==\n");
...
PRIVATE BOOL TIM_ReadFlash(UINT8 *Pdata,UINT32 bytesize)
{
    ...
    CSW_TRACE(DEBUG_PRINT_SWITCH,"TIM_ReadFlash()==111==\n");
    if(!CII_Flash_Block_Init(TM_FLASH_DATA,Pdata,bytesize))
    {
        CSW_TRACE(DEBUG_PRINT_SWITCH,"TIM_ReadFlash()==fail==\n");
        CSW_PROFILE_FUNCTION_EXIT(TIM_ReadFlash);
        TIM_ERR_RETURN(TM_READ_FLASH_ERROR);
    }

    CSW_TRACE(DEBUG_PRINT_SWITCH,"TIM_ReadFlash()==OK==[%s] [%s]\n",&Pdata[694],Pdata);
    CSW_PROFILE_FUNCTION_EXIT(TIM_ReadFlash);
    return TRUE;
}
#endif
