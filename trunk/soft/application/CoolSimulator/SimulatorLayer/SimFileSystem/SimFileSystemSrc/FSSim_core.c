...
static void fssim_init()
{
    ...
    __finally {
        ...
        MMI_FS_MappingDrive(FSSIM_SYSTEM_DRIVE, FSSIM_REMAP_SYSTEM_DRIVE);
...
/*                                                                       */
/*************************************************************************/
static int fssim_match_pattern(const TCHAR *pattern, const TCHAR *name)
{
    ASSERT(pattern != NULL && name != NULL);

    while (*pattern != 0 && *name != 0) {

        if (!wcsncmp(pattern, L"*", 1)) {

            /* *pattern is a wild character */

            if (!wcsnicmp(pattern + 1, name, 1)) {

                /* character following the wild character matches */

                pattern++;
            }
        }

        if (!wcsnicmp(pattern, name, 1) || !wcsncmp(pattern, L"?", 1)) {
    ...
    if ((*name == 0) && (*pattern == 0 || (!wcsncmp(pattern, L"*", 1) && *(pattern + 1) == 0)))
...
{   SYSTEMTIME createtime, writetime;
    ...
    memset(infoptr->FileName, ' ', sizeof(char) * 8);

    memset(infoptr->Extension, ' ', sizeof(char) * 3);
...
fssim_copy_filename_from_win32_find(WCHAR *strptr, WIN32_FIND_DATA *dataptr, UINT MaxLength)
{
    ...
    if (NameLength < MaxLength) {
        ...
        wcsncpy(strptr, dataptr->cAlternateFileName, MaxLength - 1);
        strptr[MaxLength - 1] = '\0';

    } else /* cFileName is SFN , but length not enough */ {
...
fssim_copy_dosfilename_from_dosdirentry(WCHAR *FileName, FS_DOSDirEntry *dataptr)
{
    ...
    /* count length of extenstion first */
    for (j = 2; j >= 0; j--)
    ...
    for (j = 0; j < 3; j++) {
        ...
        if (dataptr->Extension[j] != ' ')
            ...
            FileName[i + j] = dataptr->Extension[j];
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_Open(const WCHAR *FileName, UINT Flag)
{
    int fh = -1, is_exist, i;
    ...
    __try {
        ...
        if (error == ERROR_INVALID_NAME) { /* the name is not valid */
            ...
            for (i = 1; i <= FSSIM_MAX_FILE; i++) {
                ...
                if (fssim_vfgen.link[i] == map_fh) { /* the same virtual file is opened */

                    if ( (fssim_file[i].flags & Flag & FS_OPEN_SHARED) == 0) {  /* not both open-shared */
       ...
       /* set CreateFile() share mode */
...
#ifdef DEBUG_FSSIM
        fssim_printf(("MMI_FS_Open(): strbuf = "));
        fssim_print_str(strbuf);
#endif  /* DEBUG_FSSIM */
        ...
        if (fssim_file[fh].handle == INVALID_HANDLE_VALUE) {
            ...
            if (error == ERROR_PATH_NOT_FOUND) {
                ...
                fssim_printf(("CreateFile() failed in MMI_FS_Open()\n"));
                fssim_printf(("error code = %d\n", error));
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_OpenHint(const WCHAR * FileName, UINT Flag, FS_FileLocationHint *DSR_Hint)
{
    ASSERT(FileName != NULL);

    return MMI_FS_Open(FileName, Flag);
}
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_Close(FS_HANDLE File)
{
    int  map_fh=0;
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_Read(FS_HANDLE FileHandle, void *DataPtr, UINT Length, UINT *Read)
{
    UINT DummyRead;
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_Write(FS_HANDLE FileHandle, void *DataPtr, UINT Length, UINT *Written)
{
    UINT DummyWritten;
    ...
    if (Length != *Written) {
        ...
        fssim_printf(("WriteFile() failed in MMI_FS_Write()\n"));
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_SetSeekHint(FS_HANDLE FileHandle, UINT HintNum, FS_FileLocationHint *Hint)
{
    return FS_NO_ERROR;
}
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_Seek(FS_HANDLE FileHandle, int Offset, int Whence)
{
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_ReleaseFH(void * TaskId)
{
    int i;
    ...
    if (TaskId == NULL) {
        ...
        for (i = 1; i <= FSSIM_MAX_FILE; i++) {
            ...
            if (fssim_file[i].taskid == TaskId && fssim_file[i].state == STATE_INUSE && fssim_vfgen.link[i] > 0) {
                ...
                /* equivalence to MMI_FS_Close(i); with virtufl file handler */
                fssim_vfgen.link[i] = 0;
                /* equivalence to fssim_free_fh(i); */
                memset( &(fssim_file[i]), 0, sizeof(FSSIM_FILE_T));
                fssim_file[i].state = STATE_FREE;
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_Abort(UINT ActionHandle)
{
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_GetFileInfo(FS_HANDLE FileHandle, FS_FileInfo *FileInfo)
{
    int map_fh=0;
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_GetFileSize(FS_HANDLE FileHandle, UINT *Size)
{
    DWORD size_high;
    int map_fh=0;
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_SetFileTime(FS_HANDLE FileHandle, const FS_DOSDateTime * Time)
{
    FILETIME write_ft;
    SYSTEMTIME write_st;
    /* verify file handle */
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_GetAttributes(const WCHAR * FileName)
{
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_SetAttributes(const WCHAR * FileName, BYTE Attributes)
{
    TCHAR strbuf[MAX_PATH];
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_Delete(const WCHAR * FileName)
{   TCHAR strbuf[MAX_PATH];
    WIN32_FILE_ATTRIBUTE_DATA file_info;
    ...
    if( GetFileAttributesEx(strbuf, GetFileExInfoStandard, &file_info) == 0) {
        ...
        error = GetLastError();
            ...
            fssim_printf(("GetFileAttributesEx() failed in MMI_FS_Delete()\n"));
            fssim_printf(("error code = %d\n", error));
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_PseudoMerge(const WCHAR * FileName1, const WCHAR * FileName2)
{
    ...
    __try {
        ...
        f1 = MMI_FS_Open(FileName1, FS_READ_WRITE);

        if (f1 < 0)

            return f1;

        f2 = MMI_FS_Open(FileName2, FS_READ_ONLY);
        ...
        result = MMI_FS_Seek(f1, 0, FS_FILE_END);
        ...
        if (result != 0) {
            ...
            result = MMI_FS_Seek(f1, cluster_size - result, FS_FILE_END);
        }

        do {

            result = MMI_FS_Read(f2, buff, FSSIM_BUFF_SIZE * sizeof(char), &read);
            ...
            result = MMI_FS_Write(f1, buff, read, &written);
    ...
    return MMI_FS_Delete(FileName2);
}
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_GetCurrentDir(WCHAR * DirName, UINT MaxLength)
{
    TCHAR strbuf[MAX_PATH];
    int   idx;
...
#ifdef DEBUG_FSSIM
    fssim_printf(("MMI_FS_GetCurrentDir(): drive idx = %d, strbuf = ", idx));
    fssim_print_str(strbuf);
#endif  /* DEBUG_FSSIM */
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_SetCurrentDir(const WCHAR * DirName)
{
    TCHAR strbuf[MAX_PATH];
    DWORD attributes;
    int i, j;
    ...
    __try {
        ...
        fssim_printf(("MMI_FS_SetCurrentDir(): strbuf = "));
        fssim_print_str(strbuf);
        ...
        wcscpy(fssim_drvinfo[i].dir_name, strbuf);
            ...
            fssim_printf(("MMI_FS_SetCurrentDir(): fssim_drvinfo[%d].dir_name = ", i));
            fssim_print_str(fssim_drvinfo[i].dir_name);
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_CreateDir(const WCHAR *DirName)
{
    TCHAR strbuf[MAX_PATH];
    ...
    fssim_printf(("MMI_FS_CreateDir() trying to create path = "));
    fssim_print_str(strbuf);
    ...
    if (!fssim_is_name_valid(strbuf))
        ...
        if (CreateDirectory(strbuf, NULL) == 0)

            return fssim_conv_errcode(GetLastError(), "CreateDirectory() failed in MMI_FS_CreateDir()\n");
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_RemoveDir(const WCHAR * DirName)
{
    TCHAR strbuf[MAX_PATH];
    DWORD attributes, error;
    int   idx;
    ...
    if (RemoveDirectory(strbuf) == 0) {
        ...
        if (error == ERROR_DIR_NOT_EMPTY)
            ...
            return fssim_conv_errcode(error, "RemoveDirectory() failed in MMI_FS_RemoveDir()\n");
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_Extend(FS_HANDLE FileHandle, UINT Length)
{
    int TotalBytes;
    int FileSize;
    int result;
    ...
    if (TotalBytes < 0 || FileSize < 0)
        ...
        return fssim_conv_errcode(GetLastError(), "SetFilePointer() failed in MMI_FS_Extend()\n");
    ...
    if (TotalBytes > FileSize) {
        ...
        /* send back the allocated/taken cluster, because we must ensure MMI_FS_CheckFile() can do nothing */
        fssim_update_watermark_by_fh(&fssim_file[FileHandle], (FileSize - TotalBytes));

    }

    return FS_NO_ERROR;
/* Try to use SetFileValidData() function call to implement MMI_FS_Extend() but link failure, skip */
#if 0 
/* under construction !*/
/* under construction !*/
/* under construction !*/
...
int MMI_FS_Truncate(FS_HANDLE FileHandle)
{
    ...
    if (SetEndOfFile(fssim_file[FileHandle].handle) == 0) {
        ...
        return fssim_conv_errcode(GetLastError(), "SetEndOfFile() failed in MMI_FS_Truncate()\n");
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_MakeFileName(FS_HANDLE FileHandle, UINT Flag, WCHAR *FileName, UINT MaxLength)
{
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_Rename(const WCHAR *FileName, const WCHAR *NewName)
{
    TCHAR strbuf1[MAX_PATH], strbuf2[MAX_PATH];
    const TCHAR *drive1, *drive2;
    int i;
    ...
    if (MoveFile(strbuf1, strbuf2) == 0) {

        return fssim_conv_errcode(GetLastError(), "MoveFile() failed in MMI_FS_Rename()\n");
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_Move(const WCHAR * SrcFullPath, const WCHAR * DstFullPath, UINT Flag, FS_ProgressCallback Progress)
{
    ...
    __try {
            ...
            if (attributes & FILE_ATTRIBUTE_DIRECTORY) {
                ...
                if (!CreateDirectory(dst, NULL)) {
                    ...
                    if (GetLastError() != ERROR_ALREADY_EXISTS) {
                        ...
                        fssim_printf(("CreateDirectory() failed in MMI_FS_Move()\n"));
        ...
        if (error != ERROR_SUCCESS)
            ...
            return fssim_conv_errcode(error, "MoveFileWithProgress()/CopyFileEx() failed in MMI_FS_Move()\n");
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_Count(const WCHAR *FullPath, UINT Flag)
{
    TCHAR strbuf[MAX_PATH];
    DWORD attributes;
    ...
    else if (Flag & ~(FS_FILE_TYPE | FS_DIR_TYPE | FS_RECURSIVE_TYPE | FS_FILTER_SYSTEM_ATTR))
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_XDelete(const WCHAR *FullPath, UINT Flag)
{
    TCHAR strbuf[MAX_PATH];
    DWORD attributes;
    int result, drv_idx;
...
#ifdef DEBUG_FSSIM
            fssim_printf(("RemoveDirectory() failed in MMI_FS_XDelete()\n"));
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_FindFirst(const WCHAR *NamePattern, BYTE Attr, BYTE AttrMask, FS_DOSDirEntry *FileInfo, WCHAR *FileName, UINT MaxLength)
{
    TCHAR strbuf[MAX_PATH];
    HANDLE hSearch;
    WIN32_FIND_DATA data;
    ...
    if (hSearch == INVALID_HANDLE_VALUE) {
        ...
        if (error != ERROR_FILE_NOT_FOUND) {
            ...
            fssim_printf(("FindFirstFile() failed in MMI_FS_FindFirst()\n"));
    ...
    while (1) {
        ...
        if (!FindNextFile(hSearch, &data)) {
            ...
            if (error != ERROR_NO_MORE_FILES) {
                ...
                fssim_printf(("FindNextFile() failed in MMI_FS_FindFirst()\n"));
    ...
    if (is_found == 1) {
        ...
        if (fh == -1) {
            ...
            if (!FindClose(hSearch)) {
                ...
                fssim_printf(("FindClose() failed in MMI_FS_FindFirst()\n"));
        ...
        FileInfo->NTReserved = 
        fssim_copy_filename_from_win32_find(FileName, &data, MaxLength);
...
/* INPUTS                                                                */
/*                                                                       */
/*      FileHandle   -   file handle returned from MMI_FS_FindFirst      */
/*      FileInfo     -   a pointer to FS_DOSDirEntry structure           */
/*      FileName     -   buffer to store the found file                  */
/*      MaxLength    -   length of the FileName buffer                   */
/*                                                                       */
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_FindNext(FS_HANDLE FileHandle, FS_DOSDirEntry *FileInfo, WCHAR *FileName, UINT MaxLength)
{
    DWORD attr1, attr2;
    int is_found;
    WIN32_FIND_DATA data;
    ...
    do {
        ...
        if (!FindNextFile(fssim_file[FileHandle].handle, &data)) {
            ...
            if (error != ERROR_NO_MORE_FILES) {
                ...
                fssim_printf(("FindNextFile() failed in MMI_FS_FindNext()\n"));
...
/*      NoteXXX: The searching rule is just like MMI_FS_FindFirst.       */
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_FindFirstN(const WCHAR *NamePattern, FS_Pattern_Struct *PatternArray, UINT PatternNum,
                  ...
                  WCHAR *FileName, UINT MaxLength, UINT EntryIndex, UINT Flag)
{
    ...
    *(strbuf + wcslen(L"?:/")) = 0;
    ...
    f = MMI_FS_FindFirst(NamePattern, Attr, AttrMask, FileInfo, strbuf, MAX_PATH);
    ...
    __try {
        ...
        do {
            ...
            retval = MMI_FS_FindNext(f, FileInfo, strbuf, MAX_PATH);
        ...
        if (is_found == 0) {
            ...
            if ((MaxLength / sizeof(TCHAR) - 1) < wcslen(strbuf)) {
                ...
                if (Flag == FS_FIND_LFN_TRUNC) {
                    ...
                    /* use short file name */
                    fssim_copy_dosfilename_from_dosdirentry(strbuf, FileInfo);

                    memset((void*)FileName, 0, MaxLength);
                    wcsncpy(FileName, strbuf, MaxLength / sizeof(TCHAR) - 1);
...
/*                                                                       */
/*      This function is used to find N-th file.                         */
/*      NoteXXX: The searching rule is just like MMI_FS_FindNext.        */
/*               But each file should either contain an attribute        */
/*               specified in ArrayMask or match patterns specified      */
/*               in PatternArray.                                        */
/*                                                                       */
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_FindNextN(FS_HANDLE Handle, FS_Pattern_Struct * PatternArray, UINT PatternNum, 
                 BYTE ArrayMask, FS_DOSDirEntry *FileInfo, WCHAR *FileName, UINT MaxLength, UINT Flag)
{
    ...
    __try {
        ...
        do {
            ...
            retval = MMI_FS_FindNext(Handle, FileInfo, strbuf, MAX_PATH);
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_XFindReset(const WCHAR *Pattern, FS_Pattern_Struct *PatternArray, UINT PatternNum,
                  BYTE ArrayMask, BYTE AttrMask, UINT Flag, UINT Timeout, UINT *Position)
{
  ...
  *(strbuf + wcslen(L"?:/")) = 0;
  ...
  if (Flag & ~(FS_FILE_TYPE | FS_DIR_TYPE | FS_NO_SORT | FS_SORT_NAME | FS_SORT_SIZE | FS_SORT_TIME | FS_SORT_TYPE))
  ...
  if (fssim_sort_cnt != 0) {
    ...
    /* XXX Simulator semaphore not ready, so return MMI_FS_ACCESS_DENIED for all attempt */
    return FS_ACCESS_DENIED;
    
    /* there is some one executing MMI_FS_XFindxxx */
  ...
  __try {
    ...
    f = MMI_FS_FindFirstN(Pattern, PatternArray, PatternNum, ArrayMask, 0, AttrMask, &info, strbuf, MAX_PATH, 0, FS_FIND_DEFAULT);
    ...
    do {
      ...
      result = MMI_FS_FindNextN(f, PatternArray, PatternNum, ArrayMask, &info, strbuf, MAX_PATH, FS_FIND_DEFAULT);
...
/*      Position    -   pointer gotten from MMI_FS_XFindReset            */
/*      Flag        -                                                    */
/*                                                                       */
/* OUTPUTS                                                               */
/*                                                                       */
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_XFindStart(const WCHAR *Pattern, FS_DOSDirEntry *FileInfo, WCHAR *FileName,
                  UINT MaxLength, UINT Index, UINT *Position, UINT Flag)
{
    ...
    wcsncpy(FileName, fssim_sort[Index].filename, MaxLength / sizeof(TCHAR));

    if ((MaxLength / sizeof(TCHAR)) < wcslen(fssim_sort[Index].filename)) {
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_XFindClose(UINT *Position)
{
    ASSERT(Position != NULL);
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_CreateMasterBootRecord(void *SectorBuffer, const FS_PartitionRecord *DiskGeometry)
{
    return FS_NO_ERROR;
}
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_GetPartitionInfo(const WCHAR *DriveName, FS_PartitionRecord *PartitionInfo)
{
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_CommitAll(const WCHAR *DriveName)
{
    int i;
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_GetDrive(UINT Type, UINT Serial, UINT AltMask)
{
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_MappingDrive(UINT UpperOldDrv, UINT UpperNewDrv)
{
    int i;
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_GeneralFormat(const WCHAR *DriveName, UINT Level, FS_FormatCallback Progress)
{
    ...
    for (i = 1; i <= FSSIM_MAX_FILE; i++) {
        ...
        if (fssim_file[i].state == STATE_INUSE && !wcsnicmp(drive, &(fssim_drvinfo[fssim_file[i].drive].drive), 1)) {
            ...
            fssim_printf(("file handle %d is released in MMI_FS_GeneralFormat\n", i));
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_CreateBootSector(void *BootSector, const FS_PartitionRecord *Partition,
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_SplitPartition(void *MasterBootRecord, UINT Sectors)
{
    return 0;
}
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_GetDiskInfo(const WCHAR *DriveName, FS_DiskInfo *DiskInfo, int Flags)
{
    TCHAR drive = 0;
    int i;
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_GetDevType(const WCHAR *Path)
{
    int offset;
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_GetDevStatus(UINT DriveIdx, UINT Flag)
{
    int offset;

    offset = (char)DriveIdx - (char)FSSIM_SYSTEM_DRIVE;

    if ((char)DriveIdx >= 'c' && (char)DriveIdx <= 'z') {
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_TestMSDC(void *slot_id)
{
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_CloseMSDC(UINT DriveIdx, UINT Mode)
{
    return FS_NO_ERROR;
}
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_SetTrace(UINT Flag, UINT Timeout)
{
    return FS_NO_ERROR;
}
...
/* DESCRIPTION                                                           */
/*                                                                       */
/*      This function is implement to do no-thing here. See MMI_FS_Extend() */
/*                                                                       */
/* INPUTS                                                                */
/*                                                                       */
/*      FileName   -                                                     */
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_CheckFile(const WCHAR *FileName)
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_GetCheckBufferSize(const UINT DriveIdx)
{   int i, required;
    ...
    required = (fssim_drvinfo[i].totalclusters + 3) / 8 + 1;
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_CheckDrive(const UINT DriveIdx, BYTE *Checkbuffer, const UINT BufferSize)
{   int i;
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_ParseFH(FS_HANDLE FileHandle)
{
    /* verify file handle */
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_QmGetFree(const BYTE *Path)
{
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_GetClusterSize(UINT DriveIdx)
{
    int offset;
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_GetFolderSize(const WCHAR *FullPath, UINT Flag)
{
    TCHAR strbuf[MAX_PATH];
    DWORD attributes;
    FSSIM_SUM_SIZE_PARAM_T top_folder;
...
UINT32 MMI_FS_GetDirSize( CONST UINT8 * dirName)
{
   ...
   return MMI_FS_GetFolderSize((const WCHAR * )dirName, 0);
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_GetFirstCluster(FS_HANDLE FileHandle, UINT * Cluster)
{
    FILETIME create_ft;
...
/*                                                                       */
/*************************************************************************/
int MMI_FS_GenVirtualFileName(FS_HANDLE FileHandle, WCHAR * VFileNameBuf, UINT BufLength,
...
INT32 MMI_FS_GetFileSizeByName( const UINT8 * FileName)
{
  ...
  fh = MMI_FS_Open((const UINT8 *)FileName, FS_READ_ONLY);
  if (fh >= 0)
  {
    if(MMI_FS_GetFileSize(fh,(INT32*)&Size) != FS_NO_ERROR)
    {
      Size = 0;
    }
    MMI_FS_Close(fh);
