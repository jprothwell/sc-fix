...
#include "gui_config.h"  //2010-9-19 fzb add
...
#define E71_TIME_DISPLAY_X		2
#define E71_TIME_DISPLAY_Y		MMI_STATUS_BAR_HEIGHT
#define E71_DATE_DISPLAY_X		23
#define E71_DATE_DISPLAY_Y		89
...
#define C01_TIME_HOUR_X           2
#define C01_TIME_DOT_X             52
#define C01_TIME_MIN_X             65
#define C01_DATE_DISPLAY_X     115
#define C01_DATE_DISPLAY_Y     130
extern stFontAttribute wgui_dialer_box_f1;
#endif
...
extern UI_image_ID_type  idlescreen_bar_id;//070306 Alpha layer

U8 g_clock_type = ANALOG;   /* default clock for Techno Idlescreen Bar */

U16 *day_string = NULL; /* pointer storing the address of the UI_image_ID_type  of day string. */
#elif defined(__PROJECT_GALLITE_C01__) /* __MMI_UI_TECHNO_IDLESCREEN_BAR__ */ 
U16 *day_string = NULL; /* pointer storing the address of the UI_image_ID_type  of day string. */
#endif
...
void date_string_idle(UI_time *time, UI_string_type string, U32 flags)
{
    U16 day, month, day_index;
    S8 ts[32];
 ...
 *  void
 *****************************************************************************/
 #define __MMI_DATE_TIME_IN_DIFF_LANG__
void date_string(UI_time *time, UI_string_type string, U32 flags)
{
    ...
    switch (flags & DT_FLAGS_TYPE)
    {
      ...
      /***** MainLCD DataTime *****/
        ...
        case DT_ANALOG_CLOCK_SC:
        {
            switch (PhnsetGetDateFormat())  /* Get the current format of date */
            {
                case 0:
                    ...
                    sprintf((S8*) ts, "%02d", day);
                    AnsiiToUnicodeString((S8*) string, ts);
#if !defined(__MMI_MAINLCD_160X128__)&&!defined(TGT_GALLITE_R400)
                    if(IsChineseSet())
                    {
                    	UI_strcat(string, (UI_string_type)get_string(STR_GUI_DAY));
                    }
#endif
                    sprintf((S8*) ts, "-");
                    ...
                    {
                        month_str = get_string_lang((UI_image_ID_type ) (STR_GUI_MONTH_BASE + month), (S8*) "*#0044#");
                    }
                    else
                    {
#if defined(__MMI_MAINLCD_160X128__)||defined(TGT_GALLITE_R400)                    
                        month_str = NULL; //get_string((UI_image_ID_type ) (STR_GUI_MONTH_BASE + month));
			   sprintf((S8*) ts, "%02d", month);
			   AnsiiToUnicodeString((S8*) ts2, ts);
			   UI_strcat(string, (UI_string_type) ts2);
#else
                        month_str = get_string((UI_image_ID_type ) (STR_GUI_MONTH_BASE + month));
#endif
                    }
                    if (month_str)
                    {
        ...
        if(IsChineseSet())
                    {
                    	UI_strcat(string, (UI_string_type)get_string(STR_CAL_YEAR));
                    }
#endif	
                    break;
                ...
                case 5:
                    ...
                    {
                        UI_strcpy(string, (UI_string_type)L"");
                    }
                    sprintf((S8*)ts, " %02d",day);
                    AnsiiToUnicodeString((S8*) ts2, ts);
                    UI_strcat(string, (UI_string_type) ts2);
        ...
        case DT_IDLE_SCREEN_2:
        {
            switch (PhnsetGetDateFormat())
            {
                ...
                case 1:
                	...
                	{
                		UI_strcat(string, (UI_string_type)L"");
                	}
                	#endif
                	break;
                case 2:
...
#if defined(__PROJECT_GALLITE_C01__)
void time_string_idle(UI_time *t, UI_string_type s_hour, UI_string_type s_min, U32 flags)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    {
         sprintf((S8*) ts1, "%02d", hours);
         sprintf((S8*) ts2, "%02d", minutes);		      		
    }
    else
    {
    	if (hours == 12)
       ...
       {
            sprintf((S8*) ts1, "%02d", 12);
            sprintf((S8*) ts2, "%02d", minutes);		      		
       }
       else if (hours > 12)
       {
            sprintf((S8*) ts1, "%02d", hours - 12);
            sprintf((S8*) ts2, "%02d", minutes);		      		
       }
       else
       {
    ...
    AnsiiToUnicodeString((S8*) s_hour, ts1);
    AnsiiToUnicodeString((S8*) s_min, ts2);
}
#endif
    ...
    switch (flags & DT_FLAGS_TYPE)
    {
        ...
        case DT_ANALOG_CLOCK_SC:
            ...
            else
            {
		      ...
		      UI_strcat((UI_string_type) s, (UI_string_type)L" ");
		   #endif
...
void increment_ticks(UI_time *t)
{
    ...
    /*wuzc Modify Start For GDI/GUI Portting Ver:     record time update error on 2008-4-29 14:48 */
    //kal_get_time(&current_time);
    
     current_time = csw_TMGetTick();

    secs = (U32) ((current_time - last_duration_ticks + last_duration) >>14);//chenhe,16384*tick=1 ,16384=2^14,second
    //secs = (U32)(current_time - last_duration_ticks + last_duration);
    mins = secs / 60;
    hrs = mins / 60;
...
*/
	//current_time = TM_GetTime ();
	
	//current_time = global_var_uptime(FALSE);//up_time;
	extern void Aud_get_time( kal_uint32*tmpTime );
	Aud_get_time( &current_time );                   //tangjie del 20060620
//	secs= (U32)((current_time - last_duration_ticks + last_duration)*0.004615 + 0.5);
//	secs= (U32)((up_time - set_time + last_duration*256)*0.00390625 + 0.5);//注意进制间的转化
	secs = (U32)((current_time - last_duration_ticks + last_duration)*0.00390625 + 0.5);
	mins= secs/60;
	hrs= mins/60;
	t->nSec = (U8)(secs - mins*60);
	t->nMin = (U8)(mins - hrs*60);
...
extern void Symbian_idle_screen_show_digital(void);
#endif
/*- zhouqin 2011-02-16 modify for q6 320X240 */
...
#if defined(__PROJECT_GALLITE_C01__)
    UI_character_type s_hour[10] = {0};
    UI_character_type s_min[10] = {0};
    UI_character_type s_dot[4] = {':',0};
#endif	
    S32 x1, y1, x2, y2;
    bitmap *saved_graphics_context = current_LCD_device_bitmap;
    S32 old_x1, old_y1, old_x2, old_y2;
#if defined(__MMI_MAINLCD_220X176__)
	if(MMI_CATEGORY120_ID == g_dm_data.S32CatId)
		return;
#endif
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
...
/*+ zhouqin 2011-02-16 modify for q6 320X240 */
#if defined(__MMI_SYMBIAN_STYLE_IDLE__)	
	Symbian_idle_screen_show_digital();
	return;
#endif
/*- zhouqin 2011-02-16 modify for q6 320X240 */
    ...
    switch (main_LCD_dt_object.flags & DT_FLAGS_TYPE)
    {
        ...
        #endif /* __MMI_UI_TECHNO_IDLESCREEN_BAR__ */ 
#if defined(__MMI_MAINLCD_160X128__)|| defined(__MMI_MAINLCD_220X176__) 
...
#if defined(__PROJECT_GALLITE_C01__)
		      time_string_idle(&main_LCD_dt_object.time.t, (UI_string_type) s_hour, (UI_string_type) s_min, main_LCD_dt_object.flags);
#endif
                    /* calculate string width of time string */
                    gui_measure_string(s, &w, &h);

                #ifdef __MMI_UI_TECHNO_IDLESCREEN_BAR__
                    x = ANALOG_BACKGROUND_X + (ANALOG_BACKGROUND_WIDTH >> 1) - (w >> 1);
...
#if defined(__MMI_MAINLCD_220X176__)
					x = E71_TIME_DISPLAY_X - 1;
					y = E71_TIME_DISPLAY_Y - 1;
#else
#if defined(__PROJECT_GALLITE_C01__)
                    x = C01_TIME_HOUR_X;
                    y = C01_DATE_DISPLAY_Y;
#else
                    x = (UI_device_width >> 1) - (w >> 1);
                    ...
                    x=UI_DEVICE_WIDTH- main_LCD_dt_object.time.width - 2;   //130;     //mym 时钟的坐标
                    y=25;
#endif

#if defined(__MMI_BLACKBERRY_IDLE_SHORTCUT__) && defined(__MMI_MAINLCD_160X128__) 
			x = UI_device_width - main_LCD_dt_object.time.width - 2;
			y = 20;
#endif
                    ...
                    if (enable_main_LCD_date_time_display)
                    {
                        ...
                        {
#if defined(__MMI_MAINLCD_220X176__)
							gui_move_text_cursor(E71_TIME_DISPLAY_X,E71_TIME_DISPLAY_Y);	
#else
                            gui_move_text_cursor(x, y + 1);
#endif
                        }
#if defined(__PROJECT_GALLITE_C01__)
                        x2 = C01_DATE_DISPLAY_X -1;
                        ...
                        gui_show_image(0, ANALOG_BACKGROUND_Y, get_image(IMG_TECHNO_DIGITAL_CLOCK));
                        gui_pop_clip();
                    #endif 
...
#else
					gui_set_text_clip(E71_TIME_DISPLAY_X,E71_TIME_DISPLAY_Y,E71_TIME_DISPLAY_X+x2-x1,E71_TIME_DISPLAY_Y+y2-y1);
					gdi_draw_solid_rect(E71_TIME_DISPLAY_X,E71_TIME_DISPLAY_Y,E71_TIME_DISPLAY_X+x2-x1,E71_TIME_DISPLAY_Y+y2-y1,GDI_COLOR_TRANSPARENT);
                    #endif 

                    #endif 
#if defined(__PROJECT_GALLITE_C01__)
			gui_print_bordered_text(s_hour);
			gdi_image_draw_id(C01_TIME_DOT_X, y + 2,IMG_TIME_DOT);
			#if 0
			gui_move_text_cursor(C01_TIME_DOT_X, y + 8);
			gui_print_bordered_text(s_dot);
			gui_move_text_cursor(C01_TIME_DOT_X, y + 27);
			gui_print_bordered_text(s_dot);
			#endif
			gui_move_text_cursor(C01_TIME_MIN_X, y + 1);
			gui_print_bordered_text(s_min);
#else
#if defined(__MMI_NOKIA_STYLE_N800__)
						gui_print_text(s);
#else
...
#if defined(__PROJECT_GALLITE_C01__)
	     date_string_idle(&main_LCD_dt_object.date.t, (UI_string_type) s, main_LCD_dt_object.flags);
#else
...
#if defined(__MMI_MAINLCD_220X176__)
			x = E71_DATE_DISPLAY_X;
			y = E71_DATE_DISPLAY_Y;
#else
#if defined(__PROJECT_GALLITE_C01__)
            x = ((UI_device_width - C01_DATE_DISPLAY_X) >> 1) - (w >> 1) + C01_DATE_DISPLAY_X;
            y = C01_DATE_DISPLAY_Y;
#else
            x = (UI_device_width >> 1) - (w >> 1);
...
#endif
#if defined(__MMI_BLACKBERRY_IDLE_SHORTCUT__) && defined(__MMI_MAINLCD_160X128__) 
		x = 2;//((UI_device_width-w)>>1);//main_LCD_dt_object.time.x - w-2;
		y = main_LCD_dt_object.time.y + ((main_LCD_dt_object.time.height - h)>>1);
#endif
            ...
            #else /* __MMI_UI_TECHNO_IDLESCREEN_BAR__ */ 
                gdi_layer_blt_previous(x1 < old_x1 ? x1 : old_x1, y1 < old_y1 ? y1 : old_y1, x2 > old_x2 ? x2 : old_x2, y2 > old_y2 ? y2 : old_y2);      
            #endif /* __MMI_UI_TECHNO_IDLESCREEN_BAR__ */ 
...
#define DT_SOFTKEY_GAP  2
        ...
        case DT_NETWORK_SEARCH:
        {
            ...
            x = (UI_device_width >> 1) - (w >> 1);
#endif
        #if defined(__MMI_UI_DALMATIAN_SOFTKEYS__) || defined(__MMI_UI_DALMATIAN_IDLESCREEN__) || defined(__MMI_MAINLCD_128X128__) 
...
void hide_main_LCD_date_time(void)
{
    ...
    switch (main_LCD_dt_object.flags & DT_FLAGS_TYPE)
    {
        ...
        case DT_MT_CALL_SCREEN:
        {
            ...
            main_LCD_dt_object.time.hide_function(x1, main_LCD_dt_object.time.y, x2, y2);      /* call hide function of idle,netwroksearch,call screen */
#else			
...
void update_main_LCD_date_time(void)
{
    ...
    GetDateTime(&main_LCD_dt_object.date.t);
    main_LCD_dt_object.time.t = main_LCD_dt_object.date.t;
#if defined(__MMI_UI_TECHNO_IDLESCREEN_BAR__)||defined(__PROJECT_GALLITE_C01__)
    set_day_display();
#endif 
}
...
extern 	void symbian_hide_main_LCD_idle_date_time(void);
#endif
/*- zhouqin 2011-02-16 modify for q6 320X240 */
...
void set_dt_display(U32 flags)
{
    ...
    switch (flags & DT_FLAGS_TYPE)
    {
        ...
        #else 
		#if defined(__PROJECT_GALLITE_C01__)
            main_LCD_dt_object.time.font = &wgui_dialer_box_f1;
		#else
        ...
        #else 
            main_LCD_dt_object.time.text_color = gui_color(255, 255, 255);
        #endif 
	...
	/*+ zhouqin 2011-02-16 modify for q6 320X240 */
	#if defined (__MMI_BLACK_BERRY_IDLE_STYLE__) || defined (__MMI_WIN_STYLE_SUPPORT__)
		MMI_main_LCD_dt_hide_function = symbian_hide_main_LCD_idle_date_time; 
	#else
	...
	/*- zhouqin 2011-02-16 modify for q6 320X240 */
        ...
        case DT_NETWORK_SEARCH:
        {
            ...
            main_LCD_dt_object.time.hide_function = MMI_main_LCD_dt_time_hide_function;
        #if defined(__PROJECT_GALLITE_C01__) 
            main_LCD_dt_object.date.font = &MMI_medium_font/*MMI_small_font*/;
        #else 
...
void show_call_cost_UI(PU8 symbol, PU8 value)
{
    ...
    S32 tx, ty, x;
    color_t c = *current_MMI_theme->datetime_bar_AOC_text_color;
...
void default_inline_date_day_validation(UI_buffer_type text, UI_buffer_type cursor, S32 text_length)
{
    ...
    i = gui_atoi((UI_string_type) current_date_input->month_input_box.text);
    l = gui_strlen((UI_string_type) current_date_input->month_input_box.text);
	
    if ((i < g_date_month_min) && (l > 1))
    {
        i = g_date_month_min;
        gui_sprintf((UI_string_type) current_date_input->month_input_box.text, "%02d", i);
    }	
	
    i = gui_atoi((UI_string_type) current_date_input->year_input_box.text);
    l = gui_strlen((UI_string_type) current_date_input->year_input_box.text);
	
    if ((i < g_date_year_min) && (l > 3))
    {
        i = g_date_year_min;
        gui_sprintf((UI_string_type) current_date_input->year_input_box.text, "%04d", i);
    }	
}
...
#if defined(__MMI_UI_INLINE_EDIT_DEFAULT_TEXT_EFFECT__) && (defined(__MMI_MAINLCD_128X160__) || defined(__MMI_MAINLCD_160X128__)||defined (__MMI_MAINLCD_128X128__))
...
void show_date_input(date_input *d)
{
    ...
    if (r2lMMIFlag)
    {
        ...
        gui_move_text_cursor(d->s1_x + sw, d->s1_y + d->y +2);//gdm 090616 fix bug 12582
    }
    else
    {
        gui_move_text_cursor(d->s1_x, d->s1_y + d->y +2);//gdm 090616 fix bug 12582
    }
    ...
    if (r2lMMIFlag)
    {
        gui_move_text_cursor(d->s2_x + sw, d->s2_y + d->y+2 );//gdm 090616 fix bug 12582
    }
    else
    {
        gui_move_text_cursor(d->s2_x, d->s2_y + d->y +2);//gdm 090616 fix bug 12582
    }
...
void show_time_input(time_input *d)
{
    ...
    if (r2lMMIFlag)
    {
        ...
        gui_move_text_cursor(d->s1_x + sw, d->y + (d->height >> 1) - (sh >> 1));//gdm 090616 fix bug 14939 
    }
    else
    {
        gui_measure_character(d->seperator, &sw, &sh);
        gui_move_text_cursor(d->s1_x, d->y + (d->height >> 1) - (sh >> 1));//gdm 090616 fix bug 14939 
    }
    ...
    if ((d->flags & TIME_INPUT_TYPE_MASK) != TIME_INPUT_TYPE_HH_MM)
    {
        if (r2lMMIFlag)
        {
            gui_move_text_cursor(d->s2_x + sw, d->s2_y + d->y-2);//gdm 090616 fix bug 14939 
        }
        else
        {
            gui_move_text_cursor(d->s2_x, d->s2_y + d->y-2);//gdm 090616 fix bug 14939 
        }
        gui_print_character(d->seperator);
    }
    gdi_layer_blt_previous(d->x, d->y, d->x + d->width - 1, d->y + d->height - 1);
...
void create_DOW_select(DOW_select *d, S32 x, S32 y, S32 width, S32 height)
{
    ...
    d->cell_height = get_menu_item_height();
    d->x = x;
    d->y = y;
    d->width = width;
    d->height = height;
...
void show_DOW_select(DOW_select *d)
{
	...
	s32 DOW_list_Fr[]={'D','L','M','M','J','V','S'};
	#endif 

	s32	DOW_list_En[]={'S','M','T','W','T','F','S'};
...
//panx modified on 20080710
		...
		DOW_list_En,
		#endif
		#ifdef  __MMI_LANG_THAI__                                   /*trun on thai language*/
		DOW_list_En,
		#endif
		DOW_list_En,
		DOW_list_En,
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	/*Guojian Add Start for over array Ver: GreenStone  on 2009-5-9 20:21 */
	if (gCurrLangIndex < sizeof(gpDOW_list)/sizeof(s32*))
		pDOW_list = (S32 *)gpDOW_list[gCurrLangIndex];
	else
		pDOW_list = (S32 *)DOW_list_En;
	/*Guojian Add End  for over array Ver: GreenStone  on 2009-5-9 20:21 */
    gdi_layer_push_clip();
    gdi_layer_reset_clip();
    x = d->x + d->offset_x;
    y = d->y + d->offset_y;
    ...
    for (i = 0; i < 7; i++)
    {
        ...
        character_width = gui_get_character_width((UI_character_type) pDOW_list[i]);

        if (r2lMMIFlag)
        {
            gui_move_text_cursor(
                x + (d->cell_width >> 1) - (character_width >> 1) + character_width,
                y + (d->cell_height >> 1) - (character_height >> 1));
        }
        else
        {
            gui_move_text_cursor(
                x + (d->cell_width >> 1) - (character_width >> 1),
                y + (d->cell_height >> 1) - (character_height >> 1));
        }
        gui_set_text_color(text_color);
        gui_print_character((UI_character_type) pDOW_list[i]);
...
void time_period_input_show_background(time_period_input *d)
{
    ...
    y2 = y1 + d->height - 1;
//adjust TDL TIME EDIT bug 9156 and 9169
    gui_draw_filled_area(x1, y1, x2+2, y2, &date_time_input_background_filler);
    x1 = d->hours_input_box2.x;
    y1 = d->y;
    x2 = d->minutes_input_box2.x + d->minutes_input_box2.width - 1;
    y2 = y1 + d->height - 1;
    gui_draw_filled_area(x1-2, y1, x2, y2, &date_time_input_background_filler);
}
...
void show_time_period_input(time_period_input *d)
{
    ...
    if (r2lMMIFlag)
    {
        ...
        gui_move_text_cursor(d->s1_x + sw, d->s1_y + d->y);//gdm 091022 fix bug 14939 
    }
    else
    {
        gui_move_text_cursor(d->s1_x, d->s1_y + d->y);//gdm 091022 fix bug 14939 
    }
    ...
    if (r2lMMIFlag)
    {
        ...
        gui_move_text_cursor(d->s2_x + sw, d->s2_y + d->y );//gdm 091022 fix bug 14939 
    }
    else
    {
        gui_move_text_cursor(d->s2_x, d->s2_y + d->y);//gdm 091022 fix bug 14939 
    }
    ...
    if (r2lMMIFlag)
    {
        ...
        sw = gui_get_character_width(d->seperator1);
        gui_move_text_cursor(d->s3_x + sw, d->s3_y + d->y);//gdm 091022 fix bug 14939 
    }
    else
    {
        gui_move_text_cursor(d->s3_x, d->s3_y + d->y);//gdm 090616 fix bug 14939 
    }
 ...
 *  void
 *****************************************************************************/
//	extern U32 global_var_settime(BOOL bSet);     //It's redundant codes, deleted by kecx for optimizing codes on 20090610
...
/*----------------------------------------------------------------------------
Function:			set_time_duration
Description:		set main lcd time duration object
Input Parameters:	UI_time *t, U32 duration
Output Parameters:	none
Returns:			void
----------------------------------------------------------------------------*/
	extern void Aud_get_time( kal_uint32*tmpTime );
void fmsr_set_time_duration(UI_time *t, U32 duration)
{	main_LCD_time_object.duration.t=*t;	//set the duration value 
	//last_duration_ticks = TM_GetTime ();
	//last_duration_ticks = global_var_settime(FALSE);
	Aud_get_time(&last_duration_ticks);          //tangjie del end 20060620
	last_duration = duration;

   mmi_trace(TRUE, "set_time_duration:: duration = %d, t->nSec=%d, t->nMin= %d,t->nHour= %d",duration, t->nSec,t->nMin,t->nHour);
...
/*----------------------------------------------------------------------------
Function:			time_update_duration
Description:		update time duration
Input Parameters:	none
Output Parameters:	none
Returns:			void
----------------------------------------------------------------------------*/
void fmsr_time_update_duration(void)
{	

	fmsr_increment_ticks(&main_LCD_time_object.duration.t);// increment secods by 1
	time_display_duration();
	gui_start_timer(300,fmsr_time_update_duration);
}
...
/*----------------------------------------------------------------------------
Function:			set_time_display
Description:		set time duration object properties
Input Parameters:	dword flags
				S32 x
				S32 y
				S32 width
				S32 height
...
Returns:			void
----------------------------------------------------------------------------*/
void fmsr_set_time_display(dword flags, S32 x, S32 y, S32 width, S32 height)
{	
	...
	main_LCD_time_object.duration.hide_function=MMI_main_LCD_time_duration_hide_function;
	main_LCD_time_object.duration.font=&MMI_large_font;
	main_LCD_time_object.duration.text_color=*current_MMI_theme->list_normal_text_color;
	main_LCD_time_object.duration.x = x;
	main_LCD_time_object.duration.y = y;
	main_LCD_time_object.duration.width = width;
	main_LCD_time_object.duration.height = height;
	gui_cancel_timer(fmsr_time_update_duration);
	if(flags!=0)
	   gui_start_timer(300,fmsr_time_update_duration);
...
/*----------------------------------------------------------------------------
Function:			close_main_LCD_time_display
Description:		disable main lcd time display
Input Parameters:	none
Output Parameters:	none
Returns:			void
----------------------------------------------------------------------------*/
void fmsr_close_main_LCD_time_display(void)
{	
	gui_cancel_timer(fmsr_time_update_duration);
}
...
void show_main_LCD_day_display(void)
{
    ...
    color_t text_color, text_border_color;
...
#if defined(__PROJECT_GALLITE_C01__)
    x1 = C01_DATE_DISPLAY_X;
    y1 = C01_DATE_DISPLAY_Y + 25;
    x2 = UI_device_width - 1;
    y2 = y1 + h - 1;
#else
#if defined(__MMI_NOKIA_STYLE_N800__)
    x1=UI_DEVICE_WIDTH - main_LCD_dt_object.date.width - 35 ; //mym //日期的坐标
    y1=175;
#else
    ...
    if (r2lMMIFlag)
    {
        gui_move_text_cursor(x2 - (((x2 - x1) - w)>>1), y1);
    }
    else
    {
        gui_move_text_cursor(x1 + (((x2 - x1) - w)>>1), y1);
    }
#else
    if (r2lMMIFlag)
    {
