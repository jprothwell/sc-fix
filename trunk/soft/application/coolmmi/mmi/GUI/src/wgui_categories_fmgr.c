...
#define MMI_THICK_MENUITEM_HEIGHT   (37)        /* to set no of menu item to 7 */
#else 
#if defined(__PROJECT_GALLITE_C01__)
#define MMI_THICK_MENUITEM_HEIGHT   (40)
#else
        ...
        U8 *history_buffer)
{
    ...
    {
        move_fixed_list(0, (MMI_title_y + MMI_title_height) + cat212_info_box_height);  /* use two menuitem height to draw drive info */
        resize_fixed_list(
            MMI_content_width,
            MMI_content_height - cat212_info_box_height - get_current_tab_pane_height());
    }
    else
        ...
        U8 *history_buffer)
{
    ...
    ShowCategory211Screen(
        ...
        hide_info_callback,
#if defined(__PROJECT_GALLITE_C01__)
        (S32) num_of_info_box_rows * (MMI_ICONTEXT_MENUITEM_HEIGHT+4),
#else
...
/*+ zhouqin modify for dual t card  201105301*/
#if defined(DUAL_TFLASH_SUPPORT)
static BOOL MasterCardGetState =FALSE;
static BOOL SlaveCardGetState =FALSE;
void mmi_fmgr_progress(S32 index)
{
    S32 offset_x;
    S32 offset_y;
    S32 str_width;
    ...
    if(((index==1)&&(MasterCardGetState==FALSE))||
        ((index==2)&&(SlaveCardGetState==FALSE)))
    {
            ...
            UI_device_width - 1,
            (MMI_title_y + MMI_title_height) + cat212_info_box_height - 1,
            ...
            str_wait = GetString( STR_GLOBAL_PLEASE_WAIT);    
            gui_measure_string((UI_string_type) str_wait, &str_width, &str_height);
            offset_x = (UI_device_width - str_width) >> 1;
            offset_y = (MMI_title_y + MMI_title_height)+10;

            gui_move_text_cursor(offset_x, offset_y);

            gui_print_text((UI_string_type) str_wait);

            gdi_layer_unlock_frame_buffer();
            gdi_layer_blt_previous(0, 0, UI_device_width - 1, UI_device_height - 1);

            if(index==1)
                MasterCardGetState =TRUE;

            if(index==2)
                SlaveCardGetState = TRUE;
    }
...
static void ShowCategory212InfoArea(S32 item_idx)
{
    ...
    if (cat212_draw_info_callback != NULL)
    {
        cat212_draw_info_callback(
            ...
            0,                                      /* x1 */
            (MMI_title_y + MMI_title_height),       /* y1 */
            UI_device_width - 1,                    /* x2 */
            (MMI_title_y + MMI_title_height) + cat212_info_box_height - 1);       /* y2 */
    }
        ...
        U8 *history_buffer)
{
    ...
    /* register shortcut */
    register_menu_shortcut_keys();
    register_menu_shortcut_selected_only(Cat213ShortCutHdlr);
    ...
    if (is_enabled_tab_pane())
    {
        ...
        resize_fixed_list(MMI_content_width, MMI_content_height - get_current_tab_pane_height());
    }
    else
#endif /* __MMI_UI_TAB_PANE__ */ 
    {
...
static void Cat214RedrawMatrix(void)
{
    ...
    content_offset_y = MMI_title_y + MMI_title_height;
