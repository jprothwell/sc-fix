...
static UI_image_ID_type  idle_screen_wallpaper_sub_ID = 0;
...
extern void show_analog_clock(void);
...
void dm_category301_callback(dm_coordinates *coordinates)
{
    ...
    if (SUBLCD_MMI_message_string)
    {
        ...
        y = (SUBLCD_input_box.n_lines * fh) + 4 /* MULTILINE_INPUTBOX_HEIGHT_PAD */  + SUBLCD_input_box.text_y;
 ...
 *  time_diff2          [IN]        
 *  history_buffer      [IN]        History buffer
 *  STRING(?)           [IN]        Second message to be displayed (City name2)
 *  MYTIME(?)           [IN]        Time Difference for second City
 * RETURNS
 *  void
 *****************************************************************************/
...
#ifdef __MMI_KLG__
    color_t bc = gui_color(255, 255, 255);
#else 
...
void SUBLCD_UpdateWallpaper(void)
{
    ...
    else
    {
        ...
        {
            ret = gdi_image_draw_animation_file((SUB_LCD_device_width - cat312_image_width) >> 1, (SUB_LCD_device_height - cat312_image_height) >> 1, idle_screen_wallpaper_sub_name, &sublcd_wallpaper_animation_handle);       
        }
    }
    gdi_layer_pop_clip();
...
void SUBLCD_SetIdleScreenWallpaperName(UI_string_type name)
{
    ...
    pfnUnicodeStrcpy(idle_screen_wallpaper_sub_name, (S8*) name);
        ...
        U8 *history_buffer)
{
    ...
    ExitCategoryFunction = ExitCategory314Screen;
