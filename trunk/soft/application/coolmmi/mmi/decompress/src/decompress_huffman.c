#if defined(COMPRESS_USE_HUFFMAN)
/*
...
void *ch_realloc(void *ptr, unsigned newsize)
{
	OslMfree(ptr);
	return OslMalloc(newsize);
}
...
reverse_bits(unsigned char* bits, unsigned long numbits)
{
	...
	unsigned char *tmp =
	    (unsigned char*)OslMalloc(numbytes);
...
new_code(const huffman_node* leaf)
{
	...
	while(leaf && leaf->parent)
	{
		...
		if(cur_bit == 0)
		{
			...
			bits = (unsigned char*)ch_realloc(bits, newSize);
	...
	p = (huffman_code*)OslMalloc(sizeof(huffman_code));
	p->numbits = numbits;
	p->bits = bits;
	return p;
}
...
{
	huffman_node *p = (huffman_node*)OslMalloc(sizeof(huffman_node));
	p->isLeaf = 1;
	p->symbol = symbol;
	p->count = 0;
	p->parent = 0;
					  ...
					  unsigned int *pbufoutlen)
{
	...
	pc->cache = (unsigned char*)OslMalloc(cache_size);
...
static int flush_cache(buf_cache* pc)
{
	...
	if(pc->cache_cur > 0)
	{
		...
		unsigned char* tmp = ch_realloc(*pc->pbufout, newlen);
		if(!tmp)
			return 1;
					   ...
					   unsigned int to_write_len)
{
	...
	if(to_write_len > pc->cache_len - pc->cache_cur)
	{
		...
		tmp = ch_realloc(*pc->pbufout, newlen);
		if(!tmp)
			return 1;
...
calculate_huffman_codes(SymbolFrequencies * pSF)
{
	...
	pSE = (SymbolEncoder*)OslMalloc(sizeof(SymbolEncoder));
...
read_code_table(FILE* in, unsigned int *pDataBytes)
{
	...
	*pDataBytes = ntohl(*pDataBytes);
	...
	while(count-- > 0)
	{
		...
		bytes = (unsigned char*)OslMalloc(numbytes);
				 ...
				 SymbolEncoder *se)
{
	...
	for(vi = 0; vi < bufinlen; ++vi)
	{
		unsigned char uc = bufin[vi];
		huffman_code *code = (*se)[uc];
		unsigned long viIn;
		
		for(viIn= 0;viIn < code->numbits; ++viIn)
		{
			/* Add the current bit to curbyte. */
			curbyte |= get_bit(code->bits, viIn) << curbit;
						  ...
						  unsigned int *pbufoutlen)
{
	...
	buf = (unsigned char*)OslMalloc(data_count);

	/* Decode the memory. */
	p = root;
	for(; i < bufinlen && data_count > 0; ++i) 
	{
		unsigned char huff_byte = bufin[i];
		unsigned char mask = 1;
		while(data_count > 0 && mask)
		{
			p = huff_byte & mask ? p->one : p->zero;
			mask <<= 1;
