 ...
 * Description:
 * ------------
 *   This file is defines SAP for COOLSAND Bluetooth.
 *
 * Author:
 * -------
 * -------
...
{
	btbm_bd_addr_t	 bd_addr;
	unsigned char linkKey[0x10];
	unsigned char KeyType;
	unsigned int 	cod;
} bth_dev_struct;								//add by tan	

typedef struct 
{
	unsigned int 		devNum;
	bth_dev_struct	devInfo_Array[10];
	unsigned char 	pading[176];
}bth_dev_info_array;								//add by tan
