...
extern void mmi_opp_get_folder_name();

BOOL isSendObjUseFtps=1;
...
#define MMI_FTPS_LOG_STATE(i, x)                                                                            \
   {                                                                                                        \
      	FTP_DEBUG(kal_print("Function %s,Line:%d FTPS STATE_TRANS FROM STATE: %d TO STATE: %d",__FUNCTION__,__LINE__,(act_server_cntx_p+i)->ftps_state,x);); \
   }
...
#define FTPS_MAKE_RECV_PATH(x)   do {                      \
   if (g_hasMountedTFlashFS) \
   	csd_wsprintf( (kal_uint16 *)x, "%c:/", MMI_CARD_DRV);         \
   else	\
   	csd_wsprintf( (kal_uint16 *)x, "%c:/", MMI_PUBLIC_DRV);         \
   UCS2Strcat((PS8)x, (PS8)FTPS_RECV_OBJ_FILEPATH);                 \
}while(0)
#endif /* __MMI_FTS_SUPPORT__ */ 
...
WCHAR *tstftp = (WCHAR *)FTP_FOLDER_PATH;
 ...
 *  void
 *****************************************************************************/
void mmi_bt_ftp_event_hdlr(msg_type_t msg_id, void *msg)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
void mmi_bt_ftps_set_shared_fodler(PS8 path)
{
    ...
    UCS2Strcpy((PS8) ftps_cntx_p->root_folder, path);

    for (i = 0; i < FTP_MAX_CLIENT_SUPPORT; i++)
    {
        UCS2Strcpy((PS8) (act_server_cntx_p + i)->current_folder, (PS8) ftps_cntx_p->root_folder);
    }
}
...
void mmi_bt_ftpc_connect_server(mmi_bth_bt_addr bd_addr, U8 req_id)
{
    ...
    for (i = 0; i < FTP_MAX_SERVER_SUPPORT; i++)
    {
        ...
        MMI_FTP_LOG6(
            ...
            bd_addr.uap,
            bd_addr.nap);
    	 FTP_DEBUG(kal_print("mmi_bt_ftpc_connect_server (act_client_cntx_p + i)->ftpc_state =%d\n",(act_client_cntx_p + i)->ftpc_state);)
...
BOOL mmi_bt_ftpc_back_folder(void)
{
    ...
    switch (act_client_cntx_p->ftpc_state)
    {
        ...
        default:    /* Incorrect state */
            ...
            if (act_client_cntx_p->fh_ftpc_recv >= 0)
            {
                MMI_FS_Close(act_client_cntx_p->fh_ftpc_recv);
                act_client_cntx_p->fh_ftpc_recv = (FS_HANDLE) -1;
            }
            return MMI_FALSE;
    }
    return MMI_TRUE;
...
BOOL mmi_bt_ftpc_change_folder(PU16 name)
{
    ...
    if ((UCS2Strcmp((PS8) name, (PS8) L".") != 0) && (UCS2Strcmp((PS8) name, (PS8) L"..") != 0))
    {
        goep_set_folder_req_struct *req;
        ...
        MMI_FTP_ASSERT((FTP_MAX_OBJ_NAME_LEN / 2) >= (UCS2Strlen((PS8) name) + 1));

        tmp_name = OslMalloc(UCS2Strlen((PS8) name) * 2 + 2);
        memset(tmp_name, 0, UCS2Strlen((PS8) name) * 2 + 2);
        UCS2Strcpy((PS8) tmp_name, (PS8) name);

        for (i = 0; i <= UCS2Strlen((PS8) name); i++) /* includes null temination */
        {
            *(tmp_name + i) = mmi_bt_ftp_htons(*(tmp_name + i));
        }
        UCS2Strcpy((PS8) act_client_cntx_p->ftpc_obj_name, (PS8) tmp_name);
        OslMfree(tmp_name);
        ...
        UCS2Strcpy((PS8) req->folder_name, (PS8) act_client_cntx_p->ftpc_obj_name);
    ...
    else if (UCS2Strcmp((PS8) name, (PS8) L"..") == 0)
    {
...
BOOL mmi_bt_ftpc_get_file_obj(PU16 name)
{
    ...
    UCS2Strcpy((PS8) act_client_cntx_p->ftpc_obj_name, (PS8) name);
    for (i = 0; i <= UCS2Strlen((PS8) name); i++) /* includes null temination */
    {
    ...
    UCS2Strcpy((PS8) req->obj_name, (PS8) name);
...
#endif
    {
        case MMI_FTPC_TOTAL_LEN_NO_ZERO:
        {
         ...
         /*when get next file, total len may be No_First_Pkt
           ...
           */
            ...
            csd_wsprintf((U16*) recv_per_str_buf, "%d%c", prog, '%');
        }
            break;
        ...
        case MMI_FTPC_TOTAL_LEN_ZERO:
        {
           ...
           */
            ...
            {
                csd_wsprintf((U16*) recv_per_str_buf, "%dB", (U16) (recved_size));
            }
            else
            {
                csd_wsprintf((U16*) recv_per_str_buf, "%dB", 0);
            }
        }
            break;
    }
...
void mmi_bt_ftp_cntx_init(void)
{
    ...
    act_server_cntx_p->fh_ftps_recv = -1;
    act_server_cntx_p->fh_ftps_send = -1;
    act_server_cntx_p->fh_ftps_generating = -1;
#endif

#ifdef __MMI_FTC_SUPPORT__
    /* ftpc active connection base pointer init */
    act_client_cntx_p = &(ftpc_cntx_p->conn_cntx[0]);
    act_client_cntx_p->fh_ftpc_recv = -1;
#endif
...
MMI_BOOL mmi_bt_ftp_work_folder_init(void)
{
    ...
    h = MMI_FS_Open((const UINT8*) FTP_FOLDER_PATH, FS_READ_ONLY | FS_OPEN_DIR);
    if (h >= FS_NO_ERROR)
    {
        MMI_FS_Close(h);
    ...
    {
        ret = MMI_FS_CreateDir((const UINT8*) FTP_FOLDER_PATH);
        if (ret < 0)
        {
...
//        FTP_DEBUG(kal_print("ACCESS RIGHT %d\n", ftps_cntx_p->access_right);)
    ...
    h = MMI_FS_Open((const UINT8 *)FTP_SHARED_FOLDER_PATH_FILE, FS_READ_ONLY);
    ...
    if (h >= FS_NO_ERROR)
    {
        ...
        res = MMI_FS_Read(h, (void *)(ftps_cntx_p->root_folder), FMGR_MAX_PATH_LEN * ENCODING_LENGTH, (INT32 *)&read_length);
        MMI_FS_Close(h);
        if (res == FS_NO_ERROR)
        {
            MMI_FTP_LOG1(MMI_BT_FTP_FTPS_CNTX_INIT_FS_READ, res);
            if(!UCS2Strncmp((S8 *)(ftps_cntx_p->root_folder),(S8 *)L"D:",2) && !g_hasMountedTFlashFS)
            {
            FTPS_MAKE_RECV_PATH(ftps_cntx_p->root_folder);
	    ...
	    if (g_hasMountedTFlashFS != FALSE)
	    {
            ...
            h = MMI_FS_Open((const UINT8*) FTP_SHARED_FOLDER_PATH_FILE, (FS_CREATE_ALWAYS));
            if (h >= FS_NO_ERROR )
            {
            res = MMI_FS_Write(h, ftps_cntx_p->root_folder, (UCS2Strlen( ftps_cntx_p->root_folder) * 2 + 2), &read_length);
            MMI_FS_Close(h);
            }     
            }
            }
    ...
    if (g_hasMountedTFlashFS != FALSE)
    {
	     ...
	     h = MMI_FS_CreateDir((const UINT8*) ftps_cntx_p->root_folder);
	     h = MMI_FS_Open((const UINT8*) FTP_SHARED_FOLDER_PATH_FILE, (FS_CREATE_ALWAYS));
    	     if (h >= FS_NO_ERROR )
    	     {
    	        res = MMI_FS_Write(h, ftps_cntx_p->root_folder, (UCS2Strlen( ftps_cntx_p->root_folder) * 2 + 2), &read_length);
    ...
    for (i = 0; i < FTP_MAX_CLIENT_SUPPORT; i++)
    {
        ...
        if ((act_server_cntx_p + i)->fh_ftps_send >= 0)
        {
            MMI_FS_Close((act_server_cntx_p + i)->fh_ftps_send);
            (act_server_cntx_p + i)->fh_ftps_send = (FS_HANDLE) -1;
        }

        if ((act_server_cntx_p + i)->fh_ftps_recv >= 0)
        {
            MMI_FS_Close((act_server_cntx_p + i)->fh_ftps_recv);
            (act_server_cntx_p + i)->fh_ftps_recv = (FS_HANDLE) -1;
        }
...
int mmi_bt_ftps_write_obj(goep_push_ind_struct *ind, U32 i)
{
    ...
    FTP_DEBUG(kal_print("mmi_bt_ftps_write_obj 1 \n" ););
    ...
    UCS2Strcpy((PS8) all_path_name_ptr, (PS8) (act_server_cntx_p + i)->current_folder);
#if 0
/* under construction !*/
/* under construction !*/
/* under construction !*/
...
/* under construction !*/
#endif    
    UCS2Strcat((PS8) all_path_name_ptr, (PS8) (act_server_cntx_p + i)->ftps_obj_name);

    h = MMI_FS_Open((const UINT8*) all_path_name_ptr, ( FS_CREATE_ALWAYS));
    MMI_FTP_LOG1(MMI_BT_FTP_FTPS_WRITE_OBJ_OPEN, h);
    if (h < 0)
    {
        /* File open error. Error handling */
        mmi_bt_ftps_set_fs_flag((int) h);
        FTP_DEBUG(kal_print("mmi_bt_ftps_write_obj 1 h==%d \n" ,h););
    ...
    FTP_DEBUG(kal_print("mmi_bt_ftps_write_obj 2 \n" ););
    ...
    if (ind->frag_len > 0)
    {
        ...
        ret = MMI_FS_Write(h, ind->frag_ptr, ind->frag_len,  & len_written);
        ...
        if (ret < 0)
        {
            ...
            MMI_FS_Close((act_server_cntx_p + i)->fh_ftps_recv);
            MMI_FS_Delete((const UINT8*)all_path_name_ptr);
            (act_server_cntx_p + i)->fh_ftps_recv = (FS_HANDLE) -1;
...
void mmi_bt_ftps_push_obj(goep_push_ind_struct *ind, U32 i)
{
    ...
    FTP_DEBUG(kal_print("FTPS PUSH OBJ 1\n" ););
    if (isSendObjUseFtps)
    {
                memset((act_server_cntx_p + i)->current_folder, 0, FMGR_MAX_PATH_LEN * ENCODING_LENGTH);
                UCS2Strcpy((PS8) (act_server_cntx_p + i)->current_folder, MMI_OPP_CTX(recived_file_path));
    }
    /* Check default folder first */
    h = MMI_FS_Open((const UINT8*) ((act_server_cntx_p + i)->current_folder), FS_READ_ONLY | FS_OPEN_DIR);
    ...
    {
    	 FTP_DEBUG(kal_print("FTPS PUSH OBJ( h ==%d )\n",h ););
        mmi_bt_ftps_set_fs_flag((int) h);
    ...
    if (UCS2Strlen((S8*) ind->obj_name) > 0)
    {
        ...
        if (UCS2Strlen((S8*)cut_host_name) == 0)
        {
            FTP_DEBUG(kal_print("FTPS PUSH OBJ( OBJ_NAME IS ALL SPACE )\n"););            
            csd_wsprintf((kal_uint16*) (act_server_cntx_p + i)->ftps_obj_name, "new.tmp");
        }

        else
        {
            UCS2Strcpy((PS8) (act_server_cntx_p + i)->ftps_obj_name, (PS8) cut_host_name);
        }
    }
    ...
    /* check path length */
    UCS2Strcpy((PS8) path_buf, (PS8) (act_server_cntx_p + i)->current_folder);
    if (UCS2Strlen((PS8) path_buf) + UCS2Strlen((PS8) (act_server_cntx_p + i)->ftps_obj_name) >
        (FMGR_MAX_PATH_LEN - 1))
    {
    ...
    FTP_DEBUG(kal_print("FTPS PUSH OBJ 2\n" ););

    /* form complete path */
    UCS2Strcat((PS8) path_buf, (PS8) (act_server_cntx_p + i)->ftps_obj_name);
    ret = MMI_FS_GetAttributes((const UINT8*)path_buf);
    if (path_buf)
    {
        OslMfree(path_buf);
    }
	/* ret>=0: attr value, file already exists*/
	if ((ret == FS_FILE_NOT_FOUND) || (ret == FS_PATH_NOT_FOUND) || (ret == FS_ERROR_RESERVED))
	{
	...
	else if(ret < FS_NO_ERROR)
	{
		...
		{
			FTP_DEBUG(kal_print("FTPS PUSH OBJ( ret==%x )\n",ret););
			mmi_bt_ftps_set_fs_flag((int) ret);
			goto FTPS_PUSH_OBJ_ERR;
		}
	}
        FTP_DEBUG(kal_print("FTPS PUSH OBJ 3,ret = %d\n",ret ););
...
/* under construction !*/
    ...
    if (ret < 0)
    {
        FTP_DEBUG(kal_print("FTPS PUSH OBJ ret ==%d \n", ret););
        goto FTPS_PUSH_OBJ_ERR;
    }
    return;
  ...
  FTPS_PUSH_OBJ_ERR:
    ...
    FTP_DEBUG(kal_print("FTPS_PUSH_OBJ_ERR\n" ););
...
void mmi_bt_ftps_push_delete(goep_push_ind_struct *ind, U32 i)
{
    ...
    if (UCS2Strlen((S8*) ind->obj_name) > 0)
    {
        ...
        if (UCS2Strlen((const S8*)ind->obj_name) * 2 > (FTP_MAX_OBJ_NAME_LEN - 2))
        {
        ...
        if (UCS2Strlen((PS8) path_buf) + UCS2Strlen((PS8) tmp_name) > (FMGR_MAX_PATH_LEN - 1))
        {
            MMI_FTP_LOG(MMI_BT_FTP_FTPS_PUSH_DELETE_INVALID_NAME);
            /* send res with error rsp_code internal server error */
            res->rsp_code = GOEP_INTERNAL_SERVER_ERR;
        ...
        UCS2Strcat((PS8) path_buf, (PS8) tmp_name);
    }
    ...
    if ((ret = MMI_FS_Delete(
                (UINT8*) path_buf)) == -1)
    {
        ret = MMI_FS_RemoveDir(
                (UINT8*) path_buf);    
    }
...
void mmi_bt_ftps_write_obj_continue(goep_push_ind_struct *ind, U32 i)
{
    ...
    if((act_server_cntx_p + i)->fh_ftps_recv < 0) /* Cover usb plug in*/
    {
        return;
    }
    ...
    if (ind->frag_len > 0)
    {
        ...
        ret = MMI_FS_Write((act_server_cntx_p + i)->fh_ftps_recv, ind->frag_ptr, ind->frag_len, (INT32*) & len_written);
...
void mmi_bt_ftps_create_folder(goep_set_folder_ind_struct *ind, U32 i)
{
    ...
    if (UCS2Strlen((S8*) ind->folder_name) > 0)
    {
        ...
        if (UCS2Strlen((S8*)cut_host_name) == 0)
        {
            csd_wsprintf((kal_uint16*) cut_host_name, "new_folder");
        }
#if 0  
/* under construction !*/
/* under construction !*/
...
/* under construction !*/
    ...
    /* form folder path */
    UCS2Strcpy((PS8) all_path_name, (PS8) ((act_server_cntx_p + i)->current_folder));
    if (UCS2Strlen((PS8) all_path_name) + UCS2Strlen((PS8) cut_host_name) > (FMGR_MAX_PATH_LEN - 2))
    {
        /* send res to client with rsp_code server internal error */
        res->rsp_code = GOEP_INTERNAL_SERVER_ERR;
        goto FTPS_CREATE_FOLDER_SEND;
    }
    UCS2Strcat((PS8) all_path_name, (PS8) cut_host_name);
    UCS2Strcat((PS8) all_path_name, (const S8*)L"/");

    ret = MMI_FS_CreateDir((const UINT8*) all_path_name);
    ...
    {
        UCS2Strcpy((PS8) ((act_server_cntx_p + i)->current_folder), (PS8) all_path_name);
        res->rsp_code = GOEP_STATUS_SUCCESS;
        goto FTPS_CREATE_FOLDER_SEND;
    }
...
void mmi_bt_ftps_set_folder(goep_set_folder_ind_struct *ind, U32 i)
{
    ...
    if (UCS2Strlen((PS8) (ind->folder_name)) * 2 > FTP_MAX_OBJ_NAME_LEN - 2)
    {
    ...
    if ((UCS2Strcmp((PS8) folder_name, (PS8) L".") != 0) &&
        (UCS2Strcmp((PS8) folder_name, (PS8) L"..") != 0))
    {
        UCS2Strcpy((PS8) new_folder, (PS8) ((act_server_cntx_p + i)->current_folder));
        /* fail-safe for enough memory to store the folder_name + folder_path (2 = '\0','\') */
        if (UCS2Strlen((PS8) new_folder) + UCS2Strlen((PS8) folder_name) > (FMGR_MAX_PATH_LEN - 2))
        {
            FTP_DEBUG(kal_print("FTPS SET FOLDER CARRING INVALID FOLDER NAME\n"););
            /* send res with error rsp_code not found */
            res->rsp_code = GOEP_NOT_FOUND;
        ...
        /* form folder path */
        UCS2Strcat((PS8) new_folder, (PS8) folder_name);
        UCS2Strcat((PS8) new_folder, (const PS8)L"/");

        h = MMI_FS_Open((const UINT8*) new_folder, FS_READ_ONLY | FS_OPEN_DIR);
        /* if folder exist, set folder send res */
        if (h >= FS_NO_ERROR)
        {
            MMI_FS_Close(h);
            UCS2Strcpy((PS8) ((act_server_cntx_p + i)->current_folder), (PS8) new_folder);
    ...
    /* go to parent folder */
    else if (UCS2Strcmp((PS8) folder_name, (PS8) L"..") == 0)
    {
...
void mmi_bt_ftps_set_parent_folder(U32 i)
{
    ...
    NameLength = UCS2Strlen((PS8) ((act_server_cntx_p + i)->current_folder));
    UniPathName = OslMalloc((NameLength + 1) * ENCODING_LENGTH);
    if(!UniPathName)
    {
        res->rsp_code = GOEP_INTERNAL_SERVER_ERR;
        mmi_bt_ftp_send_msg(MSG_ID_GOEP_SET_FOLDER_RES, res);
    ...
    UCS2Strcpy((S8 * )UniPathName, (S8 *) ((act_server_cntx_p + i)->current_folder));
    ...
    if ((UCS2Strncmp(
                ...
                UCS2Strlen((PS8) ftps_cntx_p->root_folder)) != 0)
                || (NameLength == 0))
    {
        res->rsp_code = GOEP_INTERNAL_SERVER_ERR;
    }
    ...
    {
        FS_HANDLE temp_current_path_fh = MMI_FS_Open((const UINT8 *)UniPathName, FS_READ_ONLY | FS_OPEN_DIR);
        
        if(temp_current_path_fh >= FS_NO_ERROR)
        {
            MMI_FS_Close(temp_current_path_fh);
            UCS2Strcpy((S8 *)((act_server_cntx_p + i)->current_folder), (S8 *)UniPathName);
            res->rsp_code = GOEP_STATUS_SUCCESS;
        }
        else 
        {
 ...
 *  void
 *****************************************************************************/
volatile UINT32 BtFtpsSendTime;
volatile UINT32 BtFtpsSendFileSize;
...
void mmi_bt_ftps_pull_continue(U32 i)
{
    ...
    if (ret < 0)
    {
        FTP_DEBUG(kal_print("FTPS SEND CONTINUE FS_Read error,  ret = %d \n", ret);)
        MMI_FTPS_STATE_TRANS(i, MMI_FTPS_STATE_ACCEPTED);
        mmi_bt_ftp_blinking(MMI_BT_SERVER_ROLE, MMI_FTP_BLINK_STOP);
        ...
        if ((act_server_cntx_p + i)->send_obj_type == MMI_FTP_OBJ_TYPE_FOLDER)
        {
            FTP_DEBUG(kal_print("FTPS SEND CONTINUE ERR (FOLDER)\n");)
                ...
                MMI_FS_Delete((const UINT8*)((act_server_cntx_p + i)->folder_content));
        }
        (act_server_cntx_p + i)->send_obj_type = MMI_FTP_OBJ_TYPE_NONE;
	...
	BtFtpsSendTime = (csw_TMGetTick()-BtFtpsSendTime)/16384;
	if (BtFtpsSendTime >0)
		mmi_trace(1,"------------------------------BT BYTERATE = %d KByte/S------------------------------------\n",(BtFtpsSendFileSize/BtFtpsSendTime)/1024);
    }
...
void mmi_bt_ftps_send_obj(goep_pull_ind_struct *ind, U32 i)
{
    ...
    ret = MMI_FS_GetFileSize((act_server_cntx_p + i)->fh_ftps_send, (INT32 *)&fileSize);
    ...
    if (fileSize > 0)
    {
        ...
        res->rsp_code = GOEP_STATUS_SUCCESS;
	 BtFtpsSendTime = csw_TMGetTick ();
	 BtFtpsSendFileSize = fileSize;
    }
    /* Empty body */
    else
    {
...
void mmi_bt_ftps_send_folder_content(goep_pull_ind_struct *ind, U32 i)
{
    ...
    h = MMI_FS_Open((const UINT8*) ((act_server_cntx_p + i)->folder_content), FS_READ_ONLY);
    if (h < 0)
    {
    ...
    ret = MMI_FS_GetFileSize(h, (INT32 *)&fileSize);
...
static int mmi_bt_ftps_gen_folder_content(U32 index, PU8 folder_name)
{
    ...
    UI_character_type file_name[FMGR_MAX_FILE_NAME_LEN + 1];
    int result = 0;
    int i;
    U8 tmp_path_buf[(FMGR_MAX_FILE_NAME_LEN + 1) * ENCODING_LENGTH];
    isSendObjUseFtps = 0;
    ...
    if (UCS2Strcmp((PS8) folder_name, (PS8) ftps_cntx_p->root_folder))
    {
        composer->buflen += sprintf(composer->buf + composer->buflen, "<parent-folder />\n");
    }

    /* which means the files in folder "folder_path" */
    UCS2Strcat((S8*) folder_name, (const S8*)L"*");
    composer->fd = MMI_FS_Open((const UINT8*)((act_server_cntx_p + index)->folder_content), FS_CREATE_ALWAYS);
    /* FS error */
    if (composer->fd < 0)
    {
        return composer->fd;
    ...
    if (((act_server_cntx_p + index)->fh_ftps_generating = MMI_FS_FindFirst(
                                                            (const UINT8*) folder_name,
                                                            0,
                                                            0,
                                                            &fs_file_info,
                                                            (UINT8 *)file_name,
        ...
        0)
    {
          ...
          CONTINUE_WRITE_FOLDER_CONTENT:
            ...
            if ((fs_file_info.Attributes & (FS_ATTR_VOLUME | FS_ATTR_HIDDEN | FS_ATTR_SYSTEM)) == 0)
            {
                ...
                if (fs_file_info.Attributes & FS_ATTR_DIR)
                {
                    ...
                    if (result < 0)
                    {
                        MMI_FS_FindClose((act_server_cntx_p + index)->fh_ftps_generating);
                        MMI_FS_Close(composer->fd);
                        return result;
                    }
                    /* result >= 0 */
                    if (i > FTP_MAX_PARSE_FILE_INFO_NUMBER)
                    {
                        ...
                        {
                            result = MMI_FS_Write(composer->fd, composer->buf, composer->buflen, (INT32 *)&(composer->written));
        ...
        } while (MMI_FS_FindNext
                 ((act_server_cntx_p + index)->fh_ftps_generating, &fs_file_info, ( UINT8*)file_name,
            ...
            mmi_ftp_obj_info_struct *file_info)
{
    ...
    FTP_DEBUG(kal_print("FTPS COMPOSE FOLDER ELEMENT file name = %s\n",file_info->file_name);)
...
void mmi_bt_ftpc_cntx_init(void)
{
    ...
    for (i = 0; i < FTP_MAX_SERVER_SUPPORT; i++)
    {
        ...
        (act_client_cntx_p + i)->fh_ftpc_recv = (FS_HANDLE) -1;
...
void mmi_bt_ftpc_received_folder_init(void)
{
   ...
   {
	    /* Folder Path Init */
	    h = MMI_FS_Open((const UINT8 *)FTP_RECIVED_FOLDER_PATH_FILE, FS_READ_ONLY);
	    FTP_DEBUG(kal_print("MMI_BT_FTP_FTPS_CNTX_INIT_FS_OPEN, h=%d\n",h));
	    if (h >= FS_NO_ERROR)
	    {
	        FTP_DEBUG(kal_print("FS_OPEN NO ERROR\n");)
	            /* Read the shared folder path to the first connection context */
	            res = MMI_FS_Read(h, (act_client_cntx_p + i)->received_folder, FMGR_MAX_PATH_LEN * ENCODING_LENGTH, (INT32 *)&read_length);
	        MMI_FS_Close(h);

	        if (res == FS_NO_ERROR)
	        {
	            FTP_DEBUG(kal_print("MMI_BT_FTP_FTPS_CNTX_INIT_FS_READ, res=%d\n",res));
		     UCS2Strcpy(nPrintableStr, (act_client_cntx_p + i)->received_folder);
	        }
	        else    /* res != FS_NO_ERROR */
	        {
	            FTP_DEBUG(kal_print("FS_READ ERROR\n");)
	    	     FTPC_MAKE_RECV_PATH((act_client_cntx_p + i)->received_folder);
		     UCS2Strcpy(nPrintableStr, (act_client_cntx_p + i)->received_folder);
		     h = MMI_FS_Open((const UINT8*) FTP_RECIVED_FOLDER_PATH_FILE, (FS_CREATE_ALWAYS));
			 if (h < FS_NO_ERROR )
			 {
			        res = MMI_FS_Write(h, nPrintableStr, (UCS2Strlen(nPrintableStr) * 2 + 2), &read_length);
			        MMI_FS_Close(h);
			 }
	    ...
	    {
	        /* first time "empty shared folder file" */
	    	 MMI_FS_CreateDir((const UINT8*) FTP_FOLDER_PATH);
	        FTP_DEBUG(kal_print("FS_OPEN ERROR\n");)
	    	 FTPC_MAKE_RECV_PATH((act_client_cntx_p + i)->received_folder);
	        UCS2Strcpy(nPrintableStr, (act_client_cntx_p + i)->received_folder);
		 h = MMI_FS_Open((const UINT8*) FTP_RECIVED_FOLDER_PATH_FILE, (FS_CREATE_ALWAYS));
    ...
    h = MMI_FS_Open((const UINT8*) nPrintableStr, FS_READ_ONLY | FS_OPEN_DIR);
    ...
    {
        ret = MMI_FS_CreateDir((const UINT8*) nPrintableStr);
        ...
        {
            FTP_DEBUG(kal_print("FTPC RECEIVED FOLDER INIT -- ERROR WRITE FILE FAIL ret = %d \n",ret );)
...
int mmi_bt_ftpc_gen_dup_name(U32 i)
{
    ...
    while (fcnt < 1024)
    {
	 ...
	 UCS2Strcpy(path_buf, (act_client_cntx_p + i)->received_folder);
        /* prepare duplicated name */
        csd_wsprintf((kal_uint16*) nPrintableStr, "Dup(%d)", fcnt);
        pre_str_size = UCS2Strlen((S8*) nPrintableStr);

        /* Dup(%d)+ name */
        if (UCS2Strlen((S8*) ((act_client_cntx_p + i)->ftpc_obj_name)) + pre_str_size >
            (FTP_MAX_OBJ_NAME_LEN / 2 - 1))
        {
            ...
            /* filename -> tmp */
            offset += (U16) pre_str_size *2 + UCS2Strlen((S8*) ((act_client_cntx_p + i)->ftpc_obj_name)) * 2 + 2;
        ...
        if ((UCS2Strlen((PS8) path_buf) + UCS2Strlen((PS8) tmp_name2)) > (FMGR_MAX_PATH_LEN - 1))
        {
        ...
        /* form complete path */
        UCS2Strcat((PS8) path_buf, (PS8) tmp_name2);
        ret = MMI_FS_GetAttributes((const UINT8*)path_buf);
        /* file doesn't exist */
        if ((ret == FS_ERROR_RESERVED) ||(ret == FS_FILE_NOT_FOUND) || (ret == FS_PATH_NOT_FOUND))
        {
            /* unique name */
...
static int mmi_bt_ftpc_rename_recved_obj(U32 i)
{
    ...
    UCS2Strcpy(path_buf_actual, (act_client_cntx_p + i)->received_folder);
    UCS2Strcat((PS8) path_buf_actual, (PS8) (act_client_cntx_p + i)->ftpc_obj_name);


    ret = MMI_FS_GetAttributes((const UINT8*)path_buf_actual);
    ...
    ret = MMI_FS_Rename((const UINT8*) path_buf_tmp, (const UINT8*) path_buf_actual);
...
int mmi_bt_ftpc_write_obj(goep_pull_rsp_struct *rsp, U32 i)
{
    ...
    h = MMI_FS_Open((const UINT8*) path_buf, (FS_CREATE_ALWAYS));
    ...
    if (rsp->frag_len > 0)
    {
        ...
        ret = MMI_FS_Write(h, rsp->frag_ptr, rsp->frag_len, (INT32 *) & len_written);
        MMI_FTP_LOG1(MMI_BT_FTP_FTPC_WRITE_OBJ_WRITE, ret);

        /* Write error */
        if (ret < 0)
        {
            MMI_FS_Close((act_client_cntx_p + i)->fh_ftpc_recv);
            (act_client_cntx_p + i)->fh_ftpc_recv = (FS_HANDLE) -1;
            MMI_FS_Delete((const UINT8*) path_buf);
    ...
    if ((rsp->pkt_type == GOEP_FINAL_PKT) || (rsp->pkt_type == GOEP_SINGLE_PKT))
    {
        ...
        /* close file */
        if ((act_client_cntx_p + i)->fh_ftpc_recv >= 0)
        {
            MMI_FS_Close((act_client_cntx_p + i)->fh_ftpc_recv);
            (act_client_cntx_p + i)->fh_ftpc_recv = (FS_HANDLE) -1;
        }
...
int mmi_bt_ftpc_write_obj_continue(goep_pull_rsp_struct *rsp, U32 i)
{
    ...
    if (rsp->frag_len > 0)
    {
        ...
        MMI_FTP_ASSERT(((act_client_cntx_p + i)->fh_ftpc_recv)>=0);
        ret = MMI_FS_Write((act_client_cntx_p + i)->fh_ftpc_recv, rsp->frag_ptr, rsp->frag_len, (INT32 *) & len_written);
...
void mmi_bt_ftpc_del_tmp_obj(U32 i)
{
    ...
    h = MMI_FS_Open((const UINT8*) path_buf_tmp, FS_READ_ONLY);
    if (h >= 0)
    {
        MMI_FS_Close(h);
        MMI_FS_Delete((const UINT8*) path_buf_tmp);
    }
...
void mmi_bt_ftpc_get_folder(goep_pull_rsp_struct *rsp, U32 i)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_trace(g_sw_BT,"MMI_BT_FTP_FTPC_GET_FOLDERsp->rsp_code=%d, rsp->pkt_type=%d, rsp->to_self=%d\n", rsp->rsp_code, rsp->pkt_type, rsp->to_self);
        ...
        else
        {
            ...
            if (!ret)
            {
                ret = MMI_FS_Write(
                        ...
                        (INT32 *)&((act_client_cntx_p + i)->folder_content.written));
            }
            /* reset context */
            MMI_FS_Close((act_client_cntx_p + i)->folder_content.fd);
            (act_client_cntx_p + i)->folder_content.fd = (FS_HANDLE)-1;
            xml_close_parser((act_client_cntx_p + i)->xml_parser_p);
            OslMfree((act_client_cntx_p + i)->xml_parser_p);
            (act_client_cntx_p + i)->xml_parser_p = NULL;
    ...
    switch (rsp->rsp_code)
    {
        case GOEP_STATUS_SUCCESS:
        {
            ...
            if ((rsp->pkt_type == GOEP_FIRST_PKT) || (rsp->pkt_type == GOEP_SINGLE_PKT))
            {
                ...
                h = MMI_FS_Open((const UINT8*) ((act_client_cntx_p + i)->folder_obj), ( FS_CREATE_ALWAYS));
                MMI_FTP_LOG1(MMI_BT_FTP_FTPC_GET_FOLDER_OPEN, h);
                FTP_DEBUG(kal_print("(MMI_BT_FTP_FTPC_GET_FOLDER_OPEN ,ret=%d\n", h););
                if (h < 0)
                {
            ...
            if (rsp->frag_len > 0)
            {
                ...
                if ((act_client_cntx_p + i)->fh_ftpc_recv >= 0)
                {
                    ret = MMI_FS_Write(
                            ...
                            rsp->frag_len,
                            (INT32 *) & len_written);
                    FTP_DEBUG(kal_print("(MMI_BT_FTP_FTPC_GET_FOLDER_WRITE ,ret=%d\n", ret););
            ...
            if ((rsp->pkt_type == GOEP_FINAL_PKT) || (rsp->pkt_type == GOEP_SINGLE_PKT))
            {
                ...
                FTP_DEBUG(kal_print("(mmi_bt_ftpc_parse_folder_content ,ret=%d\n", ret););
              ...
              PARSE_DONE:
                ...
                MMI_FS_Delete((const  UINT8*)(act_client_cntx_p + i)->folder_obj);
    ...
    switch (rsp->rsp_code)
    {
        case GOEP_STATUS_SUCCESS:
        {
		    ...
		    if ((g_hasMountedTFlashFS == FALSE))
		    {
			    ...
			    INT32 read_length;
			    int res=0;
			    FS_HANDLE h = MMI_FS_Open((const UINT8 *)FTP_RECIVED_FOLDER_PATH_FILE, FS_READ_ONLY);
			    csd_wsprintf( (kal_uint16 *)path, "%c:/",'D');
	  		    if (h >= FS_NO_ERROR)
	  		    {
	  		            /* Read the shared folder path to the first connection context */
	  		            	res = MMI_FS_Read(h, (void *)tmppath, FMGR_MAX_PATH_LEN * ENCODING_LENGTH, &read_length);
					MMI_FS_Close(h);
					if (res == FS_NO_ERROR)
					{
						if (!UCS2Strncmp(tmppath, path,3))
							res = 1;
						else res = 0;
					}
					else
...
static int mmi_bt_ftpc_parse_folder_content(U32 i)
{
    ...
    {
        (act_client_cntx_p + i)->xml_parser_p = OslMalloc(sizeof(XML_PARSER_STRUCT));
    }

    memset((act_client_cntx_p + i)->xml_parser_p, 0, sizeof(XML_PARSER_STRUCT));
    memset(&((act_client_cntx_p + i)->folder_content), 0, sizeof(mmi_ftp_file_writer_struct));

    (act_client_cntx_p + i)->folder_content.fd = MMI_FS_Open((const UINT8*)(act_client_cntx_p + i)->parsed_folder, FS_CREATE_ALWAYS);
        ...
        if (attr != NULL)
        {
            while ((attr[i]) != 0)
            {
                if (!strcmp("name", attr[i]))
                {
                    ...
                    else
                    {
                        ...
                        if((FTP_MAX_OBJ_NAME_LEN - 2) > ext_name_len * 2)
                        {
                            ...
                            if (ext_name_len > 0)
                            {
                                ext_name_new = OslMalloc(ext_name_len * 2 + 2);
                ...
                else if (!strcmp("size", attr[i]))
                {
                    ...
                    {
                        csd_wsprintf((WCHAR*) (act_client_cntx_p + idx)->obj_entity.size, "%dB", (U16) (file_size));
                    }
                }
                i += 2;
            }
            if (UCS2Strlen((PS8) ((act_client_cntx_p + idx)->obj_entity.size)) == 0)
            {
                kal_wstrcpy((U16*) (act_client_cntx_p + idx)->obj_entity.size, (U16*) GetString(STR_GLOBAL_EMPTY));
            }
            if (UCS2Strlen((PS8) ((act_client_cntx_p + idx)->obj_entity.created_date)) == 0)
            {
                kal_wstrcpy(
...
void mmi_bt_ftp_disconnect_ind_hdlr(void *msg)
{
    ...
    FTP_DEBUG(kal_print("mmi_bt_ftp_disconnect_ind_hdlr ind->req_id=%d,ind->goep_conn_id = %d\n",ind->req_id,ind->goep_conn_id);)
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
#ifdef __MMI_FTS_SUPPORT__  
...
void mmi_bt_ftps_push_ind_hdlr(void *msg)
{
    ...
    /* Check the access right first. If the access right is read only, return failed directly */
    if ( !isSendObjUseFtps && ftps_cntx_p->access_right == MMI_FTP_ACCESS_RIGHT_READ_ONLY)
    {
        /* Wrong Access Right */
    ...
    mmi_trace(1,"mmi_bt_ftps_push_ind_hdlr  (act_server_cntx_p + i)->ftps_state =%d,ind->put_type=%d,ind->pkt_type = %d\n",(act_server_cntx_p + i)->ftps_state,ind->put_type,ind->pkt_type);
...
void mmi_bt_ftps_pull_ind_hdlr(void *msg)
{
    ...
    switch ((act_server_cntx_p + i)->ftps_state)
    {
        case MMI_FTPS_STATE_ACCEPTED:
            ...
            if (UCS2Strlen((PS8) ind->obj_name) > 0)
            {
                /* fail-safe for enough memory to store the obj_name */
                if (UCS2Strlen((PS8) ind->obj_name) * 2 > FTP_MAX_OBJ_NAME_LEN - 2)
                {
            ...
            if (strcmp((PS8) ind->obj_mime_type, (PS8) folder_listing_obj))
            {
                ...
                h = MMI_FS_Open((const UINT8*) path_buf, FS_READ_ONLY);
                if (h < 0)
                {
            ...
            {
                /* fail-safe for enough memory to store the obj_name + obj_path (2 = '\','\0') */
                if (UCS2Strlen((PS8) path_buf) + UCS2Strlen((PS8) tmp_name) > (FMGR_MAX_PATH_LEN - 2))
                {
                    FTP_DEBUG(kal_print("FTPS PULL IND HDLR FS_OPEN ERROR\n");)
                    goto FTPS_PULL_IND_ERR;
                }
  ...
  FTPS_PULL_IND_CONTINUE_WRITE_FOLDER:
    ...
    {
        MMI_FS_FindClose((act_server_cntx_p + i)->fh_ftps_generating);
        MMI_FS_Close(((act_server_cntx_p + i)->xml_composer).fd);
        MMI_FS_Delete((const UINT8*)((act_server_cntx_p + i)->folder_content));
    ...
    switch ((act_server_cntx_p + i)->ftps_state)
    {
        ...
        case MMI_FTPS_STATE_ACCEPTED:
            ...
            if (ind->setpath_flag == GOEP_ROOT_FOLDER)
            {
                ...
                memset((act_server_cntx_p + i)->current_folder, 0, FMGR_MAX_PATH_LEN * ENCODING_LENGTH);
                UCS2Strcpy((PS8) (act_server_cntx_p + i)->current_folder, (PS8) ftps_cntx_p->root_folder);
                temp_current_path_fh = MMI_FS_Open((const UINT8 *)((act_server_cntx_p + i)->current_folder), FS_READ_ONLY | FS_OPEN_DIR);
                if(temp_current_path_fh >= FS_NO_ERROR)
                {
                    MMI_FS_Close(temp_current_path_fh);
...
void mmi_bt_ftpc_connect_rsp_hdlr(void *msg)
{
    ...
    FTP_DEBUG(kal_print("mmi_bt_ftpc_connect_rsp_hdlr FTPC CONNECT RSPrsp->rsp_code=%d, (act_client_cntx_p + i)->ftpc_state=%d\n",rsp->rsp_code,(act_client_cntx_p + i)->ftpc_state);)
    ...
    {
        FTP_DEBUG(kal_print("FTPC CONNECT RSP ERROR (act_client_cntx_p+i)->ftpc_state=%d\n",(act_client_cntx_p+i)->ftpc_state);)
        switch ( (act_client_cntx_p+i)->ftpc_state )
        {
            /* discard this msg: connect failed cm call back has been called when connect cancel*/
            case MMI_FTPC_STATE_DISCONNECTING:
...
void mmi_bt_ftpc_pull_rsp_hdlr(void *msg)
{
    ...
    if (i == 0xff)
    {
     ...
     mmi_trace(g_sw_BT,"MMI_BT_FTP_FTPC_PULL_RSP_HDLR, (act_client_cntx_p + i)->ftpc_state=%d\n",(act_client_cntx_p + i)->ftpc_state);

   if (rsp->rsp_code == GOEP_STATUS_DISCONNECT)
   {
   	      FTP_DEBUG(kal_print("FTPC GET OBJ RSP ERROR\n");)
            if ((act_client_cntx_p+i)->ftpc_state != MMI_FTPC_STATE_IDLE)
            {
                mmi_bt_ftpc_notify_recv_err(MMI_FTPC_NOTIFY_CONNECTION_DISCONNECTED);
            }
            if ((act_client_cntx_p + i)->fh_ftpc_recv >= 0)
            {
                MMI_FS_Close((act_client_cntx_p + i)->fh_ftpc_recv);
            ...
            mmi_bt_ftp_blinking(MMI_BT_CLIENT_ROLE, MMI_FTP_BLINK_STOP);
	     if (GOEP_STATUS_DISCONNECT == rsp->rsp_code)
	     {
		mmi_bt_ftpc_set_abort_req_sent_flag();
		MMI_FTPC_STATE_TRANS(i, MMI_FTPC_STATE_DISCONNECTING);
        	mmi_bt_ftpc_send_abort(rsp->goep_conn_id);
	     }
	return;
   }
...
void mmi_bt_ftpc_abort_rsp_hdlr(void *msg)
{
   ...
   if (rsp->rsp_code == GOEP_STATUS_DISCONNECT)
   {
    ...
    mmi_trace(g_sw_BT,"MMI_BT_FTP_FTPC_ABORT_RSP_HDLR, (act_client_cntx_p + i)->ftpc_state = %d\n",(act_client_cntx_p + i)->ftpc_state);
...
void mmi_bt_ftp_send_msg(U32 msg_id, void *p_local_para)
{
    ...
    ilm_send->msg_id = (kal_uint32) msg_id;
...
void mmi_bt_ftp_connection_terminated(U32 index, MMI_BT_PROFILE_ROLE role)
{
    ...
    switch (role)
    {
        case MMI_BT_SERVER_ROLE:    /* FTPS */
            ...
            if ((act_server_cntx_p + index)->fh_ftps_send >= 0)
            {
                MMI_FS_Close((act_server_cntx_p + index)->fh_ftps_send);
                (act_server_cntx_p + index)->fh_ftps_send = (FS_HANDLE) -1;
            }
            if ((act_server_cntx_p + index)->send_obj_type == MMI_FTP_OBJ_TYPE_FOLDER)
            {
                FTP_DEBUG(kal_print("FTPS CONNECTION TERMINATED DEL FOLDER CONTENT FILE\n");)
                    /* delete the folder content file */
                    MMI_FS_Delete((const UINT8*)((act_server_cntx_p + index)->folder_content));
            }
            (act_server_cntx_p + index)->send_obj_type = MMI_FTP_OBJ_TYPE_NONE;

            if ((act_server_cntx_p + index)->fh_ftps_recv >= 0)
            {
                MMI_FS_Close((act_server_cntx_p + index)->fh_ftps_recv);
                (act_server_cntx_p + index)->fh_ftps_recv = (FS_HANDLE) -1;
            }

            UCS2Strcpy((PS8) (act_server_cntx_p + index)->current_folder, (PS8) ftps_cntx_p->root_folder);
        ...
        default:    /* FTPC */
#ifdef __MMI_FTC_SUPPORT__
            {
                ...
                if((act_client_cntx_p + index)->xml_parser_p)
                {
                    ...
                    OslMfree((act_client_cntx_p + index)->xml_parser_p);
                    (act_client_cntx_p + index)->xml_parser_p = NULL;

                    /* delete the temp xhtml file */
                    MMI_FS_Delete((const UINT8*)(act_client_cntx_p + index)->folder_obj);

                    /* delete the parsed file*/
                    if((act_client_cntx_p + index)->folder_content.fd)
                    {
                        MMI_FS_Close((act_client_cntx_p + index)->folder_content.fd);
                        MMI_FS_Delete((const UINT8*)((act_client_cntx_p + index)->parsed_folder));
                        (act_client_cntx_p + index)->folder_content.fd = (FS_HANDLE)-1; 
                    }
                }
                else
                {
                    /* Receiving */
                    if((act_client_cntx_p+index)->fh_ftpc_recv >= 0)
                    {
                        MMI_FS_Close((act_client_cntx_p+index)->fh_ftpc_recv);
                        (act_client_cntx_p + index)->fh_ftpc_recv = (FS_HANDLE) -1;
                    }
...
static void mmi_bt_ftp_obj_name_swapcpy(PU8 h_order_name, PU8 n_order_name)
{
    ...
    /*----------------------------------------------------------------*/
    U32 i;
    U32 bytes_len = UCS2Strlen((PS8) n_order_name) * 2;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    FTP_DEBUG(kal_print("FTP OBJ NAME SWAPCPY\n");)
    for (i = 0; i < UCS2Strlen((PS8) n_order_name) * 2; i += 2)    /* includes null temination */
    {
...
static int mmi_bt_ftp_write_buf2file(mmi_ftp_file_writer_struct *composer)
{
    ...
    if (composer->buflen >= FTP_WRITER_BUF_THRESHOLD)
    {
        ...
        ret = MMI_FS_Write(composer->fd, composer->buf, composer->buflen, (INT32 *)&(composer->written));
        FTP_DEBUG(kal_print("MMI_BT_FTP_WRITE_BUF2FILE_FS_WRITE, ret=%d\n",ret);)
	 if (ret == FS_DISK_FULL)
	 {
	 	 if (ret == FS_DISK_FULL)
		 {
 ...
 *  
 *****************************************************************************/
 kal_int8 *mmi_ucs2rchr(const kal_int8 * string, kal_wchar ch)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
static void mmi_bt_ftp_ucs2_cut_name_with_ext(U8 *dest, U32  dest_size, const U8 *src)
{
    ...
    /* need not to cut*/
    if(UCS2Strlen((S8 *)src_no_space) <= (dest_size/2 -1))
    {
        UCS2Strncpy((PS8)dest, (const PS8)src_no_space, (U32)(dest_size/2 -1));
    }
    else
    {
        char *p_ext_name;
        ...
        p_ext_name = mmi_ucs2rchr((const PS8)src_no_space, (U16)(_T_MMI'.'));
        if(p_ext_name)
        {
            ext_name_len = UCS2Strlen((const PS8)p_ext_name);
        }
        else
        {
            ext_name_len = 0;
        ...
        UCS2Strncpy((PS8)dest, (const PS8)src_no_space, (U32)((dest_size/2 -1) - ext_name_len));
        UCS2Strcat((PS8)dest, (const PS8)p_ext_name);
    }
}
...
static MMI_BOOL mmi_bt_ftp_check_folder_existed(U8* absolute_path)
{
    ...
    path_len = UCS2Strlen((PS8) absolute_path);
    if (path_len * 2 > (FMGR_MAX_PATH_LEN - 2))
    {
        return MMI_FALSE;
    }
        
    if ((fh_checked_folder = MMI_FS_Open((const UINT8 *)absolute_path, FS_READ_ONLY | FS_OPEN_DIR )) < FS_NO_ERROR)
    {
        ...
        for (start_pos = 0; start_pos < path_len * 2; start_pos += 2)
        {
            if(memcmp((absolute_path + start_pos), &deter, 2) == 0)
            {
                ...
                if((fh_parent = MMI_FS_Open((const UINT8 *)parent_folder, FS_READ_ONLY | FS_OPEN_DIR)) < FS_NO_ERROR)
                {
                    if(MMI_FS_CreateDir((const UINT8 *)parent_folder) < FS_NO_ERROR)
                    {
                        return MMI_FALSE;
                    }
                }
    ...
    {
        MMI_FS_Close(fh_checked_folder);
        return MMI_TRUE;
    }
...
static void mmi_bt_ftp_delete_file(const U8* absolute_path, const U8* file_name)
{
    ...
    /* form obj name */
    UCS2Strcpy((PS8) all_path_name_ptr, (PS8) absolute_path);
    UCS2Strcat((PS8) all_path_name_ptr, (PS8) file_name);

    MMI_FS_Delete((UINT8 *) all_path_name_ptr);
