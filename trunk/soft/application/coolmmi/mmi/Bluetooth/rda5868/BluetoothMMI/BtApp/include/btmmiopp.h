...
typedef struct
{
    ...
    S8 recived_file_path[FMGR_MAX_PATH_LEN *ENCODING_LENGTH];
...
#define MMI_OPS_LOG_STATE_ERR()                                                    \
   {                                                                               \
   	mmi_trace(1,"MMI_OPS_LOG_STATE_ERR MMI_OPP_CTX(ops_state) =%d\n",MMI_OPP_CTX(ops_state));\
   }
//      MMI_TRACE(MMI_TRACE_G7_MISC, MMI_OPP_STATE_ERROR, MMI_OPP_CTX(ops_state) );

#define MMI_OPC_LOG_STATE_ERR()                                                    \
   {                                                                               \
      	mmi_trace(1,"MMI_OPS_LOG_STATE_ERR MMI_OPP_CTX(opc_state) =%d\n",MMI_OPP_CTX(opc_state));\
   }
...
#define MMI_OPC_LOG_STATE(x)                                                               \
   {                                                                                       \
      	FTP_DEBUG(kal_print("Function %s,Line:%d OPC STATE_TRANS FROM STATE: %d TO STATE: %d",__FUNCTION__,__LINE__,MMI_OPP_CTX(opc_state),x);); \
    }
...
#define MMI_OPP_SND_DEFAULT_OBJNAME "COOLSAND.obj"

#define MMI_OPP_MAKE_RECV_FILENAME(x, d)     do{           \
   U8 temp_name[32];        \
   UCS2Strcpy(x, MMI_OPP_CTX(recived_file_path)); \
   csd_wsprintf((kal_uint16 *)temp_name,"%s%02d.%s", MMI_OPP_RECV_OBJ_FILENAME, d, MMI_OPP_RECV_OBJ_FILEEXT);              \
...
#define MMI_OPP_MAKE_RECV_PATH(x)     do{         \
   ...
   UCS2Strcat((PS8) x, (PS8) MMI_OPP_RECV_OBJ_FILEPATH);      \
}while(0)

#define MMI_OPP_MAKE_RECV_STORE_NAME(x, y)   do{     \
   csd_wsprintf( (kal_uint16 *)x, "%c:/", MMI_CARD_DRV);         \
   UCS2Strcat((PS8) x, (PS8) MMI_OPP_RECV_OBJ_FILEPATH);  \
   UCS2Strcat((PS8) x, (PS8) y);    \
}while(0)

#define MMI_OPP_MAKE_RECV_PATH_FLASH(x)     do{         \
   	csd_wsprintf( (kal_uint16 *)x, "%c:/",MMI_PUBLIC_DRV); \
   UCS2Strcat((PS8) x, (PS8) MMI_OPP_RECV_OBJ_FILEPATH);      \
}while(0)
#endif /* __OPP_MMI_H__ */ 
