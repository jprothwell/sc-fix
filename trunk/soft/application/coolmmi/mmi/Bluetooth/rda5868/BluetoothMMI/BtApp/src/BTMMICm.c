 ...
 * Description:
 * ------------
 *   This file is for COOLSAND bluetooth application.
 *
 * Author:
 * -------
 * -------
...
/*  customization header file */
#if defined(MMI_ON_HARDWARE_P) && !defined(MMI_ON_WIN32)
...
extern bth_dev_info_array g_paired_devInfo;				//add by tan

const U8 MMI_BT_DEF_HOST_BT_NAME[] = { 'C', 'S', 'D', 'B', 'T', 'D', 'E', 'V', 'I', 'C', 'E', '\0' };
...
#endif
#ifdef __MMI_AVRCP_SUPPORT__
MMI_BTH_AV_REMOTE_CONTROL_UUID,
#endif
#ifdef __MMI_BPP_SUPPORT__
 ...
 *****************************************************************************/
 ...
 extern void rdabt_set_default_bd_addr(void);
 #endif
void mmi_bth_init(void)
{
    ...
    {
		memset(&g_paired_devInfo.devInfo_Array[i], 0, sizeof(bth_dev_struct));
		g_paired_devInfo.devInfo_Array[i].bd_addr.lap = g_mmi_bth_cntx.paired_list[i].bd_addr.lap;
		g_paired_devInfo.devInfo_Array[i].bd_addr.nap= g_mmi_bth_cntx.paired_list[i].bd_addr.nap;
		g_paired_devInfo.devInfo_Array[i].bd_addr.uap= g_mmi_bth_cntx.paired_list[i].bd_addr.uap;
		g_paired_devInfo.devInfo_Array[i].cod = g_mmi_bth_cntx.paired_list[i].cod;
		g_paired_devInfo.devInfo_Array[i].KeyType = g_mmi_bth_cntx.paired_list[i].keytype;
		memcpy(g_paired_devInfo.devInfo_Array[i].linkKey, g_mmi_bth_cntx.paired_list[i].linkKey,16);
    }

         mmi_trace(g_sw_BT, "mmi_bth_init");
		mmi_trace_hex(g_sw_BT,16, g_mmi_bth_cntx.paired_list[0].linkKey);
		mmi_trace(g_sw_BT, "g_mmi_bth_cntx.paired_list[i].keytype=%d",g_mmi_bth_cntx.paired_list[0].keytype);

	g_paired_devInfo.devNum = g_mmi_bth_cntx.paired_dev_num;
...
//   mmi_trace(1,"rdabt_get_local_addr_req error = %d\n",error);
     ...
     ReadValue( NVRAM_CUSTOMER_BT_STORGE_FLASH_SETTED, &data, DS_BYTE , &error);

      if(error != NVRAM_READ_SUCCESS) /* Failed to read in NVRAM */
        {
          data = TRUE;
          WriteValue( NVRAM_CUSTOMER_BT_STORGE_FLASH_SETTED, &data, DS_BYTE , &error);
        }
...
#ifdef __MMI_BT_POWER_OFF__
extern  void mmi_bt_power_off(void *msg);
#endif
...
//    MMI_TRACE(MMI_TRACE_G7_MISC, MMI_BT_G7_INIT_EVENT_HDLR);
...
#ifdef __MMI_BT_POWER_OFF__
    SetProtocolEventHandler(mmi_bt_power_off, MSG_ID_BT_POWEROFF_IND);
#endif
    ...
    SetProtocolEventHandler(mmi_bth_cm_be_bond_cnf_hdler, MSG_ID_BT_BM_BE_BONDING_CNF);
...
void mmi_bth_nvram_reset_info(void)
{
   ...
   MMI_BT_ASSERT(NVRAM_EF_BT_INFO_SIZE == sizeof(mmi_bth_nvram_struct)); 
    memset(&write_struct, 0, sizeof(mmi_bth_nvram_struct));

//    MMI_TRACE(MMI_TRACE_G7_MISC, MMI_BT_G7_NVR_WRITE, write_struct.flag, write_struct.paired_dev_num);

    WriteRecord(NVRAM_EF_BT_INFO_LID, 1, &write_struct, sizeof(mmi_bth_nvram_struct), &err);
 ...
 *  void
 *****************************************************************************/
//extern  void sxs_Raise   (u32 Id, const ascii *Fmt, ...);
...
void mmi_bth_nvram_read_info(void)
{
    ...
    S16 err = 0, bth_nvram_size = 0;
     ...
     mmi_trace(g_sw_BT,"mmi_bth_nvram_read_info ::begin");
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	mmi_trace(g_sw_BT, "mmi_bth_nvram_read_info::read_struct=0x%x",&read_struct );

    bth_nvram_size = sizeof(mmi_bth_nvram_struct);
    if (bth_nvram_size != NVRAM_EF_BT_INFO_SIZE)
    {
//	asm volatile("break 1\n\tnop")
 //sxs_Raise(1024, "mmi_bth_nvram_read_info::read_struct=0x%x",&read_struct );

        //MMI_BT_ASSERT(0); /*report assertion failure when power up*/
    }
 //   MMI_BT_ASSERT(NVRAM_EF_BT_INFO_SIZE == bth_nvram_size); /*report assertion failure when power up*/


    ReadRecord(NVRAM_EF_BT_INFO_LID, 1, &read_struct, sizeof(mmi_bth_nvram_struct), &err);
    ...
    else
    {
      ...
      mmi_trace(g_sw_BT,"mmi_bth_nvram_read_info::g_mmi_bth_cntx.flag=%d, g_mmi_bth_cntx.paired_dev_num=%d", g_mmi_bth_cntx.flag, g_mmi_bth_cntx.paired_dev_num);
...
void mmi_bth_nvram_write_info(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
//    MMI_BT_ASSERT(NVRAM_EF_BT_INFO_SIZE == sizeof(mmi_bth_nvram_struct)); /*report assertion failure when power up*/
    ...
    mmi_trace(g_sw_BT, "mmi_bth_nvram_write_info::write_struct.flag=%d, write_struct.paired_dev_num=%d", write_struct.flag, write_struct.paired_dev_num);
//    MMI_TRACE(MMI_TRACE_G7_MISC, MMI_BT_G7_NVR_WRITE, write_struct.flag, write_struct.paired_dev_num);
...
void mmi_bth_init_freshdev_default(U8 *name_p)
{
    ...
    kal_sprintf((kal_char*) g_mmi_bth_cntx.host_dev.name, "%s",(kal_char*) name_p);
...
void mmi_bth_antenna_on_rsp_hdler(void *msg)
{
    ...
    /* reset bt ready mask */
	U8 result;
	bt_bm_PowerON_cnf_struct *msg_p = (bt_bm_PowerON_cnf_struct*) msg;
	...
	else
	{
		...
		MMI_BTH_RESET_FLAG(MMI_BTH_MASK_ACTIVATED);
	       mmi_trace(g_sw_BT,"before mmi_bth_is_activated= %d", mmi_bth_is_activated());
		if (g_mmi_bth_cntx.scr_cb_tbl.mmi_bt_scr_cb_activate)
		{
			(*g_mmi_bth_cntx.scr_cb_tbl.mmi_bt_scr_cb_activate) ((MMI_BOOL) result);
		       mmi_trace(g_sw_BT,"later mmi_bth_is_activated= %d", mmi_bth_is_activated());
...
void mmi_bth_ready_ind_hdler(void *msg)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	mmi_trace(g_sw_BT, "mmi_bth_ready_ind_hdler");
...
void mmi_bth_cm_discovery_cnf_hdler(void *msg)
{
	...
	mmi_trace(g_sw_BT, "mmi_bth_cm_discovery_cnf_hdler::msg_p->result = %d,msg_p->total_number=%d,g_mmi_bth_cntx.discovered_dev_num=%d", msg_p->result,msg_p->total_number,g_mmi_bth_cntx.discovered_dev_num );
   ...
   else if (msg_p->result == 14)
	{
	        /* Indicate MMI Scr */
	        if (g_mmi_bth_cntx.scr_cb_tbl.mmi_bt_scr_cb_Cant_inquiry_cfm)
	        {
	            (*g_mmi_bth_cntx.scr_cb_tbl.mmi_bt_scr_cb_Cant_inquiry_cfm) ();
	        }
...
void mmi_bth_cm_sdc_search_ind_hdler(void *msg)
{
	...
	mmi_trace(g_sw_BT, "mmi_bth_cm_sdc_search_ind_hdler:: service_profile_uuid= %X", service_profile_uuid);
    ...
    switch (g_mmi_bth_cntx.state)
    {
        case MMI_BTH_STAT_SDC_SEARCHING:
			...
			mmi_trace(g_sw_BT,"mmi_bth_cm_sdc_search_ind_hdler::MMI_BTH_STAT_SDC_SEARCHING\nservice_count=%d, service_profile_uuid=%X",
			                                            service_count, service_profile_uuid);
        ...
        case MMI_BTH_STAT_IDLE:
            ...
            /* add to paired list */
			mmi_trace(g_sw_BT,"mmi_bth_cm_sdc_search_ind_hdler:: service_count=%d, service_profile_uuid=%X",
			                                            service_count, service_profile_uuid);
		...
		mmi_trace(g_sw_BT,"g_mmi_bth_cntx.paired_list[i=%d].service_list[service_count]=%X",i,  g_mmi_bth_cntx.paired_list[i].service_list[service_count] );
		mmi_trace(g_sw_BT,"g_mmi_bth_cntx.paired_list[i=%d].service_list_num=%d",i,  g_mmi_bth_cntx.paired_list[i].service_list_num);
...
void mmi_bth_cm_sdc_search_cnf_hdler(void *msg)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
   mmi_trace(g_sw_BT, "mmi_bth_cm_sdc_search_cnf_hdler:: msg_p->searched_number=%d msg_p->result= %d, g_mmi_bth_cntx.state=%d"
                                        , msg_p->searched_number, msg_p->result, g_mmi_bth_cntx.state);
    switch (g_mmi_bth_cntx.state)
    {
        case MMI_BTH_STAT_SDC_SEARCHING:
            MMI_BTH_STATE_TRANS(MMI_BTH_STAT_IDLE);
        ...
        case MMI_BTH_STAT_IDLE:
		   ...
		   mmi_trace(g_sw_BT, "mmi_bth_cm_sdc_search_cnf_hdler::g_mmi_bth_cntx.scr_cb_tbl.mmi_bt_scr_cb_refresh_prof_list");
   		mmi_trace(g_sw_BT,"g_mmi_bth_cntx.paired_list[i=%d].service_list[service_count]=%X",i,  g_mmi_bth_cntx.paired_list[i].service_list[0] );
   		mmi_trace(g_sw_BT,"g_mmi_bth_cntx.paired_list[i=%d].service_list[service_count]=%X",i,  g_mmi_bth_cntx.paired_list[i].service_list[1] );
		mmi_trace(g_sw_BT,"g_mmi_bth_cntx.paired_list[i=%d].service_list_num=%d",i,  g_mmi_bth_cntx.paired_list[i].service_list_num);
...
void mmi_bth_cm_link_connect_accept_ind_hdler(void *msg)
{
    ...
    mmi_trace(g_sw_BT, " g_mmi_bth_cntx.state=%d, g_mmi_bth_cntx.flag=%d, g_mmi_bth_cntx.ram_flag=%d, g_mmi_bth_cntx.deactivate_flag=%d",
			     g_mmi_bth_cntx.state, g_mmi_bth_cntx.flag, g_mmi_bth_cntx.ram_flag, g_mmi_bth_cntx.deactivate_flag);

    cm_state = g_mmi_bth_cntx.state;
    
    if (cm_state != MMI_BTH_STAT_IDLE )//|| mmi_flight_mode_is_switching()== MMI_TRUE temp del
    {
	...
	mmi_trace(g_sw_BT, "paired_dev_index=%d, paired_dev_num=%d", paired_dev_index,paired_dev_num);
...
void mmi_bth_cm_bond_ind_hdler(void *msg)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	mmi_trace(g_sw_BT,"mmi_bth_cm_bond_ind_hdler:: g_mmi_bth_cntx.state=%d, g_mmi_bth_cntx.flag=%d, g_mmi_bth_cntx.ram_flag=%d, g_mmi_bth_cntx.deactivate_flag=%d",
	                                   g_mmi_bth_cntx.state, g_mmi_bth_cntx.flag, g_mmi_bth_cntx.ram_flag, g_mmi_bth_cntx.deactivate_flag);
	...
	g_mmi_bth_cntx.discovered_list[g_mmi_bth_cntx.action_index].keytype = msg_p->KeyType;
	memcpy(g_mmi_bth_cntx.discovered_list[g_mmi_bth_cntx.action_index].linkKey, msg_p->linkKey, 16);

	mmi_trace_hex(g_sw_BT,16, g_mmi_bth_cntx.discovered_list[g_mmi_bth_cntx.action_index].linkKey);
	mmi_trace(g_sw_BT, "g_mmi_bth_cntx.paired_list[i].keytype=%d",g_mmi_bth_cntx.discovered_list[g_mmi_bth_cntx.action_index].keytype);
        /* get index of paired list */
        for (i = 0; i < g_mmi_bth_cntx.paired_dev_num; i++)
        {
            if (g_mmi_bth_cntx.paired_list[i].bd_addr.lap == msg_p->bd_addr.lap
	    ...
	    mmi_trace(g_sw_BT,"g_mmi_bth_cntx.paired_dev_num=%d, i=%d, g_mmi_bth_cntx.paired_list[i].cod",
		                           g_mmi_bth_cntx.paired_dev_num, i, g_mmi_bth_cntx.paired_list[i].cod);
        if (i < g_mmi_bth_cntx.paired_dev_num)
        {
             mmi_trace(g_sw_BT, "g_mmi_bth_cntx.paired_list[i].cod =%d",g_mmi_bth_cntx.paired_list[i].cod );
	    mmi_trace_hex(g_sw_BT,16,msg_p->linkKey);
    ...
    {
    	  mmi_trace(g_sw_BT, "mmi_bth_cm_bond_ind_hdler:: MMI_BTH_GET_RAMFLAG=%d", MMI_BTH_GET_RAMFLAG(MMI_BTH_RAMMASK_FIRST_PAIR_REQ));
	  ...
	  mmi_trace(g_sw_BT, "mmi_bth_cm_bond_ind_hdler:: g_mmi_bth_cntx.paired_dev_num= %d",  g_mmi_bth_cntx.paired_dev_num);
            /* write to NVRAM */
            mmi_bth_nvram_write_info();
 ...
 *  void
 *****************************************************************************/
void mmi_bth_cm_be_bond_cnf_hdler(void *msg)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
      mmi_trace(g_sw_BT, "mmi_bth_cm_be_bond_cnf_hdler::msg_p->result= %d", msg_p->result);
    if (msg_p->result != BTBM_ADP_SUCCESS)
    {
    }
    else
    {
	 ...
	 g_mmi_bth_cntx.paired_list[i].keytype= g_mmi_bth_cntx.discovered_list[g_mmi_bth_cntx.action_index].keytype;
	memcpy(g_mmi_bth_cntx.paired_list[i].linkKey, g_mmi_bth_cntx.discovered_list[g_mmi_bth_cntx.action_index].linkKey, 16);
...
void mmi_bth_cm_bond_cnf_hdler(void *msg)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
      mmi_trace(g_sw_BT, "mmi_bth_cm_bond_cnf_hdler::msg_p->result= %d g_mmi_bth_cntx.state=%d", msg_p->result, g_mmi_bth_cntx.state);
    if (msg_p->result != BTBM_ADP_SUCCESS)
    {
        if(g_mmi_bth_cntx.state == MMI_BTH_STAT_BONDING)
        {
    ...
    else
    {
        ...
        memcpy(
            ...
            sizeof(mmi_bth_dev_struct));
      	    mmi_trace_hex(g_sw_BT,20, g_mmi_bth_cntx.discovered_list[g_mmi_bth_cntx.action_index].name);

      mmi_trace(g_sw_BT, "mmi_bth_cm_bond_cnf_hdler::g_mmi_bth_cntx.paired_dev_num=%d",
	  	                                     g_mmi_bth_cntx.paired_dev_num );
...
/*    MMI_TRACE(MMI_TRACE_G7_MISC, MMI_BT_G7_BT_CORE_STATUS,
   ...
   */ 
    ...
    switch (g_mmi_bth_cntx.state)
    {
		...
		if(g_mmi_bth_cntx.idx_of_last_hs_dev >i ) 
		{
			g_mmi_bth_cntx.idx_of_last_hs_dev--;
		}
...
*/
	...
	mmi_trace(g_sw_BT,"mmi_bth_cm_passkey_ind_hdler::bth_state=%d", bth_state);
...
/* under construction !*/
	...
	mmi_trace(g_sw_BT,"!= MMI_BTH_STAT_BONDING");
...
//            MMI_TRACE(MMI_TRACE_G7_MISC, MMI_BT_G7_MY_DEV_FUL_REJECT_PIN_REQ);
		...
		mmi_trace(g_sw_BT,"i=%d, g_mmi_bth_cntx.paired_dev_num=%d", i, g_mmi_bth_cntx.paired_dev_num);
...
void mmi_bth_cm_passkey_ind_rsp_hdler(mmi_bth_bt_addr bt_add, U8 pin_len, U8 *pin_code)
{
    ...
    MMI_BT_ASSERT(pin_len <= MMI_BTH_PASSKEY_MAX_LEN);
    memcpy(msg_p->pin_code, pin_code, pin_len + 1);
...
void mmi_bth_cm_security_level_cnf_hdler(void *msg)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
 mmi_trace(g_sw_BT, "mmi_bth_cm_security_level_cnf_hdler::g_mmi_bth_cntx.state=%d, msg_p->result, g_mmi_bth_cntx.flag=%d, g_mmi_bth_cntx.ram_flag=%d",
                                        g_mmi_bth_cntx.state, msg_p->result, g_mmi_bth_cntx.flag, g_mmi_bth_cntx.ram_flag);
...
void mmi_bth_cm_write_scanenable_mode_req_hdler(U8 connected_mode, U8 non_connected_mode)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	mmi_trace(g_sw_BT, "mmi_bth_cm_write_scanenable_mode_req_hdler::non_connected_mode & BTBM_ADP_P_OFF_I_ON=%d"
 ...
 *  void
 *****************************************************************************/
 u8 need_deactive_profile_flag=0;//zhou siyou 20100429
 extern 	 u8 opps_state(void);
void mmi_bth_cm_write_scanenable_mode_cnf_hdler(void *msg)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_trace(g_sw_BT, "mmi_bth_cm_write_scanenable_mode_cnf_hdler::g_mmi_bth_cntx.state, g_mmi_bth_cntx.flag=%d, g_mmi_bth_cntx.ram_flag=%d, g_mmi_bth_cntx.deactivate_flag=%d",
                                         g_mmi_bth_cntx.state, g_mmi_bth_cntx.flag, g_mmi_bth_cntx.ram_flag, g_mmi_bth_cntx.deactivate_flag);
    if (msg_p->result == BTBM_ADP_SUCCESS)
    {
        switch (g_mmi_bth_cntx.state)
        {
            ...
            case MMI_BTH_STAT_DEACTIVATING:
                ...
                /* deactivate profiles */
				if(opps_state())
				{	need_deactive_profile_flag=1;
				mmi_trace(g_sw_BT, "wait for mmi_bth_deactivate_profiles_req");
					return;
					}
...
void mmi_bth_cm_disallow_acl_link_establishment_cnf_hdler(void *msg)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_trace(g_sw_BT, "mmi_bth_cm_disallow_acl_link_establishment_cnf_hdler::g_mmi_bth_cntx.state, g_mmi_bth_cntx.flag=%d, g_mmi_bth_cntx.ram_flag=%d, g_mmi_bth_cntx.deactivate_flag=%d",
                                    g_mmi_bth_cntx.state, g_mmi_bth_cntx.flag, g_mmi_bth_cntx.ram_flag, g_mmi_bth_cntx.deactivate_flag);
 ...
 *  void
 *****************************************************************************/
  extern void mmi_audply_set_bt_headset(   U32 lap, U8 uap, U16 nap,U8 *name_p);
...
*/
     mmi_trace(g_sw_BT, "mmi_bth_connect_ind_hdler::   g_mmi_bth_cntx.state=%d,   profile_id=%d,  connection_id=%d",
			            g_mmi_bth_cntx.state,	profile_id, connection_id   );
    ...
    {
	mmi_trace(g_sw_BT, "connection_id=%d", connection_id);
    }
...
/* under construction !*/
	...
	mmi_audply_set_bt_headset(   lap,  uap, nap,name_p);//zhou siyou 20091029
    }
#endif
...
*/
    ...
    if (bth_state == MMI_BTH_STAT_INIT_ON ||
        ...
        bth_state == MMI_BTH_STAT_DEBONDING_ALL ||
        bth_state == MMI_BTH_STAT_RELEASE_DEV_CONNECTION || 
        bth_state == MMI_BTH_STAT_RELEASE_ALL_CONNECTION )      // || mmi_flight_mode_is_switching() == MMI_TRUE) temp del
    {
        /* reject the connect accept confirm */
        table_index = 0;
 ...
 *  void
 *****************************************************************************/
extern void mmi_audply_clean_set_bt_headset(  void);
...
/*    MMI_TRACE(MMI_TRACE_G7_MISC, MMI_BT_G7_GEN_DISCONNECT_CNF,
               ...
               (uap), ((lap & 0xff0000) >> 16), ((lap & 0x00ff00) >> 8), (lap & 0x0000ff));
*/
mmi_trace(g_sw_BT, "mmi_bth_disconnect_cnf_hdler::profile_id=%d,need_deactive_profile_flag=%d", profile_id,need_deactive_profile_flag);
    if (result == MMI_TRUE)
    {
        /* get index of paired list */
        for (i = 0; i < g_mmi_bth_cntx.existed_conn_num; i++)
...
//            MMI_TRACE(MMI_TRACE_G7_MISC, MMI_BT_G7_CONNECTION_NOT_FOUND,connection_id);
        ...
        if (profile_id == MMI_BTH_ADVANCED_AUDIO_PROFILE_UUID)
        {
	     ...
	     mmi_audply_clean_set_bt_headset();	
        }
#endif
...
void mmi_bth_sco_connect_cnf_hdler(U8 result, U32 connection_id)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
mmi_trace(g_sw_BT, "mmi_bth_sco_connect_cnf_hdler::result =%d, connection_id=%d", result, connection_id);
    switch (g_mmi_bth_cntx.state)
    {
        case MMI_BTH_STAT_RELEASE_ALL_CONNECTION:
        case MMI_BTH_STAT_RELEASE_DEV_CONNECTION:
...
void mmi_bth_sco_connect_ind_hdler(U32 connection_id)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
mmi_trace(g_sw_BT, "mmi_bth_sco_connect_cnf_hdler::g_mmi_bth_cntx.state =%d, connection_id=%d", g_mmi_bth_cntx.state, connection_id);
...
*/
mmi_trace(g_sw_BT, "mmi_bth_sco_disconnect_cnf_hdler::result=%d, connection_id=%d, g_mmi_bth_cntx.state=%d", result, connection_id, g_mmi_bth_cntx.state);
    switch (g_mmi_bth_cntx.state)
    {
        case MMI_BTH_STAT_RELEASE_ALL_CONNECTION:
        case MMI_BTH_STAT_RELEASE_DEV_CONNECTION:
...
MMI_BOOL mmi_bth_is_incall_aud_swap2bt(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	mmi_trace(g_sw_BT, "g_mmi_bth_cntx.ram_flag=%d", g_mmi_bth_cntx.ram_flag);
...
mmi_bth_dev_struct *mmi_bth_get_paired_dev_info(U16 index)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	mmi_trace(g_sw_BT, "mmi_bth_get_paired_dev_info:: index =%d", index);
...
MMI_BOOL mmi_bth_deactivate_req()
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
mmi_trace(g_sw_BT, "mmi_bth_deactivate_req::g_mmi_bth_cntx.state, g_mmi_bth_cntx.flag=%d, g_mmi_bth_cntx.ram_flag=%d, g_mmi_bth_cntx.deactivate_flag=%d",
                                    g_mmi_bth_cntx.state, g_mmi_bth_cntx.flag, g_mmi_bth_cntx.ram_flag, g_mmi_bth_cntx.deactivate_flag);
...
void mmi_bth_deactivate_profiles_req(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
mmi_trace(g_sw_BT, "mmi_bth_deactivate_profiles_req::g_mmi_bth_cntx.deactivate_flag= 0x%x", g_mmi_bth_cntx.deactivate_flag);
    while (support_profile_tbl[i].uuid != MMI_BTH_MAX_PROFILE_UUID)
    {
...
MMI_BOOL mmi_bth_activate_req(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_trace(g_sw_BT, "mmi_bth_activate_req::g_mmi_bth_cntx.deactivate_flag=%d", g_mmi_bth_cntx.deactivate_flag);
...
void mmi_bth_activate_profiles_req(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
     mmi_trace(g_sw_BT, "mmi_bth_activate_profiles_req::g_mmi_bth_cntx.deactivate_flag=%d", g_mmi_bth_cntx.deactivate_flag);
...
MMI_BOOL mmi_bth_release_dev_connection_req(mmi_bth_bt_addr *dev_add_p)
{
	...
	mmi_trace(g_sw_BT, "mmi_bth_release_dev_connection_req::i=%d, g_mmi_bth_cntx.paired_dev_num=%d",
		                             i, g_mmi_bth_cntx.paired_dev_num);
	...
	mmi_trace(g_sw_BT, "mmi_bth_release_dev_connection_req::j=%d, g_mmi_bth_cntx.existed_conn_num=%d",
		                             j, g_mmi_bth_cntx.existed_conn_num);
...
extern MMI_BOOL mmi_obex_is_can_bond(VOID);
...
#if 0 //ndef  MGR_MULTI_CONNECTION//zhou siyou 20100415
    if (!mmi_obex_is_can_bond())
	return MMI_FALSE;
	#endif
...
MMI_BOOL mmi_bth_connect_req(mmi_bth_bt_addr *dev_addr, U32 profile_id)
{
    ...
    while (support_profile_tbl[table_index].uuid != MMI_BTH_MAX_PROFILE_UUID)
    {
        if ((support_profile_tbl[table_index].uuid == supported_profile_uuid))
        {
        	mmi_trace(g_sw_BT, "mmi_bth_connect_req:: supported_profile_uuid=%d", supported_profile_uuid);
...
MMI_BOOL mmi_bth_set_connectable_req(MMI_BOOL connectable)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	         mmi_trace(g_sw_BT, "mmi_bth_set_connectable_req::connectable=%d", connectable);
...
/*****************************************************************************
 * FUNCTION
 *  mmi_bth_connect_paired_headset_with_index
 * DESCRIPTION
 ...
 *                  FALSE : otherwise
 *****************************************************************************/
static MMI_BOOL mmi_bth_connect_paired_headset_with_index(U32 paired_index, mmi_bth_hs_connect_scr_show_enum prgs_scr)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    MMI_BOOL result = MMI_FALSE;
	MMI_BOOL b_show_prgs_scr = (prgs_scr == MMI_BTH_HS_SHOW_CONNECT_SCR) ? MMI_TRUE: MMI_FALSE;
	
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
   // MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_G7_BTH_CONNECT_INDEXED_HEADSET, paired_index, prgs_scr);

    mmi_trace(1,"mmi_bth_connect_paired_headset_with_index, paired_index = %d,  prgs_scr = %d ",paired_index,prgs_scr);


    MMI_BT_ASSERT(paired_index < g_mmi_bth_cntx.paired_dev_num);

	/* reset the state */
	g_mmi_bth_cntx.hs_conn_state = MMI_BTH_HS_CONNECTING_STATE_IDLE;

	if (mmi_bth_is_service_support(paired_index, MMI_BTH_HF_PROFILE_UUID))
	{
        if (b_show_prgs_scr && g_mmi_bth_cntx.scr_cb_tbl.mmi_bt_scr_cb_show_progress_scr)
        {
            (*g_mmi_bth_cntx.scr_cb_tbl.mmi_bt_scr_cb_show_progress_scr) (SCR_BT_CONNECT_LAST_HEADSET,
                                                                          STR_BT_CONNECT,
                                                                          STR_BT_CONNECT, NULL, MMI_FALSE, NULL);
        }
	    
		result = mmi_bth_connect_req(&(g_mmi_bth_cntx.paired_list[paired_index].bd_addr), MMI_BTH_HF_PROFILE_UUID);

		if (result == MMI_TRUE)
		{
		    g_mmi_bth_cntx.hs_conn_state = MMI_BTH_HS_CONNECTING_STATE_HFP_CONNECTING;
		}
	}
	else if (mmi_bth_is_service_support(paired_index, MMI_BTH_HS_PROFILE_UUID))  
    {
		...
		result = mmi_bth_connect_req(&(g_mmi_bth_cntx.paired_list[paired_index].bd_addr), MMI_BTH_HS_PROFILE_UUID);

		if (result == MMI_TRUE)
		{
		    g_mmi_bth_cntx.hs_conn_state = MMI_BTH_HS_CONNECTING_STATE_HSP_CONNECTING;
		}
	}
	else if (g_mmi_bth_cntx.paired_list[paired_index].service_list_num == 0)
    {
...
/*****************************************************************************
 * FUNCTION
 *  mmi_bth_connect_default_headset
 * DESCRIPTION
 ...
 *                  FALSE : otherwise
 *****************************************************************************/
MMI_BOOL mmi_bth_connect_default_headset(mmi_bth_headset_connect_callback conn_cb)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
 //   MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_G7_BTH_CONNECT_DEFAULT_HEADSET_REQ, conn_cb, g_mmi_bth_cntx.idx_of_last_hs_dev);

    mmi_trace(1,"mmi_bth_connect_default_headset    , conn_cb= %d ,g_mmi_bth_cntx.idx_of_last_hs_dev =%d ",conn_cb,g_mmi_bth_cntx.idx_of_last_hs_dev);	
	  
	paired_index = g_mmi_bth_cntx.idx_of_last_hs_dev;

    if (g_mmi_bth_cntx.state != MMI_BTH_STAT_IDLE)
    {
   //     MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_G7_BTH_CONNECT_HEADSET_ERR_STATE, g_mmi_bth_cntx.state);

	  mmi_trace(1,"mmi_bth_connect_default_headset   HEADSET_ERR_STATE  , g_mmi_bth_cntx.state = %d ",g_mmi_bth_cntx.state);	
        result = MMI_FALSE;
        return result;
    }
   ...
   //         (*g_mmi_bth_cntx.scr_cb_tbl.mmi_bt_scr_cb_display_popup) ((PU8) GetString(STR_BT_NO_DEFAULT_HEADSET),
    ...
    /* connect the headset */
    result = mmi_bth_connect_paired_headset_with_index(paired_index, MMI_BTH_HS_NOT_SHOW_CONNECT_SCR);

    if (result == MMI_TRUE)
    {
    	/* connect callback */
	    g_mmi_bth_cntx.hs_conn_req_cb = conn_cb;	
    }

   // MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_G7_BTH_CONNECT_DEFAULT_HEADSET_RETURN, result);

	  mmi_trace(1,"mmi_bth_connect_default_headset  result  , result = %d ",result);
	return result;
 ...
 *                  FALSE : otherwise
 *****************************************************************************/
MMI_BOOL mmi_bth_connect_headset(mmi_bth_bt_addr *dev_addr, mmi_bth_headset_connect_callback conn_cb)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
  //  MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_G7_BTH_CONNECT_HEADSET, g_mmi_bth_cntx.state,
   //           dev_addr->lap, dev_addr->uap, dev_addr->nap,conn_cb);

      mmi_trace(1,"mmi_bth_connect_headset, g_mmi_bth_cntx.state = %d,  dev_addr->lap = %d,   dev_addr->uap =%d      ,dev_addr->nap =%d   ,  conn_cb  =%d  ",g_mmi_bth_cntx.state,dev_addr->lap,dev_addr->uap,dev_addr->nap,conn_cb);
...
//        MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_G7_BTH_CONNECT_HEADSET_ERR_STATE, g_mmi_bth_cntx.state);
      ...
      mmi_trace(1,"mmi_bth_connect_headset  HEADSET_ERR_STATE  , g_mmi_bth_cntx.state = %d ",g_mmi_bth_cntx.state);
	...
	{
	    g_mmi_bth_cntx.hs_conn_req_cb = NULL;	
	}
	
RET:
  //  MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_G7_BTH_CONNECT_HEADSET_RETURN, result);
	
	  mmi_trace(1,"mmi_bth_connect_headset  result  , result = %d ",result);
...
MMI_BOOL mmi_bth_turn_on_speech_req(MMI_BOOL speech_on)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	mmi_trace(g_sw_BT, "mmi_bth_turn_on_speech_req::speech_on=%d", speech_on);
