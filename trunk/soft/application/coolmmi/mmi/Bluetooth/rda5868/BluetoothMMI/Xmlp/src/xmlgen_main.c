...
#define XML_EQUAL   "="
#define xml_generate_get_buffer     OslMalloc
#define xml_generate_free_buffer    OslMfree
...
XG_HANDLE xml_new_generator(kal_char* dest, kal_uint32 length)
{
    ...
    {
        p->file = MMI_FS_Open((const UINT8*)dest, FS_CREATE_ALWAYS);
...
kal_int32 xml_close_generator(XG_HANDLE hd)
{
    ...
    while(((p->tagstack)[index] != NULL) && (index <XG_MAX_ELEM_DEPTH))
    {
...
{
    char charset[XG_MAX_CHARSET_LEN];
...
/* first check if parameters are correct */
    ...
    if ((encode < 0) || (encode >= XML_ENCODING_UNKNOWN))
    {
...
/* first to get xml text length */
    ...
    {
        fhandle = MMI_FS_Open((const UINT8*)text, FS_READ_ONLY);
        if (fhandle < 0)
        {
            return XG_E_FAIL;
        }
        ret = MMI_FS_GetFileSize(fhandle, &length);
        if (ret < 0)
        {
...
//            kal_printf("buffer to save will be overflow!!!!\r\n");
    ...
    else /* write xml text to file */
    {
        ...
        {
            ret = MMI_FS_Write(p->file, text, length, (INT32 *)&length);
        }
