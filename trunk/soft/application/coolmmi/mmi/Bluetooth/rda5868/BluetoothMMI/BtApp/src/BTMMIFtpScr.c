...
{                                                                                   \
   kal_wstrcpy( (U16 *)mmi_ftpc_notify_str, (U16 *)GetString(STR_GLOBAL_ABORTING) );\
}

#define MMI_FTPC_BUILD_CONNECTING_NOTY_STR()                                              \
{                                                                                         \
   kal_wstrcpy( (U16 *)mmi_ftpc_notify_str, (U16 *)GetString(STR_BT_CONNECTING) );\
}
#define MMI_FTPC_BUILD_DISCONNECTING_NOTY_STR()                                              \
{                                                                                            \
   kal_wstrcpy( (U16 *)mmi_ftpc_notify_str, (U16 *)GetString(STR_BT_DISCONNECTING) );\
}
...
{                                                                                                                            \
   ...
   if(MMI_FTP_PULL_TYPE_SINGLE == g_mmi_bt_ftp_scr_cntx_p->pull_type)                                                        \
       ...
       file_cnt = (act_client_cntx_p + ftpc_cntx_p->curr_idx )->file_cnt_cur_folder;                                         \
                                                                                                                             \
   kal_wstrcpy( (U16 *)mmi_ftpc_notify_str, (U16 *)GetString(STR_GLOBAL_RECEIVED) );                                  \
   csd_wsprintf( (U16 *)cnt, " %d ", file_cnt);                                                                              \
   kal_wstrcat( (U16 *)mmi_ftpc_notify_str, (U16*) cnt);                                                                     \
   if(file_cnt == 1)                                                                                                                         \
...
#define FTPC_UDPADE_STR() {                                                                                                                                           \
            ...
            if( (UCS2Strlen( (S8*)  GetString(STR_GLOBAL_RECEIVED) )+1) > ((sizeof(ftpc_fmgr_recv_str_buf)/2) -(sizeof(fbrwr_ftpc_recv_per_str_buf)/2)) )\
                ASSERT(MMI_FALSE);                                                                                                                                    \
                                                                                                                                                                      \
            if((act_client_cntx_p + ftpc_cntx_p->curr_idx)->total_len_type == MMI_FTPC_TOTAL_LEN_NO_FIRST_PKT)                                                        \
            {                                                                                                                                                         \
                UCS2Strcpy((S8*)ftpc_fmgr_recv_str_buf, GetString(STR_GLOBAL_RECEIVING) );                                                                  \
            }                                                                                                                                                         \
            else                                                                                                                                                      \
            {                                                                                                                                                         \
                UCS2Strcpy((S8*)ftpc_fmgr_recv_str_buf, GetString(STR_GLOBAL_RECEIVED) );                                                                \
                mmi_bt_ftpc_retrieve_get_prog_percentage(fbrwr_ftpc_recv_per_str_buf);                                                                                \
                UCS2Strcat((S8*)ftpc_fmgr_recv_str_buf, (S8*)fbrwr_ftpc_recv_per_str_buf);                                                                      \
            }                                                                                                                                                         \
}
...
#define FTPC_MULTI_UDPADE_STR() {                                                                                                                                      \
            ...
            {                                                                                                                                                          \
                UCS2Strcpy((S8*)ftpc_fmgr_recv_str_buf, GetString(STR_GLOBAL_RECEIVING) );                               \
                UCS2Strcat((PS8)ftpc_fmgr_recv_str_buf, (PS8) L"\n");                                                                                            \
            }                                                                                                                                                          \
            else                                                                                                                                                       \
            {                                                                                                                                                          \
                UCS2Strcpy((S8*)ftpc_fmgr_recv_str_buf, GetString(STR_GLOBAL_RECEIVED) );                                                                \
...
{                                                                                                                               \
   ...
   csd_wsprintf( (U16 *)cnt, "%d", (act_client_cntx_p + ftpc_cntx_p->curr_idx)->file_cnt_cur_folder);                         \
   kal_wstrcat( (U16 *)mmi_ftpc_notify_str, (U16*) cnt);                                                                        \
...
void mmi_bt_ftps_menu_hdlr_init(void)
{
    ...
    SetHiliteHandler(MENU_CONN_FTP_RECEIVE_FOLDER, mmi_bt_received_folder_highlight_hdlr);
}
...
void mmi_bt_ftps_entry_ftp_setting(void)
{
    ...
    SetHintHandler(MENU_CONN_FTP_RECEIVE_FOLDER, mmi_bt_received_folder_hint);
...
void mmi_bt_ftps_shared_folder_hint(U16 index)
{
    ...
    if (UCS2Strlen((PS8) ftps_cntx_p->root_folder) <= 41)
    {
        UCS2Strcpy((PS8) hintData[index], (PS8) ftps_cntx_p->root_folder);
    }
    else
    {
        UCS2Strncpy((PS8) hintData[index], (PS8) ftps_cntx_p->root_folder, 3);
        UCS2Strncpy(
            (PS8) (hintData[index] + 6),
            (PS8) ftps_cntx_p->root_folder + (UCS2Strlen((const S8*)ftps_cntx_p->root_folder) * 2) - (MAX_SUB_MENU_HINT_SIZE - 6 - ENCODING_LENGTH),
...
void mmi_bt_ftps_fmgr_select_path_done(void *path, int is_short)
{
    ...
    {
        UCS2Strcpy(tmp_path_buf, path);

        res = MMI_FS_Write(h, tmp_path_buf, (UCS2Strlen(tmp_path_buf) * 2 + 2), &written);
        MMI_FS_Close(h);
        if ((res != FS_NO_ERROR) || ((UCS2Strlen(tmp_path_buf) * 2 + 2) != written))
        {
            DisplayPopup(
                (PU8) GetString(STR_GLOBAL_UNFINISHED),
                IMG_GLOBAL_UNFINISHED,
...
void mmi_bt_ftps_access_right_hint(U16 index)
{
    ...
    UCS2Strcpy((PS8) hintData[index], (PS8) GetString(AccessRightStrList[ftps_cntx_p->access_right]));
}
...
void mmi_bt_ftps_entry_recv_confirm_scr(void)
{
    ...
    SetDelScrnIDCallbackHandler(SCR_FTP_NOTY_CONN_REQ, mmi_bt_ftps_authorize_confirm_del_cb);
	   ...
	   StartTimer(BT_FTPS_RECV_TIMEROUT, 15000, mmi_bt_ftps_reject_conn);
...
void mmi_bt_ftps_accept_conn(void)
{
    ...
    StopTimer(BT_FTPS_RECV_TIMEROUT);
...
void mmi_bt_ftpc_entry_canceling(void)
{
 ...
 //   FTP_DEBUG(kal_print("FTPC ENTRY CANCEL SCREEN\n");)
    mmi_trace(g_sw_BT, "mmi_bt_ftpc_entry_canceling");
...
void mmi_bt_ftpc_entry_obj_detail(void)
{
    ...
    UCS2Strcpy((PS8) info, (PS8) GetString(STR_FTP_DETAIL_DATE));
    UCS2Strcat((PS8) info, (PS8) L"\n");
    UCS2Strcat((PS8) info, (PS8) g_mmi_bt_ftp_scr_cntx_p->obj_entity.created_date);
    UCS2Strcat((PS8) info, (PS8) L"\n");

    if(g_mmi_bt_ftp_scr_cntx_p->obj_entity.type == MMI_FTP_OBJ_TYPE_FILE)
    {
        UCS2Strcat((PS8) info, (PS8) GetString(STR_FTP_DETAIL_SIZE));
        UCS2Strcat((PS8) info, (PS8) L"\n");
        UCS2Strcat((PS8) info, (PS8) g_mmi_bt_ftp_scr_cntx_p->obj_entity.size);
        UCS2Strcat((PS8) info, (PS8) L"\n");
    }
    ...
    else
    {
        ...
        if (g_mmi_bt_ftp_scr_cntx_p->pull_type == MMI_FTP_PULL_TYPE_MULTI 
            || g_mmi_bt_ftp_scr_cntx_p->pull_type == MMI_FTP_PULL_TYPE_ALL)
         {
            ...
            UCS2Strcpy((PS8) error_string, (PS8) GetString(STR_FTP_FILE_PREFIX));
            UCS2Strcat((PS8) error_string, (PS8) file_prefix);
            
            if (g_mmi_bt_ftp_scr_cntx.received_obj_cnt + 1 == 1)
            {
                UCS2Strcat((PS8) error_string, (PS8) GetString(STR_FTP_FILE_1ST));
            }
            else if (g_mmi_bt_ftp_scr_cntx.received_obj_cnt + 1 == 2)
            {
                UCS2Strcat((PS8) error_string, (PS8) GetString(STR_FTP_FILE_2ND));
            }
            else if (g_mmi_bt_ftp_scr_cntx.received_obj_cnt + 1 == 3)
            {
                UCS2Strcat((PS8) error_string, (PS8) GetString(STR_FTP_FILE_3RD));
            }
            else
            {
                UCS2Strcat((PS8) error_string, (PS8) GetString(STR_FTP_FILE_TH));
            }
            
            UCS2Strcat((PS8) error_string, (PS8) GetString(STR_FTP_FILE_UNFINISHED));
        }
        else
        {
            UCS2Strcpy((PS8) error_string, (PS8) GetString(STR_GLOBAL_UNFINISHED));
        }
#else /* __MMI_FTC_MULTIPULL_SUPPORT__ */
        UCS2Strcpy((PS8) error_string, (PS8) GetString(STR_GLOBAL_UNFINISHED));
#endif /* __MMI_FTC_MULTIPULL_SUPPORT__ */
...
U8 mmi_bt_ftpc_file_browser_del_cb(void *p)
{
    ...
    ftpc_state = mmi_bt_ftpc_get_state(0);
    FTP_DEBUG(kal_print("mmi_bt_ftpc_file_browser_del_cb ftpc_state=%d\n",ftpc_state);)
    switch (ftpc_state)
    {
...
static void mmi_bt_ftpc_connect_scr_RSK(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	mmi_trace(g_sw_BT, "mmi_bt_ftpc_connect_scr_RSK");
    mmi_bt_ftpc_entry_canceling();
    mmi_bt_ftpc_connect_cancel();
}
...
void mmi_bt_ftpc_AddHistory(U16 scr_id)
{
    ...
    UCS2Strcpy((PS8) h.inputBuffer, (PS8) & nHistory);
...
void mmi_bt_ftpc_notify_scr_browse(void)
{
    ...
    UCS2Strcpy(path_tmpbuf, (act_client_cntx_p)->received_folder);
...
int mmi_bt_ftpc_look_up_object_entity(S32 item_index)
{
    ...
    fileHandle = MMI_FS_Open((const UINT8*) act_client_cntx_p->parsed_folder, FS_READ_ONLY);

    if (fileHandle >= 0)
    {
        offset = item_index * (sizeof(mmi_ftp_obj_entity_struct));
        result = MMI_FS_Seek(fileHandle, offset, FS_FILE_BEGIN);
        ...
        result = MMI_FS_Read(fileHandle, (void*)selecting_entity_p, sizeof(mmi_ftp_obj_entity_struct), &len);
        if (result == FS_NO_ERROR)
        {
            FTP_DEBUG(kal_print("FTPC LOOK UP OBJECT ENTITY NO ERROR\n");)
            UCS2Strcpy((PS8) (g_mmi_bt_ftp_scr_cntx_p->obj_entity.name), (PS8) (selecting_entity_p->name));
        }
        else
        {
...
pBOOL mmi_bt_ftpc_browsing_item(S32 item_index, UI_string_type str_buff, PU8 *img_buff_p, U8 str_img_mask)
{
    ...
    if (result == FS_NO_ERROR)
    {
        ...
        UCS2Strcpy((PS8) str_buff, (PS8) g_mmi_bt_ftp_scr_cntx_p->obj_entity.name);
...
static void mmi_bt_ftpc_list_option_delete(void)
{
    ...
    DisplayPopup((PU8) GetString(STR_GLOBAL_DELETE), IMG_GLOBAL_ACTIVATED, 0, UI_POPUP_NOTIFYDURATION_TIME, SUCCESS_TONE);
...
static MMI_BOOL mmi_bt_ftpc_check_list(void)
{
    ...
    {
        p_down_list->p_obj_indexs_in_folder = OslMalloc(MMI_BT_FTPC_MULPULL_LIST_INIT * sizeof(U16));
    ...
    if (p_down_list->list_capability == p_down_list->list_obj_cnt)
    {
        if ((p_down_list->list_capability + MMI_BT_FTPC_MULPULL_LIST_INC) <= MMI_BT_FTPC_MULPULL_LIST_MAX)
        {
            ...
            U16 *p_new_list = OslMalloc((p_down_list->list_capability + MMI_BT_FTPC_MULPULL_LIST_INC) * sizeof(U16));
            ASSERT(p_new_list);
            ...
            /* free old buffer */
            OslMfree(p_down_list->p_obj_indexs_in_folder);
...
static void mmi_bt_ftpc_retrieve_get_multi_percentage(U16 *recv_per_str_buf)
{
    ...
    switch (g_mmi_bt_ftp_scr_cntx_p->pull_type)
    {
        case MMI_FTP_PULL_TYPE_MULTI:
            ...
            UCS2Strcat((S8*) recv_per_str_buf, (S8*) temp_str_buf);
            break;
 ...
 *****************************************************************************/
 ...
 #include "btmmiopp.h"
void mmi_bt_received_folder_hint(U16 index)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (UCS2Strlen(MMI_OPP_CTX(recived_file_path)) <= 41)
    {
        UCS2Strcpy((PS8) hintData[index], MMI_OPP_CTX(recived_file_path));
    }
    else
    {
        UCS2Strncpy((PS8) hintData[index], (PS8) MMI_OPP_CTX(recived_file_path), 3);
        UCS2Strncpy(
            (PS8) (hintData[index] + 6),
            (PS8) MMI_OPP_CTX(recived_file_path) + (UCS2Strlen((const S8*)MMI_OPP_CTX(recived_file_path)) * 2) - (MAX_SUB_MENU_HINT_SIZE - 6 - ENCODING_LENGTH),
            (MAX_SUBMENU_CHARACTERS - 3 - 1));
        hintData[index][6] = 0x7E;
        hintData[index][7] = 0;
        hintData[index][MAX_SUB_MENU_HINT_SIZE - 2] = 0;
 ...
 *  void
 *****************************************************************************/
void mmi_bt_entry_received_folder_menu(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    FTP_DEBUG(kal_print("FTPS ENTRY SHARED FOLDER MENU\n");)
#ifdef __MMI_FTC_SUPPORT__
     if((act_client_cntx_p->ftpc_state == MMI_FTPC_STATE_GETTING_OBJ) ||  (MMI_OPP_CTX(ops_state) >= MMI_OPS_ACCEPTED))
#else
    ...
    else 
    {
        ...
        mmi_fmgr_select_path_and_enter(
            ...
            filter,
            (S8*) L"root",
            mmi_bt_fmgr_select_receivepath_done);
    }
}
 ...
 *  void
 *****************************************************************************/
void mmi_bt_fmgr_select_receivepath_done(void *path, int is_short)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    {
        h = MMI_FS_CreateDir((const UINT8*) FTP_FOLDER_PATH);
    }
    ...
    else
    {
        ...
        {
            /* set root folder and all connection's current folder */
    	     	    UCS2Strcpy(MMI_OPP_CTX(recived_file_path),tmp_path_buf);
#ifdef __MMI_FTC_SUPPORT__
		    for (i = 0; i < FTP_MAX_SERVER_SUPPORT; i++)
		    {
	    	    	UCS2Strcpy((act_client_cntx_p + i)->received_folder,tmp_path_buf);
		    }
#endif	
            DisplayPopup(
                (PU8) GetString(STR_GLOBAL_DONE),
	...
	if(*tmp_path_buf=='E'||*tmp_path_buf=='e')
		{
		mmi_trace(2,"chenhe,mmi_bt_fmgr_select_receivepath_done, is flash");
		data = TRUE;
...
/*****************************************************************************
 * FUNCTION
 *  mmi_bt_ftps_Receive_folder_highlight_hdlr
 * DESCRIPTION
 *  This function is to save the highlight index for
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
void mmi_bt_received_folder_highlight_hdlr(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    FTP_DEBUG(kal_print("FTPS RECEIVE FOLDER HILITE HDLR\n");)
    ChangeLeftSoftkey(STR_GLOBAL_OK, IMG_GLOBAL_OK);
    ChangeRightSoftkey(STR_GLOBAL_BACK, IMG_GLOBAL_BACK);
    SetLeftSoftkeyFunction(mmi_bt_entry_received_folder_menu, KEY_EVENT_UP);
    SetKeyHandler(mmi_bt_entry_received_folder_menu, KEY_RIGHT_ARROW, KEY_EVENT_DOWN);
    SetRightSoftkeyFunction(GoBackHistory, KEY_EVENT_UP);
    SetKeyHandler(GoBackHistory, KEY_LEFT_ARROW, KEY_EVENT_DOWN);
}

MMI_BOOL mmi_obex_is_can_bond(VOID)
{
    mmi_trace(g_sw_BT, "mmi_bth_bond_req::MMI_OPP_CTX(ops_state) =%d, act_server_cntx_p->ftps_state =%d\n",MMI_OPP_CTX(ops_state),act_server_cntx_p->ftps_state);
	
    if ((MMI_OPP_CTX(ops_state) >= MMI_OPS_ACCEPTED) || (act_server_cntx_p->ftps_state >= MMI_FTPS_STATE_ACCEPTED))
		return MMI_FALSE;
    return MMI_TRUE;
}

MMI_BOOL mmi_obex_check_switch_application(VOID)
{
#ifdef __MMI_FTC_SUPPORT__
     mmi_trace(g_sw_BT, "mmi_bth_bond_req::MMI_OPP_CTX(opc_state) =%d, act_client_cntx_p->ftpc_state =%d\n",MMI_OPP_CTX(opc_state),act_client_cntx_p->ftpc_state);	
#else
    mmi_trace(g_sw_BT, "mmi_bth_bond_req::MMI_OPP_CTX(opc_state) =%d\n",MMI_OPP_CTX(opc_state));
#endif /*__MMI_FTC_SUPPORT__*/

#ifdef __MMI_FTC_SUPPORT__	
    if ((MMI_OPP_CTX(opc_state) >= MMI_OPC_SENDING) || (act_client_cntx_p->ftpc_state >= MMI_FTPC_STATE_CONNECTED))
#else
    if	((MMI_OPP_CTX(opc_state) >= MMI_OPC_SENDING))
#endif /*__MMI_FTC_SUPPORT__*/
    {
        DisplayPopup(
            (PU8) GetString(STR_OBEX_NOTIFY_CANNOT_SWITCH_APPLICATION),
            IMG_GLOBAL_WARNING,
            1,
            UI_POPUP_NOTIFYDURATION_TIME,
            WARNING_TONE);
