...
kal_uint8 mmi_register_auth_ind_handler(kal_uint8 *uuid, kal_uint8 uuid_len, AUTH_IND_HANDLER auth_ind_handler)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    auth_ind_handler_struct *item = OslMalloc(sizeof(auth_ind_handler_struct));
...
extern kal_uint8 SBC_buf[];
//static U8 mmi_bt_obex_profile_share_buffer[MMI_OBEX_PROFILE_SHARE_BUF_SIZE];
static U8 *mmi_bt_obex_profile_share_buffer = SBC_buf;	//This is shared with SBC_buf,that has 16KB space.
/*****************************************************************************
 * FUNCTION
