...
static kal_int32 xml_parse_prolog(XML_PARSER_STRUCT *parser);
static kal_int32 xml_parse_element(XML_PARSER_STRUCT *parser);

static kal_int32 xml_process_prolog(XML_PARSER_STRUCT *parser, kal_int32 start, kal_int32 length);
static kal_int32 xml_process_doctype(XML_PARSER_STRUCT *parser, kal_int32 start, kal_int32 length);
static kal_int32 xml_process_data(XML_PARSER_STRUCT *parser, kal_int32 length);
static kal_int32 xml_process_cdata(XML_PARSER_STRUCT *parser, kal_int32 length);
static kal_int32 xml_process_entity(XML_PARSER_STRUCT *parser, kal_char *start, kal_int32 length);
static kal_int32 xml_process_start_tag_internal(XML_PARSER_STRUCT *parser, 
                                                kal_int32 start, 
                                                kal_int32 length, 
                                                kal_char **p_name, 
                                                kal_int32 *empty);
static kal_int32 xml_process_start_tag(XML_PARSER_STRUCT *parser, kal_int32 start, kal_int32 length);
static kal_int32 xml_process_end_tag(XML_PARSER_STRUCT *parser, kal_int32 start, kal_int32 length);

static kal_char *xml_find_min_ptr(kal_char *a, kal_char *b, kal_char *c);

static kal_int32 xml_scan_named_char(XML_PARSER_STRUCT *parser, FS_HANDLE *handle, 
                    kal_char **dest, kal_char *original, kal_int32 length);
static kal_char *xml_replace_entity(XML_PARSER_STRUCT *parser, kal_char *start, kal_int32 length);
static kal_char *xml_find_min_entity_mark(kal_char *string, kal_char *end);
static kal_char *xml_find_valid_entity_start(kal_char *start, kal_char *end);
static kal_char *xml_is_pre_entity(kal_char *start, kal_char *end);
...
static void xml_save_content_data(XML_PARSER_STRUCT * parser,FS_HANDLE *handle, 
                                  kal_char **dest, const kal_char *data, kal_int32 len_data);

static kal_int32 xml_reset_end_position(kal_char *data, kal_int32 length);
static kal_int32 xml_filter_comment_pi(kal_char *data, const kal_char *filter_start, const kal_char *filter_end);

static kal_int32 xml_check_attr_duplicate(kal_char *attr, kal_char **attrlist, kal_int32 index);
static kal_int32 xml_attr_handle_entity(
                                XML_PARSER_STRUCT *parser,
                                kal_char *dest,
                                kal_char *original,
                                kal_int32 length);
static kal_int32 xml_push_start_tag(XML_PARSER_STRUCT *parser, kal_char *tagname);
static void xml_pop_start_tag(XML_PARSER_STRUCT *parser);
static void xml_free_tag_list(XML_PARSER_STRUCT *parser);

static void xml_set_error (XML_PARSER_STRUCT *parser, kal_int32 error, kal_char* err_str);


static kal_int32 xml_read_xmltext(
                            XML_PARSER_STRUCT *parser,
                            kal_char *buffer, 
                            kal_int32 from, 
                            kal_int32 size,
                            int*length);
static kal_int32 xml_seek_xmltext(XML_PARSER_STRUCT *parser, kal_int32 offset, kal_int32 ref);
static kal_int32 xml_get_curr_position(XML_PARSER_STRUCT *parser);
...
static void xml_to_lower(kal_uint8 *str);
static kal_char *xml_extract_token(XML_PARSER_STRUCT *parser, kal_char *string, kal_char *sep);
 ...
 *  void
 *****************************************************************************/
static void *xml_get_buffer(XML_PARSER_STRUCT *parser, kal_int32 size)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
void xml_close_parser(XML_PARSER_STRUCT *parser)
{
    ...
    if (parser != NULL)
    {
        ...
        if (parser->source >= FS_NO_ERROR)
        {
            ...
            parser->source = FS_ERROR_RESERVED;
        }
 ...
 *  void
 *****************************************************************************/
void xml_close_parser_offset(XML_PARSER_STRUCT *parser)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
void xml_elem_start_hdlr(void *data, const kal_char *el, const kal_char **attr, kal_int32 error)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
void xml_doctype_end_hdlr(void *data, kal_int32 error)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
void xml_general_data_hdlr(void *resv, const kal_char *el, const kal_char *data, kal_int32 len, kal_int32 error)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
kal_int32 xml_parse(XML_PARSER_STRUCT *parser, kal_wchar *file_name)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    kal_int32 result = XML_RESULT_OK;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    parser->source = DRM_open_file((const UINT8 *)file_name, FS_READ_ONLY, DRM_PERMISSION_NONE);
 ...
 *  void
 *****************************************************************************/
kal_int32 xml_parse_file_from_offset(XML_PARSER_STRUCT *parser, kal_int32 file_handle)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    parser->source = file_handle;   /* FS_Open ((WCHAR*)tmp_filename, FS_READ_ONLY); */
...
static kal_int32 xml_parse_prolog(XML_PARSER_STRUCT *parser)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    kal_char *buffer;   /* [XML_BUFFER_LENGTH]; */
    kal_char *ptr_prolog, *ptr_start, *ptr_end, *ptr_doc, *ptr_comment, *ptr_pi;
    kal_char *ptr, *temp;
    kal_int32 len, ret;
           ...
           (xml_read_xmltext(parser, buffer, parser->pos, XML_BUFFER_LENGTH - 1, &len) >= FS_NO_ERROR))
    {
        ...
        else
        {
            ...
            if (parser->pos == 0 &&
                ((ptr[0] == (kal_char)0xff && ptr[1] == (kal_char)0xfe) || (ptr[0] == (kal_char)0xfe && ptr[1] == (kal_char)0xff)))
            {
...
static kal_int32 xml_parse_element(XML_PARSER_STRUCT *parser)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    kal_char *buffer;   /* buffer[XML_BUFFER_LENGTH]; */
    kal_char *ptr_start, *ptr_end, *ptr_comment, *ptr_pi, *ptr_cdata;
    kal_char *ptr, *temp;
    kal_int32 len;
    ...
    {
        if (len == 0)
            break;  /* when len == 0, it means the end of xml file */
...
static kal_int32 xml_process_doctype(XML_PARSER_STRUCT *parser, kal_int32 start, kal_int32 length)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    kal_char *name, *ptr, *id[10];
    kal_uint32 fp, len, i = 0;
    kal_int32 ret;
    ...
    ret = xml_read_xmltext(parser, parser->ap_temp, start, length, (int*)&len);
    if (ret < 0)
    {
        free_buffer(parser, parser->ap_temp);
        parser->ap_temp = NULL;
    ...
    if (ptr != NULL)
    {
        kal_char *ptr_entity, *ptr_end, *temp;
    ...
    if (name != NULL)
    {
        ...
        if (i > 0)
        {
            ...
            {
                parser->start_doctype_hdlr(parser, (const kal_char*)name, (const kal_char **)id + 1, NULL, 0, parser->error);
            }
            else
            {
                parser->start_doctype_hdlr(parser, (const kal_char*)name, NULL, (const kal_char **)id + 1, 0, parser->error);
            }
        }
        else
        {
...
static kal_int32 xml_process_data(XML_PARSER_STRUCT *parser, kal_int32 length)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    kal_char *data, *dest = NULL;
    kal_uint32 fp, position;
    kal_int32 ret, i = 0;
    FS_HANDLE handle = 0;
    ...
    while (i > 0)   /* i is used as the rest length of content data */
    {
        ...
        if (len_data > 0)
        {
            kal_char *str1, *str2, *str3, *string;
            ...
            while ((str1 = strstr(string, XML_TAG_CDATA_START)) != NULL)        /* str1: "<![CDATA[&e2;]]>xyz<![CDATA[XXX]]>mnl" */
            {
                ...
                if (str2 != NULL)
                {
                    ...
                    xml_save_content_data(parser, &handle, &dest, str3, (kal_uint16) (str2 - str3));
    ...
    if (dest != NULL)
    {
        kal_char *ret_data = dest;

        i = strlen(dest);
        /* filter space in begin and end */
        while (ret_data[0] == ' ' || ret_data[0] == '\n' || ret_data[0] == '\r')
        {
            ret_data++;
            i--;
        }
    ...
    {
        MMI_FS_Close(handle);
        parser->data_hdlr(parser, parser->elem[parser->curr_depth-1], (kal_char*)L"Z:\\@USER\\xml_temp", 0, parser->error);
    }
 ...
 static kal_int32 xml_process_cdata(XML_PARSER_STRUCT *parser, kal_int32 length)
{
    ...
    if (i >=  (MAX_CTRL_BUFFER_SIZE-1))
    {
        handle = MMI_FS_Open((const UINT8 *)L"D:/@USER/xml_temp", FS_CREATE_ALWAYS);
        if (handle < 0)
        {
                free_buffer(parser, parser->ap_temp);
                parser->ap_temp = NULL;
    ...
    while (i > 0)   /* i is used as the rest length of content data */
    {
        ...
        ret = MMI_FS_Write(handle, parser->ap_temp, len_read, (INT32*)&len_write);
        if (ret < 0)
        {
            free_buffer(parser, parser->ap_temp);
            parser->ap_temp = NULL;
...
static kal_int32 xml_process_entity(XML_PARSER_STRUCT *parser, kal_char *start, kal_int32 length)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    kal_char *name, *pe_mark, *value, *tmp, *ptr;
    kal_int32 i;
    ...
    if (name != NULL)
    {
        ...
        if (value != NULL)
        {
            kal_char *entity_name, *entity_value;
                                                 ...
                                                 kal_int32 *empty)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    kal_char *attr[XML_MAX_ATTR_COUNT], *ptr;
    kal_char *dest[XML_MAX_ATTR_COUNT / 2];
    kal_uint32 fp, len, i = 0;
    kal_int32 ret;
    kal_char *name;
    ...
    *p_name = name;
    ...
    if (name != NULL)
    {
            ...
            while (1)
            {
                ...
                else
                {
                    ...
                    if (ch[0] == '\'' || ch[0] == '\"')
                    {
                        ...
                        else    /* single '\'' || '\"', ignore such attr, report error. */
                        {
                            xml_set_error(parser, XML_ERROR_INVALID_ATTRIBUTE, attr[i]);
                            break;
                        }
                    }
                    else    /* no find '\'' || '\"', ignore such attr, report error. */
                    {
                        xml_set_error(parser, XML_ERROR_INVALID_ATTRIBUTE, attr[i]);
                        break;
                    }
        ...
        parser->start_elem_hdlr(parser, name, (const kal_char **)attr, parser->error);
    }
...
static kal_int32 xml_process_end_tag(XML_PARSER_STRUCT *parser, kal_int32 start, kal_int32 length)
{
    ...
    if ((name != NULL) && (parser->curr_depth > 0))
    {
        if ((strcmp(name, parser->elem[parser->curr_depth-1])) != 0)
            parser->error = XML_ERROR_MISMATCH_TAG; /* check if end tag name matches start tag name */
        xml_pop_start_tag(parser);
        parser->end_elem_hdlr(parser, name, parser->error);
    }
    /*
...
static kal_int32 xml_reset_end_position(kal_char *data, kal_int32 length)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    kal_char *start_pi, *start_comment, *start_ge, *start_pe, *start_cdata, *ptr;
...
static kal_char *xml_find_valid_entity_start(kal_char *start, kal_char *end)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    kal_char *newstart, *temp1, *temp2, *temp = start + 1;
...
static kal_char *xml_is_pre_entity(kal_char *start, kal_char *end)
{
    ...
    for (i = 0; i < MAX_NAME_CHAR_TABLE_SIZE; i++)
    {
        if (strncmp(xml_named_char_table[i].named_string, start, end - start + 1) == 0)
            return (kal_char*)&(xml_named_char_table[i].named_ch);
    }
    return NULL;
                                    ...
                                    kal_char *end)
{
    ...
    if (start[1] == '#')
    {
        kal_char value_str[XML_CHAR_ENTITY_LENGTH];
 ...
 *  void
 *****************************************************************************/
static kal_int32 xml_scan_named_char(
                                XML_PARSER_STRUCT *parser,
                                FS_HANDLE *handle,
                                kal_char **dest,
                                kal_char *original,
                                kal_int32 length)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    kal_char *string, *start, *end,  *data = NULL;
                ...
                kal_int32 len_data)
{
    ...
    if ((*dest == NULL) && (*handle != 0))
        MMI_FS_Write(*handle, (void*)data, len_data, &len);
    else
    {
        ...
        {
            *handle = MMI_FS_Open((const UINT8 *)L"D:/@USER/xml_temp", FS_CREATE_ALWAYS);
            MMI_FS_Write(*handle, (void*)*dest, len_dest, &len);
            MMI_FS_Write(*handle, (void*)data, len_data, &len);
                            ...
                            int*length)
{
    ...
    if (parser->source >= 0)
    {
        ...
        return DRM_read_file(parser->source, buffer, size, (INT32*)length);
    }
    else
    {
...
static kal_int32 xml_seek_xmltext(XML_PARSER_STRUCT *parser, kal_int32 offset, kal_int32 ref)
{
    ...
    if (parser->source >= 0)
    {
        return DRM_seek_file(parser->source, offset, (kal_uint8)ref);
    }
    else
        return 0;
}
...
static void xml_pop_start_tag(XML_PARSER_STRUCT *parser)
{
    ...
    /*----------------------------------------------------------------*/
    parser->curr_depth--;
    if (parser->elem[parser->curr_depth] != NULL)
...
static kal_char *xml_extract_token(XML_PARSER_STRUCT *parser, kal_char *string, kal_char *sep)
{
    ...
    ptr = strtok_r(string, sep, &parser->token_ptr);
#else 
    ptr = strtok(string, sep);
#endif 
    return ptr;
}
