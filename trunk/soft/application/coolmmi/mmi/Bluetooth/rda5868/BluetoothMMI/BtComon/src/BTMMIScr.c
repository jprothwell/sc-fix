...
#include "eventsgprot.h"        /* hintdata */
...
void mmi_bt_init(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	mmi_trace(g_sw_BT,"mmi_bt_init:: init profile");
...
//    MMI_TRACE(MMI_TRACE_G7_MISC, MMI_BT_G7_BT_SCR_INIT);
    ...
    mmi_bt_init_scr_cb();
	#if defined(__MMI_BT_COOLSAND_SUPPORT__) && defined(__MMI_HIDD_SUPPORT__)
    mmi_bt_hid_scr_init();
    #endif 
...
void mmi_bt_init_scr_cb(void)
{
    ...
    mmi_bt_reg_scr_cb(MMI_BT_SCR_CB_CANT_INQUIRY_CFM, (void*)mmi_bt_inq_Cant);
...
#if defined(__MMI_BT_COOLSAND_SUPPORT__) && defined(__MMI_SIMAP_SUPPORT__)
...
void mmi_bt_reg_scr_cb(mmi_bt_scr_cb_type_enum type, void *fp)
{
    ...
    switch (type)
    {
        ...
        case MMI_BT_SCR_CB_INQUIRY_CFM:
            ...
            break;
	case MMI_BT_SCR_CB_CANT_INQUIRY_CFM:
		bt_core_cntx_p->scr_cb_tbl.mmi_bt_scr_cb_Cant_inquiry_cfm = (void (*)(void))fp;
		break;
...
extern void rda_bt_hfg_send_ring(void);
MMI_BOOL mmi_bt_start_hf_ring_req(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    U8 state=0;  //zhu jianguo added it. 2009.6.27
    
    state = mdi_audio_bt_get_stream_status(BT_HFP);
    kal_prompt_trace(0, "ProcessIncomingCallEvent state=%d", state);
    if((state == MDI_AUDIO_BT_STATE_OPEN) || (state == MDI_AUDIO_BT_STATE_IDLE))
    {
        rda_bt_hfg_send_ring();
    }
...
MMI_BOOL mmi_bt_is_to_display_bt_menu(void)
{
    ...
    if(mmi_bootup_get_active_flight_mode() != FLIGHTMODE_OFF) /*in flight mode*/
    {
 ...
 *  
 * RETURNS
 *  MMI_BOOL        TRUE : forward to bluetooth
 *                  FALSE : leave in phone
 *****************************************************************************/
MMI_BOOL mmi_bt_is_aud_path_2_hf(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /*----------------------------------------------------------------*/
    result = (*g_mmi_bt_scr_cntx.stack_cb_tbl.mmi_bt_is_aud_pth_set_to_hf) ();
   // MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_G7_BT_IS_AUD_PATH_2_HF, result);

	mmi_trace(g_sw_BT,"mmi_bt_is_aud_path_2_hf  result =%d, ",result);
    return result;
}
...
MMI_BOOL mmi_bt_is_incall_aud_swap2bt(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	mmi_trace(g_sw_BT,"entry mmi_bt_is_incall_aud_swap2bt");
    ...
    {
    	mmi_trace(g_sw_BT,"entry mmi_bt_is_incall_aud_swap2bt return FALSE");
        return MMI_FALSE;
    }
}
...
void mmi_bt_power_off(void *msg)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if ((*g_mmi_bt_scr_cntx.stack_cb_tbl.mmi_bt_is_pwron) () == MMI_TRUE)
    {
      /******************************/
        /* Switch power off */
      /******************************/
        mmi_trace(g_sw_BT, "mmi_bt_entry_power_switch:: power off");
        #if defined(__MMI_BT_COOLSAND_SUPPORT__) && defined(__MMI_A2DP_SUPPORT__)
        ...
        mmi_bt_show_progress_scr(SCR_BT_POWER_PRO, STR_BT_POWEROFF, STR_BT_SWITCHINGOFF, NULL, MMI_FALSE, NULL);
...
mmi_trace(g_sw_BT, "mmi_bt_reset_hw_on:: g_mmi_bt_scr_cntx.state=5d, g_mmi_bt_scr_cntx.flag=%d, g_mmi_bt_scr_cntx.blinking_flag=%d",
    ...
    if ((*g_mmi_bt_scr_cntx.stack_cb_tbl.mmi_bt_is_pwron) () == MMI_TRUE)
    {
        ...
        /* turn on BT power */
	  mmi_trace(g_sw_BT, "run mmi_bt_antenna_on_req");	
...
void mmi_bt_power_hint_handler(U16 index)
{
    ...
    UCS2Strcpy((PS8) hintData[index], hintStr);
      ...
      /******************************/
        ...
        if ((*g_mmi_bt_scr_cntx.stack_cb_tbl.mmi_bt_is_any_connection_active) ())
        {
            DisplayConfirm(
                ...
                STR_GLOBAL_NO,
                IMG_GLOBAL_NO,
                (UI_string_type) GetString(STR_BT_DISCONN_ALL_CONN_CNF),
                IMG_GLOBAL_QUESTION,
                WARNING_TONE);

            g_mmi_bt_scr_cntx.flag |= MMI_BT_CONFIRM_SWITCH_POWER_OFF;
            SetLeftSoftkeyFunction(mmi_bt_entry_release_all_connections, KEY_EVENT_UP);
            SetRightSoftkeyFunction(mmi_bt_power_switch_confirm_rsk_hdler, KEY_EVENT_UP);
            return;
        }
...
void mmi_bt_entry_release_device_connections(void)
{
    ...
    if ((*g_mmi_bt_scr_cntx.stack_cb_tbl.mmi_bt_release_dev_connection_req) (&(g_mmi_bt_scr_cntx.conn_dev_add)) ==
        MMI_FALSE)
    {
       mmi_trace(g_sw_BT, "mmi_bt_entry_release_device_connections::here display error");
        DisplayPopup(
            (U8*) GetString(STR_GLOBAL_ERROR),
            IMG_GLOBAL_ERROR,
            MMI_FALSE,
...
void mmi_bt_power_switch_on_complete(MMI_BOOL result)   /* power switch rsp */
{
    ...
    /*----------------------------------------------------------------*/
    U16 active_scr_id;
    U16 id_test = SCR_BT_MAIN;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
mmi_trace(g_sw_BT, "mmi_bt_power_switch_on_complete::g_mmi_bt_scr_cntx.flag =%d", g_mmi_bt_scr_cntx.flag);
    ...
    mmi_trace(g_sw_BT, "mmi_bt_power_switch_on_complete::active_scr_id =%d,%d,%d,%d,%d", active_scr_id,IsScreenPresent(SCR_BT_POWER_PRO),IsScreenPresent(SCR_BT_POWER_ON_CNF),IsScreenPresent(SCR_BT_MAIN),id_test);
...
void mmi_bt_inquiry(U8 max_response, U16 timeout, U32 cod, MMI_BOOL name_discovery, U16 title_str_id, U8 inquiry_type)
{
 ...
 mmi_trace(g_sw_BT, "mmi_bt_inquiry::g_mmi_bt_scr_cntx.state= %d, g_mmi_bt_scr_cntx.flag=%d, g_mmi_bt_scr_cntx.blinking_flag=%d", g_mmi_bt_scr_cntx.state, g_mmi_bt_scr_cntx.flag, g_mmi_bt_scr_cntx.blinking_flag);
    ...
    {
    mmi_trace(g_sw_BT,"@@@@@@@@@@@@@@@@@@go STR_BT_DEV_BUSY?");
...
void mmi_bt_inq_Cant(void)   /* discov cfm or discov-cancel cfm */
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    act_scr_id = GetActiveScreenId();
     ...
     if( SCR_BT_SCH_PRO == act_scr_id)
     {
	    ...
	    wgui_stop_title_animation();
			...
			mmi_trace(g_sw_BT, "mmi_bt_inq_complete::act_scr_id == SCR_BT_SCH_PRO");
	            /* GoBackHistory(); */
	            /* it is to reset MMI_BT_RAMMASK_IS_AUTO_PWR_SWTICH */
	            /* inqiry OPP dev will call mmi_bt_open() */
	            mmi_bt_close();
	            /* List Empty */
	            DisplayPopup(
	                (U8*) GetString(STR_BT_INQUIRY_CANT),
	                IMG_GLOBAL_WARNING,
	                MMI_FALSE,
	                UI_POPUP_NOTIFYDURATION_TIME,
	                (U8) EMPTY_LIST_TONE);
...
//               g_mmi_bt_scr_cntx.state, g_mmi_bt_scr_cntx.flag, g_mmi_bt_scr_cntx.blinking_flag);
    ...
    mmi_trace(g_sw_BT, "mmi_bt_entry_bonding::state =%d\n",state);
    switch (state)
    {
...
void mmi_bt_entry_passkey_input()
{
    ...
    if (inputBuffer == NULL)
    {
        ...
        UCS2Strcpy((S8*) hintData[1], (const S8*)GetString(STR_BT_ENTER_PASSCODE));
        
        /* STR_BT_ENTER_PASSCODE string */
        UCS2Strcpy((S8*) g_mmi_bt_scr_cntx.ui_buf, (const S8*)hintData[1]);
        /* "(" string */
        if (hintData[0][0] != 0)
        {
            memset(ascii_buf, 0, 5);
            memset(usc2_buf, 0, 10);
            kal_sprintf((S8*) ascii_buf,"%s", "(");
            AnsiiToUnicodeString((S8*) usc2_buf, (S8*) ascii_buf);
            UCS2Strcat((S8*) g_mmi_bt_scr_cntx.ui_buf, (const S8*)usc2_buf);
        }
        /* Device name */
        if(UCS2Strlen((const S8*)hintData) > 10)
        {
            UCS2Strncat((S8*) g_mmi_bt_scr_cntx.ui_buf, (const S8*)hintData[0],10);
            memset(ascii_buf, 0, 5);
            memset(usc2_buf, 0, 10);
            kal_sprintf((S8*) ascii_buf, "%s", "..");
            AnsiiToUnicodeString((S8*) usc2_buf, (S8*) ascii_buf);
        ...
        {
            UCS2Strcat((S8*) g_mmi_bt_scr_cntx.ui_buf, (const S8*)hintData[0]);
        }
        /* ")" string */
        if (hintData[0][0] != 0)
        {
    ...
    {
        UCS2Strcpy((S8*) g_mmi_bt_scr_cntx.pin_scr_buf, (S8*) inputBuffer);
    ...
    if (UCS2Strlen((S8*) (g_mmi_bt_scr_cntx.pin_scr_buf)) < 1)
    {
...
void mmi_bt_exit_passkey_input(void)
{
    ...
    UCS2Strcpy((S8*) Scr.inputBuffer, (S8*) g_mmi_bt_scr_cntx.pin_scr_buf);
...
void mmi_bt_send_passkey(void)
{
    ...
    UnicodeToAnsii((S8*) asc_pin_buf, (S8*) g_mmi_bt_scr_cntx.pin_scr_buf);
        ...
        if (IsBackHistory == MMI_FALSE)
        {
            if (IsScreenPresent(SCR_BT_BOND))
            {
                ...
                else if(g_mmi_bt_scr_cntx.flag_extended & MMI_BT_EXT_INQ_TYPE_PRINTER_DEV)
				{
		            ...
		            memcpy(&(g_mmi_bt_scr_cntx.selected_spec_dev), dev_p, sizeof(mmi_bt_dev_struct));
		            g_mmi_bt_scr_cntx.flag_extended &= ~MMI_BT_EXT_INQ_TYPE_PRINTER_DEV;
					#if defined(__MMI_BT_COOLSAND_SUPPORT__) && defined(__MMI_BPP_SUPPORT__)
...
/* under construction !*/
	#endif
        	mmi_trace(g_sw_BT, "entry g_mmi_bt_scr_cntx.conn_profile_id=%d", g_mmi_bt_scr_cntx.conn_profile_id);
    ...
    for (service_list_index = 0; service_list_index < dev_p->service_list_num; service_list_index++)
    {
        ...
        {
        	mmi_trace(g_sw_BT, "MMI_BT_HF_PROFILE_UUID In service_list");
    ...
    {
        	mmi_trace(g_sw_BT, "g_mmi_bt_scr_cntx.conn_profile_id = MMI_BT_HS_PROFILE_UUID");
    ...
    else
    {
        conn_profile_bit_mask &= ~MMI_BT_CONNECTING_HS_PROFILE;
        	...
        	mmi_trace(g_sw_BT, "g_mmi_bt_scr_cntx.conn_profile_id=%d", g_mmi_bt_scr_cntx.conn_profile_id);
...
void mmi_bt_connecting(mmi_bt_dev_addr *dev_addr, U32 pro_uuid)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_trace(g_sw_BT,"mmi_bt_connecting:: g_mmi_bt_scr_cntx.state=%d, g_mmi_bt_scr_cntx.flag=%d, g_mmi_bt_scr_cntx.blinking_flag=%d",
                                     g_mmi_bt_scr_cntx.state, g_mmi_bt_scr_cntx.flag, g_mmi_bt_scr_cntx.blinking_flag);
   mmi_trace(g_sw_BT, "pro_uuid=%d", pro_uuid);
    mmi_bt_clear_endkey_hdlr();
...
void mmi_bt_connect_complete(U32 connection_type, MMI_BOOL result)
{
    ...
    U8 str_output[180], str_ucs2_space[4], str_asc_space[2];
    U16 act_scr = GetActiveScreenId();
   ...
   mmi_trace(g_sw_BT, "mmi_bt_connect_complete::connection_type=%d, result=%d, connecting_profile=%d, act_scr=%d"
                                      , connection_type, result, connecting_profile, act_scr);
    ...
    memset(str_output, 0, sizeof(U8) * 180);
    memset(str_asc_space, 0, sizeof(U8) * 2);
    memset(str_ucs2_space, 0, sizeof(U8) * 4);
    kal_sprintf((S8*) str_asc_space, "%s", " ");
    AnsiiToUnicodeString((S8*) str_ucs2_space, (S8*) str_asc_space);

    UCS2Strcpy((S8*) str_output, (const S8*)str_prof);
    /* space is reserved in ref_list.txt for alphabetical languages */ 
    /* for Chinese language, space is not required */
    //UCS2Strcat((S8*) str_output, (const S8*)str_ucs2_space);
    /* check if str_prof string length + 2 + str_conn_res string length >= str_output */
    MMI_BT_ASSERT(((UCS2Strlen((const S8*)str_prof) * ENCODING_LENGTH) + 2 +
                   (UCS2Strlen((const S8*)str_conn_res) * ENCODING_LENGTH)) < (sizeof(U8) * 180));
    UCS2Strcat((S8*) str_output, (const S8*)str_conn_res);
...
void mmi_bt_entry_connect_accept_cnf(void)
{
    ...
    UCS2Strcpy((S8*) hintData[1], (const S8*)GetString(STR_BT_ACCEPT_CONNECT_FROM));
...
void mmi_bt_entry_connect_authorise(void)
{
    ...
    UCS2Strcpy((S8*) hintData[1], (const S8*)GetString(STR_BT_SWITCH_SPEECH_PATH));
...
#else
            nItems = (U16) MMI_BT_MYDEV_OPT_LAST - 1-1;
//	     ItemList[MMI_BT_MYDEV_OPT_CONN] = (U8*) GetString((U16) STR_BT_MYDEV_OPT_CONN);		
            ItemList[MMI_BT_MYDEV_OPT_DISCONN-1] = (U8*) GetString((U16) STR_BT_MYDEV_OPT_DISCONN);
            ItemList[MMI_BT_MYDEV_OPT_REN - 1-1] = (U8*) GetString((U16) STR_BT_MYDEV_OPT_REN);
            ItemList[MMI_BT_MYDEV_OPT_DEL - 1-1] = (U8*) GetString((U16) STR_BT_MYDEV_OPT_DEL);
            ItemList[MMI_BT_MYDEV_OPT_DEL_ALL - 1-1] = (U8*) GetString((U16) STR_BT_MYDEV_OPT_DEL_ALL);
            ItemList[MMI_BT_MYDEV_OPT_SUP_SERV - 1-1] = (U8*) GetString((U16) STR_BT_MYDEV_OPT_SUP_SER);
#endif
...
#else
            nItems = (U16) MMI_BT_MYDEV_OPT_LAST - 2-1;
		
            ItemList[MMI_BT_MYDEV_OPT_REN - 2-1] = (U8*) GetString((U16) STR_BT_MYDEV_OPT_REN);
            ItemList[MMI_BT_MYDEV_OPT_DEL - 2-1] = (U8*) GetString((U16) STR_BT_MYDEV_OPT_DEL);
            ItemList[MMI_BT_MYDEV_OPT_DEL_ALL - 2-1] = (U8*) GetString((U16) STR_BT_MYDEV_OPT_DEL_ALL);
            ItemList[MMI_BT_MYDEV_OPT_SUP_SERV - 2-1] = (U8*) GetString((U16) STR_BT_MYDEV_OPT_SUP_SER);
#endif
...
#else
            if (idx == MMI_BT_MYDEV_OPT_CONN )//(idx != MMI_BT_MYDEV_OPT_CONN && idx != MMI_BT_MYDEV_OPT_DISCONN)
            {
                idx++;
            }
	     else
...
void mmi_bt_entry_mydev_prof_choose(void)
{
  ...
  //  memset(&nItems, 0, MAX_SUB_MENUS);
    ...
    nItems = 0;
   mmi_trace(g_sw_BT, "mmi_bt_entry_mydev_prof_choose::dev_p= %d", dev_p);

    for (i = 0; i < dev_p->service_list_num; i++)
    {
    	mmi_trace(g_sw_BT, "dev_p->service_list[i=%d]= %X", i, dev_p->service_list[i]);
...
void mmi_bt_mydev_prof_choose_hilite_hdlr(S32 idx)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
  mmi_trace(g_sw_BT, "entry  mmi_bt_mydev_prof_choose_hilite_hdlr idx =%d", idx);
...
void mmi_bt_prof_choose_refresh_prof_list(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_trace(g_sw_BT, "mmi_bt_prof_choose_refresh_prof_list::g_mmi_bt_scr_cntx.state=%d, g_mmi_bt_scr_cntx.flag=%d, g_mmi_bt_scr_cntx.blinking_flag=%d",
                                        g_mmi_bt_scr_cntx.state, g_mmi_bt_scr_cntx.flag, g_mmi_bt_scr_cntx.blinking_flag);
    /* to avoid the error operation while bt module is busy */
    if ((mmi_bt_is_permission) (MMI_TRUE) == MMI_FALSE)
    {
...
//    MMI_TRACE(MMI_TRACE_G7_MISC, MMI_BT_G7_SCR_REN);
	...
	#ifdef __MMI_GB__
	GBSetInputboxLSKFunction(mmi_bt_entry_mydev_ren_edopt);
	#endif
...
void mmi_bt_mydev_ren_proc(void)
{
    ...
    ucs2_byte_count = (UCS2Strlen((const S8*)g_mmi_bt_scr_cntx.scr_buf) * ENCODING_LENGTH) + ENCODING_LENGTH;
...
void mmi_bt_entry_mydev_del_all(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	mmi_trace(g_sw_BT, "mmi_bt_entry_mydev_del_all");
    if (g_mmi_bt_scr_cntx.flag & MMI_BT_CONFIRM_PAIRED_DEV_DEL)
    {
        return;
    }
...
/* under construction !*/
    ...
    if (nItems == 0)
    {
        UCS2Strcpy((S8*) subMenuData[0], (S8*) GetString(STR_GLOBAL_EMPTY_LIST));
...
mmi_trace(g_sw_BT, "mmi_bt_entry_disconnect_complete::connection_type=%d, result=%d, disconnecting_conn_type=%d, act_scr=%d", \
connection_type, result, disconnecting_conn_type, act_scr);
    MMI_BT_ASSERT((connection_type < MMI_BT_LAST_CONNECTOIN_TYPE));
...
void mmi_bt_entry_set_name(void)
{
	...
	#ifdef __MMI_GB__
	GBSetInputboxLSKFunction(mmi_bt_set_name_opt);
	#endif
    SetCategory5RightSoftkeyFunction(GoBackHistory, KEY_EVENT_UP);
    RegisterInputMethodScreenCloseFunction(mmi_bt_goback_2_history);
    return;
...
void mmi_bt_entry_set_audio_pth_menu(void)
{
    ...
    ShowCategory11Screen(
        STR_BT_DEV_SET_AUDIO_PATH,GetRootTitleIcon(MENU_CONN_BT_SET_AUDIO_PATH),
        STR_GLOBAL_OK,
        IMG_GLOBAL_OK,
        STR_GLOBAL_BACK,
        IMG_GLOBAL_BACK,
...
void mmi_bt_entry_about(void)
{
    ...
    MMI_BT_ASSERT((UCS2Strlen((const S8*)GetString(STR_BT_BD_NAME)) * ENCODING_LENGTH) <=
                  (MMI_BT_SCR_BUF_SIZE - ENCODING_LENGTH));

    memset(g_mmi_bt_scr_cntx.scr_buf, 0, MMI_BT_SCR_BUF_SIZE);
    UCS2Strcpy((S8*) g_mmi_bt_scr_cntx.scr_buf, (const S8*)GetString(STR_BT_BD_NAME));

    UCS2Strcpy((S8*) g_mmi_bt_scr_cntx.ui_buf, (const S8*)g_mmi_bt_scr_cntx.scr_buf);
    UCS2Strcat((S8*) g_mmi_bt_scr_cntx.ui_buf, (const S8*)usc2_buf);
    ...
    UCS2Strcat((S8*) g_mmi_bt_scr_cntx.ui_buf, (const S8*)g_mmi_bt_scr_cntx.scr_buf);
    UCS2Strcat((S8*) g_mmi_bt_scr_cntx.ui_buf, (const S8*)usc2_buf);

    /* BD Address */
    MMI_BT_ASSERT((UCS2Strlen((const S8*)GetString(STR_BT_BD_ADDR)) * ENCODING_LENGTH) <=
                  (MMI_BT_SCR_BUF_SIZE - ENCODING_LENGTH));

    memset(g_mmi_bt_scr_cntx.scr_buf, 0, MMI_BT_SCR_BUF_SIZE);
    UCS2Strcpy((S8*) g_mmi_bt_scr_cntx.scr_buf, (const S8*)GetString(STR_BT_BD_ADDR));
    ...
    /* Profiles */
    MMI_BT_ASSERT((UCS2Strlen((const S8*)GetString(STR_BT_SERVICE)) * ENCODING_LENGTH) <=
                  (MMI_BT_SCR_BUF_SIZE - ENCODING_LENGTH));
    /* memcpy(g_mmi_bt_scr_cntx.scr_buf, GetString(STR_BT_SERVICE), (UCS2Strlen((const S8 *)GetString(STR_BT_SERVICE))*ENCODING_LENGTH)); */
    UCS2Strcpy((S8*) g_mmi_bt_scr_cntx.scr_buf, (const S8*)GetString(STR_BT_SERVICE));
    ...
    for (i = 0; i < dev_p->service_list_num; i++)
    {
        if (mmi_bt_get_profile_str_id((U16) dev_p->service_list[i], &str_p) == MMI_TRUE)
        {
            ...
            {
                UCS2Strcat((S8*) g_mmi_bt_scr_cntx.ui_buf, (const S8*)str_p);
                UCS2Strcat((S8*) g_mmi_bt_scr_cntx.ui_buf, (const S8*)usc2_buf);
            }
        }
    ...
    ui_buffer_size = UCS2Strlen((const S8*)g_mmi_bt_scr_cntx.ui_buf);
...
void mmi_bt_entry_specific_cod_device(U32 cod_mask)
{
	...
	if (IsScreenPresent(SCR_BT_MY_SPECIFIC_DEV_DUMMY) || IsScreenPresent(SCR_FTP_FILE_BROWSER))
#else
...
U8 mmi_bt_entry_release_dev_connection_confirm_del_cb(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_trace(g_sw_BT, "mmi_bt_entry_release_dev_connection_confirm_del_cb");
    /* reset confirm flag */
    /* only one confirm flag is set at one time */
    g_mmi_bt_scr_cntx.flag &= ~MMI_BT_CONFIRM_PAIRED_DEV_DEL;
...
/*****************************************************************************
 * FUNCTION
 *  mmi_bt_is_service_support_with_dev_addr
 * DESCRIPTION
 ...
 *****************************************************************************/
MMI_BOOL mmi_bt_is_service_support_with_dev_addr(mmi_bt_dev_addr *addr, U32 service_id)
{
	/*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	...
	MMI_BOOL result = MMI_FALSE;
    mmi_bt_dev_struct *dev_p;
    U32 paired_dev_num = (*g_mmi_bt_scr_cntx.stack_cb_tbl.mmi_bt_get_paired_dev_num) ();
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    ...
    {
        dev_p = (*g_mmi_bt_scr_cntx.stack_cb_tbl.mmi_bt_get_paired_dev_info) (i);
        if(dev_p->bd_addr.lap == addr->lap &&
           dev_p->bd_addr.uap == addr->uap &&
           dev_p->bd_addr.nap == addr->nap)
        {
            result = mmi_bt_is_service_support(dev_p, service_id);
            break;
        }
    }
    
   // MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_G7_BT_IS_SERVICE_SUPPORT_WITH_DEV_ADDR, addr->lap, addr->uap, addr->nap, service_id, result);
  mmi_trace(1, "mmi_bt_is_service_support_with_dev_addr, addr->lap= %d, addr->uap=%d, addr->nap=%d, service_id=%d, result=%d  ",addr->lap, addr->uap, addr->nap, service_id, result);
...
//               (g_mmi_bt_scr_cntx.prog_scr.title_str_id), (g_mmi_bt_scr_cntx.prog_scr.context_id));
    ...
    if (((g_mmi_bt_scr_cntx.prog_scr.scr_id) != 0)
        ...
        || (UCS2Strlen((const S8*)(g_mmi_bt_scr_cntx.prog_scr.context_str)) != 0)
    ...
    if (context_id)
    {
        UCS2Strcpy((S8*) (g_mmi_bt_scr_cntx.prog_scr.context_str), (S8*) GetString(context_id));
    }
    else if (context_str)
    {
        UCS2Strcpy((S8*) (g_mmi_bt_scr_cntx.prog_scr.context_str), (S8*) context_str);
    }
    else
    {
        /* error usage! */
...
//               ((g_mmi_bt_scr_cntx.prog_scr.title_str_id)), ((g_mmi_bt_scr_cntx.prog_scr.context_id)));
    ...
    MMI_BT_ASSERT(((g_mmi_bt_scr_cntx.prog_scr.scr_id) != 0)
                  && ((g_mmi_bt_scr_cntx.prog_scr.title_str_id) != 0)
                  && (UCS2Strlen((S8*) (g_mmi_bt_scr_cntx.prog_scr.context_str)) != 0));
...
//    MMI_TRACE(MMI_TRACE_G7_MISC, MMI_BT_G7_SCR_GTIMER_TO);
    ...
    state = (*g_mmi_bt_scr_cntx.stack_cb_tbl.mmi_bt_get_state) ();
	mmi_trace(g_sw_BT,"mmi_bt_scr_proc_guard_tout_hdlr::state = %d", state);
    switch (state)
    {
        case MMI_BT_STAT_INIT_ON:
 ...
 *  void
 *****************************************************************************/
void mmi_bt_show_confirm_scr(U16 scr_id, U32 msg_id, FuncPtr exit_hdlr, FuncPtr entry_hdlr)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
void mmi_bt_suppress_profile_popup(U32 profile_id, MMI_BOOL suppress_switch)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
  //  MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_G7_BT_SUPPRESS_PROFILE_POPUP, 
   // 			profile_id,	suppress_switch);


  mmi_trace(1,"mmi_bt_suppress_profile_popup ,  profile_id =%d, suppress_switch = %d",profile_id,suppress_switch);


    if(suppress_switch == MMI_TRUE)
    {
        for(i = 0; i < MMI_BT_MAX_HOST_SUPPORTED_PROFILE; i++)
        {
            if(g_mmi_bt_scr_cntx.suppress_profile[i] == 0)
            {
                g_mmi_bt_scr_cntx.suppress_profile[i] = profile_id;
                break;
            }
        }
    }
    else
    {
        for(i = 0; i < MMI_BT_MAX_HOST_SUPPORTED_PROFILE; i++)
        {
            if(g_mmi_bt_scr_cntx.suppress_profile[i] == profile_id)
            {
                g_mmi_bt_scr_cntx.suppress_profile[i] = 0;
                break;
            }
        }
    }
...
void mmi_bt_panic_ind_post_process()
{
    ...
    /*----------------------------------------------------------------*/
     ...
     mmi_trace(g_sw_BT,"g_mmi_bt_scr_cntx.flag = %d",g_mmi_bt_scr_cntx.flag);
...
MMI_BOOL mmi_bt_is_permission(MMI_BOOL is_popup)
{
	...
	mmi_trace(g_sw_BT, "mmi_bt_is_permission STATUS:: btcore_state= %d", btcore_state);
...
MMI_BOOL mmi_bt_is_hw_ready(MMI_BOOL is_popup)
{
    ...
    stack_ready = (*g_mmi_bt_scr_cntx.stack_cb_tbl.mmi_bt_get_ram_flag) (MMI_BT_RAMMASK_IS_READY);
     ...
     mmi_trace(g_sw_BT, "mmi_bt_is_hw_ready, power_status =%d  stack_ready = %d",
              power_status,            stack_ready);
 ...
 *  void
 *****************************************************************************/
void mmi_bt_set_headset_speaker_volume(S8 ui_volume_level)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
void mmi_bt_hfp_set_speaker_vol(U32 volume_gain)
{
  ...
  mmi_trace(g_sw_BT,"mmi_bt_hfp_set_speaker_vol, volume_gain=%d ",volume_gain);
...
MMI_BOOL mmi_bt_ps_stack_and_hw_init(MMI_BOOL flight_mode, MMI_BOOL sim_inserted)
{
    ...
    mmi_trace(g_sw_BT,"mmi_bt_ps_stack_and_hw_init::flight_mode=%d, sim_inserted=%d ",flight_mode, sim_inserted);
    ...
    if (!flight_mode)
    {
        ...
        /* reinit BT */
        /* mmi_bt_init(); */
	mmi_trace(g_sw_BT,"(*g_mmi_bt_scr_cntx.stack_cb_tbl.mmi_bt_is_pwron) ()= %d", (*g_mmi_bt_scr_cntx.stack_cb_tbl.mmi_bt_is_pwron) () );
        if ((*g_mmi_bt_scr_cntx.stack_cb_tbl.mmi_bt_is_pwron) () == MMI_TRUE)
        {
 ...
 *  void
 *****************************************************************************/
void mmi_bt_send_l4_msg(U32 msg_id, void *p_local_para, void *p_peer_buff)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
void mmi_bt_popup_operation_not_allowed(void)
{
    ...
    ui_buffer_size = UCS2Strlen((const S8*)GetString(STR_BT_OPERATION_NOT_ALLOWED_DIS_HFP_FIRST));
...
void mmi_bt_popup_prohibited_by_simap(void)
{
    ...
    ui_buffer_size = UCS2Strlen((const S8*)GetString(STR_BT_PROHIBIT_BY_SIMAP));
...
#elif !defined(CSD_TARGET) && !defined(__MMI_BT_SUPPORT__)
