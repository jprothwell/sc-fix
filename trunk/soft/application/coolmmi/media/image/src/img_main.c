...
void img_main(ilm_struct *ilm_ptr)
{
	...
	mmi_trace(g_sw_MED_Img, "img_main()->ilm_ptr->msg_id = %d",ilm_ptr->msg_id);
	mmi_trace(g_sw_MED_Img, "img_main()::img_context_p->state is %d", img_context_p->state);
...
{
	mmi_trace(TRUE, "Excute func : img_init()");
   img_context_p->state = IMG_IDLE;
	mmi_trace(TRUE, " img_init():img_context_p->state = %d", img_context_p->state);
   img_context_p->file_buffer_address=(kal_uint32)NULL;
   img_context_p->intmem_start_address=(kal_uint32)NULL;
   img_context_p->extmem_start_address=(kal_uint32)NULL;
   img_context_p->processing=KAL_FALSE;
   img_context_p->img_event = kal_create_event_group("img_events");
#if (defined(JPG_DECODE) || defined(JPG_ENCODE)) && defined(__CSD_TARGET__)
 //  jpeg_codec_init();
#endif
   return KAL_TRUE;
}
