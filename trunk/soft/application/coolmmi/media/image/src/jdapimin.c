...
jpeg_read_header (j_decompress_ptr cinfo, boolean require_image)
{
  ...
  //mmi_trace(1,"jpeg_consume_input finish ,retcode=%d",retcode);
  ...
  case JPEG_REACHED_EOI:
    if (require_image)		/* Complain if application wanted an image */
    	{
    	   //mmi_trace(1,"require_image error");
...
jpeg_finish_decompress (j_decompress_ptr cinfo)
{
  if ((cinfo->global_state == DSTATE_SCANNING ||
    ...
    if (cinfo->smp_cut_row_end-cinfo->smp_cut_row_start+1 ==cinfo->dest_height && //sheen
		cinfo->output_scanline < cinfo->output_height)
	{
      //ERREXIT(cinfo, JERR_TOO_LITTLE_DATA);
		mmi_trace(1,"Jpeg NOTE scanline=%d output_height=%d", cinfo->output_scanline, cinfo->output_height);
	}
