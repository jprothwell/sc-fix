...
#define     EBR_DISPLAY_INFO_SIZE               200 + MAX_EBOOK_NAME_LENGTH
...
#define     EBR_MAX_SETTINGS_STORAGE        3
#else
#define     EBR_MAX_SETTINGS_STORAGE        2
#endif
/*- zhouqin modify for dual t-flash card 20110503*/
#define     EBR_TOTAL_NUMBER_OF_ITEM_BKSHLF     18
#define     EBR_TOTAL_NUMBER_OF_ITEM_BOOK    16
#define     EBR_MAX_DRIVE_NAME_LEN  16
typedef struct
{
    ...
    S32 current_setting_highlight_item;
    S32 current_selected_bookmark;
    U8 buffer_data[EBR_RECORD_SIZE_MAX + GUI_SHARED_BYTES];
    S8 ebook_settings_memory_type[(EBR_MAX_DRIVE_NAME_LEN + 1) * ENCODING_LENGTH];
    U8 *info_buffer;
    U16 icon_id;
    BOOL entry_from_my_favorite;
} g_ebook_app_cntx;

typedef struct
{
    const S8 *ebook_jump_text_end_beg[EBR_MAX_SETTINGS_VAL_FONT];
    const S8 *ebook_settings_font_style[EBR_MAX_SETTINGS_VAL_FONT];
    const S8 *ebook_settings_font_size[EBR_MAX_SETTINGS_VAL_FONT];
    const S8 *ebook_settings_scroll_by[EBR_MAX_SETTINGS_VAL_SCROLL];
    const S8 *ebook_settings_auto_scroll[EBR_MAX_SETTINGS_VAL_SCROLL];
    const S8 *ebook_settinsg_auto_scroll_speed[EBR_MAX_SETTINGS_VAL_SCROLL_SPEED];
    const S8 *ebook_settings_full_screen[EBR_MAX_SETTINGS_VAL_SCROLL];
    const S8 *ebook_settings_encoding_format[EBR_MAX_SETTINGS_VAL_ENCD_TYPE];
    const S8 *ebook_settings_alignment_style[EBR_MAX_SETTINGS_VAL_SCROLL];
    const S8 *ebook_settings_storage[EBR_MAX_SETTINGS_STORAGE];    
}g_ebook_inline_const_string_cntx;

extern void ebr_show_book_shelf_option(void);
extern void go_back_to_bookshelf_screen(void);
extern void go_back_to_bookshelf_screen_entry(void);
...
extern void ebr_delete_global_setting_screen(void);
...
extern void ebr_app_display_pop_up_book_mark_not_sucessfully_added(void);
...
extern void ebr_duplicate_book_mark_back(void);
extern MMI_BOOL ebr_frm_is_drive_ready(S8 drive_letter);
extern void ebr_delete_no_ebook(void);
...
extern void mmi_ebr_save_drive_selection(S16 drive_letter);

extern void ebr_set_inlineHighligtItemSelectMemory(S8 drive);
extern S8 ebr_get_drive_by_inlineHighligtItemSelectMemory(void);
