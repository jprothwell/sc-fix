...
static void draw_font(  javacall_pixel*             destbuf, 
                        int                         w, 
                        int                         h,
                        int                         x, 
                        int                         y,
                        javacall_pixel              color_t,
                        int                         clipX1, 
                        int                         clipY1, 
                        int                         clipX2, 
                        int                         clipY2,
...

{
    ...
    //check whether in the clip
    if(X1 < clipX1 || X1+char_width > clipX2 || Y1 < clipY1 || Y1+char_height > clipY2)
    {
        return;
    }
...
javacall_result javacall_font_draw(javacall_pixel   color, 
                        ...
                        int                         textLen){
    //mmi_trace(1,"Font:color=%d clipX1=%d clipY1=%d clipX2=%d clipY2=%d destBufferHoriz=%d destBufferVert=%d x=%d y=%d textLen=%d",color,clipX1,clipY1,clipX2,clipY2,destBufferHoriz,destBufferVert,x,y,textLen);
    int i;
    uint32 CharSize;
    ...
    for(i = 0; i < textLen; i++)
    {
        CharSize = GetFontdata(text[i], Font, &CharData, &nWidth, &nHgt);
        draw_font(destBuffer,destBufferHoriz,destBufferVert,x,y,color,clipX1,clipY1,clipX2,clipY2,CharData,CharSize,nWidth,nHgt);
        x += nWidth;
    }
...
int javacall_font_get_width(javacall_font_face     face, 
    ...
    {
        GetFontdata(charArray[i], Font, &CharData, &nWidth, &nHgt);
        width += nWidth;
    }
    //mmi_trace(1,"Font:javacall_font_get_width=%d",width);
    return width;
